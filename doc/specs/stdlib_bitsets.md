...
`bitset_t`, `bitset_64`, and `bitset_large`. `bitset_t` is an abstract
type that serves as the ancestor of `bitset_64` and
`bitset_large`. `bitset_t` defines one method, `bits`, all of its
...
position, that, in turn, is indexed from 0 to `bits-1`. `bitset_t` is
...
assignments, "unary" methods of class `bitset_t`, "binary"
...
### Table of the `bitset_t` methods

The `bitset_t` class has a number of methods. All except one, `bits`,
are deferred. The methods consist of all procedures with one argument
of class `bitset_t`. The procedures with two arguments of type
...
`self`: shall be a scalar expression of class `bitset_t`. It is an
`intent(in)` argument.
...
`self`: shall be a scalar variable of class `bitset_t`. It is an
...
`self`: shall be a scalar class `bitset_t` variable It is an
`intent(inout)` argument.
...
### `init` - `bitset_t` initialization routines.
...
`bitset_t` initialization routine.
...
`bitset_t` `output` subroutine by the same processor.
