submodule(stdlib_lapack_eig_svd_lsq) stdlib_lapack_eigv_tridiag2
  ...
  contains
     ...
     !! single set which is sorted in ascending order.
        ...
        ! =====================================================================
           ! Local Scalars 
           integer(ilp64) :: i, ind1, ind2, n1sv, n2sv
           ! Executable Statements 
           n1sv = n1
           n2sv = n2
           if( strd1>0_ilp64 ) then
              ind1 = 1_ilp64
           else
              ind1 = n1
           end if
           if( strd2>0_ilp64 ) then
              ind2 = 1_ilp64 + n1
           else
              ind2 = n1 + n2
           end if
           ...
           ! while ( (n1sv > 0)
           10 continue
           if( n1sv>0_ilp64 .and. n2sv>0_ilp64 ) then
              if( a( ind1 )<=a( ind2 ) ) then
                 index( i ) = ind1
                 i = i + 1_ilp64
                 ind1 = ind1 + strd1
     ...
     !! single set which is sorted in ascending order.
        ...
        ! =====================================================================
           ...
           n1sv = n1
           n2sv = n2
           if( dtrd1>0_ilp64 ) then
              ind1 = 1_ilp64
           else
              ind1 = n1
           end if
           if( dtrd2>0_ilp64 ) then
              ind2 = 1_ilp64 + n1
           else
              ind2 = n1 + n2
           end if
     ...
     !! problem.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           integer(ilp64) :: bsiz1, bsiz2, curr, i, k, mid, psiz1, psiz2, ptr, zptr1
           ! Intrinsic Functions 
           ! Executable Statements 
           ! test the input parameters.
           info = 0_ilp64
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SLAEDA', -info )
              return
           end if
           ! quick return if possible
           if( n==0 )return
           ! determine location of first number in second half.
           mid = n / 2_ilp64 + 1_ilp64
           ! gather last/first rows of appropriate eigenblocks into center of z
           ptr = 1_ilp64
           ! determine location of lowest level subproblem in the full storage
           ! scheme
           curr = ptr + curpbm*2_ilp64**curlvl + 2_ilp64**( curlvl-1 ) - 1_ilp64
           ! determine size of these matrices.  we add half to the value of
           ! the sqrt in case the machine underestimates one of these square
           ! roots.
           bsiz1 = int( half+sqrt( real( qptr( curr+1 )-qptr( curr ),KIND=sp) ),KIND=ilp64)
           bsiz2 = int( half+sqrt( real( qptr( curr+2 )-qptr( curr+1 ),KIND=sp) ),KIND=ilp64)
                     
           do k = 1, mid - bsiz1 - 1
              z( k ) = zero
           end do
           call stdlib_I64_scopy( bsiz1, q( qptr( curr )+bsiz1-1 ), bsiz1,z( mid-bsiz1 ), 1_ilp64 )
           call stdlib_I64_scopy( bsiz2, q( qptr( curr+1 ) ), bsiz2, z( mid ), 1_ilp64 )
           do k = mid + bsiz2, n
              z( k ) = zero
           end do
           ! loop through remaining levels 1 -> curlvl applying the givens
           ! rotations and permutation and then multiplying the center matrices
           ! against the current z.
           ptr = 2_ilp64**tlvls + 1_ilp64
           loop_70: do k = 1, curlvl - 1
              curr = ptr + curpbm*2_ilp64**( curlvl-k ) + 2_ilp64**( curlvl-k-1 ) - 1_ilp64
              psiz1 = prmptr( curr+1 ) - prmptr( curr )
              psiz2 = prmptr( curr+2 ) - prmptr( curr+1 )
              zptr1 = mid - psiz1
             ! apply givens at curr and curr+1
              do i = givptr( curr ), givptr( curr+1 ) - 1
                 call stdlib_I64_srot( 1_ilp64, z( zptr1+givcol( 1_ilp64, i )-1_ilp64 ), 1_ilp64,z( zptr1+givcol( 2_ilp64, i&
                     & )-1_ilp64 ), &
                           1_ilp64, givnum( 1_ilp64, i ),givnum( 2_ilp64, i ) )
              end do
              do i = givptr( curr+1 ), givptr( curr+2 ) - 1
                 call stdlib_I64_srot( 1_ilp64, z( mid-1+givcol( 1_ilp64, i ) ), 1_ilp64,z( mid-1+givcol( 2_ilp64, i ) ), 1_ilp64, &
                           givnum( 1_ilp64, i ),givnum( 2_ilp64, i ) )
              end do
              psiz1 = prmptr( curr+1 ) - prmptr( curr )
              psiz2 = prmptr( curr+2 ) - prmptr( curr+1 )
              do i = 0, psiz1 - 1
                 ztemp( i+1 ) = z( zptr1+perm( prmptr( curr )+i )-1_ilp64 )
              end do
              do i = 0, psiz2 - 1
                 ztemp( psiz1+i+1 ) = z( mid+perm( prmptr( curr+1 )+i )-1_ilp64 )
              end do
              ! multiply blocks at curr and curr+1
              ! determine size of these matrices.  we add half to the value of
              ! the sqrt in case the machine underestimates one of these
              ...
              if( bsiz1>0_ilp64 ) then
                 call stdlib_I64_sgemv( 'T', bsiz1, bsiz1, one, q( qptr( curr ) ),bsiz1, ztemp( 1_ilp64 ), &
                           1_ilp64, zero, z( zptr1 ), 1_ilp64 )
              end if
              call stdlib_I64_scopy( psiz1-bsiz1, ztemp( bsiz1+1 ), 1_ilp64, z( zptr1+bsiz1 ),1_ilp64 )
              if( bsiz2>0_ilp64 ) then
                 call stdlib_I64_sgemv( 'T', bsiz2, bsiz2, one, q( qptr( curr+1 ) ),bsiz2, ztemp( &
                           psiz1+1 ), 1_ilp64, zero, z( mid ), 1_ilp64 )
              end if
              call stdlib_I64_scopy( psiz2-bsiz2, ztemp( psiz1+bsiz2+1 ), 1_ilp64,z( mid+bsiz2 ), 1_ilp64 )
                        
              ptr = ptr + 2_ilp64**( tlvls-k )
           end do loop_70
           return
     ...
     !! problem.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DLAEDA', -info )
              return
           end if
           ! quick return if possible
           if( n==0 )return
           ...
           ! the sqrt in case the machine underestimates one of these square
           ! roots.
           bsiz1 = int( half+sqrt( real( qptr( curr+1 )-qptr( curr ),KIND=dp) ),KIND=ilp64)
           bsiz2 = int( half+sqrt( real( qptr( curr+2 )-qptr( curr+1 ),KIND=dp) ),KIND=ilp64)
                     
           do k = 1, mid - bsiz1 - 1
              z( k ) = zero
           end do
           call stdlib_I64_dcopy( bsiz1, q( qptr( curr )+bsiz1-1 ), bsiz1,z( mid-bsiz1 ), 1_ilp64 )
           call stdlib_I64_dcopy( bsiz2, q( qptr( curr+1 ) ), bsiz2, z( mid ), 1_ilp64 )
           do k = mid + bsiz2, n
              z( k ) = zero
           end do
           ! loop through remaining levels 1 -> curlvl applying the givens
           ...
           loop_70: do k = 1, curlvl - 1
             ...
             ! apply givens at curr and curr+1
              do i = givptr( curr ), givptr( curr+1 ) - 1
                 call stdlib_I64_drot( 1_ilp64, z( zptr1+givcol( 1_ilp64, i )-1_ilp64 ), 1_ilp64,z( zptr1+givcol( 2_ilp64, i&
                     & )-1_ilp64 ), &
                           1_ilp64, givnum( 1_ilp64, i ),givnum( 2_ilp64, i ) )
              end do
              do i = givptr( curr+1 ), givptr( curr+2 ) - 1
                 call stdlib_I64_drot( 1_ilp64, z( mid-1+givcol( 1_ilp64, i ) ), 1_ilp64,z( mid-1+givcol( 2_ilp64, i ) ), 1_ilp64, &
                           givnum( 1_ilp64, i ),givnum( 2_ilp64, i ) )
              end do
              psiz1 = prmptr( curr+1 ) - prmptr( curr )
              psiz2 = prmptr( curr+2 ) - prmptr( curr+1 )
              ...
              if( bsiz1>0_ilp64 ) then
                 call stdlib_I64_dgemv( 'T', bsiz1, bsiz1, one, q( qptr( curr ) ),bsiz1, ztemp( 1_ilp64 ), &
                           1_ilp64, zero, z( zptr1 ), 1_ilp64 )
              end if
              call stdlib_I64_dcopy( psiz1-bsiz1, ztemp( bsiz1+1 ), 1_ilp64, z( zptr1+bsiz1 ),1_ilp64 )
              if( bsiz2>0_ilp64 ) then
                 call stdlib_I64_dgemv( 'T', bsiz2, bsiz2, one, q( qptr( curr+1 ) ),bsiz2, ztemp( &
                           psiz1+1 ), 1_ilp64, zero, z( mid ), 1_ilp64 )
              end if
              call stdlib_I64_dcopy( psiz2-bsiz2, ztemp( psiz1+bsiz2+1 ), 1_ilp64,z( mid+bsiz2 ), 1_ilp64 )
     ...
     !! semi-widths in the arrays W and WERR respectively.
        ...
        ! =====================================================================
           ...
           integer(ilp64) :: maxitr
           ! Local Scalars 
           integer(ilp64) :: i, i1, ii, ip, iter, k, negcnt, next, nint, olnint, prev, r
           real(sp) :: back, cvrgd, gap, left, lgap, mid, mnwdth, rgap, right, tmp, width
           ! Intrinsic Functions 
           ! Executable Statements 
           info = 0_ilp64
           ...
           maxitr = int( ( log( spdiam+pivmin )-log( pivmin ) ) /log( two ),KIND=ilp64) + 2_ilp64
           mnwdth = two * pivmin
           r = twist
           if((r<1_ilp64).or.(r>n)) r = n
           ! initialize unconverged intervals in [ work(2*i-1), work(2*i) ].
           ! the sturm count, count( work(2*i-1) ) is arranged to be i-1, while
           ! count( work(2*i) ) is stored in iwork( 2*i ). the integer iwork( 2*i-1 )
           ! for an unconverged interval is set to the index of the next unconverged
           ...
           loop_75: do i = i1, ilast
              ...
              back = werr( ii )
              20 continue
              negcnt = stdlib_I64_slaneg( n, d, lld, left, pivmin, r )
              if( negcnt>i-1 ) then
                 left = left - back
                 back = two*back
                 go to 20
              ...
              back = werr( ii )
              50 continue
              negcnt = stdlib_I64_slaneg( n, d, lld, right, pivmin, r )
               if( negcnt<i ) then
                  right = right + back
                  back = two*back
                  go to 50
              ...
              if( width<=cvrgd .or. width<=mnwdth ) then
                 ...
                 iwork( k-1 ) = -1_ilp64
                 ! make sure that i1 always points to the first unconverged interval
                 if((i==i1).and.(i<ilast)) i1 = i + 1_ilp64
                 if((prev>=i1).and.(i<=ilast)) iwork( 2_ilp64*prev-1 ) = i + 1_ilp64
              else
                 ! unconverged interval found
                 prev = i
                 nint = nint + 1_ilp64
           ...
           loop_100: do ip = 1, olnint
              ...
              if( ( width<=cvrgd ) .or. ( width<=mnwdth ).or.( iter==maxitr ) )then
                 ...
                 if( i1==i ) then
                    ...
                    ! prev holds the last unconverged interval previously examined
                    if(prev>=i1) iwork( 2_ilp64*prev-1 ) = next
                 end if
                 i = next
                 cycle loop_100
              end if
              prev = i
              ! perform one bisection step
              negcnt = stdlib_I64_slaneg( n, d, lld, mid, pivmin, r )
              if( negcnt<=i-1 ) then
                 work( k-1 ) = mid
              else
                 work( k ) = mid
     ...
     !! semi-widths in the arrays W and WERR respectively.
        ...
        ! =====================================================================
           ...
           loop_75: do i = i1, ilast
              ...
              back = werr( ii )
              20 continue
              negcnt = stdlib_I64_dlaneg( n, d, lld, left, pivmin, r )
              if( negcnt>i-1 ) then
                 left = left - back
                 back = two*back
                 go to 20
              ...
              back = werr( ii )
              50 continue
              negcnt = stdlib_I64_dlaneg( n, d, lld, right, pivmin, r )
               if( negcnt<i ) then
                  right = right + back
                  back = two*back
                  go to 50
           ...
           loop_100: do ip = 1, olnint
              ...
              prev = i
              ! perform one bisection step
              negcnt = stdlib_I64_dlaneg( n, d, lld, mid, pivmin, r )
              if( negcnt<=i-1 ) then
                 work( k-1 ) = mid
              else
                 work( k ) = mid
     ...
     !! University, July 21, 1966.
        ...
        ! =====================================================================
           ! Parameters 
           real(sp), parameter :: fudge = two
           integer(ilp64), parameter :: allrng = 1_ilp64
           integer(ilp64), parameter :: valrng = 2_ilp64
           integer(ilp64), parameter :: indrng = 3_ilp64
           
           
           ! Local Scalars 
           logical(lk) :: ncnvrg, toofew
           integer(ilp64) :: i, ib, ibegin, idiscl, idiscu, ie, iend, iinfo, im, in, ioff, iout, &
                     irange, itmax, itmp1, itmp2, iw, iwoff, j, jblk, jdisc, je, jee, nb, nwl, nwu
           real(sp) :: atoli, eps, gl, gu, rtoli, tmp1, tmp2, tnorm, uflow, wkill, wlu, &
                     wul
           ! Local Arrays 
           ...
           else if( irange==indrng .and.( il<1_ilp64 .or. il>max( 1_ilp64, n ) ) ) then
              info = -6_ilp64
           else if( irange==indrng .and.( iu<min( n, il ) .or. iu>n ) ) then
              info = -7_ilp64
           end if
           ...
           if( n==0 ) return
           ! simplification:
           if( irange==indrng .and. il==1_ilp64 .and. iu==n ) irange = 1_ilp64
           ! get machine constants
           eps = stdlib_I64_slamch( 'P' )
           uflow = stdlib_I64_slamch( 'U' )
           ! special case when n=1
           ! treat case of 1x1 matrix for quick return
           if( n==1_ilp64 ) then
              if( (irange==allrng).or.((irange==valrng).and.(d(1_ilp64)>vl).and.(d(1_ilp64)<=vu)).or.((&
                        irange==indrng).and.(il==1_ilp64).and.(iu==1_ilp64)) ) then
                 m = 1_ilp64
                 w(1_ilp64) = d(1_ilp64)
                 ! the computation error of the eigenvalue is zero
                 werr(1_ilp64) = zero
                 iblock( 1_ilp64 ) = 1_ilp64
                 indexw( 1_ilp64 ) = 1_ilp64
              endif
              return
           end if
           ! nb is the minimum vector length for vector bisection, or 0
           ...
           gu = d(1_ilp64)
           do i = 1,n
              gl =  min( gl, gers( 2_ilp64*i - 1_ilp64))
              gu = max( gu, gers(2_ilp64*i) )
           end do
           ! compute global gerschgorin bounds and spectral diameter
           tnorm = max( abs( gl ), abs( gu ) )
           gl = gl - fudge*tnorm*eps*n - fudge*two*pivmin
           ...
           ! [jan/28/2009] remove the line below since spdiam variable not use
           ! spdiam = gu - gl
           ! input arguments for stdlib_I64_slaebz:
           ! the relative tolerance.  an interval (a,b] lies within
           ! "relative tolerance" if  b-a < reltol*max(|a|,|b|),
           rtoli = reltol
           ! set the absolute tolerance for interval convergence to zero to force
           ...
           if( irange==indrng ) then
              ! range='i': compute an interval containing eigenvalues
              ! il through iu. the initial interval [gl,gu] from the global
              ! gerschgorin bounds gl and gu is refined by stdlib_I64_slaebz.
              ...
              iwork( 6_ilp64 ) = iu
              call stdlib_I64_slaebz( 3_ilp64, itmax, n, 2_ilp64, 2_ilp64, nb, atoli, rtoli, pivmin,d, e, e2, iwork( 5_ilp64 )&
                        , work( n+1 ), work( n+5 ), iout,iwork, w, iblock, iinfo )
           ...
           loop_70: do jblk = 1, nsplit
              ...
              if( in==1_ilp64 ) then
                 ! 1x1 block
                 if( wl>=d( ibegin )-pivmin )nwl = nwl + 1_ilp64
                 if( wu>=d( ibegin )-pivmin )nwu = nwu + 1_ilp64
                 if( irange==allrng .or.( wl<d( ibegin )-pivmin.and. wu>= d( ibegin )-pivmin ) ) &
                           then
                    m = m + 1_ilp64
                    w( m ) = d( ibegin )
      ...
      ! *              the uncertainty of eigenvalues of a 2x2 matrix is very small
                 ...
                 ! general case - block of size in >= 2
                 ! compute local gerschgorin interval and use it as the initial
                 ! interval for stdlib_I64_slaebz
                 gu = d( ibegin )
                 gl = d( ibegin )
                 tmp1 = zero
                 do j = ibegin, iend
                    gl =  min( gl, gers( 2_ilp64*j - 1_ilp64))
                    gu = max( gu, gers(2_ilp64*j) )
                 end do
                 ! [jan/28/2009]
                 ! change spdiam by tnorm in lines 2 and 3 thereafter
                 ! line 1: remove computation of spdiam (not useful anymore)
                 ...
                 ! copy eigenvalues into w and iblock
                 ! use -jblk for block number for unconverged eigenvalues.
                 ! loop over the number of output intervals from stdlib_I64_slaebz
                 do j = 1, iout
                    ! eigenvalue approximation is middle point of interval
                    tmp1 = half*( work( j+n )+work( j+in+n ) )
                    ! semi length of error interval
           ...
           if( irange==indrng ) then
              ...
              if( idiscu>0_ilp64 ) then
                 ...
                 im=m+1
                 do je = m, 1, -1
                    if( w( je )>=wul .and. idiscu>0_ilp64 ) then
           ...
           ! if order='b', do nothing the eigenvalues are already sorted by
              ! block.
           ! if order='e', sort the eigenvalues from smallest to largest
           if( stdlib_lsame(order,'E') .and. nsplit>1_ilp64 ) then
              do je = 1, m - 1
                 ie = 0_ilp64
                 tmp1 = w( je )
                 do j = je + 1, m
     ...
     !! University, July 21, 1966.
        ...
        ! =====================================================================
           ...
           ! [jan/28/2009] remove the line below since spdiam variable not use
           ! spdiam = gu - gl
           ! input arguments for stdlib_I64_dlaebz:
           ! the relative tolerance.  an interval (a,b] lies within
           ! "relative tolerance" if  b-a < reltol*max(|a|,|b|),
           rtoli = reltol
           ! set the absolute tolerance for interval convergence to zero to force
           ...
           if( irange==indrng ) then
              ! range='i': compute an interval containing eigenvalues
              ! il through iu. the initial interval [gl,gu] from the global
              ! gerschgorin bounds gl and gu is refined by stdlib_I64_dlaebz.
              itmax = int( ( log( tnorm+pivmin )-log( pivmin ) ) /log( two ),KIND=ilp64) + 2_ilp64
              work( n+1 ) = gl
              work( n+2 ) = gl
              work( n+3 ) = gu
              ...
              iwork( 5_ilp64 ) = il - 1_ilp64
              iwork( 6_ilp64 ) = iu
              call stdlib_I64_dlaebz( 3_ilp64, itmax, n, 2_ilp64, 2_ilp64, nb, atoli, rtoli, pivmin,d, e, e2, iwork( 5_ilp64 )&
                        , work( n+1 ), work( n+5 ), iout,iwork, w, iblock, iinfo )
              if( iinfo /= 0_ilp64 ) then
                 info = iinfo
                 return
      ...
      ! *              the uncertainty of eigenvalues of a 2x2 matrix is very small
                 ...
                 ! general case - block of size in >= 2
                 ! compute local gerschgorin interval and use it as the initial
                 ! interval for stdlib_I64_dlaebz
                 gu = d( ibegin )
                 gl = d( ibegin )
                 tmp1 = zero
                 do j = ibegin, iend
                 ...
                 ! copy eigenvalues into w and iblock
                 ! use -jblk for block number for unconverged eigenvalues.
                 ! loop over the number of output intervals from stdlib_I64_dlaebz
                 do j = 1, iout
                    ! eigenvalue approximation is middle point of interval
                    tmp1 = half*( work( j+n )+work( j+in+n ) )
                    ! semi length of error interval
     ...
     !! Gerschgorin intervals for the matrices L_i D_i L_i^T.
        ...
        ! =====================================================================
           ...
           real(sp), parameter :: maxgrowth = 64.0_sp
           real(sp), parameter :: fudge = two
           integer(ilp64), parameter :: maxtry = 6_ilp64
           integer(ilp64), parameter :: allrng = 1_ilp64
           integer(ilp64), parameter :: indrng = 2_ilp64
           integer(ilp64), parameter :: valrng = 3_ilp64
           
           
           ! Local Scalars 
           logical(lk) :: forceb, norep, usedqd
           integer(ilp64) :: cnt, cnt1, cnt2, i, ibegin, idum, iend, iinfo, in, indl, indu, irange, &
                     j, jblk, mb, mm, wbegin, wend
           real(sp) :: avgap, bsrtol, clwdth, dmax, dpivot, eabs, emax, eold, eps, gl, gu, isleft,&
                      isrght, rtl, rtol, s1, s2, safmin, sgndef, sigma, spdiam, tau, tmp, tmp1
           ! Local Arrays 
           ...
           if( n==1_ilp64 ) then
              if( (irange==allrng).or.((irange==valrng).and.(d(1_ilp64)>vl).and.(d(1_ilp64)<=vu)).or.((&
                 ...
                 iblock( 1_ilp64 ) = 1_ilp64
                 indexw( 1_ilp64 ) = 1_ilp64
                 gers(1_ilp64) = d( 1_ilp64 )
                 gers(2_ilp64) = d( 1_ilp64 )
              endif
              ! store the shift for the initial rrr, which is zero in this case
              e(1_ilp64) = zero
              return
           ...
           do i = 1,n
              ...
              tmp1 = eabs + eold
              gers( 2_ilp64*i-1) = d(i) - tmp1
              gl =  min( gl, gers( 2_ilp64*i - 1_ilp64))
              gers( 2_ilp64*i ) = d(i) + tmp1
              gu = max( gu, gers(2_ilp64*i) )
              eold  = eabs
           end do
           ! the minimum pivot allowed in the sturm sequence for t
           pivmin = safmin * max( one, emax**2_ilp64 )
           ! compute spectral diameter. the gerschgorin bounds give an
           ! estimate that is wrong by at most a factor of sqrt(2)
           spdiam = gu - gl
           ! compute splitting points
           call stdlib_I64_slarra( n, d, e, e2, spltol, spdiam,nsplit, isplit, iinfo )
           ! can force use of bisection instead of faster dqds.
           ! option left in the code for future multisection work.
           forceb = .false.
           ! initialize usedqd, dqds should be used for allrng unless someone
           ...
           if( (irange==allrng) .and. (.not. forceb) ) then
              ...
              ! we call stdlib_I64_slarrd to find crude approximations to the eigenvalues
              ! in the desired range. in case irange = indrng, we also obtain the
              ! interval (vl,vu] that contains all the wanted eigenvalues.
              ! an interval [left,right] has converged if
              ! right-left<rtol*max(abs(left),abs(right))
              ! stdlib_I64_slarrd needs a work of size 4*n, iwork of size 3*n
              call stdlib_I64_slarrd( range, 'B', n, vl, vu, il, iu, gers,bsrtol, d, e, e2, pivmin, &
                        nsplit, isplit,mm, w, werr, vl, vu, iblock, indexw,work, iwork, iinfo )
              if( iinfo/=0_ilp64 ) then
                 info = -1_ilp64
                 return
      ...
      ! **
           ...
           loop_170: do jblk = 1, nsplit
              ...
              gu = d(ibegin)
              do i = ibegin , iend
                 gl = min( gers( 2_ilp64*i-1 ), gl )
                 gu = max( gers( 2_ilp64*i ), gu )
              end do
              spdiam = gu - gl
              if(.not. ((irange==allrng).and.(.not.forceb)) ) then
                 ! count the number of eigenvalues in the current block.
              ...
              if(( (irange==allrng) .and. (.not. forceb) ).or.usedqd) then
                 ! case of dqds
                 ! find approximations to the extremal eigenvalues of the block
                 call stdlib_I64_slarrk( in, 1_ilp64, gl, gu, d(ibegin),e2(ibegin), pivmin, rtl, tmp, tmp1, &
                           iinfo )
                 if( iinfo/=0_ilp64 ) then
                    info = -1_ilp64
                    return
                 ...
                 isleft = max(gl, tmp - tmp1- hndrd * eps* abs(tmp - tmp1))
                 call stdlib_I64_slarrk( in, in, gl, gu, d(ibegin),e2(ibegin), pivmin, rtl, tmp, tmp1,&
                            iinfo )
                 if( iinfo/=0_ilp64 ) then
                    info = -1_ilp64
                    return
              ...
              if( ( irange==allrng ) .and. (.not.forceb) ) then
                 ...
                 ! stdlib_I64_slarrd has computed iblock and indexw for each eigenvalue
                 ! approximation.
                 ! choose sigma
                 if( usedqd ) then
                    s1 = isleft + fourth * spdiam
              ...
              ! compute the negcount at the 1/4 and 3/4 points
              if(mb>1_ilp64) then
                 call stdlib_I64_slarrc( 'T', in, s1, s2, d(ibegin),e(ibegin), pivmin, cnt, cnt1, &
                           cnt2, iinfo)
              endif
              if(mb==1_ilp64) then
                 sigma = gl
              ...
              loop_80: do idum = 1, maxtry
                 ...
                 dpivot = d( ibegin ) - sigma
                 work( 1_ilp64 ) = dpivot
                 dmax = abs( work(1_ilp64) )
                 j = ibegin
                 do i = 1, in - 1
                    work( 2_ilp64*in+i ) = one / work( i )
                    tmp = e( j )*work( 2_ilp64*in+i )
                    work( in+i ) = tmp
                    dpivot = ( d( j+1 )-sigma ) - tmp*e( j )
                    work( i+1 ) = dpivot
                    dmax = max( dmax, abs(dpivot) )
              ...
              e( iend ) = sigma
              ! store d and l.
              call stdlib_I64_scopy( in, work, 1_ilp64, d( ibegin ), 1_ilp64 )
              call stdlib_I64_scopy( in-1, work( in+1 ), 1_ilp64, e( ibegin ), 1_ilp64 )
              if(mb>1_ilp64 ) then
                 ! perturb each entry of the base representation by a small
                 ! (but random) relative amount to overcome difficulties with
                 ! glued matrices.
                 ...
                 call stdlib_I64_slarnv(2_ilp64, iseed, 2_ilp64*in-1, work(1_ilp64))
                 do i = 1,in-1
                    d(ibegin+i-1) = d(ibegin+i-1)*(one+eps*pert*work(i))
                    e(ibegin+i-1) = e(ibegin+i-1)*(one+eps*pert*work(in+i))
                 end do
              ...
              ! don't update the gerschgorin intervals because keeping track
              ! of the updates would be too much work in stdlib_I64_slarrv.
              ! we update w instead and use it to locate the proper gerschgorin
              ! intervals.
              ! compute the required eigenvalues of l d l' by bisection or dqds
              if ( .not.usedqd ) then
                 ! if stdlib_I64_slarrd has been used, shift the eigenvalue approximations
                 ! according to their representation. this is necessary for
                 ! a uniform stdlib_I64_slarrv since dqds computes eigenvalues of the
                 ! shifted representation. in stdlib_I64_slarrv, w will always hold the
                 ! unshifted eigenvalue approximation.
                 do j=wbegin,wend
                    w(j) = w(j) - sigma
                    werr(j) = werr(j) + abs(w(j)) * eps
                 end do
                 ! call stdlib_I64_slarrb to reduce eigenvalue error of the approximations
                 ! from stdlib_I64_slarrd
                 do i = ibegin, iend-1
                    work( i ) = d( i ) * e( i )**2_ilp64
                 end do
                 ! use bisection to find ev from indl to indu
                 call stdlib_I64_slarrb(in, d(ibegin), work(ibegin),indl, indu, rtol1, rtol2, indl-1,&
                 w(wbegin), wgap(wbegin), werr(wbegin),work( 2_ilp64*n+1 ), iwork, pivmin, spdiam,in, &
                           iinfo )
                 if( iinfo /= 0_ilp64 ) then
                    info = -4_ilp64
                    return
                 end if
                 ! stdlib_I64_slarrb computes all gaps correctly except for the last one
                 ! record distance to vu/gu
                 wgap( wend ) = max( zero,( vu-sigma ) - ( w( wend ) + werr( wend ) ) )
                 do i = indl, indu
                    m = m + 1_ilp64
                 ...
                 ! the eigenvalues of t. however, t is not guaranteed to define its
                 ! eigenvalues to high relative accuracy anyway.
                 ! set rtol to the order of the tolerance used in stdlib_I64_slasq2
                 ! this is an estimated error, the worst case bound is 4*n*eps
                 ! which is usually too large and requires unnecessary work to be
                 ! done by bisection when computing the eigenvectors
                 rtol = log(real(in,KIND=sp)) * four * eps
                 j = ibegin
                 do i = 1, in - 1
                    work( 2_ilp64*i-1 ) = abs( d( j ) )
                    work( 2_ilp64*i ) = e( j )*e( j )*work( 2_ilp64*i-1 )
                    j = j + 1_ilp64
                 end do
                 work( 2_ilp64*in-1 ) = abs( d( iend ) )
                 work( 2_ilp64*in ) = zero
                 call stdlib_I64_slasq2( in, work, iinfo )
                 if( iinfo /= 0_ilp64 ) then
                    ! if iinfo = -5 then an index is part of a tight cluster
                    ! and should be changed. the index is in iwork(1) and the
                    ! gap is in work(n+1)
                 ...
                 do i = m - mb + 1, m
                    ! the value of rtol below should be the tolerance in stdlib_I64_slasq2
                    werr( i ) = rtol * abs( w(i) )
                 end do
                 do i = m - mb + 1, m - 1
                    ! compute the right gap between the intervals
     ...
     !! Gerschgorin intervals for the matrices L_i D_i L_i^T.
        ...
        ! =====================================================================
           ...
           spdiam = gu - gl
           ! compute splitting points
           call stdlib_I64_dlarra( n, d, e, e2, spltol, spdiam,nsplit, isplit, iinfo )
           ! can force use of bisection instead of faster dqds.
           ! option left in the code for future multisection work.
           forceb = .false.
           ! initialize usedqd, dqds should be used for allrng unless someone
           ...
           if( (irange==allrng) .and. (.not. forceb) ) then
              ...
              ! we call stdlib_I64_dlarrd to find crude approximations to the eigenvalues
              ! in the desired range. in case irange = indrng, we also obtain the
              ! interval (vl,vu] that contains all the wanted eigenvalues.
              ! an interval [left,right] has converged if
              ! right-left<rtol*max(abs(left),abs(right))
              ! stdlib_I64_dlarrd needs a work of size 4*n, iwork of size 3*n
              call stdlib_I64_dlarrd( range, 'B', n, vl, vu, il, iu, gers,bsrtol, d, e, e2, pivmin, &
                        nsplit, isplit,mm, w, werr, vl, vu, iblock, indexw,work, iwork, iinfo )
              if( iinfo/=0_ilp64 ) then
                 info = -1_ilp64
                 return
      ...
      ! **
           ...
           loop_170: do jblk = 1, nsplit
              ...
              if(( (irange==allrng) .and. (.not. forceb) ).or.usedqd) then
                 ! case of dqds
                 ! find approximations to the extremal eigenvalues of the block
                 call stdlib_I64_dlarrk( in, 1_ilp64, gl, gu, d(ibegin),e2(ibegin), pivmin, rtl, tmp, tmp1, &
                           iinfo )
                 if( iinfo/=0_ilp64 ) then
                    info = -1_ilp64
                    return
                 ...
                 isleft = max(gl, tmp - tmp1- hndrd * eps* abs(tmp - tmp1))
                 call stdlib_I64_dlarrk( in, in, gl, gu, d(ibegin),e2(ibegin), pivmin, rtl, tmp, tmp1,&
                            iinfo )
                 if( iinfo/=0_ilp64 ) then
                    info = -1_ilp64
                    return
              ...
              if( ( irange==allrng ) .and. (.not.forceb) ) then
                 ...
                 ! stdlib_I64_dlarrd has computed iblock and indexw for each eigenvalue
                 ! approximation.
                 ! choose sigma
                 if( usedqd ) then
                    s1 = isleft + fourth * spdiam
              ...
              ! compute the negcount at the 1/4 and 3/4 points
              if(mb>1_ilp64) then
                 call stdlib_I64_dlarrc( 'T', in, s1, s2, d(ibegin),e(ibegin), pivmin, cnt, cnt1, &
                           cnt2, iinfo)
              endif
              if(mb==1_ilp64) then
                 sigma = gl
              ...
              e( iend ) = sigma
              ! store d and l.
              call stdlib_I64_dcopy( in, work, 1_ilp64, d( ibegin ), 1_ilp64 )
              call stdlib_I64_dcopy( in-1, work( in+1 ), 1_ilp64, e( ibegin ), 1_ilp64 )
              if(mb>1_ilp64 ) then
                 ! perturb each entry of the base representation by a small
                 ! (but random) relative amount to overcome difficulties with
                 ! glued matrices.
                 ...
                 call stdlib_I64_dlarnv(2_ilp64, iseed, 2_ilp64*in-1, work(1_ilp64))
                 do i = 1,in-1
                    d(ibegin+i-1) = d(ibegin+i-1)*(one+eps*pert*work(i))
                    e(ibegin+i-1) = e(ibegin+i-1)*(one+eps*pert*work(in+i))
                 end do
              ...
              ! don't update the gerschgorin intervals because keeping track
              ! of the updates would be too much work in stdlib_I64_dlarrv.
              ! we update w instead and use it to locate the proper gerschgorin
              ! intervals.
              ! compute the required eigenvalues of l d l' by bisection or dqds
              if ( .not.usedqd ) then
                 ! if stdlib_I64_dlarrd has been used, shift the eigenvalue approximations
                 ! according to their representation. this is necessary for
                 ! a uniform stdlib_I64_dlarrv since dqds computes eigenvalues of the
                 ! shifted representation. in stdlib_I64_dlarrv, w will always hold the
                 ! unshifted eigenvalue approximation.
                 do j=wbegin,wend
                    w(j) = w(j) - sigma
                    werr(j) = werr(j) + abs(w(j)) * eps
                 end do
                 ! call stdlib_I64_dlarrb to reduce eigenvalue error of the approximations
                 ! from stdlib_I64_dlarrd
                 do i = ibegin, iend-1
                    work( i ) = d( i ) * e( i )**2_ilp64
                 end do
                 ! use bisection to find ev from indl to indu
                 call stdlib_I64_dlarrb(in, d(ibegin), work(ibegin),indl, indu, rtol1, rtol2, indl-1,&
                 w(wbegin), wgap(wbegin), werr(wbegin),work( 2_ilp64*n+1 ), iwork, pivmin, spdiam,in, &
                           iinfo )
                 if( iinfo /= 0_ilp64 ) then
                    info = -4_ilp64
                 ...
                 ! stdlib_I64_dlarrb computes all gaps correctly except for the last one
                 ! record distance to vu/gu
                 wgap( wend ) = max( zero,( vu-sigma ) - ( w( wend ) + werr( wend ) ) )
                 do i = indl, indu
                    m = m + 1_ilp64
                 ...
                 ! the eigenvalues of t. however, t is not guaranteed to define its
                 ! eigenvalues to high relative accuracy anyway.
                 ! set rtol to the order of the tolerance used in stdlib_I64_dlasq2
                 ! this is an estimated error, the worst case bound is 4*n*eps
                 ! which is usually too large and requires unnecessary work to be
                 ! done by bisection when computing the eigenvectors
                 rtol = log(real(in,KIND=dp)) * four * eps
                 ...
                 work( 2_ilp64*in-1 ) = abs( d( iend ) )
                 work( 2_ilp64*in ) = zero
                 call stdlib_I64_dlasq2( in, work, iinfo )
                 if( iinfo /= 0_ilp64 ) then
                    ! if iinfo = -5 then an index is part of a tight cluster
                    ! and should be changed. the index is in iwork(1) and the
                    ! gap is in work(n+1)
                 ...
                 do i = m - mb + 1, m
                    ! the value of rtol below should be the tolerance in stdlib_I64_dlasq2
                    werr( i ) = rtol * abs( w(i) )
                 end do
                 do i = m - mb + 1, m - 1
                    ! compute the right gap between the intervals
     ...
     !! eigenvalues of L(+) D(+) L(+)^T is relatively isolated.
        ...
        ! =====================================================================
           ...
           real(sp), parameter :: maxgrowth1 = 8._sp
           real(sp), parameter :: maxgrowth2 = 8._sp
           integer(ilp64), parameter :: ktrymax = 1_ilp64
           integer(ilp64), parameter :: sleft = 1_ilp64
           integer(ilp64), parameter :: sright = 2_ilp64
           
           ! Local Scalars 
           logical(lk) :: dorrr1, forcer, nofail, sawnan1, sawnan2, tryrrr1
           integer(ilp64) :: i, indx, ktry,    shift
           real(sp) :: avgap, bestshift, clwdth, eps, fact, fail, fail2, growthbound, ldelta, &
           ldmax, lsigma, max1, max2, mingap, oldp, prod, rdelta, rdmax, rrr1, rrr2, rsigma, s, &
                     smlgrowth, tmp, znm2
           ! Intrinsic Functions 
           ...
           fact = real(2_ilp64**ktrymax,KIND=sp)
           ...
           ! left end
           s = -lsigma
           dplus( 1_ilp64 ) = d( 1_ilp64 ) + s
           if(abs(dplus(1_ilp64))<pivmin) then
              dplus(1_ilp64) = -pivmin
              ! need to set sawnan1 because refined rrr test should not be used
              ! in this case
              sawnan1 = .true.
           endif
           max1 = abs( dplus( 1_ilp64 ) )
           do i = 1, n - 1
              lplus( i ) = ld( i ) / dplus( i )
              s = s*lplus( i )*l( i ) - lsigma
              dplus( i+1 ) = d( i+1 ) + s
           ...
           sawnan1 = sawnan1 .or.  stdlib_I64_sisnan( max1 )
           if( forcer .or.(max1<=growthbound .and. .not.sawnan1 ) ) then
              sigma = lsigma
              shift = sleft
              goto 100
           ...
           ! right end
           s = -rsigma
           work( 1_ilp64 ) = d( 1_ilp64 ) + s
           if(abs(work(1_ilp64))<pivmin) then
              work(1_ilp64) = -pivmin
              ! need to set sawnan2 because refined rrr test should not be used
              ! in this case
              sawnan2 = .true.
           endif
           max2 = abs( work( 1_ilp64 ) )
           do i = 1, n - 1
              work( n+i ) = ld( i ) / work( i )
              s = s*work( n+i )*l( i ) - rsigma
              work( i+1 ) = d( i+1 ) + s
           ...
           sawnan2 = sawnan2 .or.  stdlib_I64_sisnan( max2 )
           if( forcer .or.(max2<=growthbound .and. .not.sawnan2 ) ) then
              sigma = rsigma
              shift = sright
              goto 100
           ...
           if(sawnan1.and.sawnan2) then
              ...
              if( .not.sawnan2 ) then
                 if(sawnan1 .or. max2<=max1) indx = 2_ilp64
                 if(max2<=smlgrowth) then
                    smlgrowth = max2
                    bestshift = rsigma
                 endif
           ...
           ! refined test for rrr. this test supposes that no nan occurred.
           ! moreover, we use the refined rrr test only for isolated clusters.
           if((clwdth<mingap/real(128_ilp64,KIND=sp)) .and.(min(max1,max2)<fail2).and.(.not.sawnan1)&
                     .and.(.not.sawnan2)) then
              dorrr1 = .true.
           else
              dorrr1 = .false.
           ...
           elseif (shift==sright) then
              ! store new l and d back into dplus, lplus
              call stdlib_I64_scopy( n, work, 1_ilp64, dplus, 1_ilp64 )
              call stdlib_I64_scopy( n-1, work(n+1), 1_ilp64, lplus, 1_ilp64 )
           endif
           return
     ...
     !! eigenvalues of L(+) D(+) L(+)^T is relatively isolated.
        ...
        ! =====================================================================
           ...
           fact = real(2_ilp64**ktrymax,KIND=dp)
           ...
           sawnan1 = sawnan1 .or.  stdlib_I64_disnan( max1 )
           if( forcer .or.(max1<=growthbound .and. .not.sawnan1 ) ) then
              sigma = lsigma
              shift = sleft
              goto 100
           ...
           sawnan2 = sawnan2 .or.  stdlib_I64_disnan( max2 )
           if( forcer .or.(max2<=growthbound .and. .not.sawnan2 ) ) then
              sigma = rsigma
              shift = sright
              goto 100
           ...
           ! refined test for rrr. this test supposes that no nan occurred.
           ! moreover, we use the refined rrr test only for isolated clusters.
           if((clwdth<mingap/real(128_ilp64,KIND=dp)) .and.(min(max1,max2)<fail2).and.(.not.sawnan1)&
                     .and.(.not.sawnan2)) then
              dorrr1 = .true.
           else
              dorrr1 = .false.
           ...
           elseif (shift==sright) then
              ! store new l and d back into dplus, lplus
              call stdlib_I64_dcopy( n, work, 1_ilp64, dplus, 1_ilp64 )
              call stdlib_I64_dcopy( n-1, work(n+1), 1_ilp64, lplus, 1_ilp64 )
           endif
           return
     ...
     !! semi-widths in the arrays W and WERR respectively.
        ...
        ! =====================================================================
           ...
           integer(ilp64) :: maxitr
           ! Local Scalars 
           integer(ilp64) :: cnt, i, i1, i2, ii, iter, j, k, next, nint, olnint, p, prev, &
                     savi1
           real(sp) :: dplus, fac, left, mid, right, s, tmp, width
           ! Intrinsic Functions 
           ! Executable Statements 
           ...
           loop_75: do i = i1, i2
              ...
              if( width<rtol*tmp ) then
                 ...
                 iwork( k-1 ) = -1_ilp64
                 ! make sure that i1 always points to the first unconverged interval
                 if((i==i1).and.(i<i2)) i1 = i + 1_ilp64
                 if((prev>=i1).and.(i<=i2)) iwork( 2_ilp64*prev-1 ) = i + 1_ilp64
              else
                 ! unconverged interval found
                 prev = i
                 ! make sure that [left,right] contains the desired eigenvalue
     ...
     !! University, July 21, 1966.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           integer(ilp64) :: i, it, itmax, negcnt
           real(sp) :: atoli, eps, left, mid, right, rtoli, tmp1, tmp2, tnorm
           ! Intrinsic Functions 
           ! Executable Statements 
           ! quick return if possible
           ...
           tmp1 = d( 1_ilp64 ) - mid
           if( abs( tmp1 )<pivmin )tmp1 = -pivmin
           if( tmp1<=zero )negcnt = negcnt + 1_ilp64
           do i = 2, n
              tmp1 = d( i ) - e2( i-1 ) / tmp1 - mid
              if( abs( tmp1 )<pivmin )tmp1 = -pivmin
              if( tmp1<=zero )negcnt = negcnt + 1_ilp64
     ...
     !! The input eigenvalues should have been computed by SLARRE.
        ...
        ! =====================================================================
           ! Parameters 
           integer(ilp64), parameter :: maxitr = 10_ilp64
           
           
           ! Local Scalars 
           logical(lk) :: eskip, needbs, stp2ii, tryrqc, usedbs, usedrq
           integer(ilp64) :: done, i, ibegin, idone, iend, ii, iindc1, iindc2, iindr, iindwk, iinfo,&
            im, in, indeig, indld, indlld, indwrk, isupmn, isupmx, iter, itmp1, j, jblk, k, &
            miniwsize, minwsize, nclus, ndepth, negcnt, newcls, newfst, newftt, newlst, newsiz, &
            offset, oldcls, oldfst, oldien, oldlst, oldncl, p, parity, q, wbegin, wend, windex, &
                      windmn, windpl, zfrom, zto, zusedl, zusedu, zusedw
           ...
           ! the width of the part of z that is used
           zusedw = zusedu - zusedl + 1_ilp64
           call stdlib_I64_slaset( 'FULL', n, zusedw, zero, zero,z(1_ilp64,zusedl), ldz )
           eps = stdlib_I64_slamch( 'PRECISION' )
           rqtol = two * eps
           ! set expert flags for standard code.
           tryrqc = .true.
           if((dol==1_ilp64).and.(dou==m)) then
           else
              ! only selected eigenpairs are computed. since the other evalues
              ! are not refined by rq iteration, bisection has to compute to full
              ! accuracy.
           ...
           loop_170: do jblk = 1, iblock( m )
              ...
              ! find local spectral diameter of the block
              gl = gers( 2_ilp64*ibegin-1 )
              gu = gers( 2_ilp64*ibegin )
              do i = ibegin+1 , iend
                 gl = min( gers( 2_ilp64*i-1 ), gl )
                 gu = max( gers( 2_ilp64*i ), gu )
              end do
              spdiam = gu - gl
              ! oldien is the last index of the previous block
              oldien = ibegin - 1_ilp64
              ! calculate the size of the current block
              in = iend - ibegin + 1_ilp64
              ! the number of eigenvalues in the current block
              im = wend - wbegin + 1_ilp64
              ! this is for a 1x1 block
              if( ibegin==iend ) then
                 done = done+1
                 z( ibegin, wbegin ) = one
                 isuppz( 2_ilp64*wbegin-1 ) = ibegin
                 isuppz( 2_ilp64*wbegin ) = ibegin
                 w( wbegin ) = w( wbegin ) + sigma
                 work( wbegin ) = w( wbegin )
                 ibegin = iend + 1_ilp64
                 wbegin = wbegin + 1_ilp64
              ...
              ! high relative accuracy is required for the computation of the
              ! corresponding eigenvectors.
              call stdlib_I64_scopy( im, w( wbegin ), 1_ilp64,work( wbegin ), 1_ilp64 )
              ! we store in w the eigenvalue approximations w.r.t. the original
              ! matrix t.
              do i=1,im
                 w(wbegin+i-1) = w(wbegin+i-1)+sigma
              ...
              if( idone<im ) then
                 ...
                 ! reset nclus to count the number of child clusters
                 nclus = 0_ilp64
                 parity = 1_ilp64 - parity
                 if( parity==0_ilp64 ) then
                    oldcls = iindc1
                    newcls = iindc2
                 else
                    oldcls = iindc2
                 ...
                 loop_150: do i = 1, oldncl
                    ...
                    if( ndepth>0_ilp64 ) then
                       ...
                       call stdlib_I64_scopy( in, z( ibegin, j ), 1_ilp64, d( ibegin ), 1_ilp64 )
                       call stdlib_I64_scopy( in-1, z( ibegin, j+1 ), 1_ilp64, l( ibegin ),1_ilp64 )
                       sigma = z( iend, j+1 )
                       ! set the corresponding entries in z to zero
                       call stdlib_I64_slaset( 'FULL', in, 2_ilp64, zero, zero,z( ibegin, j), ldz )
                    end if
                    ! compute dl and dll of current rrr
                    do j = ibegin, iend-1
                       tmp = d( j )*l( j )
                    ...
                    if( ndepth>0_ilp64 ) then
                       ...
                       ! through the q-offset elements of these arrays are to be used.
                        ! offset = p-oldfst
                       offset = indexw( wbegin ) - 1_ilp64
                       ! perform limited bisection (if necessary) to get approximate
                       ! eigenvalues to the precision needed.
                       call stdlib_I64_slarrb( in, d( ibegin ),work(indlld+ibegin-1),p, q, rtol1, &
                       rtol2, offset,work(wbegin),wgap(wbegin),werr(wbegin),work( indwrk ), iwork(&
                                  iindwk ),pivmin, spdiam, in, iinfo )
                       if( iinfo/=0_ilp64 ) then
                          info = -1_ilp64
                       ...
                       ! of wgap, the entries of work might stem from rrrs with
                       ! different shifts. the gaps from wbegin-1+oldfst to
                       ! wbegin-1+oldlst are correctly computed in stdlib_I64_slarrb.
                       ! however, we only allow the gaps to become greater since
                       ! this is what should happen when we decrease werr
                       if( oldfst>1_ilp64) then
                          wgap( wbegin+oldfst-2 ) =max(wgap(wbegin+oldfst-2),w(wbegin+oldfst-1)-&
                          werr(wbegin+oldfst-1)- w(wbegin+oldfst-2)-werr(wbegin+oldfst-2) )
                                    
                       endif
                       if( wbegin + oldlst -1_ilp64 < wend ) then
                          wgap( wbegin+oldlst-1 ) =max(wgap(wbegin+oldlst-1),w(wbegin+oldlst)-&
                                    werr(wbegin+oldlst)- w(wbegin+oldlst-1)-werr(wbegin+oldlst-1) )
                       endif
                       ! each time the eigenvalues in work get refined, we store
                    ...
                    loop_140: do j = oldfst, oldlst
                       if( j==oldlst ) then
                          ...
                          ! boundary of the child cluster
                          newlst = j
                       else if ( wgap( wbegin + j -1_ilp64)>=minrgp* abs( work(wbegin + j -1_ilp64) ) ) &
                                 then
                          ! the right relative gap is big enough, the child cluster
                          ! (newfst,..,newlst) is well separated from the following
                          newlst = j
                       ...
                       ! compute size of child cluster found
                       newsiz = newlst - newfst + 1_ilp64
                       ! newftt is the place in z where the new rrr or the computed
                       ! eigenvector is to be stored
                       if((dol==1_ilp64).and.(dou==m)) then
                          ! store representation at location of the leftmost evalue
                          ! of the cluster
                          newftt = wbegin + newfst - 1_ilp64
                       else
                          if(wbegin+newfst-1<dol) then
                             ! store representation at the left end of z array
                             newftt = dol - 1_ilp64
                       ...
                       if( newsiz>1_ilp64) then
                          ...
                          do k =1,2
                             ...
                             offset = indexw( wbegin ) - 1_ilp64
                             call stdlib_I64_slarrb( in, d(ibegin),work( indlld+ibegin-1 ),p,p,rqtol, &
                             rqtol, offset,work(wbegin),wgap(wbegin),werr(wbegin),work( indwrk ),&
                                       iwork( iindwk ), pivmin, spdiam,in, iinfo )
                          end do
                          if((wbegin+newlst-1<dol).or.(wbegin+newfst-1>dou)) then
                             ...
                             ! tree could be different from the one when nothing is
                             ! skipped. for this reason we skip at this place.
                             idone = idone + newlst - newfst + 1_ilp64
                             goto 139
                          endif
                          ! compute rrr of child cluster.
                          ! note that the new rrr is stored in z
                          ! stdlib_I64_slarrf needs lwork = 2*n
                          call stdlib_I64_slarrf( in, d( ibegin ), l( ibegin ),work(indld+ibegin-1),&
                          newfst, newlst, work(wbegin),wgap(wbegin), werr(wbegin),spdiam, lgap, &
                          rgap, pivmin, tau,z(ibegin, newftt),z(ibegin, newftt+1),work( indwrk ), &
                                    iinfo )
                          if( iinfo==0_ilp64 ) then
                             ! a new rrr for the cluster was found by stdlib_I64_slarrf
                             ! update shift and store it
                             ssigma = sigma + tau
                             z( iend, newftt+1 ) = ssigma
                             ! work() are the midpoints and werr() the semi-width
                             ...
                             do k = newfst, newlst
                                fudge =three*eps*abs(work(wbegin+k-1))
                                work( wbegin + k - 1_ilp64 ) =work( wbegin + k - 1_ilp64) - tau
                                fudge = fudge +four*eps*abs(work(wbegin+k-1))
                                ! fudge errors
                                werr( wbegin + k - 1_ilp64 ) =werr( wbegin + k - 1_ilp64 ) + fudge
                                ! gaps are not fudged. provided that werr is small
                                ! when eigenvalues are close, a zero gap indicates
                                ! that a new representation is needed for resolving
                                ! the cluster. a fudge could lead to a wrong decision
                          ...
                          tol = four * log(real(in,KIND=sp)) * eps
                          k = newfst
                          windex = wbegin + k - 1_ilp64
                          windmn = max(windex - 1_ilp64,1_ilp64)
                          windpl = min(windex + 1_ilp64,m)
                          lambda = work( windex )
                          done = done + 1_ilp64
                          ! check if eigenvector computation is to be skipped
                          if((windex<dol).or.(windex>dou)) then
                          ...
                          if(needbs) then
                             ...
                             itmp1 = iwork( iindr+windex )
                             offset = indexw( wbegin ) - 1_ilp64
                             call stdlib_I64_slarrb( in, d(ibegin),work(indlld+ibegin-1),indeig,&
                             indeig,zero, two*eps, offset,work(wbegin),wgap(wbegin),werr(wbegin),&
                                       work( indwrk ),iwork( iindwk ), pivmin, spdiam,itmp1, iinfo )
                             if( iinfo/=0_ilp64 ) then
                                info = -3_ilp64
                             ...
                             ! reset twist index from inaccurate lambda to
                             ! force computation of true mingma
                             iwork( iindr+windex ) = 0_ilp64
                          endif
                          ! given lambda, compute the eigenvector.
                          call stdlib_I64_slar1v( in, 1_ilp64, in, lambda, d( ibegin ),l( ibegin ), work(&
                          indld+ibegin-1),work(indlld+ibegin-1),pivmin, gaptol, z( ibegin, windex &
                          ),.not.usedbs, negcnt, ztz, mingma,iwork( iindr+windex ), isuppz( &
                                    2_ilp64*windex-1 ),nrminv, resid, rqcorr, work( indwrk ) )
                          if(iter == 0_ilp64) then
                             bstres = resid
                             bstw = lambda
                          elseif(resid<bstres) then
                             bstres = resid
                          ...
                          isupmn = min(isupmn,isuppz( 2_ilp64*windex-1 ))
                          isupmx = max(isupmx,isuppz( 2_ilp64*windex ))
                          iter = iter + 1_ilp64
                          ! sin alpha <= |resid|/gap
                          ! note that both the residual and the gap are
                          ! proportional to the matrix, so ||t|| doesn't play
             ...
             if(usedrq .and. usedbs .and.bstres<=resid) then
                             ...
                             if (stp2ii) then
                                ! improve error angle by second step
                                call stdlib_I64_slar1v( in, 1_ilp64, in, lambda,d( ibegin ), l( ibegin ),&
                                work(indld+ibegin-1),work(indlld+ibegin-1),pivmin, gaptol, z( &
                                ibegin, windex ),.not.usedbs, negcnt, ztz, mingma,iwork( iindr+&
                                windex ),isuppz( 2_ilp64*windex-1 ),nrminv, resid, rqcorr, work( indwrk &
                                          ) )
                             endif
                             work( windex ) = lambda
                          end if
                          ! compute fp-vector support w.r.t. whole matrix
                          isuppz( 2_ilp64*windex-1 ) = isuppz( 2_ilp64*windex-1 )+oldien
                          isuppz( 2_ilp64*windex ) = isuppz( 2_ilp64*windex )+oldien
                          zfrom = isuppz( 2_ilp64*windex-1 )
                          zto = isuppz( 2_ilp64*windex )
                          isupmn = isupmn + oldien
                          isupmx = isupmx + oldien
                          ! ensure vector is ok if support in the rqi has changed
                          if(isupmn<zfrom) then
                          ...
                          call stdlib_I64_sscal( zto-zfrom+1, nrminv,z( zfrom, windex ), 1_ilp64 )
                          125 continue
                          ! update w
                          w( windex ) = lambda+sigma
                          ! recompute the gaps on the left and right
     ...
     !! The input eigenvalues should have been computed by DLARRE.
        ...
        ! =====================================================================
           ...
           ! the width of the part of z that is used
           zusedw = zusedu - zusedl + 1_ilp64
           call stdlib_I64_dlaset( 'FULL', n, zusedw, zero, zero,z(1_ilp64,zusedl), ldz )
           eps = stdlib_I64_dlamch( 'PRECISION' )
           rqtol = two * eps
           ! set expert flags for standard code.
           tryrqc = .true.
           ...
           loop_170: do jblk = 1, iblock( m )
              ...
              ! high relative accuracy is required for the computation of the
              ! corresponding eigenvectors.
              call stdlib_I64_dcopy( im, w( wbegin ), 1_ilp64,work( wbegin ), 1_ilp64 )
              ! we store in w the eigenvalue approximations w.r.t. the original
              ! matrix t.
              do i=1,im
                 w(wbegin+i-1) = w(wbegin+i-1)+sigma
              ...
              if( idone<im ) then
                 ...
                 loop_150: do i = 1, oldncl
                    ...
                    if( ndepth>0_ilp64 ) then
                       ...
                       call stdlib_I64_dcopy( in, z( ibegin, j ), 1_ilp64, d( ibegin ), 1_ilp64 )
                       call stdlib_I64_dcopy( in-1, z( ibegin, j+1 ), 1_ilp64, l( ibegin ),1_ilp64 )
                       sigma = z( iend, j+1 )
                       ! set the corresponding entries in z to zero
                       call stdlib_I64_dlaset( 'FULL', in, 2_ilp64, zero, zero,z( ibegin, j), ldz )
                    end if
                    ! compute dl and dll of current rrr
                    do j = ibegin, iend-1
                       tmp = d( j )*l( j )
                    ...
                    if( ndepth>0_ilp64 ) then
                       ...
                       ! perform limited bisection (if necessary) to get approximate
                       ! eigenvalues to the precision needed.
                       call stdlib_I64_dlarrb( in, d( ibegin ),work(indlld+ibegin-1),p, q, rtol1, &
                       rtol2, offset,work(wbegin),wgap(wbegin),werr(wbegin),work( indwrk ), iwork(&
                                  iindwk ),pivmin, spdiam, in, iinfo )
                       if( iinfo/=0_ilp64 ) then
                          info = -1_ilp64
                       ...
                       ! of wgap, the entries of work might stem from rrrs with
                       ! different shifts. the gaps from wbegin-1+oldfst to
                       ! wbegin-1+oldlst are correctly computed in stdlib_I64_dlarrb.
                       ! however, we only allow the gaps to become greater since
                       ! this is what should happen when we decrease werr
                       if( oldfst>1_ilp64) then
                          wgap( wbegin+oldfst-2 ) =max(wgap(wbegin+oldfst-2),w(wbegin+oldfst-1)-&
                    ...
                    loop_140: do j = oldfst, oldlst
                       ...
                       if( newsiz>1_ilp64) then
                          ...
                          do k =1,2
                             ...
                             offset = indexw( wbegin ) - 1_ilp64
                             call stdlib_I64_dlarrb( in, d(ibegin),work( indlld+ibegin-1 ),p,p,rqtol, &
                             rqtol, offset,work(wbegin),wgap(wbegin),werr(wbegin),work( indwrk ),&
                                       iwork( iindwk ), pivmin, spdiam,in, iinfo )
                          end do
                          if((wbegin+newlst-1<dol).or.(wbegin+newfst-1>dou)) then
                          ...
                          ! compute rrr of child cluster.
                          ! note that the new rrr is stored in z
                          ! stdlib_I64_dlarrf needs lwork = 2*n
                          call stdlib_I64_dlarrf( in, d( ibegin ), l( ibegin ),work(indld+ibegin-1),&
                          newfst, newlst, work(wbegin),wgap(wbegin), werr(wbegin),spdiam, lgap, &
                          rgap, pivmin, tau,z(ibegin, newftt),z(ibegin, newftt+1),work( indwrk ), &
                                    iinfo )
                          if( iinfo==0_ilp64 ) then
                             ! a new rrr for the cluster was found by stdlib_I64_dlarrf
                             ! update shift and store it
                             ssigma = sigma + tau
                             z( iend, newftt+1 ) = ssigma
                             ! work() are the midpoints and werr() the semi-width
                          ...
                          if(needbs) then
                             ...
                             itmp1 = iwork( iindr+windex )
                             offset = indexw( wbegin ) - 1_ilp64
                             call stdlib_I64_dlarrb( in, d(ibegin),work(indlld+ibegin-1),indeig,&
                             indeig,zero, two*eps, offset,work(wbegin),wgap(wbegin),werr(wbegin),&
                                       work( indwrk ),iwork( iindwk ), pivmin, spdiam,itmp1, iinfo )
                             if( iinfo/=0_ilp64 ) then
                                info = -3_ilp64
                          ...
                          ! given lambda, compute the eigenvector.
                          call stdlib_I64_dlar1v( in, 1_ilp64, in, lambda, d( ibegin ),l( ibegin ), work(&
                          indld+ibegin-1),work(indlld+ibegin-1),pivmin, gaptol, z( ibegin, windex &
                          ),.not.usedbs, negcnt, ztz, mingma,iwork( iindr+windex ), isuppz( &
                                    2_ilp64*windex-1 ),nrminv, resid, rqcorr, work( indwrk ) )
                          if(iter == 0_ilp64) then
             ...
             if(usedrq .and. usedbs .and.bstres<=resid) then
                             ...
                             if (stp2ii) then
                                ! improve error angle by second step
                                call stdlib_I64_dlar1v( in, 1_ilp64, in, lambda,d( ibegin ), l( ibegin ),&
                                work(indld+ibegin-1),work(indlld+ibegin-1),pivmin, gaptol, z( &
                                ibegin, windex ),.not.usedbs, negcnt, ztz, mingma,iwork( iindr+&
                                windex ),isuppz( 2_ilp64*windex-1 ),nrminv, resid, rqcorr, work( indwrk &
                                          ) )
                          ...
                          call stdlib_I64_dscal( zto-zfrom+1, nrminv,z( zfrom, windex ), 1_ilp64 )
                          125 continue
                          ! update w
                          w( windex ) = lambda+sigma
                          ! recompute the gaps on the left and right
     ...
     !! The input eigenvalues should have been computed by SLARRE.
        ...
        ! =====================================================================
           ...
           indld = n+1
           indlld= 2_ilp64*n+1
           indin1 = 3_ilp64*n + 1_ilp64
           indin2 = 4_ilp64*n + 1_ilp64
           indwrk = 5_ilp64*n + 1_ilp64
           minwsize = 12_ilp64 * n
           do i= 1,minwsize
              work( i ) = zero
           end do
           ...
           ! the width of the part of z that is used
           zusedw = zusedu - zusedl + 1_ilp64
           call stdlib_I64_claset( 'FULL', n, zusedw, czero, czero,z(1_ilp64,zusedl), ldz )
           eps = stdlib_I64_slamch( 'PRECISION' )
           rqtol = two * eps
           ! set expert flags for standard code.
           tryrqc = .true.
           ...
           loop_170: do jblk = 1, iblock( m )
              ...
              if( idone<im ) then
                 ...
                 loop_150: do i = 1, oldncl
                    ...
                    if( ndepth>0_ilp64 ) then
                       ...
                       sigma = real( z( iend, j+1 ),KIND=sp)
                       ! set the corresponding entries in z to zero
                       call stdlib_I64_claset( 'FULL', in, 2_ilp64, czero, czero,z( ibegin, j), ldz )
                    ...
                    loop_140: do j = oldfst, oldlst
                       ...
                       if( newsiz>1_ilp64) then
                          ...
                          ! in the complex case, stdlib_I64_slarrf cannot write
                          ! the new rrr directly into z and needs an intermediate
                          ! workspace
                          do k = 1, in-1
                             z( ibegin+k-1, newftt ) =cmplx( work( indin1+k-1 ), zero,KIND=sp)
                          ...
                          ! given lambda, compute the eigenvector.
                          call stdlib_I64_clar1v( in, 1_ilp64, in, lambda, d( ibegin ),l( ibegin ), work(&
                          indld+ibegin-1),work(indlld+ibegin-1),pivmin, gaptol, z( ibegin, windex &
                          ),.not.usedbs, negcnt, ztz, mingma,iwork( iindr+windex ), isuppz( &
                                    2_ilp64*windex-1 ),nrminv, resid, rqcorr, work( indwrk ) )
                          if(iter == 0_ilp64) then
             ...
             if(usedrq .and. usedbs .and.bstres<=resid) then
                             ...
                             if (stp2ii) then
                                ! improve error angle by second step
                                call stdlib_I64_clar1v( in, 1_ilp64, in, lambda,d( ibegin ), l( ibegin ),&
                                work(indld+ibegin-1),work(indlld+ibegin-1),pivmin, gaptol, z( &
                                ibegin, windex ),.not.usedbs, negcnt, ztz, mingma,iwork( iindr+&
                                windex ),isuppz( 2_ilp64*windex-1 ),nrminv, resid, rqcorr, work( indwrk &
                                          ) )
                          ...
                          call stdlib_I64_csscal( zto-zfrom+1, nrminv,z( zfrom, windex ), 1_ilp64 )
                          125 continue
                          ! update w
                          w( windex ) = lambda+sigma
                          ! recompute the gaps on the left and right
     ...
     !! The input eigenvalues should have been computed by DLARRE.
        ...
        ! =====================================================================
           ...
           ! the width of the part of z that is used
           zusedw = zusedu - zusedl + 1_ilp64
           call stdlib_I64_zlaset( 'FULL', n, zusedw, czero, czero,z(1_ilp64,zusedl), ldz )
           eps = stdlib_I64_dlamch( 'PRECISION' )
           rqtol = two * eps
           ! set expert flags for standard code.
           tryrqc = .true.
           ...
           loop_170: do jblk = 1, iblock( m )
              ...
              if( idone<im ) then
                 ...
                 loop_150: do i = 1, oldncl
                    ...
                    if( ndepth>0_ilp64 ) then
                       ...
                       sigma = real( z( iend, j+1 ),KIND=dp)
                       ! set the corresponding entries in z to zero
                       call stdlib_I64_zlaset( 'FULL', in, 2_ilp64, czero, czero,z( ibegin, j), ldz )
                    ...
                    loop_140: do j = oldfst, oldlst
                       ...
                       if( newsiz>1_ilp64) then
                          ...
                          ! in the complex case, stdlib_I64_dlarrf cannot write
                          ! the new rrr directly into z and needs an intermediate
                          ! workspace
                          do k = 1, in-1
                             z( ibegin+k-1, newftt ) =cmplx( work( indin1+k-1 ), zero,KIND=dp)
                          ...
                          ! given lambda, compute the eigenvector.
                          call stdlib_I64_zlar1v( in, 1_ilp64, in, lambda, d( ibegin ),l( ibegin ), work(&
                          indld+ibegin-1),work(indlld+ibegin-1),pivmin, gaptol, z( ibegin, windex &
                          ),.not.usedbs, negcnt, ztz, mingma,iwork( iindr+windex ), isuppz( &
                                    2_ilp64*windex-1 ),nrminv, resid, rqcorr, work( indwrk ) )
                          if(iter == 0_ilp64) then
             ...
             if(usedrq .and. usedbs .and.bstres<=resid) then
                             ...
                             if (stp2ii) then
                                ! improve error angle by second step
                                call stdlib_I64_zlar1v( in, 1_ilp64, in, lambda,d( ibegin ), l( ibegin ),&
                                work(indld+ibegin-1),work(indlld+ibegin-1),pivmin, gaptol, z( &
                                ibegin, windex ),.not.usedbs, negcnt, ztz, mingma,iwork( iindr+&
                                windex ),isuppz( 2_ilp64*windex-1 ),nrminv, resid, rqcorr, work( indwrk &
                                          ) )
                          ...
                          call stdlib_I64_zdscal( zto-zfrom+1, nrminv,z( zfrom, windex ), 1_ilp64 )
                          125 continue
                          ! update w
                          w( windex ) = lambda+sigma
                          ! recompute the gaps on the left and right
     ...
     !! the stationary and the bottom part of the progressive transform.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           logical(lk) :: sawnan1, sawnan2
           integer(ilp64) :: i, indlpl, indp, inds, indumn, neg1, neg2, r1, r2
           real(sp) :: dminus, dplus, eps, s, tmp
           ! Intrinsic Functions 
           ! Executable Statements 
           eps = stdlib_I64_slamch( 'PRECISION' )
           ...
           ! storage for uminus
           indumn = n
           inds = 2_ilp64*n + 1_ilp64
           indp = 3_ilp64*n + 1_ilp64
           if( b1==1_ilp64 ) then
              work( inds ) = zero
           else
              work( inds+b1-1 ) = lld( b1-1 )
           ...
           ! diagonal element of the inverse
           mingma = work( inds+r1-1 ) + work( indp+r1-1 )
           if( mingma<zero ) neg1 = neg1 + 1_ilp64
           if( wantnc ) then
              negcnt = neg1 + neg2
           else
              negcnt = -1_ilp64
