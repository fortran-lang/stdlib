  ...
  contains
     ...
     !! values.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           integer(ilp64) :: itmp1, itmp2, j, ji, jit, jp, kf, kfnew, kl, klnew
           real(sp) :: tmp1, tmp2
           ! Intrinsic Functions 
           ! Executable Statements 
           ! check for errors
           ...
           ! initialize nab
           if( ijob==1_ilp64 ) then
              ! compute the number of eigenvalues in the initial intervals.
              mout = 0_ilp64
              do ji = 1, minp
                 do jp = 1, 2
                    tmp1 = d( 1_ilp64 ) - ab( ji, jp )
                    if( abs( tmp1 )<pivmin )tmp1 = -pivmin
                    nab( ji, jp ) = 0_ilp64
                    if( tmp1<=zero )nab( ji, jp ) = 1_ilp64
                    do j = 2, n
                       tmp1 = d( j ) - e2( j-1 ) / tmp1 - ab( ji, jp )
                       if( abs( tmp1 )<pivmin )tmp1 = -pivmin
                       if( tmp1<=zero )nab( ji, jp ) = nab( ji, jp ) + 1_ilp64
                    end do
                 end do
                 mout = mout + nab( ji, 2_ilp64 ) - nab( ji, 1_ilp64 )
              end do
              return
           end if
           ! initialize for loop
           ...
           ! if ijob=2, initialize c.
           ! if ijob=3, use the user-supplied starting point.
           if( ijob==2_ilp64 ) then
              do ji = 1, minp
                 c( ji ) = half*( ab( ji, 1_ilp64 )+ab( ji, 2_ilp64 ) )
              end do
           end if
           ! iteration loop
           loop_130: do jit = 1, nitmax
              ! loop over intervals
              if( kl-kf+1>=nbmin .and. nbmin>0_ilp64 ) then
                 ! begin of parallel version of the loop
                 do ji = kf, kl
                    ! compute n(c), the number of eigenvalues less than c
                    work( ji ) = d( 1_ilp64 ) - c( ji )
                    iwork( ji ) = 0_ilp64
                    if( work( ji )<=pivmin ) then
                       iwork( ji ) = 1_ilp64
                       work( ji ) = min( work( ji ), -pivmin )
                 ...
                 if( ijob<=2_ilp64 ) then
                    ...
                    loop_70: do ji = kf, kl
                       ! insure that n(w) is monotone
                       iwork( ji ) = min( nab( ji, 2_ilp64 ),max( nab( ji, 1_ilp64 ), iwork( ji ) ) )
                       ! update the queue -- add intervals if both halves
                       ! contain eigenvalues.
                       if( iwork( ji )==nab( ji, 2_ilp64 ) ) then
                          ! no eigenvalue in the upper interval:
                          ! just use the lower interval.
                          ab( ji, 2_ilp64 ) = c( ji )
                       else if( iwork( ji )==nab( ji, 1_ilp64 ) ) then
                          ! no eigenvalue in the lower interval:
                          ! just use the upper interval.
                          ab( ji, 1_ilp64 ) = c( ji )
                       else
                          ...
                          if( klnew<=mmax ) then
                             ! eigenvalue in both intervals -- add upper to
                             ! queue.
                             ab( klnew, 2_ilp64 ) = ab( ji, 2_ilp64 )
                             nab( klnew, 2_ilp64 ) = nab( ji, 2_ilp64 )
                             ab( klnew, 1_ilp64 ) = c( ji )
                             nab( klnew, 1_ilp64 ) = iwork( ji )
                             ab( ji, 2_ilp64 ) = c( ji )
                             nab( ji, 2_ilp64 ) = iwork( ji )
                          else
                             info = mmax + 1_ilp64
                          end if
                       end if
                 ...
                 loop_100: do ji = kf, kl
                    ...
                    if( ijob<=2_ilp64 ) then
                       ! ijob=2: choose all intervals containing eigenvalues.
                       ! insure that n(w) is monotone
                       itmp1 = min( nab( ji, 2_ilp64 ),max( nab( ji, 1_ilp64 ), itmp1 ) )
                       ! update the queue -- add intervals if both halves
                       ! contain eigenvalues.
                       if( itmp1==nab( ji, 2_ilp64 ) ) then
                          ! no eigenvalue in the upper interval:
                          ! just use the lower interval.
                          ab( ji, 2_ilp64 ) = tmp1
                       else if( itmp1==nab( ji, 1_ilp64 ) ) then
                          ! no eigenvalue in the lower interval:
                          ! just use the upper interval.
                          ab( ji, 1_ilp64 ) = tmp1
                       else if( klnew<mmax ) then
                          ...
                          ab( klnew, 2_ilp64 ) = ab( ji, 2_ilp64 )
                          nab( klnew, 2_ilp64 ) = nab( ji, 2_ilp64 )
                          ab( klnew, 1_ilp64 ) = tmp1
                          nab( klnew, 1_ilp64 ) = itmp1
                          ab( ji, 2_ilp64 ) = tmp1
                          nab( ji, 2_ilp64 ) = itmp1
                       else
                          info = mmax + 1_ilp64
                          return
                       end if
              ...
              kfnew = kf
              loop_110: do ji = kf, kl
                 tmp1 = abs( ab( ji, 2_ilp64 )-ab( ji, 1_ilp64 ) )
                 tmp2 = max( abs( ab( ji, 2_ilp64 ) ), abs( ab( ji, 1_ilp64 ) ) )
                 if( tmp1<max( abstol, pivmin, reltol*tmp2 ) .or.nab( ji, 1_ilp64 )>=nab( ji, 2_ilp64 ) ) &
                           then
                    ! converged -- swap with position kfnew,
                                 ! then increment kfnew
                    if( ji>kfnew ) then
                       tmp1 = ab( ji, 1_ilp64 )
                       tmp2 = ab( ji, 2_ilp64 )
                       itmp1 = nab( ji, 1_ilp64 )
                       itmp2 = nab( ji, 2_ilp64 )
                       ab( ji, 1_ilp64 ) = ab( kfnew, 1_ilp64 )
                       ab( ji, 2_ilp64 ) = ab( kfnew, 2_ilp64 )
                       nab( ji, 1_ilp64 ) = nab( kfnew, 1_ilp64 )
                       nab( ji, 2_ilp64 ) = nab( kfnew, 2_ilp64 )
                       ab( kfnew, 1_ilp64 ) = tmp1
                       ab( kfnew, 2_ilp64 ) = tmp2
                       nab( kfnew, 1_ilp64 ) = itmp1
                       nab( kfnew, 2_ilp64 ) = itmp2
     ...
     !! (Tech report version in LAWN 172 with the same title.)
        ...
        ! =====================================================================
           ! Parameters 
           integer(ilp64), parameter :: blklen = 128_ilp64
           ...
           ! Local Scalars 
           integer(ilp64) :: bj, j, neg1, neg2, negcnt
           real(sp) :: bsav, dminus, dplus, gamma, p, t, tmp
           logical(lk) :: sawnan
           ! Intrinsic Functions 
           ! Executable Statements 
           ...
           loop_210: do bj = 1, r-1, blklen
              ...
              do j = bj, min(bj+blklen-1, r-1)
                 dplus = d( j ) + t
                 if( dplus<zero ) neg1 = neg1 + 1_ilp64
                 tmp = t / dplus
                 t = tmp * lld( j ) - sigma
              end do
              sawnan = stdlib_I64_sisnan( t )
           ! run a slower version of the above loop if a nan is detected.
           ! a nan should occur only with a zero pivot after an infinite
           ! pivot.  in that case, substituting 1 for t/dplus is the
           ! correct limit.
              if( sawnan ) then
                 ...
                 do j = bj, min(bj+blklen-1, r-1)
                    ...
                    if( dplus<zero ) neg1 = neg1 + 1_ilp64
                    tmp = t / dplus
                    if (stdlib_I64_sisnan(tmp)) tmp = one
                    t = tmp * lld(j) - sigma
                 end do
              end if
              negcnt = negcnt + neg1
           ...
           do bj = n-1, r, -blklen
              ...
              do j = bj, max(bj-blklen+1, r), -1
                 dminus = lld( j ) + p
                 if( dminus<zero ) neg2 = neg2 + 1_ilp64
                 tmp = p / dminus
                 p = tmp * d( j ) - sigma
              end do
              sawnan = stdlib_I64_sisnan( p )
           ! as above, run a slower version that substitutes 1 for inf/inf.
              if( sawnan ) then
                 neg2 = 0_ilp64
                 p = bsav
           ...
           ! correct limit.
              if( sawnan ) then
                 ...
                 do j = bj, min(bj+blklen-1, r-1)
                    ...
                    if( dplus<zero ) neg1 = neg1 + 1_ilp64
                    tmp = t / dplus
                    if (stdlib_I64_disnan(tmp)) tmp = one
                    t = tmp * lld(j) - sigma
                 end do
              end if
              negcnt = negcnt + neg1
     ...
     !! symmetric tridiagonal matrix using the divide and conquer method.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           integer(ilp64) :: curlvl, curprb, curr, i, igivcl, igivnm, igivpt, indxq, iperm, iprmpt, &
           iq, iqptr, iwrem, j, k, lgn, matsiz, msd2, smlsiz, smm1, spm1, spm2, submat, subpbs, &
                     tlvls
           real(sp) :: temp
           ! Intrinsic Functions 
           ...
           ! test the input parameters.
           info = 0_ilp64
           if( icompq<0_ilp64 .or. icompq>2_ilp64 ) then
              info = -1_ilp64
           else if( ( icompq==1_ilp64 ) .and. ( qsiz<max( 0_ilp64, n ) ) ) then
              info = -2_ilp64
           else if( n<0_ilp64 ) then
              info = -3_ilp64
           else if( ldq<max( 1_ilp64, n ) ) then
              info = -7_ilp64
           else if( ldqs<max( 1_ilp64, n ) ) then
              info = -9_ilp64
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SLAED0', -info )
              return
           end if
           ! quick return if possible
           if( n==0 )return
           smlsiz = stdlib_I64_ilaenv( 9_ilp64, 'SLAED0', ' ', 0_ilp64, 0_ilp64, 0_ilp64, 0_ilp64 )
           ! determine the size and placement of the submatrices, and save in
           ! the leading elements of iwork.
           iwork( 1_ilp64 ) = n
           subpbs = 1_ilp64
           ...
           if( iwork( subpbs )>smlsiz ) then
              do j = subpbs, 1, -1
                 iwork( 2_ilp64*j ) = ( iwork( j )+1_ilp64 ) / 2_ilp64
                 iwork( 2_ilp64*j-1 ) = iwork( j ) / 2_ilp64
              end do
              tlvls = tlvls + 1_ilp64
              subpbs = 2_ilp64*subpbs
              go to 10
           end if
           do j = 2, subpbs
              iwork( j ) = iwork( j ) + iwork( j-1 )
           ...
           spm1 = subpbs - 1_ilp64
           do i = 1, spm1
              submat = iwork( i ) + 1_ilp64
              smm1 = submat - 1_ilp64
              d( smm1 ) = d( smm1 ) - abs( e( smm1 ) )
              d( submat ) = d( submat ) - abs( e( smm1 ) )
           end do
           indxq = 4_ilp64*n + 3_ilp64
           if( icompq/=2_ilp64 ) then
              ! set up workspaces for eigenvalues only/accumulate new vectors
              ! routine
              temp = log( real( n,KIND=sp) ) / log( two )
              lgn = int( temp,KIND=ilp64)
              if( 2_ilp64**lgn<n )lgn = lgn + 1_ilp64
              if( 2_ilp64**lgn<n )lgn = lgn + 1_ilp64
              iprmpt = indxq + n + 1_ilp64
              iperm = iprmpt + n*lgn
              iqptr = iperm + n*lgn
              igivpt = iqptr + n + 2_ilp64
              igivcl = igivpt + n*lgn
              igivnm = 1_ilp64
              iq = igivnm + 2_ilp64*n*lgn
              iwrem = iq + n**2_ilp64 + 1_ilp64
              ! initialize pointers
              do i = 0, subpbs
                 iwork( iprmpt+i ) = 1_ilp64
                 iwork( igivpt+i ) = 1_ilp64
              end do
              iwork( iqptr ) = 1_ilp64
           end if
           ! solve each submatrix eigenproblem at the bottom of the divide and
           ...
           loop_70: do i = 0, spm1
              if( i==0_ilp64 ) then
                 submat = 1_ilp64
                 matsiz = iwork( 1_ilp64 )
              else
                 submat = iwork( i ) + 1_ilp64
                 matsiz = iwork( i+1 ) - iwork( i )
              end if
              if( icompq==2_ilp64 ) then
                 call stdlib_I64_ssteqr( 'I', matsiz, d( submat ), e( submat ),q( submat, submat ), &
                           ldq, work, info )
                 if( info/=0 )go to 130
              else
                 call stdlib_I64_ssteqr( 'I', matsiz, d( submat ), e( submat ),work( iq-1+iwork( &
                           iqptr+curr ) ), matsiz, work,info )
                 if( info/=0 )go to 130
                 if( icompq==1_ilp64 ) then
                    call stdlib_I64_sgemm( 'N', 'N', qsiz, matsiz, matsiz, one,q( 1_ilp64, submat ), ldq, &
                    work( iq-1+iwork( iqptr+curr ) ), matsiz, zero, qstore( 1_ilp64, submat ),ldqs )
                              
                 end if
                 iwork( iqptr+curr+1 ) = iwork( iqptr+curr ) + matsiz**2_ilp64
                 curr = curr + 1_ilp64
              end if
              k = 1_ilp64
              do j = submat, iwork( i+1 )
           ...
           curlvl = 1_ilp64
           80 continue
           if( subpbs>1_ilp64 ) then
              spm2 = subpbs - 2_ilp64
              loop_90: do i = 0, spm2, 2
                 if( i==0_ilp64 ) then
                    submat = 1_ilp64
                    matsiz = iwork( 2_ilp64 )
                    msd2 = iwork( 1_ilp64 )
                    curprb = 0_ilp64
                 else
                    submat = iwork( i ) + 1_ilp64
                    matsiz = iwork( i+2 ) - iwork( i )
                    msd2 = matsiz / 2_ilp64
                    curprb = curprb + 1_ilp64
                 end if
           ! merge lower order eigensystems (of size msd2 and matsiz - msd2)
           ! into an eigensystem of size matsiz.
           ! stdlib_I64_slaed1 is used only for the full eigensystem of a tridiagonal
           ! matrix.
           ! stdlib_I64_slaed7 handles the cases in which eigenvalues only or eigenvalues
           ! and eigenvectors of a full symmetric matrix (which was reduced to
           ! tridiagonal form) are desired.
                 if( icompq==2_ilp64 ) then
                    call stdlib_I64_slaed1( matsiz, d( submat ), q( submat, submat ),ldq, iwork( &
                    indxq+submat ),e( submat+msd2-1 ), msd2, work,iwork( subpbs+1 ), info )
                              
                 else
                    call stdlib_I64_slaed7( icompq, matsiz, qsiz, tlvls, curlvl, curprb,d( submat ), &
                    qstore( 1_ilp64, submat ), ldqs,iwork( indxq+submat ), e( submat+msd2-1 ),msd2, &
                    work( iq ), iwork( iqptr ),iwork( iprmpt ), iwork( iperm ),iwork( igivpt ), &
                    iwork( igivcl ),work( igivnm ), work( iwrem ),iwork( subpbs+1 ), info )
                              
                 end if
                 if( info/=0 )go to 130
                 iwork( i / 2_ilp64+1 ) = iwork( i+2 )
              end do loop_90
              subpbs = subpbs / 2_ilp64
              curlvl = curlvl + 1_ilp64
              go to 80
           end if
           ! end while
           ! re-merge the eigenvalues/vectors which were deflated at the final
           ...
           if( icompq==1_ilp64 ) then
              do i = 1, n
                 j = iwork( indxq+i )
                 work( i ) = d( j )
                 call stdlib_I64_scopy( qsiz, qstore( 1_ilp64, j ), 1_ilp64, q( 1_ilp64, i ), 1_ilp64 )
              end do
              call stdlib_I64_scopy( n, work, 1_ilp64, d, 1_ilp64 )
           else if( icompq==2_ilp64 ) then
              do i = 1, n
                 j = iwork( indxq+i )
                 work( i ) = d( j )
                 call stdlib_I64_scopy( n, q( 1_ilp64, j ), 1_ilp64, work( n*i+1 ), 1_ilp64 )
              end do
              call stdlib_I64_scopy( n, work, 1_ilp64, d, 1_ilp64 )
              call stdlib_I64_slacpy( 'A', n, n, work( n+1 ), n, q, ldq )
           else
              do i = 1, n
                 j = iwork( indxq+i )
                 work( i ) = d( j )
           ...
           go to 140
           130 continue
           info = submat*( n+1 ) + submat + matsiz - 1_ilp64
           140 continue
           return
     ...
     !! symmetric tridiagonal matrix using the divide and conquer method.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DLAED0', -info )
              return
           end if
           ! quick return if possible
           if( n==0 )return
           smlsiz = stdlib_I64_ilaenv( 9_ilp64, 'DLAED0', ' ', 0_ilp64, 0_ilp64, 0_ilp64, 0_ilp64 )
           ! determine the size and placement of the submatrices, and save in
           ! the leading elements of iwork.
           iwork( 1_ilp64 ) = n
           subpbs = 1_ilp64
           ...
           loop_70: do i = 0, spm1
              ...
              if( icompq==2_ilp64 ) then
                 call stdlib_I64_dsteqr( 'I', matsiz, d( submat ), e( submat ),q( submat, submat ), &
                           ldq, work, info )
                 if( info/=0 )go to 130
              else
                 call stdlib_I64_dsteqr( 'I', matsiz, d( submat ), e( submat ),work( iq-1+iwork( &
                           iqptr+curr ) ), matsiz, work,info )
                 if( info/=0 )go to 130
                 if( icompq==1_ilp64 ) then
                    call stdlib_I64_dgemm( 'N', 'N', qsiz, matsiz, matsiz, one,q( 1_ilp64, submat ), ldq, &
                    work( iq-1+iwork( iqptr+curr ) ), matsiz, zero, qstore( 1_ilp64, submat ),ldqs )
           ...
           ! merge lower order eigensystems (of size msd2 and matsiz - msd2)
           ! into an eigensystem of size matsiz.
           ! stdlib_I64_dlaed1 is used only for the full eigensystem of a tridiagonal
           ! matrix.
           ! stdlib_I64_dlaed7 handles the cases in which eigenvalues only or eigenvalues
           ! and eigenvectors of a full symmetric matrix (which was reduced to
           ! tridiagonal form) are desired.
                 if( icompq==2_ilp64 ) then
                    call stdlib_I64_dlaed1( matsiz, d( submat ), q( submat, submat ),ldq, iwork( &
                    indxq+submat ),e( submat+msd2-1 ), msd2, work,iwork( subpbs+1 ), info )
                              
                 else
                    call stdlib_I64_dlaed7( icompq, matsiz, qsiz, tlvls, curlvl, curprb,d( submat ), &
                    qstore( 1_ilp64, submat ), ldqs,iwork( indxq+submat ), e( submat+msd2-1 ),msd2, &
                    work( iq ), iwork( iqptr ),iwork( iprmpt ), iwork( iperm ),iwork( igivpt ), &
                    iwork( igivcl ),work( igivnm ), work( iwrem ),iwork( subpbs+1 ), info )
           ...
           if( icompq==1_ilp64 ) then
              do i = 1, n
                 j = iwork( indxq+i )
                 work( i ) = d( j )
                 call stdlib_I64_dcopy( qsiz, qstore( 1_ilp64, j ), 1_ilp64, q( 1_ilp64, i ), 1_ilp64 )
              end do
              call stdlib_I64_dcopy( n, work, 1_ilp64, d, 1_ilp64 )
           else if( icompq==2_ilp64 ) then
              do i = 1, n
                 j = iwork( indxq+i )
                 work( i ) = d( j )
                 call stdlib_I64_dcopy( n, q( 1_ilp64, j ), 1_ilp64, work( n*i+1 ), 1_ilp64 )
              end do
              call stdlib_I64_dcopy( n, work, 1_ilp64, d, 1_ilp64 )
              call stdlib_I64_dlacpy( 'A', n, n, work( n+1 ), n, q, ldq )
           else
              do i = 1, n
                 j = iwork( indxq+i )
                 work( i ) = d( j )
     ...
     !! corresponding eigenvectors of the dense or band matrix.
        ...
        ! warning:      n could be as big as qsiz!
          ...
          ! $        then
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CLAED0', -info )
              return
           end if
           ! quick return if possible
           if( n==0 )return
           smlsiz = stdlib_I64_ilaenv( 9_ilp64, 'CLAED0', ' ', 0_ilp64, 0_ilp64, 0_ilp64, 0_ilp64 )
           ! determine the size and placement of the submatrices, and save in
           ! the leading elements of iwork.
           iwork( 1_ilp64 ) = n
           subpbs = 1_ilp64
           ...
           do i = 0, spm1
              ...
              ll = iq - 1_ilp64 + iwork( iqptr+curr )
              call stdlib_I64_ssteqr( 'I', matsiz, d( submat ), e( submat ),rwork( ll ), matsiz, &
                        rwork, info )
              call stdlib_I64_clacrm( qsiz, matsiz, q( 1_ilp64, submat ), ldq, rwork( ll ),matsiz, qstore( &
                        1_ilp64, submat ), ldqs,rwork( iwrem ) )
              iwork( iqptr+curr+1 ) = iwork( iqptr+curr ) + matsiz**2_ilp64
              curr = curr + 1_ilp64
              if( info>0_ilp64 ) then
                 info = submat*( n+1 ) + submat + matsiz - 1_ilp64
           ...
           ! merge lower order eigensystems (of size msd2 and matsiz - msd2)
           ! into an eigensystem of size matsiz.  stdlib_I64_claed7 handles the case
           ! when the eigenvectors of a full or band hermitian matrix (which
           ! was reduced to tridiagonal form) are desired.
           ! i am free to use q as a valuable working space until loop 150.
                 call stdlib_I64_claed7( matsiz, msd2, qsiz, tlvls, curlvl, curprb,d( submat ), &
                 qstore( 1_ilp64, submat ), ldqs,e( submat+msd2-1 ), iwork( indxq+submat ),rwork( iq ), &
                 iwork( iqptr ), iwork( iprmpt ),iwork( iperm ), iwork( igivpt ),iwork( igivcl ), &
                           rwork( igivnm ),q( 1_ilp64, submat ), rwork( iwrem ),iwork( subpbs+1 ), info )
                 if( info>0_ilp64 ) then
                    info = submat*( n+1 ) + submat + matsiz - 1_ilp64
                    return
                 end if
           ...
           do i = 1, n
              j = iwork( indxq+i )
              rwork( i ) = d( j )
              call stdlib_I64_ccopy( qsiz, qstore( 1_ilp64, j ), 1_ilp64, q( 1_ilp64, i ), 1_ilp64 )
           end do
           call stdlib_I64_scopy( n, rwork, 1_ilp64, d, 1_ilp64 )
           return
     ...
     !! corresponding eigenvectors of the dense or band matrix.
        ...
        ! warning:      n could be as big as qsiz!
          ...
          ! $        then
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZLAED0', -info )
              return
           end if
           ! quick return if possible
           if( n==0 )return
           smlsiz = stdlib_I64_ilaenv( 9_ilp64, 'ZLAED0', ' ', 0_ilp64, 0_ilp64, 0_ilp64, 0_ilp64 )
           ! determine the size and placement of the submatrices, and save in
           ! the leading elements of iwork.
           iwork( 1_ilp64 ) = n
           subpbs = 1_ilp64
           ...
           do i = 0, spm1
              ...
              ll = iq - 1_ilp64 + iwork( iqptr+curr )
              call stdlib_I64_dsteqr( 'I', matsiz, d( submat ), e( submat ),rwork( ll ), matsiz, &
                        rwork, info )
              call stdlib_I64_zlacrm( qsiz, matsiz, q( 1_ilp64, submat ), ldq, rwork( ll ),matsiz, qstore( &
                        1_ilp64, submat ), ldqs,rwork( iwrem ) )
              iwork( iqptr+curr+1 ) = iwork( iqptr+curr ) + matsiz**2_ilp64
              curr = curr + 1_ilp64
              if( info>0_ilp64 ) then
           ...
           ! merge lower order eigensystems (of size msd2 and matsiz - msd2)
           ! into an eigensystem of size matsiz.  stdlib_I64_zlaed7 handles the case
           ! when the eigenvectors of a full or band hermitian matrix (which
           ! was reduced to tridiagonal form) are desired.
           ! i am free to use q as a valuable working space until loop 150.
                 call stdlib_I64_zlaed7( matsiz, msd2, qsiz, tlvls, curlvl, curprb,d( submat ), &
                 qstore( 1_ilp64, submat ), ldqs,e( submat+msd2-1 ), iwork( indxq+submat ),rwork( iq ), &
                 iwork( iqptr ), iwork( iprmpt ),iwork( iperm ), iwork( igivpt ),iwork( igivcl ), &
                           rwork( igivnm ),q( 1_ilp64, submat ), rwork( iwrem ),iwork( subpbs+1 ), info )
                 if( info>0_ilp64 ) then
           ...
           do i = 1, n
              j = iwork( indxq+i )
              rwork( i ) = d( j )
              call stdlib_I64_zcopy( qsiz, qstore( 1_ilp64, j ), 1_ilp64, q( 1_ilp64, i ), 1_ilp64 )
           end do
           call stdlib_I64_dcopy( n, rwork, 1_ilp64, d, 1_ilp64 )
           return
     ...
     !! the overall problem.
        ...
        ! =====================================================================
           ! Local Scalars 
           integer(ilp64) :: coltyp, cpp1, i, idlmda, indx, indxc, indxp, iq2, is, iw, iz, k, n1, &
                     n2
           ! Intrinsic Functions 
           ! Executable Statements 
           ! test the input parameters.
           ...
           if( n<0_ilp64 ) then
              ...
              info = -4_ilp64
           else if( min( 1_ilp64, n / 2_ilp64 )>cutpnt .or. ( n / 2_ilp64 )<cutpnt ) then
              info = -7_ilp64
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SLAED1', -info )
              return
           end if
           ! quick return if possible
           if( n==0 )return
           ! the following values are integer pointers which indicate
           ! the portion of the workspace
           ! used by a particular array in stdlib_I64_slaed2 and stdlib_I64_slaed3.
           iz = 1_ilp64
           idlmda = iz + n
           iw = idlmda + n
           iq2 = iw + n
           ...
           ! form the z-vector which consists of the last row of q_1 and the
           ! first row of q_2.
           call stdlib_I64_scopy( cutpnt, q( cutpnt, 1_ilp64 ), ldq, work( iz ), 1_ilp64 )
           cpp1 = cutpnt + 1_ilp64
           call stdlib_I64_scopy( n-cutpnt, q( cpp1, cpp1 ), ldq, work( iz+cutpnt ), 1_ilp64 )
           ! deflate eigenvalues.
           call stdlib_I64_slaed2( k, n, cutpnt, d, q, ldq, indxq, rho, work( iz ),work( idlmda ), &
           work( iw ), work( iq2 ),iwork( indx ), iwork( indxc ), iwork( indxp ),iwork( coltyp ), &
                     info )
           if( info/=0 )go to 20
           ! solve secular equation.
           if( k/=0_ilp64 ) then
              is = ( iwork( coltyp )+iwork( coltyp+1 ) )*cutpnt +( iwork( coltyp+1 )+iwork( &
                        coltyp+2 ) )*( n-cutpnt ) + iq2
              call stdlib_I64_slaed3( k, n, cutpnt, d, q, ldq, rho, work( idlmda ),work( iq2 ), iwork(&
                         indxc ), iwork( coltyp ),work( iw ), work( is ), info )
              if( info/=0 )go to 20
           ! prepare the indxq sorting permutation.
              n1 = k
              n2 = n - k
              call stdlib_I64_slamrg( n1, n2, d, 1_ilp64, -1_ilp64, indxq )
           else
              do i = 1, n
                 indxq( i ) = i
              end do
     ...
     !! the overall problem.
        ...
        ! =====================================================================
           ! Local Scalars 
           integer(ilp64) :: coltyp, i, idlmda, indx, indxc, indxp, iq2, is, iw, iz, k, n1, n2, &
                     zpp1
           ! Intrinsic Functions 
           ! Executable Statements 
           ! test the input parameters.
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DLAED1', -info )
              return
           end if
           ! quick return if possible
           if( n==0 )return
           ! the following values are integer pointers which indicate
           ! the portion of the workspace
           ! used by a particular array in stdlib_I64_dlaed2 and stdlib_I64_dlaed3.
           iz = 1_ilp64
           idlmda = iz + n
           iw = idlmda + n
           iq2 = iw + n
           ...
           ! form the z-vector which consists of the last row of q_1 and the
           ! first row of q_2.
           call stdlib_I64_dcopy( cutpnt, q( cutpnt, 1_ilp64 ), ldq, work( iz ), 1_ilp64 )
           zpp1 = cutpnt + 1_ilp64
           call stdlib_I64_dcopy( n-cutpnt, q( zpp1, zpp1 ), ldq, work( iz+cutpnt ), 1_ilp64 )
           ! deflate eigenvalues.
           call stdlib_I64_dlaed2( k, n, cutpnt, d, q, ldq, indxq, rho, work( iz ),work( idlmda ), &
           work( iw ), work( iq2 ),iwork( indx ), iwork( indxc ), iwork( indxp ),iwork( coltyp ), &
                     info )
           if( info/=0 )go to 20
           ! solve secular equation.
           if( k/=0_ilp64 ) then
              is = ( iwork( coltyp )+iwork( coltyp+1 ) )*cutpnt +( iwork( coltyp+1 )+iwork( &
                        coltyp+2 ) )*( n-cutpnt ) + iq2
              call stdlib_I64_dlaed3( k, n, cutpnt, d, q, ldq, rho, work( idlmda ),work( iq2 ), iwork(&
                         indxc ), iwork( coltyp ),work( iw ), work( is ), info )
              if( info/=0 )go to 20
           ! prepare the indxq sorting permutation.
              n1 = k
              n2 = n - k
              call stdlib_I64_dlamrg( n1, n2, d, 1_ilp64, -1_ilp64, indxq )
           else
              do i = 1, n
                 indxq( i ) = i
              end do
     ...
     !! equation problem is reduced by one.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           integer(ilp64) :: ct, i, imax, iq1, iq2, j, jmax, js, k2, n1p1, n2, nj, pj
           real(sp) :: c, eps, s, t, tau, tol
           ! Intrinsic Functions 
           ! Executable Statements 
           ! test the input parameters.
           ...
           if( n<0_ilp64 ) then
              ...
              info = -6_ilp64
           else if( min( 1_ilp64, ( n / 2_ilp64 ) )>n1 .or. ( n / 2_ilp64 )<n1 ) then
              info = -3_ilp64
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SLAED2', -info )
              return
           end if
           ! quick return if possible
           if( n==0 )return
           ...
           n1p1 = n1 + 1_ilp64
           if( rho<zero ) then
              call stdlib_I64_sscal( n2, mone, z( n1p1 ), 1_ilp64 )
           end if
           ! normalize z so that norm(z) = 1.  since z is the concatenation of
           ! two normalized vectors, norm2(z) = sqrt(2).
           t = one / sqrt( two )
           call stdlib_I64_sscal( n, t, z, 1_ilp64 )
           ! rho = abs( norm(z)**2 * rho )
           rho = abs( two*rho )
           ! sort the eigenvalues into increasing order
           do i = n1p1, n
           ...
           call stdlib_I64_slamrg( n1, n2, dlamda, 1_ilp64, 1_ilp64, indxc )
           do i = 1, n
              indx( i ) = indxq( indxc( i ) )
           end do
           ! calculate the allowable deflation tolerance
           imax = stdlib_I64_isamax( n, z, 1_ilp64 )
           jmax = stdlib_I64_isamax( n, d, 1_ilp64 )
           ...
           if( rho*abs( z( imax ) )<=tol ) then
              ...
              do j = 1, n
                 i = indx( j )
                 call stdlib_I64_scopy( n, q( 1_ilp64, i ), 1_ilp64, q2( iq2 ), 1_ilp64 )
                 dlamda( j ) = d( i )
                 iq2 = iq2 + n
              end do
              call stdlib_I64_slacpy( 'A', n, n, q2, n, q, ldq )
              call stdlib_I64_scopy( n, dlamda, 1_ilp64, d, 1_ilp64 )
              go to 190
           end if
           ! if there are multiple eigenvalues then the problem deflates.  here
           ! the number of equal eigenvalues are found.  as each equal
           ...
           if( rho*abs( z( nj ) )<=tol ) then
              ...
              if( abs( t*c*s )<=tol ) then
                 ...
                 z( nj ) = tau
                 z( pj ) = zero
                 if( coltyp( nj )/=coltyp( pj ) )coltyp( nj ) = 2_ilp64
                 coltyp( pj ) = 4_ilp64
                 call stdlib_I64_srot( n, q( 1_ilp64, pj ), 1_ilp64, q( 1_ilp64, nj ), 1_ilp64, c, s )
                 t = d( pj )*c**2_ilp64 + d( nj )*s**2_ilp64
                 d( nj ) = d( pj )*s**2_ilp64 + d( nj )*c**2_ilp64
                 d( pj ) = t
                 k2 = k2 - 1_ilp64
                 i = 1_ilp64
                 90 continue
           ...
           ! psm(*) = position in submatrix (of types 1 through 4)
           psm( 1_ilp64 ) = 1_ilp64
           psm( 2_ilp64 ) = 1_ilp64 + ctot( 1_ilp64 )
           ...
           i = 1_ilp64
           iq1 = 1_ilp64
           iq2 = 1_ilp64 + ( ctot( 1_ilp64 )+ctot( 2_ilp64 ) )*n1
           do j = 1, ctot( 1 )
              js = indx( i )
              call stdlib_I64_scopy( n1, q( 1_ilp64, js ), 1_ilp64, q2( iq1 ), 1_ilp64 )
              z( i ) = d( js )
              i = i + 1_ilp64
              iq1 = iq1 + n1
           end do
           do j = 1, ctot( 2 )
              js = indx( i )
              call stdlib_I64_scopy( n1, q( 1_ilp64, js ), 1_ilp64, q2( iq1 ), 1_ilp64 )
              call stdlib_I64_scopy( n2, q( n1+1, js ), 1_ilp64, q2( iq2 ), 1_ilp64 )
              z( i ) = d( js )
              i = i + 1_ilp64
              iq1 = iq1 + n1
              iq2 = iq2 + n2
           ...
           do j = 1, ctot( 4 )
              js = indx( i )
              call stdlib_I64_scopy( n, q( 1_ilp64, js ), 1_ilp64, q2( iq2 ), 1_ilp64 )
              iq2 = iq2 + n
              z( i ) = d( js )
              i = i + 1_ilp64
           end do
           ...
           ! into the last n - k slots of d and q respectively.
           if( k<n ) then
              call stdlib_I64_slacpy( 'A', n, ctot( 4_ilp64 ), q2( iq1 ), n,q( 1_ilp64, k+1 ), ldq )
              call stdlib_I64_scopy( n-k, z( k+1 ), 1_ilp64, d( k+1 ), 1_ilp64 )
           end if
           ! copy ctot into coltyp for referencing in stdlib_I64_slaed3.
           do j = 1, 4
              coltyp( j ) = ctot( j )
           end do
           190 continue
     ...
     !! equation problem is reduced by one.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DLAED2', -info )
              return
           end if
           ! quick return if possible
           if( n==0 )return
           ...
           n1p1 = n1 + 1_ilp64
           if( rho<zero ) then
              call stdlib_I64_dscal( n2, mone, z( n1p1 ), 1_ilp64 )
           end if
           ! normalize z so that norm(z) = 1.  since z is the concatenation of
           ! two normalized vectors, norm2(z) = sqrt(2).
           t = one / sqrt( two )
           call stdlib_I64_dscal( n, t, z, 1_ilp64 )
           ! rho = abs( norm(z)**2 * rho )
           rho = abs( two*rho )
           ! sort the eigenvalues into increasing order
           do i = n1p1, n
           ...
           call stdlib_I64_dlamrg( n1, n2, dlamda, 1_ilp64, 1_ilp64, indxc )
           do i = 1, n
              indx( i ) = indxq( indxc( i ) )
           end do
           ! calculate the allowable deflation tolerance
           imax = stdlib_I64_idamax( n, z, 1_ilp64 )
           jmax = stdlib_I64_idamax( n, d, 1_ilp64 )
           ...
           if( rho*abs( z( imax ) )<=tol ) then
              ...
              do j = 1, n
                 i = indx( j )
                 call stdlib_I64_dcopy( n, q( 1_ilp64, i ), 1_ilp64, q2( iq2 ), 1_ilp64 )
                 dlamda( j ) = d( i )
                 iq2 = iq2 + n
              end do
              call stdlib_I64_dlacpy( 'A', n, n, q2, n, q, ldq )
              call stdlib_I64_dcopy( n, dlamda, 1_ilp64, d, 1_ilp64 )
              go to 190
           end if
           ! if there are multiple eigenvalues then the problem deflates.  here
           ! the number of equal eigenvalues are found.  as each equal
           ...
           if( rho*abs( z( nj ) )<=tol ) then
              ...
              if( abs( t*c*s )<=tol ) then
                 ...
                 if( coltyp( nj )/=coltyp( pj ) )coltyp( nj ) = 2_ilp64
                 coltyp( pj ) = 4_ilp64
                 call stdlib_I64_drot( n, q( 1_ilp64, pj ), 1_ilp64, q( 1_ilp64, nj ), 1_ilp64, c, s )
                 t = d( pj )*c**2_ilp64 + d( nj )*s**2_ilp64
                 d( nj ) = d( pj )*s**2_ilp64 + d( nj )*c**2_ilp64
                 d( pj ) = t
                 k2 = k2 - 1_ilp64
           ...
           do j = 1, ctot( 1 )
              js = indx( i )
              call stdlib_I64_dcopy( n1, q( 1_ilp64, js ), 1_ilp64, q2( iq1 ), 1_ilp64 )
              z( i ) = d( js )
              i = i + 1_ilp64
              iq1 = iq1 + n1
           end do
           do j = 1, ctot( 2 )
              js = indx( i )
              call stdlib_I64_dcopy( n1, q( 1_ilp64, js ), 1_ilp64, q2( iq1 ), 1_ilp64 )
              call stdlib_I64_dcopy( n2, q( n1+1, js ), 1_ilp64, q2( iq2 ), 1_ilp64 )
              z( i ) = d( js )
              i = i + 1_ilp64
              iq1 = iq1 + n1
              iq2 = iq2 + n2
           ...
           do j = 1, ctot( 4 )
              js = indx( i )
              call stdlib_I64_dcopy( n, q( 1_ilp64, js ), 1_ilp64, q2( iq2 ), 1_ilp64 )
              iq2 = iq2 + n
              z( i ) = d( js )
              i = i + 1_ilp64
           end do
           ...
           ! into the last n - k slots of d and q respectively.
           if( k<n ) then
              call stdlib_I64_dlacpy( 'A', n, ctot( 4_ilp64 ), q2( iq1 ), n,q( 1_ilp64, k+1 ), ldq )
              call stdlib_I64_dcopy( n-k, z( k+1 ), 1_ilp64, d( k+1 ), 1_ilp64 )
           end if
           ! copy ctot into coltyp for referencing in stdlib_I64_dlaed3.
           do j = 1, 4
              coltyp( j ) = ctot( j )
           end do
           190 continue
     ...
     !! without guard digits, but we know of none.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           integer(ilp64) :: i, ii, iq2, j, n12, n2, n23
           real(sp) :: temp
           ! Intrinsic Functions 
           ! Executable Statements 
           ! test the input parameters.
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SLAED3', -info )
              return
           end if
           ! quick return if possible
           if( k==0 )return
           ...
           ! this code.
           do i = 1, k
              dlamda( i ) = stdlib_I64_slamc3( dlamda( i ), dlamda( i ) ) - dlamda( i )
           end do
           do j = 1, k
              call stdlib_I64_slaed4( k, j, dlamda, w, q( 1_ilp64, j ), rho, d( j ), info )
              ! if the zero finder fails, the computation is terminated.
              if( info/=0 )go to 120
           end do
           if( k==1 )go to 110
           if( k==2_ilp64 ) then
              do j = 1, k
                 w( 1_ilp64 ) = q( 1_ilp64, j )
                 w( 2_ilp64 ) = q( 2_ilp64, j )
                 ii = indx( 1_ilp64 )
                 q( 1_ilp64, j ) = w( ii )
                 ii = indx( 2_ilp64 )
                 q( 2_ilp64, j ) = w( ii )
           ...
           ! compute updated w.
           call stdlib_I64_scopy( k, w, 1_ilp64, s, 1_ilp64 )
           ! initialize w(i) = q(i,i)
           call stdlib_I64_scopy( k, q, ldq+1, w, 1_ilp64 )
           do j = 1, k
              do i = 1, j - 1
                 w( i ) = w( i )*( q( i, j ) / ( dlamda( i )-dlamda( j ) ) )
              end do
           ...
           do j = 1, k
              ...
              temp = stdlib_I64_snrm2( k, s, 1_ilp64 )
              do i = 1, k
                 ii = indx( i )
                 q( i, j ) = s( ii ) / temp
              end do
           ...
           110 continue
           n2 = n - n1
           n12 = ctot( 1_ilp64 ) + ctot( 2_ilp64 )
           n23 = ctot( 2_ilp64 ) + ctot( 3_ilp64 )
           call stdlib_I64_slacpy( 'A', n23, k, q( ctot( 1_ilp64 )+1_ilp64, 1_ilp64 ), ldq, s, n23 )
           iq2 = n1*n12 + 1_ilp64
           if( n23/=0_ilp64 ) then
              call stdlib_I64_sgemm( 'N', 'N', n2, k, n23, one, q2( iq2 ), n2, s, n23,zero, q( n1+1, &
                        1_ilp64 ), ldq )
           else
              call stdlib_I64_slaset( 'A', n2, k, zero, zero, q( n1+1, 1_ilp64 ), ldq )
           end if
           call stdlib_I64_slacpy( 'A', n12, k, q, ldq, s, n12 )
           if( n12/=0_ilp64 ) then
              call stdlib_I64_sgemm( 'N', 'N', n1, k, n12, one, q2, n1, s, n12, zero, q,ldq )
           else
              call stdlib_I64_slaset( 'A', n1, k, zero, zero, q( 1_ilp64, 1_ilp64 ), ldq )
           end if
           120 continue
           return
     ...
     !! without guard digits, but we know of none.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DLAED3', -info )
              return
           end if
           ! quick return if possible
           if( k==0 )return
           ...
           ! this code.
           do i = 1, k
              dlamda( i ) = stdlib_I64_dlamc3( dlamda( i ), dlamda( i ) ) - dlamda( i )
           end do
           do j = 1, k
              call stdlib_I64_dlaed4( k, j, dlamda, w, q( 1_ilp64, j ), rho, d( j ), info )
              ! if the zero finder fails, the computation is terminated.
              if( info/=0 )go to 120
           end do
           if( k==1 )go to 110
           ...
           ! compute updated w.
           call stdlib_I64_dcopy( k, w, 1_ilp64, s, 1_ilp64 )
           ! initialize w(i) = q(i,i)
           call stdlib_I64_dcopy( k, q, ldq+1, w, 1_ilp64 )
           do j = 1, k
              do i = 1, j - 1
                 w( i ) = w( i )*( q( i, j ) / ( dlamda( i )-dlamda( j ) ) )
              end do
           ...
           do j = 1, k
              ...
              temp = stdlib_I64_dnrm2( k, s, 1_ilp64 )
              do i = 1, k
                 ii = indx( i )
                 q( i, j ) = s( ii ) / temp
              end do
           ...
           n12 = ctot( 1_ilp64 ) + ctot( 2_ilp64 )
           n23 = ctot( 2_ilp64 ) + ctot( 3_ilp64 )
           call stdlib_I64_dlacpy( 'A', n23, k, q( ctot( 1_ilp64 )+1_ilp64, 1_ilp64 ), ldq, s, n23 )
           iq2 = n1*n12 + 1_ilp64
           if( n23/=0_ilp64 ) then
              call stdlib_I64_dgemm( 'N', 'N', n2, k, n23, one, q2( iq2 ), n2, s, n23,zero, q( n1+1, &
                        1_ilp64 ), ldq )
           else
              call stdlib_I64_dlaset( 'A', n2, k, zero, zero, q( n1+1, 1_ilp64 ), ldq )
           end if
           call stdlib_I64_dlacpy( 'A', n12, k, q, ldq, s, n12 )
           if( n12/=0_ilp64 ) then
              call stdlib_I64_dgemm( 'N', 'N', n1, k, n12, one, q2, n1, s, n12, zero, q,ldq )
           else
              call stdlib_I64_dlaset( 'A', n1, k, zero, zero, q( 1_ilp64, 1_ilp64 ), ldq )
           end if
           120 continue
           return
     ...
     !! secular equation by simpler interpolating rational functions.
        ...
        ! =====================================================================
           ...
           if( n==1_ilp64 ) then
               ! presumably, i=1 upon entry
              dlam = d( 1_ilp64 ) + rho*z( 1_ilp64 )*z( 1_ilp64 )
              delta( 1_ilp64 ) = one
              return
           end if
           if( n==2_ilp64 ) then
              call stdlib_I64_slaed5( i, d, z, delta, rho, dlam )
              return
           end if
           ! compute machine epsilon
           eps = stdlib_I64_slamch( 'EPSILON' )
           ...
           if( i==n ) then
              ...
              if( .not.swtch3 ) then
                 ...
                 if( orgati ) then
                    ...
                    zz( 3_ilp64 ) = delta( iip1 )*delta( iip1 )*( ( dpsi-temp1 )+dphi )
                 else
                    temp1 = z( iip1 ) / delta( iip1 )
                    temp1 = temp1*temp1
                    c = temp - delta( iim1 )*( dpsi+dphi ) -( d( iip1 )-d( iim1 ) )*temp1
                    zz( 1_ilp64 ) = delta( iim1 )*delta( iim1 )*( dpsi+( dphi-temp1 ) )
                    zz( 3_ilp64 ) = z( iip1 )*z( iip1 )
                 end if
                 zz( 2_ilp64 ) = z( ii )*z( ii )
                 call stdlib_I64_slaed6( niter, orgati, c, delta( iim1 ), zz, w, eta,info )
                 if( info/=0 )go to 250
              end if
              ! note, eta should be positive if w is negative, and
              ! eta should be negative otherwise. however,
              ...
              loop_240: do niter = iter, maxit
                 ...
                 if( .not.swtch3 ) then
                    ...
                    if( swtch ) then
                       c = temp - delta( iim1 )*dpsi - delta( iip1 )*dphi
                       zz( 1_ilp64 ) = delta( iim1 )*delta( iim1 )*dpsi
                       zz( 3_ilp64 ) = delta( iip1 )*delta( iip1 )*dphi
                    else
                       if( orgati ) then
                          temp1 = z( iim1 ) / delta( iim1 )
                          temp1 = temp1*temp1
     ...
     !! secular equation by simpler interpolating rational functions.
        ...
        ! =====================================================================
           ...
           if( n==2_ilp64 ) then
              call stdlib_I64_dlaed5( i, d, z, delta, rho, dlam )
              return
           end if
           ! compute machine epsilon
           eps = stdlib_I64_dlamch( 'EPSILON' )
           ...
           if( i==n ) then
              ...
              if( .not.swtch3 ) then
                 ...
                 zz( 2_ilp64 ) = z( ii )*z( ii )
                 call stdlib_I64_dlaed6( niter, orgati, c, delta( iim1 ), zz, w, eta,info )
                 if( info/=0 )go to 250
              end if
              ! note, eta should be positive if w is negative, and
              ! eta should be negative otherwise. however,
     ...
     !! Z is one.
        ...
        ! =====================================================================
           ...
           if( i==1_ilp64 ) then
              w = one + two*rho*( z( 2_ilp64 )*z( 2_ilp64 )-z( 1_ilp64 )*z( 1_ilp64 ) ) / del
              if( w>zero ) then
                 b = del + rho*( z( 1_ilp64 )*z( 1_ilp64 )+z( 2_ilp64 )*z( 2_ilp64 ) )
                 c = rho*z( 1_ilp64 )*z( 1_ilp64 )*del
                 ! b > zero, always
                 tau = two*c / ( b+sqrt( abs( b*b-four*c ) ) )
                 dlam = d( 1_ilp64 ) + tau
                 delta( 1_ilp64 ) = -z( 1_ilp64 ) / tau
                 delta( 2_ilp64 ) = z( 2_ilp64 ) / ( del-tau )
              else
                 b = -del + rho*( z( 1_ilp64 )*z( 1_ilp64 )+z( 2_ilp64 )*z( 2_ilp64 ) )
                 c = rho*z( 2_ilp64 )*z( 2_ilp64 )*del
                 if( b>zero ) then
                    tau = -two*c / ( b+sqrt( b*b+four*c ) )
                 else
                    tau = ( b-sqrt( b*b+four*c ) ) / two
                 ...
                 dlam = d( 2_ilp64 ) + tau
                 delta( 1_ilp64 ) = -z( 1_ilp64 ) / ( del+tau )
                 delta( 2_ilp64 ) = -z( 2_ilp64 ) / tau
              end if
              temp = sqrt( delta( 1_ilp64 )*delta( 1_ilp64 )+delta( 2_ilp64 )*delta( 2_ilp64 ) )
              delta( 1_ilp64 ) = delta( 1_ilp64 ) / temp
              delta( 2_ilp64 ) = delta( 2_ilp64 ) / temp
           else
           ! now i=2
              b = -del + rho*( z( 1_ilp64 )*z( 1_ilp64 )+z( 2_ilp64 )*z( 2_ilp64 ) )
              c = rho*z( 2_ilp64 )*z( 2_ilp64 )*del
     ...
     !! in some extremely rare situations.
        ...
        ! =====================================================================
           ...
           ! Local Arrays 
           real(sp) :: dscale(3_ilp64), zscale(3_ilp64)
           ! Local Scalars 
           logical(lk) :: scale
           integer(ilp64) :: i, iter, niter
           real(sp) :: a, b, base, c, ddf, df, eps, erretm, eta, f, fc, sclfac, sclinv, small1, &
                     small2, sminv1, sminv2, temp, temp1, temp2, temp3, temp4, lbd, ubd
           ! Intrinsic Functions 
           ! Executable Statements 
           ...
           niter = 1_ilp64
           tau = zero
           if( kniter==2_ilp64 ) then
              if( orgati ) then
                 temp = ( d( 3_ilp64 )-d( 2_ilp64 ) ) / two
                 c = rho + z( 1_ilp64 ) / ( ( d( 1_ilp64 )-d( 2_ilp64 ) )-temp )
                 a = c*( d( 2_ilp64 )+d( 3_ilp64 ) ) + z( 2_ilp64 ) + z( 3_ilp64 )
                 b = c*d( 2_ilp64 )*d( 3_ilp64 ) + z( 2_ilp64 )*d( 3_ilp64 ) + z( 3_ilp64 )*d( 2_ilp64 )
              else
                 temp = ( d( 1_ilp64 )-d( 2_ilp64 ) ) / two
                 c = rho + z( 3_ilp64 ) / ( ( d( 3_ilp64 )-d( 2_ilp64 ) )-temp )
                 a = c*( d( 1_ilp64 )+d( 2_ilp64 ) ) + z( 1_ilp64 ) + z( 2_ilp64 )
                 b = c*d( 1_ilp64 )*d( 2_ilp64 ) + z( 1_ilp64 )*d( 2_ilp64 ) + z( 2_ilp64 )*d( 1_ilp64 )
              end if
              temp = max( abs( a ), abs( b ), abs( c ) )
              a = a / temp
              b = b / temp
              ...
              if( tau < lbd .or. tau > ubd )tau = ( lbd+ubd )/two
              if( d(1_ilp64)==tau .or. d(2_ilp64)==tau .or. d(3_ilp64)==tau ) then
                 tau = zero
              else
                 temp = finit + tau*z(1_ilp64)/( d(1_ilp64)*( d( 1_ilp64 )-tau ) ) +tau*z(2_ilp64)/( d(2_ilp64)*( d( 2_ilp64 )-tau&
                     & ) )&
                            +tau*z(3_ilp64)/( d(3_ilp64)*( d( 3_ilp64 )-tau ) )
                 if( temp <= zero )then
                    lbd = tau
                 else
                    ubd = tau
           ...
           ! others but recomputed at each call
           eps = stdlib_I64_slamch( 'EPSILON' )
           base = stdlib_I64_slamch( 'BASE' )
           small1 = base**( int( log( stdlib_I64_slamch( 'SAFMIN' ) ) / log( base ) /three,KIND=ilp64) )
           ...
           ! when computing 1/temp**3
           if( orgati ) then
              temp = min( abs( d( 2_ilp64 )-tau ), abs( d( 3_ilp64 )-tau ) )
           else
              temp = min( abs( d( 1_ilp64 )-tau ), abs( d( 2_ilp64 )-tau ) )
           end if
           scale = .false.
           if( temp<=small1 ) then
              scale = .true.
           ...
           loop_50: do niter = iter, maxit
              if( orgati ) then
                 temp1 = dscale( 2_ilp64 ) - tau
                 temp2 = dscale( 3_ilp64 ) - tau
              else
                 temp1 = dscale( 1_ilp64 ) - tau
                 temp2 = dscale( 2_ilp64 ) - tau
              end if
              a = ( temp1+temp2 )*f - temp1*temp2*df
              b = temp1*temp2*f
              c = f - ( temp1+temp2 )*df + temp1*temp2*ddf
     ...
     !! in some extremely rare situations.
        ...
        ! =====================================================================
           ...
           ! Local Arrays 
           real(dp) :: dscale(3_ilp64), zscale(3_ilp64)
           ! Local Scalars 
           logical(lk) :: scale
           integer(ilp64) :: i, iter, niter
           real(dp) :: a, b, base, c, ddf, df, eps, erretm, eta, f, fc, sclfac, sclinv, small1, &
           ...
           ! others but recomputed at each call
           eps = stdlib_I64_dlamch( 'EPSILON' )
           base = stdlib_I64_dlamch( 'BASE' )
           small1 = base**( int( log( stdlib_I64_dlamch( 'SAFMIN' ) ) / log( base ) /three,KIND=ilp64) )
     ...
     !! the overall problem.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           integer(ilp64) :: coltyp, curr, i, idlmda, indx, indxc, indxp, iq2, is, iw, iz, k, ldq2, &
                     n1, n2, ptr
           ! Intrinsic Functions 
           ! Executable Statements 
           ! test the input parameters.
           ...
           if( icompq<0_ilp64 .or. icompq>1_ilp64 ) then
              ...
              info = -2_ilp64
           else if( icompq==1_ilp64 .and. qsiz<n ) then
              info = -3_ilp64
           else if( ldq<max( 1_ilp64, n ) ) then
              info = -9_ilp64
           else if( min( 1_ilp64, n )>cutpnt .or. n<cutpnt ) then
              info = -12_ilp64
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SLAED7', -info )
              return
           end if
           ! quick return if possible
           if( n==0 )return
           ! the following values are for bookkeeping purposes only.  they are
           ! integer pointers which indicate the portion of the workspace
           ! used by a particular array in stdlib_I64_slaed8 and stdlib_I64_slaed9.
           if( icompq==1_ilp64 ) then
              ldq2 = qsiz
           else
              ldq2 = n
           ...
           ! form the z-vector which consists of the last row of q_1 and the
           ! first row of q_2.
           ptr = 1_ilp64 + 2_ilp64**tlvls
           do i = 1, curlvl - 1
              ptr = ptr + 2_ilp64**( tlvls-i )
           end do
           curr = ptr + curpbm
           call stdlib_I64_slaeda( n, tlvls, curlvl, curpbm, prmptr, perm, givptr,givcol, givnum, &
                     qstore, qptr, work( iz ),work( iz+n ), info )
           ! when solving the final problem, we no longer need the stored data,
           ! so we will overwrite the data from this level onto the previously
           ! used storage space.
           ...
           ! sort and deflate eigenvalues.
           call stdlib_I64_slaed8( icompq, k, n, qsiz, d, q, ldq, indxq, rho, cutpnt,work( iz ), work(&
            idlmda ), work( iq2 ), ldq2,work( iw ), perm( prmptr( curr ) ), givptr( curr+1 ),&
            givcol( 1_ilp64, givptr( curr ) ),givnum( 1_ilp64, givptr( curr ) ), iwork( indxp ),iwork( indx ),&
                       info )
           prmptr( curr+1 ) = prmptr( curr ) + n
           givptr( curr+1 ) = givptr( curr+1 ) + givptr( curr )
           ! solve secular equation.
           if( k/=0_ilp64 ) then
              call stdlib_I64_slaed9( k, 1_ilp64, k, n, d, work( is ), k, rho, work( idlmda ),work( iw ), &
                        qstore( qptr( curr ) ), k, info )
              if( info/=0 )go to 30
              if( icompq==1_ilp64 ) then
                 call stdlib_I64_sgemm( 'N', 'N', qsiz, k, k, one, work( iq2 ), ldq2,qstore( qptr( &
                           curr ) ), k, zero, q, ldq )
              end if
              qptr( curr+1 ) = qptr( curr ) + k**2_ilp64
           ! prepare the indxq sorting permutation.
              n1 = k
              n2 = n - k
              call stdlib_I64_slamrg( n1, n2, d, 1_ilp64, -1_ilp64, indxq )
     ...
     !! the overall problem.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DLAED7', -info )
              return
           end if
           ! quick return if possible
           if( n==0 )return
           ! the following values are for bookkeeping purposes only.  they are
           ! integer pointers which indicate the portion of the workspace
           ! used by a particular array in stdlib_I64_dlaed8 and stdlib_I64_dlaed9.
           if( icompq==1_ilp64 ) then
              ldq2 = qsiz
           else
              ldq2 = n
           ...
           curr = ptr + curpbm
           call stdlib_I64_dlaeda( n, tlvls, curlvl, curpbm, prmptr, perm, givptr,givcol, givnum, &
                     qstore, qptr, work( iz ),work( iz+n ), info )
           ! when solving the final problem, we no longer need the stored data,
           ! so we will overwrite the data from this level onto the previously
           ! used storage space.
           ...
           ! sort and deflate eigenvalues.
           call stdlib_I64_dlaed8( icompq, k, n, qsiz, d, q, ldq, indxq, rho, cutpnt,work( iz ), work(&
            idlmda ), work( iq2 ), ldq2,work( iw ), perm( prmptr( curr ) ), givptr( curr+1 ),&
            givcol( 1_ilp64, givptr( curr ) ),givnum( 1_ilp64, givptr( curr ) ), iwork( indxp ),iwork( indx ),&
                       info )
           prmptr( curr+1 ) = prmptr( curr ) + n
           ...
           ! solve secular equation.
           if( k/=0_ilp64 ) then
              call stdlib_I64_dlaed9( k, 1_ilp64, k, n, d, work( is ), k, rho, work( idlmda ),work( iw ), &
                        qstore( qptr( curr ) ), k, info )
              if( info/=0 )go to 30
              if( icompq==1_ilp64 ) then
                 call stdlib_I64_dgemm( 'N', 'N', qsiz, k, k, one, work( iq2 ), ldq2,qstore( qptr( &
                           curr ) ), k, zero, q, ldq )
              end if
              qptr( curr+1 ) = qptr( curr ) + k**2_ilp64
           ! prepare the indxq sorting permutation.
     ...
     !! the overall problem.
        ...
        ! =====================================================================
           ! Local Scalars 
           integer(ilp64) :: coltyp, curr, i, idlmda, indx, indxc, indxp, iq, iw, iz, k, n1, n2, &
                     ptr
           ! Intrinsic Functions 
           ! Executable Statements 
           ! test the input parameters.
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CLAED7', -info )
              return
           end if
           ! quick return if possible
           if( n==0 )return
           ...
           ! sort and deflate eigenvalues.
           call stdlib_I64_claed8( k, n, qsiz, q, ldq, d, rho, cutpnt, rwork( iz ),rwork( idlmda ), &
           work, qsiz, rwork( iw ),iwork( indxp ), iwork( indx ), indxq,perm( prmptr( curr ) ), &
           givptr( curr+1 ),givcol( 1_ilp64, givptr( curr ) ),givnum( 1_ilp64, givptr( curr ) ), info )
                     
           prmptr( curr+1 ) = prmptr( curr ) + n
           givptr( curr+1 ) = givptr( curr+1 ) + givptr( curr )
           ! solve secular equation.
           if( k/=0_ilp64 ) then
              call stdlib_I64_slaed9( k, 1_ilp64, k, n, d, rwork( iq ), k, rho,rwork( idlmda ), rwork( iw ),&
                        qstore( qptr( curr ) ), k, info )
              call stdlib_I64_clacrm( qsiz, k, work, qsiz, qstore( qptr( curr ) ), k, q,ldq, rwork( &
                        iq ) )
              qptr( curr+1 ) = qptr( curr ) + k**2_ilp64
              if( info/=0_ilp64 ) then
                 return
     ...
     !! the overall problem.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZLAED7', -info )
              return
           end if
           ! quick return if possible
           if( n==0 )return
           ! the following values are for bookkeeping purposes only.  they are
           ! integer pointers which indicate the portion of the workspace
           ! used by a particular array in stdlib_I64_dlaed2 and stdlib_I64_slaed3.
           iz = 1_ilp64
           idlmda = iz + n
           iw = idlmda + n
           iq = iw + n
           ...
           ! sort and deflate eigenvalues.
           call stdlib_I64_zlaed8( k, n, qsiz, q, ldq, d, rho, cutpnt, rwork( iz ),rwork( idlmda ), &
           work, qsiz, rwork( iw ),iwork( indxp ), iwork( indx ), indxq,perm( prmptr( curr ) ), &
           givptr( curr+1 ),givcol( 1_ilp64, givptr( curr ) ),givnum( 1_ilp64, givptr( curr ) ), info )
           ...
           ! solve secular equation.
           if( k/=0_ilp64 ) then
              call stdlib_I64_dlaed9( k, 1_ilp64, k, n, d, rwork( iq ), k, rho,rwork( idlmda ), rwork( iw ),&
                        qstore( qptr( curr ) ), k, info )
              call stdlib_I64_zlacrm( qsiz, k, work, qsiz, qstore( qptr( curr ) ), k, q,ldq, rwork( &
                        iq ) )
              qptr( curr+1 ) = qptr( curr ) + k**2_ilp64
              if( info/=0_ilp64 ) then
                 return
     ...
     !! equation problem is reduced by one.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           integer(ilp64) :: i, imax, j, jlam, jmax, jp, k2, n1, n1p1, n2
           real(sp) :: c, eps, s, t, tau, tol
           ! Intrinsic Functions 
           ! Executable Statements 
           ! test the input parameters.
           ...
           if( icompq<0_ilp64 .or. icompq>1_ilp64 ) then
              ...
              info = -7_ilp64
           else if( cutpnt<min( 1_ilp64, n ) .or. cutpnt>n ) then
              info = -10_ilp64
           else if( ldq2<max( 1_ilp64, n ) ) then
              info = -14_ilp64
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SLAED8', -info )
              return
           end if
           ! need to initialize givptr to o here in case of quick exit
           ! to prevent an unspecified code behavior (usually sigfault)
           ...
           i = 1_ilp64
           j = cutpnt + 1_ilp64
           call stdlib_I64_slamrg( n1, n2, dlamda, 1_ilp64, 1_ilp64, indx )
           do i = 1, n
              d( i ) = dlamda( indx( i ) )
              z( i ) = w( indx( i ) )
           end do
           ...
           if( rho*abs( z( imax ) )<=tol ) then
              ...
              if( icompq==0_ilp64 ) then
                 ...
                 do j = 1, n
                    perm( j ) = indxq( indx( j ) )
                    call stdlib_I64_scopy( qsiz, q( 1_ilp64, perm( j ) ), 1_ilp64, q2( 1_ilp64, j ), 1_ilp64 )
                 end do
                 call stdlib_I64_slacpy( 'A', qsiz, n, q2( 1_ilp64, 1_ilp64 ), ldq2, q( 1_ilp64, 1_ilp64 ),ldq )
              end if
              return
           end if
           ! if there are multiple eigenvalues then the problem deflates.  here
           ...
           if( rho*abs( z( j ) )<=tol ) then
              ...
              if( abs( t*c*s )<=tol ) then
                 ...
                 givcol( 1_ilp64, givptr ) = indxq( indx( jlam ) )
                 givcol( 2_ilp64, givptr ) = indxq( indx( j ) )
                 givnum( 1_ilp64, givptr ) = c
                 givnum( 2_ilp64, givptr ) = s
                 if( icompq==1_ilp64 ) then
                    call stdlib_I64_srot( qsiz, q( 1_ilp64, indxq( indx( jlam ) ) ), 1_ilp64,q( 1_ilp64, indxq( indx( j &
                              ) ) ), 1_ilp64, c, s )
                 end if
                 t = d( jlam )*c*c + d( j )*s*s
                 d( j ) = d( jlam )*s*s + d( j )*c*c
           ...
           if( k<n ) then
              if( icompq==0_ilp64 ) then
                 call stdlib_I64_scopy( n-k, dlamda( k+1 ), 1_ilp64, d( k+1 ), 1_ilp64 )
              else
                 call stdlib_I64_scopy( n-k, dlamda( k+1 ), 1_ilp64, d( k+1 ), 1_ilp64 )
                 call stdlib_I64_slacpy( 'A', qsiz, n-k, q2( 1_ilp64, k+1 ), ldq2,q( 1_ilp64, k+1 ), ldq )
              end if
           end if
           return
     ...
     !! equation problem is reduced by one.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DLAED8', -info )
              return
           end if
           ! need to initialize givptr to o here in case of quick exit
           ! to prevent an unspecified code behavior (usually sigfault)
           ...
           i = 1_ilp64
           j = cutpnt + 1_ilp64
           call stdlib_I64_dlamrg( n1, n2, dlamda, 1_ilp64, 1_ilp64, indx )
           do i = 1, n
              d( i ) = dlamda( indx( i ) )
              z( i ) = w( indx( i ) )
           end do
           ...
           if( rho*abs( z( imax ) )<=tol ) then
              ...
              if( icompq==0_ilp64 ) then
                 ...
                 do j = 1, n
                    perm( j ) = indxq( indx( j ) )
                    call stdlib_I64_dcopy( qsiz, q( 1_ilp64, perm( j ) ), 1_ilp64, q2( 1_ilp64, j ), 1_ilp64 )
                 end do
                 call stdlib_I64_dlacpy( 'A', qsiz, n, q2( 1_ilp64, 1_ilp64 ), ldq2, q( 1_ilp64, 1_ilp64 ),ldq )
              end if
              return
           end if
           ! if there are multiple eigenvalues then the problem deflates.  here
           ...
           if( rho*abs( z( j ) )<=tol ) then
              ...
              if( abs( t*c*s )<=tol ) then
                 ...
                 givnum( 2_ilp64, givptr ) = s
                 if( icompq==1_ilp64 ) then
                    call stdlib_I64_drot( qsiz, q( 1_ilp64, indxq( indx( jlam ) ) ), 1_ilp64,q( 1_ilp64, indxq( indx( j &
                              ) ) ), 1_ilp64, c, s )
                 end if
                 t = d( jlam )*c*c + d( j )*s*s
                 d( j ) = d( jlam )*s*s + d( j )*c*c
           ...
           if( k<n ) then
              if( icompq==0_ilp64 ) then
                 call stdlib_I64_dcopy( n-k, dlamda( k+1 ), 1_ilp64, d( k+1 ), 1_ilp64 )
              else
                 call stdlib_I64_dcopy( n-k, dlamda( k+1 ), 1_ilp64, d( k+1 ), 1_ilp64 )
                 call stdlib_I64_dlacpy( 'A', qsiz, n-k, q2( 1_ilp64, k+1 ), ldq2,q( 1_ilp64, k+1 ), ldq )
              end if
           end if
           return
     ...
     !! equation problem is reduced by one.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CLAED8', -info )
              return
           end if
           ! need to initialize givptr to o here in case of quick exit
           ! to prevent an unspecified code behavior (usually sigfault)
           ...
           if( rho*abs( z( imax ) )<=tol ) then
              ...
              do j = 1, n
                 perm( j ) = indxq( indx( j ) )
                 call stdlib_I64_ccopy( qsiz, q( 1_ilp64, perm( j ) ), 1_ilp64, q2( 1_ilp64, j ), 1_ilp64 )
              end do
              call stdlib_I64_clacpy( 'A', qsiz, n, q2( 1_ilp64, 1_ilp64 ), ldq2, q( 1_ilp64, 1_ilp64 ), ldq )
              return
           end if
           ! if there are multiple eigenvalues then the problem deflates.  here
           ! the number of equal eigenvalues are found.  as each equal
           ...
           if( rho*abs( z( j ) )<=tol ) then
              ...
              if( abs( t*c*s )<=tol ) then
                 ...
                 givnum( 1_ilp64, givptr ) = c
                 givnum( 2_ilp64, givptr ) = s
                 call stdlib_I64_csrot( qsiz, q( 1_ilp64, indxq( indx( jlam ) ) ), 1_ilp64,q( 1_ilp64, indxq( indx( j ) &
                           ) ), 1_ilp64, c, s )
                 t = d( jlam )*c*c + d( j )*s*s
                 d( j ) = d( jlam )*s*s + d( j )*c*c
                 d( jlam ) = t
           ...
           if( k<n ) then
              call stdlib_I64_scopy( n-k, dlamda( k+1 ), 1_ilp64, d( k+1 ), 1_ilp64 )
              call stdlib_I64_clacpy( 'A', qsiz, n-k, q2( 1_ilp64, k+1 ), ldq2, q( 1_ilp64, k+1 ),ldq )
           end if
           return
     ...
     !! equation problem is reduced by one.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZLAED8', -info )
              return
           end if
           ! need to initialize givptr to o here in case of quick exit
           ! to prevent an unspecified code behavior (usually sigfault)
           ...
           if( rho*abs( z( imax ) )<=tol ) then
              ...
              do j = 1, n
                 perm( j ) = indxq( indx( j ) )
                 call stdlib_I64_zcopy( qsiz, q( 1_ilp64, perm( j ) ), 1_ilp64, q2( 1_ilp64, j ), 1_ilp64 )
              end do
              call stdlib_I64_zlacpy( 'A', qsiz, n, q2( 1_ilp64, 1_ilp64 ), ldq2, q( 1_ilp64, 1_ilp64 ), ldq )
              return
           end if
           ! if there are multiple eigenvalues then the problem deflates.  here
           ! the number of equal eigenvalues are found.  as each equal
           ...
           if( rho*abs( z( j ) )<=tol ) then
              ...
              if( abs( t*c*s )<=tol ) then
                 ...
                 givnum( 1_ilp64, givptr ) = c
                 givnum( 2_ilp64, givptr ) = s
                 call stdlib_I64_zdrot( qsiz, q( 1_ilp64, indxq( indx( jlam ) ) ), 1_ilp64,q( 1_ilp64, indxq( indx( j ) &
                           ) ), 1_ilp64, c, s )
                 t = d( jlam )*c*c + d( j )*s*s
                 d( j ) = d( jlam )*s*s + d( j )*c*c
                 d( jlam ) = t
           ...
           if( k<n ) then
              call stdlib_I64_dcopy( n-k, dlamda( k+1 ), 1_ilp64, d( k+1 ), 1_ilp64 )
              call stdlib_I64_zlacpy( 'A', qsiz, n-k, q2( 1_ilp64, k+1 ), ldq2, q( 1_ilp64, k+1 ),ldq )
           end if
           return
     ...
     !! eigenvectors for use in calculating the next level of Z vectors.
        ...
        ! =====================================================================
           ...
           if( k<0_ilp64 ) then
              info = -1_ilp64
           else if( kstart<1_ilp64 .or. kstart>max( 1_ilp64, k ) ) then
              info = -2_ilp64
           else if( max( 1_ilp64, kstop )<kstart .or. kstop>max( 1_ilp64, k ) )then
              info = -3_ilp64
           else if( n<k ) then
              info = -4_ilp64
           else if( ldq<max( 1_ilp64, k ) ) then
              info = -7_ilp64
           else if( lds<max( 1_ilp64, k ) ) then
              info = -12_ilp64
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SLAED9', -info )
              return
           end if
           ! quick return if possible
           if( k==0 )return
           ...
           if( k==1_ilp64 .or. k==2_ilp64 ) then
              do i = 1, k
                 do j = 1, k
                    s( j, i ) = q( j, i )
                 end do
           ...
           do j = 1, k
              ...
              temp = stdlib_I64_snrm2( k, q( 1_ilp64, j ), 1_ilp64 )
              do i = 1, k
                 s( i, j ) = q( i, j ) / temp
              end do
           end do
     ...
     !! eigenvectors for use in calculating the next level of Z vectors.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DLAED9', -info )
              return
           end if
           ! quick return if possible
           if( k==0 )return
           ...
           do j = 1, k
              ...
              temp = stdlib_I64_dnrm2( k, q( 1_ilp64, j ), 1_ilp64 )
              do i = 1, k
                 s( i, j ) = q( i, j ) / temp
              end do
           end do
