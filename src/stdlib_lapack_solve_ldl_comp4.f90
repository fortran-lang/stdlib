  ...
  contains
     ...
     !! for the solution.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           logical(lk) :: upper
           integer(ilp64) :: count, i, ik, j, k, kase, kk, nz
           real(sp) :: eps, lstres, s, safe1, safe2, safmin, xk
           complex(sp) :: zdum
           ! Local Arrays 
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CHPRFS', -info )
              return
           end if
           ! quick return if possible
           ...
           loop_140: do j = 1, nrhs
              ...
              call stdlib_I64_chpmv( uplo, n, -cone, ap, x( 1_ilp64, j ), 1_ilp64, cone, work, 1_ilp64 )
              ! compute componentwise relative backward error from formula
              ! max(i) ( abs(r(i)) / ( abs(a)*abs(x) + abs(b) )(i) )
              ! where abs(z) is the componentwise absolute value of the matrix
              ! or vector z.  if the i-th component of the denominator is less
              ...
              if( berr( j )>eps .and. two*berr( j )<=lstres .and.count<=itmax ) then
                 ! update solution and try again.
                 call stdlib_I64_chptrs( uplo, n, 1_ilp64, afp, ipiv, work, n, info )
     ...
     !! for the solution.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZHPRFS', -info )
              return
           end if
           ! quick return if possible
           if( n==0_ilp64 .or. nrhs==0_ilp64 ) then
           ...
           loop_140: do j = 1, nrhs
              ...
              call stdlib_I64_zhpmv( uplo, n, -cone, ap, x( 1_ilp64, j ), 1_ilp64, cone, work, 1_ilp64 )
              ! compute componentwise relative backward error from formula
              ! max(i) ( abs(r(i)) / ( abs(a)*abs(x) + abs(b) )(i) )
              ! where abs(z) is the componentwise absolute value of the matrix
              ! or vector z.  if the i-th component of the denominator is less
              ...
              if( berr( j )>eps .and. two*berr( j )<=lstres .and.count<=itmax ) then
                 ! update solution and try again.
                 call stdlib_I64_zhptrs( uplo, n, 1_ilp64, afp, ipiv, work, n, info )
     ...
     !! condition number is computed as RCOND = 1 / (ANORM * norm(inv(A))).
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CHECON_ROOK', -info )
              return
           end if
           ! quick return if possible
           rcond = zero
           ...
           if( kase/=0_ilp64 ) then
              ! multiply by inv(l*d*l**h) or inv(u*d*u**h).
              call stdlib_I64_chetrs_rook( uplo, n, 1_ilp64, a, lda, ipiv, work, n, info )
              go to 30
           end if
           ! compute the estimate of the reciprocal condition number.
           if( ainvnm/=zero )rcond = ( one / ainvnm ) / anorm
     ...
     !! condition number is computed as RCOND = 1 / (ANORM * norm(inv(A))).
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZHECON_ROOK', -info )
              return
           end if
           ! quick return if possible
           rcond = zero
           ...
           if( kase/=0_ilp64 ) then
              ! multiply by inv(l*d*l**h) or inv(u*d*u**h).
              call stdlib_I64_zhetrs_rook( uplo, n, 1_ilp64, a, lda, ipiv, work, n, info )
              go to 30
           end if
           ! compute the estimate of the reciprocal condition number.
           if( ainvnm/=zero )rcond = ( one / ainvnm ) / anorm
     ...
     !! This is the blocked version of the algorithm, calling Level 3 BLAS.
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              ! determine the block size
              nb = stdlib_I64_ilaenv( 1_ilp64, 'CHETRF_ROOK', uplo, n, -1_ilp64, -1_ilp64, -1_ilp64 )
              lwkopt = max( 1_ilp64, n*nb )
              work( 1_ilp64 ) = lwkopt
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CHETRF_ROOK', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           if( nb>1_ilp64 .and. nb<n ) then
              ...
              if( lwork<iws ) then
                 ...
                 nbmin = max( 2_ilp64, stdlib_I64_ilaenv( 2_ilp64, 'CHETRF_ROOK',uplo, n, -1_ilp64, -1_ilp64, -1_ilp64 ) )
              end if
           else
              iws = 1_ilp64
           end if
           ...
           if( upper ) then
              ! factorize a as u*d*u**t using the upper triangle of a
              ! k is the main loop index, decreasing from n to 1 in steps of
              ! kb, where kb is the number of columns factorized by stdlib_I64_clahef_rook;
              ! kb is either nb or nb-1, or k for the last block
              k = n
              10 continue
              ! if k < 1, exit from loop
              ...
              if( k>nb ) then
                 ! factorize columns k-kb+1:k of a and use blocked code to
                 ! update columns 1:k-kb
                 call stdlib_I64_clahef_rook( uplo, k, nb, kb, a, lda,ipiv, work, ldwork, iinfo )
                           
              else
                 ! use unblocked code to factorize columns 1:k of a
                 call stdlib_I64_chetf2_rook( uplo, k, a, lda, ipiv, iinfo )
                 kb = k
              end if
              ! set info on the first occurrence of a zero pivot
              ...
              if( k<=n-nb ) then
                 ! factorize columns k:k+kb-1 of a and use blocked code to
                 ! update columns k+kb:n
                 call stdlib_I64_clahef_rook( uplo, n-k+1, nb, kb, a( k, k ), lda,ipiv( k ), work, &
                           ldwork, iinfo )
              else
                 ! use unblocked code to factorize columns k:n of a
                 call stdlib_I64_chetf2_rook( uplo, n-k+1, a( k, k ), lda, ipiv( k ),iinfo )
                 kb = n - k + 1_ilp64
              end if
              ! set info on the first occurrence of a zero pivot
     ...
     !! This is the blocked version of the algorithm, calling Level 3 BLAS.
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              ! determine the block size
              nb = stdlib_I64_ilaenv( 1_ilp64, 'ZHETRF_ROOK', uplo, n, -1_ilp64, -1_ilp64, -1_ilp64 )
              lwkopt = max( 1_ilp64, n*nb )
              work( 1_ilp64 ) = lwkopt
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZHETRF_ROOK', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           if( nb>1_ilp64 .and. nb<n ) then
              ...
              if( lwork<iws ) then
                 nb = max( lwork / ldwork, 1_ilp64 )
                 nbmin = max( 2_ilp64, stdlib_I64_ilaenv( 2_ilp64, 'ZHETRF_ROOK',uplo, n, -1_ilp64, -1_ilp64, -1_ilp64 ) )
              end if
           else
              iws = 1_ilp64
           end if
           ...
           if( upper ) then
              ! factorize a as u*d*u**t using the upper triangle of a
              ! k is the main loop index, decreasing from n to 1 in steps of
              ! kb, where kb is the number of columns factorized by stdlib_I64_zlahef_rook;
              ! kb is either nb or nb-1, or k for the last block
              k = n
              10 continue
              ! if k < 1, exit from loop
              ...
              if( k>nb ) then
                 ! factorize columns k-kb+1:k of a and use blocked code to
                 ! update columns 1:k-kb
                 call stdlib_I64_zlahef_rook( uplo, k, nb, kb, a, lda,ipiv, work, ldwork, iinfo )
                           
              else
                 ! use unblocked code to factorize columns 1:k of a
                 call stdlib_I64_zhetf2_rook( uplo, k, a, lda, ipiv, iinfo )
                 kb = k
              end if
              ! set info on the first occurrence of a zero pivot
              if( info==0_ilp64 .and. iinfo>0_ilp64 )info = iinfo
              ...
              if( k<=n-nb ) then
                 ! factorize columns k:k+kb-1 of a and use blocked code to
                 ! update columns k+kb:n
                 call stdlib_I64_zlahef_rook( uplo, n-k+1, nb, kb, a( k, k ), lda,ipiv( k ), work, &
                           ldwork, iinfo )
              else
                 ! use unblocked code to factorize columns k:n of a
                 call stdlib_I64_zhetf2_rook( uplo, n-k+1, a( k, k ), lda, ipiv( k ),iinfo )
                 kb = n - k + 1_ilp64
              end if
              ! set info on the first occurrence of a zero pivot
              if( info==0_ilp64 .and. iinfo>0_ilp64 )info = iinfo + k - 1_ilp64
     ...
     !! A11 (if UPLO = 'U') or A22 (if UPLO = 'L').
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           logical(lk) :: done
           integer(ilp64) :: imax, itemp, ii, j, jb, jj, jmax, jp1, jp2, k, kk, kkw, kp, kstep, kw, &
                     p
           real(sp) :: absakk, alpha, colmax, stemp, r1, rowmax, t, sfmin
           complex(sp) :: d11, d21, d22, z
           ! Intrinsic Functions 
           ...
           if( stdlib_lsame( uplo, 'U' ) ) then
              ...
              p = k
              ! copy column k of a to column kw of w and update it
              if( k>1_ilp64 )call stdlib_I64_ccopy( k-1, a( 1_ilp64, k ), 1_ilp64, w( 1_ilp64, kw ), 1_ilp64 )
              w( k, kw ) = real( a( k, k ),KIND=sp)
              if( k<n ) then
              ...
              if( max( absakk, colmax )==zero ) then
                 ...
                 kp = k
                 a( k, k ) = real( w( k, kw ),KIND=sp)
                 if( k>1_ilp64 )call stdlib_I64_ccopy( k-1, w( 1_ilp64, kw ), 1_ilp64, a( 1_ilp64, k ), 1_ilp64 )
              else
                 ! ============================================================
                 ! begin pivot search
                 ! case(1)
                 ...
                 if( .not.( absakk<alpha*colmax ) ) then
                    ...
                    12 continue
                       ! begin pivot search loop body
                       ! copy column imax to column kw-1 of w and update it
                       if( imax>1_ilp64 )call stdlib_I64_ccopy( imax-1, a( 1_ilp64, imax ), 1_ilp64, w( 1_ilp64, kw-1 ),1_ilp64 )
                       ...
                       if( k<n ) then
                          call stdlib_I64_cgemv( 'NO TRANSPOSE', k, n-k, -cone,a( 1_ilp64, k+1 ), lda, w( &
                                    imax, kw+1 ), ldw,cone, w( 1_ilp64, kw-1 ), 1_ilp64 )
                          w( imax, kw-1 ) = real( w( imax, kw-1 ),KIND=sp)
                       end if
                       ! jmax is the column-index of the largest off-diagonal
                       ! element in row imax, and rowmax is its absolute value.
                       ...
                       if( imax>1_ilp64 ) then
                          itemp = stdlib_I64_icamax( imax-1, w( 1_ilp64, kw-1 ), 1_ilp64 )
                          stemp = cabs1( w( itemp, kw-1 ) )
                          if( stemp>rowmax ) then
                             rowmax = stemp
                             jmax = itemp
                 ...
                 ! interchange rows and columns p and k.
                 ! updated column p is already stored in column kw of w.
                 if( ( kstep==2_ilp64 ) .and. ( p/=k ) ) then
                    ! copy non-updated column k to column p of submatrix a
                    ! at step k. no need to copy element into columns
                    ! k and k-1 of a for 2-by-2 pivot, since these columns
                    ! will be later overwritten.
                    a( p, p ) = real( a( k, k ),KIND=sp)
                    call stdlib_I64_ccopy( k-1-p, a( p+1, k ), 1_ilp64, a( p, p+1 ),lda )
                    call stdlib_I64_clacgv( k-1-p, a( p, p+1 ), lda )
                    if( p>1_ilp64 )call stdlib_I64_ccopy( p-1, a( 1_ilp64, k ), 1_ilp64, a( 1_ilp64, p ), 1_ilp64 )
                    ! interchange rows k and p in the last k+1 to n columns of a
                    ! (columns k and k-1 of a for 2-by-2 pivot will be
                    ! later overwritten). interchange rows k and p
                    ! in last kkw to nb columns of w.
                    if( k<n )call stdlib_I64_cswap( n-k, a( k, k+1 ), lda, a( p, k+1 ),lda )
                    call stdlib_I64_cswap( n-kk+1, w( k, kkw ), ldw, w( p, kkw ),ldw )
                 end if
                 ! interchange rows and columns kp and kk.
                 ! updated column kp is already stored in column kkw of w.
                 if( kp/=kk ) then
              ...
              do j = ( ( k-1 ) / nb )*nb + 1, 1, -nb
                 ...
                 ! update the rectangular superdiagonal block
                 if( j>=2_ilp64 )call stdlib_I64_cgemm( 'NO TRANSPOSE', 'TRANSPOSE', j-1, jb, n-k,-cone, a( &
                           1_ilp64, k+1 ), lda, w( j, kw+1 ), ldw,cone, a( 1_ilp64, j ), lda )
              end do
              ! put u12 in standard form by partially undoing the interchanges
              ! in of rows in columns k+1:n looping backwards from k+1 to n
              j = k + 1_ilp64
              60 continue
                 ...
                 ! of the rows to swap back doesn't include diagonal element)
                 j = j + 1_ilp64
                 if( jp2/=jj .and. j<=n )call stdlib_I64_cswap( n-j+1, a( jp2, j ), lda, a( jj, j ), &
                           lda )
                 jj = jj + 1_ilp64
                 if( kstep==2_ilp64 .and. jp1/=jj .and. j<=n )call stdlib_I64_cswap( n-j+1, a( jp1, j ), &
                           lda, a( jj, j ), lda )
              if( j<n )go to 60
              ! set kb to the number of columns factorized
              kb = n - k
              ...
              if( k>1_ilp64 ) then
                 call stdlib_I64_cgemv( 'NO TRANSPOSE', n-k+1, k-1, -cone, a( k, 1_ilp64 ),lda, w( k, 1_ilp64 ), &
                           ldw, cone, w( k, k ), 1_ilp64 )
                 w( k, k ) = real( w( k, k ),KIND=sp)
              end if
              ! determine rows and columns to be interchanged and whether
              ! a 1-by-1 or 2-by-2 pivot block will be used
              ...
              if( max( absakk, colmax )==zero ) then
                 ...
                 kp = k
                 a( k, k ) = real( w( k, k ),KIND=sp)
                 if( k<n )call stdlib_I64_ccopy( n-k, w( k+1, k ), 1_ilp64, a( k+1, k ), 1_ilp64 )
              else
                 ! ============================================================
                 ! begin pivot search
                 ! case(1)
                 ...
                 if( .not.( absakk<alpha*colmax ) ) then
                    ...
                    72 continue
                       ...
                       if( k>1_ilp64 ) then
                          call stdlib_I64_cgemv( 'NO TRANSPOSE', n-k+1, k-1, -cone,a( k, 1_ilp64 ), lda, w( &
                                    imax, 1_ilp64 ), ldw,cone, w( k, k+1 ), 1_ilp64 )
                          w( imax, k+1 ) = real( w( imax, k+1 ),KIND=sp)
                       end if
                       ! jmax is the column-index of the largest off-diagonal
                       ! element in row imax, and rowmax is its absolute value.
                       ...
                       if( imax<n ) then
                          itemp = imax + stdlib_I64_icamax( n-imax, w( imax+1, k+1 ), 1_ilp64)
                          stemp = cabs1( w( itemp, k+1 ) )
                          if( stemp>rowmax ) then
                             rowmax = stemp
                             jmax = itemp
                 ...
                 if( ( kstep==2_ilp64 ) .and. ( p/=k ) ) then
                    ...
                    ! will be later overwritten.
                    a( p, p ) = real( a( k, k ),KIND=sp)
                    call stdlib_I64_ccopy( p-k-1, a( k+1, k ), 1_ilp64, a( p, k+1 ), lda )
                    call stdlib_I64_clacgv( p-k-1, a( p, k+1 ), lda )
                    if( p<n )call stdlib_I64_ccopy( n-p, a( p+1, k ), 1_ilp64, a( p+1, p ), 1_ilp64 )
                    ! interchange rows k and p in first k-1 columns of a
                    ! (columns k and k+1 of a for 2-by-2 pivot will be
                    ! later overwritten). interchange rows k and p
                    ! in first kk columns of w.
                    if( k>1_ilp64 )call stdlib_I64_cswap( k-1, a( k, 1_ilp64 ), lda, a( p, 1_ilp64 ), lda )
                    call stdlib_I64_cswap( kk, w( k, 1_ilp64 ), ldw, w( p, 1_ilp64 ), ldw )
                 end if
                 ! interchange rows and columns kp and kk.
                 ! updated column kp is already stored in column kk of w.
                 if( kp/=kk ) then
              ...
              120 continue
                 ...
                 ! of the rows to swap back doesn't include diagonal element)
                 j = j - 1_ilp64
                 if( jp2/=jj .and. j>=1_ilp64 )call stdlib_I64_cswap( j, a( jp2, 1_ilp64 ), lda, a( jj, 1_ilp64 ), lda )
                           
                 jj = jj -1_ilp64
                 if( kstep==2_ilp64 .and. jp1/=jj .and. j>=1_ilp64 )call stdlib_I64_cswap( j, a( jp1, 1_ilp64 ), lda, a(&
                            jj, 1_ilp64 ), lda )
              if( j>1 )go to 120
              ! set kb to the number of columns factorized
              kb = k - 1_ilp64
     ...
     !! A11 (if UPLO = 'U') or A22 (if UPLO = 'L').
        ...
        ! =====================================================================
           ...
           if( stdlib_lsame( uplo, 'U' ) ) then
              ...
              p = k
              ! copy column k of a to column kw of w and update it
              if( k>1_ilp64 )call stdlib_I64_zcopy( k-1, a( 1_ilp64, k ), 1_ilp64, w( 1_ilp64, kw ), 1_ilp64 )
              w( k, kw ) = real( a( k, k ),KIND=dp)
              if( k<n ) then
              ...
              if( max( absakk, colmax )==zero ) then
                 ...
                 kp = k
                 a( k, k ) = real( w( k, kw ),KIND=dp)
                 if( k>1_ilp64 )call stdlib_I64_zcopy( k-1, w( 1_ilp64, kw ), 1_ilp64, a( 1_ilp64, k ), 1_ilp64 )
              else
                 ! ============================================================
                 ! begin pivot search
                 ! case(1)
                 ...
                 if( .not.( absakk<alpha*colmax ) ) then
                    ...
                    12 continue
                       ! begin pivot search loop body
                       ! copy column imax to column kw-1 of w and update it
                       if( imax>1_ilp64 )call stdlib_I64_zcopy( imax-1, a( 1_ilp64, imax ), 1_ilp64, w( 1_ilp64, kw-1 ),1_ilp64 )
                       ...
                       if( k<n ) then
                          call stdlib_I64_zgemv( 'NO TRANSPOSE', k, n-k, -cone,a( 1_ilp64, k+1 ), lda, w( &
                                    imax, kw+1 ), ldw,cone, w( 1_ilp64, kw-1 ), 1_ilp64 )
                          w( imax, kw-1 ) = real( w( imax, kw-1 ),KIND=dp)
                       end if
                       ! jmax is the column-index of the largest off-diagonal
                       ...
                       if( imax>1_ilp64 ) then
                          itemp = stdlib_I64_izamax( imax-1, w( 1_ilp64, kw-1 ), 1_ilp64 )
                          dtemp = cabs1( w( itemp, kw-1 ) )
                          if( dtemp>rowmax ) then
                             rowmax = dtemp
                             jmax = itemp
                 ...
                 if( ( kstep==2_ilp64 ) .and. ( p/=k ) ) then
                    ...
                    ! will be later overwritten.
                    a( p, p ) = real( a( k, k ),KIND=dp)
                    call stdlib_I64_zcopy( k-1-p, a( p+1, k ), 1_ilp64, a( p, p+1 ),lda )
                    call stdlib_I64_zlacgv( k-1-p, a( p, p+1 ), lda )
                    if( p>1_ilp64 )call stdlib_I64_zcopy( p-1, a( 1_ilp64, k ), 1_ilp64, a( 1_ilp64, p ), 1_ilp64 )
                    ! interchange rows k and p in the last k+1 to n columns of a
                    ! (columns k and k-1 of a for 2-by-2 pivot will be
                    ! later overwritten). interchange rows k and p
                    ! in last kkw to nb columns of w.
                    if( k<n )call stdlib_I64_zswap( n-k, a( k, k+1 ), lda, a( p, k+1 ),lda )
                    call stdlib_I64_zswap( n-kk+1, w( k, kkw ), ldw, w( p, kkw ),ldw )
                 end if
                 ! interchange rows and columns kp and kk.
                 ! updated column kp is already stored in column kkw of w.
                 if( kp/=kk ) then
              ...
              do j = ( ( k-1 ) / nb )*nb + 1, 1, -nb
                 ...
                 ! update the rectangular superdiagonal block
                 if( j>=2_ilp64 )call stdlib_I64_zgemm( 'NO TRANSPOSE', 'TRANSPOSE', j-1, jb, n-k,-cone, a( &
                           1_ilp64, k+1 ), lda, w( j, kw+1 ), ldw,cone, a( 1_ilp64, j ), lda )
              end do
              ! put u12 in standard form by partially undoing the interchanges
              ! in of rows in columns k+1:n looping backwards from k+1 to n
              ...
              60 continue
                 ...
                 ! of the rows to swap back doesn't include diagonal element)
                 j = j + 1_ilp64
                 if( jp2/=jj .and. j<=n )call stdlib_I64_zswap( n-j+1, a( jp2, j ), lda, a( jj, j ), &
                           lda )
                 jj = jj + 1_ilp64
                 if( kstep==2_ilp64 .and. jp1/=jj .and. j<=n )call stdlib_I64_zswap( n-j+1, a( jp1, j ), &
                           lda, a( jj, j ), lda )
              if( j<n )go to 60
              ! set kb to the number of columns factorized
              kb = n - k
              ...
              if( k>1_ilp64 ) then
                 call stdlib_I64_zgemv( 'NO TRANSPOSE', n-k+1, k-1, -cone, a( k, 1_ilp64 ),lda, w( k, 1_ilp64 ), &
                           ldw, cone, w( k, k ), 1_ilp64 )
                 w( k, k ) = real( w( k, k ),KIND=dp)
              end if
              ! determine rows and columns to be interchanged and whether
              ...
              if( max( absakk, colmax )==zero ) then
                 ...
                 kp = k
                 a( k, k ) = real( w( k, k ),KIND=dp)
                 if( k<n )call stdlib_I64_zcopy( n-k, w( k+1, k ), 1_ilp64, a( k+1, k ), 1_ilp64 )
              else
                 ! ============================================================
                 ! begin pivot search
                 ! case(1)
                 ...
                 if( .not.( absakk<alpha*colmax ) ) then
                    ...
                    72 continue
                       ...
                       if( k>1_ilp64 ) then
                          call stdlib_I64_zgemv( 'NO TRANSPOSE', n-k+1, k-1, -cone,a( k, 1_ilp64 ), lda, w( &
                                    imax, 1_ilp64 ), ldw,cone, w( k, k+1 ), 1_ilp64 )
                          w( imax, k+1 ) = real( w( imax, k+1 ),KIND=dp)
                       end if
                       ! jmax is the column-index of the largest off-diagonal
                       ...
                       if( imax<n ) then
                          itemp = imax + stdlib_I64_izamax( n-imax, w( imax+1, k+1 ), 1_ilp64)
                          dtemp = cabs1( w( itemp, k+1 ) )
                          if( dtemp>rowmax ) then
                             rowmax = dtemp
                             jmax = itemp
                 ...
                 if( ( kstep==2_ilp64 ) .and. ( p/=k ) ) then
                    ...
                    ! will be later overwritten.
                    a( p, p ) = real( a( k, k ),KIND=dp)
                    call stdlib_I64_zcopy( p-k-1, a( k+1, k ), 1_ilp64, a( p, k+1 ), lda )
                    call stdlib_I64_zlacgv( p-k-1, a( p, k+1 ), lda )
                    if( p<n )call stdlib_I64_zcopy( n-p, a( p+1, k ), 1_ilp64, a( p+1, p ), 1_ilp64 )
                    ! interchange rows k and p in first k-1 columns of a
                    ! (columns k and k+1 of a for 2-by-2 pivot will be
                    ! later overwritten). interchange rows k and p
                    ! in first kk columns of w.
                    if( k>1_ilp64 )call stdlib_I64_zswap( k-1, a( k, 1_ilp64 ), lda, a( p, 1_ilp64 ), lda )
                    call stdlib_I64_zswap( kk, w( k, 1_ilp64 ), ldw, w( p, 1_ilp64 ), ldw )
                 end if
                 ! interchange rows and columns kp and kk.
                 ! updated column kp is already stored in column kk of w.
                 if( kp/=kk ) then
              ...
              120 continue
                 ...
                 ! of the rows to swap back doesn't include diagonal element)
                 j = j - 1_ilp64
                 if( jp2/=jj .and. j>=1_ilp64 )call stdlib_I64_zswap( j, a( jp2, 1_ilp64 ), lda, a( jj, 1_ilp64 ), lda )
                           
                 jj = jj -1_ilp64
                 if( kstep==2_ilp64 .and. jp1/=jj .and. j>=1_ilp64 )call stdlib_I64_zswap( j, a( jp1, 1_ilp64 ), lda, a(&
                            jj, 1_ilp64 ), lda )
              if( j>1 )go to 120
              ! set kb to the number of columns factorized
              kb = k - 1_ilp64
     ...
     !! This is the unblocked version of the algorithm, calling Level 2 BLAS.
        ...
        ! ======================================================================
           ...
           ! Local Scalars 
           logical(lk) :: done, upper
           integer(ilp64) :: i, ii, imax, itemp, j, jmax, k, kk, kp, kstep, p
           real(sp) :: absakk, alpha, colmax, d, d11, d22, r1, stemp, rowmax, tt, sfmin
           complex(sp) :: d12, d21, t, wk, wkm1, wkp1, z
           ! Intrinsic Functions 
           ! Statement Functions 
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CHETF2_ROOK', -info )
              return
           end if
           ! initialize alpha for use in choosing pivot block size.
           alpha = ( one+sqrt( sevten ) ) / eight
              ...
              if( ( max( absakk, colmax )==zero ) ) then
                 ...
                 if( .not.( absakk<alpha*colmax ) ) then
                    ...
                    12 continue
                       ...
                       if( imax>1_ilp64 ) then
                          itemp = stdlib_I64_icamax( imax-1, a( 1_ilp64, imax ), 1_ilp64 )
                          stemp = cabs1( a( itemp, imax ) )
                          if( stemp>rowmax ) then
                             rowmax = stemp
                             jmax = itemp
                 ...
                 if( ( kstep==2_ilp64 ) .and. ( p/=k ) ) then
                    ! (1) swap columnar parts
                    if( p>1_ilp64 )call stdlib_I64_cswap( p-1, a( 1_ilp64, k ), 1_ilp64, a( 1_ilp64, p ), 1_ilp64 )
                    ! (2) swap and conjugate middle parts
                    do j = p + 1, k - 1
                       t = conjg( a( j, k ) )
                       a( j, k ) = conjg( a( p, j ) )
                 ...
                 if( kp/=kk ) then
                    ! (1) swap columnar parts
                    if( kp>1_ilp64 )call stdlib_I64_cswap( kp-1, a( 1_ilp64, kk ), 1_ilp64, a( 1_ilp64, kp ), 1_ilp64 )
                    ! (2) swap and conjugate middle parts
                    do j = kp + 1, kk - 1
                       t = conjg( a( j, kk ) )
                       a( j, kk ) = conjg( a( kp, j ) )
                    ...
                    if( k>1_ilp64 ) then
                       ...
                       if( abs( real( a( k, k ),KIND=sp) )>=sfmin ) then
                          ...
                          ! a := a - u(k)*d(k)*u(k)**t
                             ! = a - w(k)*1/d(k)*w(k)**t
                          d11 = one / real( a( k, k ),KIND=sp)
                          call stdlib_I64_cher( uplo, k-1, -d11, a( 1_ilp64, k ), 1_ilp64, a, lda )
                          ! store u(k) in column k
                          call stdlib_I64_csscal( k-1, d11, a( 1_ilp64, k ), 1_ilp64 )
                       else
                          ! store l(k) in column k
                          d11 = real( a( k, k ),KIND=sp)
                          do ii = 1, k - 1
              ...
              if( max( absakk, colmax )==zero ) then
                 ...
                 if( .not.( absakk<alpha*colmax ) ) then
                    ...
                    42 continue
                       ...
                       if( imax<n ) then
                          itemp = imax + stdlib_I64_icamax( n-imax, a( imax+1, imax ),1_ilp64 )
                          stemp = cabs1( a( itemp, imax ) )
                          if( stemp>rowmax ) then
                             rowmax = stemp
                             jmax = itemp
                 ...
                 if( ( kstep==2_ilp64 ) .and. ( p/=k ) ) then
                    ! (1) swap columnar parts
                    if( p<n )call stdlib_I64_cswap( n-p, a( p+1, k ), 1_ilp64, a( p+1, p ), 1_ilp64 )
                    ! (2) swap and conjugate middle parts
                    do j = k + 1, p - 1
                       t = conjg( a( j, k ) )
                       a( j, k ) = conjg( a( p, j ) )
                    ...
                    if( k<n ) then
                       ...
                       if( abs( real( a( k, k ),KIND=sp) )>=sfmin ) then
                          ...
                          ! a := a - l(k)*d(k)*l(k)**t
                             ! = a - w(k)*(1/d(k))*w(k)**t
                          d11 = one / real( a( k, k ),KIND=sp)
                          call stdlib_I64_cher( uplo, n-k, -d11, a( k+1, k ), 1_ilp64,a( k+1, k+1 ), lda )
                                    
                          ! store l(k) in column k
                          call stdlib_I64_csscal( n-k, d11, a( k+1, k ), 1_ilp64 )
                       else
                          ! store l(k) in column k
                          d11 = real( a( k, k ),KIND=sp)
                          do ii = k + 1, n
     ...
     !! This is the unblocked version of the algorithm, calling Level 2 BLAS.
        ...
        ! ======================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZHETF2_ROOK', -info )
              return
           end if
           ! initialize alpha for use in choosing pivot block size.
           alpha = ( one+sqrt( sevten ) ) / eight
              ...
              if( ( max( absakk, colmax )==zero ) ) then
                 ...
                 if( .not.( absakk<alpha*colmax ) ) then
                    ...
                    12 continue
                       ...
                       if( imax>1_ilp64 ) then
                          itemp = stdlib_I64_izamax( imax-1, a( 1_ilp64, imax ), 1_ilp64 )
                          dtemp = cabs1( a( itemp, imax ) )
                          if( dtemp>rowmax ) then
                             rowmax = dtemp
                             jmax = itemp
                 ...
                 if( ( kstep==2_ilp64 ) .and. ( p/=k ) ) then
                    ! (1) swap columnar parts
                    if( p>1_ilp64 )call stdlib_I64_zswap( p-1, a( 1_ilp64, k ), 1_ilp64, a( 1_ilp64, p ), 1_ilp64 )
                    ! (2) swap and conjugate middle parts
                    do j = p + 1, k - 1
                       t = conjg( a( j, k ) )
                       a( j, k ) = conjg( a( p, j ) )
                 ...
                 if( kp/=kk ) then
                    ! (1) swap columnar parts
                    if( kp>1_ilp64 )call stdlib_I64_zswap( kp-1, a( 1_ilp64, kk ), 1_ilp64, a( 1_ilp64, kp ), 1_ilp64 )
                    ! (2) swap and conjugate middle parts
                    do j = kp + 1, kk - 1
                       t = conjg( a( j, kk ) )
                       a( j, kk ) = conjg( a( kp, j ) )
                    ...
                    if( k>1_ilp64 ) then
                       ...
                       if( abs( real( a( k, k ),KIND=dp) )>=sfmin ) then
                          ...
                          ! a := a - u(k)*d(k)*u(k)**t
                             ! = a - w(k)*1/d(k)*w(k)**t
                          d11 = one / real( a( k, k ),KIND=dp)
                          call stdlib_I64_zher( uplo, k-1, -d11, a( 1_ilp64, k ), 1_ilp64, a, lda )
                          ! store u(k) in column k
                          call stdlib_I64_zdscal( k-1, d11, a( 1_ilp64, k ), 1_ilp64 )
                       else
                          ! store l(k) in column k
                          d11 = real( a( k, k ),KIND=dp)
                          do ii = 1, k - 1
              ...
              if( max( absakk, colmax )==zero ) then
                 ...
                 if( .not.( absakk<alpha*colmax ) ) then
                    ...
                    42 continue
                       ...
                       if( imax<n ) then
                          itemp = imax + stdlib_I64_izamax( n-imax, a( imax+1, imax ),1_ilp64 )
                          dtemp = cabs1( a( itemp, imax ) )
                          if( dtemp>rowmax ) then
                             rowmax = dtemp
                             jmax = itemp
                 ...
                 if( ( kstep==2_ilp64 ) .and. ( p/=k ) ) then
                    ! (1) swap columnar parts
                    if( p<n )call stdlib_I64_zswap( n-p, a( p+1, k ), 1_ilp64, a( p+1, p ), 1_ilp64 )
                    ! (2) swap and conjugate middle parts
                    do j = k + 1, p - 1
                       t = conjg( a( j, k ) )
                       a( j, k ) = conjg( a( p, j ) )
                    ...
                    if( k<n ) then
                       ...
                       if( abs( real( a( k, k ),KIND=dp) )>=sfmin ) then
                          ...
                          ! a := a - l(k)*d(k)*l(k)**t
                             ! = a - w(k)*(1/d(k))*w(k)**t
                          d11 = one / real( a( k, k ),KIND=dp)
                          call stdlib_I64_zher( uplo, n-k, -d11, a( k+1, k ), 1_ilp64,a( k+1, k+1 ), lda )
                                    
                          ! store l(k) in column k
                          call stdlib_I64_zdscal( n-k, d11, a( k+1, k ), 1_ilp64 )
                       else
                          ! store l(k) in column k
                          d11 = real( a( k, k ),KIND=dp)
                          do ii = k + 1, n
     ...
     !! A = L*D*L**H computed by CHETRF_ROOK.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CHETRS_ROOK', -info )
              return
           end if
           ! quick return if possible
           if( n==0 .or. nrhs==0 )return
     ...
     !! A = L*D*L**H computed by ZHETRF_ROOK.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZHETRS_ROOK', -info )
              return
           end if
           ! quick return if possible
           if( n==0 .or. nrhs==0 )return
     ...
     !! CHETRF_ROOK.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CHETRI_ROOK', -info )
              return
           end if
           ! quick return if possible
           if( n==0 )return
           ...
           if( upper ) then
              ...
              if( kstep==1_ilp64 ) then
                 ...
                 kp = ipiv( k )
                 if( kp/=k ) then
                    if( kp>1_ilp64 )call stdlib_I64_cswap( kp-1, a( 1_ilp64, k ), 1_ilp64, a( 1_ilp64, kp ), 1_ilp64 )
                    do j = kp + 1, k - 1
                       temp = conjg( a( j, k ) )
                       a( j, k ) = conjg( a( kp, j ) )
                       a( kp, j ) = temp
     ...
     !! ZHETRF_ROOK.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZHETRI_ROOK', -info )
              return
           end if
           ! quick return if possible
           if( n==0 )return
           ...
           if( upper ) then
              ...
              if( kstep==1_ilp64 ) then
                 ...
                 kp = ipiv( k )
                 if( kp/=k ) then
                    if( kp>1_ilp64 )call stdlib_I64_zswap( kp-1, a( 1_ilp64, k ), 1_ilp64, a( 1_ilp64, kp ), 1_ilp64 )
                    do j = kp + 1, k - 1
                       temp = conjg( a( j, k ) )
                       a( j, k ) = conjg( a( kp, j ) )
                       a( kp, j ) = temp
     ...
     !! For more information see Further Details section.
        ...
        ! =====================================================================
           ! Local Scalars 
           logical(lk) :: lquery, upper
           integer(ilp64) :: i, iinfo, ip, iws, k, kb, ldwork, lwkopt, nb, nbmin
           ! Intrinsic Functions 
           ! Executable Statements 
           ! test the input parameters.
           info = 0_ilp64
           ...
           if( info==0_ilp64 ) then
              ! determine the block size
              nb = stdlib_I64_ilaenv( 1_ilp64, 'CHETRF_RK', uplo, n, -1_ilp64, -1_ilp64, -1_ilp64 )
              lwkopt = n*nb
              work( 1_ilp64 ) = lwkopt
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CHETRF_RK', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           if( nb>1_ilp64 .and. nb<n ) then
              ...
              if( lwork<iws ) then
                 nb = max( lwork / ldwork, 1_ilp64 )
                 nbmin = max( 2_ilp64, stdlib_I64_ilaenv( 2_ilp64, 'CHETRF_RK',uplo, n, -1_ilp64, -1_ilp64, -1_ilp64 ) )
              end if
           else
              iws = 1_ilp64
           end if
           ...
           if( upper ) then
              ! factorize a as u*d*u**t using the upper triangle of a
              ! k is the main loop index, decreasing from n to 1 in steps of
              ! kb, where kb is the number of columns factorized by stdlib_I64_clahef_rk;
              ! kb is either nb or nb-1, or k for the last block
              k = n
              10 continue
              ! if k < 1, exit from loop
              ...
              if( k>nb ) then
                 ! factorize columns k-kb+1:k of a and use blocked code to
                 ! update columns 1:k-kb
                 call stdlib_I64_clahef_rk( uplo, k, nb, kb, a, lda, e,ipiv, work, ldwork, iinfo )
                           
              else
                 ! use unblocked code to factorize columns 1:k of a
                 call stdlib_I64_chetf2_rk( uplo, k, a, lda, e, ipiv, iinfo )
                 kb = k
              end if
              ! set info on the first occurrence of a zero pivot
              if( info==0_ilp64 .and. iinfo>0_ilp64 )info = iinfo
              ...
              if( k<n ) then
                 do i = k, ( k - kb + 1 ), -1
                    ip = abs( ipiv( i ) )
                    if( ip/=i ) then
                       call stdlib_I64_cswap( n-k, a( i, k+1 ), lda,a( ip, k+1 ), lda )
                    end if
                 end do
              end if
              ! decrease k and return to the start of the main loop
              ...
              if( k<=n-nb ) then
                 ! factorize columns k:k+kb-1 of a and use blocked code to
                 ! update columns k+kb:n
                 call stdlib_I64_clahef_rk( uplo, n-k+1, nb, kb, a( k, k ), lda, e( k ),ipiv( k ), &
                           work, ldwork, iinfo )
              else
                 ! use unblocked code to factorize columns k:n of a
                 call stdlib_I64_chetf2_rk( uplo, n-k+1, a( k, k ), lda, e( k ),ipiv( k ), iinfo )
              ...
              if( k>1_ilp64 ) then
                 do i = k, ( k + kb - 1 ), 1
                    ip = abs( ipiv( i ) )
                    if( ip/=i ) then
                       call stdlib_I64_cswap( k-1, a( i, 1_ilp64 ), lda,a( ip, 1_ilp64 ), lda )
                    end if
                 end do
              end if
              ! increase k and return to the start of the main loop
     ...
     !! For more information see Further Details section.
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              ! determine the block size
              nb = stdlib_I64_ilaenv( 1_ilp64, 'ZHETRF_RK', uplo, n, -1_ilp64, -1_ilp64, -1_ilp64 )
              lwkopt = n*nb
              work( 1_ilp64 ) = lwkopt
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZHETRF_RK', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           if( nb>1_ilp64 .and. nb<n ) then
              ...
              if( lwork<iws ) then
                 nb = max( lwork / ldwork, 1_ilp64 )
                 nbmin = max( 2_ilp64, stdlib_I64_ilaenv( 2_ilp64, 'ZHETRF_RK',uplo, n, -1_ilp64, -1_ilp64, -1_ilp64 ) )
              end if
           else
              iws = 1_ilp64
           end if
           ...
           if( upper ) then
              ! factorize a as u*d*u**t using the upper triangle of a
              ! k is the main loop index, decreasing from n to 1 in steps of
              ! kb, where kb is the number of columns factorized by stdlib_I64_zlahef_rk;
              ! kb is either nb or nb-1, or k for the last block
              k = n
              10 continue
              ! if k < 1, exit from loop
              ...
              if( k>nb ) then
                 ! factorize columns k-kb+1:k of a and use blocked code to
                 ! update columns 1:k-kb
                 call stdlib_I64_zlahef_rk( uplo, k, nb, kb, a, lda, e,ipiv, work, ldwork, iinfo )
                           
              else
                 ! use unblocked code to factorize columns 1:k of a
                 call stdlib_I64_zhetf2_rk( uplo, k, a, lda, e, ipiv, iinfo )
                 kb = k
              end if
              ! set info on the first occurrence of a zero pivot
              if( info==0_ilp64 .and. iinfo>0_ilp64 )info = iinfo
              ...
              if( k<n ) then
                 do i = k, ( k - kb + 1 ), -1
                    ip = abs( ipiv( i ) )
                    if( ip/=i ) then
                       call stdlib_I64_zswap( n-k, a( i, k+1 ), lda,a( ip, k+1 ), lda )
                    end if
                 end do
              end if
              ! decrease k and return to the start of the main loop
              ...
              if( k<=n-nb ) then
                 ! factorize columns k:k+kb-1 of a and use blocked code to
                 ! update columns k+kb:n
                 call stdlib_I64_zlahef_rk( uplo, n-k+1, nb, kb, a( k, k ), lda, e( k ),ipiv( k ), &
                           work, ldwork, iinfo )
              else
                 ! use unblocked code to factorize columns k:n of a
                 call stdlib_I64_zhetf2_rk( uplo, n-k+1, a( k, k ), lda, e( k ),ipiv( k ), iinfo )
              ...
              if( k>1_ilp64 ) then
                 do i = k, ( k + kb - 1 ), 1
                    ip = abs( ipiv( i ) )
                    if( ip/=i ) then
                       call stdlib_I64_zswap( k-1, a( i, 1_ilp64 ), lda,a( ip, 1_ilp64 ), lda )
                    end if
                 end do
              end if
              ! increase k and return to the start of the main loop
     ...
     !! A11 (if UPLO = 'U') or A22 (if UPLO = 'L').
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           logical(lk) :: done
           integer(ilp64) :: imax, itemp, ii, j, jb, jj, jmax, k, kk, kkw, kp, kstep, kw, p
           real(sp) :: absakk, alpha, colmax, stemp, r1, rowmax, t, sfmin
           complex(sp) :: d11, d21, d22, z
           ! Intrinsic Functions 
           ! Statement Functions 
     ...
     !! For more information see Further Details section.
        ...
        ! ======================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CHETF2_RK', -info )
              return
           end if
           ! initialize alpha for use in choosing pivot block size.
           alpha = ( one+sqrt( sevten ) ) / eight
     ...
     !! For more information see Further Details section.
        ...
        ! ======================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZHETF2_RK', -info )
              return
           end if
           ! initialize alpha for use in choosing pivot block size.
           alpha = ( one+sqrt( sevten ) ) / eight
     ...
     !! This is the blocked version of the algorithm, calling Level 3 BLAS.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           logical(lk) :: lquery, upper
           integer(ilp64) :: j, lwkopt
           integer(ilp64) :: nb, mj, nj, k1, k2, j1, j2, j3, jb
           complex(sp) :: alpha
           ! Intrinsic Functions 
           ! Executable Statements 
           ! determine the block size
           nb = stdlib_I64_ilaenv( 1_ilp64, 'CHETRF_AA', uplo, n, -1_ilp64, -1_ilp64, -1_ilp64 )
           ! test the input parameters.
           info = 0_ilp64
           upper = stdlib_lsame( uplo, 'U' )
           lquery = ( lwork==-1_ilp64 )
           if( .not.upper .and. .not.stdlib_lsame( uplo, 'L' ) ) then
              ...
              info = -4_ilp64
           else if( lwork<( 2_ilp64*n ) .and. .not.lquery ) then
              info = -7_ilp64
           end if
           if( info==0_ilp64 ) then
              lwkopt = (nb+1)*n
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CHETRF_AA', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           if( upper ) then
              ...
              ! .....................................................
              ! copy first row a(1, 1:n) into h(1:n) (stored in work(1:n))
              call stdlib_I64_ccopy( n, a( 1_ilp64, 1_ilp64 ), lda, work( 1_ilp64 ), 1_ilp64 )
              ! j is the main loop index, increasing from 1 to n in steps of
              ! jb, where jb is the number of columns factorized by stdlib_I64_clahef;
              ! jb is either nb, or n-j+1 for the last block
              j = 0_ilp64
              10 continue
              if( j>=n )go to 20
              ...
              k1 = max(1_ilp64, j)-j
              ! panel factorization
              call stdlib_I64_clahef_aa( uplo, 2_ilp64-k1, n-j, jb,a( max(1_ilp64, j), j+1 ), lda,ipiv( j+1 ), &
                        work, n, work( n*nb+1 ) )
              ! adjust ipiv and apply it back (j-th step picks (j+1)-th pivot)
              do j2 = j+2, min(n, j+jb+1)
                 ipiv( j2 ) = ipiv( j2 ) + j
                 if( (j2/=ipiv(j2)) .and. ((j1-k1)>2_ilp64) ) then
                    call stdlib_I64_cswap( j1-k1-2, a( 1_ilp64, j2 ), 1_ilp64,a( 1_ilp64, ipiv(j2) ), 1_ilp64 )
                 end if
              end do
              j = j + jb
              ! trailing submatrix update, where
              ...
              if( j<n ) then
                ! if the first panel and jb=1 (nb=1), then nothing to do
                 if( j1>1_ilp64 .or. jb>1_ilp64 ) then
                    ! merge rank-1 update with blas-3 update
                    alpha = conjg( a( j, j+1 ) )
                    a( j, j+1 ) = cone
                    call stdlib_I64_ccopy( n-j, a( j-1, j+1 ), lda,work( (j+1-j1+1)+jb*n ), 1_ilp64 )
                              
                    call stdlib_I64_cscal( n-j, alpha, work( (j+1-j1+1)+jb*n ), 1_ilp64 )
                    ! k1 identifies if the previous column of the panel has been
                     ! explicitly stored, e.g., k1=0 and k2=1 for the first panel,
                     ! and k1=1 and k2=0 for the rest
                    if( j1>1_ilp64 ) then
                    ...
                    do j2 = j+1, n, nb
                       nj = min( nb, n-j2+1 )
                       ! update (j2, j2) diagonal block with stdlib_I64_cgemv
                       j3 = j2
                       do mj = nj-1, 1, -1
                          call stdlib_I64_cgemm( 'CONJUGATE TRANSPOSE', 'TRANSPOSE',1_ilp64, mj, jb+1,-cone,&
                           a( j1-k2, j3 ), lda,work( (j3-j1+1)+k1*n ), n,cone, a( j3, j3 ), lda )
                                     
                          j3 = j3 + 1_ilp64
                       end do
                       ! update off-diagonal block of j2-th block row with stdlib_I64_cgemm
                       call stdlib_I64_cgemm( 'CONJUGATE TRANSPOSE', 'TRANSPOSE',nj, n-j3+1, jb+1,-&
                       cone, a( j1-k2, j2 ), lda,work( (j3-j1+1)+k1*n ), n,cone, a( j2, j3 ), lda &
                                 )
                    end do
                    ! recover t( j, j+1 )
                 ...
                 ! work(j+1, 1) stores h(j+1, 1)
                 call stdlib_I64_ccopy( n-j, a( j+1, j+1 ), lda, work( 1_ilp64 ), 1_ilp64 )
              end if
              go to 10
           else
              ! .....................................................
              ...
              ! copy first column a(1:n, 1) into h(1:n, 1)
               ! (stored in work(1:n))
              call stdlib_I64_ccopy( n, a( 1_ilp64, 1_ilp64 ), 1_ilp64, work( 1_ilp64 ), 1_ilp64 )
              ! j is the main loop index, increasing from 1 to n in steps of
              ! jb, where jb is the number of columns factorized by stdlib_I64_clahef;
              ! jb is either nb, or n-j+1 for the last block
              j = 0_ilp64
              ...
              k1 = max(1_ilp64, j)-j
              ! panel factorization
              call stdlib_I64_clahef_aa( uplo, 2_ilp64-k1, n-j, jb,a( j+1, max(1_ilp64, j) ), lda,ipiv( j+1 ), &
                        work, n, work( n*nb+1 ) )
              ! adjust ipiv and apply it back (j-th step picks (j+1)-th pivot)
              do j2 = j+2, min(n, j+jb+1)
                 ipiv( j2 ) = ipiv( j2 ) + j
                 if( (j2/=ipiv(j2)) .and. ((j1-k1)>2_ilp64) ) then
                    call stdlib_I64_cswap( j1-k1-2, a( j2, 1_ilp64 ), lda,a( ipiv(j2), 1_ilp64 ), lda )
                 end if
              end do
              j = j + jb
              ! trailing submatrix update, where
              ...
              if( j<n ) then
                ! if the first panel and jb=1 (nb=1), then nothing to do
                 if( j1>1_ilp64 .or. jb>1_ilp64 ) then
                    ...
                    alpha = conjg( a( j+1, j ) )
                    a( j+1, j ) = cone
                    call stdlib_I64_ccopy( n-j, a( j+1, j-1 ), 1_ilp64,work( (j+1-j1+1)+jb*n ), 1_ilp64 )
                    call stdlib_I64_cscal( n-j, alpha, work( (j+1-j1+1)+jb*n ), 1_ilp64 )
                    ! k1 identifies if the previous column of the panel has been
                     ! explicitly stored, e.g., k1=0 and k2=1 for the first panel,
                     ! and k1=1 and k2=0 for the rest
                    ...
                    do j2 = j+1, n, nb
                       ...
                       j3 = j2
                       do mj = nj-1, 1, -1
                          call stdlib_I64_cgemm( 'NO TRANSPOSE', 'CONJUGATE TRANSPOSE',mj, 1_ilp64, jb+1,-&
                          cone, work( (j3-j1+1)+k1*n ), n,a( j3, j1-k2 ), lda,cone, a( j3, j3 ), &
                                    lda )
                          j3 = j3 + 1_ilp64
                       end do
                       ! update off-diagonal block of j2-th block column with stdlib_I64_cgemm
                       call stdlib_I64_cgemm( 'NO TRANSPOSE', 'CONJUGATE TRANSPOSE',n-j3+1, nj, jb+1,-&
                       cone, work( (j3-j1+1)+k1*n ), n,a( j2, j1-k2 ), lda,cone, a( j3, j2 ), lda &
                                 )
                    end do
                    ! recover t( j+1, j )
                 ...
                 ! work(j+1, 1) stores h(j+1, 1)
                 call stdlib_I64_ccopy( n-j, a( j+1, j+1 ), 1_ilp64, work( 1_ilp64 ), 1_ilp64 )
              end if
              go to 11
           end if
           20 continue
     ...
     !! This is the blocked version of the algorithm, calling Level 3 BLAS.
        ...
        ! =====================================================================
           ...
           ! Executable Statements 
           ! determine the block size
           nb = stdlib_I64_ilaenv( 1_ilp64, 'ZHETRF_AA', uplo, n, -1_ilp64, -1_ilp64, -1_ilp64 )
           ! test the input parameters.
           info = 0_ilp64
           upper = stdlib_lsame( uplo, 'U' )
           lquery = ( lwork==-1_ilp64 )
           if( .not.upper .and. .not.stdlib_lsame( uplo, 'L' ) ) then
              ...
              info = -4_ilp64
           else if( lwork<max( 1_ilp64, 2_ilp64*n ) .and. .not.lquery ) then
              info = -7_ilp64
           end if
           if( info==0_ilp64 ) then
              lwkopt = (nb+1)*n
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZHETRF_AA', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           if( upper ) then
              ...
              ! .....................................................
              ! copy first row a(1, 1:n) into h(1:n) (stored in work(1:n))
              call stdlib_I64_zcopy( n, a( 1_ilp64, 1_ilp64 ), lda, work( 1_ilp64 ), 1_ilp64 )
              ! j is the main loop index, increasing from 1 to n in steps of
              ! jb, where jb is the number of columns factorized by stdlib_I64_zlahef;
              ! jb is either nb, or n-j+1 for the last block
              j = 0_ilp64
              10 continue
              if( j>=n )go to 20
              ...
              k1 = max(1_ilp64, j)-j
              ! panel factorization
              call stdlib_I64_zlahef_aa( uplo, 2_ilp64-k1, n-j, jb,a( max(1_ilp64, j), j+1 ), lda,ipiv( j+1 ), &
                        work, n, work( n*nb+1 ) )
              ! adjust ipiv and apply it back (j-th step picks (j+1)-th pivot)
              do j2 = j+2, min(n, j+jb+1)
                 ipiv( j2 ) = ipiv( j2 ) + j
                 if( (j2/=ipiv(j2)) .and. ((j1-k1)>2_ilp64) ) then
                    call stdlib_I64_zswap( j1-k1-2, a( 1_ilp64, j2 ), 1_ilp64,a( 1_ilp64, ipiv(j2) ), 1_ilp64 )
                 end if
              end do
              j = j + jb
              ! trailing submatrix update, where
              ...
              if( j<n ) then
                ! if the first panel and jb=1 (nb=1), then nothing to do
                 if( j1>1_ilp64 .or. jb>1_ilp64 ) then
                    ...
                    alpha = conjg( a( j, j+1 ) )
                    a( j, j+1 ) = cone
                    call stdlib_I64_zcopy( n-j, a( j-1, j+1 ), lda,work( (j+1-j1+1)+jb*n ), 1_ilp64 )
                              
                    call stdlib_I64_zscal( n-j, alpha, work( (j+1-j1+1)+jb*n ), 1_ilp64 )
                    ! k1 identifies if the previous column of the panel has been
                     ! explicitly stored, e.g., k1=0 and k2=1 for the first panel,
                     ! and k1=1 and k2=0 for the rest
                    if( j1>1_ilp64 ) then
                    ...
                    do j2 = j+1, n, nb
                       nj = min( nb, n-j2+1 )
                       ! update (j2, j2) diagonal block with stdlib_I64_zgemv
                       j3 = j2
                       do mj = nj-1, 1, -1
                          call stdlib_I64_zgemm( 'CONJUGATE TRANSPOSE', 'TRANSPOSE',1_ilp64, mj, jb+1,-cone,&
                           a( j1-k2, j3 ), lda,work( (j3-j1+1)+k1*n ), n,cone, a( j3, j3 ), lda )
                                     
                          j3 = j3 + 1_ilp64
                       end do
                       ! update off-diagonal block of j2-th block row with stdlib_I64_zgemm
                       call stdlib_I64_zgemm( 'CONJUGATE TRANSPOSE', 'TRANSPOSE',nj, n-j3+1, jb+1,-&
                       cone, a( j1-k2, j2 ), lda,work( (j3-j1+1)+k1*n ), n,cone, a( j2, j3 ), lda &
                                 )
                    end do
                    ! recover t( j, j+1 )
                 ...
                 ! work(j+1, 1) stores h(j+1, 1)
                 call stdlib_I64_zcopy( n-j, a( j+1, j+1 ), lda, work( 1_ilp64 ), 1_ilp64 )
              end if
              go to 10
           else
              ! .....................................................
              ...
              ! copy first column a(1:n, 1) into h(1:n, 1)
               ! (stored in work(1:n))
              call stdlib_I64_zcopy( n, a( 1_ilp64, 1_ilp64 ), 1_ilp64, work( 1_ilp64 ), 1_ilp64 )
              ! j is the main loop index, increasing from 1 to n in steps of
              ! jb, where jb is the number of columns factorized by stdlib_I64_zlahef;
              ! jb is either nb, or n-j+1 for the last block
              j = 0_ilp64
              ...
              k1 = max(1_ilp64, j)-j
              ! panel factorization
              call stdlib_I64_zlahef_aa( uplo, 2_ilp64-k1, n-j, jb,a( j+1, max(1_ilp64, j) ), lda,ipiv( j+1 ), &
                        work, n, work( n*nb+1 ) )
              ! adjust ipiv and apply it back (j-th step picks (j+1)-th pivot)
              do j2 = j+2, min(n, j+jb+1)
                 ipiv( j2 ) = ipiv( j2 ) + j
                 if( (j2/=ipiv(j2)) .and. ((j1-k1)>2_ilp64) ) then
                    call stdlib_I64_zswap( j1-k1-2, a( j2, 1_ilp64 ), lda,a( ipiv(j2), 1_ilp64 ), lda )
                 end if
              end do
              j = j + jb
              ! trailing submatrix update, where
              ...
              if( j<n ) then
                ! if the first panel and jb=1 (nb=1), then nothing to do
                 if( j1>1_ilp64 .or. jb>1_ilp64 ) then
                    ...
                    alpha = conjg( a( j+1, j ) )
                    a( j+1, j ) = cone
                    call stdlib_I64_zcopy( n-j, a( j+1, j-1 ), 1_ilp64,work( (j+1-j1+1)+jb*n ), 1_ilp64 )
                    call stdlib_I64_zscal( n-j, alpha, work( (j+1-j1+1)+jb*n ), 1_ilp64 )
                    ! k1 identifies if the previous column of the panel has been
                     ! explicitly stored, e.g., k1=0 and k2=1 for the first panel,
                     ! and k1=1 and k2=0 for the rest
                    ...
                    do j2 = j+1, n, nb
                       ...
                       j3 = j2
                       do mj = nj-1, 1, -1
                          call stdlib_I64_zgemm( 'NO TRANSPOSE', 'CONJUGATE TRANSPOSE',mj, 1_ilp64, jb+1,-&
                          cone, work( (j3-j1+1)+k1*n ), n,a( j3, j1-k2 ), lda,cone, a( j3, j3 ), &
                                    lda )
                          j3 = j3 + 1_ilp64
                       end do
                       ! update off-diagonal block of j2-th block column with stdlib_I64_zgemm
                       call stdlib_I64_zgemm( 'NO TRANSPOSE', 'CONJUGATE TRANSPOSE',n-j3+1, nj, jb+1,-&
                       cone, work( (j3-j1+1)+k1*n ), n,a( j2, j1-k2 ), lda,cone, a( j3, j2 ), lda &
                                 )
                    end do
                    ! recover t( j+1, j )
                 ...
                 ! work(j+1, 1) stores h(j+1, 1)
                 call stdlib_I64_zcopy( n-j, a( j+1, j+1 ), 1_ilp64, work( 1_ilp64 ), 1_ilp64 )
              end if
              go to 11
           end if
           20 continue
     ...
     !! the diagonal and subdiagonal of A are overwritten by those of T.
        ...
        ! =====================================================================
           ...
           if( stdlib_lsame( uplo, 'U' ) ) then
              ...
              if ( j>min(m, nb) )go to 20
              ! k is the column to be factorized
               ! when being called from stdlib_I64_chetrf_aa,
               ! > for the first block column, j1 is 1, hence j1+j-1 is j,
               ! > for the rest of the columns, j1 is 2, and j1+j-1 is j+1,
              k = j1+j-1
              if( j==m ) then
              ...
              ! k is the column to be factorized
               ...
               ! > for the rest of the columns, k is j+1, skipping only the
                 ! first column
                 call stdlib_I64_clacgv( j-k1, a( 1_ilp64, j ), 1_ilp64 )
                 call stdlib_I64_cgemv( 'NO TRANSPOSE', mj, j-k1,-cone, h( j, k1 ), ldh,a( 1_ilp64, j ), 1_ilp64,&
                           cone, h( j, j ), 1_ilp64 )
                 call stdlib_I64_clacgv( j-k1, a( 1_ilp64, j ), 1_ilp64 )
              end if
              ! copy h(i:n, i) into work
              call stdlib_I64_ccopy( mj, h( j, j ), 1_ilp64, work( 1_ilp64 ), 1_ilp64 )
              if( j>k1 ) then
                 ! compute work := work - l(j-1, j:n) * t(j-1,j),
                  ! where a(j-1, j) stores t(j-1, j) and a(j-2, j:n) stores u(j-1, j:n)
                 alpha = -conjg( a( k-1, j ) )
                 call stdlib_I64_caxpy( mj, alpha, a( k-2, j ), lda, work( 1_ilp64 ), 1_ilp64 )
              end if
              ! set a(j, j) = t(j, j)
              a( k, j ) = real( work( 1_ilp64 ),KIND=sp)
              if( j<m ) then
                 ! compute work(2:n) = t(j, j) l(j, (j+1):n)
                  ! where a(j, j) stores t(j, j) and a(j-1, (j+1):n) stores u(j, (j+1):n)
                 if( k>1_ilp64 ) then
                    alpha = -a( k, j )
                    call stdlib_I64_caxpy( m-j, alpha, a( k-1, j+1 ), lda,work( 2_ilp64 ), 1_ilp64 )
                 endif
                 ! find max(|work(2:n)|)
                 i2 = stdlib_I64_icamax( m-j, work( 2_ilp64 ), 1_ilp64 ) + 1_ilp64
                 piv = work( i2 )
                 ! apply hermitian pivot
                 if( (i2/=2_ilp64) .and. (piv/=0_ilp64) ) then
                    ! swap work(i1) and work(i2)
                    i1 = 2_ilp64
                    work( i2 ) = work( i1 )
                    work( i1 ) = piv
                    ...
                    i1 = i1+j-1
                    i2 = i2+j-1
                    call stdlib_I64_cswap( i2-i1-1, a( j1+i1-1, i1+1 ), lda,a( j1+i1, i2 ), 1_ilp64 )
                              
                    call stdlib_I64_clacgv( i2-i1, a( j1+i1-1, i1+1 ), lda )
                    call stdlib_I64_clacgv( i2-i1-1, a( j1+i1, i2 ), 1_ilp64 )
                    ! swap a(i1, i2+1:n) with a(i2, i2+1:n)
                    if( i2<m )call stdlib_I64_cswap( m-i2, a( j1+i1-1, i2+1 ), lda,a( j1+i2-1, i2+1 ),&
                               lda )
                    ! swap a(i1, i1) with a(i2,i2)
                    piv = a( i1+j1-1, i1 )
                    a( j1+i1-1, i1 ) = a( j1+i2-1, i2 )
                    a( j1+i2-1, i2 ) = piv
                    ! swap h(i1, 1:j1) with h(i2, 1:j1)
                    call stdlib_I64_cswap( i1-1, h( i1, 1_ilp64 ), ldh, h( i2, 1_ilp64 ), ldh )
                    ipiv( i1 ) = i2
                    if( i1>(k1-1) ) then
                       ! swap l(1:i1-1, i1) with l(1:i1-1, i2),
                        ! skipping the first column
                       call stdlib_I64_cswap( i1-k1+1, a( 1_ilp64, i1 ), 1_ilp64,a( 1_ilp64, i2 ), 1_ilp64 )
                    end if
                 else
                    ipiv( j+1 ) = j+1
                 endif
                 ! set a(j, j+1) = t(j, j+1)
                 a( k, j+1 ) = work( 2_ilp64 )
                 if( j<nb ) then
                    ! copy a(j+1:n, j+1) into h(j:n, j),
                    call stdlib_I64_ccopy( m-j, a( k+1, j+1 ), lda,h( j+1, j+1 ), 1_ilp64 )
                 end if
                 ! compute l(j+2, j+1) = work( 3:n ) / t(j, j+1),
                  ! where a(j, j+1) = t(j, j+1) and a(j+2:n, j) = l(j+2:n, j+1)
                 if( j<(m-1) ) then
                    if( a( k, j+1 )/=czero ) then
                       alpha = cone / a( k, j+1 )
                       call stdlib_I64_ccopy( m-j-1, work( 3_ilp64 ), 1_ilp64, a( k, j+2 ), lda )
                       call stdlib_I64_cscal( m-j-1, alpha, a( k, j+2 ), lda )
                    else
                       call stdlib_I64_claset( 'FULL', 1_ilp64, m-j-1, czero, czero,a( k, j+2 ), lda)
              ...
              ! k is the column to be factorized
               ...
               ! > for the rest of the columns, k is j+1, skipping only the
                 ! first column
                 call stdlib_I64_clacgv( j-k1, a( j, 1_ilp64 ), lda )
                 call stdlib_I64_cgemv( 'NO TRANSPOSE', mj, j-k1,-cone, h( j, k1 ), ldh,a( j, 1_ilp64 ), &
                           lda,cone, h( j, j ), 1_ilp64 )
                 call stdlib_I64_clacgv( j-k1, a( j, 1_ilp64 ), lda )
              end if
              ! copy h(j:n, j) into work
              call stdlib_I64_ccopy( mj, h( j, j ), 1_ilp64, work( 1_ilp64 ), 1_ilp64 )
              if( j>k1 ) then
                 ! compute work := work - l(j:n, j-1) * t(j-1,j),
                  ! where a(j-1, j) = t(j-1, j) and a(j, j-2) = l(j, j-1)
                 alpha = -conjg( a( j, k-1 ) )
                 call stdlib_I64_caxpy( mj, alpha, a( j, k-2 ), 1_ilp64, work( 1_ilp64 ), 1_ilp64 )
              end if
              ! set a(j, j) = t(j, j)
              a( j, k ) = real( work( 1_ilp64 ),KIND=sp)
              if( j<m ) then
                 ! compute work(2:n) = t(j, j) l((j+1):n, j)
                  ! where a(j, j) = t(j, j) and a((j+1):n, j-1) = l((j+1):n, j)
                 if( k>1_ilp64 ) then
                    alpha = -a( j, k )
                    call stdlib_I64_caxpy( m-j, alpha, a( j+1, k-1 ), 1_ilp64,work( 2_ilp64 ), 1_ilp64 )
                 endif
                 ! find max(|work(2:n)|)
                 i2 = stdlib_I64_icamax( m-j, work( 2_ilp64 ), 1_ilp64 ) + 1_ilp64
                 piv = work( i2 )
                 ...
                 if( (i2/=2_ilp64) .and. (piv/=0_ilp64) ) then
                    ...
                    i1 = i1+j-1
                    i2 = i2+j-1
                    call stdlib_I64_cswap( i2-i1-1, a( i1+1, j1+i1-1 ), 1_ilp64,a( i2, j1+i1 ), lda )
                              
                    call stdlib_I64_clacgv( i2-i1, a( i1+1, j1+i1-1 ), 1_ilp64 )
                    call stdlib_I64_clacgv( i2-i1-1, a( i2, j1+i1 ), lda )
                    ! swap a(i2+1:n, i1) with a(i2+1:n, i2)
                    if( i2<m )call stdlib_I64_cswap( m-i2, a( i2+1, j1+i1-1 ), 1_ilp64,a( i2+1, j1+i2-1 ), &
                              1_ilp64 )
                    ! swap a(i1, i1) with a(i2, i2)
                    piv = a( i1, j1+i1-1 )
                    a( i1, j1+i1-1 ) = a( i2, j1+i2-1 )
                    ...
                    if( i1>(k1-1) ) then
                       ! swap l(1:i1-1, i1) with l(1:i1-1, i2),
                        ! skipping the first column
                       call stdlib_I64_cswap( i1-k1+1, a( i1, 1_ilp64 ), lda,a( i2, 1_ilp64 ), lda )
                    end if
                 else
                    ipiv( j+1 ) = j+1
                 endif
                 ! set a(j+1, j) = t(j+1, j)
                 a( j+1, k ) = work( 2_ilp64 )
                 if( j<nb ) then
                    ! copy a(j+1:n, j+1) into h(j+1:n, j),
                    call stdlib_I64_ccopy( m-j, a( j+1, k+1 ), 1_ilp64,h( j+1, j+1 ), 1_ilp64 )
                 end if
                 ! compute l(j+2, j+1) = work( 3:n ) / t(j, j+1),
                  ! where a(j, j+1) = t(j, j+1) and a(j+2:n, j) = l(j+2:n, j+1)
                 if( j<(m-1) ) then
                    if( a( j+1, k )/=czero ) then
                       alpha = cone / a( j+1, k )
                       call stdlib_I64_ccopy( m-j-1, work( 3_ilp64 ), 1_ilp64, a( j+2, k ), 1_ilp64 )
                       call stdlib_I64_cscal( m-j-1, alpha, a( j+2, k ), 1_ilp64 )
                    else
                       call stdlib_I64_claset( 'FULL', m-j-1, 1_ilp64, czero, czero,a( j+2, k ), lda )
     ...
     !! the diagonal and subdiagonal of A are overwritten by those of T.
        ...
        ! =====================================================================
           ...
           if( stdlib_lsame( uplo, 'U' ) ) then
              ...
              if ( j>min(m, nb) )go to 20
              ! k is the column to be factorized
               ! when being called from stdlib_I64_zhetrf_aa,
               ! > for the first block column, j1 is 1, hence j1+j-1 is j,
               ! > for the rest of the columns, j1 is 2, and j1+j-1 is j+1,
              k = j1+j-1
              if( j==m ) then
              ...
              ! k is the column to be factorized
               ...
               ! > for the rest of the columns, k is j+1, skipping only the
                 ! first column
                 call stdlib_I64_zlacgv( j-k1, a( 1_ilp64, j ), 1_ilp64 )
                 call stdlib_I64_zgemv( 'NO TRANSPOSE', mj, j-k1,-cone, h( j, k1 ), ldh,a( 1_ilp64, j ), 1_ilp64,&
                           cone, h( j, j ), 1_ilp64 )
                 call stdlib_I64_zlacgv( j-k1, a( 1_ilp64, j ), 1_ilp64 )
              end if
              ! copy h(i:n, i) into work
              call stdlib_I64_zcopy( mj, h( j, j ), 1_ilp64, work( 1_ilp64 ), 1_ilp64 )
              if( j>k1 ) then
                 ! compute work := work - l(j-1, j:n) * t(j-1,j),
                  ! where a(j-1, j) stores t(j-1, j) and a(j-2, j:n) stores u(j-1, j:n)
                 alpha = -conjg( a( k-1, j ) )
                 call stdlib_I64_zaxpy( mj, alpha, a( k-2, j ), lda, work( 1_ilp64 ), 1_ilp64 )
              end if
              ! set a(j, j) = t(j, j)
              a( k, j ) = real( work( 1_ilp64 ),KIND=dp)
              if( j<m ) then
                 ! compute work(2:n) = t(j, j) l(j, (j+1):n)
                  ! where a(j, j) stores t(j, j) and a(j-1, (j+1):n) stores u(j, (j+1):n)
                 if( k>1_ilp64 ) then
                    alpha = -a( k, j )
                    call stdlib_I64_zaxpy( m-j, alpha, a( k-1, j+1 ), lda,work( 2_ilp64 ), 1_ilp64 )
                 endif
                 ! find max(|work(2:n)|)
                 i2 = stdlib_I64_izamax( m-j, work( 2_ilp64 ), 1_ilp64 ) + 1_ilp64
                 piv = work( i2 )
                 ! apply hermitian pivot
                 if( (i2/=2_ilp64) .and. (piv/=0_ilp64) ) then
                    ! swap work(i1) and work(i2)
                    ...
                    i1 = i1+j-1
                    i2 = i2+j-1
                    call stdlib_I64_zswap( i2-i1-1, a( j1+i1-1, i1+1 ), lda,a( j1+i1, i2 ), 1_ilp64 )
                              
                    call stdlib_I64_zlacgv( i2-i1, a( j1+i1-1, i1+1 ), lda )
                    call stdlib_I64_zlacgv( i2-i1-1, a( j1+i1, i2 ), 1_ilp64 )
                    ! swap a(i1, i2+1:n) with a(i2, i2+1:n)
                    if( i2<m )call stdlib_I64_zswap( m-i2, a( j1+i1-1, i2+1 ), lda,a( j1+i2-1, i2+1 ),&
                               lda )
                    ! swap a(i1, i1) with a(i2,i2)
                    piv = a( i1+j1-1, i1 )
                    a( j1+i1-1, i1 ) = a( j1+i2-1, i2 )
                    a( j1+i2-1, i2 ) = piv
                    ! swap h(i1, 1:j1) with h(i2, 1:j1)
                    call stdlib_I64_zswap( i1-1, h( i1, 1_ilp64 ), ldh, h( i2, 1_ilp64 ), ldh )
                    ipiv( i1 ) = i2
                    if( i1>(k1-1) ) then
                       ! swap l(1:i1-1, i1) with l(1:i1-1, i2),
                        ! skipping the first column
                       call stdlib_I64_zswap( i1-k1+1, a( 1_ilp64, i1 ), 1_ilp64,a( 1_ilp64, i2 ), 1_ilp64 )
                    end if
                 else
                    ipiv( j+1 ) = j+1
                 endif
                 ...
                 if( j<nb ) then
                    ! copy a(j+1:n, j+1) into h(j:n, j),
                    call stdlib_I64_zcopy( m-j, a( k+1, j+1 ), lda,h( j+1, j+1 ), 1_ilp64 )
                 end if
                 ! compute l(j+2, j+1) = work( 3:n ) / t(j, j+1),
                  ! where a(j, j+1) = t(j, j+1) and a(j+2:n, j) = l(j+2:n, j+1)
                 if( j<(m-1) ) then
                    if( a( k, j+1 )/=czero ) then
                       alpha = cone / a( k, j+1 )
                       call stdlib_I64_zcopy( m-j-1, work( 3_ilp64 ), 1_ilp64, a( k, j+2 ), lda )
                       call stdlib_I64_zscal( m-j-1, alpha, a( k, j+2 ), lda )
                    else
                       call stdlib_I64_zlaset( 'FULL', 1_ilp64, m-j-1, czero, czero,a( k, j+2 ), lda)
              ...
              ! k is the column to be factorized
               ...
               ! > for the rest of the columns, k is j+1, skipping only the
                 ! first column
                 call stdlib_I64_zlacgv( j-k1, a( j, 1_ilp64 ), lda )
                 call stdlib_I64_zgemv( 'NO TRANSPOSE', mj, j-k1,-cone, h( j, k1 ), ldh,a( j, 1_ilp64 ), &
                           lda,cone, h( j, j ), 1_ilp64 )
                 call stdlib_I64_zlacgv( j-k1, a( j, 1_ilp64 ), lda )
              end if
              ! copy h(j:n, j) into work
              ...
              if( j>k1 ) then
                 ! compute work := work - l(j:n, j-1) * t(j-1,j),
                  ! where a(j-1, j) = t(j-1, j) and a(j, j-2) = l(j, j-1)
                 alpha = -conjg( a( j, k-1 ) )
                 call stdlib_I64_zaxpy( mj, alpha, a( j, k-2 ), 1_ilp64, work( 1_ilp64 ), 1_ilp64 )
              end if
              ! set a(j, j) = t(j, j)
              a( j, k ) = real( work( 1_ilp64 ),KIND=dp)
              if( j<m ) then
                 ! compute work(2:n) = t(j, j) l((j+1):n, j)
                  ! where a(j, j) = t(j, j) and a((j+1):n, j-1) = l((j+1):n, j)
                 if( k>1_ilp64 ) then
                    alpha = -a( j, k )
                    call stdlib_I64_zaxpy( m-j, alpha, a( j+1, k-1 ), 1_ilp64,work( 2_ilp64 ), 1_ilp64 )
                 endif
                 ! find max(|work(2:n)|)
                 i2 = stdlib_I64_izamax( m-j, work( 2_ilp64 ), 1_ilp64 ) + 1_ilp64
                 piv = work( i2 )
                 ...
                 if( (i2/=2_ilp64) .and. (piv/=0_ilp64) ) then
                    ...
                    i1 = i1+j-1
                    i2 = i2+j-1
                    call stdlib_I64_zswap( i2-i1-1, a( i1+1, j1+i1-1 ), 1_ilp64,a( i2, j1+i1 ), lda )
                              
                    call stdlib_I64_zlacgv( i2-i1, a( i1+1, j1+i1-1 ), 1_ilp64 )
                    call stdlib_I64_zlacgv( i2-i1-1, a( i2, j1+i1 ), lda )
                    ! swap a(i2+1:n, i1) with a(i2+1:n, i2)
                    if( i2<m )call stdlib_I64_zswap( m-i2, a( i2+1, j1+i1-1 ), 1_ilp64,a( i2+1, j1+i2-1 ), &
                              1_ilp64 )
                    ! swap a(i1, i1) with a(i2, i2)
                    piv = a( i1, j1+i1-1 )
                    a( i1, j1+i1-1 ) = a( i2, j1+i2-1 )
                    ...
                    if( i1>(k1-1) ) then
                       ! swap l(1:i1-1, i1) with l(1:i1-1, i2),
                        ! skipping the first column
                       call stdlib_I64_zswap( i1-k1+1, a( i1, 1_ilp64 ), lda,a( i2, 1_ilp64 ), lda )
                    end if
                 else
                    ipiv( j+1 ) = j+1
                 endif
                 ...
                 if( j<nb ) then
                    ! copy a(j+1:n, j+1) into h(j+1:n, j),
                    call stdlib_I64_zcopy( m-j, a( j+1, k+1 ), 1_ilp64,h( j+1, j+1 ), 1_ilp64 )
                 end if
                 ! compute l(j+2, j+1) = work( 3:n ) / t(j, j+1),
                  ! where a(j, j+1) = t(j, j+1) and a(j+2:n, j) = l(j+2:n, j+1)
                 if( j<(m-1) ) then
                    if( a( j+1, k )/=czero ) then
                       alpha = cone / a( j+1, k )
                       call stdlib_I64_zcopy( m-j-1, work( 3_ilp64 ), 1_ilp64, a( j+2, k ), 1_ilp64 )
                       call stdlib_I64_zscal( m-j-1, alpha, a( j+2, k ), 1_ilp64 )
                    else
                       call stdlib_I64_zlaset( 'FULL', m-j-1, 1_ilp64, czero, czero,a( j+2, k ), lda )
     ...
     !! A = L*T*L**H computed by CHETRF_AA.
        ...
        ! =====================================================================
           ...
           if( .not.upper .and. .not.stdlib_lsame( uplo, 'L' ) ) then
              ...
              info = -8_ilp64
           else if( lwork<max( 1_ilp64, 3_ilp64*n-2 ) .and. .not.lquery ) then
              info = -10_ilp64
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CHETRS_AA', -info )
              return
           else if( lquery ) then
              lwkopt = (3_ilp64*n-2)
              work( 1_ilp64 ) = lwkopt
           ...
           if( upper ) then
              ...
              if( n>1_ilp64 ) then
                 ...
                 ! compute u**h \ b -> b    [ (u**h \p**t * b) ]
                 call stdlib_I64_ctrsm( 'L', 'U', 'C', 'U', n-1, nrhs, cone, a( 1_ilp64, 2_ilp64 ),lda, b( 2_ilp64, 1_ilp64 ),&
                            ldb)
              end if
              ! 2) solve with triangular matrix t
              ! compute t \ b -> b   [ t \ (u**h \p**t * b) ]
              call stdlib_I64_clacpy( 'F', 1_ilp64, n, a(1_ilp64, 1_ilp64), lda+1, work(n), 1_ilp64)
              if( n>1_ilp64 ) then
                  call stdlib_I64_clacpy( 'F', 1_ilp64, n-1, a( 1_ilp64, 2_ilp64 ), lda+1, work( 2_ilp64*n ), 1_ilp64)
                  call stdlib_I64_clacpy( 'F', 1_ilp64, n-1, a( 1_ilp64, 2_ilp64 ), lda+1, work( 1_ilp64 ), 1_ilp64)
                  call stdlib_I64_clacgv( n-1, work( 1_ilp64 ), 1_ilp64 )
              end if
              call stdlib_I64_cgtsv(n, nrhs, work(1_ilp64), work(n), work(2_ilp64*n), b, ldb,info)
              ! 3) backward substitution with u
              if( n>1_ilp64 ) then
                 ! compute u \ b -> b   [ u \ (t \ (u**h \p**t * b) ) ]
                 call stdlib_I64_ctrsm( 'L', 'U', 'N', 'U', n-1, nrhs, cone, a( 1_ilp64, 2_ilp64 ),lda, b(2_ilp64, 1_ilp64), &
                           ldb)
                 ! pivot, p * b  -> b [ p * (u \ (t \ (u**h \p**t * b) )) ]
                 k = n
                 do while ( k>=1 )
              ...
              if( n>1_ilp64 ) then
                 ...
                 ! compute l \ b -> b    [ (l \p**t * b) ]
                 call stdlib_I64_ctrsm( 'L', 'L', 'N', 'U', n-1, nrhs, cone, a( 2_ilp64, 1_ilp64),lda, b(2_ilp64, 1_ilp64), &
                           ldb )
              end if
              ! 2) solve with triangular matrix t
              ! compute t \ b -> b   [ t \ (l \p**t * b) ]
              call stdlib_I64_clacpy( 'F', 1_ilp64, n, a(1_ilp64, 1_ilp64), lda+1, work(n), 1_ilp64)
              if( n>1_ilp64 ) then
                  call stdlib_I64_clacpy( 'F', 1_ilp64, n-1, a( 2_ilp64, 1_ilp64 ), lda+1, work( 1_ilp64 ), 1_ilp64 )
                  call stdlib_I64_clacpy( 'F', 1_ilp64, n-1, a( 2_ilp64, 1_ilp64 ), lda+1, work( 2_ilp64*n ), 1_ilp64)
                  call stdlib_I64_clacgv( n-1, work( 2_ilp64*n ), 1_ilp64 )
              end if
              call stdlib_I64_cgtsv(n, nrhs, work(1_ilp64), work(n), work(2_ilp64*n), b, ldb,info)
              ! 3) backward substitution with l**h
              if( n>1_ilp64 ) then
                 ! compute (l**h \ b) -> b   [ l**h \ (t \ (l \p**t * b) ) ]
                 call stdlib_I64_ctrsm( 'L', 'L', 'C', 'U', n-1, nrhs, cone, a( 2_ilp64, 1_ilp64 ),lda, b( 2_ilp64, 1_ilp64 ),&
                            ldb )
                 ! pivot, p * b -> b  [ p * (l**h \ (t \ (l \p**t * b) )) ]
                 k = n
                 do while ( k>=1 )
     ...
     !! A = L*T*L**H computed by ZHETRF_AA.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZHETRS_AA', -info )
              return
           else if( lquery ) then
              lwkopt = (3_ilp64*n-2)
              work( 1_ilp64 ) = lwkopt
           ...
           if( upper ) then
              ...
              if( n>1_ilp64 ) then
                 ...
                 ! compute u**h \ b -> b    [ (u**h \p**t * b) ]
                 call stdlib_I64_ztrsm( 'L', 'U', 'C', 'U', n-1, nrhs, cone, a( 1_ilp64, 2_ilp64 ),lda, b( 2_ilp64, 1_ilp64 ),&
                            ldb )
              end if
              ! 2) solve with triangular matrix t
              ! compute t \ b -> b   [ t \ (u**h \p**t * b) ]
              call stdlib_I64_zlacpy( 'F', 1_ilp64, n, a(1_ilp64, 1_ilp64), lda+1, work(n), 1_ilp64 )
              if( n>1_ilp64 ) then
                  call stdlib_I64_zlacpy( 'F', 1_ilp64, n-1, a( 1_ilp64, 2_ilp64 ), lda+1, work( 2_ilp64*n ), 1_ilp64)
                  call stdlib_I64_zlacpy( 'F', 1_ilp64, n-1, a( 1_ilp64, 2_ilp64 ), lda+1, work( 1_ilp64 ), 1_ilp64 )
                  call stdlib_I64_zlacgv( n-1, work( 1_ilp64 ), 1_ilp64 )
              end if
              call stdlib_I64_zgtsv( n, nrhs, work(1_ilp64), work(n), work(2_ilp64*n), b, ldb,info )
              ! 3) backward substitution with u
              if( n>1_ilp64 ) then
                 ! compute u \ b -> b   [ u \ (t \ (u**h \p**t * b) ) ]
                 call stdlib_I64_ztrsm( 'L', 'U', 'N', 'U', n-1, nrhs, cone, a( 1_ilp64, 2_ilp64 ),lda, b(2_ilp64, 1_ilp64), &
                           ldb)
                 ! pivot, p * b  [ p * (u**h \ (t \ (u \p**t * b) )) ]
                 do k = n, 1, -1
                    kp = ipiv( k )
              ...
              if( n>1_ilp64 ) then
                 ...
                 ! compute l \ b -> b    [ (l \p**t * b) ]
                 call stdlib_I64_ztrsm( 'L', 'L', 'N', 'U', n-1, nrhs, cone, a( 2_ilp64, 1_ilp64 ),lda, b(2_ilp64, 1_ilp64), &
                           ldb)
              end if
              ! 2) solve with triangular matrix t
              ! compute t \ b -> b   [ t \ (l \p**t * b) ]
              call stdlib_I64_zlacpy( 'F', 1_ilp64, n, a(1_ilp64, 1_ilp64), lda+1, work(n), 1_ilp64)
              if( n>1_ilp64 ) then
                  call stdlib_I64_zlacpy( 'F', 1_ilp64, n-1, a( 2_ilp64, 1_ilp64 ), lda+1, work( 1_ilp64 ), 1_ilp64)
                  call stdlib_I64_zlacpy( 'F', 1_ilp64, n-1, a( 2_ilp64, 1_ilp64 ), lda+1, work( 2_ilp64*n ), 1_ilp64)
                  call stdlib_I64_zlacgv( n-1, work( 2_ilp64*n ), 1_ilp64 )
              end if
              call stdlib_I64_zgtsv(n, nrhs, work(1_ilp64), work(n), work(2_ilp64*n), b, ldb,info)
              ! 3) backward substitution with l**h
              if( n>1_ilp64 ) then
                 ! compute l**h \ b -> b   [ l**h \ (t \ (l \p**t * b) ) ]
                 call stdlib_I64_ztrsm( 'L', 'L', 'C', 'U', n-1, nrhs, cone, a( 2_ilp64, 1_ilp64 ),lda, b( 2_ilp64, 1_ilp64 ),&
                            ldb)
                 ! pivot, p * b  [ p * (l**h \ (t \ (l \p**t * b) )) ]
                 do k = n, 1, -1
                    kp = ipiv( k )
