  ...
  contains
     ...
     !! B = (0 R)*Q,   A = Z*T*Q.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           logical(lk) :: lquery
           integer(ilp64) :: lopt, lwkmin, lwkopt, mn, nb, nb1, nb2, nb3, nb4, nr
           ! Intrinsic Functions 
           ! Executable Statements 
           ! test the input parameters
           info = 0_ilp64
           ...
           if( m<0_ilp64 ) then
              ...
              info = -2_ilp64
           else if( p<0_ilp64 .or. p>n .or. p<n-m ) then
              info = -3_ilp64
           ...
           if( info==0_ilp64) then
              if( n==0_ilp64 ) then
                 ...
                 nb3 = stdlib_I64_ilaenv( 1_ilp64, 'SORMQR', ' ', m, n, p, -1_ilp64 )
                 nb4 = stdlib_I64_ilaenv( 1_ilp64, 'SORMRQ', ' ', m, n, p, -1_ilp64 )
                 nb = max( nb1, nb2, nb3, nb4 )
                 lwkmin = m + n + p
                 lwkopt = p + mn + max( m, n )*nb
              end if
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SGGLSE', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           ! where t12 and r11 are upper triangular, and q and z are
           ! orthogonal.
           call stdlib_I64_sggrqf( p, m, n, b, ldb, work, a, lda, work( p+1 ),work( p+mn+1 ), lwork-p-&
                     mn, info )
           lopt = work( p+mn+1 )
           ! update c = z**t *c = ( c1 ) n-p
                                ! ( c2 ) m+p-n
           call stdlib_I64_sormqr( 'LEFT', 'TRANSPOSE', m, 1_ilp64, mn, a, lda, work( p+1 ),c, max( 1_ilp64, m ), &
                     work( p+mn+1 ), lwork-p-mn, info )
           lopt = max( lopt, int( work( p+mn+1 ),KIND=ilp64) )
           ! solve t12*x2 = d for x2
           if( p>0_ilp64 ) then
              call stdlib_I64_strtrs( 'UPPER', 'NO TRANSPOSE', 'NON-UNIT', p, 1_ilp64,b( 1_ilp64, n-p+1 ), ldb, d,&
                         p, info )
              if( info>0_ilp64 ) then
                 info = 1_ilp64
                 return
              ...
              ! put the solution in x
              call stdlib_I64_scopy( p, d, 1_ilp64, x( n-p+1 ), 1_ilp64 )
              ! update c1
              call stdlib_I64_sgemv( 'NO TRANSPOSE', n-p, p, -one, a( 1_ilp64, n-p+1 ), lda,d, 1_ilp64, one, c, 1_ilp64 &
                        )
           end if
           ! solve r11*x1 = c1 for x1
           if( n>p ) then
              call stdlib_I64_strtrs( 'UPPER', 'NO TRANSPOSE', 'NON-UNIT', n-p, 1_ilp64,a, lda, c, n-p, &
                        info )
              if( info>0_ilp64 ) then
                 info = 2_ilp64
                 return
              ...
              ! put the solutions in x
              call stdlib_I64_scopy( n-p, c, 1_ilp64, x, 1_ilp64 )
           end if
           ! compute the residual vector:
           if( m<n ) then
              nr = m + p - n
              if( nr>0_ilp64 )call stdlib_I64_sgemv( 'NO TRANSPOSE', nr, n-m, -one, a( n-p+1, m+1 ),lda, d( &
                        nr+1 ), 1_ilp64, one, c( n-p+1 ), 1_ilp64 )
           else
              nr = p
           end if
           if( nr>0_ilp64 ) then
              call stdlib_I64_strmv( 'UPPER', 'NO TRANSPOSE', 'NON UNIT', nr,a( n-p+1, n-p+1 ), lda, &
                        d, 1_ilp64 )
              call stdlib_I64_saxpy( nr, -one, d, 1_ilp64, c( n-p+1 ), 1_ilp64 )
           end if
           ! backward transformation x = q**t*x
           call stdlib_I64_sormrq( 'LEFT', 'TRANSPOSE', n, 1_ilp64, p, b, ldb, work( 1_ilp64 ), x,n, work( p+mn+1 &
                     ), lwork-p-mn, info )
           work( 1_ilp64 ) = p + mn + max( lopt, int( work( p+mn+1 ),KIND=ilp64) )
           return
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64) then
              if( n==0_ilp64 ) then
                 ...
                 nb3 = stdlib_I64_ilaenv( 1_ilp64, 'DORMQR', ' ', m, n, p, -1_ilp64 )
                 nb4 = stdlib_I64_ilaenv( 1_ilp64, 'DORMRQ', ' ', m, n, p, -1_ilp64 )
                 nb = max( nb1, nb2, nb3, nb4 )
                 lwkmin = m + n + p
                 lwkopt = p + mn + max( m, n )*nb
              end if
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DGGLSE', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           ! where t12 and r11 are upper triangular, and q and z are
           ! orthogonal.
           call stdlib_I64_dggrqf( p, m, n, b, ldb, work, a, lda, work( p+1 ),work( p+mn+1 ), lwork-p-&
                     mn, info )
           lopt = work( p+mn+1 )
           ! update c = z**t *c = ( c1 ) n-p
                                ! ( c2 ) m+p-n
           call stdlib_I64_dormqr( 'LEFT', 'TRANSPOSE', m, 1_ilp64, mn, a, lda, work( p+1 ),c, max( 1_ilp64, m ), &
                     work( p+mn+1 ), lwork-p-mn, info )
           lopt = max( lopt, int( work( p+mn+1 ),KIND=ilp64) )
           ! solve t12*x2 = d for x2
           if( p>0_ilp64 ) then
              call stdlib_I64_dtrtrs( 'UPPER', 'NO TRANSPOSE', 'NON-UNIT', p, 1_ilp64,b( 1_ilp64, n-p+1 ), ldb, d,&
                         p, info )
              if( info>0_ilp64 ) then
                 info = 1_ilp64
                 return
              ...
              ! put the solution in x
              call stdlib_I64_dcopy( p, d, 1_ilp64, x( n-p+1 ), 1_ilp64 )
              ! update c1
              call stdlib_I64_dgemv( 'NO TRANSPOSE', n-p, p, -one, a( 1_ilp64, n-p+1 ), lda,d, 1_ilp64, one, c, 1_ilp64 &
                        )
           end if
           ! solve r11*x1 = c1 for x1
           if( n>p ) then
              call stdlib_I64_dtrtrs( 'UPPER', 'NO TRANSPOSE', 'NON-UNIT', n-p, 1_ilp64,a, lda, c, n-p, &
                        info )
              if( info>0_ilp64 ) then
                 info = 2_ilp64
                 return
              ...
              ! put the solutions in x
              call stdlib_I64_dcopy( n-p, c, 1_ilp64, x, 1_ilp64 )
           end if
           ! compute the residual vector:
           if( m<n ) then
              nr = m + p - n
              if( nr>0_ilp64 )call stdlib_I64_dgemv( 'NO TRANSPOSE', nr, n-m, -one, a( n-p+1, m+1 ),lda, d( &
                        nr+1 ), 1_ilp64, one, c( n-p+1 ), 1_ilp64 )
           else
              nr = p
           end if
           if( nr>0_ilp64 ) then
              call stdlib_I64_dtrmv( 'UPPER', 'NO TRANSPOSE', 'NON UNIT', nr,a( n-p+1, n-p+1 ), lda, &
                        d, 1_ilp64 )
              call stdlib_I64_daxpy( nr, -one, d, 1_ilp64, c( n-p+1 ), 1_ilp64 )
           end if
           ! backward transformation x = q**t*x
           call stdlib_I64_dormrq( 'LEFT', 'TRANSPOSE', n, 1_ilp64, p, b, ldb, work( 1_ilp64 ), x,n, work( p+mn+1 &
                     ), lwork-p-mn, info )
           work( 1_ilp64 ) = p + mn + max( lopt, int( work( p+mn+1 ),KIND=ilp64) )
           return
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64) then
              if( n==0_ilp64 ) then
                 ...
                 nb3 = stdlib_I64_ilaenv( 1_ilp64, 'CUNMQR', ' ', m, n, p, -1_ilp64 )
                 nb4 = stdlib_I64_ilaenv( 1_ilp64, 'CUNMRQ', ' ', m, n, p, -1_ilp64 )
                 nb = max( nb1, nb2, nb3, nb4 )
                 lwkmin = m + n + p
                 lwkopt = p + mn + max( m, n )*nb
              end if
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CGGLSE', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           ! where t12 and r11 are upper triangular, and q and z are
           ! unitary.
           call stdlib_I64_cggrqf( p, m, n, b, ldb, work, a, lda, work( p+1 ),work( p+mn+1 ), lwork-p-&
                     mn, info )
           lopt = real( work( p+mn+1 ),KIND=sp)
           ! update c = z**h *c = ( c1 ) n-p
                             ! ( c2 ) m+p-n
           call stdlib_I64_cunmqr( 'LEFT', 'CONJUGATE TRANSPOSE', m, 1_ilp64, mn, a, lda,work( p+1 ), c, &
                     max( 1_ilp64, m ), work( p+mn+1 ),lwork-p-mn, info )
           lopt = max( lopt, int( work( p+mn+1 ),KIND=ilp64) )
           ! solve t12*x2 = d for x2
           if( p>0_ilp64 ) then
              call stdlib_I64_ctrtrs( 'UPPER', 'NO TRANSPOSE', 'NON-UNIT', p, 1_ilp64,b( 1_ilp64, n-p+1 ), ldb, d,&
                         p, info )
              if( info>0_ilp64 ) then
                 info = 1_ilp64
                 return
              ...
              ! put the solution in x
           call stdlib_I64_ccopy( p, d, 1_ilp64, x( n-p+1 ), 1_ilp64 )
              ! update c1
              call stdlib_I64_cgemv( 'NO TRANSPOSE', n-p, p, -cone, a( 1_ilp64, n-p+1 ), lda,d, 1_ilp64, cone, c, &
                        1_ilp64 )
           end if
           ! solve r11*x1 = c1 for x1
           if( n>p ) then
              call stdlib_I64_ctrtrs( 'UPPER', 'NO TRANSPOSE', 'NON-UNIT', n-p, 1_ilp64,a, lda, c, n-p, &
                        info )
              if( info>0_ilp64 ) then
                 info = 2_ilp64
                 return
              ...
              ! put the solutions in x
              call stdlib_I64_ccopy( n-p, c, 1_ilp64, x, 1_ilp64 )
           end if
           ! compute the residual vector:
           if( m<n ) then
              nr = m + p - n
              if( nr>0_ilp64 )call stdlib_I64_cgemv( 'NO TRANSPOSE', nr, n-m, -cone, a( n-p+1, m+1 ),lda, d(&
                         nr+1 ), 1_ilp64, cone, c( n-p+1 ), 1_ilp64 )
           else
              nr = p
           end if
           if( nr>0_ilp64 ) then
              call stdlib_I64_ctrmv( 'UPPER', 'NO TRANSPOSE', 'NON UNIT', nr,a( n-p+1, n-p+1 ), lda, &
                        d, 1_ilp64 )
              call stdlib_I64_caxpy( nr, -cone, d, 1_ilp64, c( n-p+1 ), 1_ilp64 )
           end if
           ! backward transformation x = q**h*x
           call stdlib_I64_cunmrq( 'LEFT', 'CONJUGATE TRANSPOSE', n, 1_ilp64, p, b, ldb,work( 1_ilp64 ), x, n, &
                     work( p+mn+1 ), lwork-p-mn, info )
           work( 1_ilp64 ) = p + mn + max( lopt, int( work( p+mn+1 ),KIND=ilp64) )
           return
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64) then
              if( n==0_ilp64 ) then
                 ...
                 nb3 = stdlib_I64_ilaenv( 1_ilp64, 'ZUNMQR', ' ', m, n, p, -1_ilp64 )
                 nb4 = stdlib_I64_ilaenv( 1_ilp64, 'ZUNMRQ', ' ', m, n, p, -1_ilp64 )
                 nb = max( nb1, nb2, nb3, nb4 )
                 lwkmin = m + n + p
                 lwkopt = p + mn + max( m, n )*nb
              end if
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZGGLSE', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           ! where t12 and r11 are upper triangular, and q and z are
           ! unitary.
           call stdlib_I64_zggrqf( p, m, n, b, ldb, work, a, lda, work( p+1 ),work( p+mn+1 ), lwork-p-&
                     mn, info )
           lopt = real( work( p+mn+1 ),KIND=dp)
           ! update c = z**h *c = ( c1 ) n-p
                             ! ( c2 ) m+p-n
           call stdlib_I64_zunmqr( 'LEFT', 'CONJUGATE TRANSPOSE', m, 1_ilp64, mn, a, lda,work( p+1 ), c, &
                     max( 1_ilp64, m ), work( p+mn+1 ),lwork-p-mn, info )
           lopt = max( lopt, int( work( p+mn+1 ),KIND=ilp64) )
           ! solve t12*x2 = d for x2
           if( p>0_ilp64 ) then
              call stdlib_I64_ztrtrs( 'UPPER', 'NO TRANSPOSE', 'NON-UNIT', p, 1_ilp64,b( 1_ilp64, n-p+1 ), ldb, d,&
                         p, info )
              if( info>0_ilp64 ) then
                 info = 1_ilp64
                 return
              ...
              ! put the solution in x
              call stdlib_I64_zcopy( p, d, 1_ilp64, x( n-p+1 ), 1_ilp64 )
              ! update c1
              call stdlib_I64_zgemv( 'NO TRANSPOSE', n-p, p, -cone, a( 1_ilp64, n-p+1 ), lda,d, 1_ilp64, cone, c, &
                        1_ilp64 )
           end if
           ! solve r11*x1 = c1 for x1
           if( n>p ) then
              call stdlib_I64_ztrtrs( 'UPPER', 'NO TRANSPOSE', 'NON-UNIT', n-p, 1_ilp64,a, lda, c, n-p, &
                        info )
              if( info>0_ilp64 ) then
                 info = 2_ilp64
                 return
              ...
              ! put the solutions in x
              call stdlib_I64_zcopy( n-p, c, 1_ilp64, x, 1_ilp64 )
           end if
           ! compute the residual vector:
           if( m<n ) then
              nr = m + p - n
              if( nr>0_ilp64 )call stdlib_I64_zgemv( 'NO TRANSPOSE', nr, n-m, -cone, a( n-p+1, m+1 ),lda, d(&
                         nr+1 ), 1_ilp64, cone, c( n-p+1 ), 1_ilp64 )
           else
              nr = p
           end if
           if( nr>0_ilp64 ) then
              call stdlib_I64_ztrmv( 'UPPER', 'NO TRANSPOSE', 'NON UNIT', nr,a( n-p+1, n-p+1 ), lda, &
                        d, 1_ilp64 )
              call stdlib_I64_zaxpy( nr, -cone, d, 1_ilp64, c( n-p+1 ), 1_ilp64 )
           end if
           ! backward transformation x = q**h*x
           call stdlib_I64_zunmrq( 'LEFT', 'CONJUGATE TRANSPOSE', n, 1_ilp64, p, b, ldb,work( 1_ilp64 ), x, n, &
                     work( p+mn+1 ), lwork-p-mn, info )
           work( 1_ilp64 ) = p + mn + max( lopt, int( work( p+mn+1 ),KIND=ilp64) )
           return
     ...
     !! where inv(B) denotes the inverse of B.
        ...
        ! ===================================================================
           ...
           ! Local Scalars 
           logical(lk) :: lquery
           integer(ilp64) :: i, lopt, lwkmin, lwkopt, nb, nb1, nb2, nb3, nb4, np
           ! Intrinsic Functions 
           ! Executable Statements 
           ! test the input parameters
           info = 0_ilp64
           ...
           if( n<0_ilp64 ) then
              ...
              info = -2_ilp64
           else if( p<0_ilp64 .or. p<n-m ) then
              info = -3_ilp64
           ...
           if( info==0_ilp64) then
              if( n==0_ilp64 ) then
                 ...
                 nb1 = stdlib_I64_ilaenv( 1_ilp64, 'SGEQRF', ' ', n, m, -1_ilp64, -1_ilp64 )
                 nb2 = stdlib_I64_ilaenv( 1_ilp64, 'SGERQF', ' ', n, m, -1_ilp64, -1_ilp64 )
                 nb3 = stdlib_I64_ilaenv( 1_ilp64, 'SORMQR', ' ', n, m, p, -1_ilp64 )
                 nb4 = stdlib_I64_ilaenv( 1_ilp64, 'SORMRQ', ' ', n, m, p, -1_ilp64 )
                 nb = max( nb1, nb2, nb3, nb4 )
                 lwkmin = m + n + p
                 lwkopt = m + np + max( n, p )*nb
              end if
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SGGGLM', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           ! where r11 and t22 are upper triangular, and q and z are
           ! orthogonal.
           call stdlib_I64_sggqrf( n, m, p, a, lda, work, b, ldb, work( m+1 ),work( m+np+1 ), lwork-m-&
                     np, info )
           lopt = work( m+np+1 )
           ! update left-hand-side vector d = q**t*d = ( d1 ) m
                                                     ! ( d2 ) n-m
           call stdlib_I64_sormqr( 'LEFT', 'TRANSPOSE', n, 1_ilp64, m, a, lda, work, d,max( 1_ilp64, n ), work( m+&
                     np+1 ), lwork-m-np, info )
           lopt = max( lopt, int( work( m+np+1 ),KIND=ilp64) )
           ! solve t22*y2 = d2 for y2
           if( n>m ) then
              call stdlib_I64_strtrs( 'UPPER', 'NO TRANSPOSE', 'NON UNIT', n-m, 1_ilp64,b( m+1, m+p-n+1 ), &
                        ldb, d( m+1 ), n-m, info )
              if( info>0_ilp64 ) then
                 info = 1_ilp64
                 return
              end if
              call stdlib_I64_scopy( n-m, d( m+1 ), 1_ilp64, y( m+p-n+1 ), 1_ilp64 )
           end if
           ! set y1 = 0
           do i = 1, m + p - n
              y( i ) = zero
           ...
           ! update d1 = d1 - t12*y2
           call stdlib_I64_sgemv( 'NO TRANSPOSE', m, n-m, -one, b( 1_ilp64, m+p-n+1 ), ldb,y( m+p-n+1 ), 1_ilp64, &
                     one, d, 1_ilp64 )
           ! solve triangular system: r11*x = d1
           if( m>0_ilp64 ) then
              call stdlib_I64_strtrs( 'UPPER', 'NO TRANSPOSE', 'NON UNIT', m, 1_ilp64, a, lda,d, m, info )
              ...
              ! copy d to x
              call stdlib_I64_scopy( m, d, 1_ilp64, x, 1_ilp64 )
           end if
           ! backward transformation y = z**t *y
           call stdlib_I64_sormrq( 'LEFT', 'TRANSPOSE', p, 1_ilp64, np,b( max( 1_ilp64, n-p+1 ), 1_ilp64 ), ldb, work( &
                     m+1 ), y,max( 1_ilp64, p ), work( m+np+1 ), lwork-m-np, info )
           work( 1_ilp64 ) = m + np + max( lopt, int( work( m+np+1 ),KIND=ilp64) )
           return
        ...
        ! ===================================================================
           ...
           if( info==0_ilp64) then
              if( n==0_ilp64 ) then
                 ...
                 nb1 = stdlib_I64_ilaenv( 1_ilp64, 'DGEQRF', ' ', n, m, -1_ilp64, -1_ilp64 )
                 nb2 = stdlib_I64_ilaenv( 1_ilp64, 'DGERQF', ' ', n, m, -1_ilp64, -1_ilp64 )
                 nb3 = stdlib_I64_ilaenv( 1_ilp64, 'DORMQR', ' ', n, m, p, -1_ilp64 )
                 nb4 = stdlib_I64_ilaenv( 1_ilp64, 'DORMRQ', ' ', n, m, p, -1_ilp64 )
                 nb = max( nb1, nb2, nb3, nb4 )
                 lwkmin = m + n + p
                 lwkopt = m + np + max( n, p )*nb
              end if
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DGGGLM', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           ! where r11 and t22 are upper triangular, and q and z are
           ! orthogonal.
           call stdlib_I64_dggqrf( n, m, p, a, lda, work, b, ldb, work( m+1 ),work( m+np+1 ), lwork-m-&
                     np, info )
           lopt = work( m+np+1 )
           ! update left-hand-side vector d = q**t*d = ( d1 ) m
                                                     ! ( d2 ) n-m
           call stdlib_I64_dormqr( 'LEFT', 'TRANSPOSE', n, 1_ilp64, m, a, lda, work, d,max( 1_ilp64, n ), work( m+&
                     np+1 ), lwork-m-np, info )
           lopt = max( lopt, int( work( m+np+1 ),KIND=ilp64) )
           ! solve t22*y2 = d2 for y2
           if( n>m ) then
              call stdlib_I64_dtrtrs( 'UPPER', 'NO TRANSPOSE', 'NON UNIT', n-m, 1_ilp64,b( m+1, m+p-n+1 ), &
                        ldb, d( m+1 ), n-m, info )
              if( info>0_ilp64 ) then
                 info = 1_ilp64
                 return
              end if
              call stdlib_I64_dcopy( n-m, d( m+1 ), 1_ilp64, y( m+p-n+1 ), 1_ilp64 )
           end if
           ! set y1 = 0
           do i = 1, m + p - n
              y( i ) = zero
           ...
           ! update d1 = d1 - t12*y2
           call stdlib_I64_dgemv( 'NO TRANSPOSE', m, n-m, -one, b( 1_ilp64, m+p-n+1 ), ldb,y( m+p-n+1 ), 1_ilp64, &
                     one, d, 1_ilp64 )
           ! solve triangular system: r11*x = d1
           if( m>0_ilp64 ) then
              call stdlib_I64_dtrtrs( 'UPPER', 'NO TRANSPOSE', 'NON UNIT', m, 1_ilp64, a, lda,d, m, info )
              ...
              ! copy d to x
              call stdlib_I64_dcopy( m, d, 1_ilp64, x, 1_ilp64 )
           end if
           ! backward transformation y = z**t *y
           call stdlib_I64_dormrq( 'LEFT', 'TRANSPOSE', p, 1_ilp64, np,b( max( 1_ilp64, n-p+1 ), 1_ilp64 ), ldb, work( &
                     m+1 ), y,max( 1_ilp64, p ), work( m+np+1 ), lwork-m-np, info )
           work( 1_ilp64 ) = m + np + max( lopt, int( work( m+np+1 ),KIND=ilp64) )
           return
        ...
        ! ===================================================================
           ...
           if( info==0_ilp64) then
              if( n==0_ilp64 ) then
                 ...
                 nb1 = stdlib_I64_ilaenv( 1_ilp64, 'CGEQRF', ' ', n, m, -1_ilp64, -1_ilp64 )
                 nb2 = stdlib_I64_ilaenv( 1_ilp64, 'CGERQF', ' ', n, m, -1_ilp64, -1_ilp64 )
                 nb3 = stdlib_I64_ilaenv( 1_ilp64, 'CUNMQR', ' ', n, m, p, -1_ilp64 )
                 nb4 = stdlib_I64_ilaenv( 1_ilp64, 'CUNMRQ', ' ', n, m, p, -1_ilp64 )
                 nb = max( nb1, nb2, nb3, nb4 )
                 lwkmin = m + n + p
                 lwkopt = m + np + max( n, p )*nb
              end if
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CGGGLM', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           ! where r11 and t22 are upper triangular, and q and z are
           ! unitary.
           call stdlib_I64_cggqrf( n, m, p, a, lda, work, b, ldb, work( m+1 ),work( m+np+1 ), lwork-m-&
                     np, info )
           lopt = real( work( m+np+1 ),KIND=sp)
           ! update left-hand-side vector d = q**h*d = ( d1 ) m
                                                     ! ( d2 ) n-m
           call stdlib_I64_cunmqr( 'LEFT', 'CONJUGATE TRANSPOSE', n, 1_ilp64, m, a, lda, work,d, max( 1_ilp64, n )&
                     , work( m+np+1 ), lwork-m-np, info )
           lopt = max( lopt, int( work( m+np+1 ),KIND=ilp64) )
           ! solve t22*y2 = d2 for y2
           if( n>m ) then
              call stdlib_I64_ctrtrs( 'UPPER', 'NO TRANSPOSE', 'NON UNIT', n-m, 1_ilp64,b( m+1, m+p-n+1 ), &
                        ldb, d( m+1 ), n-m, info )
              if( info>0_ilp64 ) then
                 info = 1_ilp64
                 return
              end if
              call stdlib_I64_ccopy( n-m, d( m+1 ), 1_ilp64, y( m+p-n+1 ), 1_ilp64 )
           end if
           ! set y1 = 0
           do i = 1, m + p - n
              y( i ) = czero
           ...
           ! update d1 = d1 - t12*y2
           call stdlib_I64_cgemv( 'NO TRANSPOSE', m, n-m, -cone, b( 1_ilp64, m+p-n+1 ), ldb,y( m+p-n+1 ), 1_ilp64,&
                      cone, d, 1_ilp64 )
           ! solve triangular system: r11*x = d1
           if( m>0_ilp64 ) then
              call stdlib_I64_ctrtrs( 'UPPER', 'NO TRANSPOSE', 'NON UNIT', m, 1_ilp64, a, lda,d, m, info )
              ...
              ! copy d to x
              call stdlib_I64_ccopy( m, d, 1_ilp64, x, 1_ilp64 )
           end if
           ! backward transformation y = z**h *y
           call stdlib_I64_cunmrq( 'LEFT', 'CONJUGATE TRANSPOSE', p, 1_ilp64, np,b( max( 1_ilp64, n-p+1 ), 1_ilp64 ), &
                     ldb, work( m+1 ), y,max( 1_ilp64, p ), work( m+np+1 ), lwork-m-np, info )
           work( 1_ilp64 ) = m + np + max( lopt, int( work( m+np+1 ),KIND=ilp64) )
           return
        ...
        ! ===================================================================
           ...
           if( info==0_ilp64) then
              if( n==0_ilp64 ) then
                 ...
                 nb1 = stdlib_I64_ilaenv( 1_ilp64, 'ZGEQRF', ' ', n, m, -1_ilp64, -1_ilp64 )
                 nb2 = stdlib_I64_ilaenv( 1_ilp64, 'ZGERQF', ' ', n, m, -1_ilp64, -1_ilp64 )
                 nb3 = stdlib_I64_ilaenv( 1_ilp64, 'ZUNMQR', ' ', n, m, p, -1_ilp64 )
                 nb4 = stdlib_I64_ilaenv( 1_ilp64, 'ZUNMRQ', ' ', n, m, p, -1_ilp64 )
                 nb = max( nb1, nb2, nb3, nb4 )
                 lwkmin = m + n + p
                 lwkopt = m + np + max( n, p )*nb
              end if
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZGGGLM', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           ! where r11 and t22 are upper triangular, and q and z are
           ! unitary.
           call stdlib_I64_zggqrf( n, m, p, a, lda, work, b, ldb, work( m+1 ),work( m+np+1 ), lwork-m-&
                     np, info )
           lopt = real( work( m+np+1 ),KIND=dp)
           ! update left-hand-side vector d = q**h*d = ( d1 ) m
                                                     ! ( d2 ) n-m
           call stdlib_I64_zunmqr( 'LEFT', 'CONJUGATE TRANSPOSE', n, 1_ilp64, m, a, lda, work,d, max( 1_ilp64, n )&
                     , work( m+np+1 ), lwork-m-np, info )
           lopt = max( lopt, int( work( m+np+1 ),KIND=ilp64) )
           ! solve t22*y2 = d2 for y2
           if( n>m ) then
              call stdlib_I64_ztrtrs( 'UPPER', 'NO TRANSPOSE', 'NON UNIT', n-m, 1_ilp64,b( m+1, m+p-n+1 ), &
                        ldb, d( m+1 ), n-m, info )
              if( info>0_ilp64 ) then
                 info = 1_ilp64
                 return
              end if
              call stdlib_I64_zcopy( n-m, d( m+1 ), 1_ilp64, y( m+p-n+1 ), 1_ilp64 )
           end if
           ! set y1 = 0
           do i = 1, m + p - n
              y( i ) = czero
           ...
           ! update d1 = d1 - t12*y2
           call stdlib_I64_zgemv( 'NO TRANSPOSE', m, n-m, -cone, b( 1_ilp64, m+p-n+1 ), ldb,y( m+p-n+1 ), 1_ilp64,&
                      cone, d, 1_ilp64 )
           ! solve triangular system: r11*x = d1
           if( m>0_ilp64 ) then
              call stdlib_I64_ztrtrs( 'UPPER', 'NO TRANSPOSE', 'NON UNIT', m, 1_ilp64, a, lda,d, m, info )
              ...
              ! copy d to x
              call stdlib_I64_zcopy( m, d, 1_ilp64, x, 1_ilp64 )
           end if
           ! backward transformation y = z**h *y
           call stdlib_I64_zunmrq( 'LEFT', 'CONJUGATE TRANSPOSE', p, 1_ilp64, np,b( max( 1_ilp64, n-p+1 ), 1_ilp64 ), &
                     ldb, work( m+1 ), y,max( 1_ilp64, p ), work( m+np+1 ), lwork-m-np, info )
           work( 1_ilp64 ) = m + np + max( lopt, int( work( m+np+1 ),KIND=ilp64) )
           return
