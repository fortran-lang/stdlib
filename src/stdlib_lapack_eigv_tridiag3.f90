  ...
  contains
     ...
     !! real symmetric tridiagonal matrix A.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SSTEV ', -info )
              return
           end if
           ! quick return if possible
           if( n==0 )return
           if( n==1_ilp64 ) then
              if( wantz )z( 1_ilp64, 1_ilp64 ) = one
              return
           end if
           ! get machine constants.
           ...
           ! scale matrix to allowable range, if necessary.
           iscale = 0_ilp64
           tnrm = stdlib_I64_slanst( 'M', n, d, e )
           if( tnrm>zero .and. tnrm<rmin ) then
              iscale = 1_ilp64
              sigma = rmin / tnrm
           else if( tnrm>rmax ) then
           ...
           if( iscale==1_ilp64 ) then
              call stdlib_I64_sscal( n, sigma, d, 1_ilp64 )
              call stdlib_I64_sscal( n-1, sigma, e( 1_ilp64 ), 1_ilp64 )
           end if
           ! for eigenvalues only, call stdlib_I64_ssterf.  for eigenvalues and
           ! eigenvectors, call stdlib_I64_ssteqr.
           if( .not.wantz ) then
              call stdlib_I64_ssterf( n, d, e, info )
           else
              call stdlib_I64_ssteqr( 'I', n, d, e, z, ldz, work, info )
           end if
           ! if matrix was scaled, then rescale eigenvalues appropriately.
           if( iscale==1_ilp64 ) then
              if( info==0_ilp64 ) then
              ...
              call stdlib_I64_sscal( imax, one / sigma, d, 1_ilp64 )
           end if
           return
     ...
     !! real symmetric tridiagonal matrix A.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DSTEV ', -info )
              return
           end if
           ! quick return if possible
           if( n==0 )return
           ...
           ! scale matrix to allowable range, if necessary.
           iscale = 0_ilp64
           tnrm = stdlib_I64_dlanst( 'M', n, d, e )
           if( tnrm>zero .and. tnrm<rmin ) then
              iscale = 1_ilp64
              sigma = rmin / tnrm
           else if( tnrm>rmax ) then
           ...
           if( iscale==1_ilp64 ) then
              call stdlib_I64_dscal( n, sigma, d, 1_ilp64 )
              call stdlib_I64_dscal( n-1, sigma, e( 1_ilp64 ), 1_ilp64 )
           end if
           ! for eigenvalues only, call stdlib_I64_dsterf.  for eigenvalues and
           ! eigenvectors, call stdlib_I64_dsteqr.
           if( .not.wantz ) then
              call stdlib_I64_dsterf( n, d, e, info )
           else
              call stdlib_I64_dsteqr( 'I', n, d, e, z, ldz, work, info )
           end if
           ! if matrix was scaled, then rescale eigenvalues appropriately.
           if( iscale==1_ilp64 ) then
              if( info==0_ilp64 ) then
              ...
              call stdlib_I64_dscal( imax, one / sigma, d, 1_ilp64 )
           end if
           return
     ...
     !! without guard digits, but we know of none.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           logical(lk) :: lquery, wantz
           integer(ilp64) :: iscale, liwmin, lwmin
           real(sp) :: bignum, eps, rmax, rmin, safmin, sigma, smlnum, tnrm
           ! Intrinsic Functions 
           ! Executable Statements 
           ! test the input parameters.
           ...
           liwmin = 1_ilp64
           lwmin = 1_ilp64
           if( n>1_ilp64 .and. wantz ) then
              lwmin = 1_ilp64 + 4_ilp64*n + n**2_ilp64
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SSTEVD', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           ! for eigenvalues only, call stdlib_I64_ssterf.  for eigenvalues and
           ! eigenvectors, call stdlib_I64_sstedc.
           if( .not.wantz ) then
              call stdlib_I64_ssterf( n, d, e, info )
           else
              call stdlib_I64_sstedc( 'I', n, d, e, z, ldz, work, lwork, iwork, liwork,info )
           end if
           ! if matrix was scaled, then rescale eigenvalues appropriately.
           if( iscale==1_ilp64 )call stdlib_I64_sscal( n, one / sigma, d, 1_ilp64 )
           work( 1_ilp64 ) = lwmin
           iwork( 1_ilp64 ) = liwmin
           return
     ...
     !! without guard digits, but we know of none.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DSTEVD', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           ! for eigenvalues only, call stdlib_I64_dsterf.  for eigenvalues and
           ! eigenvectors, call stdlib_I64_dstedc.
           if( .not.wantz ) then
              call stdlib_I64_dsterf( n, d, e, info )
           else
              call stdlib_I64_dstedc( 'I', n, d, e, z, ldz, work, lwork, iwork, liwork,info )
           end if
           ! if matrix was scaled, then rescale eigenvalues appropriately.
           if( iscale==1_ilp64 )call stdlib_I64_dscal( n, one / sigma, d, 1_ilp64 )
           work( 1_ilp64 ) = lwmin
           iwork( 1_ilp64 ) = liwmin
           return
     ...
     !! manner.
        ...
        ! =====================================================================
           ...
           logical(lk) :: alleig, indeig, test, lquery, valeig, wantz, tryrac
           character :: order
           integer(ilp64) :: i, ieeeok, imax, indibl, indifl, indisp, indiwo, iscale, j, jj, liwmin,&
                      lwmin, nsplit
           real(sp) :: bignum, eps, rmax, rmin, safmin, sigma, smlnum, tmp1, tnrm, vll, &
                     vuu
           ! Intrinsic Functions 
           ! Executable Statements 
           ! test the input parameters.
           ieeeok = stdlib_I64_ilaenv( 10_ilp64, 'SSTEVR', 'N', 1_ilp64, 2_ilp64, 3_ilp64, 4_ilp64 )
           wantz = stdlib_lsame( jobz, 'V' )
           alleig = stdlib_lsame( range, 'A' )
           valeig = stdlib_lsame( range, 'V' )
           indeig = stdlib_lsame( range, 'I' )
           lquery = ( ( lwork==-1_ilp64 ) .or. ( liwork==-1_ilp64 ) )
           lwmin = max( 1_ilp64, 20_ilp64*n )
           liwmin = max(1_ilp64, 10_ilp64*n )
           info = 0_ilp64
           if( .not.( wantz .or. stdlib_lsame( jobz, 'N' ) ) ) then
              info = -1_ilp64
           else if( .not.( alleig .or. valeig .or. indeig ) ) then
              ...
              if( valeig ) then
                 if( n>0_ilp64 .and. vu<=vl )info = -7_ilp64
              else if( indeig ) then
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SSTEVR', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           if( n==1_ilp64 ) then
              if( alleig .or. indeig ) then
                 ...
                 if( vl<d( 1_ilp64 ) .and. vu>=d( 1_ilp64 ) ) then
                    m = 1_ilp64
                    w( 1_ilp64 ) = d( 1_ilp64 )
                 end if
              end if
           ...
           ! initialize indices into workspaces.  note: these indices are used only
           ! if stdlib_I64_ssterf or stdlib_I64_sstemr fail.
           ! iwork(indibl:indibl+m-1) corresponds to iblock in stdlib_I64_sstebz and
           ! stores the block indices of each of the m<=n eigenvalues.
           indibl = 1_ilp64
           ! iwork(indisp:indisp+nsplit-1) corresponds to isplit in stdlib_I64_sstebz and
           ! stores the starting and finishing indices of each block.
           indisp = indibl + n
           ! iwork(indifl:indifl+n-1) stores the indices of eigenvectors
           ! that corresponding to eigenvectors that fail to converge in
           ! stdlib_I64_sstein.  this information is discarded; if any fail, the driver
           ! returns info > 0.
           indifl = indisp + n
           ! indiwo is the offset of the remaining integer workspace.
           indiwo = indisp + n
           ! if all eigenvalues are desired, then
           ! call stdlib_I64_ssterf or stdlib_I64_sstemr.  if this fails for some eigenvalue, then
           ! try stdlib_I64_sstebz.
           test = .false.
           if( indeig ) then
           ...
           if( ( alleig .or. test ) .and. ieeeok==1_ilp64 ) then
              call stdlib_I64_scopy( n-1, e( 1_ilp64 ), 1_ilp64, work( 1_ilp64 ), 1_ilp64 )
              if( .not.wantz ) then
                 call stdlib_I64_scopy( n, d, 1_ilp64, w, 1_ilp64 )
                 call stdlib_I64_ssterf( n, w, work, info )
              else
                 call stdlib_I64_scopy( n, d, 1_ilp64, work( n+1 ), 1_ilp64 )
                 if (abstol <= two*n*eps) then
                    tryrac = .true.
                 else
                    tryrac = .false.
                 end if
                 call stdlib_I64_sstemr( jobz, 'A', n, work( n+1 ), work, vl, vu, il,iu, m, w, z, ldz,&
                            n, isuppz, tryrac,work( 2_ilp64*n+1 ), lwork-2*n, iwork, liwork, info )
              end if
              if( info==0_ilp64 ) then
                 m = n
                 go to 10
           ...
           ! otherwise, call stdlib_I64_sstebz and, if eigenvectors are desired, stdlib_I64_sstein.
           if( wantz ) then
              order = 'B'
           else
              order = 'E'
           end if
           call stdlib_I64_sstebz( range, order, n, vll, vuu, il, iu, abstol, d, e, m,nsplit, w, &
                     iwork( indibl ), iwork( indisp ), work,iwork( indiwo ), info )
           if( wantz ) then
              call stdlib_I64_sstein( n, d, e, m, w, iwork( indibl ), iwork( indisp ),z, ldz, work, &
                        iwork( indiwo ), iwork( indifl ),info )
           end if
           ! if matrix was scaled, then rescale eigenvalues appropriately.
           10 continue
           if( iscale==1_ilp64 ) then
              ...
              call stdlib_I64_sscal( imax, one / sigma, w, 1_ilp64 )
           end if
           ! if eigenvalues are not in order, then sort them, along with
           ! eigenvectors.
           if( wantz ) then
     ...
     !! manner.
        ...
        ! =====================================================================
           ...
           logical(lk) :: alleig, indeig, test, lquery, valeig, wantz, tryrac
           character :: order
           integer(ilp64) :: i, ieeeok, imax, indibl, indifl, indisp, indiwo, iscale, itmp1, j, jj, &
                     liwmin, lwmin, nsplit
           real(dp) :: bignum, eps, rmax, rmin, safmin, sigma, smlnum, tmp1, tnrm, vll, &
                     vuu
           ! Intrinsic Functions 
           ! Executable Statements 
           ! test the input parameters.
           ieeeok = stdlib_I64_ilaenv( 10_ilp64, 'DSTEVR', 'N', 1_ilp64, 2_ilp64, 3_ilp64, 4_ilp64 )
           wantz = stdlib_lsame( jobz, 'V' )
           alleig = stdlib_lsame( range, 'A' )
           valeig = stdlib_lsame( range, 'V' )
           indeig = stdlib_lsame( range, 'I' )
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DSTEVR', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           ! initialize indices into workspaces.  note: these indices are used only
           ! if stdlib_I64_dsterf or stdlib_I64_dstemr fail.
           ! iwork(indibl:indibl+m-1) corresponds to iblock in stdlib_I64_dstebz and
           ! stores the block indices of each of the m<=n eigenvalues.
           indibl = 1_ilp64
           ! iwork(indisp:indisp+nsplit-1) corresponds to isplit in stdlib_I64_dstebz and
           ! stores the starting and finishing indices of each block.
           indisp = indibl + n
           ! iwork(indifl:indifl+n-1) stores the indices of eigenvectors
           ! that corresponding to eigenvectors that fail to converge in
           ! stdlib_I64_dstein.  this information is discarded; if any fail, the driver
           ! returns info > 0.
           indifl = indisp + n
           ! indiwo is the offset of the remaining integer workspace.
           indiwo = indisp + n
           ! if all eigenvalues are desired, then
           ! call stdlib_I64_dsterf or stdlib_I64_dstemr.  if this fails for some eigenvalue, then
           ! try stdlib_I64_dstebz.
           test = .false.
           if( indeig ) then
              if( il==1_ilp64 .and. iu==n ) then
                 test = .true.
           ...
           if( ( alleig .or. test ) .and. ieeeok==1_ilp64 ) then
              call stdlib_I64_dcopy( n-1, e( 1_ilp64 ), 1_ilp64, work( 1_ilp64 ), 1_ilp64 )
              if( .not.wantz ) then
                 call stdlib_I64_dcopy( n, d, 1_ilp64, w, 1_ilp64 )
                 call stdlib_I64_dsterf( n, w, work, info )
              else
                 call stdlib_I64_dcopy( n, d, 1_ilp64, work( n+1 ), 1_ilp64 )
                 if (abstol <= two*n*eps) then
                    tryrac = .true.
                 else
                    tryrac = .false.
                 end if
                 call stdlib_I64_dstemr( jobz, 'A', n, work( n+1 ), work, vl, vu, il,iu, m, w, z, ldz,&
                            n, isuppz, tryrac,work( 2_ilp64*n+1 ), lwork-2*n, iwork, liwork, info )
              end if
              if( info==0_ilp64 ) then
                 m = n
           ...
           ! otherwise, call stdlib_I64_dstebz and, if eigenvectors are desired, stdlib_I64_dstein.
           if( wantz ) then
              order = 'B'
           else
              order = 'E'
           end if
           call stdlib_I64_dstebz( range, order, n, vll, vuu, il, iu, abstol, d, e, m,nsplit, w, &
                     iwork( indibl ), iwork( indisp ), work,iwork( indiwo ), info )
           if( wantz ) then
              call stdlib_I64_dstein( n, d, e, m, w, iwork( indibl ), iwork( indisp ),z, ldz, work, &
                        iwork( indiwo ), iwork( indifl ),info )
           end if
           ! if matrix was scaled, then rescale eigenvalues appropriately.
           10 continue
           if( iscale==1_ilp64 ) then
              ...
              call stdlib_I64_dscal( imax, one / sigma, w, 1_ilp64 )
           end if
           ! if eigenvalues are not in order, then sort them, along with
           ! eigenvectors.
           if( wantz ) then
     ...
     !! or a range of indices for the desired eigenvalues.
        ...
        ! =====================================================================
           ...
           logical(lk) :: alleig, indeig, test, valeig, wantz
           character :: order
           integer(ilp64) :: i, imax, indibl, indisp, indiwo, indwrk, iscale, itmp1, j, jj, &
                     nsplit
           real(sp) :: bignum, eps, rmax, rmin, safmin, sigma, smlnum, tmp1, tnrm, vll, &
                     vuu
           ! Intrinsic Functions 
           ...
           if( info==0_ilp64 ) then
              if( ldz<1_ilp64 .or. ( wantz .and. ldz<n ) )info = -14_ilp64
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SSTEVX', -info )
              return
           end if
           ! quick return if possible
           m = 0_ilp64
           ...
           ! if all eigenvalues are desired and abstol is less than zero, then
           ! call stdlib_I64_ssterf or stdlib_I64_ssteqr.  if this fails for some eigenvalue, then
           ! try stdlib_I64_sstebz.
           test = .false.
           if( indeig ) then
              if( il==1_ilp64 .and. iu==n ) then
           ...
           if( ( alleig .or. test ) .and. ( abstol<=zero ) ) then
              ...
              if( .not.wantz ) then
                 ...
                 call stdlib_I64_ssteqr( 'I', n, w, work, z, ldz, work( indwrk ), info )
                 if( info==0_ilp64 ) then
                    do i = 1, n
                       ifail( i ) = 0_ilp64
                    end do
     ...
     !! or a range of indices for the desired eigenvalues.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DSTEVX', -info )
              return
           end if
           ! quick return if possible
           m = 0_ilp64
           ...
           ! if all eigenvalues are desired and abstol is less than zero, then
           ! call stdlib_I64_dsterf or stdlib_I64_ssteqr.  if this fails for some eigenvalue, then
           ! try stdlib_I64_dstebz.
           test = .false.
           if( indeig ) then
              if( il==1_ilp64 .and. iu==n ) then
           ...
           if( ( alleig .or. test ) .and. ( abstol<=zero ) ) then
              ...
              if( .not.wantz ) then
                 ...
                 call stdlib_I64_dsteqr( 'I', n, w, work, z, ldz, work( indwrk ), info )
                 if( info==0_ilp64 ) then
                    do i = 1, n
                       ifail( i ) = 0_ilp64
                    end do
           ...
           ! otherwise, call stdlib_I64_dstebz and, if eigenvectors are desired, stdlib_I64_sstein.
           if( wantz ) then
              order = 'B'
           else
              order = 'E'
     ...
     !! these eigenvalues range over many orders of magnitude.)
        ...
        ! =====================================================================
           ...
           ! Local Arrays 
           real(sp) :: c(1_ilp64,1_ilp64), vt(1_ilp64,1_ilp64)
           ! Local Scalars 
           integer(ilp64) :: i, icompz, nru
           ! Intrinsic Functions 
           ! Executable Statements 
           ! test the input parameters.
           info = 0_ilp64
           ...
           if( icompz<0_ilp64 ) then
              info = -1_ilp64
           else if( n<0_ilp64 ) then
              info = -2_ilp64
           else if( ( ldz<1_ilp64 ) .or. ( icompz>0_ilp64 .and. ldz<max( 1_ilp64,n ) ) ) then
              info = -6_ilp64
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SPTEQR', -info )
              return
           end if
           ! quick return if possible
           if( n==0 )return
           if( n==1_ilp64 ) then
              if( icompz>0_ilp64 )z( 1_ilp64, 1_ilp64 ) = one
              return
           end if
           if( icompz==2_ilp64 )call stdlib_I64_slaset( 'FULL', n, n, zero, one, z, ldz )
           ! call stdlib_I64_spttrf to factor the matrix.
           ...
           ! call stdlib_I64_sbdsqr to compute the singular values/vectors of the
           ! bidiagonal factor.
           if( icompz>0_ilp64 ) then
              nru = n
           else
           ...
           call stdlib_I64_sbdsqr( 'LOWER', n, 0_ilp64, nru, 0_ilp64, d, e, vt, 1_ilp64, z, ldz, c, 1_ilp64,work, info )
     ...
     !! these eigenvalues range over many orders of magnitude.)
        ...
        ! =====================================================================
           ...
           ! Local Arrays 
           real(dp) :: c(1_ilp64,1_ilp64), vt(1_ilp64,1_ilp64)
           ! Local Scalars 
           integer(ilp64) :: i, icompz, nru
           ! Intrinsic Functions 
           ! Executable Statements 
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DPTEQR', -info )
              return
           end if
           ! quick return if possible
           if( n==0 )return
           ...
           if( icompz==2_ilp64 )call stdlib_I64_dlaset( 'FULL', n, n, zero, one, z, ldz )
           ! call stdlib_I64_dpttrf to factor the matrix.
           ...
           ! call stdlib_I64_dbdsqr to compute the singular values/vectors of the
           ! bidiagonal factor.
           if( icompz>0_ilp64 ) then
              nru = n
           else
           ...
           call stdlib_I64_dbdsqr( 'LOWER', n, 0_ilp64, nru, 0_ilp64, d, e, vt, 1_ilp64, z, ldz, c, 1_ilp64,work, info )
     ...
     !! matrix, if these eigenvalues range over many orders of magnitude.)
        ...
        ! ====================================================================
           ...
           ! Local Arrays 
           complex(sp) :: c(1_ilp64,1_ilp64), vt(1_ilp64,1_ilp64)
           ! Local Scalars 
           integer(ilp64) :: i, icompz, nru
           ! Intrinsic Functions 
           ! Executable Statements 
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CPTEQR', -info )
              return
           end if
           ! quick return if possible
           if( n==0 )return
           if( n==1_ilp64 ) then
              if( icompz>0_ilp64 )z( 1_ilp64, 1_ilp64 ) = cone
              return
           end if
           if( icompz==2_ilp64 )call stdlib_I64_claset( 'FULL', n, n, czero, cone, z, ldz )
           ! call stdlib_I64_spttrf to factor the matrix.
           call stdlib_I64_spttrf( n, d, e, info )
           if( info/=0 )return
           do i = 1, n
           ...
           ! call stdlib_I64_cbdsqr to compute the singular values/vectors of the
           ! bidiagonal factor.
           if( icompz>0_ilp64 ) then
              nru = n
           else
           ...
           call stdlib_I64_cbdsqr( 'LOWER', n, 0_ilp64, nru, 0_ilp64, d, e, vt, 1_ilp64, z, ldz, c, 1_ilp64,work, info )
     ...
     !! matrix, if these eigenvalues range over many orders of magnitude.)
        ...
        ! ====================================================================
           ...
           ! Local Arrays 
           complex(dp) :: c(1_ilp64,1_ilp64), vt(1_ilp64,1_ilp64)
           ! Local Scalars 
           integer(ilp64) :: i, icompz, nru
           ! Intrinsic Functions 
           ! Executable Statements 
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZPTEQR', -info )
              return
           end if
           ! quick return if possible
           if( n==0 )return
           ...
           if( icompz==2_ilp64 )call stdlib_I64_zlaset( 'FULL', n, n, czero, cone, z, ldz )
           ! call stdlib_I64_dpttrf to factor the matrix.
           call stdlib_I64_dpttrf( n, d, e, info )
           if( info/=0 )return
           do i = 1, n
           ...
           ! call stdlib_I64_zbdsqr to compute the singular values/vectors of the
           ! bidiagonal factor.
           if( icompz>0_ilp64 ) then
              nru = n
           else
           ...
           call stdlib_I64_zbdsqr( 'LOWER', n, 0_ilp64, nru, 0_ilp64, d, e, vt, 1_ilp64, z, ldz, c, 1_ilp64,work, info )
     ...
     !! University, July 21, 1966.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           logical(lk) :: ncnvrg, toofew
           integer(ilp64) :: ib, ibegin, idiscl, idiscu, ie, iend, iinfo, im, in, ioff, iorder, &
                     iout, irange, itmax, itmp1, iw, iwoff, j, jb, jdisc, je, nb, nwl, nwu
           real(sp) :: atoli, bnorm, gl, gu, pivmin, rtoli, safemn, tmp1, tmp2, tnorm, ulp, wkill,&
                      wl, wlu, wu, wul
           ! Local Arrays 
           integer(ilp64) :: idumma(1_ilp64)
           ! Intrinsic Functions 
           ! Executable Statements 
           info = 0_ilp64
           ! decode range
           ...
           ! check for errors
           if( irange<=0_ilp64 ) then
              info = -1_ilp64
           else if( iorder<=0_ilp64 ) then
              info = -2_ilp64
           else if( n<0_ilp64 ) then
              info = -3_ilp64
           else if( irange==2_ilp64 ) then
              if( vl>=vu ) info = -5_ilp64
           else if( irange==3_ilp64 .and. ( il<1_ilp64 .or. il>max( 1_ilp64, n ) ) )then
              info = -6_ilp64
           else if( irange==3_ilp64 .and. ( iu<min( n, il ) .or. iu>n ) )then
              info = -7_ilp64
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SSTEBZ', -info )
              return
           end if
           ! initialize error flags
           info = 0_ilp64
           ...
           if( n==0 )return
           ! simplifications:
           if( irange==3_ilp64 .and. il==1_ilp64 .and. iu==n )irange = 1_ilp64
           ! get machine constants
           ! nb is the minimum vector length for vector bisection, or 0
           ! if only scalar is to be done.
           safemn = stdlib_I64_slamch( 'S' )
           ulp = stdlib_I64_slamch( 'P' )
           rtoli = ulp*relfac
           nb = stdlib_I64_ilaenv( 1_ilp64, 'SSTEBZ', ' ', n, -1_ilp64, -1_ilp64, -1_ilp64 )
           if( nb<=1_ilp64 )nb = 0_ilp64
           ! special case when n=1
           if( n==1_ilp64 ) then
              nsplit = 1_ilp64
              isplit( 1_ilp64 ) = 1_ilp64
              if( irange==2_ilp64 .and. ( vl>=d( 1_ilp64 ) .or. vu<d( 1_ilp64 ) ) ) then
                 m = 0_ilp64
              else
                 w( 1_ilp64 ) = d( 1_ilp64 )
                 iblock( 1_ilp64 ) = 1_ilp64
                 m = 1_ilp64
              end if
              return
           end if
           ...
           do j = 2, n
              tmp1 = e( j-1 )**2_ilp64
              if( abs( d( j )*d( j-1 ) )*ulp**2_ilp64+safemn>tmp1 ) then
                 isplit( nsplit ) = j - 1_ilp64
                 nsplit = nsplit + 1_ilp64
                 work( j-1 ) = zero
              else
                 work( j-1 ) = tmp1
                 pivmin = max( pivmin, tmp1 )
           ...
           if( irange==3_ilp64 ) then
              ...
              gu = gu + fudge*tnorm*ulp*n + fudge*pivmin
              ! compute iteration parameters
              itmax = int( ( log( tnorm+pivmin )-log( pivmin ) ) /log( two ),KIND=ilp64) + 2_ilp64
              if( abstol<=zero ) then
                 atoli = ulp*tnorm
              else
                 atoli = abstol
              ...
              work( n+5 ) = gl
              work( n+6 ) = gu
              iwork( 1_ilp64 ) = -1_ilp64
              iwork( 2_ilp64 ) = -1_ilp64
              iwork( 3_ilp64 ) = n + 1_ilp64
              iwork( 4_ilp64 ) = n + 1_ilp64
              iwork( 5_ilp64 ) = il - 1_ilp64
              iwork( 6_ilp64 ) = iu
              call stdlib_I64_slaebz( 3_ilp64, itmax, n, 2_ilp64, 2_ilp64, nb, atoli, rtoli, pivmin, d, e,work, iwork( &
                        5_ilp64 ), work( n+1 ), work( n+5 ), iout,iwork, w, iblock, iinfo )
              if( iwork( 6_ilp64 )==iu ) then
                 wl = work( n+1 )
                 wlu = work( n+3 )
                 nwl = iwork( 1_ilp64 )
                 wu = work( n+4 )
              ...
              if( nwl<0_ilp64 .or. nwl>=n .or. nwu<1_ilp64 .or. nwu>n ) then
                 info = 4_ilp64
                 return
              end if
           else
              ! range='a' or 'v' -- set atoli
              tnorm = max( abs( d( 1_ilp64 ) )+abs( e( 1_ilp64 ) ),abs( d( n ) )+abs( e( n-1 ) ) )
              do j = 2, n - 1
                 tnorm = max( tnorm, abs( d( j ) )+abs( e( j-1 ) )+abs( e( j ) ) )
              end do
              if( abstol<=zero ) then
           ...
           loop_70: do jb = 1, nsplit
              ...
              if( in==1_ilp64 ) then
                 ! special case -- in=1
                 if( irange==1_ilp64 .or. wl>=d( ibegin )-pivmin )nwl = nwl + 1_ilp64
                 if( irange==1_ilp64 .or. wu>=d( ibegin )-pivmin )nwu = nwu + 1_ilp64
                 if( irange==1_ilp64 .or. ( wl<d( ibegin )-pivmin .and. wu>=d( ibegin )-pivmin ) ) &
                           then
                    m = m + 1_ilp64
                    w( m ) = d( ibegin )
                    iblock( m ) = jb
                 ...
                 work( n+1 ) = gl
                 work( n+in+1 ) = gu
                 call stdlib_I64_slaebz( 1_ilp64, 0_ilp64, in, in, 1_ilp64, nb, atoli, rtoli, pivmin,d( ibegin ), e( &
                 ibegin ), work( ibegin ),idumma, work( n+1 ), work( n+2*in+1 ), im,iwork, w( m+1 &
                           ), iblock( m+1 ), iinfo )
                 nwl = nwl + iwork( 1_ilp64 )
                 nwu = nwu + iwork( in+1 )
                 iwoff = m - iwork( 1_ilp64 )
                 ! compute eigenvalues
                 itmax = int( ( log( gu-gl+pivmin )-log( pivmin ) ) /log( two ),KIND=ilp64) + &
                           2_ilp64
                 call stdlib_I64_slaebz( 2_ilp64, itmax, in, in, 1_ilp64, nb, atoli, rtoli, pivmin,d( ibegin ), e(&
                  ibegin ), work( ibegin ),idumma, work( n+1 ), work( n+2*in+1 ), iout,iwork, w( &
                            m+1 ), iblock( m+1 ), iinfo )
                 ! copy eigenvalues into w and iblock
                 ! use -jb for block number for unconverged eigenvalues.
           ...
           if( irange==3_ilp64 ) then
              im = 0_ilp64
              idiscl = il - 1_ilp64 - nwl
              idiscu = nwu - iu
              if( idiscl>0_ilp64 .or. idiscu>0_ilp64 ) then
                 do je = 1, m
                    if( w( je )<=wlu .and. idiscl>0_ilp64 ) then
                       idiscl = idiscl - 1_ilp64
                    else if( w( je )>=wul .and. idiscu>0_ilp64 ) then
                       idiscu = idiscu - 1_ilp64
                    else
                       im = im + 1_ilp64
                       w( im ) = w( je )
                       iblock( im ) = iblock( je )
              ...
              if( idiscl>0_ilp64 .or. idiscu>0_ilp64 ) then
                 ...
                 ! (if n(w) is monotone non-decreasing, this should never
                     ! happen.)
                 if( idiscl>0_ilp64 ) then
                    wkill = wu
                    do jdisc = 1, idiscl
                       iw = 0_ilp64
                       do je = 1, m
                          if( iblock( je )/=0_ilp64 .and.( w( je )<wkill .or. iw==0_ilp64 ) ) then
                             iw = je
                             wkill = w( je )
                          end if
                       end do
           ...
           ! if order='b', do nothing -- the eigenvalues are already sorted
              ! by block.
           ! if order='e', sort the eigenvalues from smallest to largest
           if( iorder==1_ilp64 .and. nsplit>1_ilp64 ) then
              do je = 1, m - 1
                 ie = 0_ilp64
                 tmp1 = w( je )
                 do j = je + 1, m
           ...
           info = 0_ilp64
           if( ncnvrg )info = info + 1_ilp64
           if( toofew )info = info + 2_ilp64
           return
     ...
     !! University, July 21, 1966.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DSTEBZ', -info )
              return
           end if
           ! initialize error flags
           info = 0_ilp64
           ...
           ! nb is the minimum vector length for vector bisection, or 0
           ! if only scalar is to be done.
           safemn = stdlib_I64_dlamch( 'S' )
           ulp = stdlib_I64_dlamch( 'P' )
           rtoli = ulp*relfac
           nb = stdlib_I64_ilaenv( 1_ilp64, 'DSTEBZ', ' ', n, -1_ilp64, -1_ilp64, -1_ilp64 )
           if( nb<=1_ilp64 )nb = 0_ilp64
           ! special case when n=1
           if( n==1_ilp64 ) then
              nsplit = 1_ilp64
           ...
           if( irange==3_ilp64 ) then
              ...
              iwork( 5_ilp64 ) = il - 1_ilp64
              iwork( 6_ilp64 ) = iu
              call stdlib_I64_dlaebz( 3_ilp64, itmax, n, 2_ilp64, 2_ilp64, nb, atoli, rtoli, pivmin, d, e,work, iwork( &
                        5_ilp64 ), work( n+1 ), work( n+5 ), iout,iwork, w, iblock, iinfo )
              if( iwork( 6_ilp64 )==iu ) then
                 wl = work( n+1 )
                 wlu = work( n+3 )
           ...
           loop_70: do jb = 1, nsplit
              ...
              if( in==1_ilp64 ) then
                 ...
                 work( n+1 ) = gl
                 work( n+in+1 ) = gu
                 call stdlib_I64_dlaebz( 1_ilp64, 0_ilp64, in, in, 1_ilp64, nb, atoli, rtoli, pivmin,d( ibegin ), e( &
                 ibegin ), work( ibegin ),idumma, work( n+1 ), work( n+2*in+1 ), im,iwork, w( m+1 &
                           ), iblock( m+1 ), iinfo )
                 nwl = nwl + iwork( 1_ilp64 )
                 nwu = nwu + iwork( in+1 )
                 ...
                 itmax = int( ( log( gu-gl+pivmin )-log( pivmin ) ) /log( two ),KIND=ilp64) + &
                           2_ilp64
                 call stdlib_I64_dlaebz( 2_ilp64, itmax, in, in, 1_ilp64, nb, atoli, rtoli, pivmin,d( ibegin ), e(&
                  ibegin ), work( ibegin ),idumma, work( n+1 ), work( n+2*in+1 ), iout,iwork, w( &
                            m+1 ), iblock( m+1 ), iinfo )
                 ! copy eigenvalues into w and iblock
                 ! use -jb for block number for unconverged eigenvalues.
     ...
     !! using the Pal-Walker-Kahan variant of the QL or QR algorithm.
        ...
        ! =====================================================================
           ! Parameters 
           integer(ilp64), parameter :: maxit = 30_ilp64
           
           
           ! Local Scalars 
           integer(ilp64) :: i, iscale, jtot, l, l1, lend, lendsv, lsv, m, nmaxit
           real(sp) :: alpha, anorm, bb, c, eps, eps2, gamma, oldc, oldgam, p, r, rt1, rt2, rte, &
                     s, safmax, safmin, sigma, ssfmax, ssfmin
           ! Intrinsic Functions 
           ! Executable Statements 
           ...
           if( n<0_ilp64 ) then
              info = -1_ilp64
              call stdlib_I64_xerbla( 'SSTERF', -info )
              return
           end if
           if( n<=1 )return
           ! determine the unit roundoff for this environment.
           ...
           if( lend==l )go to 10
           ! scale submatrix in rows and columns l to lend
           anorm = stdlib_I64_slanst( 'M', lend-l+1, d( l ), e( l ) )
           iscale = 0_ilp64
           if( anorm==zero )go to 10
           if( anorm>ssfmax ) then
              iscale = 1_ilp64
              call stdlib_I64_slascl( 'G', 0_ilp64, 0_ilp64, anorm, ssfmax, lend-l+1, 1_ilp64, d( l ), n,info )
              call stdlib_I64_slascl( 'G', 0_ilp64, 0_ilp64, anorm, ssfmax, lend-l, 1_ilp64, e( l ), n,info )
           else if( anorm<ssfmin ) then
              iscale = 2_ilp64
              call stdlib_I64_slascl( 'G', 0_ilp64, 0_ilp64, anorm, ssfmin, lend-l+1, 1_ilp64, d( l ), n,info )
              call stdlib_I64_slascl( 'G', 0_ilp64, 0_ilp64, anorm, ssfmin, lend-l, 1_ilp64, e( l ), n,info )
           end if
           do i = l, lend - 1
              e( i ) = e( i )**2_ilp64
           end do
           ...
           if( lend>=l ) then
              ...
              if( m==l+1 ) then
                 rte = sqrt( e( l ) )
                 call stdlib_I64_slae2( d( l ), rte, d( l+1 ), rt1, rt2 )
                 d( l ) = rt1
                 d( l+1 ) = rt2
                 e( l ) = zero
                 l = l + 2_ilp64
           ...
           ! undo scaling if necessary
           150 continue
           if( iscale==1_ilp64 )call stdlib_I64_slascl( 'G', 0_ilp64, 0_ilp64, ssfmax, anorm, lendsv-lsv+1, 1_ilp64,d( lsv ), &
                     n, info )
           if( iscale==2_ilp64 )call stdlib_I64_slascl( 'G', 0_ilp64, 0_ilp64, ssfmin, anorm, lendsv-lsv+1, 1_ilp64,d( lsv ), &
                     n, info )
           ! check for no convergence to an eigenvalue after a total
           ! of n*maxit iterations.
           if( jtot<nmaxit )go to 10
           ...
           ! sort eigenvalues in increasing order.
           170 continue
           call stdlib_I64_slasrt( 'I', n, d, info )
           180 continue
           return
     ...
     !! using the Pal-Walker-Kahan variant of the QL or QR algorithm.
        ...
        ! =====================================================================
           ...
           if( n<0_ilp64 ) then
              info = -1_ilp64
              call stdlib_I64_xerbla( 'DSTERF', -info )
              return
           end if
           if( n<=1 )return
           ! determine the unit roundoff for this environment.
           ...
           if( lend==l )go to 10
           ! scale submatrix in rows and columns l to lend
           anorm = stdlib_I64_dlanst( 'M', lend-l+1, d( l ), e( l ) )
           iscale = 0_ilp64
           if( anorm==zero )go to 10
           if( (anorm>ssfmax) ) then
              iscale = 1_ilp64
              call stdlib_I64_dlascl( 'G', 0_ilp64, 0_ilp64, anorm, ssfmax, lend-l+1, 1_ilp64, d( l ), n,info )
              call stdlib_I64_dlascl( 'G', 0_ilp64, 0_ilp64, anorm, ssfmax, lend-l, 1_ilp64, e( l ), n,info )
           else if( anorm<ssfmin ) then
              iscale = 2_ilp64
              call stdlib_I64_dlascl( 'G', 0_ilp64, 0_ilp64, anorm, ssfmin, lend-l+1, 1_ilp64, d( l ), n,info )
              call stdlib_I64_dlascl( 'G', 0_ilp64, 0_ilp64, anorm, ssfmin, lend-l, 1_ilp64, e( l ), n,info )
           end if
           do i = l, lend - 1
              e( i ) = e( i )**2_ilp64
           end do
           ...
           if( lend>=l ) then
              ...
              if( m==l+1 ) then
                 rte = sqrt( e( l ) )
                 call stdlib_I64_dlae2( d( l ), rte, d( l+1 ), rt1, rt2 )
                 d( l ) = rt1
                 d( l+1 ) = rt2
                 e( l ) = zero
                 l = l + 2_ilp64
           ...
           ! undo scaling if necessary
           150 continue
           if( iscale==1_ilp64 )call stdlib_I64_dlascl( 'G', 0_ilp64, 0_ilp64, ssfmax, anorm, lendsv-lsv+1, 1_ilp64,d( lsv ), &
                     n, info )
           if( iscale==2_ilp64 )call stdlib_I64_dlascl( 'G', 0_ilp64, 0_ilp64, ssfmin, anorm, lendsv-lsv+1, 1_ilp64,d( lsv ), &
                     n, info )
           ! check for no convergence to an eigenvalue after a total
           ! of n*maxit iterations.
           if( jtot<nmaxit )go to 10
           ...
           ! sort eigenvalues in increasing order.
           170 continue
           call stdlib_I64_dlasrt( 'I', n, d, info )
           180 continue
           return
     ...
     !! without guard digits, but we know of none.  See SLAED3 for details.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           logical(lk) :: lquery
           integer(ilp64) :: finish, i, icompz, ii, j, k, lgn, liwmin, lwmin, m, smlsiz, start, &
                     storez, strtrw
           real(sp) :: eps, orgnrm, p, tiny
           ! Intrinsic Functions 
           ! Executable Statements 
           ...
           if( info==0_ilp64 ) then
              ! compute the workspace requirements
              smlsiz = stdlib_I64_ilaenv( 9_ilp64, 'SSTEDC', ' ', 0_ilp64, 0_ilp64, 0_ilp64, 0_ilp64 )
              if( n<=1_ilp64 .or. icompz==0_ilp64 ) then
                 liwmin = 1_ilp64
                 lwmin = 1_ilp64
              else if( n<=smlsiz ) then
                 liwmin = 1_ilp64
                 ...
                 lgn = int( log( real( n,KIND=sp) )/log( two ),KIND=ilp64)
                 if( 2_ilp64**lgn<n )lgn = lgn + 1_ilp64
                 if( 2_ilp64**lgn<n )lgn = lgn + 1_ilp64
                 if( icompz==1_ilp64 ) then
                    lwmin = 1_ilp64 + 3_ilp64*n + 2_ilp64*n*lgn + 4_ilp64*n**2_ilp64
                    liwmin = 6_ilp64 + 6_ilp64*n + 5_ilp64*n*lgn
                 else if( icompz==2_ilp64 ) then
                    lwmin = 1_ilp64 + 4_ilp64*n + n**2_ilp64
                    liwmin = 3_ilp64 + 5_ilp64*n
                 end if
              end if
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SSTEDC', -info )
              return
           else if (lquery) then
              return
           end if
           ...
           ! eigenvalues, which requires (3n + 3n**2) real workspace and
           ! (2 + 5n + 2n lg(n)) integer workspace.
           ! since on many architectures stdlib_I64_ssterf is much faster than any other
           ! algorithm for finding eigenvalues only, it is used here
           ! as the default. if the conditional clause is removed, then
           ! information on the size of workspace needs to be changed.
           ! if compz = 'n', use stdlib_ssterf to compute the eigenvalues.
           ...
           ! solve the problem with another solver.
           if( n<=smlsiz ) then
              call stdlib_I64_ssteqr( compz, n, d, e, z, ldz, work, info )
           else
              ! if compz = 'v', the z matrix must be stored elsewhere for later
              ! use.
              if( icompz==1_ilp64 ) then
              ...
              if( icompz==2_ilp64 ) then
                 call stdlib_I64_slaset( 'FULL', n, n, zero, one, z, ldz )
              end if
              ! scale.
              ...
              if( start<=n ) then
                 ...
                 if( finish<n ) then
                    tiny = eps*sqrt( abs( d( finish ) ) )*sqrt( abs( d( finish+1 ) ) )
                    if( abs( e( finish ) )>tiny ) then
                       finish = finish + 1_ilp64
                       go to 20
                    end if
                 end if
                 ! (sub) problem determined.  compute its size and solve it.
                 m = finish - start + 1_ilp64
                 if( m==1_ilp64 ) then
                    start = finish + 1_ilp64
                    go to 10
                 end if
                 if( m>smlsiz ) then
                    ! scale.
                    orgnrm = stdlib_I64_slanst( 'M', m, d( start ), e( start ) )
                    call stdlib_I64_slascl( 'G', 0_ilp64, 0_ilp64, orgnrm, one, m, 1_ilp64, d( start ), m,info )
                    call stdlib_I64_slascl( 'G', 0_ilp64, 0_ilp64, orgnrm, one, m-1, 1_ilp64, e( start ),m-1, info )
                    ...
                    call stdlib_I64_slaed0( icompz, n, m, d( start ), e( start ),z( strtrw, start ), &
                              ldz, work( 1_ilp64 ), n,work( storez ), iwork, info )
                    if( info/=0_ilp64 ) then
                       info = ( info / ( m+1 )+start-1 )*( n+1 ) +mod( info, ( m+1 ) ) + start - &
                                 1_ilp64
                       go to 50
                    ...
                    ! scale back.
                    call stdlib_I64_slascl( 'G', 0_ilp64, 0_ilp64, one, orgnrm, m, 1_ilp64, d( start ), m,info )
                 else
                    if( icompz==1_ilp64 ) then
                       ! since qr won't update a z matrix which is larger than
                       ! the length of d, we must solve the sub-problem in a
                       ! workspace and then multiply back into z.
                       call stdlib_I64_ssteqr( 'I', m, d( start ), e( start ), work, m,work( m*m+1 ), &
                                 info )
                       call stdlib_I64_slacpy( 'A', n, m, z( 1_ilp64, start ), ldz,work( storez ), n )
                                 
                       call stdlib_I64_sgemm( 'N', 'N', n, m, m, one,work( storez ), n, work, m, zero,&
                                 z( 1_ilp64, start ), ldz )
                    else if( icompz==2_ilp64 ) then
                       call stdlib_I64_ssteqr( 'I', m, d( start ), e( start ),z( start, start ), ldz, &
                                 work, info )
                    else
                       call stdlib_I64_ssterf( m, d( start ), e( start ), info )
                    end if
                    if( info/=0_ilp64 ) then
                       info = start*( n+1 ) + finish
                       go to 50
              ...
              if( icompz==0_ilp64 ) then
                ...
                do ii = 2, n
                   ...
                   if( k/=i ) then
                      d( k ) = d( i )
                      d( i ) = p
                      call stdlib_I64_sswap( n, z( 1_ilp64, i ), 1_ilp64, z( 1_ilp64, k ), 1_ilp64 )
                   end if
                end do
              end if
           end if
     ...
     !! without guard digits, but we know of none.  See DLAED3 for details.
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              ! compute the workspace requirements
              smlsiz = stdlib_I64_ilaenv( 9_ilp64, 'DSTEDC', ' ', 0_ilp64, 0_ilp64, 0_ilp64, 0_ilp64 )
              if( n<=1_ilp64 .or. icompz==0_ilp64 ) then
                 liwmin = 1_ilp64
                 lwmin = 1_ilp64
              else if( n<=smlsiz ) then
                 ...
                 lgn = int( log( real( n,KIND=dp) )/log( two ),KIND=ilp64)
                 if( 2_ilp64**lgn<n )lgn = lgn + 1_ilp64
                 if( 2_ilp64**lgn<n )lgn = lgn + 1_ilp64
                 if( icompz==1_ilp64 ) then
                    lwmin = 1_ilp64 + 3_ilp64*n + 2_ilp64*n*lgn + 4_ilp64*n**2_ilp64
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DSTEDC', -info )
              return
           else if (lquery) then
              return
           end if
           ...
           ! eigenvalues, which requires (3n + 3n**2) real workspace and
           ! (2 + 5n + 2n lg(n)) integer workspace.
           ! since on many architectures stdlib_I64_dsterf is much faster than any other
           ! algorithm for finding eigenvalues only, it is used here
           ! as the default. if the conditional clause is removed, then
           ! information on the size of workspace needs to be changed.
           ! if compz = 'n', use stdlib_dsterf to compute the eigenvalues.
           ...
           ! solve the problem with another solver.
           if( n<=smlsiz ) then
              call stdlib_I64_dsteqr( compz, n, d, e, z, ldz, work, info )
           else
              ! if compz = 'v', the z matrix must be stored elsewhere for later
              ! use.
              if( icompz==1_ilp64 ) then
              ...
              if( icompz==2_ilp64 ) then
                 call stdlib_I64_dlaset( 'FULL', n, n, zero, one, z, ldz )
              end if
              ! scale.
              ...
              if( start<=n ) then
                 ...
                 if( m>smlsiz ) then
                    ! scale.
                    orgnrm = stdlib_I64_dlanst( 'M', m, d( start ), e( start ) )
                    call stdlib_I64_dlascl( 'G', 0_ilp64, 0_ilp64, orgnrm, one, m, 1_ilp64, d( start ), m,info )
                    call stdlib_I64_dlascl( 'G', 0_ilp64, 0_ilp64, orgnrm, one, m-1, 1_ilp64, e( start ),m-1, info )
                    ...
                    call stdlib_I64_dlaed0( icompz, n, m, d( start ), e( start ),z( strtrw, start ), &
                              ldz, work( 1_ilp64 ), n,work( storez ), iwork, info )
                    if( info/=0_ilp64 ) then
                       info = ( info / ( m+1 )+start-1 )*( n+1 ) +mod( info, ( m+1 ) ) + start - &
                                 1_ilp64
                    ...
                    ! scale back.
                    call stdlib_I64_dlascl( 'G', 0_ilp64, 0_ilp64, one, orgnrm, m, 1_ilp64, d( start ), m,info )
                 else
                    if( icompz==1_ilp64 ) then
                       ! since qr won't update a z matrix which is larger than
                       ! the length of d, we must solve the sub-problem in a
                       ! workspace and then multiply back into z.
                       call stdlib_I64_dsteqr( 'I', m, d( start ), e( start ), work, m,work( m*m+1 ), &
                                 info )
                       call stdlib_I64_dlacpy( 'A', n, m, z( 1_ilp64, start ), ldz,work( storez ), n )
                                 
                       call stdlib_I64_dgemm( 'N', 'N', n, m, m, one,work( storez ), n, work, m, zero,&
                                 z( 1_ilp64, start ), ldz )
                    else if( icompz==2_ilp64 ) then
                       call stdlib_I64_dsteqr( 'I', m, d( start ), e( start ),z( start, start ), ldz, &
                                 work, info )
                    else
                       call stdlib_I64_dsterf( m, d( start ), e( start ), info )
                    end if
                    if( info/=0_ilp64 ) then
                       info = start*( n+1 ) + finish
                       go to 50
              ...
              if( icompz==0_ilp64 ) then
                ...
                do ii = 2, n
                   ...
                   if( k/=i ) then
                      d( k ) = d( i )
                      d( i ) = p
                      call stdlib_I64_dswap( n, z( 1_ilp64, i ), 1_ilp64, z( 1_ilp64, k ), 1_ilp64 )
                   end if
                end do
              end if
           end if
     ...
     !! without guard digits, but we know of none.  See SLAED3 for details.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           logical(lk) :: lquery
           integer(ilp64) :: finish, i, icompz, ii, j, k, lgn, liwmin, ll, lrwmin, lwmin, m, smlsiz,&
                      start
           real(sp) :: eps, orgnrm, p, tiny
           ! Intrinsic Functions 
           ! Executable Statements 
           ...
           if( info==0_ilp64 ) then
              ! compute the workspace requirements
              smlsiz = stdlib_I64_ilaenv( 9_ilp64, 'CSTEDC', ' ', 0_ilp64, 0_ilp64, 0_ilp64, 0_ilp64 )
              if( n<=1_ilp64 .or. icompz==0_ilp64 ) then
                 lwmin = 1_ilp64
                 liwmin = 1_ilp64
                 lrwmin = 1_ilp64
                 ...
                 lwmin = 1_ilp64
                 liwmin = 1_ilp64
                 lrwmin = 2_ilp64*( n - 1_ilp64 )
              else if( icompz==1_ilp64 ) then
                 lgn = int( log( real( n,KIND=sp) ) / log( two ),KIND=ilp64)
                 if( 2_ilp64**lgn<n )lgn = lgn + 1_ilp64
                 if( 2_ilp64**lgn<n )lgn = lgn + 1_ilp64
                 lwmin = n*n
                 lrwmin = 1_ilp64 + 3_ilp64*n + 2_ilp64*n*lgn + 4_ilp64*n**2_ilp64
                 liwmin = 6_ilp64 + 6_ilp64*n + 5_ilp64*n*lgn
              else if( icompz==2_ilp64 ) then
                 lwmin = 1_ilp64
                 lrwmin = 1_ilp64 + 4_ilp64*n + 2_ilp64*n**2_ilp64
                 liwmin = 3_ilp64 + 5_ilp64*n
              end if
              work( 1_ilp64 ) = lwmin
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CSTEDC', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           ! solve the problem with another solver.
           if( n<=smlsiz ) then
              call stdlib_I64_csteqr( compz, n, d, e, z, ldz, rwork, info )
           else
              ! if compz = 'i', we simply call stdlib_I64_sstedc instead.
              if( icompz==2_ilp64 ) then
                 call stdlib_I64_slaset( 'FULL', n, n, zero, one, rwork, n )
                 ll = n*n + 1_ilp64
                 call stdlib_I64_sstedc( 'I', n, d, e, rwork, n,rwork( ll ), lrwork-ll+1, iwork, &
                           liwork, info )
                 do j = 1, n
                    do i = 1, n
                       z( i, j ) = rwork( ( j-1 )*n+i )
              ...
              if( start<=n ) then
                 ...
                 if( m>smlsiz ) then
                    ...
                    call stdlib_I64_claed0( n, m, d( start ), e( start ), z( 1_ilp64, start ),ldz, work, n, &
                              rwork, iwork, info )
                    if( info>0_ilp64 ) then
                       info = ( info / ( m+1 )+start-1 )*( n+1 ) +mod( info, ( m+1 ) ) + start - &
                                 1_ilp64
                    ...
                    call stdlib_I64_ssteqr( 'I', m, d( start ), e( start ), rwork, m,rwork( m*m+1 ), &
                              info )
                    call stdlib_I64_clacrm( n, m, z( 1_ilp64, start ), ldz, rwork, m, work, n,rwork( m*m+1 )&
                               )
                    call stdlib_I64_clacpy( 'A', n, m, work, n, z( 1_ilp64, start ), ldz )
                    if( info>0_ilp64 ) then
                       info = start*( n+1 ) + finish
                       go to 70
                    end if
              ...
              do ii = 2, n
                ...
                if( k/=i ) then
                   d( k ) = d( i )
                   d( i ) = p
                   call stdlib_I64_cswap( n, z( 1_ilp64, i ), 1_ilp64, z( 1_ilp64, k ), 1_ilp64 )
                end if
              end do
           end if
           70 continue
     ...
     !! without guard digits, but we know of none.  See DLAED3 for details.
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              ! compute the workspace requirements
              smlsiz = stdlib_I64_ilaenv( 9_ilp64, 'ZSTEDC', ' ', 0_ilp64, 0_ilp64, 0_ilp64, 0_ilp64 )
              if( n<=1_ilp64 .or. icompz==0_ilp64 ) then
                 lwmin = 1_ilp64
                 liwmin = 1_ilp64
                 lrwmin = 1_ilp64
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZSTEDC', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           ! solve the problem with another solver.
           if( n<=smlsiz ) then
              call stdlib_I64_zsteqr( compz, n, d, e, z, ldz, rwork, info )
           else
              ! if compz = 'i', we simply call stdlib_I64_dstedc instead.
              if( icompz==2_ilp64 ) then
                 call stdlib_I64_dlaset( 'FULL', n, n, zero, one, rwork, n )
                 ll = n*n + 1_ilp64
                 call stdlib_I64_dstedc( 'I', n, d, e, rwork, n,rwork( ll ), lrwork-ll+1, iwork, &
                           liwork, info )
                 do j = 1, n
                    do i = 1, n
                       z( i, j ) = rwork( ( j-1 )*n+i )
              ...
              if( start<=n ) then
                 ...
                 if( m>smlsiz ) then
                    ...
                    call stdlib_I64_zlaed0( n, m, d( start ), e( start ), z( 1_ilp64, start ),ldz, work, n, &
                              rwork, iwork, info )
                    if( info>0_ilp64 ) then
                       info = ( info / ( m+1 )+start-1 )*( n+1 ) +mod( info, ( m+1 ) ) + start - &
                                 1_ilp64
                    ...
                    call stdlib_I64_dsteqr( 'I', m, d( start ), e( start ), rwork, m,rwork( m*m+1 ), &
                              info )
                    call stdlib_I64_zlacrm( n, m, z( 1_ilp64, start ), ldz, rwork, m, work, n,rwork( m*m+1 )&
                               )
                    call stdlib_I64_zlacpy( 'A', n, m, work, n, z( 1_ilp64, start ), ldz )
                    if( info>0_ilp64 ) then
                       info = start*( n+1 ) + finish
                       go to 70
                    end if
              ...
              do ii = 2, n
                ...
                if( k/=i ) then
                   d( k ) = d( i )
                   d( i ) = p
                   call stdlib_I64_zswap( n, z( 1_ilp64, i ), 1_ilp64, z( 1_ilp64, k ), 1_ilp64 )
                end if
              end do
           end if
           70 continue
     ...
     !! do not conform to the IEEE-754 standard.
        ...
        ! =====================================================================
           ...
           info = 0_ilp64
           tryrac = .false.
           call stdlib_I64_sstemr( jobz, range, n, d, e, vl, vu, il, iu,m, w, z, ldz, n, isuppz, &
                     tryrac, work, lwork,iwork, liwork, info )
     ...
     !! do not conform to the IEEE-754 standard.
        ...
        ! =====================================================================
           ...
           info = 0_ilp64
           tryrac = .false.
           call stdlib_I64_dstemr( jobz, range, n, d, e, vl, vu, il, iu,m, w, z, ldz, n, isuppz, &
                     tryrac, work, lwork,iwork, liwork, info )
     ...
     !! do not conform to the IEEE-754 standard.
        ...
        ! =====================================================================
           ...
           info = 0_ilp64
           tryrac = .false.
           call stdlib_I64_cstemr( jobz, range, n, d, e, vl, vu, il, iu,m, w, z, ldz, n, isuppz, &
                     tryrac, work, lwork,iwork, liwork, info )
     ...
     !! do not conform to the IEEE-754 standard.
        ...
        ! =====================================================================
           ...
           info = 0_ilp64
           tryrac = .false.
           call stdlib_I64_zstemr( jobz, range, n, d, e, vl, vu, il, iu,m, w, z, ldz, n, isuppz, &
                     tryrac, work, lwork,iwork, liwork, info )
     ...
     !! specified by an internal parameter MAXITS (currently set to 5).
        ...
        ! =====================================================================
           ...
           real(sp), parameter :: odm3 = 1.0e-3_sp
           real(sp), parameter :: odm1 = 1.0e-1_sp
           integer(ilp64), parameter :: maxits = 5_ilp64
           integer(ilp64), parameter :: extra = 2_ilp64
           
           
           ! Local Scalars 
           integer(ilp64) :: b1, blksiz, bn, gpind, i, iinfo, indrv1, indrv2, indrv3, indrv4, &
                     indrv5, its, j, j1, jblk, jmax, nblk, nrmchk
           real(sp) :: ctr, eps, eps1, nrm, onenrm, ortol, pertol, scl, sep, stpcrt, tol, xj, &
                     xjm
           ! Local Arrays 
           integer(ilp64) :: iseed(4_ilp64)
           ! Intrinsic Functions 
           ! Executable Statements 
           ! test the input parameters.
           info = 0_ilp64
           ...
           if( n<0_ilp64 ) then
              info = -1_ilp64
           else if( m<0_ilp64 .or. m>n ) then
              info = -4_ilp64
           else if( ldz<max( 1_ilp64, n ) ) then
              info = -9_ilp64
           else
              do j = 2, m
                 if( iblock( j )<iblock( j-1 ) ) then
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SSTEIN', -info )
              return
           end if
           ! quick return if possible
           if( n==0_ilp64 .or. m==0_ilp64 ) then
              return
           else if( n==1_ilp64 ) then
              z( 1_ilp64, 1_ilp64 ) = one
              return
           end if
           ! get machine constants.
           eps = stdlib_I64_slamch( 'PRECISION' )
           ! initialize seed for random number generator stdlib_I64_slarnv.
           do i = 1, 4
              iseed( i ) = 1_ilp64
           end do
           ! initialize pointers.
           ...
           loop_160: do nblk = 1, iblock( m )
              ! find starting and ending indices of block nblk.
              if( nblk==1_ilp64 ) then
                 b1 = 1_ilp64
              else
                 b1 = isplit( nblk-1 ) + 1_ilp64
              end if
              bn = isplit( nblk )
              blksiz = bn - b1 + 1_ilp64
              if( blksiz==1 )go to 60
              gpind = j1
              ! compute reorthogonalization criterion and stopping criterion.
              onenrm = abs( d( b1 ) ) + abs( e( b1 ) )
              ...
              loop_150: do j = j1, m
                 ...
                 xj = w( j )
                 ! skip all the work if the block size is one.
                 if( blksiz==1_ilp64 ) then
                    work( indrv1+1 ) = one
                    go to 120
                 end if
                 ! if eigenvalues j and j-1 are too close, add a relatively
                 ! small perturbation.
                 if( jblk>1_ilp64 ) then
                    eps1 = abs( eps*xj )
                    pertol = ten*eps1
                    sep = xj - xjm
                    if( sep<pertol )xj = xjm + pertol
                 ...
                 nrmchk = 0_ilp64
                 ! get random starting vector.
                 call stdlib_I64_slarnv( 2_ilp64, iseed, blksiz, work( indrv1+1 ) )
                 ! copy the matrix t so it won't be destroyed in factorization.
                 call stdlib_I64_scopy( blksiz, d( b1 ), 1_ilp64, work( indrv4+1 ), 1_ilp64 )
                 call stdlib_I64_scopy( blksiz-1, e( b1 ), 1_ilp64, work( indrv2+2 ), 1_ilp64 )
                 call stdlib_I64_scopy( blksiz-1, e( b1 ), 1_ilp64, work( indrv3+1 ), 1_ilp64 )
                 ! compute lu factors with partial pivoting  ( pt = lu )
                 tol = zero
                 call stdlib_I64_slagtf( blksiz, work( indrv4+1 ), xj, work( indrv2+2 ),work( indrv3+&
                           1_ilp64 ), tol, work( indrv5+1 ), iwork,iinfo )
                 ! update iteration count.
                 70 continue
                 its = its + 1_ilp64
                 if( its>maxits )go to 100
                 ! normalize and scale the righthand side vector pb.
                 jmax = stdlib_I64_isamax( blksiz, work( indrv1+1 ), 1_ilp64 )
                 scl = blksiz*onenrm*max( eps,abs( work( indrv4+blksiz ) ) ) /abs( work( indrv1+&
                           jmax ) )
                 call stdlib_I64_sscal( blksiz, scl, work( indrv1+1 ), 1_ilp64 )
                 ! solve the system lu = pb.
                 call stdlib_I64_slagts( -1_ilp64, blksiz, work( indrv4+1 ), work( indrv2+2 ),work( indrv3+&
                           1_ilp64 ), work( indrv5+1 ), iwork,work( indrv1+1 ), tol, iinfo )
                 ! reorthogonalize by modified gram-schmidt if eigenvalues are
                 ! close enough.
                 if( jblk==1 )go to 90
                 if( abs( xj-xjm )>ortol )gpind = j
                 if( gpind/=j ) then
                    do i = gpind, j - 1
                       ctr = -stdlib_I64_sdot( blksiz, work( indrv1+1 ), 1_ilp64, z( b1, i ),1_ilp64 )
                       call stdlib_I64_saxpy( blksiz, ctr, z( b1, i ), 1_ilp64,work( indrv1+1 ), 1_ilp64 )
                    end do
                 end if
                 ! check the infinity norm of the iterate.
                 90 continue
                 ...
                 ! accept iterate as jth eigenvector.
                 110 continue
                 scl = one / stdlib_I64_snrm2( blksiz, work( indrv1+1 ), 1_ilp64 )
                 jmax = stdlib_I64_isamax( blksiz, work( indrv1+1 ), 1_ilp64 )
                 if( work( indrv1+jmax )<zero )scl = -scl
                 call stdlib_I64_sscal( blksiz, scl, work( indrv1+1 ), 1_ilp64 )
                 120 continue
     ...
     !! specified by an internal parameter MAXITS (currently set to 5).
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DSTEIN', -info )
              return
           end if
           ! quick return if possible
           if( n==0_ilp64 .or. m==0_ilp64 ) then
           ...
           ! get machine constants.
           eps = stdlib_I64_dlamch( 'PRECISION' )
           ! initialize seed for random number generator stdlib_I64_dlarnv.
           do i = 1, 4
              iseed( i ) = 1_ilp64
           end do
           ! initialize pointers.
           ...
           loop_160: do nblk = 1, iblock( m )
              ...
              loop_150: do j = j1, m
                 ...
                 nrmchk = 0_ilp64
                 ! get random starting vector.
                 call stdlib_I64_dlarnv( 2_ilp64, iseed, blksiz, work( indrv1+1 ) )
                 ! copy the matrix t so it won't be destroyed in factorization.
                 call stdlib_I64_dcopy( blksiz, d( b1 ), 1_ilp64, work( indrv4+1 ), 1_ilp64 )
                 call stdlib_I64_dcopy( blksiz-1, e( b1 ), 1_ilp64, work( indrv2+2 ), 1_ilp64 )
                 call stdlib_I64_dcopy( blksiz-1, e( b1 ), 1_ilp64, work( indrv3+1 ), 1_ilp64 )
                 ! compute lu factors with partial pivoting  ( pt = lu )
                 tol = zero
                 call stdlib_I64_dlagtf( blksiz, work( indrv4+1 ), xj, work( indrv2+2 ),work( indrv3+&
                           1_ilp64 ), tol, work( indrv5+1 ), iwork,iinfo )
                 ! update iteration count.
                 70 continue
                 its = its + 1_ilp64
                 if( its>maxits )go to 100
                 ! normalize and scale the righthand side vector pb.
                 jmax = stdlib_I64_idamax( blksiz, work( indrv1+1 ), 1_ilp64 )
                 scl = blksiz*onenrm*max( eps,abs( work( indrv4+blksiz ) ) ) /abs( work( indrv1+&
                           jmax ) )
                 call stdlib_I64_dscal( blksiz, scl, work( indrv1+1 ), 1_ilp64 )
                 ! solve the system lu = pb.
                 call stdlib_I64_dlagts( -1_ilp64, blksiz, work( indrv4+1 ), work( indrv2+2 ),work( indrv3+&
                           1_ilp64 ), work( indrv5+1 ), iwork,work( indrv1+1 ), tol, iinfo )
                 ! reorthogonalize by modified gram-schmidt if eigenvalues are
                 ! close enough.
                 if( jblk==1 )go to 90
                 ...
                 if( gpind/=j ) then
                    do i = gpind, j - 1
                       ztr = -stdlib_I64_ddot( blksiz, work( indrv1+1 ), 1_ilp64, z( b1, i ),1_ilp64 )
                       call stdlib_I64_daxpy( blksiz, ztr, z( b1, i ), 1_ilp64,work( indrv1+1 ), 1_ilp64 )
                    end do
                 end if
                 ! check the infinity norm of the iterate.
                 90 continue
                 ...
                 ! accept iterate as jth eigenvector.
                 110 continue
                 scl = one / stdlib_I64_dnrm2( blksiz, work( indrv1+1 ), 1_ilp64 )
                 jmax = stdlib_I64_idamax( blksiz, work( indrv1+1 ), 1_ilp64 )
                 if( work( indrv1+jmax )<zero )scl = -scl
                 call stdlib_I64_dscal( blksiz, scl, work( indrv1+1 ), 1_ilp64 )
                 120 continue
     ...
     !! which was reduced to tridiagonal form.
       ...
       ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CSTEIN', -info )
              return
           end if
           ! quick return if possible
           if( n==0_ilp64 .or. m==0_ilp64 ) then
     ...
     !! which was reduced to tridiagonal form.
       ...
       ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZSTEIN', -info )
              return
           end if
           ! quick return if possible
           if( n==0_ilp64 .or. m==0_ilp64 ) then
     ...
     !! zero divisors.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           logical(lk) :: alleig, indeig, lquery, valeig, wantz, zquery
           integer(ilp64) :: i, ibegin, iend, ifirst, iil, iindbl, iindw, iindwk, iinfo, iinspl, &
           iiu, ilast, in, indd, inde2, inderr, indgp, indgrs, indwrk, itmp, itmp2, j, jblk, jj, &
                     liwmin, lwmin, nsplit, nzcmin, offset, wbegin, wend
           real(sp) :: bignum, cs, eps, pivmin, r1, r2, rmax, rmin, rtol1, rtol2, safmin, scale, &
                     smlnum, sn, thresh, tmp, tnrm, wl, wu
           ...
           lquery = ( ( lwork==-1_ilp64 ).or.( liwork==-1_ilp64 ) )
           zquery = ( nzc==-1_ilp64 )
           ! stdlib_I64_sstemr needs work of size 6*n, iwork of size 3*n.
           ! in addition, stdlib_I64_slarre needs work of size 6*n, iwork of size 5*n.
           ! furthermore, stdlib_I64_slarrv needs work of size 12*n, iwork of size 7*n.
           if( wantz ) then
              lwmin = 18_ilp64*n
              liwmin = 10_ilp64*n
           else
           ...
           if( valeig ) then
              ! we do not reference vl, vu in the cases range = 'i','a'
              ! the interval (wl, wu] contains all the wanted eigenvalues.
              ! it is either given by the user or computed in stdlib_I64_slarre.
              wl = vl
              wu = vu
           elseif( indeig ) then
              ! we do not reference il, iu in the cases range = 'v','a'
           ...
           if( .not.( wantz .or. stdlib_lsame( jobz, 'N' ) ) ) then
              ...
              info = -3_ilp64
           else if( valeig .and. n>0_ilp64 .and. wu<=wl ) then
              info = -7_ilp64
           else if( indeig .and. ( iil<1_ilp64 .or. iil>n ) ) then
              info = -8_ilp64
           else if( indeig .and. ( iiu<iil .or. iiu>n ) ) then
              info = -9_ilp64
           else if( ldz<1_ilp64 .or. ( wantz .and. ldz<n ) ) then
           ...
           if( info==0_ilp64 ) then
              ...
              if( wantz .and. alleig ) then
                 ...
                 call stdlib_I64_slarrc( 'T', n, vl, vu, d, e, safmin,nzcmin, itmp, itmp2, info )
              ...
              if( zquery .and. info==0_ilp64 ) then
                 z( 1_ilp64,1_ilp64 ) = nzcmin
              else if( nzc<nzcmin .and. .not.zquery ) then
                 info = -14_ilp64
              end if
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SSTEMR', -info )
              return
           else if( lquery .or. zquery ) then
              return
           end if
           ...
           if( n==1_ilp64 ) then
              if( alleig .or. indeig ) then
                 ...
                 if( wl<d( 1_ilp64 ) .and. wu>=d( 1_ilp64 ) ) then
                    m = 1_ilp64
                    w( 1_ilp64 ) = d( 1_ilp64 )
                 end if
              end if
              if( wantz.and.(.not.zquery) ) then
                 z( 1_ilp64, 1_ilp64 ) = one
                 isuppz(1_ilp64) = 1_ilp64
                 isuppz(2_ilp64) = 1_ilp64
              end if
              return
           end if
           if( n==2_ilp64 ) then
              if( .not.wantz ) then
                 call stdlib_I64_slae2( d(1_ilp64), e(1_ilp64), d(2_ilp64), r1, r2 )
              else if( wantz.and.(.not.zquery) ) then
                 call stdlib_I64_slaev2( d(1_ilp64), e(1_ilp64), d(2_ilp64), r1, r2, cs, sn )
              end if
              if( alleig.or.(valeig.and.(r2>wl).and.(r2<=wu)).or.(indeig.and.(iil==1_ilp64)) ) &
                        then
                 m = m+1
                 w( m ) = r2
                 if( wantz.and.(.not.zquery) ) then
                    ...
                    if (sn/=zero) then
                       if (cs/=zero) then
                          isuppz(2_ilp64*m-1) = 1_ilp64
                          isuppz(2_ilp64*m) = 2_ilp64
                       else
                          isuppz(2_ilp64*m-1) = 1_ilp64
                          isuppz(2_ilp64*m) = 1_ilp64
                       end if
                    else
                       isuppz(2_ilp64*m-1) = 2_ilp64
                       isuppz(2_ilp64*m) = 2_ilp64
                    end if
                 endif
              endif
              if( alleig.or.(valeig.and.(r1>wl).and.(r1<=wu)).or.(indeig.and.(iiu==2_ilp64)) ) &
                        then
                 m = m+1
                 w( m ) = r1
                 if( wantz.and.(.not.zquery) ) then
           ...
           ! continue with general n
              indgrs = 1_ilp64
              inderr = 2_ilp64*n + 1_ilp64
              indgp = 3_ilp64*n + 1_ilp64
              indd = 4_ilp64*n + 1_ilp64
              inde2 = 5_ilp64*n + 1_ilp64
              indwrk = 6_ilp64*n + 1_ilp64
              iinspl = 1_ilp64
              iindbl = n + 1_ilp64
              iindw = 2_ilp64*n + 1_ilp64
              iindwk = 3_ilp64*n + 1_ilp64
              ! scale matrix to allowable range, if necessary.
              ! the allowable range is related to the pivmin parameter; see the
              ! comments in stdlib_I64_slarrd.  the preference for scaling small values
              ! up is heuristic; we expect users' matrices not to be close to the
              ! rmax threshold.
              scale = one
              tnrm = stdlib_I64_slanst( 'M', n, d, e )
              ...
              if( scale/=one ) then
                 call stdlib_I64_sscal( n, scale, d, 1_ilp64 )
                 call stdlib_I64_sscal( n-1, scale, e, 1_ilp64 )
                 tnrm = tnrm*scale
                 if( valeig ) then
                    ! if eigenvalues in interval have to be found,
                    ! scale (wl, wu] accordingly
              ...
              ! into smaller subblocks if the corresponding off-diagonal elements
              ! are small
              ! thresh is the splitting parameter for stdlib_I64_slarre
              ! a negative thresh forces the old splitting criterion based on the
              ! size of the off-diagonal. a positive thresh switches to splitting
              ! which preserves relative accuracy.
              if( tryrac ) then
                 ! test whether the matrix warrants the more expensive relative approach.
                 call stdlib_I64_slarrr( n, d, e, iinfo )
              else
                 ! the user does not care about relative accurately eigenvalues
                 iinfo = -1_ilp64
              endif
              ! set the splitting criterion
              if (iinfo==0_ilp64) then
                 thresh = eps
              else
                 thresh = -eps
                 ! relative accuracy is desired but t does not guarantee it
              ...
              if( tryrac ) then
                 ! copy original diagonal, needed to guarantee relative accuracy
                 call stdlib_I64_scopy(n,d,1_ilp64,work(indd),1_ilp64)
              endif
              ! store the squares of the offdiagonal values of t
              do j = 1, n-1
                 work( inde2+j-1 ) = e(j)**2_ilp64
              end do
              ! set the tolerance parameters for bisection
              if( .not.wantz ) then
                 ! stdlib_I64_slarre computes the eigenvalues to full precision.
                 rtol1 = four * eps
                 rtol2 = four * eps
              else
                 ! stdlib_I64_slarre computes the eigenvalues to less than full precision.
                 ! stdlib_I64_slarrv will refine the eigenvalue approximations, and we can
                 ! need less accurate initial bisection in stdlib_I64_slarre.
                 ! note: these settings do only affect the subset case and stdlib_I64_slarre
                 rtol1 = max( sqrt(eps)*5.0e-2_sp, four * eps )
                 rtol2 = max( sqrt(eps)*5.0e-3_sp, four * eps )
              endif
              call stdlib_I64_slarre( range, n, wl, wu, iil, iiu, d, e,work(inde2), rtol1, rtol2, &
              thresh, nsplit,iwork( iinspl ), m, w, work( inderr ),work( indgp ), iwork( iindbl ),&
              iwork( iindw ), work( indgrs ), pivmin,work( indwrk ), iwork( iindwk ), iinfo )
                        
              if( iinfo/=0_ilp64 ) then
                 info = 10_ilp64 + abs( iinfo )
                 return
              end if
              ! note that if range /= 'v', stdlib_I64_slarre computes bounds on the desired
              ! part of the spectrum. all desired eigenvalues are contained in
              ! (wl,wu]
              if( wantz ) then
                 ! compute the desired eigenvectors corresponding to the computed
                 ! eigenvalues
                 call stdlib_I64_slarrv( n, wl, wu, d, e,pivmin, iwork( iinspl ), m,1_ilp64, m, minrgp, &
                 rtol1, rtol2,w, work( inderr ), work( indgp ), iwork( iindbl ),iwork( iindw ), &
                           work( indgrs ), z, ldz,isuppz, work( indwrk ), iwork( iindwk ), iinfo )
                 if( iinfo/=0_ilp64 ) then
                    info = 20_ilp64 + abs( iinfo )
                    return
                 end if
              else
                 ! stdlib_I64_slarre computes eigenvalues of the (shifted) root representation
                 ! stdlib_I64_slarrv returns the eigenvalues of the unshifted matrix.
                 ! however, if the eigenvectors are not desired by the user, we need
                 ! to apply the corresponding shifts from stdlib_I64_slarre to obtain the
                 ! eigenvalues of the original matrix.
                 do j = 1, m
                    itmp = iwork( iindbl+j-1 )
                    w( j ) = w( j ) + e( iwork( iinspl+itmp-1 ) )
              ...
              if ( tryrac ) then
                 ...
                 loop_39: do jblk = 1, iwork( iindbl+m-1 )
                    ...
                    offset = iwork(iindw+wbegin-1)-1_ilp64
                    ifirst = iwork(iindw+wbegin-1)
                    ilast = iwork(iindw+wend-1)
                    rtol2 = four * eps
                    call stdlib_I64_slarrj( in,work(indd+ibegin-1), work(inde2+ibegin-1),ifirst, &
                    ilast, rtol2, offset, w(wbegin),work( inderr+wbegin-1 ),work( indwrk ), iwork(&
                               iindwk ), pivmin,tnrm, iinfo )
                    ibegin = iend + 1_ilp64
                    wbegin = wend + 1_ilp64
              ...
              ! if matrix was scaled, then rescale eigenvalues appropriately.
              if( scale/=one ) then
                 call stdlib_I64_sscal( m, one / scale, w, 1_ilp64 )
              end if
           end if
           ! if eigenvalues are not in increasing order, then sort them,
           ! possibly along with eigenvectors.
           if( nsplit>1_ilp64 .or. n==2_ilp64 ) then
              if( .not. wantz ) then
                 call stdlib_I64_slasrt( 'I', m, w, iinfo )
                 if( iinfo/=0_ilp64 ) then
                    info = 3_ilp64
                    return
                 end if
                 ...
                 do j = 1, m - 1
                    ...
                    if( i/=0_ilp64 ) then
                       ...
                       if( wantz ) then
                          call stdlib_I64_sswap( n, z( 1_ilp64, i ), 1_ilp64, z( 1_ilp64, j ), 1_ilp64 )
                          itmp = isuppz( 2_ilp64*i-1 )
                          isuppz( 2_ilp64*i-1 ) = isuppz( 2_ilp64*j-1 )
                          isuppz( 2_ilp64*j-1 ) = itmp
                          itmp = isuppz( 2_ilp64*i )
                          isuppz( 2_ilp64*i ) = isuppz( 2_ilp64*j )
                          isuppz( 2_ilp64*j ) = itmp
                       end if
                    end if
                 end do
              end if
     ...
     !! zero divisors.
        ...
        ! =====================================================================
           ...
           lquery = ( ( lwork==-1_ilp64 ).or.( liwork==-1_ilp64 ) )
           zquery = ( nzc==-1_ilp64 )
           ! stdlib_I64_dstemr needs work of size 6*n, iwork of size 3*n.
           ! in addition, stdlib_I64_dlarre needs work of size 6*n, iwork of size 5*n.
           ! furthermore, stdlib_I64_dlarrv needs work of size 12*n, iwork of size 7*n.
           if( wantz ) then
              lwmin = 18_ilp64*n
              liwmin = 10_ilp64*n
           else
           ...
           if( valeig ) then
              ! we do not reference vl, vu in the cases range = 'i','a'
              ! the interval (wl, wu] contains all the wanted eigenvalues.
              ! it is either given by the user or computed in stdlib_I64_dlarre.
              wl = vl
              wu = vu
           elseif( indeig ) then
              ! we do not reference il, iu in the cases range = 'v','a'
           ...
           if( info==0_ilp64 ) then
              ...
              if( wantz .and. alleig ) then
                 ...
                 call stdlib_I64_dlarrc( 'T', n, vl, vu, d, e, safmin,nzcmin, itmp, itmp2, info )
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DSTEMR', -info )
              return
           else if( lquery .or. zquery ) then
              return
           end if
           ...
           if( n==2_ilp64 ) then
              if( .not.wantz ) then
                 call stdlib_I64_dlae2( d(1_ilp64), e(1_ilp64), d(2_ilp64), r1, r2 )
              else if( wantz.and.(.not.zquery) ) then
                 call stdlib_I64_dlaev2( d(1_ilp64), e(1_ilp64), d(2_ilp64), r1, r2, cs, sn )
              end if
              if( alleig.or.(valeig.and.(r2>wl).and.(r2<=wu)).or.(indeig.and.(iil==1_ilp64)) ) &
                        then
                 m = m+1
           ...
           ! continue with general n
              ...
              ! scale matrix to allowable range, if necessary.
              ! the allowable range is related to the pivmin parameter; see the
              ! comments in stdlib_I64_dlarrd.  the preference for scaling small values
              ! up is heuristic; we expect users' matrices not to be close to the
              ! rmax threshold.
              scale = one
              tnrm = stdlib_I64_dlanst( 'M', n, d, e )
              ...
              if( scale/=one ) then
                 call stdlib_I64_dscal( n, scale, d, 1_ilp64 )
                 call stdlib_I64_dscal( n-1, scale, e, 1_ilp64 )
                 tnrm = tnrm*scale
                 if( valeig ) then
                    ! if eigenvalues in interval have to be found,
                    ! scale (wl, wu] accordingly
              ...
              ! into smaller subblocks if the corresponding off-diagonal elements
              ! are small
              ! thresh is the splitting parameter for stdlib_I64_dlarre
              ! a negative thresh forces the old splitting criterion based on the
              ! size of the off-diagonal. a positive thresh switches to splitting
              ! which preserves relative accuracy.
              if( tryrac ) then
                 ! test whether the matrix warrants the more expensive relative approach.
                 call stdlib_I64_dlarrr( n, d, e, iinfo )
              else
                 ! the user does not care about relative accurately eigenvalues
                 iinfo = -1_ilp64
              endif
              ...
              if( tryrac ) then
                 ! copy original diagonal, needed to guarantee relative accuracy
                 call stdlib_I64_dcopy(n,d,1_ilp64,work(indd),1_ilp64)
              endif
              ! store the squares of the offdiagonal values of t
              do j = 1, n-1
                 work( inde2+j-1 ) = e(j)**2_ilp64
              ...
              ! set the tolerance parameters for bisection
              if( .not.wantz ) then
                 ! stdlib_I64_dlarre computes the eigenvalues to full precision.
                 rtol1 = four * eps
                 rtol2 = four * eps
              else
                 ! stdlib_I64_dlarre computes the eigenvalues to less than full precision.
                 ! stdlib_I64_dlarrv will refine the eigenvalue approximations, and we can
                 ! need less accurate initial bisection in stdlib_I64_dlarre.
                 ! note: these settings do only affect the subset case and stdlib_I64_dlarre
                 rtol1 = sqrt(eps)
                 rtol2 = max( sqrt(eps)*5.0e-3_dp, four * eps )
              endif
              call stdlib_I64_dlarre( range, n, wl, wu, iil, iiu, d, e,work(inde2), rtol1, rtol2, &
              thresh, nsplit,iwork( iinspl ), m, w, work( inderr ),work( indgp ), iwork( iindbl ),&
              iwork( iindw ), work( indgrs ), pivmin,work( indwrk ), iwork( iindwk ), iinfo )
              ...
              ! note that if range /= 'v', stdlib_I64_dlarre computes bounds on the desired
              ! part of the spectrum. all desired eigenvalues are contained in
              ! (wl,wu]
              if( wantz ) then
                 ! compute the desired eigenvectors corresponding to the computed
                 ! eigenvalues
                 call stdlib_I64_dlarrv( n, wl, wu, d, e,pivmin, iwork( iinspl ), m,1_ilp64, m, minrgp, &
                 rtol1, rtol2,w, work( inderr ), work( indgp ), iwork( iindbl ),iwork( iindw ), &
                           work( indgrs ), z, ldz,isuppz, work( indwrk ), iwork( iindwk ), iinfo )
                 if( iinfo/=0_ilp64 ) then
                    info = 20_ilp64 + abs( iinfo )
                 ...
                 ! stdlib_I64_dlarre computes eigenvalues of the (shifted) root representation
                 ! stdlib_I64_dlarrv returns the eigenvalues of the unshifted matrix.
                 ! however, if the eigenvectors are not desired by the user, we need
                 ! to apply the corresponding shifts from stdlib_I64_dlarre to obtain the
                 ! eigenvalues of the original matrix.
                 do j = 1, m
                    itmp = iwork( iindbl+j-1 )
                    w( j ) = w( j ) + e( iwork( iinspl+itmp-1 ) )
              ...
              if ( tryrac ) then
                 ...
                 loop_39: do jblk = 1, iwork( iindbl+m-1 )
                    ...
                    ilast = iwork(iindw+wend-1)
                    rtol2 = four * eps
                    call stdlib_I64_dlarrj( in,work(indd+ibegin-1), work(inde2+ibegin-1),ifirst, &
                    ilast, rtol2, offset, w(wbegin),work( inderr+wbegin-1 ),work( indwrk ), iwork(&
                               iindwk ), pivmin,tnrm, iinfo )
                    ibegin = iend + 1_ilp64
                    wbegin = wend + 1_ilp64
              ...
              ! if matrix was scaled, then rescale eigenvalues appropriately.
              if( scale/=one ) then
                 call stdlib_I64_dscal( m, one / scale, w, 1_ilp64 )
              end if
           end if
           ! if eigenvalues are not in increasing order, then sort them,
           ! possibly along with eigenvectors.
           if( nsplit>1_ilp64 .or. n==2_ilp64 ) then
              if( .not. wantz ) then
                 call stdlib_I64_dlasrt( 'I', m, w, iinfo )
                 if( iinfo/=0_ilp64 ) then
                    info = 3_ilp64
                    return
                 end if
     ...
     !! with CUNMTR or CUPMTR.
        ...
        ! =====================================================================
           ...
           ! stdlib_I64_sstemr needs work of size 6*n, iwork of size 3*n.
           ! in addition, stdlib_I64_slarre needs work of size 6*n, iwork of size 5*n.
           ! furthermore, stdlib_I64_clarrv needs work of size 12*n, iwork of size 7*n.
           if( wantz ) then
              lwmin = 18_ilp64*n
              liwmin = 10_ilp64*n
           else
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CSTEMR', -info )
              return
           else if( lquery .or. zquery ) then
              return
           end if
           ...
           if( n==2_ilp64 ) then
              ...
              if( .not.wantz ) then
                 ...
                 ! stdlib_I64_slarre computes the eigenvalues to less than full precision.
                 ! stdlib_I64_clarrv will refine the eigenvalue approximations, and we only
                 ! need less accurate initial bisection in stdlib_I64_slarre.
                 ! note: these settings do only affect the subset case and stdlib_I64_slarre
                 rtol1 = max( sqrt(eps)*5.0e-2_sp, four * eps )
                 rtol2 = max( sqrt(eps)*5.0e-3_sp, four * eps )
              ...
              if( wantz ) then
                 ! compute the desired eigenvectors corresponding to the computed
                 ! eigenvalues
                 call stdlib_I64_clarrv( n, wl, wu, d, e,pivmin, iwork( iinspl ), m,1_ilp64, m, minrgp, &
                 rtol1, rtol2,w, work( inderr ), work( indgp ), iwork( iindbl ),iwork( iindw ), &
                           work( indgrs ), z, ldz,isuppz, work( indwrk ), iwork( iindwk ), iinfo )
                 if( iinfo/=0_ilp64 ) then
                    info = 20_ilp64 + abs( iinfo )
                 ...
                 ! stdlib_I64_slarre computes eigenvalues of the (shifted) root representation
                 ! stdlib_I64_clarrv returns the eigenvalues of the unshifted matrix.
                 ! however, if the eigenvectors are not desired by the user, we need
                 ! to apply the corresponding shifts from stdlib_I64_slarre to obtain the
                 ! eigenvalues of the original matrix.
                 do j = 1, m
     ...
     !! with ZUNMTR or ZUPMTR.
        ...
        ! =====================================================================
           ...
           ! stdlib_I64_dstemr needs work of size 6*n, iwork of size 3*n.
           ! in addition, stdlib_I64_dlarre needs work of size 6*n, iwork of size 5*n.
           ! furthermore, stdlib_I64_zlarrv needs work of size 12*n, iwork of size 7*n.
           if( wantz ) then
              lwmin = 18_ilp64*n
              liwmin = 10_ilp64*n
           else
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZSTEMR', -info )
              return
           else if( lquery .or. zquery ) then
              return
           end if
           ...
           if( n==2_ilp64 ) then
              ...
              if( .not.wantz ) then
                 ...
                 ! stdlib_I64_dlarre computes the eigenvalues to less than full precision.
                 ! stdlib_I64_zlarrv will refine the eigenvalue approximations, and we only
                 ! need less accurate initial bisection in stdlib_I64_dlarre.
                 ! note: these settings do only affect the subset case and stdlib_I64_dlarre
                 rtol1 = sqrt(eps)
                 rtol2 = max( sqrt(eps)*5.0e-3_dp, four * eps )
              ...
              if( wantz ) then
                 ! compute the desired eigenvectors corresponding to the computed
                 ! eigenvalues
                 call stdlib_I64_zlarrv( n, wl, wu, d, e,pivmin, iwork( iinspl ), m,1_ilp64, m, minrgp, &
                 rtol1, rtol2,w, work( inderr ), work( indgp ), iwork( iindbl ),iwork( iindw ), &
                           work( indgrs ), z, ldz,isuppz, work( indwrk ), iwork( iindwk ), iinfo )
                 if( iinfo/=0_ilp64 ) then
                    info = 20_ilp64 + abs( iinfo )
                 ...
                 ! stdlib_I64_dlarre computes eigenvalues of the (shifted) root representation
                 ! stdlib_I64_zlarrv returns the eigenvalues of the unshifted matrix.
                 ! however, if the eigenvectors are not desired by the user, we need
                 ! to apply the corresponding shifts from stdlib_I64_dlarre to obtain the
                 ! eigenvalues of the original matrix.
                 do j = 1, m
     ...
     !! tridiagonal form.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           integer(ilp64) :: i, icompz, ii, iscale, j, jtot, k, l, l1, lend, lendm1, lendp1, lendsv,&
                      lm1, lsv, m, mm, mm1, nm1, nmaxit
           real(sp) :: anorm, b, c, eps, eps2, f, g, p, r, rt1, rt2, s, safmax, safmin, ssfmax, &
                     ssfmin, tst
           ! Intrinsic Functions 
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SSTEQR', -info )
              return
           end if
           ! quick return if possible
           if( n==0 )return
           if( n==1_ilp64 ) then
              if( icompz==2_ilp64 )z( 1_ilp64, 1_ilp64 ) = one
              return
           end if
           ! determine the unit roundoff and over/underflow thresholds.
           eps = stdlib_I64_slamch( 'E' )
           ...
           if( lend>l ) then
              ...
              p = d( l )
              if( m==l )go to 80
              ! if remaining matrix is 2-by-2, use stdlib_slae2 or stdlib_I64_slaev2
              ! to compute its eigensystem.
              if( m==l+1 ) then
                 if( icompz>0_ilp64 ) then
                    call stdlib_I64_slaev2( d( l ), e( l ), d( l+1 ), rt1, rt2, c, s )
                    work( l ) = c
                    work( n-1+l ) = s
                    call stdlib_I64_slasr( 'R', 'V', 'B', n, 2_ilp64, work( l ),work( n-1+l ), z( 1_ilp64, l ), &
                              ldz )
                 else
                    call stdlib_I64_slae2( d( l ), e( l ), d( l+1 ), rt1, rt2 )
                 end if
                 d( l ) = rt1
                 d( l+1 ) = rt2
                 e( l ) = zero
              ...
              do i = mm1, l, -1
                 f = s*e( i )
                 b = c*e( i )
                 call stdlib_I64_slartg( g, f, c, s, r )
                 if( i/=m-1 )e( i+1 ) = r
                 g = d( i+1 ) - p
                 r = ( d( i )-g )*s + two*c*b
                 p = s*r
              ...
              if( icompz>0_ilp64 ) then
                 mm = m - l + 1_ilp64
                 call stdlib_I64_slasr( 'R', 'V', 'B', n, mm, work( l ), work( n-1+l ),z( 1_ilp64, l ), ldz &
                           )
              end if
              d( l ) = d( l ) - p
              e( l ) = g
              ...
              if( m==l-1 ) then
                 if( icompz>0_ilp64 ) then
                    call stdlib_I64_slaev2( d( l-1 ), e( l-1 ), d( l ), rt1, rt2, c, s )
                    work( m ) = c
                    work( n-1+m ) = s
                    call stdlib_I64_slasr( 'R', 'V', 'F', n, 2_ilp64, work( m ),work( n-1+m ), z( 1_ilp64, l-1 ), &
                              ldz )
                 else
                    call stdlib_I64_slae2( d( l-1 ), e( l-1 ), d( l ), rt1, rt2 )
                 end if
                 d( l-1 ) = rt1
                 d( l ) = rt2
                 e( l-1 ) = zero
              ...
              if( icompz>0_ilp64 ) then
                 mm = l - m + 1_ilp64
                 call stdlib_I64_slasr( 'R', 'V', 'F', n, mm, work( m ), work( n-1+m ),z( 1_ilp64, m ), ldz &
                           )
              end if
              d( l ) = d( l ) - p
              e( lm1 ) = g
           ...
           140 continue
           if( iscale==1_ilp64 ) then
              call stdlib_I64_slascl( 'G', 0_ilp64, 0_ilp64, ssfmax, anorm, lendsv-lsv+1, 1_ilp64,d( lsv ), n, info )
                        
              call stdlib_I64_slascl( 'G', 0_ilp64, 0_ilp64, ssfmax, anorm, lendsv-lsv, 1_ilp64, e( lsv ),n, info )
                        
           else if( iscale==2_ilp64 ) then
              call stdlib_I64_slascl( 'G', 0_ilp64, 0_ilp64, ssfmin, anorm, lendsv-lsv+1, 1_ilp64,d( lsv ), n, info )
                        
              call stdlib_I64_slascl( 'G', 0_ilp64, 0_ilp64, ssfmin, anorm, lendsv-lsv, 1_ilp64, e( lsv ),n, info )
     ...
     !! tridiagonal form.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DSTEQR', -info )
              return
           end if
           ! quick return if possible
           if( n==0 )return
           ...
           if( lend>l ) then
              ...
              p = d( l )
              if( m==l )go to 80
              ! if remaining matrix is 2-by-2, use stdlib_dlae2 or stdlib_I64_slaev2
              ! to compute its eigensystem.
              if( m==l+1 ) then
                 if( icompz>0_ilp64 ) then
                    call stdlib_I64_dlaev2( d( l ), e( l ), d( l+1 ), rt1, rt2, c, s )
                    work( l ) = c
                    work( n-1+l ) = s
                    call stdlib_I64_dlasr( 'R', 'V', 'B', n, 2_ilp64, work( l ),work( n-1+l ), z( 1_ilp64, l ), &
                              ldz )
                 else
                    call stdlib_I64_dlae2( d( l ), e( l ), d( l+1 ), rt1, rt2 )
                 end if
                 d( l ) = rt1
                 d( l+1 ) = rt2
                 e( l ) = zero
              ...
              do i = mm1, l, -1
                 f = s*e( i )
                 b = c*e( i )
                 call stdlib_I64_dlartg( g, f, c, s, r )
                 if( i/=m-1 )e( i+1 ) = r
                 g = d( i+1 ) - p
                 r = ( d( i )-g )*s + two*c*b
                 p = s*r
              ...
              if( icompz>0_ilp64 ) then
                 mm = m - l + 1_ilp64
                 call stdlib_I64_dlasr( 'R', 'V', 'B', n, mm, work( l ), work( n-1+l ),z( 1_ilp64, l ), ldz &
                           )
              end if
              d( l ) = d( l ) - p
              e( l ) = g
              ...
              if( m==l-1 ) then
                 if( icompz>0_ilp64 ) then
                    call stdlib_I64_dlaev2( d( l-1 ), e( l-1 ), d( l ), rt1, rt2, c, s )
                    work( m ) = c
                    work( n-1+m ) = s
                    call stdlib_I64_dlasr( 'R', 'V', 'F', n, 2_ilp64, work( m ),work( n-1+m ), z( 1_ilp64, l-1 ), &
                              ldz )
                 else
                    call stdlib_I64_dlae2( d( l-1 ), e( l-1 ), d( l ), rt1, rt2 )
                 end if
                 d( l-1 ) = rt1
                 d( l ) = rt2
                 e( l-1 ) = zero
              ...
              if( icompz>0_ilp64 ) then
                 mm = l - m + 1_ilp64
                 call stdlib_I64_dlasr( 'R', 'V', 'F', n, mm, work( m ), work( n-1+m ),z( 1_ilp64, m ), ldz &
                           )
              end if
              d( l ) = d( l ) - p
              e( lm1 ) = g
           ...
           140 continue
           if( iscale==1_ilp64 ) then
              call stdlib_I64_dlascl( 'G', 0_ilp64, 0_ilp64, ssfmax, anorm, lendsv-lsv+1, 1_ilp64,d( lsv ), n, info )
                        
              call stdlib_I64_dlascl( 'G', 0_ilp64, 0_ilp64, ssfmax, anorm, lendsv-lsv, 1_ilp64, e( lsv ),n, info )
                        
           else if( iscale==2_ilp64 ) then
              call stdlib_I64_dlascl( 'G', 0_ilp64, 0_ilp64, ssfmin, anorm, lendsv-lsv+1, 1_ilp64,d( lsv ), n, info )
                        
              call stdlib_I64_dlascl( 'G', 0_ilp64, 0_ilp64, ssfmin, anorm, lendsv-lsv, 1_ilp64, e( lsv ),n, info )
     ...
     !! matrix to tridiagonal form.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CSTEQR', -info )
              return
           end if
           ! quick return if possible
           if( n==0 )return
           if( n==1_ilp64 ) then
              if( icompz==2_ilp64 )z( 1_ilp64, 1_ilp64 ) = cone
              return
           end if
           ! determine the unit roundoff and over/underflow thresholds.
           eps = stdlib_I64_slamch( 'E' )
           ...
           if( lend==l )go to 10
           ! scale submatrix in rows and columns l to lend
           anorm = stdlib_I64_slanst( 'I', lend-l+1, d( l ), e( l ) )
           iscale = 0_ilp64
           if( anorm==zero )go to 10
           if( anorm>ssfmax ) then
              iscale = 1_ilp64
           ...
           if( lend>l ) then
              ...
              if( m==l+1 ) then
                 if( icompz>0_ilp64 ) then
                    ...
                    work( l ) = c
                    work( n-1+l ) = s
                    call stdlib_I64_clasr( 'R', 'V', 'B', n, 2_ilp64, work( l ),work( n-1+l ), z( 1_ilp64, l ), &
                              ldz )
                 else
                    call stdlib_I64_slae2( d( l ), e( l ), d( l+1 ), rt1, rt2 )
                 end if
              ...
              if( icompz>0_ilp64 ) then
                 mm = m - l + 1_ilp64
                 call stdlib_I64_clasr( 'R', 'V', 'B', n, mm, work( l ), work( n-1+l ),z( 1_ilp64, l ), ldz &
                           )
              end if
              d( l ) = d( l ) - p
              e( l ) = g
              ...
              if( m==l-1 ) then
                 if( icompz>0_ilp64 ) then
                    ...
                    work( m ) = c
                    work( n-1+m ) = s
                    call stdlib_I64_clasr( 'R', 'V', 'F', n, 2_ilp64, work( m ),work( n-1+m ), z( 1_ilp64, l-1 ), &
                              ldz )
                 else
                    call stdlib_I64_slae2( d( l-1 ), e( l-1 ), d( l ), rt1, rt2 )
                 end if
              ...
              if( icompz>0_ilp64 ) then
                 mm = l - m + 1_ilp64
                 call stdlib_I64_clasr( 'R', 'V', 'F', n, mm, work( m ), work( n-1+m ),z( 1_ilp64, m ), ldz &
                           )
              end if
              d( l ) = d( l ) - p
              e( lm1 ) = g
     ...
     !! matrix to tridiagonal form.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZSTEQR', -info )
              return
           end if
           ! quick return if possible
           if( n==0 )return
           ...
           if( lend==l )go to 10
           ! scale submatrix in rows and columns l to lend
           anorm = stdlib_I64_dlanst( 'I', lend-l+1, d( l ), e( l ) )
           iscale = 0_ilp64
           if( anorm==zero )go to 10
           if( anorm>ssfmax ) then
              iscale = 1_ilp64
           ...
           if( lend>l ) then
              ...
              if( m==l+1 ) then
                 if( icompz>0_ilp64 ) then
                    ...
                    work( l ) = c
                    work( n-1+l ) = s
                    call stdlib_I64_zlasr( 'R', 'V', 'B', n, 2_ilp64, work( l ),work( n-1+l ), z( 1_ilp64, l ), &
                              ldz )
                 else
                    call stdlib_I64_dlae2( d( l ), e( l ), d( l+1 ), rt1, rt2 )
                 end if
              ...
              if( icompz>0_ilp64 ) then
                 mm = m - l + 1_ilp64
                 call stdlib_I64_zlasr( 'R', 'V', 'B', n, mm, work( l ), work( n-1+l ),z( 1_ilp64, l ), ldz &
                           )
              end if
              d( l ) = d( l ) - p
              e( l ) = g
              ...
              if( m==l-1 ) then
                 if( icompz>0_ilp64 ) then
                    ...
                    work( m ) = c
                    work( n-1+m ) = s
                    call stdlib_I64_zlasr( 'R', 'V', 'F', n, 2_ilp64, work( m ),work( n-1+m ), z( 1_ilp64, l-1 ), &
                              ldz )
                 else
                    call stdlib_I64_dlae2( d( l-1 ), e( l-1 ), d( l ), rt1, rt2 )
                 end if
              ...
              if( icompz>0_ilp64 ) then
                 mm = l - m + 1_ilp64
                 call stdlib_I64_zlasr( 'R', 'V', 'F', n, mm, work( m ), work( n-1+m ),z( 1_ilp64, m ), ldz &
                           )
              end if
              d( l ) = d( l ) - p
              e( lm1 ) = g
