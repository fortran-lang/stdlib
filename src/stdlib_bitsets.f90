...
!! Error flag indicating an invalid character string
    integer, parameter, public :: char_string_too_small_error = 4
!! Error flag indicating a too small character string
    integer, parameter, public :: index_invalid_error = 5
!! Error flag indicating an invalid index
    integer, parameter, public :: integer_overflow_error = 6
!! Error flag indicating integer overflow
    integer, parameter, public :: read_failure = 7
!! Error flag indicating failure of a READ statement
    integer, parameter, public :: eof_failure = 8
!! Error flag indicating unexpected End-of-File on a READ
    integer, parameter, public :: write_failure = 9
...
!!
            import :: bitset_t
            class(bitset_t), intent(out)   :: self
...
!! `array_size_invalid_error` if `bits` is either negative or larger than 64
!! if `self` is class `bitset_64`, or the value `alloc_fault` if it failed
...
!! literal has an invlaaid character, or `array_size_invalid_error` if
!! `bits(self)` in `string` is greater than 64 for a `bitset_64`.
!!
!!#### Example
!!
!!    ```fortran
...
!! too large to be represented, the value `alloc_fault` if allocation of
!! memory for `self` failed, or `char_string_invalid_error` if the bitset
!! literal has an invlaid character, or `array_size_invalid_error` if
...
!! `bitset_large`.
    ...
    type, extends(bitset_t) :: bitset_64
!! Version: experimental
!!
...
!!
!! Initializes the bitset `self` treating `string` as a binary literal
!! `status` has the default value `success`, the value `alloc_fault` if the
!! allocation of the bits in self failed, `array_size_invalid_error` if the
!! `len(string)>64` for a `bitset_64`, or `char_string_invalid` if an invalid
...
!! has a default value of `success`. If an error occurs it has the value
!! `array_size_invalid_error` if `bits` is either negative larger than 64
!! for `self` of type `bitset_64`, or the value `alloc_fault` if it failed
!! during allocation of memory for `self`.
            class(bitset_64), intent(out)   :: self
            integer(bits_kind), intent(in) :: bits
            integer, intent(out), optional :: status
