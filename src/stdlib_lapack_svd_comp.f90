  ...
  contains
     ...
     !! If m >= n, B is upper bidiagonal; if m < n, B is lower bidiagonal.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           logical(lk) :: lquery
           integer(ilp64) :: i, iinfo, j, ldwrkx, ldwrky, lwkopt, minmn, nb, nbmin, nx, ws
           ! Intrinsic Functions 
           ! Executable Statements 
           ! test the input parameters
           info = 0_ilp64
           nb = max( 1_ilp64, stdlib_I64_ilaenv( 1_ilp64, 'SGEBRD', ' ', m, n, -1_ilp64, -1_ilp64 ) )
           lwkopt = ( m+n )*nb
           ...
           if( m<0_ilp64 ) then
              ...
              info = -4_ilp64
           else if( lwork<max( 1_ilp64, m, n ) .and. .not.lquery ) then
              info = -10_ilp64
           end if
           if( info<0_ilp64 ) then
              call stdlib_I64_xerbla( 'SGEBRD', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           ldwrkx = m
           ldwrky = n
           if( nb>1_ilp64 .and. nb<minmn ) then
              ! set the crossover point nx.
              nx = max( nb, stdlib_I64_ilaenv( 3_ilp64, 'SGEBRD', ' ', m, n, -1_ilp64, -1_ilp64 ) )
              ! determine when to switch from blocked to unblocked code.
              if( nx<minmn ) then
                 ws = ( m+n )*nb
                 if( lwork<ws ) then
                    ! not enough work space for the optimal nb, consider using
                    ! a smaller block size.
                    nbmin = stdlib_I64_ilaenv( 2_ilp64, 'SGEBRD', ' ', m, n, -1_ilp64, -1_ilp64 )
                    if( lwork>=( m+n )*nbmin ) then
                       nb = lwork / ( m+n )
                    else
                       nb = 1_ilp64
           ...
           do i = 1, minmn - nx, nb
              ...
              ! the matrices x and y which are needed to update the unreduced
              ! part of the matrix
              call stdlib_I64_slabrd( m-i+1, n-i+1, nb, a( i, i ), lda, d( i ), e( i ),tauq( i ), &
                        taup( i ), work, ldwrkx,work( ldwrkx*nb+1 ), ldwrky )
              ! update the trailing submatrix a(i+nb:m,i+nb:n), using an update
              ! of the form  a := a - v*y**t - x*u**t
              call stdlib_I64_sgemm( 'NO TRANSPOSE', 'TRANSPOSE', m-i-nb+1, n-i-nb+1,nb, -one, a( i+&
                        nb, i ), lda,work( ldwrkx*nb+nb+1 ), ldwrky, one,a( i+nb, i+nb ), lda )
              call stdlib_I64_sgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', m-i-nb+1, n-i-nb+1,nb, -one, &
                        work( nb+1 ), ldwrkx, a( i, i+nb ), lda,one, a( i+nb, i+nb ), lda )
              ! copy diagonal and off-diagonal elements of b back into a
              if( m>=n ) then
                 do j = i, i + nb - 1
           ...
           ! use unblocked code to reduce the remainder of the matrix
           call stdlib_I64_sgebd2( m-i+1, n-i+1, a( i, i ), lda, d( i ), e( i ),tauq( i ), taup( i ), &
                     work, iinfo )
           work( 1_ilp64 ) = ws
           return
     ...
     !! If m >= n, B is upper bidiagonal; if m < n, B is lower bidiagonal.
        ...
        ! =====================================================================
           ...
           ! test the input parameters
           info = 0_ilp64
           nb = max( 1_ilp64, stdlib_I64_ilaenv( 1_ilp64, 'DGEBRD', ' ', m, n, -1_ilp64, -1_ilp64 ) )
           lwkopt = ( m+n )*nb
           ...
           if( info<0_ilp64 ) then
              call stdlib_I64_xerbla( 'DGEBRD', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           if( nb>1_ilp64 .and. nb<minmn ) then
              ! set the crossover point nx.
              nx = max( nb, stdlib_I64_ilaenv( 3_ilp64, 'DGEBRD', ' ', m, n, -1_ilp64, -1_ilp64 ) )
              ! determine when to switch from blocked to unblocked code.
              if( nx<minmn ) then
                 ws = ( m+n )*nb
                 if( lwork<ws ) then
                    ! not enough work space for the optimal nb, consider using
                    ! a smaller block size.
                    nbmin = stdlib_I64_ilaenv( 2_ilp64, 'DGEBRD', ' ', m, n, -1_ilp64, -1_ilp64 )
                    if( lwork>=( m+n )*nbmin ) then
                       nb = lwork / ( m+n )
                    else
                       nb = 1_ilp64
           ...
           do i = 1, minmn - nx, nb
              ...
              ! the matrices x and y which are needed to update the unreduced
              ! part of the matrix
              call stdlib_I64_dlabrd( m-i+1, n-i+1, nb, a( i, i ), lda, d( i ), e( i ),tauq( i ), &
                        taup( i ), work, ldwrkx,work( ldwrkx*nb+1 ), ldwrky )
              ! update the trailing submatrix a(i+nb:m,i+nb:n), using an update
              ! of the form  a := a - v*y**t - x*u**t
              call stdlib_I64_dgemm( 'NO TRANSPOSE', 'TRANSPOSE', m-i-nb+1, n-i-nb+1,nb, -one, a( i+&
                        nb, i ), lda,work( ldwrkx*nb+nb+1 ), ldwrky, one,a( i+nb, i+nb ), lda )
              call stdlib_I64_dgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', m-i-nb+1, n-i-nb+1,nb, -one, &
                        work( nb+1 ), ldwrkx, a( i, i+nb ), lda,one, a( i+nb, i+nb ), lda )
              ! copy diagonal and off-diagonal elements of b back into a
              if( m>=n ) then
                 do j = i, i + nb - 1
           ...
           ! use unblocked code to reduce the remainder of the matrix
           call stdlib_I64_dgebd2( m-i+1, n-i+1, a( i, i ), lda, d( i ), e( i ),tauq( i ), taup( i ), &
                     work, iinfo )
           work( 1_ilp64 ) = ws
           return
     ...
     !! If m >= n, B is upper bidiagonal; if m < n, B is lower bidiagonal.
        ...
        ! =====================================================================
           ...
           ! test the input parameters
           info = 0_ilp64
           nb = max( 1_ilp64, stdlib_I64_ilaenv( 1_ilp64, 'CGEBRD', ' ', m, n, -1_ilp64, -1_ilp64 ) )
           lwkopt = ( m+n )*nb
           work( 1_ilp64 ) = real( lwkopt,KIND=sp)
           lquery = ( lwork==-1_ilp64 )
           if( m<0_ilp64 ) then
           ...
           if( info<0_ilp64 ) then
              call stdlib_I64_xerbla( 'CGEBRD', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           if( nb>1_ilp64 .and. nb<minmn ) then
              ! set the crossover point nx.
              nx = max( nb, stdlib_I64_ilaenv( 3_ilp64, 'CGEBRD', ' ', m, n, -1_ilp64, -1_ilp64 ) )
              ! determine when to switch from blocked to unblocked code.
              if( nx<minmn ) then
                 ws = ( m+n )*nb
                 if( lwork<ws ) then
                    ! not enough work space for the optimal nb, consider using
                    ! a smaller block size.
                    nbmin = stdlib_I64_ilaenv( 2_ilp64, 'CGEBRD', ' ', m, n, -1_ilp64, -1_ilp64 )
                    if( lwork>=( m+n )*nbmin ) then
                       nb = lwork / ( m+n )
                    else
                       nb = 1_ilp64
           ...
           do i = 1, minmn - nx, nb
              ...
              ! the matrices x and y which are needed to update the unreduced
              ! part of the matrix
              call stdlib_I64_clabrd( m-i+1, n-i+1, nb, a( i, i ), lda, d( i ), e( i ),tauq( i ), &
                        taup( i ), work, ldwrkx,work( ldwrkx*nb+1 ), ldwrky )
              ! update the trailing submatrix a(i+ib:m,i+ib:n), using
              ! an update of the form  a := a - v*y**h - x*u**h
              call stdlib_I64_cgemm( 'NO TRANSPOSE', 'CONJUGATE TRANSPOSE', m-i-nb+1,n-i-nb+1, nb, -&
              cone, a( i+nb, i ), lda,work( ldwrkx*nb+nb+1 ), ldwrky, cone,a( i+nb, i+nb ), lda )
                        
              call stdlib_I64_cgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', m-i-nb+1, n-i-nb+1,nb, -cone, &
                        work( nb+1 ), ldwrkx, a( i, i+nb ), lda,cone, a( i+nb, i+nb ), lda )
              ! copy diagonal and off-diagonal elements of b back into a
              if( m>=n ) then
                 do j = i, i + nb - 1
           ...
           ! use unblocked code to reduce the remainder of the matrix
           call stdlib_I64_cgebd2( m-i+1, n-i+1, a( i, i ), lda, d( i ), e( i ),tauq( i ), taup( i ), &
                     work, iinfo )
           work( 1_ilp64 ) = ws
           return
     ...
     !! If m >= n, B is upper bidiagonal; if m < n, B is lower bidiagonal.
        ...
        ! =====================================================================
           ...
           ! test the input parameters
           info = 0_ilp64
           nb = max( 1_ilp64, stdlib_I64_ilaenv( 1_ilp64, 'ZGEBRD', ' ', m, n, -1_ilp64, -1_ilp64 ) )
           lwkopt = ( m+n )*nb
           work( 1_ilp64 ) = real( lwkopt,KIND=dp)
           lquery = ( lwork==-1_ilp64 )
           if( m<0_ilp64 ) then
           ...
           if( info<0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZGEBRD', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           if( nb>1_ilp64 .and. nb<minmn ) then
              ! set the crossover point nx.
              nx = max( nb, stdlib_I64_ilaenv( 3_ilp64, 'ZGEBRD', ' ', m, n, -1_ilp64, -1_ilp64 ) )
              ! determine when to switch from blocked to unblocked code.
              if( nx<minmn ) then
                 ws = ( m+n )*nb
                 if( lwork<ws ) then
                    ! not enough work space for the optimal nb, consider using
                    ! a smaller block size.
                    nbmin = stdlib_I64_ilaenv( 2_ilp64, 'ZGEBRD', ' ', m, n, -1_ilp64, -1_ilp64 )
                    if( lwork>=( m+n )*nbmin ) then
                       nb = lwork / ( m+n )
                    else
                       nb = 1_ilp64
           ...
           do i = 1, minmn - nx, nb
              ...
              ! the matrices x and y which are needed to update the unreduced
              ! part of the matrix
              call stdlib_I64_zlabrd( m-i+1, n-i+1, nb, a( i, i ), lda, d( i ), e( i ),tauq( i ), &
                        taup( i ), work, ldwrkx,work( ldwrkx*nb+1 ), ldwrky )
              ! update the trailing submatrix a(i+ib:m,i+ib:n), using
              ! an update of the form  a := a - v*y**h - x*u**h
              call stdlib_I64_zgemm( 'NO TRANSPOSE', 'CONJUGATE TRANSPOSE', m-i-nb+1,n-i-nb+1, nb, -&
              cone, a( i+nb, i ), lda,work( ldwrkx*nb+nb+1 ), ldwrky, cone,a( i+nb, i+nb ), lda )
                        
              call stdlib_I64_zgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', m-i-nb+1, n-i-nb+1,nb, -cone, &
                        work( nb+1 ), ldwrkx, a( i, i+nb ), lda,cone, a( i+nb, i+nb ), lda )
              ! copy diagonal and off-diagonal elements of b back into a
              if( m>=n ) then
                 do j = i, i + nb - 1
           ...
           ! use unblocked code to reduce the remainder of the matrix
           call stdlib_I64_zgebd2( m-i+1, n-i+1, a( i, i ), lda, d( i ), e( i ),tauq( i ), taup( i ), &
                     work, iinfo )
           work( 1_ilp64 ) = ws
           return
     ...
     !! If m >= n, B is upper bidiagonal; if m < n, B is lower bidiagonal.
        ...
        ! =====================================================================
           ...
           if( info<0_ilp64 ) then
              call stdlib_I64_xerbla( 'SGEBD2', -info )
              return
           end if
           if( m>=n ) then
              ! reduce to upper bidiagonal form
              do i = 1, n
                 ! generate elementary reflector h(i) to annihilate a(i+1:m,i)
                 call stdlib_I64_slarfg( m-i+1, a( i, i ), a( min( i+1, m ), i ), 1_ilp64,tauq( i ) )
                           
                 d( i ) = a( i, i )
                 a( i, i ) = one
                 ! apply h(i) to a(i:m,i+1:n) from the left
                 if( i<n )call stdlib_I64_slarf( 'LEFT', m-i+1, n-i, a( i, i ), 1_ilp64, tauq( i ),a( i, i+&
                           1_ilp64 ), lda, work )
                 a( i, i ) = d( i )
                 if( i<n ) then
                    ! generate elementary reflector g(i) to annihilate
                    ! a(i,i+2:n)
                    call stdlib_I64_slarfg( n-i, a( i, i+1 ), a( i, min( i+2, n ) ),lda, taup( i ) )
                              
                    e( i ) = a( i, i+1 )
                    a( i, i+1 ) = one
                    ! apply g(i) to a(i+1:m,i+1:n) from the right
                    call stdlib_I64_slarf( 'RIGHT', m-i, n-i, a( i, i+1 ), lda,taup( i ), a( i+1, i+1 &
                              ), lda, work )
                    a( i, i+1 ) = e( i )
                 else
                    taup( i ) = zero
              ...
              do i = 1, m
                 ! generate elementary reflector g(i) to annihilate a(i,i+1:n)
                 call stdlib_I64_slarfg( n-i+1, a( i, i ), a( i, min( i+1, n ) ), lda,taup( i ) )
                           
                 d( i ) = a( i, i )
                 a( i, i ) = one
                 ! apply g(i) to a(i+1:m,i:n) from the right
                 if( i<m )call stdlib_I64_slarf( 'RIGHT', m-i, n-i+1, a( i, i ), lda,taup( i ), a( i+&
                           1_ilp64, i ), lda, work )
                 a( i, i ) = d( i )
                 if( i<m ) then
                    ! generate elementary reflector h(i) to annihilate
                    ! a(i+2:m,i)
                    call stdlib_I64_slarfg( m-i, a( i+1, i ), a( min( i+2, m ), i ), 1_ilp64,tauq( i ) )
                              
                    e( i ) = a( i+1, i )
                    a( i+1, i ) = one
                    ! apply h(i) to a(i+1:m,i+1:n) from the left
                    call stdlib_I64_slarf( 'LEFT', m-i, n-i, a( i+1, i ), 1_ilp64, tauq( i ),a( i+1, i+1 ), &
                              lda, work )
                    a( i+1, i ) = e( i )
                 else
                    tauq( i ) = zero
     ...
     !! If m >= n, B is upper bidiagonal; if m < n, B is lower bidiagonal.
        ...
        ! =====================================================================
           ...
           if( info<0_ilp64 ) then
              call stdlib_I64_xerbla( 'DGEBD2', -info )
              return
           end if
           if( m>=n ) then
              ! reduce to upper bidiagonal form
              do i = 1, n
                 ! generate elementary reflector h(i) to annihilate a(i+1:m,i)
                 call stdlib_I64_dlarfg( m-i+1, a( i, i ), a( min( i+1, m ), i ), 1_ilp64,tauq( i ) )
                           
                 d( i ) = a( i, i )
                 a( i, i ) = one
                 ! apply h(i) to a(i:m,i+1:n) from the left
                 if( i<n )call stdlib_I64_dlarf( 'LEFT', m-i+1, n-i, a( i, i ), 1_ilp64, tauq( i ),a( i, i+&
                           1_ilp64 ), lda, work )
                 a( i, i ) = d( i )
                 if( i<n ) then
                    ! generate elementary reflector g(i) to annihilate
                    ! a(i,i+2:n)
                    call stdlib_I64_dlarfg( n-i, a( i, i+1 ), a( i, min( i+2, n ) ),lda, taup( i ) )
                              
                    e( i ) = a( i, i+1 )
                    a( i, i+1 ) = one
                    ! apply g(i) to a(i+1:m,i+1:n) from the right
                    call stdlib_I64_dlarf( 'RIGHT', m-i, n-i, a( i, i+1 ), lda,taup( i ), a( i+1, i+1 &
                              ), lda, work )
                    a( i, i+1 ) = e( i )
                 else
                    taup( i ) = zero
              ...
              do i = 1, m
                 ! generate elementary reflector g(i) to annihilate a(i,i+1:n)
                 call stdlib_I64_dlarfg( n-i+1, a( i, i ), a( i, min( i+1, n ) ), lda,taup( i ) )
                           
                 d( i ) = a( i, i )
                 a( i, i ) = one
                 ! apply g(i) to a(i+1:m,i:n) from the right
                 if( i<m )call stdlib_I64_dlarf( 'RIGHT', m-i, n-i+1, a( i, i ), lda,taup( i ), a( i+&
                           1_ilp64, i ), lda, work )
                 a( i, i ) = d( i )
                 if( i<m ) then
                    ! generate elementary reflector h(i) to annihilate
                    ! a(i+2:m,i)
                    call stdlib_I64_dlarfg( m-i, a( i+1, i ), a( min( i+2, m ), i ), 1_ilp64,tauq( i ) )
                              
                    e( i ) = a( i+1, i )
                    a( i+1, i ) = one
                    ! apply h(i) to a(i+1:m,i+1:n) from the left
                    call stdlib_I64_dlarf( 'LEFT', m-i, n-i, a( i+1, i ), 1_ilp64, tauq( i ),a( i+1, i+1 ), &
                              lda, work )
                    a( i+1, i ) = e( i )
                 else
                    tauq( i ) = zero
     ...
     !! If m >= n, B is upper bidiagonal; if m < n, B is lower bidiagonal.
        ...
        ! =====================================================================
           ...
           if( info<0_ilp64 ) then
              call stdlib_I64_xerbla( 'CGEBD2', -info )
              return
           end if
           if( m>=n ) then
              ! reduce to upper bidiagonal form
              do i = 1, n
                 ! generate elementary reflector h(i) to annihilate a(i+1:m,i)
                 alpha = a( i, i )
                 call stdlib_I64_clarfg( m-i+1, alpha, a( min( i+1, m ), i ), 1_ilp64,tauq( i ) )
                 d( i ) = real( alpha,KIND=sp)
                 a( i, i ) = cone
                 ! apply h(i)**h to a(i:m,i+1:n) from the left
                 if( i<n )call stdlib_I64_clarf( 'LEFT', m-i+1, n-i, a( i, i ), 1_ilp64,conjg( tauq( i ) ), &
                           a( i, i+1 ), lda, work )
                 a( i, i ) = d( i )
                 if( i<n ) then
                    ! generate elementary reflector g(i) to annihilate
                    ...
                    alpha = a( i, i+1 )
                    call stdlib_I64_clarfg( n-i, alpha, a( i, min( i+2, n ) ),lda, taup( i ) )
                    e( i ) = real( alpha,KIND=sp)
                    a( i, i+1 ) = cone
                    ! apply g(i) to a(i+1:m,i+1:n) from the right
                    call stdlib_I64_clarf( 'RIGHT', m-i, n-i, a( i, i+1 ), lda,taup( i ), a( i+1, i+1 &
                              ), lda, work )
                    call stdlib_I64_clacgv( n-i, a( i, i+1 ), lda )
                    a( i, i+1 ) = e( i )
                 else
              ...
              do i = 1, m
                 ...
                 alpha = a( i, i )
                 call stdlib_I64_clarfg( n-i+1, alpha, a( i, min( i+1, n ) ), lda,taup( i ) )
                 d( i ) = real( alpha,KIND=sp)
                 a( i, i ) = cone
                 ! apply g(i) to a(i+1:m,i:n) from the right
                 if( i<m )call stdlib_I64_clarf( 'RIGHT', m-i, n-i+1, a( i, i ), lda,taup( i ), a( i+&
                           1_ilp64, i ), lda, work )
                 call stdlib_I64_clacgv( n-i+1, a( i, i ), lda )
                 a( i, i ) = d( i )
                 if( i<m ) then
                    ...
                    ! a(i+2:m,i)
                    alpha = a( i+1, i )
                    call stdlib_I64_clarfg( m-i, alpha, a( min( i+2, m ), i ), 1_ilp64,tauq( i ) )
                    e( i ) = real( alpha,KIND=sp)
                    a( i+1, i ) = cone
                    ! apply h(i)**h to a(i+1:m,i+1:n) from the left
                    call stdlib_I64_clarf( 'LEFT', m-i, n-i, a( i+1, i ), 1_ilp64,conjg( tauq( i ) ), a( i+&
                              1_ilp64, i+1 ), lda,work )
                    a( i+1, i ) = e( i )
                 else
                    tauq( i ) = czero
                 end if
     ...
     !! If m >= n, B is upper bidiagonal; if m < n, B is lower bidiagonal.
        ...
        ! =====================================================================
           ...
           if( info<0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZGEBD2', -info )
              return
           end if
           if( m>=n ) then
              ! reduce to upper bidiagonal form
              do i = 1, n
                 ! generate elementary reflector h(i) to annihilate a(i+1:m,i)
                 alpha = a( i, i )
                 call stdlib_I64_zlarfg( m-i+1, alpha, a( min( i+1, m ), i ), 1_ilp64,tauq( i ) )
                 d( i ) = real( alpha,KIND=dp)
                 a( i, i ) = cone
                 ! apply h(i)**h to a(i:m,i+1:n) from the left
                 if( i<n )call stdlib_I64_zlarf( 'LEFT', m-i+1, n-i, a( i, i ), 1_ilp64,conjg( tauq( i ) ), &
                           a( i, i+1 ), lda, work )
                 a( i, i ) = d( i )
                 if( i<n ) then
                    ! generate elementary reflector g(i) to annihilate
                    ...
                    alpha = a( i, i+1 )
                    call stdlib_I64_zlarfg( n-i, alpha, a( i, min( i+2, n ) ), lda,taup( i ) )
                    e( i ) = real( alpha,KIND=dp)
                    a( i, i+1 ) = cone
                    ! apply g(i) to a(i+1:m,i+1:n) from the right
                    call stdlib_I64_zlarf( 'RIGHT', m-i, n-i, a( i, i+1 ), lda,taup( i ), a( i+1, i+1 &
                              ), lda, work )
                    call stdlib_I64_zlacgv( n-i, a( i, i+1 ), lda )
                    a( i, i+1 ) = e( i )
                 else
              ...
              do i = 1, m
                 ...
                 alpha = a( i, i )
                 call stdlib_I64_zlarfg( n-i+1, alpha, a( i, min( i+1, n ) ), lda,taup( i ) )
                 d( i ) = real( alpha,KIND=dp)
                 a( i, i ) = cone
                 ! apply g(i) to a(i+1:m,i:n) from the right
                 if( i<m )call stdlib_I64_zlarf( 'RIGHT', m-i, n-i+1, a( i, i ), lda,taup( i ), a( i+&
                           1_ilp64, i ), lda, work )
                 call stdlib_I64_zlacgv( n-i+1, a( i, i ), lda )
                 a( i, i ) = d( i )
                 if( i<m ) then
                    ...
                    ! a(i+2:m,i)
                    alpha = a( i+1, i )
                    call stdlib_I64_zlarfg( m-i, alpha, a( min( i+2, m ), i ), 1_ilp64,tauq( i ) )
                    e( i ) = real( alpha,KIND=dp)
                    a( i+1, i ) = cone
                    ! apply h(i)**h to a(i+1:m,i+1:n) from the left
                    call stdlib_I64_zlarf( 'LEFT', m-i, n-i, a( i+1, i ), 1_ilp64,conjg( tauq( i ) ), a( i+&
                              1_ilp64, i+1 ), lda,work )
                    a( i+1, i ) = e( i )
                 else
                    tauq( i ) = czero
     ...
     !! Q**T*C for a given matrix C.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           logical(lk) :: wantb, wantc, wantpt, wantq
           integer(ilp64) :: i, inca, j, j1, j2, kb, kb1, kk, klm, klu1, kun, l, minmn, ml, ml0, mn,&
                      mu, mu0, nr, nrt
           real(sp) :: ra, rb, rc, rs
           ! Intrinsic Functions 
           ! Executable Statements 
           ...
           if( .not.wantq .and. .not.wantpt .and. .not.stdlib_lsame( vect, 'N' ) )then
              ...
              info = -8_ilp64
           else if( ldq<1_ilp64 .or. wantq .and. ldq<max( 1_ilp64, m ) ) then
              info = -12_ilp64
           else if( ldpt<1_ilp64 .or. wantpt .and. ldpt<max( 1_ilp64, n ) ) then
              info = -14_ilp64
           else if( ldc<1_ilp64 .or. wantc .and. ldc<max( 1_ilp64, m ) ) then
              info = -16_ilp64
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SGBBRD', -info )
              return
           end if
           ! initialize q and p**t to the unit matrix, if needed
           if( wantq )call stdlib_I64_slaset( 'FULL', m, m, zero, one, q, ldq )
           if( wantpt )call stdlib_I64_slaset( 'FULL', n, n, zero, one, pt, ldpt )
           ! quick return if possible.
           if( m==0 .or. n==0 )return
           minmn = min( m, n )
           if( kl+ku>1_ilp64 ) then
              ! reduce to upper bidiagonal form if ku > 0; if ku = 0, reduce
              ! first to lower bidiagonal form and then transform to upper
              ! bidiagonal
              if( ku>0_ilp64 ) then
              ...
              loop_90: do i = 1, minmn
                 ...
                 loop_80: do kk = 1, kb
                    ...
                    ! generate plane rotations to annihilate nonzero elements
                    ! which have been created below the band
                    if( nr>0_ilp64 )call stdlib_I64_slargv( nr, ab( klu1, j1-klm-1 ), inca,work( j1 ), kb1, &
                              work( mn+j1 ), kb1 )
                    ! apply plane rotations from the left
                    do l = 1, kb
                       if( j2-klm+l-1>n ) then
                       ...
                       if( nrt>0_ilp64 )call stdlib_I64_slartv( nrt, ab( klu1-l, j1-klm+l-1 ), inca,ab( &
                                 klu1-l+1, j1-klm+l-1 ), inca,work( mn+j1 ), work( j1 ), kb1 )
                    end do
                    if( ml>ml0 ) then
                       if( ml<=m-i+1 ) then
                          ! generate plane rotation to annihilate a(i+ml-1,i)
                          ! within the band, and apply rotation from the left
                          call stdlib_I64_slartg( ab( ku+ml-1, i ), ab( ku+ml, i ),work( mn+i+ml-1 ), &
                                    work( i+ml-1 ),ra )
                          ab( ku+ml-1, i ) = ra
                          if( i<n )call stdlib_I64_srot( min( ku+ml-2, n-i ),ab( ku+ml-2, i+1 ), ldab-&
                                    1_ilp64,ab( ku+ml-1, i+1 ), ldab-1,work( mn+i+ml-1 ), work( i+ml-1 ) )
                       end if
                       nr = nr + 1_ilp64
                       j1 = j1 - kb1
                    end if
                    if( wantq ) then
                       ! accumulate product of plane rotations in q
                       do j = j1, j2, kb1
                          call stdlib_I64_srot( m, q( 1_ilp64, j-1 ), 1_ilp64, q( 1_ilp64, j ), 1_ilp64,work( mn+j ), work( j &
                                    ) )
                       end do
                    end if
                    if( wantc ) then
                       ! apply plane rotations to c
                       do j = j1, j2, kb1
                          call stdlib_I64_srot( ncc, c( j-1, 1_ilp64 ), ldc, c( j, 1_ilp64 ), ldc,work( mn+j ), &
                                    work( j ) )
                       end do
                    end if
                    if( j2+kun>n ) then
                    ...
                    do j = j1, j2, kb1
                       ! create nonzero element a(j-1,j+ku) above the band
                       ! and store it in work(n+1:2*n)
                       work( j+kun ) = work( j )*ab( 1_ilp64, j+kun )
                       ab( 1_ilp64, j+kun ) = work( mn+j )*ab( 1_ilp64, j+kun )
                    end do
                    ! generate plane rotations to annihilate nonzero elements
                    ! which have been generated above the band
                    if( nr>0_ilp64 )call stdlib_I64_slargv( nr, ab( 1_ilp64, j1+kun-1 ), inca,work( j1+kun ), kb1,&
                               work( mn+j1+kun ),kb1 )
                    ! apply plane rotations from the right
                    do l = 1, kb
                       if( j2+l-1>m ) then
                       ...
                       if( nrt>0_ilp64 )call stdlib_I64_slartv( nrt, ab( l+1, j1+kun-1 ), inca,ab( l, j1+&
                                 kun ), inca,work( mn+j1+kun ), work( j1+kun ),kb1 )
                    end do
                    if( ml==ml0 .and. mu>mu0 ) then
                       if( mu<=n-i+1 ) then
                          ! generate plane rotation to annihilate a(i,i+mu-1)
                          ! within the band, and apply rotation from the right
                          call stdlib_I64_slartg( ab( ku-mu+3, i+mu-2 ),ab( ku-mu+2, i+mu-1 ),work( &
                                    mn+i+mu-1 ), work( i+mu-1 ),ra )
                          ab( ku-mu+3, i+mu-2 ) = ra
                          call stdlib_I64_srot( min( kl+mu-2, m-i ),ab( ku-mu+4, i+mu-2 ), 1_ilp64,ab( ku-&
                                    mu+3, i+mu-1 ), 1_ilp64,work( mn+i+mu-1 ), work( i+mu-1 ) )
                       end if
                       nr = nr + 1_ilp64
                       j1 = j1 - kb1
                    end if
                    if( wantpt ) then
                       ! accumulate product of plane rotations in p**t
                       do j = j1, j2, kb1
                          call stdlib_I64_srot( n, pt( j+kun-1, 1_ilp64 ), ldpt,pt( j+kun, 1_ilp64 ), ldpt, work( &
                                    mn+j+kun ),work( j+kun ) )
                       end do
                    end if
                    if( j2+kb>m ) then
           ...
           if( ku==0_ilp64 .and. kl>0_ilp64 ) then
              ! a has been reduced to lower bidiagonal form
              ! transform lower bidiagonal form to upper bidiagonal by applying
              ! plane rotations from the left, storing diagonal elements in d
              ! and off-diagonal elements in e
              do i = 1, min( m-1, n )
                 call stdlib_I64_slartg( ab( 1_ilp64, i ), ab( 2_ilp64, i ), rc, rs, ra )
                 d( i ) = ra
                 if( i<n ) then
                    e( i ) = rs*ab( 1_ilp64, i+1 )
                    ab( 1_ilp64, i+1 ) = rc*ab( 1_ilp64, i+1 )
                 end if
                 if( wantq )call stdlib_I64_srot( m, q( 1_ilp64, i ), 1_ilp64, q( 1_ilp64, i+1 ), 1_ilp64, rc, rs )
                 if( wantc )call stdlib_I64_srot( ncc, c( i, 1_ilp64 ), ldc, c( i+1, 1_ilp64 ), ldc, rc,rs )
                           
              end do
              if( m<=n )d( m ) = ab( 1_ilp64, m )
           else if( ku>0_ilp64 ) then
              ! a has been reduced to upper bidiagonal form
              if( m<n ) then
                 ! annihilate a(m,m+1) by applying plane rotations from the
                 ...
                 rb = ab( ku, m+1 )
                 do i = m, 1, -1
                    call stdlib_I64_slartg( ab( ku+1, i ), rb, rc, rs, ra )
                    d( i ) = ra
                    if( i>1_ilp64 ) then
                       rb = -rs*ab( ku, i )
                       e( i-1 ) = rc*ab( ku, i )
                    end if
                    if( wantpt )call stdlib_I64_srot( n, pt( i, 1_ilp64 ), ldpt, pt( m+1, 1_ilp64 ), ldpt,rc, rs )
     ...
     !! Q**T*C for a given matrix C.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DGBBRD', -info )
              return
           end if
           ! initialize q and p**t to the unit matrix, if needed
           if( wantq )call stdlib_I64_dlaset( 'FULL', m, m, zero, one, q, ldq )
           if( wantpt )call stdlib_I64_dlaset( 'FULL', n, n, zero, one, pt, ldpt )
           ! quick return if possible.
           if( m==0 .or. n==0 )return
           minmn = min( m, n )
           if( kl+ku>1_ilp64 ) then
              ...
              loop_90: do i = 1, minmn
                 ...
                 loop_80: do kk = 1, kb
                    ...
                    ! generate plane rotations to annihilate nonzero elements
                    ! which have been created below the band
                    if( nr>0_ilp64 )call stdlib_I64_dlargv( nr, ab( klu1, j1-klm-1 ), inca,work( j1 ), kb1, &
                              work( mn+j1 ), kb1 )
                    ! apply plane rotations from the left
                    do l = 1, kb
                       if( j2-klm+l-1>n ) then
                       ...
                       if( nrt>0_ilp64 )call stdlib_I64_dlartv( nrt, ab( klu1-l, j1-klm+l-1 ), inca,ab( &
                                 klu1-l+1, j1-klm+l-1 ), inca,work( mn+j1 ), work( j1 ), kb1 )
                    end do
                    if( ml>ml0 ) then
                       if( ml<=m-i+1 ) then
                          ! generate plane rotation to annihilate a(i+ml-1,i)
                          ! within the band, and apply rotation from the left
                          call stdlib_I64_dlartg( ab( ku+ml-1, i ), ab( ku+ml, i ),work( mn+i+ml-1 ), &
                                    work( i+ml-1 ),ra )
                          ab( ku+ml-1, i ) = ra
                          if( i<n )call stdlib_I64_drot( min( ku+ml-2, n-i ),ab( ku+ml-2, i+1 ), ldab-&
                                    1_ilp64,ab( ku+ml-1, i+1 ), ldab-1,work( mn+i+ml-1 ), work( i+ml-1 ) )
                       end if
                       nr = nr + 1_ilp64
                       j1 = j1 - kb1
                    ...
                    if( wantq ) then
                       ! accumulate product of plane rotations in q
                       do j = j1, j2, kb1
                          call stdlib_I64_drot( m, q( 1_ilp64, j-1 ), 1_ilp64, q( 1_ilp64, j ), 1_ilp64,work( mn+j ), work( j &
                                    ) )
                       end do
                    end if
                    if( wantc ) then
                       ! apply plane rotations to c
                       do j = j1, j2, kb1
                          call stdlib_I64_drot( ncc, c( j-1, 1_ilp64 ), ldc, c( j, 1_ilp64 ), ldc,work( mn+j ), &
                                    work( j ) )
                       end do
                    end if
                    if( j2+kun>n ) then
                    ...
                    ! generate plane rotations to annihilate nonzero elements
                    ! which have been generated above the band
                    if( nr>0_ilp64 )call stdlib_I64_dlargv( nr, ab( 1_ilp64, j1+kun-1 ), inca,work( j1+kun ), kb1,&
                               work( mn+j1+kun ),kb1 )
                    ! apply plane rotations from the right
                    do l = 1, kb
                       if( j2+l-1>m ) then
                       ...
                       if( nrt>0_ilp64 )call stdlib_I64_dlartv( nrt, ab( l+1, j1+kun-1 ), inca,ab( l, j1+&
                                 kun ), inca,work( mn+j1+kun ), work( j1+kun ),kb1 )
                    end do
                    if( ml==ml0 .and. mu>mu0 ) then
                       if( mu<=n-i+1 ) then
                          ! generate plane rotation to annihilate a(i,i+mu-1)
                          ! within the band, and apply rotation from the right
                          call stdlib_I64_dlartg( ab( ku-mu+3, i+mu-2 ),ab( ku-mu+2, i+mu-1 ),work( &
                                    mn+i+mu-1 ), work( i+mu-1 ),ra )
                          ab( ku-mu+3, i+mu-2 ) = ra
                          call stdlib_I64_drot( min( kl+mu-2, m-i ),ab( ku-mu+4, i+mu-2 ), 1_ilp64,ab( ku-&
                                    mu+3, i+mu-1 ), 1_ilp64,work( mn+i+mu-1 ), work( i+mu-1 ) )
                       end if
                       nr = nr + 1_ilp64
                       j1 = j1 - kb1
                    ...
                    if( wantpt ) then
                       ! accumulate product of plane rotations in p**t
                       do j = j1, j2, kb1
                          call stdlib_I64_drot( n, pt( j+kun-1, 1_ilp64 ), ldpt,pt( j+kun, 1_ilp64 ), ldpt, work( &
                                    mn+j+kun ),work( j+kun ) )
                       end do
                    end if
                    if( j2+kb>m ) then
           ...
           if( ku==0_ilp64 .and. kl>0_ilp64 ) then
              ...
              ! and off-diagonal elements in e
              do i = 1, min( m-1, n )
                 call stdlib_I64_dlartg( ab( 1_ilp64, i ), ab( 2_ilp64, i ), rc, rs, ra )
                 d( i ) = ra
                 if( i<n ) then
                    e( i ) = rs*ab( 1_ilp64, i+1 )
                    ab( 1_ilp64, i+1 ) = rc*ab( 1_ilp64, i+1 )
                 end if
                 if( wantq )call stdlib_I64_drot( m, q( 1_ilp64, i ), 1_ilp64, q( 1_ilp64, i+1 ), 1_ilp64, rc, rs )
                 if( wantc )call stdlib_I64_drot( ncc, c( i, 1_ilp64 ), ldc, c( i+1, 1_ilp64 ), ldc, rc,rs )
              ...
              if( m<n ) then
                 ...
                 rb = ab( ku, m+1 )
                 do i = m, 1, -1
                    call stdlib_I64_dlartg( ab( ku+1, i ), rb, rc, rs, ra )
                    d( i ) = ra
                    if( i>1_ilp64 ) then
                       rb = -rs*ab( ku, i )
                       e( i-1 ) = rc*ab( ku, i )
                    end if
                    if( wantpt )call stdlib_I64_drot( n, pt( i, 1_ilp64 ), ldpt, pt( m+1, 1_ilp64 ), ldpt,rc, rs )
     ...
     !! Q**H*C for a given matrix C.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           logical(lk) :: wantb, wantc, wantpt, wantq
           integer(ilp64) :: i, inca, j, j1, j2, kb, kb1, kk, klm, klu1, kun, l, minmn, ml, ml0, mu,&
                      mu0, nr, nrt
           real(sp) :: abst, rc
           complex(sp) :: ra, rb, rs, t
           ! Intrinsic Functions 
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CGBBRD', -info )
              return
           end if
           ! initialize q and p**h to the unit matrix, if needed
           if( wantq )call stdlib_I64_claset( 'FULL', m, m, czero, cone, q, ldq )
           if( wantpt )call stdlib_I64_claset( 'FULL', n, n, czero, cone, pt, ldpt )
           ! quick return if possible.
           if( m==0 .or. n==0 )return
           minmn = min( m, n )
           if( kl+ku>1_ilp64 ) then
              ...
              loop_90: do i = 1, minmn
                 ...
                 loop_80: do kk = 1, kb
                    ...
                    ! generate plane rotations to annihilate nonzero elements
                    ! which have been created below the band
                    if( nr>0_ilp64 )call stdlib_I64_clargv( nr, ab( klu1, j1-klm-1 ), inca,work( j1 ), kb1, &
                              rwork( j1 ), kb1 )
                    ! apply plane rotations from the left
                    do l = 1, kb
                       if( j2-klm+l-1>n ) then
                       ...
                       if( nrt>0_ilp64 )call stdlib_I64_clartv( nrt, ab( klu1-l, j1-klm+l-1 ), inca,ab( &
                                 klu1-l+1, j1-klm+l-1 ), inca,rwork( j1 ), work( j1 ), kb1 )
                    end do
                    if( ml>ml0 ) then
                       if( ml<=m-i+1 ) then
                          ! generate plane rotation to annihilate a(i+ml-1,i)
                          ! within the band, and apply rotation from the left
                          call stdlib_I64_clartg( ab( ku+ml-1, i ), ab( ku+ml, i ),rwork( i+ml-1 ), &
                                    work( i+ml-1 ), ra )
                          ab( ku+ml-1, i ) = ra
                          if( i<n )call stdlib_I64_crot( min( ku+ml-2, n-i ),ab( ku+ml-2, i+1 ), ldab-&
                                    1_ilp64,ab( ku+ml-1, i+1 ), ldab-1,rwork( i+ml-1 ), work( i+ml-1 ) )
                       end if
                       nr = nr + 1_ilp64
                       j1 = j1 - kb1
                    end if
                    if( wantq ) then
                       ! accumulate product of plane rotations in q
                       do j = j1, j2, kb1
                          call stdlib_I64_crot( m, q( 1_ilp64, j-1 ), 1_ilp64, q( 1_ilp64, j ), 1_ilp64,rwork( j ), conjg( &
                                    work( j ) ) )
                       end do
                    end if
                    if( wantc ) then
                       ! apply plane rotations to c
                       do j = j1, j2, kb1
                          call stdlib_I64_crot( ncc, c( j-1, 1_ilp64 ), ldc, c( j, 1_ilp64 ), ldc,rwork( j ), &
                                    work( j ) )
                       end do
                    end if
                    if( j2+kun>n ) then
                    ...
                    do j = j1, j2, kb1
                       ...
                       ! and store it in work(n+1:2*n)
                       work( j+kun ) = work( j )*ab( 1_ilp64, j+kun )
                       ab( 1_ilp64, j+kun ) = rwork( j )*ab( 1_ilp64, j+kun )
                    end do
                    ! generate plane rotations to annihilate nonzero elements
                    ! which have been generated above the band
                    if( nr>0_ilp64 )call stdlib_I64_clargv( nr, ab( 1_ilp64, j1+kun-1 ), inca,work( j1+kun ), kb1,&
                               rwork( j1+kun ),kb1 )
                    ! apply plane rotations from the right
                    do l = 1, kb
                       if( j2+l-1>m ) then
                       ...
                       if( nrt>0_ilp64 )call stdlib_I64_clartv( nrt, ab( l+1, j1+kun-1 ), inca,ab( l, j1+&
                                 kun ), inca,rwork( j1+kun ), work( j1+kun ), kb1 )
                    end do
                    if( ml==ml0 .and. mu>mu0 ) then
                       if( mu<=n-i+1 ) then
                          ! generate plane rotation to annihilate a(i,i+mu-1)
                          ! within the band, and apply rotation from the right
                          call stdlib_I64_clartg( ab( ku-mu+3, i+mu-2 ),ab( ku-mu+2, i+mu-1 ),rwork( &
                                    i+mu-1 ), work( i+mu-1 ), ra )
                          ab( ku-mu+3, i+mu-2 ) = ra
                          call stdlib_I64_crot( min( kl+mu-2, m-i ),ab( ku-mu+4, i+mu-2 ), 1_ilp64,ab( ku-&
                                    mu+3, i+mu-1 ), 1_ilp64,rwork( i+mu-1 ), work( i+mu-1 ) )
                       end if
                       nr = nr + 1_ilp64
                       j1 = j1 - kb1
                    end if
                    if( wantpt ) then
                       ! accumulate product of plane rotations in p**h
                       do j = j1, j2, kb1
                          call stdlib_I64_crot( n, pt( j+kun-1, 1_ilp64 ), ldpt,pt( j+kun, 1_ilp64 ), ldpt, rwork(&
                                     j+kun ),conjg( work( j+kun ) ) )
                       end do
                    end if
                    if( j2+kb>m ) then
           ...
           if( ku==0_ilp64 .and. kl>0_ilp64 ) then
              ...
              ! elements on subdiagonal elements
              do i = 1, min( m-1, n )
                 call stdlib_I64_clartg( ab( 1_ilp64, i ), ab( 2_ilp64, i ), rc, rs, ra )
                 ab( 1_ilp64, i ) = ra
                 if( i<n ) then
                    ab( 2_ilp64, i ) = rs*ab( 1_ilp64, i+1 )
                    ab( 1_ilp64, i+1 ) = rc*ab( 1_ilp64, i+1 )
                 end if
                 if( wantq )call stdlib_I64_crot( m, q( 1_ilp64, i ), 1_ilp64, q( 1_ilp64, i+1 ), 1_ilp64, rc,conjg( rs ) )
                           
                 if( wantc )call stdlib_I64_crot( ncc, c( i, 1_ilp64 ), ldc, c( i+1, 1_ilp64 ), ldc, rc,rs )
                           
              end do
           else
              ! a has been reduced to complex upper bidiagonal form or is
              ! diagonal
              if( ku>0_ilp64 .and. m<n ) then
                 ! annihilate a(m,m+1) by applying plane rotations from the
                 ! right
                 rb = ab( ku, m+1 )
                 do i = m, 1, -1
                    call stdlib_I64_clartg( ab( ku+1, i ), rb, rc, rs, ra )
                    ab( ku+1, i ) = ra
                    if( i>1_ilp64 ) then
                       rb = -conjg( rs )*ab( ku, i )
                       ab( ku, i ) = rc*ab( ku, i )
                    end if
                    if( wantpt )call stdlib_I64_crot( n, pt( i, 1_ilp64 ), ldpt, pt( m+1, 1_ilp64 ), ldpt,rc, &
                              conjg( rs ) )
                 end do
              end if
           end if
           ...
           loop_120: do i = 1, minmn
              ...
              if( wantq )call stdlib_I64_cscal( m, t, q( 1_ilp64, i ), 1_ilp64 )
              if( wantc )call stdlib_I64_cscal( ncc, conjg( t ), c( i, 1_ilp64 ), ldc )
              if( i<minmn ) then
                 if( ku==0_ilp64 .and. kl==0_ilp64 ) then
                    e( i ) = zero
                    t = ab( 1_ilp64, i+1 )
                    ...
                    if( wantpt )call stdlib_I64_cscal( n, t, pt( i+1, 1_ilp64 ), ldpt )
                    t = ab( ku+1, i+1 )*conjg( t )
                 end if
              end if
           end do loop_120
     ...
     !! Q**H*C for a given matrix C.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZGBBRD', -info )
              return
           end if
           ! initialize q and p**h to the unit matrix, if needed
           if( wantq )call stdlib_I64_zlaset( 'FULL', m, m, czero, cone, q, ldq )
           if( wantpt )call stdlib_I64_zlaset( 'FULL', n, n, czero, cone, pt, ldpt )
           ! quick return if possible.
           if( m==0 .or. n==0 )return
           minmn = min( m, n )
           if( kl+ku>1_ilp64 ) then
              ...
              loop_90: do i = 1, minmn
                 ...
                 loop_80: do kk = 1, kb
                    ...
                    ! generate plane rotations to annihilate nonzero elements
                    ! which have been created below the band
                    if( nr>0_ilp64 )call stdlib_I64_zlargv( nr, ab( klu1, j1-klm-1 ), inca,work( j1 ), kb1, &
                              rwork( j1 ), kb1 )
                    ! apply plane rotations from the left
                    do l = 1, kb
                       if( j2-klm+l-1>n ) then
                       ...
                       if( nrt>0_ilp64 )call stdlib_I64_zlartv( nrt, ab( klu1-l, j1-klm+l-1 ), inca,ab( &
                                 klu1-l+1, j1-klm+l-1 ), inca,rwork( j1 ), work( j1 ), kb1 )
                    end do
                    if( ml>ml0 ) then
                       if( ml<=m-i+1 ) then
                          ! generate plane rotation to annihilate a(i+ml-1,i)
                          ! within the band, and apply rotation from the left
                          call stdlib_I64_zlartg( ab( ku+ml-1, i ), ab( ku+ml, i ),rwork( i+ml-1 ), &
                                    work( i+ml-1 ), ra )
                          ab( ku+ml-1, i ) = ra
                          if( i<n )call stdlib_I64_zrot( min( ku+ml-2, n-i ),ab( ku+ml-2, i+1 ), ldab-&
                                    1_ilp64,ab( ku+ml-1, i+1 ), ldab-1,rwork( i+ml-1 ), work( i+ml-1 ) )
                       end if
                       nr = nr + 1_ilp64
                       j1 = j1 - kb1
                    ...
                    if( wantq ) then
                       ! accumulate product of plane rotations in q
                       do j = j1, j2, kb1
                          call stdlib_I64_zrot( m, q( 1_ilp64, j-1 ), 1_ilp64, q( 1_ilp64, j ), 1_ilp64,rwork( j ), conjg( &
                                    work( j ) ) )
                       end do
                    end if
                    if( wantc ) then
                       ! apply plane rotations to c
                       do j = j1, j2, kb1
                          call stdlib_I64_zrot( ncc, c( j-1, 1_ilp64 ), ldc, c( j, 1_ilp64 ), ldc,rwork( j ), &
                                    work( j ) )
                       end do
                    end if
                    if( j2+kun>n ) then
                    ...
                    ! generate plane rotations to annihilate nonzero elements
                    ! which have been generated above the band
                    if( nr>0_ilp64 )call stdlib_I64_zlargv( nr, ab( 1_ilp64, j1+kun-1 ), inca,work( j1+kun ), kb1,&
                               rwork( j1+kun ),kb1 )
                    ! apply plane rotations from the right
                    do l = 1, kb
                       if( j2+l-1>m ) then
                       ...
                       if( nrt>0_ilp64 )call stdlib_I64_zlartv( nrt, ab( l+1, j1+kun-1 ), inca,ab( l, j1+&
                                 kun ), inca,rwork( j1+kun ), work( j1+kun ), kb1 )
                    end do
                    if( ml==ml0 .and. mu>mu0 ) then
                       if( mu<=n-i+1 ) then
                          ! generate plane rotation to annihilate a(i,i+mu-1)
                          ! within the band, and apply rotation from the right
                          call stdlib_I64_zlartg( ab( ku-mu+3, i+mu-2 ),ab( ku-mu+2, i+mu-1 ),rwork( &
                                    i+mu-1 ), work( i+mu-1 ), ra )
                          ab( ku-mu+3, i+mu-2 ) = ra
                          call stdlib_I64_zrot( min( kl+mu-2, m-i ),ab( ku-mu+4, i+mu-2 ), 1_ilp64,ab( ku-&
                                    mu+3, i+mu-1 ), 1_ilp64,rwork( i+mu-1 ), work( i+mu-1 ) )
                       end if
                       nr = nr + 1_ilp64
                       j1 = j1 - kb1
                    ...
                    if( wantpt ) then
                       ! accumulate product of plane rotations in p**h
                       do j = j1, j2, kb1
                          call stdlib_I64_zrot( n, pt( j+kun-1, 1_ilp64 ), ldpt,pt( j+kun, 1_ilp64 ), ldpt, rwork(&
                                     j+kun ),conjg( work( j+kun ) ) )
                       end do
                    end if
                    if( j2+kb>m ) then
           ...
           if( ku==0_ilp64 .and. kl>0_ilp64 ) then
              ...
              ! elements on subdiagonal elements
              do i = 1, min( m-1, n )
                 call stdlib_I64_zlartg( ab( 1_ilp64, i ), ab( 2_ilp64, i ), rc, rs, ra )
                 ab( 1_ilp64, i ) = ra
                 if( i<n ) then
                    ab( 2_ilp64, i ) = rs*ab( 1_ilp64, i+1 )
                    ab( 1_ilp64, i+1 ) = rc*ab( 1_ilp64, i+1 )
                 end if
                 if( wantq )call stdlib_I64_zrot( m, q( 1_ilp64, i ), 1_ilp64, q( 1_ilp64, i+1 ), 1_ilp64, rc,conjg( rs ) )
                           
                 if( wantc )call stdlib_I64_zrot( ncc, c( i, 1_ilp64 ), ldc, c( i+1, 1_ilp64 ), ldc, rc,rs )
              ...
              if( ku>0_ilp64 .and. m<n ) then
                 ...
                 rb = ab( ku, m+1 )
                 do i = m, 1, -1
                    call stdlib_I64_zlartg( ab( ku+1, i ), rb, rc, rs, ra )
                    ab( ku+1, i ) = ra
                    if( i>1_ilp64 ) then
                       rb = -conjg( rs )*ab( ku, i )
                       ab( ku, i ) = rc*ab( ku, i )
                    end if
                    if( wantpt )call stdlib_I64_zrot( n, pt( i, 1_ilp64 ), ldpt, pt( m+1, 1_ilp64 ), ldpt,rc, &
                              conjg( rs ) )
                 end do
              end if
           end if
           ...
           loop_120: do i = 1, minmn
              ...
              if( wantq )call stdlib_I64_zscal( m, t, q( 1_ilp64, i ), 1_ilp64 )
              if( wantc )call stdlib_I64_zscal( ncc, conjg( t ), c( i, 1_ilp64 ), ldc )
              if( i<minmn ) then
                 if( ku==0_ilp64 .and. kl==0_ilp64 ) then
                    e( i ) = zero
                    t = ab( 1_ilp64, i+1 )
                    ...
                    if( wantpt )call stdlib_I64_zscal( n, t, pt( i+1, 1_ilp64 ), ldpt )
                    t = ab( ku+1, i+1 )*conjg( t )
                 end if
              end if
           end do loop_120
     ...
     !! parameters (marked by [TP]) are available for the implementer.
        ...
        ! =====================================================================
           ...
           real(sp) :: aapp, aapp0, aapq, aaqq, apoaq, aqoap, big, bigtheta, cs, mxaapq, mxsinj, &
                     rootbig, rooteps, rootsfmin, roottol, small, sn, t, temp1, theta, thsign
           integer(ilp64) :: blskip, emptsw, i, ibr, ierr, igl, ijblsk, ir1, iswrot, jbc, jgl, kbl, &
                     lkahead, mvl, nbl, notrot, p, pskipped, q, rowskip, swband
           logical(lk) :: applv, rotok, rsvec
           ! Local Arrays 
           real(sp) :: fastr(5_ilp64)
           ! Intrinsic Functions 
           ! Executable Statements 
           ! test the input parameters.
           applv = stdlib_lsame( jobv, 'A' )
           ...
           if( .not.( rsvec .or. applv .or. stdlib_lsame( jobv, 'N' ) ) ) then
              ...
              info = -5_ilp64
           else if( ( rsvec.or.applv ) .and. ( mv<0_ilp64 ) ) then
              info = -8_ilp64
           else if( ( rsvec.and.( ldv<n ) ).or.( applv.and.( ldv<mv ) ) ) then
              info = -10_ilp64
           else if( tol<=eps ) then
              info = -13_ilp64
           else if( nsweep<0_ilp64 ) then
              info = -14_ilp64
           else if( lwork<m ) then
              info = -16_ilp64
           else
           ...
           ! #:(
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SGSVJ0', -info )
              return
           end if
           if( rsvec ) then
              mvl = n
           ...
           roottol = sqrt( tol )
           ! .. row-cyclic jacobi svd algorithm with column pivoting ..
           emptsw = ( n*( n-1 ) ) / 2_ilp64
           notrot = 0_ilp64
           fastr( 1_ilp64 ) = zero
           ! .. row-cyclic pivot strategy with de rijk's pivoting ..
           swband = 0_ilp64
      ! [tp] swband is a tuning parameter. it is meaningful and effective
           ! if stdlib_I64_sgesvj is used as a computational routine in the preconditioned
           ! jacobi svd algorithm stdlib_I64_sgesvj. for sweeps i=1:swband the procedure
           ! ......
           kbl = min( 8_ilp64, n )
      ! [tp] kbl is a tuning parameter that defines the tile size in the
           ! tiling of the p-q loops of pivot pairs. in general, an optimal
           ...
           ! parameters of the computer's memory.
           nbl = n / kbl
           if( ( nbl*kbl )/=n )nbl = nbl + 1_ilp64
           blskip = ( kbl**2_ilp64 ) + 1_ilp64
      ! [tp] blkskip is a tuning parameter that depends on swband and kbl.
           rowskip = min( 5_ilp64, kbl )
      ! [tp] rowskip is a tuning parameter.
           lkahead = 1_ilp64
      ! [tp] lkahead is a tuning parameter.
           swband = 0_ilp64
           ...
           ! .. go go go ...
              ...
              pskipped = 0_ilp64
              loop_2000: do ibr = 1, nbl
                 igl = ( ibr-1 )*kbl + 1_ilp64
                 loop_1002: do ir1 = 0, min( lkahead, nbl-ibr )
                    igl = igl + ir1*kbl
                    loop_2001: do p = igl, min( igl+kbl-1, n-1 )
           ! .. de rijk's pivoting
                       q = stdlib_I64_isamax( n-p+1, sva( p ), 1_ilp64 ) + p - 1_ilp64
                       if( p/=q ) then
                          call stdlib_I64_sswap( m, a( 1_ilp64, p ), 1_ilp64, a( 1_ilp64, q ), 1_ilp64 )
                          if( rsvec )call stdlib_I64_sswap( mvl, v( 1_ilp64, p ), 1_ilp64,v( 1_ilp64, q ), 1_ilp64 )
                          temp1 = sva( p )
                          sva( p ) = sva( q )
                          sva( q ) = temp1
                          temp1 = d( p )
              ...
              ! norm computation.
              ! caveat:
              ! some blas implementations compute stdlib_I64_snrm2(m,a(1,p),1)
              ! as sqrt(stdlib_I64_sdot(m,a(1,p),1,a(1,p),1)), which may result in
              ! overflow for ||a(:,p)||_2 > sqrt(overflow_threshold), and
              ! underflow for ||a(:,p)||_2 < sqrt(underflow_threshold).
              ! hence, stdlib_I64_snrm2 cannot be trusted, not even in the case when
              ! the true norm is far from the under(over)flow boundaries.
              ! if properly implemented stdlib_I64_snrm2 is available, the if-then-else
              ! below should read "aapp = stdlib_I64_snrm2( m, a(1,p), 1 ) * d(p)".
                          if( ( sva( p )<rootbig ) .and.( sva( p )>rootsfmin ) ) then
                             sva( p ) = stdlib_I64_snrm2( m, a( 1_ilp64, p ), 1_ilp64 )*d( p )
                          else
                             temp1 = zero
                             aapp = one
                             call stdlib_I64_slassq( m, a( 1_ilp64, p ), 1_ilp64, temp1, aapp )
                             sva( p ) = temp1*sqrt( aapp )*d( p )
                          end if
                          aapp = sva( p )
                       else
                          ...
                          loop_2002: do q = p + 1, min( igl+kbl-1, n )
                             ...
                             if( aaqq>zero ) then
                                ...
                                if( aaqq>=one ) then
                                   rotok = ( small*aapp )<=aaqq
                                   if( aapp<( big / aaqq ) ) then
                                      aapq = ( stdlib_I64_sdot( m, a( 1_ilp64, p ), 1_ilp64, a( 1_ilp64,q ), 1_ilp64 )*d( p )&
                                                *d( q ) / aaqq )/ aapp
                                   else
                                      call stdlib_I64_scopy( m, a( 1_ilp64, p ), 1_ilp64, work, 1_ilp64 )
                                      call stdlib_I64_slascl( 'G', 0_ilp64, 0_ilp64, aapp, d( p ),m, 1_ilp64, work, lda,&
                                                 ierr )
                                      aapq = stdlib_I64_sdot( m, work, 1_ilp64, a( 1_ilp64, q ),1_ilp64 )*d( q ) / &
                                                aaqq
                                   end if
                                else
                                   rotok = aapp<=( aaqq / small )
                                   if( aapp>( small / aaqq ) ) then
                                      ...
                                      call stdlib_I64_scopy( m, a( 1_ilp64, q ), 1_ilp64, work, 1_ilp64 )
                                      call stdlib_I64_slascl( 'G', 0_ilp64, 0_ilp64, aaqq, d( q ),m, 1_ilp64, work, lda,&
                                                 ierr )
                                      aapq = stdlib_I64_sdot( m, work, 1_ilp64, a( 1_ilp64, p ),1_ilp64 )*d( p ) / &
                                                aapp
                                   end if
                                end if
                                mxaapq = max( mxaapq, abs( aapq ) )
              ! to rotate or not to rotate, that is the question ...
                 ...
                 ! rotated = rotated + one
                                   ...
                                   if( rotok ) then
                                      ...
                                      if( abs( theta )>bigtheta ) then
                                         t = half / theta
                                         fastr( 3_ilp64 ) = t*d( p ) / d( q )
                                         fastr( 4_ilp64 ) = -t*d( q ) / d( p )
                                         call stdlib_I64_srotm( m, a( 1_ilp64, p ), 1_ilp64,a( 1_ilp64, q ), 1_ilp64, fastr )
                                                   
                                         if( rsvec )call stdlib_I64_srotm( mvl,v( 1_ilp64, p ), 1_ilp64,v( 1_ilp64, q ),&
                                                    1_ilp64,fastr )
                                         sva( q ) = aaqq*sqrt( max( zero,one+t*apoaq*aapq ) )
                       ...
                       ! Choose Correct Signum For Theta And Rotate
                                         ...
                                         if( d( p )>=one ) then
                                            if( d( q )>=one ) then
                                               fastr( 3_ilp64 ) = t*apoaq
                                               fastr( 4_ilp64 ) = -t*aqoap
                                               d( p ) = d( p )*cs
                                               d( q ) = d( q )*cs
                                               call stdlib_I64_srotm( m, a( 1_ilp64, p ), 1_ilp64,a( 1_ilp64, q ), 1_ilp64,&
                                                         fastr )
                                               if( rsvec )call stdlib_I64_srotm( mvl,v( 1_ilp64, p ), 1_ilp64, v( &
                                                         1_ilp64, q ),1_ilp64, fastr )
                                            else
                                               call stdlib_I64_saxpy( m, -t*aqoap,a( 1_ilp64, q ), 1_ilp64,a( 1_ilp64, &
                                                         p ), 1_ilp64 )
                                               call stdlib_I64_saxpy( m, cs*sn*apoaq,a( 1_ilp64, p ), 1_ilp64,a( &
                                                         1_ilp64, q ), 1_ilp64 )
                                               d( p ) = d( p )*cs
                                               d( q ) = d( q ) / cs
                                               if( rsvec ) then
                                                  call stdlib_I64_saxpy( mvl, -t*aqoap,v( 1_ilp64, q ), 1_ilp64,v(&
                                                             1_ilp64, p ), 1_ilp64 )
                                                  call stdlib_I64_saxpy( mvl,cs*sn*apoaq,v( 1_ilp64, p ), 1_ilp64,&
                                                            v( 1_ilp64, q ), 1_ilp64 )
                                               end if
                                            end if
                                         else
                                            if( d( q )>=one ) then
                                               call stdlib_I64_saxpy( m, t*apoaq,a( 1_ilp64, p ), 1_ilp64,a( 1_ilp64, q &
                                                         ), 1_ilp64 )
                                               call stdlib_I64_saxpy( m, -cs*sn*aqoap,a( 1_ilp64, q ), 1_ilp64,a( &
                                                         1_ilp64, p ), 1_ilp64 )
                                               d( p ) = d( p ) / cs
                                               d( q ) = d( q )*cs
                                               if( rsvec ) then
                                                  call stdlib_I64_saxpy( mvl, t*apoaq,v( 1_ilp64, p ), 1_ilp64,v( &
                                                            1_ilp64, q ), 1_ilp64 )
                                                  call stdlib_I64_saxpy( mvl,-cs*sn*aqoap,v( 1_ilp64, q ), &
                                                            1_ilp64,v( 1_ilp64, p ), 1_ilp64 )
                                               end if
                                            else
                                               if( d( p )>=d( q ) ) then
                                                  call stdlib_I64_saxpy( m, -t*aqoap,a( 1_ilp64, q ), 1_ilp64,a( &
                                                            1_ilp64, p ), 1_ilp64 )
                                                  call stdlib_I64_saxpy( m, cs*sn*apoaq,a( 1_ilp64, p ), 1_ilp64,&
                                                            a( 1_ilp64, q ), 1_ilp64 )
                                                  d( p ) = d( p )*cs
                                                  d( q ) = d( q ) / cs
                                                  if( rsvec ) then
                                                     call stdlib_I64_saxpy( mvl,-t*aqoap,v( 1_ilp64, q ), 1_ilp64,&
                                                               v( 1_ilp64, p ), 1_ilp64 )
                                                     call stdlib_I64_saxpy( mvl,cs*sn*apoaq,v( 1_ilp64, p ),&
                                                                1_ilp64,v( 1_ilp64, q ), 1_ilp64 )
                                                  end if
                                               else
                                                  call stdlib_I64_saxpy( m, t*apoaq,a( 1_ilp64, p ), 1_ilp64,a( 1_ilp64,&
                                                             q ), 1_ilp64 )
                                                  call stdlib_I64_saxpy( m,-cs*sn*aqoap,a( 1_ilp64, q ), 1_ilp64,&
                                                            a( 1_ilp64, p ), 1_ilp64 )
                                                  d( p ) = d( p ) / cs
                                                  d( q ) = d( q )*cs
                                                  if( rsvec ) then
                                                     call stdlib_I64_saxpy( mvl,t*apoaq, v( 1_ilp64, p ),1_ilp64, &
                                                               v( 1_ilp64, q ), 1_ilp64 )
                                                     call stdlib_I64_saxpy( mvl,-cs*sn*aqoap,v( 1_ilp64, q )&
                                                               , 1_ilp64,v( 1_ilp64, p ), 1_ilp64 )
                                                  end if
                    ...
                    ! .. have to use modified gram-schmidt like transformation
                                      call stdlib_I64_scopy( m, a( 1_ilp64, p ), 1_ilp64, work, 1_ilp64 )
                                      call stdlib_I64_slascl( 'G', 0_ilp64, 0_ilp64, aapp, one, m,1_ilp64, work, lda, &
                                                ierr )
                                      call stdlib_I64_slascl( 'G', 0_ilp64, 0_ilp64, aaqq, one, m,1_ilp64, a( 1_ilp64, q ), &
                                                lda, ierr )
                                      temp1 = -aapq*d( p ) / d( q )
                                      call stdlib_I64_saxpy( m, temp1, work, 1_ilp64,a( 1_ilp64, q ), 1_ilp64 )
                                      call stdlib_I64_slascl( 'G', 0_ilp64, 0_ilp64, one, aaqq, m,1_ilp64, a( 1_ilp64, q ), &
                                                lda, ierr )
                                      sva( q ) = aaqq*sqrt( max( zero,one-aapq*aapq ) )
                                      mxsinj = max( mxsinj, sfmin )
                                   end if
                 ...
                 ! in the case of cancellation in updating sva(q), sva(p)
                 ! recompute sva(q), sva(p).
                                   if( ( sva( q ) / aaqq )**2_ilp64<=rooteps )then
                                      if( ( aaqq<rootbig ) .and.( aaqq>rootsfmin ) ) then
                                         sva( q ) = stdlib_I64_snrm2( m, a( 1_ilp64, q ), 1_ilp64 )*d( q )
                                      else
                                         t = zero
                                         aaqq = one
                                         call stdlib_I64_slassq( m, a( 1_ilp64, q ), 1_ilp64, t,aaqq )
                                         sva( q ) = t*sqrt( aaqq )*d( q )
                                      end if
                                   end if
                                   if( ( aapp / aapp0 )<=rooteps ) then
                                      if( ( aapp<rootbig ) .and.( aapp>rootsfmin ) ) then
                                         aapp = stdlib_I64_snrm2( m, a( 1_ilp64, p ), 1_ilp64 )*d( p )
                                      else
                                         t = zero
                                         aapp = one
                                         call stdlib_I64_slassq( m, a( 1_ilp64, p ), 1_ilp64, t,aapp )
                                         aapp = t*sqrt( aapp )*d( p )
                                      end if
                                      sva( p ) = aapp
                                   end if
              ...
              ! a(:,p) and a(:,q) already numerically orthogonal
                                   if( ir1==0_ilp64 )notrot = notrot + 1_ilp64
                                   pskipped = pskipped + 1_ilp64
                                end if
                             else
              ! a(:,q) is zero column
                                if( ir1==0_ilp64 )notrot = notrot + 1_ilp64
           ...
           ! bailed out of q-loop
                          ...
                          sva( p ) = aapp
                          if( ( ir1==0_ilp64 ) .and. ( aapp==zero ) )notrot = notrot + min( igl+kbl-1, &
                                    n ) - p
                       end if
                    end do loop_2001
           ! end of the p-loop
           ...
           ! M X 2 Jacobi Svd 
              ...
              ! to rotate or not to rotate, that is the question ...
                 ...
                 ! rotated  = rotated + 1
                       ...
                       ! Choose Correct Signum For Theta And Rotate
                                      ...
                                      if( aapp>aaqq ) then
                                         ...
                                         call stdlib_I64_scopy( m, a( 1_ilp64, q ), 1_ilp64, work,1_ilp64 )
                                         call stdlib_I64_slascl( 'G', 0_ilp64, 0_ilp64, aaqq, one,m, 1_ilp64, work, lda,&
                                                    ierr )
                                         call stdlib_I64_slascl( 'G', 0_ilp64, 0_ilp64, aapp, one,m, 1_ilp64, a( 1_ilp64, p ),&
                                                    lda,ierr )
                                         temp1 = -aapq*d( q ) / d( p )
                                         call stdlib_I64_saxpy( m, temp1, work, 1_ilp64,a( 1_ilp64, p ), 1_ilp64 )
                                                   
                                         call stdlib_I64_slascl( 'G', 0_ilp64, 0_ilp64, one, aapp,m, 1_ilp64, a( 1_ilp64, p ),&
                                                    lda,ierr )
                                         sva( p ) = aapp*sqrt( max( zero,one-aapq*aapq ) )
                                         mxsinj = max( mxsinj, sfmin )
                                      end if
                 ...
                 ! .. recompute sva(q)
                                   ...
                                   if( ( aapp / aapp0 )**2_ilp64<=rooteps ) then
                                      if( ( aapp<rootbig ) .and.( aapp>rootsfmin ) ) then
                                         aapp = stdlib_I64_snrm2( m, a( 1_ilp64, p ), 1_ilp64 )*d( p )
                                      else
                                         t = zero
                    ...
                    ! end of ok rotation
                                   ...
                                   notrot = notrot + 1_ilp64
                                   pskipped = pskipped + 1_ilp64
                                   ijblsk = ijblsk + 1_ilp64
                                end if
                             else
                                notrot = notrot + 1_ilp64
                                pskipped = pskipped + 1_ilp64
              ...
              2203 continue
                          ...
                          if( aapp==zero )notrot = notrot +min( jgl+kbl-1, n ) - jgl + 1_ilp64
                          if( aapp<zero )notrot = 0_ilp64
                       end if
                    end do loop_2100
           ! end of the p-loop
                 end do loop_2010
      ...
      ! 2000 :: end of the ibr-loop
           ! .. update sva(n)
              if( ( sva( n )<rootbig ) .and. ( sva( n )>rootsfmin ) )then
                 sva( n ) = stdlib_I64_snrm2( m, a( 1_ilp64, n ), 1_ilp64 )*d( n )
              else
                 t = zero
                 aapp = one
                 call stdlib_I64_slassq( m, a( 1_ilp64, n ), 1_ilp64, t, aapp )
                 sva( n ) = t*sqrt( aapp )*d( n )
              end if
           ! additional steering devices
              if( ( i<swband ) .and. ( ( mxaapq<=roottol ) .or.( iswrot<=n ) ) )swband = i
     ...
     !! parameters (marked by [TP]) are available for the implementer.
        ...
        ! =====================================================================
           ...
           ! #:(
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DGSVJ0', -info )
              return
           end if
           if( rsvec ) then
              mvl = n
           ...
           ! .. go go go ...
              ...
              loop_2000: do ibr = 1, nbl
                 ...
                 loop_1002: do ir1 = 0, min( lkahead, nbl-ibr )
                    ...
                    loop_2001: do p = igl, min( igl+kbl-1, n-1 )
           ! .. de rijk's pivoting
                       q = stdlib_I64_idamax( n-p+1, sva( p ), 1_ilp64 ) + p - 1_ilp64
                       if( p/=q ) then
                          call stdlib_I64_dswap( m, a( 1_ilp64, p ), 1_ilp64, a( 1_ilp64, q ), 1_ilp64 )
                          if( rsvec )call stdlib_I64_dswap( mvl, v( 1_ilp64, p ), 1_ilp64,v( 1_ilp64, q ), 1_ilp64 )
                          temp1 = sva( p )
                          sva( p ) = sva( q )
                          sva( q ) = temp1
                          temp1 = d( p )
              ...
              ! norm computation.
              ! caveat:
              ! some blas implementations compute stdlib_I64_dnrm2(m,a(1,p),1)
              ! as sqrt(stdlib_I64_ddot(m,a(1,p),1,a(1,p),1)), which may result in
              ! overflow for ||a(:,p)||_2 > sqrt(overflow_threshold), and
              ! underflow for ||a(:,p)||_2 < sqrt(underflow_threshold).
              ! hence, stdlib_I64_dnrm2 cannot be trusted, not even in the case when
              ! the true norm is far from the under(over)flow boundaries.
              ! if properly implemented stdlib_I64_dnrm2 is available, the if-then-else
              ! below should read "aapp = stdlib_I64_dnrm2( m, a(1,p), 1 ) * d(p)".
                          if( ( sva( p )<rootbig ) .and.( sva( p )>rootsfmin ) ) then
                             sva( p ) = stdlib_I64_dnrm2( m, a( 1_ilp64, p ), 1_ilp64 )*d( p )
                          else
                             temp1 = zero
                             aapp = one
                             call stdlib_I64_dlassq( m, a( 1_ilp64, p ), 1_ilp64, temp1, aapp )
                             sva( p ) = temp1*sqrt( aapp )*d( p )
                          end if
                          aapp = sva( p )
                       else
                          ...
                          loop_2002: do q = p + 1, min( igl+kbl-1, n )
                             ...
                             if( aaqq>zero ) then
                                ...
                                if( aaqq>=one ) then
                                   rotok = ( small*aapp )<=aaqq
                                   if( aapp<( big / aaqq ) ) then
                                      aapq = ( stdlib_I64_ddot( m, a( 1_ilp64, p ), 1_ilp64, a( 1_ilp64,q ), 1_ilp64 )*d( p )&
                                                *d( q ) / aaqq )/ aapp
                                   else
                                      call stdlib_I64_dcopy( m, a( 1_ilp64, p ), 1_ilp64, work, 1_ilp64 )
                                      call stdlib_I64_dlascl( 'G', 0_ilp64, 0_ilp64, aapp, d( p ),m, 1_ilp64, work, lda,&
                                                 ierr )
                                      aapq = stdlib_I64_ddot( m, work, 1_ilp64, a( 1_ilp64, q ),1_ilp64 )*d( q ) / &
                                                aaqq
                                   end if
                                else
                                   rotok = aapp<=( aaqq / small )
                                   if( aapp>( small / aaqq ) ) then
                                      ...
                                      call stdlib_I64_dcopy( m, a( 1_ilp64, q ), 1_ilp64, work, 1_ilp64 )
                                      call stdlib_I64_dlascl( 'G', 0_ilp64, 0_ilp64, aaqq, d( q ),m, 1_ilp64, work, lda,&
                                                 ierr )
                                      aapq = stdlib_I64_ddot( m, work, 1_ilp64, a( 1_ilp64, p ),1_ilp64 )*d( p ) / &
                                                aapp
                                   end if
                                end if
                                mxaapq = max( mxaapq, abs( aapq ) )
              ! to rotate or not to rotate, that is the question ...
                 ...
                 ! rotated = rotated + one
                                   ...
                                   if( rotok ) then
                                      ...
                                      if( abs( theta )>bigtheta ) then
                                         ...
                                         fastr( 3_ilp64 ) = t*d( p ) / d( q )
                                         fastr( 4_ilp64 ) = -t*d( q ) / d( p )
                                         call stdlib_I64_drotm( m, a( 1_ilp64, p ), 1_ilp64,a( 1_ilp64, q ), 1_ilp64, fastr )
                                                   
                                         if( rsvec )call stdlib_I64_drotm( mvl,v( 1_ilp64, p ), 1_ilp64,v( 1_ilp64, q ),&
                                                    1_ilp64,fastr )
                                         sva( q ) = aaqq*sqrt( max( zero,one+t*apoaq*aapq ) )
                       ...
                       ! Choose Correct Signum For Theta And Rotate
                                         ...
                                         if( d( p )>=one ) then
                                            if( d( q )>=one ) then
                                               ...
                                               d( p ) = d( p )*cs
                                               d( q ) = d( q )*cs
                                               call stdlib_I64_drotm( m, a( 1_ilp64, p ), 1_ilp64,a( 1_ilp64, q ), 1_ilp64,&
                                                         fastr )
                                               if( rsvec )call stdlib_I64_drotm( mvl,v( 1_ilp64, p ), 1_ilp64, v( &
                                                         1_ilp64, q ),1_ilp64, fastr )
                                            else
                                               call stdlib_I64_daxpy( m, -t*aqoap,a( 1_ilp64, q ), 1_ilp64,a( 1_ilp64, &
                                                         p ), 1_ilp64 )
                                               call stdlib_I64_daxpy( m, cs*sn*apoaq,a( 1_ilp64, p ), 1_ilp64,a( &
                                                         1_ilp64, q ), 1_ilp64 )
                                               d( p ) = d( p )*cs
                                               d( q ) = d( q ) / cs
                                               if( rsvec ) then
                                                  call stdlib_I64_daxpy( mvl, -t*aqoap,v( 1_ilp64, q ), 1_ilp64,v(&
                                                             1_ilp64, p ), 1_ilp64 )
                                                  call stdlib_I64_daxpy( mvl,cs*sn*apoaq,v( 1_ilp64, p ), 1_ilp64,&
                                                            v( 1_ilp64, q ), 1_ilp64 )
                                               end if
                                            end if
                                         else
                                            if( d( q )>=one ) then
                                               call stdlib_I64_daxpy( m, t*apoaq,a( 1_ilp64, p ), 1_ilp64,a( 1_ilp64, q &
                                                         ), 1_ilp64 )
                                               call stdlib_I64_daxpy( m, -cs*sn*aqoap,a( 1_ilp64, q ), 1_ilp64,a( &
                                                         1_ilp64, p ), 1_ilp64 )
                                               d( p ) = d( p ) / cs
                                               d( q ) = d( q )*cs
                                               if( rsvec ) then
                                                  call stdlib_I64_daxpy( mvl, t*apoaq,v( 1_ilp64, p ), 1_ilp64,v( &
                                                            1_ilp64, q ), 1_ilp64 )
                                                  call stdlib_I64_daxpy( mvl,-cs*sn*aqoap,v( 1_ilp64, q ), &
                                                            1_ilp64,v( 1_ilp64, p ), 1_ilp64 )
                                               end if
                                            else
                                               if( d( p )>=d( q ) ) then
                                                  call stdlib_I64_daxpy( m, -t*aqoap,a( 1_ilp64, q ), 1_ilp64,a( &
                                                            1_ilp64, p ), 1_ilp64 )
                                                  call stdlib_I64_daxpy( m, cs*sn*apoaq,a( 1_ilp64, p ), 1_ilp64,&
                                                            a( 1_ilp64, q ), 1_ilp64 )
                                                  d( p ) = d( p )*cs
                                                  d( q ) = d( q ) / cs
                                                  if( rsvec ) then
                                                     call stdlib_I64_daxpy( mvl,-t*aqoap,v( 1_ilp64, q ), 1_ilp64,&
                                                               v( 1_ilp64, p ), 1_ilp64 )
                                                     call stdlib_I64_daxpy( mvl,cs*sn*apoaq,v( 1_ilp64, p ),&
                                                                1_ilp64,v( 1_ilp64, q ), 1_ilp64 )
                                                  end if
                                               else
                                                  call stdlib_I64_daxpy( m, t*apoaq,a( 1_ilp64, p ), 1_ilp64,a( 1_ilp64,&
                                                             q ), 1_ilp64 )
                                                  call stdlib_I64_daxpy( m,-cs*sn*aqoap,a( 1_ilp64, q ), 1_ilp64,&
                                                            a( 1_ilp64, p ), 1_ilp64 )
                                                  d( p ) = d( p ) / cs
                                                  d( q ) = d( q )*cs
                                                  if( rsvec ) then
                                                     call stdlib_I64_daxpy( mvl,t*apoaq, v( 1_ilp64, p ),1_ilp64, &
                                                               v( 1_ilp64, q ), 1_ilp64 )
                                                     call stdlib_I64_daxpy( mvl,-cs*sn*aqoap,v( 1_ilp64, q )&
                                                               , 1_ilp64,v( 1_ilp64, p ), 1_ilp64 )
                                                  end if
                    ...
                    ! .. have to use modified gram-schmidt like transformation
                                      call stdlib_I64_dcopy( m, a( 1_ilp64, p ), 1_ilp64, work, 1_ilp64 )
                                      call stdlib_I64_dlascl( 'G', 0_ilp64, 0_ilp64, aapp, one, m,1_ilp64, work, lda, &
                                                ierr )
                                      call stdlib_I64_dlascl( 'G', 0_ilp64, 0_ilp64, aaqq, one, m,1_ilp64, a( 1_ilp64, q ), &
                                                lda, ierr )
                                      temp1 = -aapq*d( p ) / d( q )
                                      call stdlib_I64_daxpy( m, temp1, work, 1_ilp64,a( 1_ilp64, q ), 1_ilp64 )
                                      call stdlib_I64_dlascl( 'G', 0_ilp64, 0_ilp64, one, aaqq, m,1_ilp64, a( 1_ilp64, q ), &
                                                lda, ierr )
                                      sva( q ) = aaqq*sqrt( max( zero,one-aapq*aapq ) )
                                      mxsinj = max( mxsinj, sfmin )
                                   end if
                 ...
                 ! recompute sva(q), sva(p).
                                   if( ( sva( q ) / aaqq )**2_ilp64<=rooteps )then
                                      if( ( aaqq<rootbig ) .and.( aaqq>rootsfmin ) ) then
                                         sva( q ) = stdlib_I64_dnrm2( m, a( 1_ilp64, q ), 1_ilp64 )*d( q )
                                      else
                                         t = zero
                                         aaqq = one
                                         call stdlib_I64_dlassq( m, a( 1_ilp64, q ), 1_ilp64, t,aaqq )
                                         sva( q ) = t*sqrt( aaqq )*d( q )
                                      end if
                                   end if
                                   if( ( aapp / aapp0 )<=rooteps ) then
                                      if( ( aapp<rootbig ) .and.( aapp>rootsfmin ) ) then
                                         aapp = stdlib_I64_dnrm2( m, a( 1_ilp64, p ), 1_ilp64 )*d( p )
                                      else
                                         t = zero
                                         aapp = one
                                         call stdlib_I64_dlassq( m, a( 1_ilp64, p ), 1_ilp64, t,aapp )
                                         aapp = t*sqrt( aapp )*d( p )
                                      end if
                                      sva( p ) = aapp
                                   end if
                 ...
                 ! rotated  = rotated + 1
                       ...
                       ! Choose Correct Signum For Theta And Rotate
                                      ...
                                      if( aapp>aaqq ) then
                                         ...
                                         call stdlib_I64_dcopy( m, a( 1_ilp64, q ), 1_ilp64, work,1_ilp64 )
                                         call stdlib_I64_dlascl( 'G', 0_ilp64, 0_ilp64, aaqq, one,m, 1_ilp64, work, lda,&
                                                    ierr )
                                         call stdlib_I64_dlascl( 'G', 0_ilp64, 0_ilp64, aapp, one,m, 1_ilp64, a( 1_ilp64, p ),&
                                                    lda,ierr )
                                         temp1 = -aapq*d( q ) / d( p )
                                         call stdlib_I64_daxpy( m, temp1, work, 1_ilp64,a( 1_ilp64, p ), 1_ilp64 )
                                                   
                                         call stdlib_I64_dlascl( 'G', 0_ilp64, 0_ilp64, one, aapp,m, 1_ilp64, a( 1_ilp64, p ),&
                                                    lda,ierr )
                                         sva( p ) = aapp*sqrt( max( zero,one-aapq*aapq ) )
                                         mxsinj = max( mxsinj, sfmin )
                                      end if
      ...
      ! 2000 :: end of the ibr-loop
           ! .. update sva(n)
              if( ( sva( n )<rootbig ) .and. ( sva( n )>rootsfmin ) )then
                 sva( n ) = stdlib_I64_dnrm2( m, a( 1_ilp64, n ), 1_ilp64 )*d( n )
              else
                 t = zero
                 aapp = one
                 call stdlib_I64_dlassq( m, a( 1_ilp64, n ), 1_ilp64, t, aapp )
                 sva( n ) = t*sqrt( aapp )*d( n )
              end if
           ! additional steering devices
              if( ( i<swband ) .and. ( ( mxaapq<=roottol ) .or.( iswrot<=n ) ) )swband = i
     ...
     !! parameters (marked by [TP]) are available for the implementer.
        ...
        ! =====================================================================
           ...
           ! #:(
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CGSVJ0', -info )
              return
           end if
           if( rsvec ) then
              mvl = n
           ...
           swband = 0_ilp64
      ! [tp] swband is a tuning parameter [tp]. it is meaningful and effective
           ! if stdlib_I64_cgesvj is used as a computational routine in the preconditioned
           ! jacobi svd algorithm stdlib_I64_cgejsv. for sweeps i=1:swband the procedure
           ! works on pivots inside a band-like region around the diagonal.
           ! the boundaries are determined dynamically, based on the number of
           ! pivots above a threshold.
           kbl = min( 8_ilp64, n )
      ...
      ! [tp] lkahead is a tuning parameter.
           ...
           ! .. de rijk's pivoting
                       q = stdlib_I64_isamax( n-p+1, sva( p ), 1_ilp64 ) + p - 1_ilp64
                       if( p/=q ) then
                          call stdlib_I64_cswap( m, a( 1_ilp64, p ), 1_ilp64, a( 1_ilp64, q ), 1_ilp64 )
                          if( rsvec )call stdlib_I64_cswap( mvl, v( 1_ilp64, p ), 1_ilp64,v( 1_ilp64, q ), 1_ilp64 )
                          temp1 = sva( p )
                          sva( p ) = sva( q )
                          sva( q ) = temp1
                          aapq = d(p)
              ...
              ! caveat:
              ! unfortunately, some blas implementations compute sncrm2(m,a(1,p),1)
              ! as sqrt(s=stdlib_I64_cdotc(m,a(1,p),1,a(1,p),1)), which may cause the result to
              ! overflow for ||a(:,p)||_2 > sqrt(overflow_threshold), and to
              ! underflow for ||a(:,p)||_2 < sqrt(underflow_threshold).
              ! hence, stdlib_I64_scnrm2 cannot be trusted, not even in the case when
              ! the true norm is far from the under(over)flow boundaries.
              ! if properly implemented stdlib_I64_scnrm2 is available, the if-then-else-end if
              ! below should be replaced with "aapp = stdlib_I64_scnrm2( m, a(1,p), 1 )".
                          if( ( sva( p )<rootbig ) .and.( sva( p )>rootsfmin ) ) then
                             sva( p ) = stdlib_I64_scnrm2( m, a( 1_ilp64, p ), 1_ilp64 )
                          else
                             temp1 = zero
                             aapp = one
                             call stdlib_I64_classq( m, a( 1_ilp64, p ), 1_ilp64, temp1, aapp )
                             sva( p ) = temp1*sqrt( aapp )
                          end if
                          aapp = sva( p )
                       else
                          ...
                          loop_2002: do q = p + 1, min( igl+kbl-1, n )
                             ...
                             if( aaqq>zero ) then
                                ...
                                if( aaqq>=one ) then
                                   rotok = ( small*aapp )<=aaqq
                                   if( aapp<( big / aaqq ) ) then
                                      aapq = ( stdlib_I64_cdotc( m, a( 1_ilp64, p ), 1_ilp64,a( 1_ilp64, q ), 1_ilp64 ) / &
                                                aaqq ) / aapp
                                   else
                                      call stdlib_I64_ccopy( m, a( 1_ilp64, p ), 1_ilp64,work, 1_ilp64 )
                                      call stdlib_I64_clascl( 'G', 0_ilp64, 0_ilp64, aapp, one,m, 1_ilp64, work, lda, &
                                                ierr )
                                      aapq = stdlib_I64_cdotc( m, work, 1_ilp64,a( 1_ilp64, q ), 1_ilp64 ) / &
                                                aaqq
                                   end if
                                else
                                   rotok = aapp<=( aaqq / small )
                                   if( aapp>( small / aaqq ) ) then
                                      ...
                                      call stdlib_I64_ccopy( m, a( 1_ilp64, q ), 1_ilp64,work, 1_ilp64 )
                                      call stdlib_I64_clascl( 'G', 0_ilp64, 0_ilp64, aaqq,one, m, 1_ilp64,work, lda, &
                                                ierr )
                                      aapq = stdlib_I64_cdotc( m, a( 1_ilp64, p ), 1_ilp64,work, 1_ilp64 ) / &
                                                aapp
                                   end if
                                end if
                                 ! aapq = aapq * conjg( cwork(p) ) * cwork(q)
      ...
      ! [rtd]      rotated = rotated + one
                                   ...
                                   if( rotok ) then
                                      ...
                                      if( abs( theta )>bigtheta ) then
                                         t  = half / theta
                                         cs = one
                                         call stdlib_I64_crot( m, a(1_ilp64,p), 1_ilp64, a(1_ilp64,q), 1_ilp64,cs, conjg(ompq)&
                                                   *t )
                                         if ( rsvec ) then
                                             call stdlib_I64_crot( mvl, v(1_ilp64,p), 1_ilp64,v(1_ilp64,q), 1_ilp64, cs, &
                                                       conjg(ompq)*t )
                                         end if
                                         sva( q ) = aaqq*sqrt( max( zero,one+t*apoaq*aapq1 ) )
                    ...
                    ! .. have to use modified gram-schmidt like transformation
                                      ...
                                      call stdlib_I64_clascl( 'G', 0_ilp64, 0_ilp64, aapp, one, m,1_ilp64, work, lda,&
                                                ierr )
                                      call stdlib_I64_clascl( 'G', 0_ilp64, 0_ilp64, aaqq, one, m,1_ilp64, a( 1_ilp64, q ), &
                                                lda, ierr )
                                      call stdlib_I64_caxpy( m, -aapq, work, 1_ilp64,a( 1_ilp64, q ), 1_ilp64 )
                                      call stdlib_I64_clascl( 'G', 0_ilp64, 0_ilp64, one, aaqq, m,1_ilp64, a( 1_ilp64, q ), &
                                                lda, ierr )
                                      sva( q ) = aaqq*sqrt( max( zero,one-aapq1*aapq1 ) )
                                      mxsinj = max( mxsinj, sfmin )
                                   end if
                 ...
                 ! recompute sva(q), sva(p).
                                   if( ( sva( q ) / aaqq )**2_ilp64<=rooteps )then
                                      if( ( aaqq<rootbig ) .and.( aaqq>rootsfmin ) ) then
                                         sva( q ) = stdlib_I64_scnrm2( m, a( 1_ilp64, q ), 1_ilp64 )
                                      else
                                         t = zero
                                         aaqq = one
                                         call stdlib_I64_classq( m, a( 1_ilp64, q ), 1_ilp64, t,aaqq )
                                         sva( q ) = t*sqrt( aaqq )
                                      end if
                                   end if
                                   if( ( aapp / aapp0 )<=rooteps ) then
                                      if( ( aapp<rootbig ) .and.( aapp>rootsfmin ) ) then
                                         aapp = stdlib_I64_scnrm2( m, a( 1_ilp64, p ), 1_ilp64 )
                                      else
                                         t = zero
                                         aapp = one
                                         call stdlib_I64_classq( m, a( 1_ilp64, p ), 1_ilp64, t,aapp )
                                         aapp = t*sqrt( aapp )
                                      end if
                                      sva( p ) = aapp
                                   end if
           ...
           ! M X 2 Jacobi Svd 
              ! safe gram matrix computation
                                if( aaqq>=one ) then
                                   ...
                                   if( aapp>( small / aaqq ) ) then
                                      aapq = ( stdlib_I64_cdotc( m, a( 1_ilp64, p ), 1_ilp64,a( 1_ilp64, q ), 1_ilp64 ) / max(&
                                                aaqq,aapp) )/ min(aaqq,aapp)
                                   else
                                      call stdlib_I64_ccopy( m, a( 1_ilp64, q ), 1_ilp64,work, 1_ilp64 )
                                      call stdlib_I64_clascl( 'G', 0_ilp64, 0_ilp64, aaqq,one, m, 1_ilp64,work, lda, &
                    ...
                    ! .. have to use modified gram-schmidt like transformation
                                    if( aapp>aaqq ) then
                                        ...
                                        call stdlib_I64_ccopy( m, a( 1_ilp64, q ), 1_ilp64,work, 1_ilp64 )
                                         call stdlib_I64_clascl( 'G', 0_ilp64, 0_ilp64, aaqq, one,m, 1_ilp64, work,lda,&
                                                   ierr )
                                         call stdlib_I64_clascl( 'G', 0_ilp64, 0_ilp64, aapp, one,m, 1_ilp64, a( 1_ilp64, p ),&
                                                    lda,ierr )
                                         call stdlib_I64_caxpy( m, -conjg(aapq),work, 1_ilp64, a( 1_ilp64, p ), 1_ilp64 &
                                                   )
                                         call stdlib_I64_clascl( 'G', 0_ilp64, 0_ilp64, one, aapp,m, 1_ilp64, a( 1_ilp64, p ),&
                                                    lda,ierr )
                                         sva( p ) = aapp*sqrt( max( zero,one-aapq1*aapq1 ) )
      ...
      ! 2000 :: end of the ibr-loop
           ! .. update sva(n)
              if( ( sva( n )<rootbig ) .and. ( sva( n )>rootsfmin ) )then
                 sva( n ) = stdlib_I64_scnrm2( m, a( 1_ilp64, n ), 1_ilp64 )
              else
                 t = zero
                 aapp = one
                 call stdlib_I64_classq( m, a( 1_ilp64, n ), 1_ilp64, t, aapp )
                 sva( n ) = t*sqrt( aapp )
              end if
           ! additional steering devices
              if( ( i<swband ) .and. ( ( mxaapq<=roottol ) .or.( iswrot<=n ) ) )swband = i
     ...
     !! parameters (marked by [TP]) are available for the implementer.
        ...
        ! =====================================================================
           ...
           ! #:(
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZGSVJ0', -info )
              return
           end if
           if( rsvec ) then
              mvl = n
           ...
           swband = 0_ilp64
      ! [tp] swband is a tuning parameter [tp]. it is meaningful and effective
           ! if stdlib_I64_zgesvj is used as a computational routine in the preconditioned
           ! jacobi svd algorithm stdlib_I64_zgejsv. for sweeps i=1:swband the procedure
           ! works on pivots inside a band-like region around the diagonal.
           ! the boundaries are determined dynamically, based on the number of
           ! pivots above a threshold.
           kbl = min( 8_ilp64, n )
      ...
      ! [tp] lkahead is a tuning parameter.
           ...
           ! .. de rijk's pivoting
                       q = stdlib_I64_idamax( n-p+1, sva( p ), 1_ilp64 ) + p - 1_ilp64
                       if( p/=q ) then
                          call stdlib_I64_zswap( m, a( 1_ilp64, p ), 1_ilp64, a( 1_ilp64, q ), 1_ilp64 )
                          if( rsvec )call stdlib_I64_zswap( mvl, v( 1_ilp64, p ), 1_ilp64,v( 1_ilp64, q ), 1_ilp64 )
                          temp1 = sva( p )
                          sva( p ) = sva( q )
                          sva( q ) = temp1
                          aapq = d(p)
              ...
              ! caveat:
              ! unfortunately, some blas implementations compute sncrm2(m,a(1,p),1)
              ! as sqrt(s=stdlib_I64_zdotc(m,a(1,p),1,a(1,p),1)), which may cause the result to
              ! overflow for ||a(:,p)||_2 > sqrt(overflow_threshold), and to
              ! underflow for ||a(:,p)||_2 < sqrt(underflow_threshold).
              ! hence, stdlib_I64_dznrm2 cannot be trusted, not even in the case when
              ! the true norm is far from the under(over)flow boundaries.
              ! if properly implemented stdlib_I64_dznrm2 is available, the if-then-else-end if
              ! below should be replaced with "aapp = stdlib_I64_dznrm2( m, a(1,p), 1 )".
                          if( ( sva( p )<rootbig ) .and.( sva( p )>rootsfmin ) ) then
                             sva( p ) = stdlib_I64_dznrm2( m, a( 1_ilp64, p ), 1_ilp64 )
                          else
                             temp1 = zero
                             aapp = one
                             call stdlib_I64_zlassq( m, a( 1_ilp64, p ), 1_ilp64, temp1, aapp )
                             sva( p ) = temp1*sqrt( aapp )
                          end if
                          aapp = sva( p )
                       else
                          ...
                          loop_2002: do q = p + 1, min( igl+kbl-1, n )
                             ...
                             if( aaqq>zero ) then
                                ...
                                if( aaqq>=one ) then
                                   rotok = ( small*aapp )<=aaqq
                                   if( aapp<( big / aaqq ) ) then
                                      aapq = ( stdlib_I64_zdotc( m, a( 1_ilp64, p ), 1_ilp64,a( 1_ilp64, q ), 1_ilp64 ) / &
                                                aaqq ) / aapp
                                   else
                                      call stdlib_I64_zcopy( m, a( 1_ilp64, p ), 1_ilp64,work, 1_ilp64 )
                                      call stdlib_I64_zlascl( 'G', 0_ilp64, 0_ilp64, aapp, one,m, 1_ilp64, work, lda, &
                                                ierr )
                                      aapq = stdlib_I64_zdotc( m, work, 1_ilp64,a( 1_ilp64, q ), 1_ilp64 ) / &
                                                aaqq
                                   end if
                                else
                                   rotok = aapp<=( aaqq / small )
                                   if( aapp>( small / aaqq ) ) then
                                      ...
                                      call stdlib_I64_zcopy( m, a( 1_ilp64, q ), 1_ilp64,work, 1_ilp64 )
                                      call stdlib_I64_zlascl( 'G', 0_ilp64, 0_ilp64, aaqq,one, m, 1_ilp64,work, lda, &
                                                ierr )
                                      aapq = stdlib_I64_zdotc( m, a( 1_ilp64, p ), 1_ilp64,work, 1_ilp64 ) / &
                                                aapp
                                   end if
                                end if
                                 ! aapq = aapq * conjg( cwork(p) ) * cwork(q)
      ...
      ! [rtd]      rotated = rotated + one
                                   ...
                                   if( rotok ) then
                                      ...
                                      if( abs( theta )>bigtheta ) then
                                         t  = half / theta
                                         cs = one
                                         call stdlib_I64_zrot( m, a(1_ilp64,p), 1_ilp64, a(1_ilp64,q), 1_ilp64,cs, conjg(ompq)&
                                                   *t )
                                         if ( rsvec ) then
                                             call stdlib_I64_zrot( mvl, v(1_ilp64,p), 1_ilp64,v(1_ilp64,q), 1_ilp64, cs, &
                                                       conjg(ompq)*t )
                                         end if
                                         sva( q ) = aaqq*sqrt( max( zero,one+t*apoaq*aapq1 ) )
                    ...
                    ! .. have to use modified gram-schmidt like transformation
                                      ...
                                      call stdlib_I64_zlascl( 'G', 0_ilp64, 0_ilp64, aapp, one, m,1_ilp64, work, lda,&
                                                ierr )
                                      call stdlib_I64_zlascl( 'G', 0_ilp64, 0_ilp64, aaqq, one, m,1_ilp64, a( 1_ilp64, q ), &
                                                lda, ierr )
                                      call stdlib_I64_zaxpy( m, -aapq, work, 1_ilp64,a( 1_ilp64, q ), 1_ilp64 )
                                      call stdlib_I64_zlascl( 'G', 0_ilp64, 0_ilp64, one, aaqq, m,1_ilp64, a( 1_ilp64, q ), &
                                                lda, ierr )
                                      sva( q ) = aaqq*sqrt( max( zero,one-aapq1*aapq1 ) )
                                      mxsinj = max( mxsinj, sfmin )
                                   end if
                 ...
                 ! recompute sva(q), sva(p).
                                   if( ( sva( q ) / aaqq )**2_ilp64<=rooteps )then
                                      if( ( aaqq<rootbig ) .and.( aaqq>rootsfmin ) ) then
                                         sva( q ) = stdlib_I64_dznrm2( m, a( 1_ilp64, q ), 1_ilp64 )
                                      else
                                         t = zero
                                         aaqq = one
                                         call stdlib_I64_zlassq( m, a( 1_ilp64, q ), 1_ilp64, t,aaqq )
                                         sva( q ) = t*sqrt( aaqq )
                                      end if
                                   end if
                                   if( ( aapp / aapp0 )<=rooteps ) then
                                      if( ( aapp<rootbig ) .and.( aapp>rootsfmin ) ) then
                                         aapp = stdlib_I64_dznrm2( m, a( 1_ilp64, p ), 1_ilp64 )
                                      else
                                         t = zero
                                         aapp = one
                                         call stdlib_I64_zlassq( m, a( 1_ilp64, p ), 1_ilp64, t,aapp )
                                         aapp = t*sqrt( aapp )
                                      end if
                                      sva( p ) = aapp
                                   end if
           ...
           ! M X 2 Jacobi Svd 
              ! safe gram matrix computation
                                if( aaqq>=one ) then
                                   ...
                                   if( aapp>( small / aaqq ) ) then
                                      aapq = ( stdlib_I64_zdotc( m, a( 1_ilp64, p ), 1_ilp64,a( 1_ilp64, q ), 1_ilp64 ) / max(&
                                                aaqq,aapp) )/ min(aaqq,aapp)
                                   else
                                      call stdlib_I64_zcopy( m, a( 1_ilp64, q ), 1_ilp64,work, 1_ilp64 )
                                      call stdlib_I64_zlascl( 'G', 0_ilp64, 0_ilp64, aaqq,one, m, 1_ilp64,work, lda, &
                    ...
                    ! .. have to use modified gram-schmidt like transformation
                                    if( aapp>aaqq ) then
                                        ...
                                        call stdlib_I64_zcopy( m, a( 1_ilp64, q ), 1_ilp64,work, 1_ilp64 )
                                         call stdlib_I64_zlascl( 'G', 0_ilp64, 0_ilp64, aaqq, one,m, 1_ilp64, work,lda,&
                                                   ierr )
                                         call stdlib_I64_zlascl( 'G', 0_ilp64, 0_ilp64, aapp, one,m, 1_ilp64, a( 1_ilp64, p ),&
                                                    lda,ierr )
                                         call stdlib_I64_zaxpy( m, -conjg(aapq),work, 1_ilp64, a( 1_ilp64, p ), 1_ilp64 &
                                                   )
                                         call stdlib_I64_zlascl( 'G', 0_ilp64, 0_ilp64, one, aapp,m, 1_ilp64, a( 1_ilp64, p ),&
                                                    lda,ierr )
                                         sva( p ) = aapp*sqrt( max( zero,one-aapq1*aapq1 ) )
      ...
      ! 2000 :: end of the ibr-loop
           ! .. update sva(n)
              if( ( sva( n )<rootbig ) .and. ( sva( n )>rootsfmin ) )then
                 sva( n ) = stdlib_I64_dznrm2( m, a( 1_ilp64, n ), 1_ilp64 )
              else
                 t = zero
                 aapp = one
                 call stdlib_I64_zlassq( m, a( 1_ilp64, n ), 1_ilp64, t, aapp )
                 sva( n ) = t*sqrt( aapp )
              end if
           ! additional steering devices
              if( ( i<swband ) .and. ( ( mxaapq<=roottol ) .or.( iswrot<=n ) ) )swband = i
     ...
     !! is given in TOL.
        ...
        ! =====================================================================
           ...
           mxsinj, rootbig, rooteps, rootsfmin, roottol, small, sn, t, temp1, theta, &
                     thsign
           integer(ilp64) :: blskip, emptsw, i, ibr, igl, ierr, ijblsk, iswrot, jbc, jgl, kbl, mvl, &
                     notrot, nblc, nblr, p, pskipped, q, rowskip, swband
           logical(lk) :: applv, rotok, rsvec
           ! Local Arrays 
           real(sp) :: fastr(5_ilp64)
           ...
           ! #:(
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SGSVJ1', -info )
              return
           end if
           if( rsvec ) then
              mvl = n
           ...
           kbl = min( 8_ilp64, n )
           nblr = n1 / kbl
           if( ( nblr*kbl )/=n1 )nblr = nblr + 1_ilp64
           ! .. the tiling is nblr-by-nblc [tiles]
           nblc = ( n-n1 ) / kbl
           if( ( nblc*kbl )/=( n-n1 ) )nblc = nblc + 1_ilp64
           blskip = ( kbl**2_ilp64 ) + 1_ilp64
      ! [tp] blkskip is a tuning parameter that depends on swband and kbl.
           rowskip = min( 5_ilp64, kbl )
      ! [tp] rowskip is a tuning parameter.
      ...
      ! [tp] swband is a tuning parameter. it is meaningful and effective
           ! if stdlib_I64_sgesvj is used as a computational routine in the preconditioned
           ! jacobi svd algorithm stdlib_I64_sgesvj.
           ! | *   *   * [x] [x] [x]|
           ! | *   *   * [x] [x] [x]|    row-cycling in the nblr-by-nblc [x] blocks.
           ! | *   *   * [x] [x] [x]|    row-cyclic pivoting inside each [x] block.
           ! |[x] [x] [x] *   *   * |
     ...
     !! is given in TOL.
        ...
        ! =====================================================================
           ...
           ! #:(
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DGSVJ1', -info )
              return
           end if
           if( rsvec ) then
              mvl = n
     ...
     !! is given in TOL.
        ...
        ! =====================================================================
           ...
           ! #:(
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CGSVJ1', -info )
              return
           end if
           if( rsvec ) then
              mvl = n
      ...
      ! [tp] swband is a tuning parameter. it is meaningful and effective
           ! if stdlib_I64_cgesvj is used as a computational routine in the preconditioned
           ! jacobi svd algorithm stdlib_I64_cgejsv.
           ! | *   *   * [x] [x] [x]|
           ! | *   *   * [x] [x] [x]|    row-cycling in the nblr-by-nblc [x] blocks.
           ! | *   *   * [x] [x] [x]|    row-cyclic pivoting inside each [x] block.
           ! |[x] [x] [x] *   *   * |
     ...
     !! is given in TOL.
        ...
        ! =====================================================================
           ...
           ! #:(
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZGSVJ1', -info )
              return
           end if
           if( rsvec ) then
              mvl = n
      ...
      ! [tp] swband is a tuning parameter. it is meaningful and effective
           ! if stdlib_I64_zgesvj is used as a computational routine in the preconditioned
           ! jacobi svd algorithm stdlib_I64_zgejsv.
           ! | *   *   * [x] [x] [x]|
           ! | *   *   * [x] [x] [x]|    row-cycling in the nblr-by-nblc [x] blocks.
           ! | *   *   * [x] [x] [x]|    row-cyclic pivoting inside each [x] block.
           ! |[x] [x] [x] *   *   * |
     ...
     !! may be postmultiplied into input matrices U1, V1, or Q1.
        ...
        ! =====================================================================
           ! Parameters 
           integer(ilp64), parameter :: maxit = 40_ilp64
           real(sp), parameter :: hugenum = huge(zero)
           
           
           ! Local Scalars 
           logical(lk) :: initq, initu, initv, upper, wantq, wantu, wantv
           integer(ilp64) :: i, j, kcycle
           real(sp) :: a1, a2, a3, b1, b2, b3, csq, csu, csv, error, gamma, rwk, snq, snu, snv, &
                     ssmin
           ! Intrinsic Functions 
           ! Executable Statements 
           ...
           if( .not.( initu .or. wantu .or. stdlib_lsame( jobu, 'N' ) ) ) then
              ...
              info = -4_ilp64
           else if( p<0_ilp64 ) then
              info = -5_ilp64
           else if( n<0_ilp64 ) then
              info = -6_ilp64
           else if( lda<max( 1_ilp64, m ) ) then
              info = -10_ilp64
           else if( ldb<max( 1_ilp64, p ) ) then
              info = -12_ilp64
           else if( ldu<1_ilp64 .or. ( wantu .and. ldu<m ) ) then
              info = -18_ilp64
           else if( ldv<1_ilp64 .or. ( wantv .and. ldv<p ) ) then
              info = -20_ilp64
           else if( ldq<1_ilp64 .or. ( wantq .and. ldq<n ) ) then
              info = -22_ilp64
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'STGSJA', -info )
              return
           end if
           ! initialize u, v and q, if necessary
           if( initu )call stdlib_I64_slaset( 'FULL', m, m, zero, one, u, ldu )
           if( initv )call stdlib_I64_slaset( 'FULL', p, p, zero, one, v, ldv )
           if( initq )call stdlib_I64_slaset( 'FULL', n, n, zero, one, q, ldq )
           ! loop until convergence
           upper = .false.
           loop_40: do kcycle = 1, maxit
              upper = .not.upper
              loop_20: do i = 1, l - 1
                 loop_10: do j = i + 1, l
                    ...
                    call stdlib_I64_slags2( upper, a1, a2, a3, b1, b2, b3, csu, snu,csv, snv, csq, &
                              snq )
                    ! update (k+i)-th and (k+j)-th rows of matrix a: u**t *a
                    if( k+j<=m )call stdlib_I64_srot( l, a( k+j, n-l+1 ), lda, a( k+i, n-l+1 ),lda, &
                              csu, snu )
                    ! update i-th and j-th rows of matrix b: v**t *b
                    call stdlib_I64_srot( l, b( j, n-l+1 ), ldb, b( i, n-l+1 ), ldb,csv, snv )
                    ! update (n-l+i)-th and (n-l+j)-th columns of matrices
                    ! a and b: a*q and b*q
                    call stdlib_I64_srot( min( k+l, m ), a( 1_ilp64, n-l+j ), 1_ilp64,a( 1_ilp64, n-l+i ), 1_ilp64, csq, snq )
                              
                    call stdlib_I64_srot( l, b( 1_ilp64, n-l+j ), 1_ilp64, b( 1_ilp64, n-l+i ), 1_ilp64, csq,snq )
                    if( upper ) then
                       if( k+i<=m )a( k+i, n-l+j ) = zero
                       b( i, n-l+j ) = zero
                    else
                    ...
                    ! update orthogonal matrices u, v, q, if desired.
                    if( wantu .and. k+j<=m )call stdlib_I64_srot( m, u( 1_ilp64, k+j ), 1_ilp64, u( 1_ilp64, k+i ), 1_ilp64, &
                              csu,snu )
                    if( wantv )call stdlib_I64_srot( p, v( 1_ilp64, j ), 1_ilp64, v( 1_ilp64, i ), 1_ilp64, csv, snv )
                    if( wantq )call stdlib_I64_srot( n, q( 1_ilp64, n-l+j ), 1_ilp64, q( 1_ilp64, n-l+i ), 1_ilp64, csq,snq )
              ...
              if( .not.upper ) then
                 ...
                 error = zero
                 do i = 1, min( l, m-k )
                    call stdlib_I64_scopy( l-i+1, a( k+i, n-l+i ), lda, work, 1_ilp64 )
                    call stdlib_I64_scopy( l-i+1, b( i, n-l+i ), ldb, work( l+1 ), 1_ilp64 )
                    call stdlib_I64_slapll( l-i+1, work, 1_ilp64, work( l+1 ), 1_ilp64, ssmin )
                    error = max( error, ssmin )
                 end do
                 if( abs( error )<=min( tola, tolb ) )go to 50
              end if
           ...
           do i = 1, min( l, m-k )
              ...
              if( (gamma<=hugenum).and.(gamma>=-hugenum) ) then
                 ! change sign if necessary
                 if( gamma<zero ) then
                    call stdlib_I64_sscal( l-i+1, -one, b( i, n-l+i ), ldb )
                    if( wantv )call stdlib_I64_sscal( p, -one, v( 1_ilp64, i ), 1_ilp64 )
                 end if
                 call stdlib_I64_slartg( abs( gamma ), one, beta( k+i ), alpha( k+i ),rwk )
                 if( alpha( k+i )>=beta( k+i ) ) then
                    call stdlib_I64_sscal( l-i+1, one / alpha( k+i ), a( k+i, n-l+i ),lda )
                 else
                    call stdlib_I64_sscal( l-i+1, one / beta( k+i ), b( i, n-l+i ),ldb )
                    call stdlib_I64_scopy( l-i+1, b( i, n-l+i ), ldb, a( k+i, n-l+i ),lda )
                 end if
              else
                 alpha( k+i ) = zero
                 beta( k+i ) = one
     ...
     !! may be postmultiplied into input matrices U1, V1, or Q1.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DTGSJA', -info )
              return
           end if
           ! initialize u, v and q, if necessary
           if( initu )call stdlib_I64_dlaset( 'FULL', m, m, zero, one, u, ldu )
           if( initv )call stdlib_I64_dlaset( 'FULL', p, p, zero, one, v, ldv )
           if( initq )call stdlib_I64_dlaset( 'FULL', n, n, zero, one, q, ldq )
           ! loop until convergence
           upper = .false.
           loop_40: do kcycle = 1, maxit
              upper = .not.upper
              loop_20: do i = 1, l - 1
                 loop_10: do j = i + 1, l
                    ...
                    call stdlib_I64_dlags2( upper, a1, a2, a3, b1, b2, b3, csu, snu,csv, snv, csq, &
                              snq )
                    ! update (k+i)-th and (k+j)-th rows of matrix a: u**t *a
                    if( k+j<=m )call stdlib_I64_drot( l, a( k+j, n-l+1 ), lda, a( k+i, n-l+1 ),lda, &
                              csu, snu )
                    ! update i-th and j-th rows of matrix b: v**t *b
                    call stdlib_I64_drot( l, b( j, n-l+1 ), ldb, b( i, n-l+1 ), ldb,csv, snv )
                    ! update (n-l+i)-th and (n-l+j)-th columns of matrices
                    ! a and b: a*q and b*q
                    call stdlib_I64_drot( min( k+l, m ), a( 1_ilp64, n-l+j ), 1_ilp64,a( 1_ilp64, n-l+i ), 1_ilp64, csq, snq )
                              
                    call stdlib_I64_drot( l, b( 1_ilp64, n-l+j ), 1_ilp64, b( 1_ilp64, n-l+i ), 1_ilp64, csq,snq )
                    if( upper ) then
                       if( k+i<=m )a( k+i, n-l+j ) = zero
                       b( i, n-l+j ) = zero
                    else
                    ...
                    ! update orthogonal matrices u, v, q, if desired.
                    if( wantu .and. k+j<=m )call stdlib_I64_drot( m, u( 1_ilp64, k+j ), 1_ilp64, u( 1_ilp64, k+i ), 1_ilp64, &
                              csu,snu )
                    if( wantv )call stdlib_I64_drot( p, v( 1_ilp64, j ), 1_ilp64, v( 1_ilp64, i ), 1_ilp64, csv, snv )
                    if( wantq )call stdlib_I64_drot( n, q( 1_ilp64, n-l+j ), 1_ilp64, q( 1_ilp64, n-l+i ), 1_ilp64, csq,snq )
              ...
              if( .not.upper ) then
                 ...
                 error = zero
                 do i = 1, min( l, m-k )
                    call stdlib_I64_dcopy( l-i+1, a( k+i, n-l+i ), lda, work, 1_ilp64 )
                    call stdlib_I64_dcopy( l-i+1, b( i, n-l+i ), ldb, work( l+1 ), 1_ilp64 )
                    call stdlib_I64_dlapll( l-i+1, work, 1_ilp64, work( l+1 ), 1_ilp64, ssmin )
                    error = max( error, ssmin )
                 end do
                 if( abs( error )<=min( tola, tolb ) )go to 50
              end if
           ...
           do i = 1, min( l, m-k )
              ...
              if( (gamma<=hugenum).and.(gamma>=-hugenum) ) then
                 ! change sign if necessary
                 if( gamma<zero ) then
                    call stdlib_I64_dscal( l-i+1, -one, b( i, n-l+i ), ldb )
                    if( wantv )call stdlib_I64_dscal( p, -one, v( 1_ilp64, i ), 1_ilp64 )
                 end if
                 call stdlib_I64_dlartg( abs( gamma ), one, beta( k+i ), alpha( k+i ),rwk )
                 if( alpha( k+i )>=beta( k+i ) ) then
                    call stdlib_I64_dscal( l-i+1, one / alpha( k+i ), a( k+i, n-l+i ),lda )
                 else
                    call stdlib_I64_dscal( l-i+1, one / beta( k+i ), b( i, n-l+i ),ldb )
                    call stdlib_I64_dcopy( l-i+1, b( i, n-l+i ), ldb, a( k+i, n-l+i ),lda )
                 end if
              else
                 alpha( k+i ) = zero
                 beta( k+i ) = one
     ...
     !! may be postmultiplied into input matrices U1, V1, or Q1.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CTGSJA', -info )
              return
           end if
           ! initialize u, v and q, if necessary
           if( initu )call stdlib_I64_claset( 'FULL', m, m, czero, cone, u, ldu )
           if( initv )call stdlib_I64_claset( 'FULL', p, p, czero, cone, v, ldv )
           if( initq )call stdlib_I64_claset( 'FULL', n, n, czero, cone, q, ldq )
           ! loop until convergence
           upper = .false.
           loop_40: do kcycle = 1, maxit
              upper = .not.upper
              loop_20: do i = 1, l - 1
                 loop_10: do j = i + 1, l
                    ...
                    call stdlib_I64_clags2( upper, a1, a2, a3, b1, b2, b3, csu, snu,csv, snv, csq, &
                              snq )
                    ! update (k+i)-th and (k+j)-th rows of matrix a: u**h *a
                    if( k+j<=m )call stdlib_I64_crot( l, a( k+j, n-l+1 ), lda, a( k+i, n-l+1 ),lda, &
                              csu, conjg( snu ) )
                    ! update i-th and j-th rows of matrix b: v**h *b
                    call stdlib_I64_crot( l, b( j, n-l+1 ), ldb, b( i, n-l+1 ), ldb,csv, conjg( snv ) &
                              )
                    ! update (n-l+i)-th and (n-l+j)-th columns of matrices
                    ! a and b: a*q and b*q
                    call stdlib_I64_crot( min( k+l, m ), a( 1_ilp64, n-l+j ), 1_ilp64,a( 1_ilp64, n-l+i ), 1_ilp64, csq, snq )
                              
                    call stdlib_I64_crot( l, b( 1_ilp64, n-l+j ), 1_ilp64, b( 1_ilp64, n-l+i ), 1_ilp64, csq,snq )
                    if( upper ) then
                       if( k+i<=m )a( k+i, n-l+j ) = czero
                       b( i, n-l+j ) = czero
                    else
                    ...
                    b( j, n-l+j ) = real( b( j, n-l+j ),KIND=sp)
                    ! update unitary matrices u, v, q, if desired.
                    if( wantu .and. k+j<=m )call stdlib_I64_crot( m, u( 1_ilp64, k+j ), 1_ilp64, u( 1_ilp64, k+i ), 1_ilp64, &
                              csu,snu )
                    if( wantv )call stdlib_I64_crot( p, v( 1_ilp64, j ), 1_ilp64, v( 1_ilp64, i ), 1_ilp64, csv, snv )
                    if( wantq )call stdlib_I64_crot( n, q( 1_ilp64, n-l+j ), 1_ilp64, q( 1_ilp64, n-l+i ), 1_ilp64, csq,snq )
              ...
              if( .not.upper ) then
                 ...
                 error = zero
                 do i = 1, min( l, m-k )
                    call stdlib_I64_ccopy( l-i+1, a( k+i, n-l+i ), lda, work, 1_ilp64 )
                    call stdlib_I64_ccopy( l-i+1, b( i, n-l+i ), ldb, work( l+1 ), 1_ilp64 )
                    call stdlib_I64_clapll( l-i+1, work, 1_ilp64, work( l+1 ), 1_ilp64, ssmin )
                    error = max( error, ssmin )
                 end do
                 if( abs( error )<=min( tola, tolb ) )go to 50
              end if
           ...
           do i = 1, min( l, m-k )
              ...
              if( (gamma<=hugenum).and.(gamma>=-hugenum) ) then
                 if( gamma<zero ) then
                    call stdlib_I64_csscal( l-i+1, -one, b( i, n-l+i ), ldb )
                    if( wantv )call stdlib_I64_csscal( p, -one, v( 1_ilp64, i ), 1_ilp64 )
                 end if
                 call stdlib_I64_slartg( abs( gamma ), one, beta( k+i ), alpha( k+i ),rwk )
                 if( alpha( k+i )>=beta( k+i ) ) then
                    call stdlib_I64_csscal( l-i+1, one / alpha( k+i ), a( k+i, n-l+i ),lda )
                 else
                    call stdlib_I64_csscal( l-i+1, one / beta( k+i ), b( i, n-l+i ),ldb )
                    call stdlib_I64_ccopy( l-i+1, b( i, n-l+i ), ldb, a( k+i, n-l+i ),lda )
                 end if
              else
                 alpha( k+i ) = zero
                 beta( k+i ) = one
     ...
     !! may be postmultiplied into input matrices U1, V1, or Q1.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZTGSJA', -info )
              return
           end if
           ! initialize u, v and q, if necessary
           if( initu )call stdlib_I64_zlaset( 'FULL', m, m, czero, cone, u, ldu )
           if( initv )call stdlib_I64_zlaset( 'FULL', p, p, czero, cone, v, ldv )
           if( initq )call stdlib_I64_zlaset( 'FULL', n, n, czero, cone, q, ldq )
           ! loop until convergence
           upper = .false.
           loop_40: do kcycle = 1, maxit
              upper = .not.upper
              loop_20: do i = 1, l - 1
                 loop_10: do j = i + 1, l
                    ...
                    call stdlib_I64_zlags2( upper, a1, a2, a3, b1, b2, b3, csu, snu,csv, snv, csq, &
                              snq )
                    ! update (k+i)-th and (k+j)-th rows of matrix a: u**h *a
                    if( k+j<=m )call stdlib_I64_zrot( l, a( k+j, n-l+1 ), lda, a( k+i, n-l+1 ),lda, &
                              csu, conjg( snu ) )
                    ! update i-th and j-th rows of matrix b: v**h *b
                    call stdlib_I64_zrot( l, b( j, n-l+1 ), ldb, b( i, n-l+1 ), ldb,csv, conjg( snv ) &
                              )
                    ! update (n-l+i)-th and (n-l+j)-th columns of matrices
                    ! a and b: a*q and b*q
                    call stdlib_I64_zrot( min( k+l, m ), a( 1_ilp64, n-l+j ), 1_ilp64,a( 1_ilp64, n-l+i ), 1_ilp64, csq, snq )
                              
                    call stdlib_I64_zrot( l, b( 1_ilp64, n-l+j ), 1_ilp64, b( 1_ilp64, n-l+i ), 1_ilp64, csq,snq )
                    if( upper ) then
                       if( k+i<=m )a( k+i, n-l+j ) = czero
                       b( i, n-l+j ) = czero
                    else
                    ...
                    b( j, n-l+j ) = real( b( j, n-l+j ),KIND=dp)
                    ! update unitary matrices u, v, q, if desired.
                    if( wantu .and. k+j<=m )call stdlib_I64_zrot( m, u( 1_ilp64, k+j ), 1_ilp64, u( 1_ilp64, k+i ), 1_ilp64, &
                              csu,snu )
                    if( wantv )call stdlib_I64_zrot( p, v( 1_ilp64, j ), 1_ilp64, v( 1_ilp64, i ), 1_ilp64, csv, snv )
                    if( wantq )call stdlib_I64_zrot( n, q( 1_ilp64, n-l+j ), 1_ilp64, q( 1_ilp64, n-l+i ), 1_ilp64, csq,snq )
              ...
              if( .not.upper ) then
                 ...
                 error = zero
                 do i = 1, min( l, m-k )
                    call stdlib_I64_zcopy( l-i+1, a( k+i, n-l+i ), lda, work, 1_ilp64 )
                    call stdlib_I64_zcopy( l-i+1, b( i, n-l+i ), ldb, work( l+1 ), 1_ilp64 )
                    call stdlib_I64_zlapll( l-i+1, work, 1_ilp64, work( l+1 ), 1_ilp64, ssmin )
                    error = max( error, ssmin )
                 end do
                 if( abs( error )<=min( tola, tolb ) )go to 50
              end if
           ...
           do i = 1, min( l, m-k )
              ...
              if( (gamma<=hugenum).and.(gamma>=-hugenum) ) then
                 if( gamma<zero ) then
                    call stdlib_I64_zdscal( l-i+1, -one, b( i, n-l+i ), ldb )
                    if( wantv )call stdlib_I64_zdscal( p, -one, v( 1_ilp64, i ), 1_ilp64 )
                 end if
                 call stdlib_I64_dlartg( abs( gamma ), one, beta( k+i ), alpha( k+i ),rwk )
                 if( alpha( k+i )>=beta( k+i ) ) then
                    call stdlib_I64_zdscal( l-i+1, one / alpha( k+i ), a( k+i, n-l+i ),lda )
                 else
                    call stdlib_I64_zdscal( l-i+1, one / beta( k+i ), b( i, n-l+i ),ldb )
                    call stdlib_I64_zcopy( l-i+1, b( i, n-l+i ), ldb, a( k+i, n-l+i ),lda )
                 end if
              else
                 alpha( k+i ) = zero
                 beta( k+i ) = one
     ...
     !! an N-by-N matrix.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           logical(lk) :: lquery, wantq
           integer(ilp64) :: i, iinfo, j, lwkopt, mn
           ! Intrinsic Functions 
           ! Executable Statements 
           ! test the input arguments
           info = 0_ilp64
           ...
           if( .not.wantq .and. .not.stdlib_lsame( vect, 'P' ) ) then
              ...
              info = -2_ilp64
           else if( n<0_ilp64 .or. ( wantq .and. ( n>m .or. n<min( m,k ) ) ) .or. ( .not.wantq .and. ( &
                     m>n .or. m<min( n, k ) ) ) ) then
              info = -3_ilp64
              ...
              info = -6_ilp64
           else if( lwork<max( 1_ilp64, mn ) .and. .not.lquery ) then
              info = -9_ilp64
           end if
           if( info==0_ilp64 ) then
              work( 1_ilp64 ) = 1_ilp64
              if( wantq ) then
                 if( m>=k ) then
                    call stdlib_I64_cungqr( m, n, k, a, lda, tau, work, -1_ilp64, iinfo )
                 else
                    if( m>1_ilp64 ) then
                       call stdlib_I64_cungqr( m-1, m-1, m-1, a, lda, tau, work, -1_ilp64,iinfo )
                    end if
                 end if
              else
                 if( k<n ) then
                    call stdlib_I64_cunglq( m, n, k, a, lda, tau, work, -1_ilp64, iinfo )
                 else
                    if( n>1_ilp64 ) then
                       call stdlib_I64_cunglq( n-1, n-1, n-1, a, lda, tau, work, -1_ilp64,iinfo )
                    end if
                 end if
              end if
              lwkopt = real( work( 1_ilp64 ),KIND=sp)
              lwkopt = max (lwkopt, mn)
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CUNGBR', -info )
              return
           else if( lquery ) then
              work( 1_ilp64 ) = lwkopt
              return
           ...
           if( wantq ) then
              ! form q, determined by a call to stdlib_I64_cgebrd to reduce an m-by-k
              ! matrix
              if( m>=k ) then
                 ! if m >= k, assume m >= n >= k
                 call stdlib_I64_cungqr( m, n, k, a, lda, tau, work, lwork, iinfo )
              else
                 ! if m < k, assume m = n
                 ! shift the vectors which define the elementary reflectors cone
                 ! column to the right, and set the first row and column of q
                 ...
                 if( m>1_ilp64 ) then
                    ! form q(2:m,2:m)
                    call stdlib_I64_cungqr( m-1, m-1, m-1, a( 2_ilp64, 2_ilp64 ), lda, tau, work,lwork, iinfo )
                              
                 end if
              end if
           else
              ! form p**h, determined by a call to stdlib_I64_cgebrd to reduce a k-by-n
              ! matrix
              if( k<n ) then
                 ! if k < n, assume k <= m <= n
                 call stdlib_I64_cunglq( m, n, k, a, lda, tau, work, lwork, iinfo )
              else
                 ! if k >= n, assume m = n
                 ! shift the vectors which define the elementary reflectors cone
                 ! row downward, and set the first row and column of p**h to
                 ...
                 if( n>1_ilp64 ) then
                    ! form p**h(2:n,2:n)
                    call stdlib_I64_cunglq( n-1, n-1, n-1, a( 2_ilp64, 2_ilp64 ), lda, tau, work,lwork, iinfo )
     ...
     !! an N-by-N matrix.
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              ...
              if( wantq ) then
                 if( m>=k ) then
                    call stdlib_I64_zungqr( m, n, k, a, lda, tau, work, -1_ilp64, iinfo )
                 else
                    if( m>1_ilp64 ) then
                       call stdlib_I64_zungqr( m-1, m-1, m-1, a, lda, tau, work, -1_ilp64,iinfo )
                    end if
                 end if
              else
                 if( k<n ) then
                    call stdlib_I64_zunglq( m, n, k, a, lda, tau, work, -1_ilp64, iinfo )
                 else
                    if( n>1_ilp64 ) then
                       call stdlib_I64_zunglq( n-1, n-1, n-1, a, lda, tau, work, -1_ilp64,iinfo )
                    end if
                 end if
              end if
              lwkopt = real( work( 1_ilp64 ),KIND=dp)
              lwkopt = max (lwkopt, mn)
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZUNGBR', -info )
              return
           else if( lquery ) then
              work( 1_ilp64 ) = lwkopt
              return
           ...
           if( wantq ) then
              ! form q, determined by a call to stdlib_I64_zgebrd to reduce an m-by-k
              ! matrix
              if( m>=k ) then
                 ! if m >= k, assume m >= n >= k
                 call stdlib_I64_zungqr( m, n, k, a, lda, tau, work, lwork, iinfo )
              else
                 ! if m < k, assume m = n
                 ! shift the vectors which define the elementary reflectors cone
                 ! column to the right, and set the first row and column of q
                 ...
                 if( m>1_ilp64 ) then
                    ! form q(2:m,2:m)
                    call stdlib_I64_zungqr( m-1, m-1, m-1, a( 2_ilp64, 2_ilp64 ), lda, tau, work,lwork, iinfo )
                              
                 end if
              end if
           else
              ! form p**h, determined by a call to stdlib_I64_zgebrd to reduce a k-by-n
              ! matrix
              if( k<n ) then
                 ! if k < n, assume k <= m <= n
                 call stdlib_I64_zunglq( m, n, k, a, lda, tau, work, lwork, iinfo )
              else
                 ! if k >= n, assume m = n
                 ! shift the vectors which define the elementary reflectors cone
                 ! row downward, and set the first row and column of p**h to
                 ...
                 if( n>1_ilp64 ) then
                    ! form p**h(2:n,2:n)
                    call stdlib_I64_zunglq( n-1, n-1, n-1, a( 2_ilp64, 2_ilp64 ), lda, tau, work,lwork, iinfo )
     ...
     !! an N-by-N matrix.
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              ...
              if( wantq ) then
                 if( m>=k ) then
                    call stdlib_I64_sorgqr( m, n, k, a, lda, tau, work, -1_ilp64, iinfo )
                 else
                    if( m>1_ilp64 ) then
                       call stdlib_I64_sorgqr( m-1, m-1, m-1, a, lda, tau, work, -1_ilp64,iinfo )
                    end if
                 end if
              else
                 if( k<n ) then
                    call stdlib_I64_sorglq( m, n, k, a, lda, tau, work, -1_ilp64, iinfo )
                 else
                    if( n>1_ilp64 ) then
                       call stdlib_I64_sorglq( n-1, n-1, n-1, a, lda, tau, work, -1_ilp64,iinfo )
                    end if
                 end if
              end if
              lwkopt = work( 1_ilp64 )
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SORGBR', -info )
              return
           else if( lquery ) then
              work( 1_ilp64 ) = lwkopt
              return
           ...
           if( wantq ) then
              ! form q, determined by a call to stdlib_I64_sgebrd to reduce an m-by-k
              ! matrix
              if( m>=k ) then
                 ! if m >= k, assume m >= n >= k
                 call stdlib_I64_sorgqr( m, n, k, a, lda, tau, work, lwork, iinfo )
              else
                 ! if m < k, assume m = n
                 ! shift the vectors which define the elementary reflectors one
                 ! column to the right, and set the first row and column of q
                 ...
                 if( m>1_ilp64 ) then
                    ! form q(2:m,2:m)
                    call stdlib_I64_sorgqr( m-1, m-1, m-1, a( 2_ilp64, 2_ilp64 ), lda, tau, work,lwork, iinfo )
                              
                 end if
              end if
           else
              ! form p**t, determined by a call to stdlib_I64_sgebrd to reduce a k-by-n
              ! matrix
              if( k<n ) then
                 ! if k < n, assume k <= m <= n
                 call stdlib_I64_sorglq( m, n, k, a, lda, tau, work, lwork, iinfo )
              else
                 ! if k >= n, assume m = n
                 ! shift the vectors which define the elementary reflectors one
                 ! row downward, and set the first row and column of p**t to
                 ...
                 if( n>1_ilp64 ) then
                    ! form p**t(2:n,2:n)
                    call stdlib_I64_sorglq( n-1, n-1, n-1, a( 2_ilp64, 2_ilp64 ), lda, tau, work,lwork, iinfo )
     ...
     !! an N-by-N matrix.
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              ...
              if( wantq ) then
                 if( m>=k ) then
                    call stdlib_I64_dorgqr( m, n, k, a, lda, tau, work, -1_ilp64, iinfo )
                 else
                    if( m>1_ilp64 ) then
                       call stdlib_I64_dorgqr( m-1, m-1, m-1, a, lda, tau, work, -1_ilp64,iinfo )
                    end if
                 end if
              else
                 if( k<n ) then
                    call stdlib_I64_dorglq( m, n, k, a, lda, tau, work, -1_ilp64, iinfo )
                 else
                    if( n>1_ilp64 ) then
                       call stdlib_I64_dorglq( n-1, n-1, n-1, a, lda, tau, work, -1_ilp64,iinfo )
                    end if
                 end if
              end if
              lwkopt = work( 1_ilp64 )
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DORGBR', -info )
              return
           else if( lquery ) then
              work( 1_ilp64 ) = lwkopt
              return
           ...
           if( wantq ) then
              ! form q, determined by a call to stdlib_I64_dgebrd to reduce an m-by-k
              ! matrix
              if( m>=k ) then
                 ! if m >= k, assume m >= n >= k
                 call stdlib_I64_dorgqr( m, n, k, a, lda, tau, work, lwork, iinfo )
              else
                 ! if m < k, assume m = n
                 ! shift the vectors which define the elementary reflectors one
                 ! column to the right, and set the first row and column of q
                 ...
                 if( m>1_ilp64 ) then
                    ! form q(2:m,2:m)
                    call stdlib_I64_dorgqr( m-1, m-1, m-1, a( 2_ilp64, 2_ilp64 ), lda, tau, work,lwork, iinfo )
                              
                 end if
              end if
           else
              ! form p**t, determined by a call to stdlib_I64_dgebrd to reduce a k-by-n
              ! matrix
              if( k<n ) then
                 ! if k < n, assume k <= m <= n
                 call stdlib_I64_dorglq( m, n, k, a, lda, tau, work, lwork, iinfo )
              else
                 ! if k >= n, assume m = n
                 ! shift the vectors which define the elementary reflectors one
                 ! row downward, and set the first row and column of p**t to
                 ...
                 if( n>1_ilp64 ) then
                    ! form p**t(2:n,2:n)
                    call stdlib_I64_dorglq( n-1, n-1, n-1, a( 2_ilp64, 2_ilp64 ), lda, tau, work,lwork, iinfo )
     ...
     !! if k >= nq, P = G(1) G(2) . . . G(nq-1).
        ...
        ! =====================================================================
           ...
           if( .not.applyq .and. .not.stdlib_lsame( vect, 'P' ) ) then
              ...
              info = -6_ilp64
           else if( ( applyq .and. lda<max( 1_ilp64, nq ) ) .or.( .not.applyq .and. lda<max( 1_ilp64, min( nq,&
                      k ) ) ) )then
              info = -8_ilp64
           ...
           if( info==0_ilp64 ) then
              if( m>0_ilp64 .and. n>0_ilp64 ) then
                 if( applyq ) then
                    if( left ) then
                    ...
                    if( left ) then
                       nb = stdlib_I64_ilaenv( 1_ilp64, 'CUNMLQ', side // trans, m-1, n, m-1,-1_ilp64 )
                    else
                       nb = stdlib_I64_ilaenv( 1_ilp64, 'CUNMLQ', side // trans, m, n-1, n-1,-1_ilp64 )
                    end if
                 end if
                 lwkopt = nw*nb
              else
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CUNMBR', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           if( applyq ) then
              ! apply q
              if( nq>=k ) then
                 ! q was determined by a call to stdlib_I64_cgebrd with nq >= k
                 call stdlib_I64_cunmqr( side, trans, m, n, k, a, lda, tau, c, ldc,work, lwork, iinfo &
                           )
              else if( nq>1_ilp64 ) then
                 ! q was determined by a call to stdlib_I64_cgebrd with nq < k
                 if( left ) then
                    mi = m - 1_ilp64
                    ni = n
                 ...
                 call stdlib_I64_cunmqr( side, trans, mi, ni, nq-1, a( 2_ilp64, 1_ilp64 ), lda, tau,c( i1, i2 ), &
                           ldc, work, lwork, iinfo )
              end if
           else
              ! apply p
              ...
              if( nq>k ) then
                 ! p was determined by a call to stdlib_I64_cgebrd with nq > k
                 call stdlib_I64_cunmlq( side, transt, m, n, k, a, lda, tau, c, ldc,work, lwork, &
                           iinfo )
              else if( nq>1_ilp64 ) then
                 ! p was determined by a call to stdlib_I64_cgebrd with nq <= k
                 if( left ) then
                 ...
                 call stdlib_I64_cunmlq( side, transt, mi, ni, nq-1, a( 1_ilp64, 2_ilp64 ), lda,tau, c( i1, i2 ), &
                           ldc, work, lwork, iinfo )
              end if
           end if
           work( 1_ilp64 ) = lwkopt
     ...
     !! if k >= nq, P = G(1) G(2) . . . G(nq-1).
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              if( m>0_ilp64 .and. n>0_ilp64 ) then
                 if( applyq ) then
                    ...
                    if( left ) then
                       nb = stdlib_I64_ilaenv( 1_ilp64, 'ZUNMLQ', side // trans, m-1, n, m-1,-1_ilp64 )
                    else
                       nb = stdlib_I64_ilaenv( 1_ilp64, 'ZUNMLQ', side // trans, m, n-1, n-1,-1_ilp64 )
                    end if
                 end if
                 lwkopt = nw*nb
              else
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZUNMBR', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           if( applyq ) then
              ! apply q
              if( nq>=k ) then
                 ! q was determined by a call to stdlib_I64_zgebrd with nq >= k
                 call stdlib_I64_zunmqr( side, trans, m, n, k, a, lda, tau, c, ldc,work, lwork, iinfo &
                           )
              else if( nq>1_ilp64 ) then
                 ! q was determined by a call to stdlib_I64_zgebrd with nq < k
                 if( left ) then
                 ...
                 call stdlib_I64_zunmqr( side, trans, mi, ni, nq-1, a( 2_ilp64, 1_ilp64 ), lda, tau,c( i1, i2 ), &
                           ldc, work, lwork, iinfo )
              end if
           else
              ! apply p
              ...
              if( nq>k ) then
                 ! p was determined by a call to stdlib_I64_zgebrd with nq > k
                 call stdlib_I64_zunmlq( side, transt, m, n, k, a, lda, tau, c, ldc,work, lwork, &
                           iinfo )
              else if( nq>1_ilp64 ) then
                 ! p was determined by a call to stdlib_I64_zgebrd with nq <= k
                 if( left ) then
                 ...
                 call stdlib_I64_zunmlq( side, transt, mi, ni, nq-1, a( 1_ilp64, 2_ilp64 ), lda,tau, c( i1, i2 ), &
                           ldc, work, lwork, iinfo )
              end if
           end if
           work( 1_ilp64 ) = lwkopt
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              if( applyq ) then
                 ...
                 if( left ) then
                    nb = stdlib_I64_ilaenv( 1_ilp64, 'SORMLQ', side // trans, m-1, n, m-1,-1_ilp64 )
                 else
                    nb = stdlib_I64_ilaenv( 1_ilp64, 'SORMLQ', side // trans, m, n-1, n-1,-1_ilp64 )
                 end if
              end if
              lwkopt = nw*nb
              work( 1_ilp64 ) = lwkopt
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SORMBR', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           if( applyq ) then
              ! apply q
              if( nq>=k ) then
                 ! q was determined by a call to stdlib_I64_sgebrd with nq >= k
                 call stdlib_I64_sormqr( side, trans, m, n, k, a, lda, tau, c, ldc,work, lwork, iinfo &
                           )
              else if( nq>1_ilp64 ) then
                 ! q was determined by a call to stdlib_I64_sgebrd with nq < k
                 if( left ) then
                 ...
                 call stdlib_I64_sormqr( side, trans, mi, ni, nq-1, a( 2_ilp64, 1_ilp64 ), lda, tau,c( i1, i2 ), &
                           ldc, work, lwork, iinfo )
              end if
           else
              ! apply p
              ...
              if( nq>k ) then
                 ! p was determined by a call to stdlib_I64_sgebrd with nq > k
                 call stdlib_I64_sormlq( side, transt, m, n, k, a, lda, tau, c, ldc,work, lwork, &
                           iinfo )
              else if( nq>1_ilp64 ) then
                 ! p was determined by a call to stdlib_I64_sgebrd with nq <= k
                 if( left ) then
                 ...
                 call stdlib_I64_sormlq( side, transt, mi, ni, nq-1, a( 1_ilp64, 2_ilp64 ), lda,tau, c( i1, i2 ), &
                           ldc, work, lwork, iinfo )
              end if
           end if
           work( 1_ilp64 ) = lwkopt
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              if( applyq ) then
                 ...
                 if( left ) then
                    nb = stdlib_I64_ilaenv( 1_ilp64, 'DORMLQ', side // trans, m-1, n, m-1,-1_ilp64 )
                 else
                    nb = stdlib_I64_ilaenv( 1_ilp64, 'DORMLQ', side // trans, m, n-1, n-1,-1_ilp64 )
                 end if
              end if
              lwkopt = nw*nb
              work( 1_ilp64 ) = lwkopt
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DORMBR', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           if( applyq ) then
              ! apply q
              if( nq>=k ) then
                 ! q was determined by a call to stdlib_I64_dgebrd with nq >= k
                 call stdlib_I64_dormqr( side, trans, m, n, k, a, lda, tau, c, ldc,work, lwork, iinfo &
                           )
              else if( nq>1_ilp64 ) then
                 ! q was determined by a call to stdlib_I64_dgebrd with nq < k
                 if( left ) then
                 ...
                 call stdlib_I64_dormqr( side, trans, mi, ni, nq-1, a( 2_ilp64, 1_ilp64 ), lda, tau,c( i1, i2 ), &
                           ldc, work, lwork, iinfo )
              end if
           else
              ! apply p
              ...
              if( nq>k ) then
                 ! p was determined by a call to stdlib_I64_dgebrd with nq > k
                 call stdlib_I64_dormlq( side, transt, m, n, k, a, lda, tau, c, ldc,work, lwork, &
                           iinfo )
              else if( nq>1_ilp64 ) then
                 ! p was determined by a call to stdlib_I64_dgebrd with nq <= k
                 if( left ) then
                 ...
                 call stdlib_I64_dormlq( side, transt, mi, ni, nq-1, a( 1_ilp64, 2_ilp64 ), lda,tau, c( i1, i2 ), &
                           ldc, work, lwork, iinfo )
              end if
           end if
           work( 1_ilp64 ) = lwkopt
