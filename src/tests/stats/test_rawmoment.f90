...
contains
    subroutine test_sp(x1, x2)
        ...
        call assert( abs(moment(x1, order, center = 0.) - mean(x1)) < sptol)
        call assert( abs(moment(x1, order, 1, center = 0.) - mean(x1)) < sptol)

        print*,' test_sp_1dim_mask', order
        call assert( ieee_is_nan(moment(x1, order, center = 0., mask = .false.)))
        call assert( ieee_is_nan(moment(x1, order, dim = 1, center = 0., mask = .false.)))

        print*,' test_sp_1dim_mask_array', order
        call assert( abs(moment(x1, order, center = 0., mask = (x1 < 5)) -&
                          mean(x1, mask = (x1 < 5)) ) < sptol)
        call assert( abs(moment(x1, order, dim = 1, center = 0., mask = (x1 < 5)) -&
        ...
        call assert( ieee_is_nan(moment(x2, order, center = 0., mask = .false.)))
        ...
        call assert( all( abs( moment(x3, order, dim = 2, center = zero3_2) -&
                               mean(x3, 2)) < sptol))
        call assert( all( abs( moment(x3, order, dim = 3, center = zero3_3) -&
                               mean(x3, 3)) < sptol))

        print*,' test_sp_3dim_mask', order
        call assert( ieee_is_nan(moment(x3, order, center = 0., mask = .false.)))
        ...
        call assert( any(ieee_is_nan(moment(x3, order, dim = 2, center = zero3_2,&
                         mask = .false.))))
        call assert( any(ieee_is_nan(moment(x3, order, dim = 3, center = zero3_3,&
                         mask = .false.))))
        ...
        !1dim
        print*,' test_sp_1dim', order
        call assert( abs(moment(x1, order, center = 0.) - mean(x1**2)) < sptol)
        call assert( abs(moment(x1, order, 1, center = 0.) - mean(x1**2, 1)) < sptol)
    ...
    subroutine test_int32(x1, x2)
        ...
        !1dim
        print*,' test_sp_1dim', order
        call assert( abs(moment(x1, order, center = 0._dp) - mean(x1)) < sptol)
        call assert( abs(moment(x1, order, 1, center = 0._dp) - mean(x1)) < sptol)

        print*,' test_sp_1dim_mask', order
        call assert( ieee_is_nan(moment(x1, order, center = 0._dp, mask = .false.)))
        call assert( ieee_is_nan(moment(x1, order, dim = 1, center = 0._dp,&
                               mask = .false.)))

        print*,' test_sp_1dim_mask_array', order
        call assert( abs(moment(x1, order, center = 0._dp, mask = (x1 < 5)) -&
                          mean(x1, mask = (x1 < 5)) ) < sptol)
        call assert( abs(moment(x1, order, dim = 1, center = 0._dp, mask = (x1 < 5)) -&
                          mean(x1, dim = 1, mask = (x1 < 5))) < sptol)
        ...
        !1dim
        print*,' test_sp_1dim', order
        call assert( abs(moment(x1, order, center = 0._dp) - mean(x1**2)) < sptol)
        call assert( abs(moment(x1, order, 1, center = 0._dp) - mean(x1**2, 1)) < sptol)
    ...
    subroutine test_csp(x1, x2)
        ...
        !1dim
        print*,' test_sp_1dim', order
        call assert( abs(moment(x1, order, center = (0., 0.)) - mean(x1)) < sptol)
        call assert( abs(moment(x1, order, 1, center = (0., 0.)) - mean(x1, 1)) < sptol)

        print*,' test_sp_1dim_mask', order
        call assert( ieee_is_nan(abs(moment(x1, order, center = (0., 0.),&
                               mask = .false.))))
        call assert( ieee_is_nan(abs(moment(x1, order, dim = 1, center = (0., 0.),&
                               mask = .false.))))

        print*,' test_sp_1dim_mask_array', order
        call assert( abs(moment(x1, order, center = (0., 0.), mask = (aimag(x1) == 0)) -&
                          mean(x1, aimag(x1) == 0)) < sptol)
        call assert( abs(moment(x1, order, dim = 1, center = (0., 0.),&
                          mask = (aimag(x1) == 0)) -&
        ...
        !1dim
        print*,' test_sp_1dim', order
        call assert( abs(moment(x1, order, center = (0., 0.)) - mean(x1**2)) < sptol)
        call assert( abs(moment(x1, order, 1, center = (0., 0.)) -&
                          mean(x1**2, 1)) < sptol)
