program test_distribution_uniform
	...
	a(:,:,:) = -1.0
	b(:,:,:) = 2.0
    x = reshape(uni_rvs(-1.0,2.0,60),[3,4,5]) ! uniform random variates array
    ...
    subroutine test_uni_rvs_0
      ...
      subroutine test_uni_rvs_csp
          ...
          res(11:15) = uni_rvs(loc, scale, k)     ! 3 dummies
          call check(all(abs(res%re - ans%re) < sptol) .and.                &
                     all(abs(res%im - ans%im) < sptol),                     &
      ...
      subroutine test_uni_rvs_cdp
          ...
          res(11:15) = uni_rvs(loc, scale, k)     ! 3 dummies
          call check(all(abs(res%re - ans%re) < dptol) .and.                &
                     all(abs(res%im - ans%im) < dptol),                     &
      ...
      subroutine test_uni_rvs_cqp
          ...
          res(11:15) = uni_rvs(loc, scale, k)     ! 3 dummies
          call check(all(abs(res%re - ans%re) < qptol) .and.                &
                     all(abs(res%im - ans%im) < qptol),                     &
