  ...
  contains
     ...
     !! real symmetric matrix A.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           logical(lk) :: lower, lquery, wantz
           integer(ilp64) :: iinfo, imax, inde, indtau, indwrk, iscale, llwork, lwkopt, nb
           real(sp) :: anrm, bignum, eps, rmax, rmin, safmin, sigma, smlnum
           ! Intrinsic Functions 
           ! Executable Statements 
           ! test the input parameters.
           ...
           if( info==0_ilp64 ) then
              ...
              lwkopt = max( 1_ilp64, ( nb+2 )*n )
              work( 1_ilp64 ) = lwkopt
              if( lwork<max( 1_ilp64, 3_ilp64*n-1 ) .and. .not.lquery )info = -8_ilp64
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SSYEV ', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           if( n==1_ilp64 ) then
              w( 1_ilp64 ) = a( 1_ilp64, 1_ilp64 )
              work( 1_ilp64 ) = 2_ilp64
              if( wantz )a( 1_ilp64, 1_ilp64 ) = one
              return
           end if
           ! get machine constants.
           ...
           rmax = sqrt( bignum )
           ! scale matrix to allowable range, if necessary.
           anrm = stdlib_I64_slansy( 'M', uplo, n, a, lda, work )
           iscale = 0_ilp64
           if( anrm>zero .and. anrm<rmin ) then
              iscale = 1_ilp64
              sigma = rmin / anrm
           ...
           if( iscale==1_ilp64 )call stdlib_I64_slascl( uplo, 0_ilp64, 0_ilp64, one, sigma, n, n, a, lda, info )
           ! call stdlib_I64_ssytrd to reduce symmetric matrix to tridiagonal form.
           inde = 1_ilp64
           indtau = inde + n
           indwrk = indtau + n
           llwork = lwork - indwrk + 1_ilp64
           call stdlib_I64_ssytrd( uplo, n, a, lda, w, work( inde ), work( indtau ),work( indwrk ), &
                     llwork, iinfo )
           ! for eigenvalues only, call stdlib_I64_ssterf.  for eigenvectors, first call
           ! stdlib_I64_sorgtr to generate the orthogonal matrix, then call stdlib_I64_ssteqr.
           if( .not.wantz ) then
              call stdlib_I64_ssterf( n, w, work( inde ), info )
           else
              call stdlib_I64_sorgtr( uplo, n, a, lda, work( indtau ), work( indwrk ),llwork, iinfo )
                        
              call stdlib_I64_ssteqr( jobz, n, w, work( inde ), a, lda, work( indtau ),info )
           end if
           ! if matrix was scaled, then rescale eigenvalues appropriately.
     ...
     !! real symmetric matrix A.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DSYEV ', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           rmax = sqrt( bignum )
           ! scale matrix to allowable range, if necessary.
           anrm = stdlib_I64_dlansy( 'M', uplo, n, a, lda, work )
           iscale = 0_ilp64
           if( anrm>zero .and. anrm<rmin ) then
              iscale = 1_ilp64
              sigma = rmin / anrm
           ...
           if( iscale==1_ilp64 )call stdlib_I64_dlascl( uplo, 0_ilp64, 0_ilp64, one, sigma, n, n, a, lda, info )
           ! call stdlib_I64_dsytrd to reduce symmetric matrix to tridiagonal form.
           inde = 1_ilp64
           indtau = inde + n
           indwrk = indtau + n
           llwork = lwork - indwrk + 1_ilp64
           call stdlib_I64_dsytrd( uplo, n, a, lda, w, work( inde ), work( indtau ),work( indwrk ), &
                     llwork, iinfo )
           ! for eigenvalues only, call stdlib_I64_dsterf.  for eigenvectors, first call
           ! stdlib_I64_dorgtr to generate the orthogonal matrix, then call stdlib_I64_dsteqr.
           if( .not.wantz ) then
              call stdlib_I64_dsterf( n, w, work( inde ), info )
           else
              call stdlib_I64_dorgtr( uplo, n, a, lda, work( indtau ), work( indwrk ),llwork, iinfo )
                        
              call stdlib_I64_dsteqr( jobz, n, w, work( inde ), a, lda, work( indtau ),info )
           end if
           ! if matrix was scaled, then rescale eigenvalues appropriately.
           if( iscale==1_ilp64 ) then
              if( info==0_ilp64 ) then
     ...
     !! workspace than SSYEVX.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           logical(lk) :: lower, lquery, wantz
           integer(ilp64) :: iinfo, inde, indtau, indwk2, indwrk, iscale, liopt, liwmin, llwork, &
                     llwrk2, lopt, lwmin
           real(sp) :: anrm, bignum, eps, rmax, rmin, safmin, sigma, smlnum
           ! Intrinsic Functions 
           ! Executable Statements 
           ...
           if( info==0_ilp64 ) then
              if( n<=1_ilp64 ) then
                 ...
                 lopt = max( lwmin, 2_ilp64*n +stdlib_I64_ilaenv( 1_ilp64, 'SSYTRD', uplo, n, -1_ilp64, -1_ilp64, -1_ilp64 ) )
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SSYEVD', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           call stdlib_I64_ssytrd( uplo, n, a, lda, w, work( inde ), work( indtau ),work( indwrk ), &
                     llwork, iinfo )
           ! for eigenvalues only, call stdlib_I64_ssterf.  for eigenvectors, first call
           ! stdlib_I64_sstedc to generate the eigenvector matrix, work(indwrk), of the
           ! tridiagonal matrix, then call stdlib_I64_sormtr to multiply it by the
           ! householder transformations stored in a.
           if( .not.wantz ) then
              call stdlib_I64_ssterf( n, w, work( inde ), info )
           else
              call stdlib_I64_sstedc( 'I', n, w, work( inde ), work( indwrk ), n,work( indwk2 ), &
                        llwrk2, iwork, liwork, info )
              call stdlib_I64_sormtr( 'L', uplo, 'N', n, n, a, lda, work( indtau ),work( indwrk ), n, &
                        work( indwk2 ), llwrk2, iinfo )
              call stdlib_I64_slacpy( 'A', n, n, work( indwrk ), n, a, lda )
           end if
           ! if matrix was scaled, then rescale eigenvalues appropriately.
           if( iscale==1_ilp64 )call stdlib_I64_sscal( n, one / sigma, w, 1_ilp64 )
           work( 1_ilp64 ) = lopt
           iwork( 1_ilp64 ) = liopt
           return
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              if( n<=1_ilp64 ) then
                 ...
                 lopt = max( lwmin, 2_ilp64*n +stdlib_I64_ilaenv( 1_ilp64, 'DSYTRD', uplo, n, -1_ilp64, -1_ilp64, -1_ilp64 ) )
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DSYEVD', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           call stdlib_I64_dsytrd( uplo, n, a, lda, w, work( inde ), work( indtau ),work( indwrk ), &
                     llwork, iinfo )
           ! for eigenvalues only, call stdlib_I64_dsterf.  for eigenvectors, first call
           ! stdlib_I64_dstedc to generate the eigenvector matrix, work(indwrk), of the
           ! tridiagonal matrix, then call stdlib_I64_dormtr to multiply it by the
           ! householder transformations stored in a.
           if( .not.wantz ) then
              call stdlib_I64_dsterf( n, w, work( inde ), info )
           else
              call stdlib_I64_dstedc( 'I', n, w, work( inde ), work( indwrk ), n,work( indwk2 ), &
                        llwrk2, iwork, liwork, info )
              call stdlib_I64_dormtr( 'L', uplo, 'N', n, n, a, lda, work( indtau ),work( indwrk ), n, &
                        work( indwk2 ), llwrk2, iinfo )
              call stdlib_I64_dlacpy( 'A', n, n, work( indwrk ), n, a, lda )
           end if
           ! if matrix was scaled, then rescale eigenvalues appropriately.
           if( iscale==1_ilp64 )call stdlib_I64_dscal( n, one / sigma, w, 1_ilp64 )
           work( 1_ilp64 ) = lopt
           iwork( 1_ilp64 ) = liopt
           return
     ...
     !! manner.
       ...
       ! =====================================================================
           ...
           logical(lk) :: alleig, indeig, lower, lquery, test, valeig, wantz, tryrac
           character :: order
           integer(ilp64) :: i, ieeeok, iinfo, imax, indd, inddd, inde, indee, indibl, indifl, &
           indisp, indiwo, indtau, indwk, indwkn, iscale, j, jj, liwmin, llwork, llwrkn, lwkopt, &
                     lwmin, nb, nsplit
           real(sp) :: abstll, anrm, bignum, eps, rmax, rmin, safmin, sigma, smlnum, tmp1, vll, &
                     vuu
           ...
           ! Executable Statements 
           ! test the input parameters.
           ieeeok = stdlib_I64_ilaenv( 10_ilp64, 'SSYEVR', 'N', 1_ilp64, 2_ilp64, 3_ilp64, 4_ilp64 )
           lower = stdlib_lsame( uplo, 'L' )
           wantz = stdlib_lsame( jobz, 'V' )
           alleig = stdlib_lsame( range, 'A' )
           valeig = stdlib_lsame( range, 'V' )
           ...
           if( .not.( wantz .or. stdlib_lsame( jobz, 'N' ) ) ) then
              ...
              if( valeig ) then
                 if( n>0_ilp64 .and. vu<=vl )info = -8_ilp64
              else if( indeig ) then
           ...
           if( info==0_ilp64 ) then
              nb = stdlib_I64_ilaenv( 1_ilp64, 'SSYTRD', uplo, n, -1_ilp64, -1_ilp64, -1_ilp64 )
              nb = max( nb, stdlib_I64_ilaenv( 1_ilp64, 'SORMTR', uplo, n, -1_ilp64, -1_ilp64, -1_ilp64 ) )
              lwkopt = max( ( nb+1 )*n, lwmin )
              work( 1_ilp64 ) = lwkopt
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SSYEVR', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           if( n==1_ilp64 ) then
              work( 1_ilp64 ) = 26_ilp64
              if( alleig .or. indeig ) then
                 m = 1_ilp64
                 w( 1_ilp64 ) = a( 1_ilp64, 1_ilp64 )
              else
                 if( vl<a( 1_ilp64, 1_ilp64 ) .and. vu>=a( 1_ilp64, 1_ilp64 ) ) then
                    m = 1_ilp64
                    w( 1_ilp64 ) = a( 1_ilp64, 1_ilp64 )
                 end if
              end if
           ...
           if( iscale==1_ilp64 ) then
              if( lower ) then
                 do j = 1, n
                    call stdlib_I64_sscal( n-j+1, sigma, a( j, j ), 1_ilp64 )
                 end do
              else
                 do j = 1, n
                    call stdlib_I64_sscal( j, sigma, a( 1_ilp64, j ), 1_ilp64 )
                 end do
              end if
              if( abstol>0_ilp64 )abstll = abstol*sigma
              if( valeig ) then
                 vll = vl*sigma
                 vuu = vu*sigma
              end if
           ...
           ! initialize indices into workspaces.  note: the iwork indices are
           ! used only if stdlib_I64_ssterf or stdlib_I64_sstemr fail.
           ! work(indtau:indtau+n-1) stores the scalar factors of the
           ! elementary reflectors used in stdlib_I64_ssytrd.
           indtau = 1_ilp64
           ! work(indd:indd+n-1) stores the tridiagonal's diagonal entries.
           indd = indtau + n
           ! work(inde:inde+n-1) stores the off-diagonal entries of the
           ! tridiagonal matrix from stdlib_I64_ssytrd.
           inde = indd + n
           ! work(inddd:inddd+n-1) is a copy of the diagonal entries over
           ! -written by stdlib_I64_sstemr (the stdlib_I64_ssterf path copies the diagonal to w).
           inddd = inde + n
           ! work(indee:indee+n-1) is a copy of the off-diagonal entries over
           ! -written while computing the eigenvalues in stdlib_I64_ssterf and stdlib_I64_sstemr.
           indee = inddd + n
           ! indwk is the starting offset of the left-over workspace, and
           ! llwork is the remaining workspace size.
           indwk = indee + n
           llwork = lwork - indwk + 1_ilp64
           ...
           indiwo = indifl + n
           ! call stdlib_I64_ssytrd to reduce symmetric matrix to tridiagonal form.
           call stdlib_I64_ssytrd( uplo, n, a, lda, work( indd ), work( inde ),work( indtau ), work( &
                     indwk ), llwork, iinfo )
           ! if all eigenvalues are desired
           ! then call stdlib_I64_ssterf or stdlib_I64_sstemr and stdlib_I64_sormtr.
           test = .false.
           if( indeig ) then
           ...
           if( ( alleig.or.test ) .and. ( ieeeok==1_ilp64 ) ) then
              if( .not.wantz ) then
                 ...
                 call stdlib_I64_scopy( n-1, work( inde ), 1_ilp64, work( indee ), 1_ilp64 )
                 call stdlib_I64_scopy( n, work( indd ), 1_ilp64, work( inddd ), 1_ilp64 )
                 if (abstol <= two*n*eps) then
                    tryrac = .true.
                 else
                    tryrac = .false.
                 end if
                 call stdlib_I64_sstemr( jobz, 'A', n, work( inddd ), work( indee ),vl, vu, il, iu, m,&
                            w, z, ldz, n, isuppz,tryrac, work( indwk ), lwork, iwork, liwork,info )
              ! apply orthogonal matrix used in reduction to tridiagonal
              ! form to eigenvectors returned by stdlib_I64_sstemr.
                 if( wantz .and. info==0_ilp64 ) then
                    indwkn = inde
                    llwrkn = lwork - indwkn + 1_ilp64
                    call stdlib_I64_sormtr( 'L', uplo, 'N', n, m, a, lda,work( indtau ), z, ldz, work(&
                               indwkn ),llwrkn, iinfo )
                 end if
              end if
              if( info==0_ilp64 ) then
                 ! everything worked.  skip stdlib_I64_sstebz/stdlib_I64_sstein.  iwork(:) are
                 ! undefined.
                 m = n
                 go to 30
              end if
           ...
           ! also call stdlib_I64_sstebz and stdlib_I64_sstein if stdlib_I64_sstemr fails.
           if( wantz ) then
              order = 'B'
           else
              order = 'E'
           end if
           call stdlib_I64_sstebz( range, order, n, vll, vuu, il, iu, abstll,work( indd ), work( inde &
           ), m, nsplit, w,iwork( indibl ), iwork( indisp ), work( indwk ),iwork( indiwo ), info )
           ...
           ! if matrix was scaled, then rescale eigenvalues appropriately.
        ! jump here if stdlib_I64_sstemr/stdlib_I64_sstein succeeded.
        30 continue
           if( iscale==1_ilp64 ) then
              if( info==0_ilp64 ) then
                 imax = m
           ...
           ! if eigenvalues are not in order, then sort them, along with
           ! eigenvectors.  note: we do not sort the ifail portion of iwork.
           ! it may not be initialized (if stdlib_I64_sstemr/stdlib_I64_sstein succeeded), and we do
           ! not return this detailed information to the user.
           if( wantz ) then
              do j = 1, m - 1
                 i = 0_ilp64
     ...
     !! manner.
       ...
       ! =====================================================================
           ...
           ! Executable Statements 
           ! test the input parameters.
           ieeeok = stdlib_I64_ilaenv( 10_ilp64, 'DSYEVR', 'N', 1_ilp64, 2_ilp64, 3_ilp64, 4_ilp64 )
           lower = stdlib_lsame( uplo, 'L' )
           wantz = stdlib_lsame( jobz, 'V' )
           alleig = stdlib_lsame( range, 'A' )
           valeig = stdlib_lsame( range, 'V' )
           ...
           if( info==0_ilp64 ) then
              nb = stdlib_I64_ilaenv( 1_ilp64, 'DSYTRD', uplo, n, -1_ilp64, -1_ilp64, -1_ilp64 )
              nb = max( nb, stdlib_I64_ilaenv( 1_ilp64, 'DORMTR', uplo, n, -1_ilp64, -1_ilp64, -1_ilp64 ) )
              lwkopt = max( ( nb+1 )*n, lwmin )
              work( 1_ilp64 ) = lwkopt
              iwork( 1_ilp64 ) = liwmin
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DSYEVR', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           if( iscale==1_ilp64 ) then
              if( lower ) then
                 do j = 1, n
                    call stdlib_I64_dscal( n-j+1, sigma, a( j, j ), 1_ilp64 )
                 end do
              else
                 do j = 1, n
                    call stdlib_I64_dscal( j, sigma, a( 1_ilp64, j ), 1_ilp64 )
                 end do
              end if
              if( abstol>0_ilp64 )abstll = abstol*sigma
              if( valeig ) then
           ...
           ! initialize indices into workspaces.  note: the iwork indices are
           ! used only if stdlib_I64_dsterf or stdlib_I64_dstemr fail.
           ! work(indtau:indtau+n-1) stores the scalar factors of the
           ! elementary reflectors used in stdlib_I64_dsytrd.
           indtau = 1_ilp64
           ! work(indd:indd+n-1) stores the tridiagonal's diagonal entries.
           indd = indtau + n
           ! work(inde:inde+n-1) stores the off-diagonal entries of the
           ! tridiagonal matrix from stdlib_I64_dsytrd.
           inde = indd + n
           ! work(inddd:inddd+n-1) is a copy of the diagonal entries over
           ! -written by stdlib_I64_dstemr (the stdlib_I64_dsterf path copies the diagonal to w).
           inddd = inde + n
           ! work(indee:indee+n-1) is a copy of the off-diagonal entries over
           ! -written while computing the eigenvalues in stdlib_I64_dsterf and stdlib_I64_dstemr.
           indee = inddd + n
           ! indwk is the starting offset of the left-over workspace, and
           ! llwork is the remaining workspace size.
           indwk = indee + n
           ...
           indiwo = indifl + n
           ! call stdlib_I64_dsytrd to reduce symmetric matrix to tridiagonal form.
           call stdlib_I64_dsytrd( uplo, n, a, lda, work( indd ), work( inde ),work( indtau ), work( &
                     indwk ), llwork, iinfo )
           ! if all eigenvalues are desired
           ! then call stdlib_I64_dsterf or stdlib_I64_dstemr and stdlib_I64_dormtr.
           if( ( alleig .or. ( indeig .and. il==1_ilp64 .and. iu==n ) ) .and.ieeeok==1_ilp64 ) then
              if( .not.wantz ) then
                 ...
                 call stdlib_I64_dcopy( n-1, work( inde ), 1_ilp64, work( indee ), 1_ilp64 )
                 call stdlib_I64_dcopy( n, work( indd ), 1_ilp64, work( inddd ), 1_ilp64 )
                 if (abstol <= two*n*eps) then
                    tryrac = .true.
                 else
                    tryrac = .false.
                 end if
                 call stdlib_I64_dstemr( jobz, 'A', n, work( inddd ), work( indee ),vl, vu, il, iu, m,&
                            w, z, ldz, n, isuppz,tryrac, work( indwk ), lwork, iwork, liwork,info )
              ! apply orthogonal matrix used in reduction to tridiagonal
              ! form to eigenvectors returned by stdlib_I64_dstemr.
                 if( wantz .and. info==0_ilp64 ) then
                    indwkn = inde
                    llwrkn = lwork - indwkn + 1_ilp64
                    call stdlib_I64_dormtr( 'L', uplo, 'N', n, m, a, lda,work( indtau ), z, ldz, work(&
                               indwkn ),llwrkn, iinfo )
                 end if
              end if
              if( info==0_ilp64 ) then
                 ! everything worked.  skip stdlib_I64_dstebz/stdlib_I64_dstein.  iwork(:) are
                 ! undefined.
                 m = n
                 go to 30
              end if
           ...
           ! also call stdlib_I64_dstebz and stdlib_I64_dstein if stdlib_I64_dstemr fails.
           if( wantz ) then
              order = 'B'
           else
              order = 'E'
           end if
           call stdlib_I64_dstebz( range, order, n, vll, vuu, il, iu, abstll,work( indd ), work( inde &
           ), m, nsplit, w,iwork( indibl ), iwork( indisp ), work( indwk ),iwork( indiwo ), info )
           ...
           ! if matrix was scaled, then rescale eigenvalues appropriately.
        ! jump here if stdlib_I64_dstemr/stdlib_I64_dstein succeeded.
        30 continue
           if( iscale==1_ilp64 ) then
              if( info==0_ilp64 ) then
                 imax = m
           ...
           ! if eigenvalues are not in order, then sort them, along with
           ! eigenvectors.  note: we do not sort the ifail portion of iwork.
           ! it may not be initialized (if stdlib_I64_dstemr/stdlib_I64_dstein succeeded), and we do
           ! not return this detailed information to the user.
           if( wantz ) then
              do j = 1, m - 1
                 i = 0_ilp64
     ...
     !! for the desired eigenvalues.
       ...
       ! =====================================================================
           ...
           logical(lk) :: alleig, indeig, lower, lquery, test, valeig, wantz
           character :: order
           integer(ilp64) :: i, iinfo, imax, indd, inde, indee, indibl, indisp, indiwo, indtau, &
           indwkn, indwrk, iscale, itmp1, j, jj, llwork, llwrkn, lwkmin, lwkopt, nb, &
                     nsplit
           real(sp) :: abstll, anrm, bignum, eps, rmax, rmin, safmin, sigma, smlnum, tmp1, vll, &
                     vuu
           ...
           if( info==0_ilp64 ) then
              ...
              if( lwork<lwkmin .and. .not.lquery )info = -17_ilp64
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SSYEVX', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           call stdlib_I64_ssytrd( uplo, n, a, lda, work( indd ), work( inde ),work( indtau ), work( &
                     indwrk ), llwork, iinfo )
           ! if all eigenvalues are desired and abstol is less than or equal to
           ! zero, then call stdlib_I64_ssterf or stdlib_I64_sorgtr and stdlib_I64_ssteqr.  if this fails for
           ! some eigenvalue, then try stdlib_I64_sstebz.
           test = .false.
           if( indeig ) then
              if( il==1_ilp64 .and. iu==n ) then
                 test = .true.
           ...
           if( ( alleig .or. test ) .and. ( abstol<=zero ) ) then
              ...
              if( .not.wantz ) then
                 ...
                 call stdlib_I64_slacpy( 'A', n, n, a, lda, z, ldz )
                 call stdlib_I64_sorgtr( uplo, n, z, ldz, work( indtau ),work( indwrk ), llwork, &
                           iinfo )
                 call stdlib_I64_scopy( n-1, work( inde ), 1_ilp64, work( indee ), 1_ilp64 )
     ...
     !! for the desired eigenvalues.
       ...
       ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DSYEVX', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           call stdlib_I64_dsytrd( uplo, n, a, lda, work( indd ), work( inde ),work( indtau ), work( &
                     indwrk ), llwork, iinfo )
           ! if all eigenvalues are desired and abstol is less than or equal to
           ! zero, then call stdlib_I64_dsterf or stdlib_I64_dorgtr and stdlib_I64_ssteqr.  if this fails for
           ! some eigenvalue, then try stdlib_I64_dstebz.
           test = .false.
           if( indeig ) then
              if( il==1_ilp64 .and. iu==n ) then
                 test = .true.
           ...
           if( ( alleig .or. test ) .and. ( abstol<=zero ) ) then
              ...
              if( .not.wantz ) then
                 ...
                 call stdlib_I64_dlacpy( 'A', n, n, a, lda, z, ldz )
                 call stdlib_I64_dorgtr( uplo, n, z, ldz, work( indtau ),work( indwrk ), llwork, &
                           iinfo )
                 call stdlib_I64_dcopy( n-1, work( inde ), 1_ilp64, work( indee ), 1_ilp64 )
     ...
     !! real symmetric matrix A in packed storage.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           logical(lk) :: wantz
           integer(ilp64) :: iinfo, imax, inde, indtau, indwrk, iscale
           real(sp) :: anrm, bignum, eps, rmax, rmin, safmin, sigma, smlnum
           ! Intrinsic Functions 
           ! Executable Statements 
           ! test the input parameters.
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SSPEV ', -info )
              return
           end if
           ! quick return if possible
           if( n==0 )return
           if( n==1_ilp64 ) then
              w( 1_ilp64 ) = ap( 1_ilp64 )
              if( wantz )z( 1_ilp64, 1_ilp64 ) = one
              return
           end if
           ! get machine constants.
           ...
           rmax = sqrt( bignum )
           ! scale matrix to allowable range, if necessary.
           anrm = stdlib_I64_slansp( 'M', uplo, n, ap, work )
           iscale = 0_ilp64
           if( anrm>zero .and. anrm<rmin ) then
              iscale = 1_ilp64
              sigma = rmin / anrm
           ...
           if( iscale==1_ilp64 ) then
              call stdlib_I64_sscal( ( n*( n+1 ) ) / 2_ilp64, sigma, ap, 1_ilp64 )
           end if
           ! call stdlib_I64_ssptrd to reduce symmetric packed matrix to tridiagonal form.
           inde = 1_ilp64
           indtau = inde + n
           call stdlib_I64_ssptrd( uplo, n, ap, w, work( inde ), work( indtau ), iinfo )
           ! for eigenvalues only, call stdlib_I64_ssterf.  for eigenvectors, first call
           ! stdlib_I64_sopgtr to generate the orthogonal matrix, then call stdlib_I64_ssteqr.
           if( .not.wantz ) then
              call stdlib_I64_ssterf( n, w, work( inde ), info )
           else
              indwrk = indtau + n
              call stdlib_I64_sopgtr( uplo, n, ap, work( indtau ), z, ldz,work( indwrk ), iinfo )
                        
              call stdlib_I64_ssteqr( jobz, n, w, work( inde ), z, ldz, work( indtau ),info )
           end if
           ! if matrix was scaled, then rescale eigenvalues appropriately.
           if( iscale==1_ilp64 ) then
              if( info==0_ilp64 ) then
     ...
     !! real symmetric matrix A in packed storage.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DSPEV ', -info )
              return
           end if
           ! quick return if possible
           if( n==0 )return
           ...
           rmax = sqrt( bignum )
           ! scale matrix to allowable range, if necessary.
           anrm = stdlib_I64_dlansp( 'M', uplo, n, ap, work )
           iscale = 0_ilp64
           if( anrm>zero .and. anrm<rmin ) then
              iscale = 1_ilp64
              sigma = rmin / anrm
           ...
           if( iscale==1_ilp64 ) then
              call stdlib_I64_dscal( ( n*( n+1 ) ) / 2_ilp64, sigma, ap, 1_ilp64 )
           end if
           ! call stdlib_I64_dsptrd to reduce symmetric packed matrix to tridiagonal form.
           inde = 1_ilp64
           indtau = inde + n
           call stdlib_I64_dsptrd( uplo, n, ap, w, work( inde ), work( indtau ), iinfo )
           ! for eigenvalues only, call stdlib_I64_dsterf.  for eigenvectors, first call
           ! stdlib_I64_dopgtr to generate the orthogonal matrix, then call stdlib_I64_dsteqr.
           if( .not.wantz ) then
              call stdlib_I64_dsterf( n, w, work( inde ), info )
           else
              indwrk = indtau + n
              call stdlib_I64_dopgtr( uplo, n, ap, work( indtau ), z, ldz,work( indwrk ), iinfo )
                        
              call stdlib_I64_dsteqr( jobz, n, w, work( inde ), z, ldz, work( indtau ),info )
           end if
           ! if matrix was scaled, then rescale eigenvalues appropriately.
           if( iscale==1_ilp64 ) then
              if( info==0_ilp64 ) then
     ...
     !! without guard digits, but we know of none.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           logical(lk) :: lquery, wantz
           integer(ilp64) :: iinfo, inde, indtau, indwrk, iscale, liwmin, llwork, lwmin
           real(sp) :: anrm, bignum, eps, rmax, rmin, safmin, sigma, smlnum
           ! Intrinsic Functions 
           ! Executable Statements 
           ! test the input parameters.
           ...
           if( info==0_ilp64 ) then
              if( n<=1_ilp64 ) then
                 ...
                 if( wantz ) then
                    liwmin = 3_ilp64 + 5_ilp64*n
                    lwmin = 1_ilp64 + 6_ilp64*n + n**2_ilp64
                 else
                    liwmin = 1_ilp64
                    lwmin = 2_ilp64*n
                 end if
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SSPEVD', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           ! for eigenvalues only, call stdlib_I64_ssterf.  for eigenvectors, first call
           ! stdlib_I64_sstedc to generate the eigenvector matrix, work(indwrk), of the
           ! tridiagonal matrix, then call stdlib_I64_sopmtr to multiply it by the
           ! householder transformations represented in ap.
           if( .not.wantz ) then
              call stdlib_I64_ssterf( n, w, work( inde ), info )
           else
              indwrk = indtau + n
              llwork = lwork - indwrk + 1_ilp64
              call stdlib_I64_sstedc( 'I', n, w, work( inde ), z, ldz, work( indwrk ),llwork, iwork, &
                        liwork, info )
              call stdlib_I64_sopmtr( 'L', uplo, 'N', n, n, ap, work( indtau ), z, ldz,work( indwrk ),&
                         iinfo )
           end if
           ! if matrix was scaled, then rescale eigenvalues appropriately.
           if( iscale==1_ilp64 )call stdlib_I64_sscal( n, one / sigma, w, 1_ilp64 )
     ...
     !! without guard digits, but we know of none.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DSPEVD', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           ! for eigenvalues only, call stdlib_I64_dsterf.  for eigenvectors, first call
           ! stdlib_I64_dstedc to generate the eigenvector matrix, work(indwrk), of the
           ! tridiagonal matrix, then call stdlib_I64_dopmtr to multiply it by the
           ! householder transformations represented in ap.
           if( .not.wantz ) then
              call stdlib_I64_dsterf( n, w, work( inde ), info )
           else
              indwrk = indtau + n
              llwork = lwork - indwrk + 1_ilp64
              call stdlib_I64_dstedc( 'I', n, w, work( inde ), z, ldz, work( indwrk ),llwork, iwork, &
                        liwork, info )
              call stdlib_I64_dopmtr( 'L', uplo, 'N', n, n, ap, work( indtau ), z, ldz,work( indwrk ),&
                         iinfo )
           end if
           ! if matrix was scaled, then rescale eigenvalues appropriately.
           if( iscale==1_ilp64 )call stdlib_I64_dscal( n, one / sigma, w, 1_ilp64 )
     ...
     !! indices for the desired eigenvalues.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SSPEVX', -info )
              return
           end if
           ! quick return if possible
           m = 0_ilp64
           ...
           if( n==1_ilp64 ) then
              if( alleig .or. indeig ) then
                 ...
                 if( vl<ap( 1_ilp64 ) .and. vu>=ap( 1_ilp64 ) ) then
                    m = 1_ilp64
                    w( 1_ilp64 ) = ap( 1_ilp64 )
                 end if
              end if
           ...
           indd = inde + n
           indwrk = indd + n
           call stdlib_I64_ssptrd( uplo, n, ap, work( indd ), work( inde ),work( indtau ), iinfo )
                     
           ! if all eigenvalues are desired and abstol is less than or equal
           ! to zero, then call stdlib_I64_ssterf or stdlib_I64_sopgtr and stdlib_I64_ssteqr.  if this fails
           ! for some eigenvalue, then try stdlib_I64_sstebz.
           test = .false.
           if (indeig) then
              if (il==1_ilp64 .and. iu==n) then
                 test = .true.
           ...
           if( wantz ) then
              ...
              ! apply orthogonal matrix used in reduction to tridiagonal
              ! form to eigenvectors returned by stdlib_I64_sstein.
              call stdlib_I64_sopmtr( 'L', uplo, 'N', n, m, ap, work( indtau ), z, ldz,work( indwrk ),&
                         iinfo )
           end if
           ! if matrix was scaled, then rescale eigenvalues appropriately.
           20 continue
     ...
     !! indices for the desired eigenvalues.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DSPEVX', -info )
              return
           end if
           ! quick return if possible
           m = 0_ilp64
           ...
           indd = inde + n
           indwrk = indd + n
           call stdlib_I64_dsptrd( uplo, n, ap, work( indd ), work( inde ),work( indtau ), iinfo )
                     
           ! if all eigenvalues are desired and abstol is less than or equal
           ! to zero, then call stdlib_I64_dsterf or stdlib_I64_dopgtr and stdlib_I64_ssteqr.  if this fails
           ! for some eigenvalue, then try stdlib_I64_dstebz.
           test = .false.
           if (indeig) then
              if (il==1_ilp64 .and. iu==n) then
                 test = .true.
           ...
           if( wantz ) then
              ...
              ! apply orthogonal matrix used in reduction to tridiagonal
              ! form to eigenvectors returned by stdlib_I64_dstein.
              call stdlib_I64_dopmtr( 'L', uplo, 'N', n, m, ap, work( indtau ), z, ldz,work( indwrk ),&
                         iinfo )
           end if
           ! if matrix was scaled, then rescale eigenvalues appropriately.
           20 continue
     ...
     !! a real symmetric band matrix A.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           logical(lk) :: lower, wantz
           integer(ilp64) :: iinfo, imax, inde, indwrk, iscale
           real(sp) :: anrm, bignum, eps, rmax, rmin, safmin, sigma, smlnum
           ! Intrinsic Functions 
           ! Executable Statements 
           ! test the input parameters.
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SSBEV ', -info )
              return
           end if
           ! quick return if possible
           if( n==0 )return
           if( n==1_ilp64 ) then
              if( lower ) then
                 w( 1_ilp64 ) = ab( 1_ilp64, 1_ilp64 )
              else
                 w( 1_ilp64 ) = ab( kd+1, 1_ilp64 )
              end if
              if( wantz )z( 1_ilp64, 1_ilp64 ) = one
              return
           end if
           ...
           rmax = sqrt( bignum )
           ! scale matrix to allowable range, if necessary.
           anrm = stdlib_I64_slansb( 'M', uplo, n, kd, ab, ldab, work )
           iscale = 0_ilp64
           if( anrm>zero .and. anrm<rmin ) then
              iscale = 1_ilp64
              sigma = rmin / anrm
           ...
           if( iscale==1_ilp64 ) then
              if( lower ) then
                 call stdlib_I64_slascl( 'B', kd, kd, one, sigma, n, n, ab, ldab, info )
              else
                 call stdlib_I64_slascl( 'Q', kd, kd, one, sigma, n, n, ab, ldab, info )
              end if
           end if
           ! call stdlib_I64_ssbtrd to reduce symmetric band matrix to tridiagonal form.
           inde = 1_ilp64
           indwrk = inde + n
           call stdlib_I64_ssbtrd( jobz, uplo, n, kd, ab, ldab, w, work( inde ), z, ldz,work( indwrk )&
                     , iinfo )
     ...
     !! a real symmetric band matrix A.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DSBEV ', -info )
              return
           end if
           ! quick return if possible
           if( n==0 )return
           ...
           rmax = sqrt( bignum )
           ! scale matrix to allowable range, if necessary.
           anrm = stdlib_I64_dlansb( 'M', uplo, n, kd, ab, ldab, work )
           iscale = 0_ilp64
           if( anrm>zero .and. anrm<rmin ) then
              iscale = 1_ilp64
              sigma = rmin / anrm
           ...
           if( iscale==1_ilp64 ) then
              if( lower ) then
                 call stdlib_I64_dlascl( 'B', kd, kd, one, sigma, n, n, ab, ldab, info )
              else
                 call stdlib_I64_dlascl( 'Q', kd, kd, one, sigma, n, n, ab, ldab, info )
              end if
           end if
           ! call stdlib_I64_dsbtrd to reduce symmetric band matrix to tridiagonal form.
           inde = 1_ilp64
           indwrk = inde + n
           call stdlib_I64_dsbtrd( jobz, uplo, n, kd, ab, ldab, w, work( inde ), z, ldz,work( indwrk )&
                     , iinfo )
     ...
     !! without guard digits, but we know of none.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           logical(lk) :: lower, lquery, wantz
           integer(ilp64) :: iinfo, inde, indwk2, indwrk, iscale, liwmin, llwrk2, lwmin
           real(sp) :: anrm, bignum, eps, rmax, rmin, safmin, sigma, smlnum
           ! Intrinsic Functions 
           ! Executable Statements 
           ! test the input parameters.
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SSBEVD', -info )
              return
           else if( lquery ) then
              return
           end if
     ...
     !! without guard digits, but we know of none.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DSBEVD', -info )
              return
           else if( lquery ) then
              return
           end if
     ...
     !! indices for the desired eigenvalues.
        ...
        ! =====================================================================
           ...
           logical(lk) :: alleig, indeig, lower, test, valeig, wantz
           character :: order
           integer(ilp64) :: i, iinfo, imax, indd, inde, indee, indibl, indisp, indiwo, indwrk, &
                     iscale, itmp1, j, jj, nsplit
           real(sp) :: abstll, anrm, bignum, eps, rmax, rmin, safmin, sigma, smlnum, tmp1, vll, &
                     vuu
           ! Intrinsic Functions 
           ...
           if( .not.( wantz .or. stdlib_lsame( jobz, 'N' ) ) ) then
              ...
              info = -7_ilp64
           else if( wantz .and. ldq<max( 1_ilp64, n ) ) then
              info = -9_ilp64
           else
              if( valeig ) then
           ...
           if( info==0_ilp64 ) then
              if( ldz<1_ilp64 .or. ( wantz .and. ldz<n ) )info = -18_ilp64
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SSBEVX', -info )
              return
           end if
           ! quick return if possible
           m = 0_ilp64
           ...
           if( n==1_ilp64 ) then
              m = 1_ilp64
              if( lower ) then
                 tmp1 = ab( 1_ilp64, 1_ilp64 )
              else
                 tmp1 = ab( kd+1, 1_ilp64 )
              end if
              if( valeig ) then
                 if( .not.( vl<tmp1 .and. vu>=tmp1 ) )m = 0_ilp64
              end if
              if( m==1_ilp64 ) then
                 w( 1_ilp64 ) = tmp1
                 if( wantz )z( 1_ilp64, 1_ilp64 ) = one
           ...
           inde = indd + n
           indwrk = inde + n
           call stdlib_I64_ssbtrd( jobz, uplo, n, kd, ab, ldab, work( indd ),work( inde ), q, ldq, &
                     work( indwrk ), iinfo )
           ! if all eigenvalues are desired and abstol is less than or equal
     ...
     !! indices for the desired eigenvalues.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DSBEVX', -info )
              return
           end if
           ! quick return if possible
           m = 0_ilp64
           ...
           inde = indd + n
           indwrk = inde + n
           call stdlib_I64_dsbtrd( jobz, uplo, n, kd, ab, ldab, work( indd ),work( inde ), q, ldq, &
                     work( indwrk ), iinfo )
           ! if all eigenvalues are desired and abstol is less than or equal
     ...
     !! complex Hermitian matrix A.
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              ...
              lwkopt = max( 1_ilp64, ( nb+1 )*n )
              work( 1_ilp64 ) = lwkopt
              if( lwork<max( 1_ilp64, 2_ilp64*n-1 ) .and. .not.lquery )info = -8_ilp64
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CHEEV ', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           if( n==1_ilp64 ) then
              w( 1_ilp64 ) = real( a( 1_ilp64, 1_ilp64 ),KIND=sp)
              work( 1_ilp64 ) = 1_ilp64
              if( wantz )a( 1_ilp64, 1_ilp64 ) = cone
              return
           end if
           ! get machine constants.
           safmin = stdlib_I64_slamch( 'SAFE MINIMUM' )
           ...
           rmax = sqrt( bignum )
           ! scale matrix to allowable range, if necessary.
           anrm = stdlib_I64_clanhe( 'M', uplo, n, a, lda, rwork )
           iscale = 0_ilp64
           if( anrm>zero .and. anrm<rmin ) then
              iscale = 1_ilp64
              sigma = rmin / anrm
           ...
           if( iscale==1_ilp64 )call stdlib_I64_clascl( uplo, 0_ilp64, 0_ilp64, one, sigma, n, n, a, lda, info )
           ! call stdlib_I64_chetrd to reduce hermitian matrix to tridiagonal form.
           inde = 1_ilp64
           indtau = 1_ilp64
           indwrk = indtau + n
           llwork = lwork - indwrk + 1_ilp64
           call stdlib_I64_chetrd( uplo, n, a, lda, w, rwork( inde ), work( indtau ),work( indwrk ), &
                     llwork, iinfo )
           ! for eigenvalues only, call stdlib_I64_ssterf.  for eigenvectors, first call
           ! stdlib_I64_cungtr to generate the unitary matrix, then call stdlib_I64_csteqr.
           if( .not.wantz ) then
              call stdlib_I64_ssterf( n, w, rwork( inde ), info )
           else
              call stdlib_I64_cungtr( uplo, n, a, lda, work( indtau ), work( indwrk ),llwork, iinfo )
                        
              indwrk = inde + n
              call stdlib_I64_csteqr( jobz, n, w, rwork( inde ), a, lda,rwork( indwrk ), info )
     ...
     !! complex Hermitian matrix A.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZHEEV ', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           if( n==1_ilp64 ) then
              w( 1_ilp64 ) = real( a( 1_ilp64, 1_ilp64 ),KIND=dp)
              work( 1_ilp64 ) = 1_ilp64
              if( wantz )a( 1_ilp64, 1_ilp64 ) = cone
              return
           end if
           ...
           rmax = sqrt( bignum )
           ! scale matrix to allowable range, if necessary.
           anrm = stdlib_I64_zlanhe( 'M', uplo, n, a, lda, rwork )
           iscale = 0_ilp64
           if( anrm>zero .and. anrm<rmin ) then
              iscale = 1_ilp64
              sigma = rmin / anrm
           ...
           if( iscale==1_ilp64 )call stdlib_I64_zlascl( uplo, 0_ilp64, 0_ilp64, one, sigma, n, n, a, lda, info )
           ! call stdlib_I64_zhetrd to reduce hermitian matrix to tridiagonal form.
           inde = 1_ilp64
           indtau = 1_ilp64
           indwrk = indtau + n
           llwork = lwork - indwrk + 1_ilp64
           call stdlib_I64_zhetrd( uplo, n, a, lda, w, rwork( inde ), work( indtau ),work( indwrk ), &
                     llwork, iinfo )
           ! for eigenvalues only, call stdlib_I64_dsterf.  for eigenvectors, first call
           ! stdlib_I64_zungtr to generate the unitary matrix, then call stdlib_I64_zsteqr.
           if( .not.wantz ) then
              call stdlib_I64_dsterf( n, w, rwork( inde ), info )
           else
              call stdlib_I64_zungtr( uplo, n, a, lda, work( indtau ), work( indwrk ),llwork, iinfo )
                        
              indwrk = inde + n
              call stdlib_I64_zsteqr( jobz, n, w, rwork( inde ), a, lda,rwork( indwrk ), info )
     ...
     !! without guard digits, but we know of none.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           logical(lk) :: lower, lquery, wantz
           integer(ilp64) :: iinfo, imax, inde, indrwk, indtau, indwk2, indwrk, iscale, liopt, &
                     liwmin, llrwk, llwork, llwrk2, lopt, lropt, lrwmin, lwmin
           real(sp) :: anrm, bignum, eps, rmax, rmin, safmin, sigma, smlnum
           ! Intrinsic Functions 
           ! Executable Statements 
           ...
           if( info==0_ilp64 ) then
              if( n<=1_ilp64 ) then
                 ...
                 lopt = max( lwmin, n +stdlib_I64_ilaenv( 1_ilp64, 'CHETRD', uplo, n, -1_ilp64, -1_ilp64, -1_ilp64 ) )
                 lropt = lrwmin
                 liopt = liwmin
              end if
              work( 1_ilp64 ) = lopt
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CHEEVD', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           llwork = lwork - indwrk + 1_ilp64
           llwrk2 = lwork - indwk2 + 1_ilp64
           llrwk = lrwork - indrwk + 1_ilp64
           call stdlib_I64_chetrd( uplo, n, a, lda, w, rwork( inde ), work( indtau ),work( indwrk ), &
                     llwork, iinfo )
           ! for eigenvalues only, call stdlib_I64_ssterf.  for eigenvectors, first call
           ! stdlib_I64_cstedc to generate the eigenvector matrix, work(indwrk), of the
           ! tridiagonal matrix, then call stdlib_I64_cunmtr to multiply it to the
           ! householder transformations represented as householder vectors in
           ! a.
           if( .not.wantz ) then
              call stdlib_I64_ssterf( n, w, rwork( inde ), info )
           else
              call stdlib_I64_cstedc( 'I', n, w, rwork( inde ), work( indwrk ), n,work( indwk2 ), &
                        llwrk2, rwork( indrwk ), llrwk,iwork, liwork, info )
              call stdlib_I64_cunmtr( 'L', uplo, 'N', n, n, a, lda, work( indtau ),work( indwrk ), n, &
                        work( indwk2 ), llwrk2, iinfo )
              call stdlib_I64_clacpy( 'A', n, n, work( indwrk ), n, a, lda )
           end if
           ! if matrix was scaled, then rescale eigenvalues appropriately.
           if( iscale==1_ilp64 ) then
              if( info==0_ilp64 ) then
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              if( n<=1_ilp64 ) then
                 ...
                 lopt = max( lwmin, n +stdlib_I64_ilaenv( 1_ilp64, 'ZHETRD', uplo, n, -1_ilp64, -1_ilp64, -1_ilp64 ) )
                 lropt = lrwmin
                 liopt = liwmin
              end if
              work( 1_ilp64 ) = lopt
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZHEEVD', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           call stdlib_I64_zhetrd( uplo, n, a, lda, w, rwork( inde ), work( indtau ),work( indwrk ), &
                     llwork, iinfo )
           ! for eigenvalues only, call stdlib_I64_dsterf.  for eigenvectors, first call
           ! stdlib_I64_zstedc to generate the eigenvector matrix, work(indwrk), of the
           ! tridiagonal matrix, then call stdlib_I64_zunmtr to multiply it to the
           ! householder transformations represented as householder vectors in
           ! a.
           if( .not.wantz ) then
              call stdlib_I64_dsterf( n, w, rwork( inde ), info )
           else
              call stdlib_I64_zstedc( 'I', n, w, rwork( inde ), work( indwrk ), n,work( indwk2 ), &
                        llwrk2, rwork( indrwk ), llrwk,iwork, liwork, info )
              call stdlib_I64_zunmtr( 'L', uplo, 'N', n, n, a, lda, work( indtau ),work( indwrk ), n, &
                        work( indwk2 ), llwrk2, iinfo )
              call stdlib_I64_zlacpy( 'A', n, n, work( indwrk ), n, a, lda )
           end if
           ! if matrix was scaled, then rescale eigenvalues appropriately.
           if( iscale==1_ilp64 ) then
              if( info==0_ilp64 ) then
     ...
     !! manner.
       ...
       ! =====================================================================
           ...
           logical(lk) :: alleig, indeig, lower, lquery, test, valeig, wantz, tryrac
           character :: order
           integer(ilp64) :: i, ieeeok, iinfo, imax, indibl, indifl, indisp, indiwo, indrd, indrdd, &
           indre, indree, indrwk, indtau, indwk, indwkn, iscale, itmp1, j, jj, liwmin, llwork, &
                     llrwork, llwrkn, lrwmin, lwkopt, lwmin, nb, nsplit
           real(sp) :: abstll, anrm, bignum, eps, rmax, rmin, safmin, sigma, smlnum, tmp1, vll, &
                     vuu
           ...
           ! Executable Statements 
           ! test the input parameters.
           ieeeok = stdlib_I64_ilaenv( 10_ilp64, 'CHEEVR', 'N', 1_ilp64, 2_ilp64, 3_ilp64, 4_ilp64 )
           lower = stdlib_lsame( uplo, 'L' )
           wantz = stdlib_lsame( jobz, 'V' )
           alleig = stdlib_lsame( range, 'A' )
           valeig = stdlib_lsame( range, 'V' )
           indeig = stdlib_lsame( range, 'I' )
           lquery = ( ( lwork==-1_ilp64 ) .or. ( lrwork==-1_ilp64 ) .or.( liwork==-1_ilp64 ) )
           lrwmin = max( 1_ilp64, 24_ilp64*n )
           liwmin = max( 1_ilp64, 10_ilp64*n )
           lwmin = max( 1_ilp64, 2_ilp64*n )
           info = 0_ilp64
           if( .not.( wantz .or. stdlib_lsame( jobz, 'N' ) ) ) then
              info = -1_ilp64
           else if( .not.( alleig .or. valeig .or. indeig ) ) then
           ...
           if( info==0_ilp64 ) then
              nb = stdlib_I64_ilaenv( 1_ilp64, 'CHETRD', uplo, n, -1_ilp64, -1_ilp64, -1_ilp64 )
              nb = max( nb, stdlib_I64_ilaenv( 1_ilp64, 'CUNMTR', uplo, n, -1_ilp64, -1_ilp64, -1_ilp64 ) )
              lwkopt = max( ( nb+1 )*n, lwmin )
              work( 1_ilp64 ) = lwkopt
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CHEEVR', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           if( n==1_ilp64 ) then
              ...
              if( alleig .or. indeig ) then
                 ...
                 if( vl<real( a( 1_ilp64, 1_ilp64 ),KIND=sp) .and. vu>=real( a( 1_ilp64, 1_ilp64 ),KIND=sp) )then
                    m = 1_ilp64
                    w( 1_ilp64 ) = real( a( 1_ilp64, 1_ilp64 ),KIND=sp)
                 end if
              end if
           ...
           anrm = stdlib_I64_clansy( 'M', uplo, n, a, lda, rwork )
           if( anrm>zero .and. anrm<rmin ) then
              iscale = 1_ilp64
              sigma = rmin / anrm
           else if( anrm>rmax ) then
           ...
           if( iscale==1_ilp64 ) then
              if( lower ) then
                 do j = 1, n
                    call stdlib_I64_csscal( n-j+1, sigma, a( j, j ), 1_ilp64 )
                 end do
              else
                 do j = 1, n
                    call stdlib_I64_csscal( j, sigma, a( 1_ilp64, j ), 1_ilp64 )
                 end do
              end if
              if( abstol>0_ilp64 )abstll = abstol*sigma
              if( valeig ) then
           ...
           ! initialize indices into workspaces.  note: the iwork indices are
           ! used only if stdlib_I64_ssterf or stdlib_I64_cstemr fail.
           ! work(indtau:indtau+n-1) stores the complex scalar factors of the
           ! elementary reflectors used in stdlib_I64_chetrd.
           indtau = 1_ilp64
           ! indwk is the starting offset of the remaining complex workspace,
           ! and llwork is the remaining complex workspace size.
           indwk = indtau + n
           ...
           indrd = 1_ilp64
           ! rwork(indre:indre+n-1) stores the off-diagonal entries of the
           ! tridiagonal matrix from stdlib_I64_chetrd.
           indre = indrd + n
           ! rwork(indrdd:indrdd+n-1) is a copy of the diagonal entries over
           ! -written by stdlib_I64_cstemr (the stdlib_I64_ssterf path copies the diagonal to w).
           indrdd = indre + n
           ! rwork(indree:indree+n-1) is a copy of the off-diagonal entries over
           ! -written while computing the eigenvalues in stdlib_I64_ssterf and stdlib_I64_cstemr.
           indree = indrdd + n
           ! indrwk is the starting offset of the left-over real workspace, and
           ! llrwork is the remaining workspace size.
           indrwk = indree + n
           llrwork = lrwork - indrwk + 1_ilp64
           ! iwork(indibl:indibl+m-1) corresponds to iblock in stdlib_I64_sstebz and
           ! stores the block indices of each of the m<=n eigenvalues.
           indibl = 1_ilp64
           ! iwork(indisp:indisp+nsplit-1) corresponds to isplit in stdlib_I64_sstebz and
           ...
           indiwo = indifl + n
           ! call stdlib_I64_chetrd to reduce hermitian matrix to tridiagonal form.
           call stdlib_I64_chetrd( uplo, n, a, lda, rwork( indrd ), rwork( indre ),work( indtau ), &
                     work( indwk ), llwork, iinfo )
           ! if all eigenvalues are desired
           ! then call stdlib_I64_ssterf or stdlib_I64_cstemr and stdlib_I64_cunmtr.
           test = .false.
           if( indeig ) then
              if( il==1_ilp64 .and. iu==n ) then
                 test = .true.
           ...
           if( ( alleig.or.test ) .and. ( ieeeok==1_ilp64 ) ) then
              if( .not.wantz ) then
                 call stdlib_I64_scopy( n, rwork( indrd ), 1_ilp64, w, 1_ilp64 )
                 call stdlib_I64_scopy( n-1, rwork( indre ), 1_ilp64, rwork( indree ), 1_ilp64 )
                 call stdlib_I64_ssterf( n, w, rwork( indree ), info )
              else
                 call stdlib_I64_scopy( n-1, rwork( indre ), 1_ilp64, rwork( indree ), 1_ilp64 )
                 call stdlib_I64_scopy( n, rwork( indrd ), 1_ilp64, rwork( indrdd ), 1_ilp64 )
                 if (abstol <= two*n*eps) then
                    tryrac = .true.
                 else
                    tryrac = .false.
                 end if
                 call stdlib_I64_cstemr( jobz, 'A', n, rwork( indrdd ),rwork( indree ), vl, vu, il, &
                 iu, m, w,z, ldz, n, isuppz, tryrac,rwork( indrwk ), llrwork,iwork, liwork, info )
                           
                 ! apply unitary matrix used in reduction to tridiagonal
                 ! form to eigenvectors returned by stdlib_I64_cstemr.
                 if( wantz .and. info==0_ilp64 ) then
                    indwkn = indwk
                    llwrkn = lwork - indwkn + 1_ilp64
                    call stdlib_I64_cunmtr( 'L', uplo, 'N', n, m, a, lda,work( indtau ), z, ldz, work(&
                               indwkn ),llwrkn, iinfo )
                 end if
              end if
              if( info==0_ilp64 ) then
           ...
           ! otherwise, call stdlib_I64_sstebz and, if eigenvectors are desired, stdlib_I64_cstein.
           ! also call stdlib_I64_sstebz and stdlib_I64_cstein if stdlib_I64_cstemr fails.
           if( wantz ) then
              order = 'B'
           else
              order = 'E'
           end if
           call stdlib_I64_sstebz( range, order, n, vll, vuu, il, iu, abstll,rwork( indrd ), rwork( &
           indre ), m, nsplit, w,iwork( indibl ), iwork( indisp ), rwork( indrwk ),iwork( indiwo )&
                     , info )
           if( wantz ) then
              call stdlib_I64_cstein( n, rwork( indrd ), rwork( indre ), m, w,iwork( indibl ), iwork( &
                        indisp ), z, ldz,rwork( indrwk ), iwork( indiwo ), iwork( indifl ),info )
              ! apply unitary matrix used in reduction to tridiagonal
     ...
     !! manner.
        ...
        ! =====================================================================
           ...
           ! Executable Statements 
           ! test the input parameters.
           ieeeok = stdlib_I64_ilaenv( 10_ilp64, 'ZHEEVR', 'N', 1_ilp64, 2_ilp64, 3_ilp64, 4_ilp64 )
           lower = stdlib_lsame( uplo, 'L' )
           wantz = stdlib_lsame( jobz, 'V' )
           alleig = stdlib_lsame( range, 'A' )
           valeig = stdlib_lsame( range, 'V' )
           ...
           if( info==0_ilp64 ) then
              nb = stdlib_I64_ilaenv( 1_ilp64, 'ZHETRD', uplo, n, -1_ilp64, -1_ilp64, -1_ilp64 )
              nb = max( nb, stdlib_I64_ilaenv( 1_ilp64, 'ZUNMTR', uplo, n, -1_ilp64, -1_ilp64, -1_ilp64 ) )
              lwkopt = max( ( nb+1 )*n, lwmin )
              work( 1_ilp64 ) = lwkopt
              rwork( 1_ilp64 ) = lrwmin
              iwork( 1_ilp64 ) = liwmin
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZHEEVR', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           if( n==1_ilp64 ) then
              ...
              if( alleig .or. indeig ) then
                 ...
                 if( vl<real( a( 1_ilp64, 1_ilp64 ),KIND=dp) .and. vu>=real( a( 1_ilp64, 1_ilp64 ),KIND=dp) )then
                    m = 1_ilp64
                    w( 1_ilp64 ) = real( a( 1_ilp64, 1_ilp64 ),KIND=dp)
                 end if
              end if
           ...
           anrm = stdlib_I64_zlansy( 'M', uplo, n, a, lda, rwork )
           if( anrm>zero .and. anrm<rmin ) then
              iscale = 1_ilp64
              sigma = rmin / anrm
           else if( anrm>rmax ) then
           ...
           if( iscale==1_ilp64 ) then
              if( lower ) then
                 do j = 1, n
                    call stdlib_I64_zdscal( n-j+1, sigma, a( j, j ), 1_ilp64 )
                 end do
              else
                 do j = 1, n
                    call stdlib_I64_zdscal( j, sigma, a( 1_ilp64, j ), 1_ilp64 )
                 end do
              end if
              if( abstol>0_ilp64 )abstll = abstol*sigma
              if( valeig ) then
           ...
           ! initialize indices into workspaces.  note: the iwork indices are
           ! used only if stdlib_I64_dsterf or stdlib_I64_zstemr fail.
           ! work(indtau:indtau+n-1) stores the complex scalar factors of the
           ! elementary reflectors used in stdlib_I64_zhetrd.
           indtau = 1_ilp64
           ! indwk is the starting offset of the remaining complex workspace,
           ! and llwork is the remaining complex workspace size.
           indwk = indtau + n
           ...
           indrd = 1_ilp64
           ! rwork(indre:indre+n-1) stores the off-diagonal entries of the
           ! tridiagonal matrix from stdlib_I64_zhetrd.
           indre = indrd + n
           ! rwork(indrdd:indrdd+n-1) is a copy of the diagonal entries over
           ! -written by stdlib_I64_zstemr (the stdlib_I64_dsterf path copies the diagonal to w).
           indrdd = indre + n
           ! rwork(indree:indree+n-1) is a copy of the off-diagonal entries over
           ! -written while computing the eigenvalues in stdlib_I64_dsterf and stdlib_I64_zstemr.
           indree = indrdd + n
           ! indrwk is the starting offset of the left-over real workspace, and
           ! llrwork is the remaining workspace size.
           indrwk = indree + n
           ...
           indiwo = indifl + n
           ! call stdlib_I64_zhetrd to reduce hermitian matrix to tridiagonal form.
           call stdlib_I64_zhetrd( uplo, n, a, lda, rwork( indrd ), rwork( indre ),work( indtau ), &
                     work( indwk ), llwork, iinfo )
           ! if all eigenvalues are desired
           ! then call stdlib_I64_dsterf or stdlib_I64_zstemr and stdlib_I64_zunmtr.
           test = .false.
           if( indeig ) then
              if( il==1_ilp64 .and. iu==n ) then
                 test = .true.
           ...
           if( ( alleig.or.test ) .and. ( ieeeok==1_ilp64 ) ) then
              if( .not.wantz ) then
                 call stdlib_I64_dcopy( n, rwork( indrd ), 1_ilp64, w, 1_ilp64 )
                 call stdlib_I64_dcopy( n-1, rwork( indre ), 1_ilp64, rwork( indree ), 1_ilp64 )
                 call stdlib_I64_dsterf( n, w, rwork( indree ), info )
              else
                 call stdlib_I64_dcopy( n-1, rwork( indre ), 1_ilp64, rwork( indree ), 1_ilp64 )
                 call stdlib_I64_dcopy( n, rwork( indrd ), 1_ilp64, rwork( indrdd ), 1_ilp64 )
                 if (abstol <= two*n*eps) then
                    tryrac = .true.
                 else
                    tryrac = .false.
                 end if
                 call stdlib_I64_zstemr( jobz, 'A', n, rwork( indrdd ),rwork( indree ), vl, vu, il, &
                 iu, m, w,z, ldz, n, isuppz, tryrac,rwork( indrwk ), llrwork,iwork, liwork, info )
                           
                 ! apply unitary matrix used in reduction to tridiagonal
                 ! form to eigenvectors returned by stdlib_I64_zstemr.
                 if( wantz .and. info==0_ilp64 ) then
                    indwkn = indwk
                    llwrkn = lwork - indwkn + 1_ilp64
                    call stdlib_I64_zunmtr( 'L', uplo, 'N', n, m, a, lda,work( indtau ), z, ldz, work(&
                               indwkn ),llwrkn, iinfo )
                 end if
              end if
              if( info==0_ilp64 ) then
           ...
           ! otherwise, call stdlib_I64_dstebz and, if eigenvectors are desired, stdlib_I64_zstein.
           ! also call stdlib_I64_dstebz and stdlib_I64_zstein if stdlib_I64_zstemr fails.
           if( wantz ) then
              order = 'B'
           else
              order = 'E'
           end if
           call stdlib_I64_dstebz( range, order, n, vll, vuu, il, iu, abstll,rwork( indrd ), rwork( &
           indre ), m, nsplit, w,iwork( indibl ), iwork( indisp ), rwork( indrwk ),iwork( indiwo )&
                     , info )
           if( wantz ) then
              call stdlib_I64_zstein( n, rwork( indrd ), rwork( indre ), m, w,iwork( indibl ), iwork( &
                        indisp ), z, ldz,rwork( indrwk ), iwork( indiwo ), iwork( indifl ),info )
              ! apply unitary matrix used in reduction to tridiagonal
     ...
     !! indices for the desired eigenvalues.
        ...
        ! =====================================================================
           ...
           logical(lk) :: alleig, indeig, lower, lquery, test, valeig, wantz
           character :: order
           integer(ilp64) :: i, iinfo, imax, indd, inde, indee, indibl, indisp, indiwk, indrwk, &
                     indtau, indwrk, iscale, itmp1, j, jj, llwork, lwkmin, lwkopt, nb, nsplit
           real(sp) :: abstll, anrm, bignum, eps, rmax, rmin, safmin, sigma, smlnum, tmp1, vll, &
                     vuu
           ! Intrinsic Functions 
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CHEEVX', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           if( n==1_ilp64 ) then
              ...
              if( wantz )z( 1_ilp64, 1_ilp64 ) = cone
              return
           end if
           ! get machine constants.
           safmin = stdlib_I64_slamch( 'SAFE MINIMUM' )
           ...
           indwrk = indtau + n
           llwork = lwork - indwrk + 1_ilp64
           call stdlib_I64_chetrd( uplo, n, a, lda, rwork( indd ), rwork( inde ),work( indtau ), work(&
                      indwrk ), llwork, iinfo )
           ! if all eigenvalues are desired and abstol is less than or equal to
           ! zero, then call stdlib_I64_ssterf or stdlib_I64_cungtr and stdlib_I64_csteqr.  if this fails for
           ! some eigenvalue, then try stdlib_I64_sstebz.
           test = .false.
           if( indeig ) then
              if( il==1_ilp64 .and. iu==n ) then
           ...
           if( ( alleig .or. test ) .and. ( abstol<=zero ) ) then
              ...
              if( .not.wantz ) then
                 ...
                 call stdlib_I64_clacpy( 'A', n, n, a, lda, z, ldz )
                 call stdlib_I64_cungtr( uplo, n, z, ldz, work( indtau ),work( indwrk ), llwork, &
                           iinfo )
                 call stdlib_I64_scopy( n-1, rwork( inde ), 1_ilp64, rwork( indee ), 1_ilp64 )
           ...
           indisp = indibl + n
           indiwk = indisp + n
           call stdlib_I64_sstebz( range, order, n, vll, vuu, il, iu, abstll,rwork( indd ), rwork( &
           inde ), m, nsplit, w,iwork( indibl ), iwork( indisp ), rwork( indrwk ),iwork( indiwk ),&
                      info )
           if( wantz ) then
     ...
     !! indices for the desired eigenvalues.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZHEEVX', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           indwrk = indtau + n
           llwork = lwork - indwrk + 1_ilp64
           call stdlib_I64_zhetrd( uplo, n, a, lda, rwork( indd ), rwork( inde ),work( indtau ), work(&
                      indwrk ), llwork, iinfo )
           ! if all eigenvalues are desired and abstol is less than or equal to
           ! zero, then call stdlib_I64_dsterf or stdlib_I64_zungtr and stdlib_I64_zsteqr.  if this fails for
           ! some eigenvalue, then try stdlib_I64_dstebz.
           test = .false.
           if( indeig ) then
              if( il==1_ilp64 .and. iu==n ) then
           ...
           if( ( alleig .or. test ) .and. ( abstol<=zero ) ) then
              ...
              if( .not.wantz ) then
                 ...
                 call stdlib_I64_zlacpy( 'A', n, n, a, lda, z, ldz )
                 call stdlib_I64_zungtr( uplo, n, z, ldz, work( indtau ),work( indwrk ), llwork, &
                           iinfo )
                 call stdlib_I64_dcopy( n-1, rwork( inde ), 1_ilp64, rwork( indee ), 1_ilp64 )
           ...
           indisp = indibl + n
           indiwk = indisp + n
           call stdlib_I64_dstebz( range, order, n, vll, vuu, il, iu, abstll,rwork( indd ), rwork( &
           inde ), m, nsplit, w,iwork( indibl ), iwork( indisp ), rwork( indrwk ),iwork( indiwk ),&
                      info )
           if( wantz ) then
     ...
     !! complex Hermitian matrix in packed storage.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           logical(lk) :: wantz
           integer(ilp64) :: iinfo, imax, inde, indrwk, indtau, indwrk, iscale
           real(sp) :: anrm, bignum, eps, rmax, rmin, safmin, sigma, smlnum
           ! Intrinsic Functions 
           ! Executable Statements 
           ! test the input parameters.
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CHPEV ', -info )
              return
           end if
           ! quick return if possible
           if( n==0 )return
           if( n==1_ilp64 ) then
              w( 1_ilp64 ) = real( ap( 1_ilp64 ),KIND=sp)
              rwork( 1_ilp64 ) = 1_ilp64
              if( wantz )z( 1_ilp64, 1_ilp64 ) = one
              return
           end if
           ! get machine constants.
           ...
           rmax = sqrt( bignum )
           ! scale matrix to allowable range, if necessary.
           anrm = stdlib_I64_clanhp( 'M', uplo, n, ap, rwork )
           iscale = 0_ilp64
           if( anrm>zero .and. anrm<rmin ) then
              iscale = 1_ilp64
              sigma = rmin / anrm
           ...
           if( iscale==1_ilp64 ) then
              call stdlib_I64_csscal( ( n*( n+1 ) ) / 2_ilp64, sigma, ap, 1_ilp64 )
           end if
           ! call stdlib_I64_chptrd to reduce hermitian packed matrix to tridiagonal form.
           inde = 1_ilp64
           indtau = 1_ilp64
           call stdlib_I64_chptrd( uplo, n, ap, w, rwork( inde ), work( indtau ),iinfo )
           ! for eigenvalues only, call stdlib_I64_ssterf.  for eigenvectors, first call
           ! stdlib_I64_cupgtr to generate the orthogonal matrix, then call stdlib_I64_csteqr.
           if( .not.wantz ) then
              call stdlib_I64_ssterf( n, w, rwork( inde ), info )
           else
              indwrk = indtau + n
              call stdlib_I64_cupgtr( uplo, n, ap, work( indtau ), z, ldz,work( indwrk ), iinfo )
                        
              indrwk = inde + n
              call stdlib_I64_csteqr( jobz, n, w, rwork( inde ), z, ldz,rwork( indrwk ), info )
     ...
     !! complex Hermitian matrix in packed storage.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZHPEV ', -info )
              return
           end if
           ! quick return if possible
           if( n==0 )return
           if( n==1_ilp64 ) then
              w( 1_ilp64 ) = real( ap( 1_ilp64 ),KIND=dp)
              rwork( 1_ilp64 ) = 1_ilp64
              if( wantz )z( 1_ilp64, 1_ilp64 ) = one
              return
           end if
           ...
           rmax = sqrt( bignum )
           ! scale matrix to allowable range, if necessary.
           anrm = stdlib_I64_zlanhp( 'M', uplo, n, ap, rwork )
           iscale = 0_ilp64
           if( anrm>zero .and. anrm<rmin ) then
              iscale = 1_ilp64
              sigma = rmin / anrm
           ...
           if( iscale==1_ilp64 ) then
              call stdlib_I64_zdscal( ( n*( n+1 ) ) / 2_ilp64, sigma, ap, 1_ilp64 )
           end if
           ! call stdlib_I64_zhptrd to reduce hermitian packed matrix to tridiagonal form.
           inde = 1_ilp64
           indtau = 1_ilp64
           call stdlib_I64_zhptrd( uplo, n, ap, w, rwork( inde ), work( indtau ),iinfo )
           ! for eigenvalues only, call stdlib_I64_dsterf.  for eigenvectors, first call
           ! stdlib_I64_zupgtr to generate the orthogonal matrix, then call stdlib_I64_zsteqr.
           if( .not.wantz ) then
              call stdlib_I64_dsterf( n, w, rwork( inde ), info )
           else
              indwrk = indtau + n
              call stdlib_I64_zupgtr( uplo, n, ap, work( indtau ), z, ldz,work( indwrk ), iinfo )
                        
              indrwk = inde + n
              call stdlib_I64_zsteqr( jobz, n, w, rwork( inde ), z, ldz,rwork( indrwk ), info )
     ...
     !! without guard digits, but we know of none.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           logical(lk) :: lquery, wantz
           integer(ilp64) :: iinfo, imax, inde, indrwk, indtau, indwrk, iscale, liwmin, llrwk, &
                     llwrk, lrwmin, lwmin
           real(sp) :: anrm, bignum, eps, rmax, rmin, safmin, sigma, smlnum
           ! Intrinsic Functions 
           ! Executable Statements 
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CHPEVD', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           indrwk = inde + n
           indwrk = indtau + n
           llwrk = lwork - indwrk + 1_ilp64
           llrwk = lrwork - indrwk + 1_ilp64
           call stdlib_I64_chptrd( uplo, n, ap, w, rwork( inde ), work( indtau ),iinfo )
           ! for eigenvalues only, call stdlib_I64_ssterf.  for eigenvectors, first call
           ! stdlib_I64_cupgtr to generate the orthogonal matrix, then call stdlib_I64_cstedc.
           if( .not.wantz ) then
              call stdlib_I64_ssterf( n, w, rwork( inde ), info )
           else
              call stdlib_I64_cstedc( 'I', n, w, rwork( inde ), z, ldz, work( indwrk ),llwrk, rwork( &
                        indrwk ), llrwk, iwork, liwork,info )
              call stdlib_I64_cupmtr( 'L', uplo, 'N', n, n, ap, work( indtau ), z, ldz,work( indwrk ),&
                         iinfo )
           end if
           ! if matrix was scaled, then rescale eigenvalues appropriately.
           if( iscale==1_ilp64 ) then
     ...
     !! without guard digits, but we know of none.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZHPEVD', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           call stdlib_I64_zhptrd( uplo, n, ap, w, rwork( inde ), work( indtau ),iinfo )
           ! for eigenvalues only, call stdlib_I64_dsterf.  for eigenvectors, first call
           ! stdlib_I64_zupgtr to generate the orthogonal matrix, then call stdlib_I64_zstedc.
           if( .not.wantz ) then
              call stdlib_I64_dsterf( n, w, rwork( inde ), info )
           else
              call stdlib_I64_zstedc( 'I', n, w, rwork( inde ), z, ldz, work( indwrk ),llwrk, rwork( &
                        indrwk ), llrwk, iwork, liwork,info )
              call stdlib_I64_zupmtr( 'L', uplo, 'N', n, n, ap, work( indtau ), z, ldz,work( indwrk ),&
                         iinfo )
           end if
           ! if matrix was scaled, then rescale eigenvalues appropriately.
           if( iscale==1_ilp64 ) then
     ...
     !! values or a range of indices for the desired eigenvalues.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CHPEVX', -info )
              return
           end if
           ! quick return if possible
           m = 0_ilp64
           ...
           if( n==1_ilp64 ) then
              if( alleig .or. indeig ) then
                 ...
                 if( vl<real( ap( 1_ilp64 ),KIND=sp) .and. vu>=real( ap( 1_ilp64 ),KIND=sp) ) then
                    m = 1_ilp64
                    w( 1_ilp64 ) = real( ap( 1_ilp64 ),KIND=sp)
                 end if
              end if
           ...
           indtau = 1_ilp64
           indwrk = indtau + n
           call stdlib_I64_chptrd( uplo, n, ap, rwork( indd ), rwork( inde ),work( indtau ), iinfo )
                     
           ! if all eigenvalues are desired and abstol is less than or equal
           ! to zero, then call stdlib_I64_ssterf or stdlib_I64_cupgtr and stdlib_I64_csteqr.  if this fails
           ! for some eigenvalue, then try stdlib_I64_sstebz.
           test = .false.
           if (indeig) then
              if (il==1_ilp64 .and. iu==n) then
           ...
           if( wantz ) then
              ...
              ! form to eigenvectors returned by stdlib_I64_cstein.
              indwrk = indtau + n
              call stdlib_I64_cupmtr( 'L', uplo, 'N', n, m, ap, work( indtau ), z, ldz,work( indwrk ),&
                         iinfo )
           end if
           ! if matrix was scaled, then rescale eigenvalues appropriately.
           20 continue
     ...
     !! values or a range of indices for the desired eigenvalues.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZHPEVX', -info )
              return
           end if
           ! quick return if possible
           m = 0_ilp64
           ...
           if( n==1_ilp64 ) then
              if( alleig .or. indeig ) then
                 ...
                 if( vl<real( ap( 1_ilp64 ),KIND=dp) .and. vu>=real( ap( 1_ilp64 ),KIND=dp) ) then
                    m = 1_ilp64
                    w( 1_ilp64 ) = real( ap( 1_ilp64 ),KIND=dp)
                 end if
              end if
           ...
           indtau = 1_ilp64
           indwrk = indtau + n
           call stdlib_I64_zhptrd( uplo, n, ap, rwork( indd ), rwork( inde ),work( indtau ), iinfo )
                     
           ! if all eigenvalues are desired and abstol is less than or equal
           ! to zero, then call stdlib_I64_dsterf or stdlib_I64_zupgtr and stdlib_I64_zsteqr.  if this fails
           ! for some eigenvalue, then try stdlib_I64_dstebz.
           test = .false.
           if (indeig) then
              if (il==1_ilp64 .and. iu==n) then
           ...
           if( wantz ) then
              ...
              ! form to eigenvectors returned by stdlib_I64_zstein.
              indwrk = indtau + n
              call stdlib_I64_zupmtr( 'L', uplo, 'N', n, m, ap, work( indtau ), z, ldz,work( indwrk ),&
                         iinfo )
           end if
           ! if matrix was scaled, then rescale eigenvalues appropriately.
           20 continue
     ...
     !! a complex Hermitian band matrix A.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           logical(lk) :: lower, wantz
           integer(ilp64) :: iinfo, imax, inde, indrwk, iscale
           real(sp) :: anrm, bignum, eps, rmax, rmin, safmin, sigma, smlnum
           ! Intrinsic Functions 
           ! Executable Statements 
           ! test the input parameters.
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CHBEV ', -info )
              return
           end if
           ! quick return if possible
           if( n==0 )return
           if( n==1_ilp64 ) then
              if( lower ) then
                 w( 1_ilp64 ) = real( ab( 1_ilp64, 1_ilp64 ),KIND=sp)
              else
                 w( 1_ilp64 ) = real( ab( kd+1, 1_ilp64 ),KIND=sp)
              end if
              if( wantz )z( 1_ilp64, 1_ilp64 ) = one
              return
           end if
           ...
           rmax = sqrt( bignum )
           ! scale matrix to allowable range, if necessary.
           anrm = stdlib_I64_clanhb( 'M', uplo, n, kd, ab, ldab, rwork )
           iscale = 0_ilp64
           if( anrm>zero .and. anrm<rmin ) then
              iscale = 1_ilp64
              sigma = rmin / anrm
           ...
           if( iscale==1_ilp64 ) then
              if( lower ) then
                 call stdlib_I64_clascl( 'B', kd, kd, one, sigma, n, n, ab, ldab, info )
              else
                 call stdlib_I64_clascl( 'Q', kd, kd, one, sigma, n, n, ab, ldab, info )
              end if
           end if
           ! call stdlib_I64_chbtrd to reduce hermitian band matrix to tridiagonal form.
           inde = 1_ilp64
           call stdlib_I64_chbtrd( jobz, uplo, n, kd, ab, ldab, w, rwork( inde ), z,ldz, work, iinfo )
     ...
     !! a complex Hermitian band matrix A.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZHBEV ', -info )
              return
           end if
           ! quick return if possible
           if( n==0 )return
           if( n==1_ilp64 ) then
              if( lower ) then
                 w( 1_ilp64 ) = real( ab( 1_ilp64, 1_ilp64 ),KIND=dp)
              else
                 w( 1_ilp64 ) = real( ab( kd+1, 1_ilp64 ),KIND=dp)
              end if
              if( wantz )z( 1_ilp64, 1_ilp64 ) = one
              return
           end if
           ...
           rmax = sqrt( bignum )
           ! scale matrix to allowable range, if necessary.
           anrm = stdlib_I64_zlanhb( 'M', uplo, n, kd, ab, ldab, rwork )
           iscale = 0_ilp64
           if( anrm>zero .and. anrm<rmin ) then
              iscale = 1_ilp64
              sigma = rmin / anrm
           ...
           if( iscale==1_ilp64 ) then
              if( lower ) then
                 call stdlib_I64_zlascl( 'B', kd, kd, one, sigma, n, n, ab, ldab, info )
              else
                 call stdlib_I64_zlascl( 'Q', kd, kd, one, sigma, n, n, ab, ldab, info )
              end if
           end if
           ! call stdlib_I64_zhbtrd to reduce hermitian band matrix to tridiagonal form.
           inde = 1_ilp64
           call stdlib_I64_zhbtrd( jobz, uplo, n, kd, ab, ldab, w, rwork( inde ), z,ldz, work, iinfo )
     ...
     !! without guard digits, but we know of none.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           logical(lk) :: lower, lquery, wantz
           integer(ilp64) :: iinfo, imax, inde, indwk2, indwrk, iscale, liwmin, llrwk, llwk2, &
                     lrwmin, lwmin
           real(sp) :: anrm, bignum, eps, rmax, rmin, safmin, sigma, smlnum
           ! Intrinsic Functions 
           ! Executable Statements 
           ...
           wantz = stdlib_lsame( jobz, 'V' )
           lower = stdlib_lsame( uplo, 'L' )
           lquery = ( lwork==-1_ilp64 .or. liwork==-1_ilp64 .or. lrwork==-1_ilp64 )
           info = 0_ilp64
           if( n<=1_ilp64 ) then
              lwmin = 1_ilp64
              lrwmin = 1_ilp64
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CHBEVD', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           indwrk = inde + n
           indwk2 = 1_ilp64 + n*n
           llwk2 = lwork - indwk2 + 1_ilp64
           llrwk = lrwork - indwrk + 1_ilp64
           call stdlib_I64_chbtrd( jobz, uplo, n, kd, ab, ldab, w, rwork( inde ), z,ldz, work, iinfo )
     ...
     !! without guard digits, but we know of none.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZHBEVD', -info )
              return
           else if( lquery ) then
              return
           end if
     ...
     !! indices for the desired eigenvalues.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CHBEVX', -info )
              return
           end if
           ! quick return if possible
           m = 0_ilp64
           ...
           if( n==1_ilp64 ) then
              m = 1_ilp64
              if( lower ) then
                 ctmp1 = ab( 1_ilp64, 1_ilp64 )
              else
                 ctmp1 = ab( kd+1, 1_ilp64 )
              end if
              tmp1 = real( ctmp1,KIND=sp)
              if( valeig ) then
                 if( .not.( vl<tmp1 .and. vu>=tmp1 ) )m = 0_ilp64
           ...
           indrwk = inde + n
           indwrk = 1_ilp64
           call stdlib_I64_chbtrd( jobz, uplo, n, kd, ab, ldab, rwork( indd ),rwork( inde ), q, ldq, &
                     work( indwrk ), iinfo )
           ! if all eigenvalues are desired and abstol is less than or equal
     ...
     !! indices for the desired eigenvalues.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZHBEVX', -info )
              return
           end if
           ! quick return if possible
           m = 0_ilp64
           ...
           indrwk = inde + n
           indwrk = 1_ilp64
           call stdlib_I64_zhbtrd( jobz, uplo, n, kd, ab, ldab, rwork( indd ),rwork( inde ), q, ldq, &
                     work( indwrk ), iinfo )
           ! if all eigenvalues are desired and abstol is less than or equal
