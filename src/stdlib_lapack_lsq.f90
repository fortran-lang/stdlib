  ...
  contains
     ...
     !! value.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           logical(lk) :: lquery
           integer(ilp64) :: bdspac, bl, chunk, i, iascl, ibscl, ie, il, itau, itaup, itauq, iwork, &
                     ldwork, maxmn, maxwrk, minmn, minwrk, mm, mnthr
           integer(ilp64) :: lwork_sgeqrf, lwork_sormqr, lwork_sgebrd, lwork_sormbr, lwork_sorgbr, &
                     lwork_sormlq
           real(sp) :: anrm, bignum, bnrm, eps, sfmin, smlnum, thr
           ! Local Arrays 
           real(sp) :: dum(1_ilp64)
           ...
           if( m<0_ilp64 ) then
              ...
              info = -5_ilp64
           else if( ldb<max( 1_ilp64, maxmn ) ) then
              info = -7_ilp64
           end if
           ! compute workspace
            ! (note: comments in the code beginning "workspace:" describe the
             ...
             ! as well as the preferred amount for good performance.
             ! nb refers to the optimal block size for the immediately
             ! following subroutine, as returned by stdlib_I64_ilaenv.)
           if( info==0_ilp64 ) then
              minwrk = 1_ilp64
              maxwrk = 1_ilp64
              if( minmn>0_ilp64 ) then
                 mm = m
                 mnthr = stdlib_I64_ilaenv( 6_ilp64, 'SGELSS', ' ', m, n, nrhs, -1_ilp64 )
                 if( m>=n .and. m>=mnthr ) then
                    ! path 1a - overdetermined, with many more rows than
                              ! columns
                    ! compute space needed for stdlib_I64_sgeqrf
                    call stdlib_I64_sgeqrf( m, n, a, lda, dum(1_ilp64), dum(1_ilp64), -1_ilp64, info )
                    lwork_sgeqrf=dum(1_ilp64)
                    ! compute space needed for stdlib_I64_sormqr
                    call stdlib_I64_sormqr( 'L', 'T', m, nrhs, n, a, lda, dum(1_ilp64), b,ldb, dum(1_ilp64), -1_ilp64, &
                              info )
                    lwork_sormqr=dum(1_ilp64)
                    mm = n
                    maxwrk = max( maxwrk, n + lwork_sgeqrf )
                    maxwrk = max( maxwrk, n + lwork_sormqr )
                 end if
                 if( m>=n ) then
                    ! path 1 - overdetermined or exactly determined
                    ! compute workspace needed for stdlib_I64_sbdsqr
                    bdspac = max( 1_ilp64, 5_ilp64*n )
                    ! compute space needed for stdlib_I64_sgebrd
                    call stdlib_I64_sgebrd( mm, n, a, lda, s, dum(1_ilp64), dum(1_ilp64),dum(1_ilp64), dum(1_ilp64), -1_ilp64,&
                        & info &
                              )
                    lwork_sgebrd=dum(1_ilp64)
                    ! compute space needed for stdlib_I64_sormbr
                    call stdlib_I64_sormbr( 'Q', 'L', 'T', mm, nrhs, n, a, lda, dum(1_ilp64),b, ldb, dum(1_ilp64),&
                               -1_ilp64, info )
                    lwork_sormbr=dum(1_ilp64)
                    ! compute space needed for stdlib_I64_sorgbr
                    call stdlib_I64_sorgbr( 'P', n, n, n, a, lda, dum(1_ilp64),dum(1_ilp64), -1_ilp64, info )
                    lwork_sorgbr=dum(1_ilp64)
                    ! compute total workspace needed
                    maxwrk = max( maxwrk, 3_ilp64*n + lwork_sgebrd )
                    maxwrk = max( maxwrk, 3_ilp64*n + lwork_sormbr )
                    maxwrk = max( maxwrk, 3_ilp64*n + lwork_sorgbr )
                    maxwrk = max( maxwrk, bdspac )
                    maxwrk = max( maxwrk, n*nrhs )
                    minwrk = max( 3_ilp64*n + mm, 3_ilp64*n + nrhs, bdspac )
                    maxwrk = max( minwrk, maxwrk )
                 end if
                 if( n>m ) then
                    ! compute workspace needed for stdlib_I64_sbdsqr
                    bdspac = max( 1_ilp64, 5_ilp64*m )
                    minwrk = max( 3_ilp64*m+nrhs, 3_ilp64*m+n, bdspac )
                    if( n>=mnthr ) then
                       ! path 2a - underdetermined, with many more columns
                       ! than rows
                       ! compute space needed for stdlib_I64_sgebrd
                       call stdlib_I64_sgebrd( m, m, a, lda, s, dum(1_ilp64), dum(1_ilp64),dum(1_ilp64), dum(1_ilp64), -1_ilp64, &
                                 info )
                       lwork_sgebrd=dum(1_ilp64)
                       ! compute space needed for stdlib_I64_sormbr
                       call stdlib_I64_sormbr( 'Q', 'L', 'T', m, nrhs, n, a, lda,dum(1_ilp64), b, ldb, dum(&
                                 1_ilp64), -1_ilp64, info )
                       lwork_sormbr=dum(1_ilp64)
                       ! compute space needed for stdlib_I64_sorgbr
                       call stdlib_I64_sorgbr( 'P', m, m, m, a, lda, dum(1_ilp64),dum(1_ilp64), -1_ilp64, info )
                       lwork_sorgbr=dum(1_ilp64)
                       ! compute space needed for stdlib_I64_sormlq
                       call stdlib_I64_sormlq( 'L', 'T', n, nrhs, m, a, lda, dum(1_ilp64),b, ldb, dum(1_ilp64), -&
                                 1_ilp64, info )
                       lwork_sormlq=dum(1_ilp64)
                       ! compute total workspace needed
                       maxwrk = m + m*stdlib_I64_ilaenv( 1_ilp64, 'SGELQF', ' ', m, n, -1_ilp64,-1_ilp64 )
                       maxwrk = max( maxwrk, m*m + 4_ilp64*m + lwork_sgebrd )
                       maxwrk = max( maxwrk, m*m + 4_ilp64*m + lwork_sormbr )
                       maxwrk = max( maxwrk, m*m + 4_ilp64*m + lwork_sorgbr )
                       maxwrk = max( maxwrk, m*m + m + bdspac )
                       if( nrhs>1_ilp64 ) then
                          maxwrk = max( maxwrk, m*m + m + m*nrhs )
                       else
                          maxwrk = max( maxwrk, m*m + 2_ilp64*m )
                       end if
                       maxwrk = max( maxwrk, m + lwork_sormlq )
                    else
                       ! path 2 - underdetermined
                       ! compute space needed for stdlib_I64_sgebrd
                       call stdlib_I64_sgebrd( m, n, a, lda, s, dum(1_ilp64), dum(1_ilp64),dum(1_ilp64), dum(1_ilp64), -1_ilp64, &
                                 info )
                       lwork_sgebrd=dum(1_ilp64)
                       ! compute space needed for stdlib_I64_sormbr
                       call stdlib_I64_sormbr( 'Q', 'L', 'T', m, nrhs, m, a, lda,dum(1_ilp64), b, ldb, dum(&
                                 1_ilp64), -1_ilp64, info )
                       lwork_sormbr=dum(1_ilp64)
                       ! compute space needed for stdlib_I64_sorgbr
                       call stdlib_I64_sorgbr( 'P', m, n, m, a, lda, dum(1_ilp64),dum(1_ilp64), -1_ilp64, info )
                       lwork_sorgbr=dum(1_ilp64)
                       maxwrk = 3_ilp64*m + lwork_sgebrd
                       maxwrk = max( maxwrk, 3_ilp64*m + lwork_sormbr )
                       maxwrk = max( maxwrk, 3_ilp64*m + lwork_sorgbr )
                       maxwrk = max( maxwrk, bdspac )
                       maxwrk = max( maxwrk, n*nrhs )
                    end if
                 end if
              ...
              work( 1_ilp64 ) = maxwrk
              if( lwork<minwrk .and. .not.lquery )info = -12_ilp64
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SGELSS', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           sfmin = stdlib_I64_slamch( 'S' )
           smlnum = sfmin / eps
           bignum = one / smlnum
           call stdlib_I64_slabad( smlnum, bignum )
           ! scale a if max element outside range [smlnum,bignum]
           anrm = stdlib_I64_slange( 'M', m, n, a, lda, work )
           iascl = 0_ilp64
           if( anrm>zero .and. anrm<smlnum ) then
              ! scale matrix norm up to smlnum
              call stdlib_I64_slascl( 'G', 0_ilp64, 0_ilp64, anrm, smlnum, m, n, a, lda, info )
              iascl = 1_ilp64
           else if( anrm>bignum ) then
              ! scale matrix norm down to bignum
              call stdlib_I64_slascl( 'G', 0_ilp64, 0_ilp64, anrm, bignum, m, n, a, lda, info )
              iascl = 2_ilp64
           else if( anrm==zero ) then
              ! matrix all zero. return zero solution.
              call stdlib_I64_slaset( 'F', max( m, n ), nrhs, zero, zero, b, ldb )
              call stdlib_I64_slaset( 'F', minmn, 1_ilp64, zero, zero, s, minmn )
              rank = 0_ilp64
              go to 70
           end if
           ! scale b if max element outside range [smlnum,bignum]
           bnrm = stdlib_I64_slange( 'M', m, nrhs, b, ldb, work )
           ibscl = 0_ilp64
           if( bnrm>zero .and. bnrm<smlnum ) then
              ! scale matrix norm up to smlnum
              call stdlib_I64_slascl( 'G', 0_ilp64, 0_ilp64, bnrm, smlnum, m, nrhs, b, ldb, info )
              ibscl = 1_ilp64
           else if( bnrm>bignum ) then
              ! scale matrix norm down to bignum
              call stdlib_I64_slascl( 'G', 0_ilp64, 0_ilp64, bnrm, bignum, m, nrhs, b, ldb, info )
              ibscl = 2_ilp64
           end if
           ! overdetermined case
           if( m>=n ) then
              ...
              if( m>=mnthr ) then
                 ...
                 ! compute a=q*r
                 ! (workspace: need 2*n, prefer n+n*nb)
                 call stdlib_I64_sgeqrf( m, n, a, lda, work( itau ), work( iwork ),lwork-iwork+1, &
                           info )
                 ! multiply b by transpose(q)
                 ! (workspace: need n+nrhs, prefer n+nrhs*nb)
                 call stdlib_I64_sormqr( 'L', 'T', m, nrhs, n, a, lda, work( itau ), b,ldb, work( &
                           iwork ), lwork-iwork+1, info )
                 ! zero out below r
                 if( n>1_ilp64 )call stdlib_I64_slaset( 'L', n-1, n-1, zero, zero, a( 2_ilp64, 1_ilp64 ), lda )
              end if
              ie = 1_ilp64
              itauq = ie + n
              itaup = itauq + n
              ...
              ! bidiagonalize r in a
              ! (workspace: need 3*n+mm, prefer 3*n+(mm+n)*nb)
              call stdlib_I64_sgebrd( mm, n, a, lda, s, work( ie ), work( itauq ),work( itaup ), work(&
                         iwork ), lwork-iwork+1,info )
              ! multiply b by transpose of left bidiagonalizing vectors of r
              ! (workspace: need 3*n+nrhs, prefer 3*n+nrhs*nb)
              call stdlib_I64_sormbr( 'Q', 'L', 'T', mm, nrhs, n, a, lda, work( itauq ),b, ldb, work( &
                        iwork ), lwork-iwork+1, info )
              ! generate right bidiagonalizing vectors of r in a
              ! (workspace: need 4*n-1, prefer 3*n+(n-1)*nb)
              call stdlib_I64_sorgbr( 'P', n, n, n, a, lda, work( itaup ),work( iwork ), lwork-iwork+&
                        1_ilp64, info )
              iwork = ie + n
              ! perform bidiagonal qr iteration
                ! multiply b by transpose of left singular vectors
                ! compute right singular vectors in a
              ! (workspace: need bdspac)
              call stdlib_I64_sbdsqr( 'U', n, n, 0_ilp64, nrhs, s, work( ie ), a, lda, dum,1_ilp64, b, ldb, work( &
                        iwork ), info )
              if( info/=0 )go to 70
              ! multiply b by reciprocals of singular values
              thr = max( rcond*s( 1_ilp64 ), sfmin )
              if( rcond<zero )thr = max( eps*s( 1_ilp64 ), sfmin )
              rank = 0_ilp64
              do i = 1, n
                 if( s( i )>thr ) then
                    call stdlib_I64_srscl( nrhs, s( i ), b( i, 1_ilp64 ), ldb )
                    rank = rank + 1_ilp64
                 else
                    call stdlib_I64_slaset( 'F', 1_ilp64, nrhs, zero, zero, b( i, 1_ilp64 ), ldb )
                 end if
              end do
              ! multiply b by right singular vectors
              ! (workspace: need n, prefer n*nrhs)
              if( lwork>=ldb*nrhs .and. nrhs>1_ilp64 ) then
                 call stdlib_I64_sgemm( 'T', 'N', n, nrhs, n, one, a, lda, b, ldb, zero,work, ldb )
                           
                 call stdlib_I64_slacpy( 'G', n, nrhs, work, ldb, b, ldb )
              else if( nrhs>1_ilp64 ) then
                 chunk = lwork / n
                 do i = 1, nrhs, chunk
                    bl = min( nrhs-i+1, chunk )
                    call stdlib_I64_sgemm( 'T', 'N', n, bl, n, one, a, lda, b( 1_ilp64, i ),ldb, zero, work,&
                               n )
                    call stdlib_I64_slacpy( 'G', n, bl, work, n, b( 1_ilp64, i ), ldb )
                 end do
              else
                 call stdlib_I64_sgemv( 'T', n, n, one, a, lda, b, 1_ilp64, zero, work, 1_ilp64 )
                 call stdlib_I64_scopy( n, work, 1_ilp64, b, 1_ilp64 )
              end if
           else if( n>=mnthr .and. lwork>=4_ilp64*m+m*m+max( m, 2_ilp64*m-4, nrhs, n-3*m ) ) then
              ! path 2a - underdetermined, with many more columns than rows
              ! and sufficient workspace for an efficient algorithm
              ldwork = m
              if( lwork>=max( 4_ilp64*m+m*lda+max( m, 2_ilp64*m-4, nrhs, n-3*m ),m*lda+m+m*nrhs ) )ldwork = &
                        lda
              itau = 1_ilp64
              iwork = m + 1_ilp64
              ! compute a=l*q
              ! (workspace: need 2*m, prefer m+m*nb)
              call stdlib_I64_sgelqf( m, n, a, lda, work( itau ), work( iwork ),lwork-iwork+1, info )
                        
              il = iwork
              ! copy l to work(il), zeroing out above it
              call stdlib_I64_slacpy( 'L', m, m, a, lda, work( il ), ldwork )
              call stdlib_I64_slaset( 'U', m-1, m-1, zero, zero, work( il+ldwork ),ldwork )
              ie = il + ldwork*m
              itauq = ie + m
              itaup = itauq + m
              iwork = itaup + m
              ! bidiagonalize l in work(il)
              ! (workspace: need m*m+5*m, prefer m*m+4*m+2*m*nb)
              call stdlib_I64_sgebrd( m, m, work( il ), ldwork, s, work( ie ),work( itauq ), work( &
                        itaup ), work( iwork ),lwork-iwork+1, info )
              ! multiply b by transpose of left bidiagonalizing vectors of l
              ! (workspace: need m*m+4*m+nrhs, prefer m*m+4*m+nrhs*nb)
              call stdlib_I64_sormbr( 'Q', 'L', 'T', m, nrhs, m, work( il ), ldwork,work( itauq ), b, &
                        ldb, work( iwork ),lwork-iwork+1, info )
              ! generate right bidiagonalizing vectors of r in work(il)
              ! (workspace: need m*m+5*m-1, prefer m*m+4*m+(m-1)*nb)
              call stdlib_I64_sorgbr( 'P', m, m, m, work( il ), ldwork, work( itaup ),work( iwork ), &
                        lwork-iwork+1, info )
              iwork = ie + m
              ! perform bidiagonal qr iteration,
                 ! computing right singular vectors of l in work(il) and
                 ! multiplying b by transpose of left singular vectors
              ! (workspace: need m*m+m+bdspac)
              call stdlib_I64_sbdsqr( 'U', m, m, 0_ilp64, nrhs, s, work( ie ), work( il ),ldwork, a, lda, b,&
                         ldb, work( iwork ), info )
              if( info/=0 )go to 70
              ! multiply b by reciprocals of singular values
              thr = max( rcond*s( 1_ilp64 ), sfmin )
              ...
              ! multiply b by right singular vectors of l in work(il)
              ! (workspace: need m*m+2*m, prefer m*m+m+m*nrhs)
              if( lwork>=ldb*nrhs+iwork-1 .and. nrhs>1_ilp64 ) then
                 call stdlib_I64_sgemm( 'T', 'N', m, nrhs, m, one, work( il ), ldwork,b, ldb, zero, &
                           work( iwork ), ldb )
                 call stdlib_I64_slacpy( 'G', m, nrhs, work( iwork ), ldb, b, ldb )
              else if( nrhs>1_ilp64 ) then
                 chunk = ( lwork-iwork+1 ) / m
                 do i = 1, nrhs, chunk
                    bl = min( nrhs-i+1, chunk )
                    call stdlib_I64_sgemm( 'T', 'N', m, bl, m, one, work( il ), ldwork,b( 1_ilp64, i ), ldb,&
                               zero, work( iwork ), m )
                    call stdlib_I64_slacpy( 'G', m, bl, work( iwork ), m, b( 1_ilp64, i ),ldb )
                 end do
              else
                 call stdlib_I64_sgemv( 'T', m, m, one, work( il ), ldwork, b( 1_ilp64, 1_ilp64 ),1_ilp64, zero, work( &
                           iwork ), 1_ilp64 )
                 call stdlib_I64_scopy( m, work( iwork ), 1_ilp64, b( 1_ilp64, 1_ilp64 ), 1_ilp64 )
              end if
              ! zero out below first m rows of b
              call stdlib_I64_slaset( 'F', n-m, nrhs, zero, zero, b( m+1, 1_ilp64 ), ldb )
              iwork = itau + m
              ! multiply transpose(q) by b
              ! (workspace: need m+nrhs, prefer m+nrhs*nb)
              call stdlib_I64_sormlq( 'L', 'T', n, nrhs, m, a, lda, work( itau ), b,ldb, work( iwork )&
                        , lwork-iwork+1, info )
           else
              ! path 2 - remaining underdetermined cases
              ie = 1_ilp64
              ...
              ! bidiagonalize a
              ! (workspace: need 3*m+n, prefer 3*m+(m+n)*nb)
              call stdlib_I64_sgebrd( m, n, a, lda, s, work( ie ), work( itauq ),work( itaup ), work( &
                        iwork ), lwork-iwork+1,info )
              ! multiply b by transpose of left bidiagonalizing vectors
              ! (workspace: need 3*m+nrhs, prefer 3*m+nrhs*nb)
              call stdlib_I64_sormbr( 'Q', 'L', 'T', m, nrhs, n, a, lda, work( itauq ),b, ldb, work( &
                        iwork ), lwork-iwork+1, info )
              ! generate right bidiagonalizing vectors in a
              ! (workspace: need 4*m, prefer 3*m+m*nb)
              call stdlib_I64_sorgbr( 'P', m, n, m, a, lda, work( itaup ),work( iwork ), lwork-iwork+&
                        1_ilp64, info )
              iwork = ie + m
              ! perform bidiagonal qr iteration,
                 ! computing right singular vectors of a in a and
                 ! multiplying b by transpose of left singular vectors
              ! (workspace: need bdspac)
              call stdlib_I64_sbdsqr( 'L', m, n, 0_ilp64, nrhs, s, work( ie ), a, lda, dum,1_ilp64, b, ldb, work( &
                        iwork ), info )
              if( info/=0 )go to 70
              ! multiply b by reciprocals of singular values
              thr = max( rcond*s( 1_ilp64 ), sfmin )
              ...
              ! (workspace: need n, prefer n*nrhs)
              if( lwork>=ldb*nrhs .and. nrhs>1_ilp64 ) then
                 call stdlib_I64_sgemm( 'T', 'N', n, nrhs, m, one, a, lda, b, ldb, zero,work, ldb )
                           
                 call stdlib_I64_slacpy( 'F', n, nrhs, work, ldb, b, ldb )
              else if( nrhs>1_ilp64 ) then
                 chunk = lwork / n
                 do i = 1, nrhs, chunk
                    bl = min( nrhs-i+1, chunk )
                    call stdlib_I64_sgemm( 'T', 'N', n, bl, m, one, a, lda, b( 1_ilp64, i ),ldb, zero, work,&
                               n )
                    call stdlib_I64_slacpy( 'F', n, bl, work, n, b( 1_ilp64, i ), ldb )
                 end do
              else
                 call stdlib_I64_sgemv( 'T', m, n, one, a, lda, b, 1_ilp64, zero, work, 1_ilp64 )
                 call stdlib_I64_scopy( n, work, 1_ilp64, b, 1_ilp64 )
              end if
           end if
           ! undo scaling
           if( iascl==1_ilp64 ) then
              call stdlib_I64_slascl( 'G', 0_ilp64, 0_ilp64, anrm, smlnum, n, nrhs, b, ldb, info )
              call stdlib_I64_slascl( 'G', 0_ilp64, 0_ilp64, smlnum, anrm, minmn, 1_ilp64, s, minmn,info )
           else if( iascl==2_ilp64 ) then
              call stdlib_I64_slascl( 'G', 0_ilp64, 0_ilp64, anrm, bignum, n, nrhs, b, ldb, info )
              call stdlib_I64_slascl( 'G', 0_ilp64, 0_ilp64, bignum, anrm, minmn, 1_ilp64, s, minmn,info )
           end if
           if( ibscl==1_ilp64 ) then
              call stdlib_I64_slascl( 'G', 0_ilp64, 0_ilp64, smlnum, bnrm, n, nrhs, b, ldb, info )
           else if( ibscl==2_ilp64 ) then
              call stdlib_I64_slascl( 'G', 0_ilp64, 0_ilp64, bignum, bnrm, n, nrhs, b, ldb, info )
           end if
           70 continue
           work( 1_ilp64 ) = maxwrk
           return
     ...
     !! value.
        ...
        ! =====================================================================
           ...
           integer(ilp64) :: bdspac, bl, chunk, i, iascl, ibscl, ie, il, itau, itaup, itauq, iwork, &
                     ldwork, maxmn, maxwrk, minmn, minwrk, mm, mnthr
           integer(ilp64) :: lwork_dgeqrf, lwork_dormqr, lwork_dgebrd, lwork_dormbr, lwork_dorgbr, &
                     lwork_dormlq, lwork_dgelqf
           real(dp) :: anrm, bignum, bnrm, eps, sfmin, smlnum, thr
           ! Local Arrays 
           real(dp) :: dum(1_ilp64)
           ...
           if( info==0_ilp64 ) then
              ...
              if( minmn>0_ilp64 ) then
                 mm = m
                 mnthr = stdlib_I64_ilaenv( 6_ilp64, 'DGELSS', ' ', m, n, nrhs, -1_ilp64 )
                 if( m>=n .and. m>=mnthr ) then
                    ! path 1a - overdetermined, with many more rows than
                              ! columns
                    ! compute space needed for stdlib_I64_dgeqrf
                    call stdlib_I64_dgeqrf( m, n, a, lda, dum(1_ilp64), dum(1_ilp64), -1_ilp64, info )
                    lwork_dgeqrf=dum(1_ilp64)
                    ! compute space needed for stdlib_I64_dormqr
                    call stdlib_I64_dormqr( 'L', 'T', m, nrhs, n, a, lda, dum(1_ilp64), b,ldb, dum(1_ilp64), -1_ilp64, &
                              info )
                    lwork_dormqr=dum(1_ilp64)
                    mm = n
                    maxwrk = max( maxwrk, n + lwork_dgeqrf )
                    maxwrk = max( maxwrk, n + lwork_dormqr )
                 end if
                 if( m>=n ) then
                    ! path 1 - overdetermined or exactly determined
                    ! compute workspace needed for stdlib_I64_dbdsqr
                    bdspac = max( 1_ilp64, 5_ilp64*n )
                    ! compute space needed for stdlib_I64_dgebrd
                    call stdlib_I64_dgebrd( mm, n, a, lda, s, dum(1_ilp64), dum(1_ilp64),dum(1_ilp64), dum(1_ilp64), -1_ilp64,&
                        & info &
                              )
                    lwork_dgebrd=dum(1_ilp64)
                    ! compute space needed for stdlib_I64_dormbr
                    call stdlib_I64_dormbr( 'Q', 'L', 'T', mm, nrhs, n, a, lda, dum(1_ilp64),b, ldb, dum(1_ilp64),&
                               -1_ilp64, info )
                    lwork_dormbr=dum(1_ilp64)
                    ! compute space needed for stdlib_I64_dorgbr
                    call stdlib_I64_dorgbr( 'P', n, n, n, a, lda, dum(1_ilp64),dum(1_ilp64), -1_ilp64, info )
                    lwork_dorgbr=dum(1_ilp64)
                    ! compute total workspace needed
                    maxwrk = max( maxwrk, 3_ilp64*n + lwork_dgebrd )
                    maxwrk = max( maxwrk, 3_ilp64*n + lwork_dormbr )
                    maxwrk = max( maxwrk, 3_ilp64*n + lwork_dorgbr )
                    maxwrk = max( maxwrk, bdspac )
                    maxwrk = max( maxwrk, n*nrhs )
                    minwrk = max( 3_ilp64*n + mm, 3_ilp64*n + nrhs, bdspac )
                    maxwrk = max( minwrk, maxwrk )
                 ...
                 if( n>m ) then
                    ...
                    if( n>=mnthr ) then
                       ! path 2a - underdetermined, with many more columns
                       ! than rows
                       ! compute space needed for stdlib_I64_dgelqf
                       call stdlib_I64_dgelqf( m, n, a, lda, dum(1_ilp64), dum(1_ilp64),-1_ilp64, info )
                       lwork_dgelqf=dum(1_ilp64)
                       ! compute space needed for stdlib_I64_dgebrd
                       call stdlib_I64_dgebrd( m, m, a, lda, s, dum(1_ilp64), dum(1_ilp64),dum(1_ilp64), dum(1_ilp64), -1_ilp64, &
                                 info )
                       lwork_dgebrd=dum(1_ilp64)
                       ! compute space needed for stdlib_I64_dormbr
                       call stdlib_I64_dormbr( 'Q', 'L', 'T', m, nrhs, n, a, lda,dum(1_ilp64), b, ldb, dum(&
                                 1_ilp64), -1_ilp64, info )
                       lwork_dormbr=dum(1_ilp64)
                       ! compute space needed for stdlib_I64_dorgbr
                       call stdlib_I64_dorgbr( 'P', m, m, m, a, lda, dum(1_ilp64),dum(1_ilp64), -1_ilp64, info )
                       lwork_dorgbr=dum(1_ilp64)
                       ! compute space needed for stdlib_I64_dormlq
                       call stdlib_I64_dormlq( 'L', 'T', n, nrhs, m, a, lda, dum(1_ilp64),b, ldb, dum(1_ilp64), -&
                                 1_ilp64, info )
                       lwork_dormlq=dum(1_ilp64)
                       ! compute total workspace needed
                       maxwrk = m + lwork_dgelqf
                       maxwrk = max( maxwrk, m*m + 4_ilp64*m + lwork_dgebrd )
                       maxwrk = max( maxwrk, m*m + 4_ilp64*m + lwork_dormbr )
                       maxwrk = max( maxwrk, m*m + 4_ilp64*m + lwork_dorgbr )
                       maxwrk = max( maxwrk, m*m + m + bdspac )
                       if( nrhs>1_ilp64 ) then
                          maxwrk = max( maxwrk, m*m + m + m*nrhs )
                       else
                       ...
                       ! path 2 - underdetermined
                       ! compute space needed for stdlib_I64_dgebrd
                       call stdlib_I64_dgebrd( m, n, a, lda, s, dum(1_ilp64), dum(1_ilp64),dum(1_ilp64), dum(1_ilp64), -1_ilp64, &
                                 info )
                       lwork_dgebrd=dum(1_ilp64)
                       ! compute space needed for stdlib_I64_dormbr
                       call stdlib_I64_dormbr( 'Q', 'L', 'T', m, nrhs, m, a, lda,dum(1_ilp64), b, ldb, dum(&
                                 1_ilp64), -1_ilp64, info )
                       lwork_dormbr=dum(1_ilp64)
                       ! compute space needed for stdlib_I64_dorgbr
                       call stdlib_I64_dorgbr( 'P', m, n, m, a, lda, dum(1_ilp64),dum(1_ilp64), -1_ilp64, info )
                       lwork_dorgbr=dum(1_ilp64)
                       maxwrk = 3_ilp64*m + lwork_dgebrd
                       maxwrk = max( maxwrk, 3_ilp64*m + lwork_dormbr )
                       maxwrk = max( maxwrk, 3_ilp64*m + lwork_dorgbr )
                       maxwrk = max( maxwrk, bdspac )
                       maxwrk = max( maxwrk, n*nrhs )
                    end if
                 end if
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DGELSS', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           sfmin = stdlib_I64_dlamch( 'S' )
           smlnum = sfmin / eps
           bignum = one / smlnum
           call stdlib_I64_dlabad( smlnum, bignum )
           ! scale a if max element outside range [smlnum,bignum]
           anrm = stdlib_I64_dlange( 'M', m, n, a, lda, work )
           iascl = 0_ilp64
           if( anrm>zero .and. anrm<smlnum ) then
              ! scale matrix norm up to smlnum
              call stdlib_I64_dlascl( 'G', 0_ilp64, 0_ilp64, anrm, smlnum, m, n, a, lda, info )
              iascl = 1_ilp64
           else if( anrm>bignum ) then
              ! scale matrix norm down to bignum
              call stdlib_I64_dlascl( 'G', 0_ilp64, 0_ilp64, anrm, bignum, m, n, a, lda, info )
              iascl = 2_ilp64
           else if( anrm==zero ) then
              ! matrix all zero. return zero solution.
              call stdlib_I64_dlaset( 'F', max( m, n ), nrhs, zero, zero, b, ldb )
              call stdlib_I64_dlaset( 'F', minmn, 1_ilp64, zero, zero, s, minmn )
              rank = 0_ilp64
              go to 70
           end if
           ! scale b if max element outside range [smlnum,bignum]
           bnrm = stdlib_I64_dlange( 'M', m, nrhs, b, ldb, work )
           ibscl = 0_ilp64
           if( bnrm>zero .and. bnrm<smlnum ) then
              ! scale matrix norm up to smlnum
              call stdlib_I64_dlascl( 'G', 0_ilp64, 0_ilp64, bnrm, smlnum, m, nrhs, b, ldb, info )
              ibscl = 1_ilp64
           else if( bnrm>bignum ) then
              ! scale matrix norm down to bignum
              call stdlib_I64_dlascl( 'G', 0_ilp64, 0_ilp64, bnrm, bignum, m, nrhs, b, ldb, info )
              ibscl = 2_ilp64
           end if
           ! overdetermined case
           if( m>=n ) then
              ...
              if( m>=mnthr ) then
                 ...
                 ! compute a=q*r
                 ! (workspace: need 2*n, prefer n+n*nb)
                 call stdlib_I64_dgeqrf( m, n, a, lda, work( itau ), work( iwork ),lwork-iwork+1, &
                           info )
                 ! multiply b by transpose(q)
                 ! (workspace: need n+nrhs, prefer n+nrhs*nb)
                 call stdlib_I64_dormqr( 'L', 'T', m, nrhs, n, a, lda, work( itau ), b,ldb, work( &
                           iwork ), lwork-iwork+1, info )
                 ! zero out below r
                 if( n>1_ilp64 )call stdlib_I64_dlaset( 'L', n-1, n-1, zero, zero, a( 2_ilp64, 1_ilp64 ), lda )
              end if
              ie = 1_ilp64
              itauq = ie + n
              itaup = itauq + n
              ...
              ! bidiagonalize r in a
              ! (workspace: need 3*n+mm, prefer 3*n+(mm+n)*nb)
              call stdlib_I64_dgebrd( mm, n, a, lda, s, work( ie ), work( itauq ),work( itaup ), work(&
                         iwork ), lwork-iwork+1,info )
              ! multiply b by transpose of left bidiagonalizing vectors of r
              ! (workspace: need 3*n+nrhs, prefer 3*n+nrhs*nb)
              call stdlib_I64_dormbr( 'Q', 'L', 'T', mm, nrhs, n, a, lda, work( itauq ),b, ldb, work( &
                        iwork ), lwork-iwork+1, info )
              ! generate right bidiagonalizing vectors of r in a
              ! (workspace: need 4*n-1, prefer 3*n+(n-1)*nb)
              call stdlib_I64_dorgbr( 'P', n, n, n, a, lda, work( itaup ),work( iwork ), lwork-iwork+&
                        1_ilp64, info )
              iwork = ie + n
              ! perform bidiagonal qr iteration
                ! multiply b by transpose of left singular vectors
                ! compute right singular vectors in a
              ! (workspace: need bdspac)
              call stdlib_I64_dbdsqr( 'U', n, n, 0_ilp64, nrhs, s, work( ie ), a, lda, dum,1_ilp64, b, ldb, work( &
                        iwork ), info )
              if( info/=0 )go to 70
              ! multiply b by reciprocals of singular values
              thr = max( rcond*s( 1_ilp64 ), sfmin )
              ...
              do i = 1, n
                 if( s( i )>thr ) then
                    call stdlib_I64_drscl( nrhs, s( i ), b( i, 1_ilp64 ), ldb )
                    rank = rank + 1_ilp64
                 else
                    call stdlib_I64_dlaset( 'F', 1_ilp64, nrhs, zero, zero, b( i, 1_ilp64 ), ldb )
                 end if
              end do
              ! multiply b by right singular vectors
              ! (workspace: need n, prefer n*nrhs)
              if( lwork>=ldb*nrhs .and. nrhs>1_ilp64 ) then
                 call stdlib_I64_dgemm( 'T', 'N', n, nrhs, n, one, a, lda, b, ldb, zero,work, ldb )
                           
                 call stdlib_I64_dlacpy( 'G', n, nrhs, work, ldb, b, ldb )
              else if( nrhs>1_ilp64 ) then
                 chunk = lwork / n
                 do i = 1, nrhs, chunk
                    bl = min( nrhs-i+1, chunk )
                    call stdlib_I64_dgemm( 'T', 'N', n, bl, n, one, a, lda, b( 1_ilp64, i ),ldb, zero, work,&
                               n )
                    call stdlib_I64_dlacpy( 'G', n, bl, work, n, b( 1_ilp64, i ), ldb )
                 end do
              else
                 call stdlib_I64_dgemv( 'T', n, n, one, a, lda, b, 1_ilp64, zero, work, 1_ilp64 )
                 call stdlib_I64_dcopy( n, work, 1_ilp64, b, 1_ilp64 )
              end if
           else if( n>=mnthr .and. lwork>=4_ilp64*m+m*m+max( m, 2_ilp64*m-4, nrhs, n-3*m ) ) then
              ! path 2a - underdetermined, with many more columns than rows
              ! and sufficient workspace for an efficient algorithm
              ...
              ! compute a=l*q
              ! (workspace: need 2*m, prefer m+m*nb)
              call stdlib_I64_dgelqf( m, n, a, lda, work( itau ), work( iwork ),lwork-iwork+1, info )
                        
              il = iwork
              ! copy l to work(il), zeroing out above it
              call stdlib_I64_dlacpy( 'L', m, m, a, lda, work( il ), ldwork )
              call stdlib_I64_dlaset( 'U', m-1, m-1, zero, zero, work( il+ldwork ),ldwork )
              ie = il + ldwork*m
              itauq = ie + m
              itaup = itauq + m
              iwork = itaup + m
              ! bidiagonalize l in work(il)
              ! (workspace: need m*m+5*m, prefer m*m+4*m+2*m*nb)
              call stdlib_I64_dgebrd( m, m, work( il ), ldwork, s, work( ie ),work( itauq ), work( &
                        itaup ), work( iwork ),lwork-iwork+1, info )
              ! multiply b by transpose of left bidiagonalizing vectors of l
              ! (workspace: need m*m+4*m+nrhs, prefer m*m+4*m+nrhs*nb)
              call stdlib_I64_dormbr( 'Q', 'L', 'T', m, nrhs, m, work( il ), ldwork,work( itauq ), b, &
                        ldb, work( iwork ),lwork-iwork+1, info )
              ! generate right bidiagonalizing vectors of r in work(il)
              ! (workspace: need m*m+5*m-1, prefer m*m+4*m+(m-1)*nb)
              call stdlib_I64_dorgbr( 'P', m, m, m, work( il ), ldwork, work( itaup ),work( iwork ), &
                        lwork-iwork+1, info )
              iwork = ie + m
              ! perform bidiagonal qr iteration,
                 ! computing right singular vectors of l in work(il) and
                 ! multiplying b by transpose of left singular vectors
              ! (workspace: need m*m+m+bdspac)
              call stdlib_I64_dbdsqr( 'U', m, m, 0_ilp64, nrhs, s, work( ie ), work( il ),ldwork, a, lda, b,&
                         ldb, work( iwork ), info )
              if( info/=0 )go to 70
              ! multiply b by reciprocals of singular values
              thr = max( rcond*s( 1_ilp64 ), sfmin )
              ...
              ! (workspace: need m*m+2*m, prefer m*m+m+m*nrhs)
              if( lwork>=ldb*nrhs+iwork-1 .and. nrhs>1_ilp64 ) then
                 call stdlib_I64_dgemm( 'T', 'N', m, nrhs, m, one, work( il ), ldwork,b, ldb, zero, &
                           work( iwork ), ldb )
                 call stdlib_I64_dlacpy( 'G', m, nrhs, work( iwork ), ldb, b, ldb )
              else if( nrhs>1_ilp64 ) then
                 chunk = ( lwork-iwork+1 ) / m
                 do i = 1, nrhs, chunk
                    bl = min( nrhs-i+1, chunk )
                    call stdlib_I64_dgemm( 'T', 'N', m, bl, m, one, work( il ), ldwork,b( 1_ilp64, i ), ldb,&
                               zero, work( iwork ), m )
                    call stdlib_I64_dlacpy( 'G', m, bl, work( iwork ), m, b( 1_ilp64, i ),ldb )
                 end do
              else
                 call stdlib_I64_dgemv( 'T', m, m, one, work( il ), ldwork, b( 1_ilp64, 1_ilp64 ),1_ilp64, zero, work( &
                           iwork ), 1_ilp64 )
                 call stdlib_I64_dcopy( m, work( iwork ), 1_ilp64, b( 1_ilp64, 1_ilp64 ), 1_ilp64 )
              end if
              ! zero out below first m rows of b
              call stdlib_I64_dlaset( 'F', n-m, nrhs, zero, zero, b( m+1, 1_ilp64 ), ldb )
              iwork = itau + m
              ! multiply transpose(q) by b
              ! (workspace: need m+nrhs, prefer m+nrhs*nb)
              call stdlib_I64_dormlq( 'L', 'T', n, nrhs, m, a, lda, work( itau ), b,ldb, work( iwork )&
                        , lwork-iwork+1, info )
           else
              ! path 2 - remaining underdetermined cases
              ie = 1_ilp64
              ...
              ! bidiagonalize a
              ! (workspace: need 3*m+n, prefer 3*m+(m+n)*nb)
              call stdlib_I64_dgebrd( m, n, a, lda, s, work( ie ), work( itauq ),work( itaup ), work( &
                        iwork ), lwork-iwork+1,info )
              ! multiply b by transpose of left bidiagonalizing vectors
              ! (workspace: need 3*m+nrhs, prefer 3*m+nrhs*nb)
              call stdlib_I64_dormbr( 'Q', 'L', 'T', m, nrhs, n, a, lda, work( itauq ),b, ldb, work( &
                        iwork ), lwork-iwork+1, info )
              ! generate right bidiagonalizing vectors in a
              ! (workspace: need 4*m, prefer 3*m+m*nb)
              call stdlib_I64_dorgbr( 'P', m, n, m, a, lda, work( itaup ),work( iwork ), lwork-iwork+&
                        1_ilp64, info )
              iwork = ie + m
              ! perform bidiagonal qr iteration,
                 ! computing right singular vectors of a in a and
                 ! multiplying b by transpose of left singular vectors
              ! (workspace: need bdspac)
              call stdlib_I64_dbdsqr( 'L', m, n, 0_ilp64, nrhs, s, work( ie ), a, lda, dum,1_ilp64, b, ldb, work( &
                        iwork ), info )
              if( info/=0 )go to 70
              ! multiply b by reciprocals of singular values
              thr = max( rcond*s( 1_ilp64 ), sfmin )
              ...
              ! (workspace: need n, prefer n*nrhs)
              if( lwork>=ldb*nrhs .and. nrhs>1_ilp64 ) then
                 call stdlib_I64_dgemm( 'T', 'N', n, nrhs, m, one, a, lda, b, ldb, zero,work, ldb )
                           
                 call stdlib_I64_dlacpy( 'F', n, nrhs, work, ldb, b, ldb )
              else if( nrhs>1_ilp64 ) then
                 chunk = lwork / n
                 do i = 1, nrhs, chunk
                    bl = min( nrhs-i+1, chunk )
                    call stdlib_I64_dgemm( 'T', 'N', n, bl, m, one, a, lda, b( 1_ilp64, i ),ldb, zero, work,&
                               n )
                    call stdlib_I64_dlacpy( 'F', n, bl, work, n, b( 1_ilp64, i ), ldb )
                 end do
              else
                 call stdlib_I64_dgemv( 'T', m, n, one, a, lda, b, 1_ilp64, zero, work, 1_ilp64 )
                 call stdlib_I64_dcopy( n, work, 1_ilp64, b, 1_ilp64 )
              end if
           end if
           ! undo scaling
           if( iascl==1_ilp64 ) then
              call stdlib_I64_dlascl( 'G', 0_ilp64, 0_ilp64, anrm, smlnum, n, nrhs, b, ldb, info )
              call stdlib_I64_dlascl( 'G', 0_ilp64, 0_ilp64, smlnum, anrm, minmn, 1_ilp64, s, minmn,info )
           else if( iascl==2_ilp64 ) then
              call stdlib_I64_dlascl( 'G', 0_ilp64, 0_ilp64, anrm, bignum, n, nrhs, b, ldb, info )
              call stdlib_I64_dlascl( 'G', 0_ilp64, 0_ilp64, bignum, anrm, minmn, 1_ilp64, s, minmn,info )
           end if
           if( ibscl==1_ilp64 ) then
              call stdlib_I64_dlascl( 'G', 0_ilp64, 0_ilp64, smlnum, bnrm, n, nrhs, b, ldb, info )
           else if( ibscl==2_ilp64 ) then
              call stdlib_I64_dlascl( 'G', 0_ilp64, 0_ilp64, bignum, bnrm, n, nrhs, b, ldb, info )
           end if
           70 continue
           work( 1_ilp64 ) = maxwrk
           return
     ...
     !! value.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           logical(lk) :: lquery
           integer(ilp64) :: bl, chunk, i, iascl, ibscl, ie, il, irwork, itau, itaup, itauq, iwork, &
                     ldwork, maxmn, maxwrk, minmn, minwrk, mm, mnthr
           integer(ilp64) :: lwork_cgeqrf, lwork_cunmqr, lwork_cgebrd, lwork_cunmbr, lwork_cungbr, &
                     lwork_cunmlq, lwork_cgelqf
           real(sp) :: anrm, bignum, bnrm, eps, sfmin, smlnum, thr
           ! Local Arrays 
           complex(sp) :: dum(1_ilp64)
           ! Intrinsic Functions 
           ! Executable Statements 
           ! test the input arguments
           info = 0_ilp64
           ...
           ! compute workspace
            ! (note: comments in the code beginning "workspace:" describe the
             ...
             ! cworkspace refers to complex workspace, and rworkspace refers
             ! to real workspace. nb refers to the optimal block size for the
             ! immediately following subroutine, as returned by stdlib_I64_ilaenv.)
           if( info==0_ilp64 ) then
              minwrk = 1_ilp64
              maxwrk = 1_ilp64
              if( minmn>0_ilp64 ) then
                 mm = m
                 mnthr = stdlib_I64_ilaenv( 6_ilp64, 'CGELSS', ' ', m, n, nrhs, -1_ilp64 )
                 if( m>=n .and. m>=mnthr ) then
                    ! path 1a - overdetermined, with many more rows than
                              ! columns
                    ! compute space needed for stdlib_I64_cgeqrf
                    call stdlib_I64_cgeqrf( m, n, a, lda, dum(1_ilp64), dum(1_ilp64), -1_ilp64, info )
                    lwork_cgeqrf = real( dum(1_ilp64),KIND=sp)
                    ! compute space needed for stdlib_I64_cunmqr
                    call stdlib_I64_cunmqr( 'L', 'C', m, nrhs, n, a, lda, dum(1_ilp64), b,ldb, dum(1_ilp64), -1_ilp64, &
                              info )
                    lwork_cunmqr = real( dum(1_ilp64),KIND=sp)
                    mm = n
                    maxwrk = max( maxwrk, n + n*stdlib_I64_ilaenv( 1_ilp64, 'CGEQRF', ' ', m,n, -1_ilp64, -1_ilp64 ) )
                              
                    maxwrk = max( maxwrk, n + nrhs*stdlib_I64_ilaenv( 1_ilp64, 'CUNMQR', 'LC',m, nrhs, n, -&
                              1_ilp64 ) )
                 end if
                 if( m>=n ) then
                    ! path 1 - overdetermined or exactly determined
                    ! compute space needed for stdlib_I64_cgebrd
                    call stdlib_I64_cgebrd( mm, n, a, lda, s, s, dum(1_ilp64), dum(1_ilp64), dum(1_ilp64),-1_ilp64, info )
                              
                    lwork_cgebrd = real( dum(1_ilp64),KIND=sp)
                    ! compute space needed for stdlib_I64_cunmbr
                    call stdlib_I64_cunmbr( 'Q', 'L', 'C', mm, nrhs, n, a, lda, dum(1_ilp64),b, ldb, dum(1_ilp64),&
                               -1_ilp64, info )
                    lwork_cunmbr = real( dum(1_ilp64),KIND=sp)
                    ! compute space needed for stdlib_I64_cungbr
                    call stdlib_I64_cungbr( 'P', n, n, n, a, lda, dum(1_ilp64),dum(1_ilp64), -1_ilp64, info )
                    lwork_cungbr = real( dum(1_ilp64),KIND=sp)
                    ! compute total workspace needed
                    maxwrk = max( maxwrk, 2_ilp64*n + lwork_cgebrd )
                    maxwrk = max( maxwrk, 2_ilp64*n + lwork_cunmbr )
                    maxwrk = max( maxwrk, 2_ilp64*n + lwork_cungbr )
                    maxwrk = max( maxwrk, n*nrhs )
                    minwrk = 2_ilp64*n + max( nrhs, m )
                 end if
                 if( n>m ) then
                    minwrk = 2_ilp64*m + max( nrhs, n )
                    if( n>=mnthr ) then
                       ! path 2a - underdetermined, with many more columns
                       ! than rows
                       ! compute space needed for stdlib_I64_cgelqf
                       call stdlib_I64_cgelqf( m, n, a, lda, dum(1_ilp64), dum(1_ilp64),-1_ilp64, info )
                       lwork_cgelqf = real( dum(1_ilp64),KIND=sp)
                       ! compute space needed for stdlib_I64_cgebrd
                       call stdlib_I64_cgebrd( m, m, a, lda, s, s, dum(1_ilp64), dum(1_ilp64),dum(1_ilp64), -1_ilp64, info )
                                 
                       lwork_cgebrd = real( dum(1_ilp64),KIND=sp)
                       ! compute space needed for stdlib_I64_cunmbr
                       call stdlib_I64_cunmbr( 'Q', 'L', 'C', m, nrhs, n, a, lda,dum(1_ilp64), b, ldb, dum(&
                                 1_ilp64), -1_ilp64, info )
                       lwork_cunmbr = real( dum(1_ilp64),KIND=sp)
                       ! compute space needed for stdlib_I64_cungbr
                       call stdlib_I64_cungbr( 'P', m, m, m, a, lda, dum(1_ilp64),dum(1_ilp64), -1_ilp64, info )
                       lwork_cungbr = real( dum(1_ilp64),KIND=sp)
                       ! compute space needed for stdlib_I64_cunmlq
                       call stdlib_I64_cunmlq( 'L', 'C', n, nrhs, m, a, lda, dum(1_ilp64),b, ldb, dum(1_ilp64), -&
                                 1_ilp64, info )
                       lwork_cunmlq = real( dum(1_ilp64),KIND=sp)
                       ! compute total workspace needed
                       maxwrk = m + lwork_cgelqf
                       maxwrk = max( maxwrk, 3_ilp64*m + m*m + lwork_cgebrd )
                       maxwrk = max( maxwrk, 3_ilp64*m + m*m + lwork_cunmbr )
                       maxwrk = max( maxwrk, 3_ilp64*m + m*m + lwork_cungbr )
                       if( nrhs>1_ilp64 ) then
                          maxwrk = max( maxwrk, m*m + m + m*nrhs )
                       else
                          maxwrk = max( maxwrk, m*m + 2_ilp64*m )
                       ...
                       ! path 2 - underdetermined
                       ! compute space needed for stdlib_I64_cgebrd
                       call stdlib_I64_cgebrd( m, n, a, lda, s, s, dum(1_ilp64), dum(1_ilp64),dum(1_ilp64), -1_ilp64, info )
                                 
                       lwork_cgebrd = real( dum(1_ilp64),KIND=sp)
                       ! compute space needed for stdlib_I64_cunmbr
                       call stdlib_I64_cunmbr( 'Q', 'L', 'C', m, nrhs, m, a, lda,dum(1_ilp64), b, ldb, dum(&
                                 1_ilp64), -1_ilp64, info )
                       lwork_cunmbr = real( dum(1_ilp64),KIND=sp)
                       ! compute space needed for stdlib_I64_cungbr
                       call stdlib_I64_cungbr( 'P', m, n, m, a, lda, dum(1_ilp64),dum(1_ilp64), -1_ilp64, info )
                       lwork_cungbr = real( dum(1_ilp64),KIND=sp)
                       maxwrk = 2_ilp64*m + lwork_cgebrd
                       maxwrk = max( maxwrk, 2_ilp64*m + lwork_cunmbr )
                       maxwrk = max( maxwrk, 2_ilp64*m + lwork_cungbr )
                       maxwrk = max( maxwrk, n*nrhs )
                    end if
                 end if
                 maxwrk = max( minwrk, maxwrk )
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CGELSS', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           call stdlib_I64_slabad( smlnum, bignum )
           ! scale a if max element outside range [smlnum,bignum]
           anrm = stdlib_I64_clange( 'M', m, n, a, lda, rwork )
           iascl = 0_ilp64
           if( anrm>zero .and. anrm<smlnum ) then
              ! scale matrix norm up to smlnum
              call stdlib_I64_clascl( 'G', 0_ilp64, 0_ilp64, anrm, smlnum, m, n, a, lda, info )
              iascl = 1_ilp64
           else if( anrm>bignum ) then
              ! scale matrix norm down to bignum
              call stdlib_I64_clascl( 'G', 0_ilp64, 0_ilp64, anrm, bignum, m, n, a, lda, info )
              iascl = 2_ilp64
           else if( anrm==zero ) then
              ! matrix all zero. return zero solution.
              call stdlib_I64_claset( 'F', max( m, n ), nrhs, czero, czero, b, ldb )
              call stdlib_I64_slaset( 'F', minmn, 1_ilp64, zero, zero, s, minmn )
              rank = 0_ilp64
              go to 70
           end if
           ! scale b if max element outside range [smlnum,bignum]
           bnrm = stdlib_I64_clange( 'M', m, nrhs, b, ldb, rwork )
           ibscl = 0_ilp64
           if( bnrm>zero .and. bnrm<smlnum ) then
              ! scale matrix norm up to smlnum
              call stdlib_I64_clascl( 'G', 0_ilp64, 0_ilp64, bnrm, smlnum, m, nrhs, b, ldb, info )
              ibscl = 1_ilp64
           else if( bnrm>bignum ) then
              ! scale matrix norm down to bignum
              call stdlib_I64_clascl( 'G', 0_ilp64, 0_ilp64, bnrm, bignum, m, nrhs, b, ldb, info )
              ibscl = 2_ilp64
           end if
           ! overdetermined case
           if( m>=n ) then
              ...
              if( m>=mnthr ) then
                 ...
                 ! (cworkspace: need 2*n, prefer n+n*nb)
                 ! (rworkspace: none)
                 call stdlib_I64_cgeqrf( m, n, a, lda, work( itau ), work( iwork ),lwork-iwork+1, &
                           info )
                 ! multiply b by transpose(q)
                 ! (cworkspace: need n+nrhs, prefer n+nrhs*nb)
                 ! (rworkspace: none)
                 call stdlib_I64_cunmqr( 'L', 'C', m, nrhs, n, a, lda, work( itau ), b,ldb, work( &
                           iwork ), lwork-iwork+1, info )
                 ! zero out below r
                 if( n>1_ilp64 )call stdlib_I64_claset( 'L', n-1, n-1, czero, czero, a( 2_ilp64, 1_ilp64 ),lda )
              end if
              ie = 1_ilp64
              itauq = 1_ilp64
              itaup = itauq + n
              ...
              ! (cworkspace: need 2*n+mm, prefer 2*n+(mm+n)*nb)
              ! (rworkspace: need n)
              call stdlib_I64_cgebrd( mm, n, a, lda, s, rwork( ie ), work( itauq ),work( itaup ), &
                        work( iwork ), lwork-iwork+1,info )
              ! multiply b by transpose of left bidiagonalizing vectors of r
              ! (cworkspace: need 2*n+nrhs, prefer 2*n+nrhs*nb)
              ! (rworkspace: none)
              call stdlib_I64_cunmbr( 'Q', 'L', 'C', mm, nrhs, n, a, lda, work( itauq ),b, ldb, work( &
                        iwork ), lwork-iwork+1, info )
              ! generate right bidiagonalizing vectors of r in a
              ! (cworkspace: need 3*n-1, prefer 2*n+(n-1)*nb)
              ! (rworkspace: none)
              call stdlib_I64_cungbr( 'P', n, n, n, a, lda, work( itaup ),work( iwork ), lwork-iwork+&
                        1_ilp64, info )
              irwork = ie + n
              ! perform bidiagonal qr iteration
                ! multiply b by transpose of left singular vectors
              ...
              ! (cworkspace: none)
              ! (rworkspace: need bdspac)
              call stdlib_I64_cbdsqr( 'U', n, n, 0_ilp64, nrhs, s, rwork( ie ), a, lda, dum,1_ilp64, b, ldb, &
                        rwork( irwork ), info )
              if( info/=0 )go to 70
              ! multiply b by reciprocals of singular values
              thr = max( rcond*s( 1_ilp64 ), sfmin )
              ...
              do i = 1, n
                 if( s( i )>thr ) then
                    call stdlib_I64_csrscl( nrhs, s( i ), b( i, 1_ilp64 ), ldb )
                    rank = rank + 1_ilp64
                 else
                    call stdlib_I64_claset( 'F', 1_ilp64, nrhs, czero, czero, b( i, 1_ilp64 ), ldb )
                 end if
              end do
              ! multiply b by right singular vectors
              ! (cworkspace: need n, prefer n*nrhs)
              ! (rworkspace: none)
              if( lwork>=ldb*nrhs .and. nrhs>1_ilp64 ) then
                 call stdlib_I64_cgemm( 'C', 'N', n, nrhs, n, cone, a, lda, b, ldb,czero, work, ldb )
                           
                 call stdlib_I64_clacpy( 'G', n, nrhs, work, ldb, b, ldb )
              else if( nrhs>1_ilp64 ) then
                 chunk = lwork / n
                 do i = 1, nrhs, chunk
                    bl = min( nrhs-i+1, chunk )
                    call stdlib_I64_cgemm( 'C', 'N', n, bl, n, cone, a, lda, b( 1_ilp64, i ),ldb, czero, &
                              work, n )
                    call stdlib_I64_clacpy( 'G', n, bl, work, n, b( 1_ilp64, i ), ldb )
                 end do
              else
                 call stdlib_I64_cgemv( 'C', n, n, cone, a, lda, b, 1_ilp64, czero, work, 1_ilp64 )
                 call stdlib_I64_ccopy( n, work, 1_ilp64, b, 1_ilp64 )
              end if
           else if( n>=mnthr .and. lwork>=3_ilp64*m+m*m+max( m, nrhs, n-2*m ) )then
              ! underdetermined case, m much less than n
              ! path 2a - underdetermined, with many more columns than rows
              ! and sufficient workspace for an efficient algorithm
              ldwork = m
              if( lwork>=3_ilp64*m+m*lda+max( m, nrhs, n-2*m ) )ldwork = lda
              itau = 1_ilp64
              iwork = m + 1_ilp64
              ! compute a=l*q
              ! (cworkspace: need 2*m, prefer m+m*nb)
              ! (rworkspace: none)
              call stdlib_I64_cgelqf( m, n, a, lda, work( itau ), work( iwork ),lwork-iwork+1, info )
                        
              il = iwork
              ! copy l to work(il), zeroing out above it
              call stdlib_I64_clacpy( 'L', m, m, a, lda, work( il ), ldwork )
              call stdlib_I64_claset( 'U', m-1, m-1, czero, czero, work( il+ldwork ),ldwork )
              ie = 1_ilp64
              itauq = il + ldwork*m
              itaup = itauq + m
              iwork = itaup + m
              ...
              ! (cworkspace: need m*m+4*m, prefer m*m+3*m+2*m*nb)
              ! (rworkspace: need m)
              call stdlib_I64_cgebrd( m, m, work( il ), ldwork, s, rwork( ie ),work( itauq ), work( &
                        itaup ), work( iwork ),lwork-iwork+1, info )
              ! multiply b by transpose of left bidiagonalizing vectors of l
              ! (cworkspace: need m*m+3*m+nrhs, prefer m*m+3*m+nrhs*nb)
              ! (rworkspace: none)
              call stdlib_I64_cunmbr( 'Q', 'L', 'C', m, nrhs, m, work( il ), ldwork,work( itauq ), b, &
                        ldb, work( iwork ),lwork-iwork+1, info )
              ! generate right bidiagonalizing vectors of r in work(il)
              ! (cworkspace: need m*m+4*m-1, prefer m*m+3*m+(m-1)*nb)
              ! (rworkspace: none)
              call stdlib_I64_cungbr( 'P', m, m, m, work( il ), ldwork, work( itaup ),work( iwork ), &
                        lwork-iwork+1, info )
              irwork = ie + m
              ! perform bidiagonal qr iteration, computing right singular
              ! vectors of l in work(il) and multiplying b by transpose of
              ...
              ! (cworkspace: need m*m)
              ! (rworkspace: need bdspac)
              call stdlib_I64_cbdsqr( 'U', m, m, 0_ilp64, nrhs, s, rwork( ie ), work( il ),ldwork, a, lda, &
                        b, ldb, rwork( irwork ), info )
              if( info/=0 )go to 70
              ! multiply b by reciprocals of singular values
              thr = max( rcond*s( 1_ilp64 ), sfmin )
              ...
              ! (rworkspace: none)
              if( lwork>=ldb*nrhs+iwork-1 .and. nrhs>1_ilp64 ) then
                 call stdlib_I64_cgemm( 'C', 'N', m, nrhs, m, cone, work( il ), ldwork,b, ldb, czero, &
                           work( iwork ), ldb )
                 call stdlib_I64_clacpy( 'G', m, nrhs, work( iwork ), ldb, b, ldb )
              else if( nrhs>1_ilp64 ) then
                 chunk = ( lwork-iwork+1 ) / m
                 do i = 1, nrhs, chunk
                    bl = min( nrhs-i+1, chunk )
                    call stdlib_I64_cgemm( 'C', 'N', m, bl, m, cone, work( il ), ldwork,b( 1_ilp64, i ), &
                              ldb, czero, work( iwork ), m )
                    call stdlib_I64_clacpy( 'G', m, bl, work( iwork ), m, b( 1_ilp64, i ),ldb )
                 end do
              else
                 call stdlib_I64_cgemv( 'C', m, m, cone, work( il ), ldwork, b( 1_ilp64, 1_ilp64 ),1_ilp64, czero, work(&
                            iwork ), 1_ilp64 )
                 call stdlib_I64_ccopy( m, work( iwork ), 1_ilp64, b( 1_ilp64, 1_ilp64 ), 1_ilp64 )
              end if
              ! zero out below first m rows of b
              call stdlib_I64_claset( 'F', n-m, nrhs, czero, czero, b( m+1, 1_ilp64 ), ldb )
              iwork = itau + m
              ! multiply transpose(q) by b
              ! (cworkspace: need m+nrhs, prefer m+nhrs*nb)
              ! (rworkspace: none)
              call stdlib_I64_cunmlq( 'L', 'C', n, nrhs, m, a, lda, work( itau ), b,ldb, work( iwork )&
                        , lwork-iwork+1, info )
           else
              ! path 2 - remaining underdetermined cases
              ie = 1_ilp64
              ...
              ! (cworkspace: need 3*m, prefer 2*m+(m+n)*nb)
              ! (rworkspace: need n)
              call stdlib_I64_cgebrd( m, n, a, lda, s, rwork( ie ), work( itauq ),work( itaup ), work(&
                         iwork ), lwork-iwork+1,info )
              ! multiply b by transpose of left bidiagonalizing vectors
              ! (cworkspace: need 2*m+nrhs, prefer 2*m+nrhs*nb)
              ! (rworkspace: none)
              call stdlib_I64_cunmbr( 'Q', 'L', 'C', m, nrhs, n, a, lda, work( itauq ),b, ldb, work( &
                        iwork ), lwork-iwork+1, info )
              ! generate right bidiagonalizing vectors in a
              ! (cworkspace: need 3*m, prefer 2*m+m*nb)
              ! (rworkspace: none)
              call stdlib_I64_cungbr( 'P', m, n, m, a, lda, work( itaup ),work( iwork ), lwork-iwork+&
                        1_ilp64, info )
              irwork = ie + m
              ! perform bidiagonal qr iteration,
                 ! computing right singular vectors of a in a and
              ...
              ! (cworkspace: none)
              ! (rworkspace: need bdspac)
              call stdlib_I64_cbdsqr( 'L', m, n, 0_ilp64, nrhs, s, rwork( ie ), a, lda, dum,1_ilp64, b, ldb, &
                        rwork( irwork ), info )
              if( info/=0 )go to 70
              ! multiply b by reciprocals of singular values
              thr = max( rcond*s( 1_ilp64 ), sfmin )
              ...
              ! (rworkspace: none)
              if( lwork>=ldb*nrhs .and. nrhs>1_ilp64 ) then
                 call stdlib_I64_cgemm( 'C', 'N', n, nrhs, m, cone, a, lda, b, ldb,czero, work, ldb )
                 ...
                 do i = 1, nrhs, chunk
                    bl = min( nrhs-i+1, chunk )
                    call stdlib_I64_cgemm( 'C', 'N', n, bl, m, cone, a, lda, b( 1_ilp64, i ),ldb, czero, &
                              work, n )
                    call stdlib_I64_clacpy( 'F', n, bl, work, n, b( 1_ilp64, i ), ldb )
                 end do
              else
                 call stdlib_I64_cgemv( 'C', m, n, cone, a, lda, b, 1_ilp64, czero, work, 1_ilp64 )
                 call stdlib_I64_ccopy( n, work, 1_ilp64, b, 1_ilp64 )
              end if
           end if
           ! undo scaling
           if( iascl==1_ilp64 ) then
              call stdlib_I64_clascl( 'G', 0_ilp64, 0_ilp64, anrm, smlnum, n, nrhs, b, ldb, info )
              call stdlib_I64_slascl( 'G', 0_ilp64, 0_ilp64, smlnum, anrm, minmn, 1_ilp64, s, minmn,info )
           else if( iascl==2_ilp64 ) then
              call stdlib_I64_clascl( 'G', 0_ilp64, 0_ilp64, anrm, bignum, n, nrhs, b, ldb, info )
              call stdlib_I64_slascl( 'G', 0_ilp64, 0_ilp64, bignum, anrm, minmn, 1_ilp64, s, minmn,info )
           end if
           if( ibscl==1_ilp64 ) then
              call stdlib_I64_clascl( 'G', 0_ilp64, 0_ilp64, smlnum, bnrm, n, nrhs, b, ldb, info )
           else if( ibscl==2_ilp64 ) then
              call stdlib_I64_clascl( 'G', 0_ilp64, 0_ilp64, bignum, bnrm, n, nrhs, b, ldb, info )
           end if
           70 continue
           work( 1_ilp64 ) = maxwrk
           return
     ...
     !! value.
        ...
        ! =====================================================================
           ...
           integer(ilp64) :: bl, chunk, i, iascl, ibscl, ie, il, irwork, itau, itaup, itauq, iwork, &
                     ldwork, maxmn, maxwrk, minmn, minwrk, mm, mnthr
           integer(ilp64) :: lwork_zgeqrf, lwork_zunmqr, lwork_zgebrd, lwork_zunmbr, lwork_zungbr, &
                     lwork_zunmlq, lwork_zgelqf
           real(dp) :: anrm, bignum, bnrm, eps, sfmin, smlnum, thr
           ! Local Arrays 
           complex(dp) :: dum(1_ilp64)
           ! Intrinsic Functions 
           ! Executable Statements 
           ! test the input arguments
           info = 0_ilp64
           ...
           if( info==0_ilp64 ) then
              ...
              if( minmn>0_ilp64 ) then
                 mm = m
                 mnthr = stdlib_I64_ilaenv( 6_ilp64, 'ZGELSS', ' ', m, n, nrhs, -1_ilp64 )
                 if( m>=n .and. m>=mnthr ) then
                    ! path 1a - overdetermined, with many more rows than
                              ! columns
                    ! compute space needed for stdlib_I64_zgeqrf
                    call stdlib_I64_zgeqrf( m, n, a, lda, dum(1_ilp64), dum(1_ilp64), -1_ilp64, info )
                    lwork_zgeqrf = real( dum(1_ilp64),KIND=dp)
                    ! compute space needed for stdlib_I64_zunmqr
                    call stdlib_I64_zunmqr( 'L', 'C', m, nrhs, n, a, lda, dum(1_ilp64), b,ldb, dum(1_ilp64), -1_ilp64, &
                              info )
                    lwork_zunmqr = real( dum(1_ilp64),KIND=dp)
                    mm = n
                    maxwrk = max( maxwrk, n + n*stdlib_I64_ilaenv( 1_ilp64, 'ZGEQRF', ' ', m,n, -1_ilp64, -1_ilp64 ) )
                              
                    maxwrk = max( maxwrk, n + nrhs*stdlib_I64_ilaenv( 1_ilp64, 'ZUNMQR', 'LC',m, nrhs, n, -&
                              1_ilp64 ) )
                 end if
                 if( m>=n ) then
                    ! path 1 - overdetermined or exactly determined
                    ! compute space needed for stdlib_I64_zgebrd
                    call stdlib_I64_zgebrd( mm, n, a, lda, s, s, dum(1_ilp64), dum(1_ilp64), dum(1_ilp64),-1_ilp64, info )
                              
                    lwork_zgebrd = real( dum(1_ilp64),KIND=dp)
                    ! compute space needed for stdlib_I64_zunmbr
                    call stdlib_I64_zunmbr( 'Q', 'L', 'C', mm, nrhs, n, a, lda, dum(1_ilp64),b, ldb, dum(1_ilp64),&
                               -1_ilp64, info )
                    lwork_zunmbr = real( dum(1_ilp64),KIND=dp)
                    ! compute space needed for stdlib_I64_zungbr
                    call stdlib_I64_zungbr( 'P', n, n, n, a, lda, dum(1_ilp64),dum(1_ilp64), -1_ilp64, info )
                    lwork_zungbr = real( dum(1_ilp64),KIND=dp)
                    ! compute total workspace needed
                    maxwrk = max( maxwrk, 2_ilp64*n + lwork_zgebrd )
                    maxwrk = max( maxwrk, 2_ilp64*n + lwork_zunmbr )
                    maxwrk = max( maxwrk, 2_ilp64*n + lwork_zungbr )
                    maxwrk = max( maxwrk, n*nrhs )
                    minwrk = 2_ilp64*n + max( nrhs, m )
                 end if
                 if( n>m ) then
                    ...
                    if( n>=mnthr ) then
                       ! path 2a - underdetermined, with many more columns
                       ! than rows
                       ! compute space needed for stdlib_I64_zgelqf
                       call stdlib_I64_zgelqf( m, n, a, lda, dum(1_ilp64), dum(1_ilp64),-1_ilp64, info )
                       lwork_zgelqf = real( dum(1_ilp64),KIND=dp)
                       ! compute space needed for stdlib_I64_zgebrd
                       call stdlib_I64_zgebrd( m, m, a, lda, s, s, dum(1_ilp64), dum(1_ilp64),dum(1_ilp64), -1_ilp64, info )
                                 
                       lwork_zgebrd = real( dum(1_ilp64),KIND=dp)
                       ! compute space needed for stdlib_I64_zunmbr
                       call stdlib_I64_zunmbr( 'Q', 'L', 'C', m, nrhs, n, a, lda,dum(1_ilp64), b, ldb, dum(&
                                 1_ilp64), -1_ilp64, info )
                       lwork_zunmbr = real( dum(1_ilp64),KIND=dp)
                       ! compute space needed for stdlib_I64_zungbr
                       call stdlib_I64_zungbr( 'P', m, m, m, a, lda, dum(1_ilp64),dum(1_ilp64), -1_ilp64, info )
                       lwork_zungbr = real( dum(1_ilp64),KIND=dp)
                       ! compute space needed for stdlib_I64_zunmlq
                       call stdlib_I64_zunmlq( 'L', 'C', n, nrhs, m, a, lda, dum(1_ilp64),b, ldb, dum(1_ilp64), -&
                                 1_ilp64, info )
                       lwork_zunmlq = real( dum(1_ilp64),KIND=dp)
                       ! compute total workspace needed
                       maxwrk = m + lwork_zgelqf
                       maxwrk = max( maxwrk, 3_ilp64*m + m*m + lwork_zgebrd )
                       maxwrk = max( maxwrk, 3_ilp64*m + m*m + lwork_zunmbr )
                       maxwrk = max( maxwrk, 3_ilp64*m + m*m + lwork_zungbr )
                       if( nrhs>1_ilp64 ) then
                          maxwrk = max( maxwrk, m*m + m + m*nrhs )
                       else
                          maxwrk = max( maxwrk, m*m + 2_ilp64*m )
                       ...
                       ! path 2 - underdetermined
                       ! compute space needed for stdlib_I64_zgebrd
                       call stdlib_I64_zgebrd( m, n, a, lda, s, s, dum(1_ilp64), dum(1_ilp64),dum(1_ilp64), -1_ilp64, info )
                                 
                       lwork_zgebrd = real( dum(1_ilp64),KIND=dp)
                       ! compute space needed for stdlib_I64_zunmbr
                       call stdlib_I64_zunmbr( 'Q', 'L', 'C', m, nrhs, m, a, lda,dum(1_ilp64), b, ldb, dum(&
                                 1_ilp64), -1_ilp64, info )
                       lwork_zunmbr = real( dum(1_ilp64),KIND=dp)
                       ! compute space needed for stdlib_I64_zungbr
                       call stdlib_I64_zungbr( 'P', m, n, m, a, lda, dum(1_ilp64),dum(1_ilp64), -1_ilp64, info )
                       lwork_zungbr = real( dum(1_ilp64),KIND=dp)
                       maxwrk = 2_ilp64*m + lwork_zgebrd
                       maxwrk = max( maxwrk, 2_ilp64*m + lwork_zunmbr )
                       maxwrk = max( maxwrk, 2_ilp64*m + lwork_zungbr )
                       maxwrk = max( maxwrk, n*nrhs )
                    end if
                 end if
                 maxwrk = max( minwrk, maxwrk )
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZGELSS', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           call stdlib_I64_dlabad( smlnum, bignum )
           ! scale a if max element outside range [smlnum,bignum]
           anrm = stdlib_I64_zlange( 'M', m, n, a, lda, rwork )
           iascl = 0_ilp64
           if( anrm>zero .and. anrm<smlnum ) then
              ! scale matrix norm up to smlnum
              call stdlib_I64_zlascl( 'G', 0_ilp64, 0_ilp64, anrm, smlnum, m, n, a, lda, info )
              iascl = 1_ilp64
           else if( anrm>bignum ) then
              ! scale matrix norm down to bignum
              call stdlib_I64_zlascl( 'G', 0_ilp64, 0_ilp64, anrm, bignum, m, n, a, lda, info )
              iascl = 2_ilp64
           else if( anrm==zero ) then
              ! matrix all zero. return zero solution.
              call stdlib_I64_zlaset( 'F', max( m, n ), nrhs, czero, czero, b, ldb )
              call stdlib_I64_dlaset( 'F', minmn, 1_ilp64, zero, zero, s, minmn )
              rank = 0_ilp64
              go to 70
           end if
           ! scale b if max element outside range [smlnum,bignum]
           bnrm = stdlib_I64_zlange( 'M', m, nrhs, b, ldb, rwork )
           ibscl = 0_ilp64
           if( bnrm>zero .and. bnrm<smlnum ) then
              ! scale matrix norm up to smlnum
              call stdlib_I64_zlascl( 'G', 0_ilp64, 0_ilp64, bnrm, smlnum, m, nrhs, b, ldb, info )
              ibscl = 1_ilp64
           else if( bnrm>bignum ) then
              ! scale matrix norm down to bignum
              call stdlib_I64_zlascl( 'G', 0_ilp64, 0_ilp64, bnrm, bignum, m, nrhs, b, ldb, info )
              ibscl = 2_ilp64
           end if
           ! overdetermined case
           if( m>=n ) then
              ...
              if( m>=mnthr ) then
                 ...
                 ! (cworkspace: need 2*n, prefer n+n*nb)
                 ! (rworkspace: none)
                 call stdlib_I64_zgeqrf( m, n, a, lda, work( itau ), work( iwork ),lwork-iwork+1, &
                           info )
                 ! multiply b by transpose(q)
                 ! (cworkspace: need n+nrhs, prefer n+nrhs*nb)
                 ! (rworkspace: none)
                 call stdlib_I64_zunmqr( 'L', 'C', m, nrhs, n, a, lda, work( itau ), b,ldb, work( &
                           iwork ), lwork-iwork+1, info )
                 ! zero out below r
                 if( n>1_ilp64 )call stdlib_I64_zlaset( 'L', n-1, n-1, czero, czero, a( 2_ilp64, 1_ilp64 ),lda )
              end if
              ie = 1_ilp64
              itauq = 1_ilp64
              itaup = itauq + n
              ...
              ! (cworkspace: need 2*n+mm, prefer 2*n+(mm+n)*nb)
              ! (rworkspace: need n)
              call stdlib_I64_zgebrd( mm, n, a, lda, s, rwork( ie ), work( itauq ),work( itaup ), &
                        work( iwork ), lwork-iwork+1,info )
              ! multiply b by transpose of left bidiagonalizing vectors of r
              ! (cworkspace: need 2*n+nrhs, prefer 2*n+nrhs*nb)
              ! (rworkspace: none)
              call stdlib_I64_zunmbr( 'Q', 'L', 'C', mm, nrhs, n, a, lda, work( itauq ),b, ldb, work( &
                        iwork ), lwork-iwork+1, info )
              ! generate right bidiagonalizing vectors of r in a
              ! (cworkspace: need 3*n-1, prefer 2*n+(n-1)*nb)
              ! (rworkspace: none)
              call stdlib_I64_zungbr( 'P', n, n, n, a, lda, work( itaup ),work( iwork ), lwork-iwork+&
                        1_ilp64, info )
              irwork = ie + n
              ! perform bidiagonal qr iteration
                ! multiply b by transpose of left singular vectors
              ...
              ! (cworkspace: none)
              ! (rworkspace: need bdspac)
              call stdlib_I64_zbdsqr( 'U', n, n, 0_ilp64, nrhs, s, rwork( ie ), a, lda, dum,1_ilp64, b, ldb, &
                        rwork( irwork ), info )
              if( info/=0 )go to 70
              ! multiply b by reciprocals of singular values
              thr = max( rcond*s( 1_ilp64 ), sfmin )
              ...
              do i = 1, n
                 if( s( i )>thr ) then
                    call stdlib_I64_zdrscl( nrhs, s( i ), b( i, 1_ilp64 ), ldb )
                    rank = rank + 1_ilp64
                 else
                    call stdlib_I64_zlaset( 'F', 1_ilp64, nrhs, czero, czero, b( i, 1_ilp64 ), ldb )
                 end if
              end do
              ! multiply b by right singular vectors
              ! (cworkspace: need n, prefer n*nrhs)
              ! (rworkspace: none)
              if( lwork>=ldb*nrhs .and. nrhs>1_ilp64 ) then
                 call stdlib_I64_zgemm( 'C', 'N', n, nrhs, n, cone, a, lda, b, ldb,czero, work, ldb )
                           
                 call stdlib_I64_zlacpy( 'G', n, nrhs, work, ldb, b, ldb )
              else if( nrhs>1_ilp64 ) then
                 chunk = lwork / n
                 do i = 1, nrhs, chunk
                    bl = min( nrhs-i+1, chunk )
                    call stdlib_I64_zgemm( 'C', 'N', n, bl, n, cone, a, lda, b( 1_ilp64, i ),ldb, czero, &
                              work, n )
                    call stdlib_I64_zlacpy( 'G', n, bl, work, n, b( 1_ilp64, i ), ldb )
                 end do
              else
                 call stdlib_I64_zgemv( 'C', n, n, cone, a, lda, b, 1_ilp64, czero, work, 1_ilp64 )
                 call stdlib_I64_zcopy( n, work, 1_ilp64, b, 1_ilp64 )
              end if
           else if( n>=mnthr .and. lwork>=3_ilp64*m+m*m+max( m, nrhs, n-2*m ) )then
              ! underdetermined case, m much less than n
              ! path 2a - underdetermined, with many more columns than rows
              ...
              ! (cworkspace: need 2*m, prefer m+m*nb)
              ! (rworkspace: none)
              call stdlib_I64_zgelqf( m, n, a, lda, work( itau ), work( iwork ),lwork-iwork+1, info )
                        
              il = iwork
              ! copy l to work(il), zeroing out above it
              call stdlib_I64_zlacpy( 'L', m, m, a, lda, work( il ), ldwork )
              call stdlib_I64_zlaset( 'U', m-1, m-1, czero, czero, work( il+ldwork ),ldwork )
              ie = 1_ilp64
              itauq = il + ldwork*m
              itaup = itauq + m
              iwork = itaup + m
              ...
              ! (cworkspace: need m*m+4*m, prefer m*m+3*m+2*m*nb)
              ! (rworkspace: need m)
              call stdlib_I64_zgebrd( m, m, work( il ), ldwork, s, rwork( ie ),work( itauq ), work( &
                        itaup ), work( iwork ),lwork-iwork+1, info )
              ! multiply b by transpose of left bidiagonalizing vectors of l
              ! (cworkspace: need m*m+3*m+nrhs, prefer m*m+3*m+nrhs*nb)
              ! (rworkspace: none)
              call stdlib_I64_zunmbr( 'Q', 'L', 'C', m, nrhs, m, work( il ), ldwork,work( itauq ), b, &
                        ldb, work( iwork ),lwork-iwork+1, info )
              ! generate right bidiagonalizing vectors of r in work(il)
              ! (cworkspace: need m*m+4*m-1, prefer m*m+3*m+(m-1)*nb)
              ! (rworkspace: none)
              call stdlib_I64_zungbr( 'P', m, m, m, work( il ), ldwork, work( itaup ),work( iwork ), &
                        lwork-iwork+1, info )
              irwork = ie + m
              ! perform bidiagonal qr iteration, computing right singular
              ! vectors of l in work(il) and multiplying b by transpose of
              ...
              ! (cworkspace: need m*m)
              ! (rworkspace: need bdspac)
              call stdlib_I64_zbdsqr( 'U', m, m, 0_ilp64, nrhs, s, rwork( ie ), work( il ),ldwork, a, lda, &
                        b, ldb, rwork( irwork ), info )
              if( info/=0 )go to 70
              ! multiply b by reciprocals of singular values
              thr = max( rcond*s( 1_ilp64 ), sfmin )
              ...
              ! (rworkspace: none)
              if( lwork>=ldb*nrhs+iwork-1 .and. nrhs>1_ilp64 ) then
                 call stdlib_I64_zgemm( 'C', 'N', m, nrhs, m, cone, work( il ), ldwork,b, ldb, czero, &
                           work( iwork ), ldb )
                 call stdlib_I64_zlacpy( 'G', m, nrhs, work( iwork ), ldb, b, ldb )
              else if( nrhs>1_ilp64 ) then
                 chunk = ( lwork-iwork+1 ) / m
                 do i = 1, nrhs, chunk
                    bl = min( nrhs-i+1, chunk )
                    call stdlib_I64_zgemm( 'C', 'N', m, bl, m, cone, work( il ), ldwork,b( 1_ilp64, i ), &
                              ldb, czero, work( iwork ), m )
                    call stdlib_I64_zlacpy( 'G', m, bl, work( iwork ), m, b( 1_ilp64, i ),ldb )
                 end do
              else
                 call stdlib_I64_zgemv( 'C', m, m, cone, work( il ), ldwork, b( 1_ilp64, 1_ilp64 ),1_ilp64, czero, work(&
                            iwork ), 1_ilp64 )
                 call stdlib_I64_zcopy( m, work( iwork ), 1_ilp64, b( 1_ilp64, 1_ilp64 ), 1_ilp64 )
              end if
              ! zero out below first m rows of b
              call stdlib_I64_zlaset( 'F', n-m, nrhs, czero, czero, b( m+1, 1_ilp64 ), ldb )
              iwork = itau + m
              ! multiply transpose(q) by b
              ! (cworkspace: need m+nrhs, prefer m+nhrs*nb)
              ! (rworkspace: none)
              call stdlib_I64_zunmlq( 'L', 'C', n, nrhs, m, a, lda, work( itau ), b,ldb, work( iwork )&
                        , lwork-iwork+1, info )
           else
              ! path 2 - remaining underdetermined cases
              ie = 1_ilp64
              ...
              ! (cworkspace: need 3*m, prefer 2*m+(m+n)*nb)
              ! (rworkspace: need n)
              call stdlib_I64_zgebrd( m, n, a, lda, s, rwork( ie ), work( itauq ),work( itaup ), work(&
                         iwork ), lwork-iwork+1,info )
              ! multiply b by transpose of left bidiagonalizing vectors
              ! (cworkspace: need 2*m+nrhs, prefer 2*m+nrhs*nb)
              ! (rworkspace: none)
              call stdlib_I64_zunmbr( 'Q', 'L', 'C', m, nrhs, n, a, lda, work( itauq ),b, ldb, work( &
                        iwork ), lwork-iwork+1, info )
              ! generate right bidiagonalizing vectors in a
              ! (cworkspace: need 3*m, prefer 2*m+m*nb)
              ! (rworkspace: none)
              call stdlib_I64_zungbr( 'P', m, n, m, a, lda, work( itaup ),work( iwork ), lwork-iwork+&
                        1_ilp64, info )
              irwork = ie + m
              ! perform bidiagonal qr iteration,
                 ! computing right singular vectors of a in a and
              ...
              ! (cworkspace: none)
              ! (rworkspace: need bdspac)
              call stdlib_I64_zbdsqr( 'L', m, n, 0_ilp64, nrhs, s, rwork( ie ), a, lda, dum,1_ilp64, b, ldb, &
                        rwork( irwork ), info )
              if( info/=0 )go to 70
              ! multiply b by reciprocals of singular values
              thr = max( rcond*s( 1_ilp64 ), sfmin )
              ...
              ! (rworkspace: none)
              if( lwork>=ldb*nrhs .and. nrhs>1_ilp64 ) then
                 call stdlib_I64_zgemm( 'C', 'N', n, nrhs, m, cone, a, lda, b, ldb,czero, work, ldb )
                 ...
                 do i = 1, nrhs, chunk
                    bl = min( nrhs-i+1, chunk )
                    call stdlib_I64_zgemm( 'C', 'N', n, bl, m, cone, a, lda, b( 1_ilp64, i ),ldb, czero, &
                              work, n )
                    call stdlib_I64_zlacpy( 'F', n, bl, work, n, b( 1_ilp64, i ), ldb )
                 end do
              else
                 call stdlib_I64_zgemv( 'C', m, n, cone, a, lda, b, 1_ilp64, czero, work, 1_ilp64 )
                 call stdlib_I64_zcopy( n, work, 1_ilp64, b, 1_ilp64 )
              end if
           end if
           ! undo scaling
           if( iascl==1_ilp64 ) then
              call stdlib_I64_zlascl( 'G', 0_ilp64, 0_ilp64, anrm, smlnum, n, nrhs, b, ldb, info )
              call stdlib_I64_dlascl( 'G', 0_ilp64, 0_ilp64, smlnum, anrm, minmn, 1_ilp64, s, minmn,info )
           else if( iascl==2_ilp64 ) then
              call stdlib_I64_zlascl( 'G', 0_ilp64, 0_ilp64, anrm, bignum, n, nrhs, b, ldb, info )
              call stdlib_I64_dlascl( 'G', 0_ilp64, 0_ilp64, bignum, anrm, minmn, 1_ilp64, s, minmn,info )
           end if
           if( ibscl==1_ilp64 ) then
              call stdlib_I64_zlascl( 'G', 0_ilp64, 0_ilp64, smlnum, bnrm, n, nrhs, b, ldb, info )
           else if( ibscl==2_ilp64 ) then
              call stdlib_I64_zlascl( 'G', 0_ilp64, 0_ilp64, bignum, bnrm, n, nrhs, b, ldb, info )
           end if
           70 continue
           work( 1_ilp64 ) = maxwrk
           return
     ...
     !! more simple.
        ...
        ! =====================================================================
           ! Parameters 
           integer(ilp64), parameter :: imax = 1_ilp64
           integer(ilp64), parameter :: imin = 2_ilp64
           
           
           ! Local Scalars 
           logical(lk) :: lquery
           integer(ilp64) :: i, iascl, ibscl, ismax, ismin, j, lwkmin, lwkopt, mn, nb, nb1, nb2, &
                     nb3, nb4
           real(sp) :: anrm, bignum, bnrm, c1, c2, s1, s2, smax, smaxpr, smin, sminpr, smlnum, &
                     wsize
           ! Intrinsic Functions 
           ...
           if( m<0_ilp64 ) then
              ...
              info = -5_ilp64
           else if( ldb<max( 1_ilp64, m, n ) ) then
              info = -7_ilp64
           end if
           ! figure out optimal block size
           if( info==0_ilp64 ) then
              if( mn==0_ilp64 .or. nrhs==0_ilp64 ) then
                 lwkmin = 1_ilp64
                 lwkopt = 1_ilp64
              else
                 nb1 = stdlib_I64_ilaenv( 1_ilp64, 'SGEQRF', ' ', m, n, -1_ilp64, -1_ilp64 )
                 nb2 = stdlib_I64_ilaenv( 1_ilp64, 'SGERQF', ' ', m, n, -1_ilp64, -1_ilp64 )
                 nb3 = stdlib_I64_ilaenv( 1_ilp64, 'SORMQR', ' ', m, n, nrhs, -1_ilp64 )
                 nb4 = stdlib_I64_ilaenv( 1_ilp64, 'SORMRQ', ' ', m, n, nrhs, -1_ilp64 )
                 nb = max( nb1, nb2, nb3, nb4 )
                 lwkmin = mn + max( 2_ilp64*mn, n + 1_ilp64, mn + nrhs )
                 lwkopt = max( lwkmin,mn + 2_ilp64*n + nb*( n + 1_ilp64 ), 2_ilp64*mn + nb*nrhs )
              end if
              work( 1_ilp64 ) = lwkopt
              if( lwork<lwkmin .and. .not.lquery ) then
                 info = -12_ilp64
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SGELSY', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           ! get machine parameters
           smlnum = stdlib_I64_slamch( 'S' ) / stdlib_I64_slamch( 'P' )
           bignum = one / smlnum
           call stdlib_I64_slabad( smlnum, bignum )
           ! scale a, b if max entries outside range [smlnum,bignum]
           anrm = stdlib_I64_slange( 'M', m, n, a, lda, work )
           ...
           ! compute qr factorization with column pivoting of a:
              ! a * p = q * r
           call stdlib_I64_sgeqp3( m, n, a, lda, jpvt, work( 1_ilp64 ), work( mn+1 ),lwork-mn, info )
           ...
           work( ismin ) = one
           work( ismax ) = one
           smax = abs( a( 1_ilp64, 1_ilp64 ) )
           smin = smax
           if( abs( a( 1_ilp64, 1_ilp64 ) )==zero ) then
              rank = 0_ilp64
              call stdlib_I64_slaset( 'F', max( m, n ), nrhs, zero, zero, b, ldb )
              go to 70
           else
           ...
           if( rank<mn ) then
              i = rank + 1_ilp64
              call stdlib_I64_slaic1( imin, rank, work( ismin ), smin, a( 1_ilp64, i ),a( i, i ), sminpr, &
                        s1, c1 )
              call stdlib_I64_slaic1( imax, rank, work( ismax ), smax, a( 1_ilp64, i ),a( i, i ), smaxpr, &
                        s2, c2 )
              if( smaxpr*rcond<=sminpr ) then
                 do i = 1, rank
                    work( ismin+i-1 ) = s1*work( ismin+i-1 )
           ...
           ! where r11 = r(1:rank,1:rank)
           ! [r11,r12] = [ t11, 0 ] * y
           if( rank<n )call stdlib_I64_stzrzf( rank, n, a, lda, work( mn+1 ), work( 2_ilp64*mn+1 ),lwork-&
                     2_ilp64*mn, info )
           ! workspace: 2*mn.
           ! details of householder rotations stored in work(mn+1:2*mn)
           ! b(1:m,1:nrhs) := q**t * b(1:m,1:nrhs)
           call stdlib_I64_sormqr( 'LEFT', 'TRANSPOSE', m, nrhs, mn, a, lda, work( 1_ilp64 ),b, ldb, work( &
                     2_ilp64*mn+1 ), lwork-2*mn, info )
           wsize = max( wsize, 2_ilp64*mn+work( 2_ilp64*mn+1 ) )
           ! workspace: 2*mn+nb*nrhs.
           ! b(1:rank,1:nrhs) := inv(t11) * b(1:rank,1:nrhs)
           call stdlib_I64_strsm( 'LEFT', 'UPPER', 'NO TRANSPOSE', 'NON-UNIT', rank,nrhs, one, a, lda,&
                      b, ldb )
           do j = 1, nrhs
              do i = rank + 1, n
                 b( i, j ) = zero
           ...
           ! b(1:n,1:nrhs) := y**t * b(1:n,1:nrhs)
           if( rank<n ) then
              call stdlib_I64_sormrz( 'LEFT', 'TRANSPOSE', n, nrhs, rank, n-rank, a,lda, work( mn+1 ),&
                         b, ldb, work( 2_ilp64*mn+1 ),lwork-2*mn, info )
           end if
           ! workspace: 2*mn+nrhs.
           ! b(1:n,1:nrhs) := p * b(1:n,1:nrhs)
           do j = 1, nrhs
              ...
              call stdlib_I64_scopy( n, work( 1_ilp64 ), 1_ilp64, b( 1_ilp64, j ), 1_ilp64 )
           end do
           ! workspace: n.
           ! undo scaling
           if( iascl==1_ilp64 ) then
              call stdlib_I64_slascl( 'G', 0_ilp64, 0_ilp64, anrm, smlnum, n, nrhs, b, ldb, info )
              call stdlib_I64_slascl( 'U', 0_ilp64, 0_ilp64, smlnum, anrm, rank, rank, a, lda,info )
           else if( iascl==2_ilp64 ) then
              call stdlib_I64_slascl( 'G', 0_ilp64, 0_ilp64, anrm, bignum, n, nrhs, b, ldb, info )
              call stdlib_I64_slascl( 'U', 0_ilp64, 0_ilp64, bignum, anrm, rank, rank, a, lda,info )
           end if
           if( ibscl==1_ilp64 ) then
              call stdlib_I64_slascl( 'G', 0_ilp64, 0_ilp64, smlnum, bnrm, n, nrhs, b, ldb, info )
           else if( ibscl==2_ilp64 ) then
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              if( mn==0_ilp64 .or. nrhs==0_ilp64 ) then
                 ...
                 nb1 = stdlib_I64_ilaenv( 1_ilp64, 'DGEQRF', ' ', m, n, -1_ilp64, -1_ilp64 )
                 nb2 = stdlib_I64_ilaenv( 1_ilp64, 'DGERQF', ' ', m, n, -1_ilp64, -1_ilp64 )
                 nb3 = stdlib_I64_ilaenv( 1_ilp64, 'DORMQR', ' ', m, n, nrhs, -1_ilp64 )
                 nb4 = stdlib_I64_ilaenv( 1_ilp64, 'DORMRQ', ' ', m, n, nrhs, -1_ilp64 )
                 nb = max( nb1, nb2, nb3, nb4 )
                 lwkmin = mn + max( 2_ilp64*mn, n + 1_ilp64, mn + nrhs )
                 lwkopt = max( lwkmin,mn + 2_ilp64*n + nb*( n + 1_ilp64 ), 2_ilp64*mn + nb*nrhs )
              end if
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DGELSY', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           ! get machine parameters
           smlnum = stdlib_I64_dlamch( 'S' ) / stdlib_I64_dlamch( 'P' )
           bignum = one / smlnum
           call stdlib_I64_dlabad( smlnum, bignum )
           ! scale a, b if max entries outside range [smlnum,bignum]
           anrm = stdlib_I64_dlange( 'M', m, n, a, lda, work )
           ...
           ! compute qr factorization with column pivoting of a:
              ! a * p = q * r
           call stdlib_I64_dgeqp3( m, n, a, lda, jpvt, work( 1_ilp64 ), work( mn+1 ),lwork-mn, info )
           ...
           if( rank<mn ) then
              i = rank + 1_ilp64
              call stdlib_I64_dlaic1( imin, rank, work( ismin ), smin, a( 1_ilp64, i ),a( i, i ), sminpr, &
                        s1, c1 )
              call stdlib_I64_dlaic1( imax, rank, work( ismax ), smax, a( 1_ilp64, i ),a( i, i ), smaxpr, &
                        s2, c2 )
              if( smaxpr*rcond<=sminpr ) then
                 do i = 1, rank
                    work( ismin+i-1 ) = s1*work( ismin+i-1 )
           ...
           ! where r11 = r(1:rank,1:rank)
           ! [r11,r12] = [ t11, 0 ] * y
           if( rank<n )call stdlib_I64_dtzrzf( rank, n, a, lda, work( mn+1 ), work( 2_ilp64*mn+1 ),lwork-&
                     2_ilp64*mn, info )
           ! workspace: 2*mn.
           ! details of householder rotations stored in work(mn+1:2*mn)
           ! b(1:m,1:nrhs) := q**t * b(1:m,1:nrhs)
           call stdlib_I64_dormqr( 'LEFT', 'TRANSPOSE', m, nrhs, mn, a, lda, work( 1_ilp64 ),b, ldb, work( &
                     2_ilp64*mn+1 ), lwork-2*mn, info )
           wsize = max( wsize, 2_ilp64*mn+work( 2_ilp64*mn+1 ) )
           ! workspace: 2*mn+nb*nrhs.
           ! b(1:rank,1:nrhs) := inv(t11) * b(1:rank,1:nrhs)
           call stdlib_I64_dtrsm( 'LEFT', 'UPPER', 'NO TRANSPOSE', 'NON-UNIT', rank,nrhs, one, a, lda,&
                      b, ldb )
           do j = 1, nrhs
              do i = rank + 1, n
                 b( i, j ) = zero
           ...
           ! b(1:n,1:nrhs) := y**t * b(1:n,1:nrhs)
           if( rank<n ) then
              call stdlib_I64_dormrz( 'LEFT', 'TRANSPOSE', n, nrhs, rank, n-rank, a,lda, work( mn+1 ),&
                         b, ldb, work( 2_ilp64*mn+1 ),lwork-2*mn, info )
           end if
           ! workspace: 2*mn+nrhs.
           ! b(1:n,1:nrhs) := p * b(1:n,1:nrhs)
           do j = 1, nrhs
              ...
              call stdlib_I64_dcopy( n, work( 1_ilp64 ), 1_ilp64, b( 1_ilp64, j ), 1_ilp64 )
           end do
           ! workspace: n.
           ! undo scaling
           if( iascl==1_ilp64 ) then
              call stdlib_I64_dlascl( 'G', 0_ilp64, 0_ilp64, anrm, smlnum, n, nrhs, b, ldb, info )
              call stdlib_I64_dlascl( 'U', 0_ilp64, 0_ilp64, smlnum, anrm, rank, rank, a, lda,info )
           else if( iascl==2_ilp64 ) then
              call stdlib_I64_dlascl( 'G', 0_ilp64, 0_ilp64, anrm, bignum, n, nrhs, b, ldb, info )
              call stdlib_I64_dlascl( 'U', 0_ilp64, 0_ilp64, bignum, anrm, rank, rank, a, lda,info )
           end if
           if( ibscl==1_ilp64 ) then
              call stdlib_I64_dlascl( 'G', 0_ilp64, 0_ilp64, smlnum, bnrm, n, nrhs, b, ldb, info )
           else if( ibscl==2_ilp64 ) then
     ...
     !! o Matrix B (the right hand side) is updated with Blas-3.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           logical(lk) :: lquery
           integer(ilp64) :: i, iascl, ibscl, ismax, ismin, j, lwkopt, mn, nb, nb1, nb2, nb3, &
                     nb4
           real(sp) :: anrm, bignum, bnrm, smax, smaxpr, smin, sminpr, smlnum, wsize
           complex(sp) :: c1, c2, s1, s2
           ! Intrinsic Functions 
           ...
           ! test the input arguments.
           info = 0_ilp64
           nb1 = stdlib_I64_ilaenv( 1_ilp64, 'CGEQRF', ' ', m, n, -1_ilp64, -1_ilp64 )
           nb2 = stdlib_I64_ilaenv( 1_ilp64, 'CGERQF', ' ', m, n, -1_ilp64, -1_ilp64 )
           nb3 = stdlib_I64_ilaenv( 1_ilp64, 'CUNMQR', ' ', m, n, nrhs, -1_ilp64 )
           nb4 = stdlib_I64_ilaenv( 1_ilp64, 'CUNMRQ', ' ', m, n, nrhs, -1_ilp64 )
           nb = max( nb1, nb2, nb3, nb4 )
           lwkopt = max( 1_ilp64, mn+2*n+nb*(n+1), 2_ilp64*mn+nb*nrhs )
           work( 1_ilp64 ) = cmplx( lwkopt,KIND=sp)
           lquery = ( lwork==-1_ilp64 )
           if( m<0_ilp64 ) then
              info = -1_ilp64
           else if( n<0_ilp64 ) then
              ...
              info = -7_ilp64
           else if( lwork<( mn+max( 2_ilp64*mn, n+1, mn+nrhs ) ) .and..not.lquery ) then
              info = -12_ilp64
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CGELSY', -info )
              return
           else if( lquery ) then
              return
           end if
           ! quick return if possible
           if( min( m, n, nrhs )==0_ilp64 ) then
              rank = 0_ilp64
              return
           end if
           ! get machine parameters
           ...
           ! compute qr factorization with column pivoting of a:
              ! a * p = q * r
           call stdlib_I64_cgeqp3( m, n, a, lda, jpvt, work( 1_ilp64 ), work( mn+1 ),lwork-mn, rwork, info )
           ...
           if( rank<mn ) then
              i = rank + 1_ilp64
              call stdlib_I64_claic1( imin, rank, work( ismin ), smin, a( 1_ilp64, i ),a( i, i ), sminpr, &
                        s1, c1 )
              call stdlib_I64_claic1( imax, rank, work( ismax ), smax, a( 1_ilp64, i ),a( i, i ), smaxpr, &
                        s2, c2 )
              if( smaxpr*rcond<=sminpr ) then
                 do i = 1, rank
                    work( ismin+i-1 ) = s1*work( ismin+i-1 )
           ...
           ! where r11 = r(1:rank,1:rank)
           ! [r11,r12] = [ t11, 0 ] * y
           if( rank<n )call stdlib_I64_ctzrzf( rank, n, a, lda, work( mn+1 ), work( 2_ilp64*mn+1 ),lwork-&
                     2_ilp64*mn, info )
           ! complex workspace: 2*mn.
           ! details of householder rotations stored in work(mn+1:2*mn)
           ! b(1:m,1:nrhs) := q**h * b(1:m,1:nrhs)
           call stdlib_I64_cunmqr( 'LEFT', 'CONJUGATE TRANSPOSE', m, nrhs, mn, a, lda,work( 1_ilp64 ), b, &
                     ldb, work( 2_ilp64*mn+1 ), lwork-2*mn, info )
           wsize = max( wsize, 2_ilp64*mn+real( work( 2_ilp64*mn+1 ),KIND=sp) )
           ! complex workspace: 2*mn+nb*nrhs.
           ! b(1:rank,1:nrhs) := inv(t11) * b(1:rank,1:nrhs)
           call stdlib_I64_ctrsm( 'LEFT', 'UPPER', 'NO TRANSPOSE', 'NON-UNIT', rank,nrhs, cone, a, &
                     lda, b, ldb )
           do j = 1, nrhs
              do i = rank + 1, n
                 b( i, j ) = czero
           ...
           ! b(1:n,1:nrhs) := y**h * b(1:n,1:nrhs)
           if( rank<n ) then
              call stdlib_I64_cunmrz( 'LEFT', 'CONJUGATE TRANSPOSE', n, nrhs, rank,n-rank, a, lda, &
                        work( mn+1 ), b, ldb,work( 2_ilp64*mn+1 ), lwork-2*mn, info )
           end if
           ! complex workspace: 2*mn+nrhs.
           ! b(1:n,1:nrhs) := p * b(1:n,1:nrhs)
           do j = 1, nrhs
              ...
              call stdlib_I64_ccopy( n, work( 1_ilp64 ), 1_ilp64, b( 1_ilp64, j ), 1_ilp64 )
           end do
           ! complex workspace: n.
           ! undo scaling
           if( iascl==1_ilp64 ) then
              call stdlib_I64_clascl( 'G', 0_ilp64, 0_ilp64, anrm, smlnum, n, nrhs, b, ldb, info )
              call stdlib_I64_clascl( 'U', 0_ilp64, 0_ilp64, smlnum, anrm, rank, rank, a, lda,info )
           else if( iascl==2_ilp64 ) then
              call stdlib_I64_clascl( 'G', 0_ilp64, 0_ilp64, anrm, bignum, n, nrhs, b, ldb, info )
              call stdlib_I64_clascl( 'U', 0_ilp64, 0_ilp64, bignum, anrm, rank, rank, a, lda,info )
           end if
           if( ibscl==1_ilp64 ) then
              call stdlib_I64_clascl( 'G', 0_ilp64, 0_ilp64, smlnum, bnrm, n, nrhs, b, ldb, info )
           else if( ibscl==2_ilp64 ) then
     ...
     !! o Matrix B (the right hand side) is updated with Blas-3.
        ...
        ! =====================================================================
           ...
           ! test the input arguments.
           info = 0_ilp64
           nb1 = stdlib_I64_ilaenv( 1_ilp64, 'ZGEQRF', ' ', m, n, -1_ilp64, -1_ilp64 )
           nb2 = stdlib_I64_ilaenv( 1_ilp64, 'ZGERQF', ' ', m, n, -1_ilp64, -1_ilp64 )
           nb3 = stdlib_I64_ilaenv( 1_ilp64, 'ZUNMQR', ' ', m, n, nrhs, -1_ilp64 )
           nb4 = stdlib_I64_ilaenv( 1_ilp64, 'ZUNMRQ', ' ', m, n, nrhs, -1_ilp64 )
           nb = max( nb1, nb2, nb3, nb4 )
           lwkopt = max( 1_ilp64, mn+2*n+nb*( n+1 ), 2_ilp64*mn+nb*nrhs )
           work( 1_ilp64 ) = cmplx( lwkopt,KIND=dp)
           lquery = ( lwork==-1_ilp64 )
           if( m<0_ilp64 ) then
              info = -1_ilp64
           else if( n<0_ilp64 ) then
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZGELSY', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           ! compute qr factorization with column pivoting of a:
              ! a * p = q * r
           call stdlib_I64_zgeqp3( m, n, a, lda, jpvt, work( 1_ilp64 ), work( mn+1 ),lwork-mn, rwork, info )
           ...
           if( rank<mn ) then
              i = rank + 1_ilp64
              call stdlib_I64_zlaic1( imin, rank, work( ismin ), smin, a( 1_ilp64, i ),a( i, i ), sminpr, &
                        s1, c1 )
              call stdlib_I64_zlaic1( imax, rank, work( ismax ), smax, a( 1_ilp64, i ),a( i, i ), smaxpr, &
                        s2, c2 )
              if( smaxpr*rcond<=sminpr ) then
                 do i = 1, rank
                    work( ismin+i-1 ) = s1*work( ismin+i-1 )
           ...
           ! where r11 = r(1:rank,1:rank)
           ! [r11,r12] = [ t11, 0 ] * y
           if( rank<n )call stdlib_I64_ztzrzf( rank, n, a, lda, work( mn+1 ), work( 2_ilp64*mn+1 ),lwork-&
                     2_ilp64*mn, info )
           ! complex workspace: 2*mn.
           ! details of householder rotations stored in work(mn+1:2*mn)
           ! b(1:m,1:nrhs) := q**h * b(1:m,1:nrhs)
           call stdlib_I64_zunmqr( 'LEFT', 'CONJUGATE TRANSPOSE', m, nrhs, mn, a, lda,work( 1_ilp64 ), b, &
                     ldb, work( 2_ilp64*mn+1 ), lwork-2*mn, info )
           wsize = max( wsize, 2_ilp64*mn+real( work( 2_ilp64*mn+1 ),KIND=dp) )
           ! complex workspace: 2*mn+nb*nrhs.
           ! b(1:rank,1:nrhs) := inv(t11) * b(1:rank,1:nrhs)
           call stdlib_I64_ztrsm( 'LEFT', 'UPPER', 'NO TRANSPOSE', 'NON-UNIT', rank,nrhs, cone, a, &
                     lda, b, ldb )
           do j = 1, nrhs
              do i = rank + 1, n
                 b( i, j ) = czero
           ...
           ! b(1:n,1:nrhs) := y**h * b(1:n,1:nrhs)
           if( rank<n ) then
              call stdlib_I64_zunmrz( 'LEFT', 'CONJUGATE TRANSPOSE', n, nrhs, rank,n-rank, a, lda, &
                        work( mn+1 ), b, ldb,work( 2_ilp64*mn+1 ), lwork-2*mn, info )
           end if
           ! complex workspace: 2*mn+nrhs.
           ! b(1:n,1:nrhs) := p * b(1:n,1:nrhs)
           do j = 1, nrhs
              ...
              call stdlib_I64_zcopy( n, work( 1_ilp64 ), 1_ilp64, b( 1_ilp64, j ), 1_ilp64 )
           end do
           ! complex workspace: n.
           ! undo scaling
           if( iascl==1_ilp64 ) then
              call stdlib_I64_zlascl( 'G', 0_ilp64, 0_ilp64, anrm, smlnum, n, nrhs, b, ldb, info )
              call stdlib_I64_zlascl( 'U', 0_ilp64, 0_ilp64, smlnum, anrm, rank, rank, a, lda,info )
           else if( iascl==2_ilp64 ) then
              call stdlib_I64_zlascl( 'G', 0_ilp64, 0_ilp64, anrm, bignum, n, nrhs, b, ldb, info )
              call stdlib_I64_zlascl( 'U', 0_ilp64, 0_ilp64, bignum, anrm, rank, rank, a, lda,info )
           end if
           if( ibscl==1_ilp64 ) then
              call stdlib_I64_zlascl( 'G', 0_ilp64, 0_ilp64, smlnum, bnrm, n, nrhs, b, ldb, info )
           else if( ibscl==2_ilp64 ) then
     ...
     !! matrix X.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           logical(lk) :: lquery, tpsd
           integer(ilp64) :: brow, i, iascl, ibscl, j, mn, nb, scllen, wsize
           real(sp) :: anrm, bignum, bnrm, smlnum
           ! Local Arrays 
           real(sp) :: rwork(1_ilp64)
           ! Intrinsic Functions 
           ! Executable Statements 
           ! test the input arguments.
           info = 0_ilp64
           ...
           if( .not.( stdlib_lsame( trans, 'N' ) .or. stdlib_lsame( trans, 'T' ) ) ) then
              ...
              info = -8_ilp64
           else if( lwork<max( 1_ilp64, mn + max( mn, nrhs ) ) .and..not.lquery ) then
              info = -10_ilp64
           end if
           ! figure out optimal block size
           if( info==0_ilp64 .or. info==-10_ilp64 ) then
              tpsd = .true.
              if( stdlib_lsame( trans, 'N' ) )tpsd = .false.
              if( m>=n ) then
                 nb = stdlib_I64_ilaenv( 1_ilp64, 'SGEQRF', ' ', m, n, -1_ilp64, -1_ilp64 )
                 if( tpsd ) then
                    nb = max( nb, stdlib_I64_ilaenv( 1_ilp64, 'SORMQR', 'LN', m, nrhs, n,-1_ilp64 ) )
                 else
                    nb = max( nb, stdlib_I64_ilaenv( 1_ilp64, 'SORMQR', 'LT', m, nrhs, n,-1_ilp64 ) )
                 end if
              else
                 nb = stdlib_I64_ilaenv( 1_ilp64, 'SGELQF', ' ', m, n, -1_ilp64, -1_ilp64 )
                 if( tpsd ) then
                    nb = max( nb, stdlib_I64_ilaenv( 1_ilp64, 'SORMLQ', 'LT', n, nrhs, m,-1_ilp64 ) )
                 else
                    nb = max( nb, stdlib_I64_ilaenv( 1_ilp64, 'SORMLQ', 'LN', n, nrhs, m,-1_ilp64 ) )
                 end if
              end if
              wsize = max( 1_ilp64, mn + max( mn, nrhs )*nb )
              work( 1_ilp64 ) = real( wsize,KIND=sp)
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SGELS ', -info )
              return
           else if( lquery ) then
              return
           end if
           ! quick return if possible
           if( min( m, n, nrhs )==0_ilp64 ) then
              call stdlib_I64_slaset( 'FULL', max( m, n ), nrhs, zero, zero, b, ldb )
              return
           end if
           ! get machine parameters
           smlnum = stdlib_I64_slamch( 'S' ) / stdlib_I64_slamch( 'P' )
           ...
           call stdlib_I64_slabad( smlnum, bignum )
           ! scale a, b if max element outside range [smlnum,bignum]
           anrm = stdlib_I64_slange( 'M', m, n, a, lda, rwork )
           iascl = 0_ilp64
           if( anrm>zero .and. anrm<smlnum ) then
              ! scale matrix norm up to smlnum
              call stdlib_I64_slascl( 'G', 0_ilp64, 0_ilp64, anrm, smlnum, m, n, a, lda, info )
           ...
           brow = m
           if( tpsd )brow = n
           bnrm = stdlib_I64_slange( 'M', brow, nrhs, b, ldb, rwork )
           ibscl = 0_ilp64
           if( bnrm>zero .and. bnrm<smlnum ) then
              ! scale matrix norm up to smlnum
              call stdlib_I64_slascl( 'G', 0_ilp64, 0_ilp64, bnrm, smlnum, brow, nrhs, b, ldb,info )
              ibscl = 1_ilp64
           else if( bnrm>bignum ) then
              ! scale matrix norm down to bignum
              call stdlib_I64_slascl( 'G', 0_ilp64, 0_ilp64, bnrm, bignum, brow, nrhs, b, ldb,info )
              ibscl = 2_ilp64
           end if
           if( m>=n ) then
              ! compute qr factorization of a
              call stdlib_I64_sgeqrf( m, n, a, lda, work( 1_ilp64 ), work( mn+1 ), lwork-mn,info )
              ! workspace at least n, optimally n*nb
              if( .not.tpsd ) then
                 ! least-squares problem min || a * x - b ||
                 ! b(1:m,1:nrhs) := q**t * b(1:m,1:nrhs)
                 call stdlib_I64_sormqr( 'LEFT', 'TRANSPOSE', m, nrhs, n, a, lda,work( 1_ilp64 ), b, ldb, &
                           work( mn+1 ), lwork-mn,info )
                 ! workspace at least nrhs, optimally nrhs*nb
                 ! b(1:n,1:nrhs) := inv(r) * b(1:n,1:nrhs)
                 call stdlib_I64_strtrs( 'UPPER', 'NO TRANSPOSE', 'NON-UNIT', n, nrhs,a, lda, b, ldb, &
                           info )
                 if( info>0_ilp64 ) then
                    return
                 end if
                 ...
                 ! underdetermined system of equations a**t * x = b
                 ! b(1:n,1:nrhs) := inv(r**t) * b(1:n,1:nrhs)
                 call stdlib_I64_strtrs( 'UPPER', 'TRANSPOSE', 'NON-UNIT', n, nrhs,a, lda, b, ldb, &
                           info )
                 if( info>0_ilp64 ) then
                    return
                 end if
                 ...
                 ! b(1:m,1:nrhs) := q(1:n,:) * b(1:n,1:nrhs)
                 call stdlib_I64_sormqr( 'LEFT', 'NO TRANSPOSE', m, nrhs, n, a, lda,work( 1_ilp64 ), b, ldb,&
                            work( mn+1 ), lwork-mn,info )
                 ! workspace at least nrhs, optimally nrhs*nb
                 scllen = m
              end if
              ...
              ! compute lq factorization of a
              call stdlib_I64_sgelqf( m, n, a, lda, work( 1_ilp64 ), work( mn+1 ), lwork-mn,info )
              ! workspace at least m, optimally m*nb.
              if( .not.tpsd ) then
                 ! underdetermined system of equations a * x = b
                 ! b(1:m,1:nrhs) := inv(l) * b(1:m,1:nrhs)
                 call stdlib_I64_strtrs( 'LOWER', 'NO TRANSPOSE', 'NON-UNIT', m, nrhs,a, lda, b, ldb, &
                           info )
                 if( info>0_ilp64 ) then
                    return
                 end if
                 ...
                 ! b(1:n,1:nrhs) := q(1:n,:)**t * b(1:m,1:nrhs)
                 call stdlib_I64_sormlq( 'LEFT', 'TRANSPOSE', n, nrhs, m, a, lda,work( 1_ilp64 ), b, ldb, &
                           work( mn+1 ), lwork-mn,info )
                 ! workspace at least nrhs, optimally nrhs*nb
                 scllen = n
              else
                 ! overdetermined system min || a**t * x - b ||
                 ! b(1:n,1:nrhs) := q * b(1:n,1:nrhs)
                 call stdlib_I64_sormlq( 'LEFT', 'NO TRANSPOSE', n, nrhs, m, a, lda,work( 1_ilp64 ), b, ldb,&
                            work( mn+1 ), lwork-mn,info )
                 ! workspace at least nrhs, optimally nrhs*nb
                 ! b(1:m,1:nrhs) := inv(l**t) * b(1:m,1:nrhs)
                 call stdlib_I64_strtrs( 'LOWER', 'TRANSPOSE', 'NON-UNIT', m, nrhs,a, lda, b, ldb, &
                           info )
                 if( info>0_ilp64 ) then
                    return
                 end if
           ...
           ! undo scaling
           if( iascl==1_ilp64 ) then
              call stdlib_I64_slascl( 'G', 0_ilp64, 0_ilp64, anrm, smlnum, scllen, nrhs, b, ldb,info )
           else if( iascl==2_ilp64 ) then
              call stdlib_I64_slascl( 'G', 0_ilp64, 0_ilp64, anrm, bignum, scllen, nrhs, b, ldb,info )
           end if
           if( ibscl==1_ilp64 ) then
              call stdlib_I64_slascl( 'G', 0_ilp64, 0_ilp64, smlnum, bnrm, scllen, nrhs, b, ldb,info )
           else if( ibscl==2_ilp64 ) then
              call stdlib_I64_slascl( 'G', 0_ilp64, 0_ilp64, bignum, bnrm, scllen, nrhs, b, ldb,info )
           end if
           50 continue
           work( 1_ilp64 ) = real( wsize,KIND=sp)
           return
     ...
     !! matrix X.
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 .or. info==-10_ilp64 ) then
              ...
              if( stdlib_lsame( trans, 'N' ) )tpsd = .false.
              if( m>=n ) then
                 nb = stdlib_I64_ilaenv( 1_ilp64, 'DGEQRF', ' ', m, n, -1_ilp64, -1_ilp64 )
                 if( tpsd ) then
                    nb = max( nb, stdlib_I64_ilaenv( 1_ilp64, 'DORMQR', 'LN', m, nrhs, n,-1_ilp64 ) )
                 else
                    nb = max( nb, stdlib_I64_ilaenv( 1_ilp64, 'DORMQR', 'LT', m, nrhs, n,-1_ilp64 ) )
                 end if
              else
                 nb = stdlib_I64_ilaenv( 1_ilp64, 'DGELQF', ' ', m, n, -1_ilp64, -1_ilp64 )
                 if( tpsd ) then
                    nb = max( nb, stdlib_I64_ilaenv( 1_ilp64, 'DORMLQ', 'LT', n, nrhs, m,-1_ilp64 ) )
                 else
                    nb = max( nb, stdlib_I64_ilaenv( 1_ilp64, 'DORMLQ', 'LN', n, nrhs, m,-1_ilp64 ) )
                 end if
              end if
              wsize = max( 1_ilp64, mn+max( mn, nrhs )*nb )
              work( 1_ilp64 ) = real( wsize,KIND=dp)
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DGELS ', -info )
              return
           else if( lquery ) then
              return
           end if
           ! quick return if possible
           if( min( m, n, nrhs )==0_ilp64 ) then
              call stdlib_I64_dlaset( 'FULL', max( m, n ), nrhs, zero, zero, b, ldb )
              return
           end if
           ! get machine parameters
           smlnum = stdlib_I64_dlamch( 'S' ) / stdlib_I64_dlamch( 'P' )
           ...
           call stdlib_I64_dlabad( smlnum, bignum )
           ! scale a, b if max element outside range [smlnum,bignum]
           anrm = stdlib_I64_dlange( 'M', m, n, a, lda, rwork )
           iascl = 0_ilp64
           if( anrm>zero .and. anrm<smlnum ) then
              ! scale matrix norm up to smlnum
              call stdlib_I64_dlascl( 'G', 0_ilp64, 0_ilp64, anrm, smlnum, m, n, a, lda, info )
           ...
           brow = m
           if( tpsd )brow = n
           bnrm = stdlib_I64_dlange( 'M', brow, nrhs, b, ldb, rwork )
           ibscl = 0_ilp64
           if( bnrm>zero .and. bnrm<smlnum ) then
              ! scale matrix norm up to smlnum
              call stdlib_I64_dlascl( 'G', 0_ilp64, 0_ilp64, bnrm, smlnum, brow, nrhs, b, ldb,info )
              ibscl = 1_ilp64
           else if( bnrm>bignum ) then
              ! scale matrix norm down to bignum
              call stdlib_I64_dlascl( 'G', 0_ilp64, 0_ilp64, bnrm, bignum, brow, nrhs, b, ldb,info )
              ibscl = 2_ilp64
           end if
           if( m>=n ) then
              ! compute qr factorization of a
              call stdlib_I64_dgeqrf( m, n, a, lda, work( 1_ilp64 ), work( mn+1 ), lwork-mn,info )
              ! workspace at least n, optimally n*nb
              if( .not.tpsd ) then
                 ! least-squares problem min || a * x - b ||
                 ! b(1:m,1:nrhs) := q**t * b(1:m,1:nrhs)
                 call stdlib_I64_dormqr( 'LEFT', 'TRANSPOSE', m, nrhs, n, a, lda,work( 1_ilp64 ), b, ldb, &
                           work( mn+1 ), lwork-mn,info )
                 ! workspace at least nrhs, optimally nrhs*nb
                 ! b(1:n,1:nrhs) := inv(r) * b(1:n,1:nrhs)
                 call stdlib_I64_dtrtrs( 'UPPER', 'NO TRANSPOSE', 'NON-UNIT', n, nrhs,a, lda, b, ldb, &
                           info )
                 if( info>0_ilp64 ) then
                    return
                 end if
                 ...
                 ! underdetermined system of equations a**t * x = b
                 ! b(1:n,1:nrhs) := inv(r**t) * b(1:n,1:nrhs)
                 call stdlib_I64_dtrtrs( 'UPPER', 'TRANSPOSE', 'NON-UNIT', n, nrhs,a, lda, b, ldb, &
                           info )
                 if( info>0_ilp64 ) then
                    return
                 end if
                 ...
                 ! b(1:m,1:nrhs) := q(1:n,:) * b(1:n,1:nrhs)
                 call stdlib_I64_dormqr( 'LEFT', 'NO TRANSPOSE', m, nrhs, n, a, lda,work( 1_ilp64 ), b, ldb,&
                            work( mn+1 ), lwork-mn,info )
                 ! workspace at least nrhs, optimally nrhs*nb
                 scllen = m
              end if
              ...
              ! compute lq factorization of a
              call stdlib_I64_dgelqf( m, n, a, lda, work( 1_ilp64 ), work( mn+1 ), lwork-mn,info )
              ! workspace at least m, optimally m*nb.
              if( .not.tpsd ) then
                 ! underdetermined system of equations a * x = b
                 ! b(1:m,1:nrhs) := inv(l) * b(1:m,1:nrhs)
                 call stdlib_I64_dtrtrs( 'LOWER', 'NO TRANSPOSE', 'NON-UNIT', m, nrhs,a, lda, b, ldb, &
                           info )
                 if( info>0_ilp64 ) then
                    return
                 end if
                 ...
                 ! b(1:n,1:nrhs) := q(1:n,:)**t * b(1:m,1:nrhs)
                 call stdlib_I64_dormlq( 'LEFT', 'TRANSPOSE', n, nrhs, m, a, lda,work( 1_ilp64 ), b, ldb, &
                           work( mn+1 ), lwork-mn,info )
                 ! workspace at least nrhs, optimally nrhs*nb
                 scllen = n
              else
                 ! overdetermined system min || a**t * x - b ||
                 ! b(1:n,1:nrhs) := q * b(1:n,1:nrhs)
                 call stdlib_I64_dormlq( 'LEFT', 'NO TRANSPOSE', n, nrhs, m, a, lda,work( 1_ilp64 ), b, ldb,&
                            work( mn+1 ), lwork-mn,info )
                 ! workspace at least nrhs, optimally nrhs*nb
                 ! b(1:m,1:nrhs) := inv(l**t) * b(1:m,1:nrhs)
                 call stdlib_I64_dtrtrs( 'LOWER', 'TRANSPOSE', 'NON-UNIT', m, nrhs,a, lda, b, ldb, &
                           info )
                 if( info>0_ilp64 ) then
                    return
                 end if
           ...
           ! undo scaling
           if( iascl==1_ilp64 ) then
              call stdlib_I64_dlascl( 'G', 0_ilp64, 0_ilp64, anrm, smlnum, scllen, nrhs, b, ldb,info )
           else if( iascl==2_ilp64 ) then
              call stdlib_I64_dlascl( 'G', 0_ilp64, 0_ilp64, anrm, bignum, scllen, nrhs, b, ldb,info )
           end if
           if( ibscl==1_ilp64 ) then
              call stdlib_I64_dlascl( 'G', 0_ilp64, 0_ilp64, smlnum, bnrm, scllen, nrhs, b, ldb,info )
           else if( ibscl==2_ilp64 ) then
              call stdlib_I64_dlascl( 'G', 0_ilp64, 0_ilp64, bignum, bnrm, scllen, nrhs, b, ldb,info )
           end if
           50 continue
           work( 1_ilp64 ) = real( wsize,KIND=dp)
           return
     ...
     !! matrix X.
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 .or. info==-10_ilp64 ) then
              ...
              if( stdlib_lsame( trans, 'N' ) )tpsd = .false.
              if( m>=n ) then
                 nb = stdlib_I64_ilaenv( 1_ilp64, 'CGEQRF', ' ', m, n, -1_ilp64, -1_ilp64 )
                 if( tpsd ) then
                    nb = max( nb, stdlib_I64_ilaenv( 1_ilp64, 'CUNMQR', 'LN', m, nrhs, n,-1_ilp64 ) )
                 else
                    nb = max( nb, stdlib_I64_ilaenv( 1_ilp64, 'CUNMQR', 'LC', m, nrhs, n,-1_ilp64 ) )
                 end if
              else
                 nb = stdlib_I64_ilaenv( 1_ilp64, 'CGELQF', ' ', m, n, -1_ilp64, -1_ilp64 )
                 if( tpsd ) then
                    nb = max( nb, stdlib_I64_ilaenv( 1_ilp64, 'CUNMLQ', 'LC', n, nrhs, m,-1_ilp64 ) )
                 else
                    nb = max( nb, stdlib_I64_ilaenv( 1_ilp64, 'CUNMLQ', 'LN', n, nrhs, m,-1_ilp64 ) )
                 end if
              end if
              wsize = max( 1_ilp64, mn + max( mn, nrhs )*nb )
              work( 1_ilp64 ) = real( wsize,KIND=sp)
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CGELS ', -info )
              return
           else if( lquery ) then
              return
           end if
           ! quick return if possible
           if( min( m, n, nrhs )==0_ilp64 ) then
              call stdlib_I64_claset( 'FULL', max( m, n ), nrhs, czero, czero, b, ldb )
              return
           end if
           ! get machine parameters
           smlnum = stdlib_I64_slamch( 'S' ) / stdlib_I64_slamch( 'P' )
           ...
           brow = m
           if( tpsd )brow = n
           bnrm = stdlib_I64_clange( 'M', brow, nrhs, b, ldb, rwork )
           ibscl = 0_ilp64
           if( bnrm>zero .and. bnrm<smlnum ) then
              ! scale matrix norm up to smlnum
              call stdlib_I64_clascl( 'G', 0_ilp64, 0_ilp64, bnrm, smlnum, brow, nrhs, b, ldb,info )
              ibscl = 1_ilp64
           else if( bnrm>bignum ) then
              ! scale matrix norm down to bignum
              call stdlib_I64_clascl( 'G', 0_ilp64, 0_ilp64, bnrm, bignum, brow, nrhs, b, ldb,info )
              ibscl = 2_ilp64
           end if
           if( m>=n ) then
              ! compute qr factorization of a
              call stdlib_I64_cgeqrf( m, n, a, lda, work( 1_ilp64 ), work( mn+1 ), lwork-mn,info )
              ! workspace at least n, optimally n*nb
              if( .not.tpsd ) then
                 ! least-squares problem min || a * x - b ||
                 ! b(1:m,1:nrhs) := q**h * b(1:m,1:nrhs)
                 call stdlib_I64_cunmqr( 'LEFT', 'CONJUGATE TRANSPOSE', m, nrhs, n, a,lda, work( 1_ilp64 ), &
                           b, ldb, work( mn+1 ), lwork-mn,info )
                 ! workspace at least nrhs, optimally nrhs*nb
                 ! b(1:n,1:nrhs) := inv(r) * b(1:n,1:nrhs)
                 call stdlib_I64_ctrtrs( 'UPPER', 'NO TRANSPOSE', 'NON-UNIT', n, nrhs,a, lda, b, ldb, &
                           info )
                 if( info>0_ilp64 ) then
                    return
                 end if
                 ...
                 ! underdetermined system of equations a**t * x = b
                 ! b(1:n,1:nrhs) := inv(r**h) * b(1:n,1:nrhs)
                 call stdlib_I64_ctrtrs( 'UPPER', 'CONJUGATE TRANSPOSE','NON-UNIT',n, nrhs, a, lda, b,&
                            ldb, info )
                 if( info>0_ilp64 ) then
                    return
                 end if
                 ...
                 ! b(1:m,1:nrhs) := q(1:n,:) * b(1:n,1:nrhs)
                 call stdlib_I64_cunmqr( 'LEFT', 'NO TRANSPOSE', m, nrhs, n, a, lda,work( 1_ilp64 ), b, ldb,&
                            work( mn+1 ), lwork-mn,info )
                 ! workspace at least nrhs, optimally nrhs*nb
                 scllen = m
              end if
              ...
              ! compute lq factorization of a
              call stdlib_I64_cgelqf( m, n, a, lda, work( 1_ilp64 ), work( mn+1 ), lwork-mn,info )
              ! workspace at least m, optimally m*nb.
              if( .not.tpsd ) then
                 ! underdetermined system of equations a * x = b
                 ! b(1:m,1:nrhs) := inv(l) * b(1:m,1:nrhs)
                 call stdlib_I64_ctrtrs( 'LOWER', 'NO TRANSPOSE', 'NON-UNIT', m, nrhs,a, lda, b, ldb, &
                           info )
                 if( info>0_ilp64 ) then
                    return
                 end if
                 ...
                 ! b(1:n,1:nrhs) := q(1:n,:)**h * b(1:m,1:nrhs)
                 call stdlib_I64_cunmlq( 'LEFT', 'CONJUGATE TRANSPOSE', n, nrhs, m, a,lda, work( 1_ilp64 ), &
                           b, ldb, work( mn+1 ), lwork-mn,info )
                 ! workspace at least nrhs, optimally nrhs*nb
                 scllen = n
              else
                 ! overdetermined system min || a**h * x - b ||
                 ! b(1:n,1:nrhs) := q * b(1:n,1:nrhs)
                 call stdlib_I64_cunmlq( 'LEFT', 'NO TRANSPOSE', n, nrhs, m, a, lda,work( 1_ilp64 ), b, ldb,&
                            work( mn+1 ), lwork-mn,info )
                 ! workspace at least nrhs, optimally nrhs*nb
                 ! b(1:m,1:nrhs) := inv(l**h) * b(1:m,1:nrhs)
                 call stdlib_I64_ctrtrs( 'LOWER', 'CONJUGATE TRANSPOSE', 'NON-UNIT',m, nrhs, a, lda, &
                           b, ldb, info )
                 if( info>0_ilp64 ) then
                    return
                 end if
           ...
           ! undo scaling
           if( iascl==1_ilp64 ) then
              call stdlib_I64_clascl( 'G', 0_ilp64, 0_ilp64, anrm, smlnum, scllen, nrhs, b, ldb,info )
           else if( iascl==2_ilp64 ) then
              call stdlib_I64_clascl( 'G', 0_ilp64, 0_ilp64, anrm, bignum, scllen, nrhs, b, ldb,info )
           end if
           if( ibscl==1_ilp64 ) then
              call stdlib_I64_clascl( 'G', 0_ilp64, 0_ilp64, smlnum, bnrm, scllen, nrhs, b, ldb,info )
           else if( ibscl==2_ilp64 ) then
              call stdlib_I64_clascl( 'G', 0_ilp64, 0_ilp64, bignum, bnrm, scllen, nrhs, b, ldb,info )
           end if
           50 continue
           work( 1_ilp64 ) = real( wsize,KIND=sp)
           return
     ...
     !! matrix X.
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 .or. info==-10_ilp64 ) then
              ...
              if( stdlib_lsame( trans, 'N' ) )tpsd = .false.
              if( m>=n ) then
                 nb = stdlib_I64_ilaenv( 1_ilp64, 'ZGEQRF', ' ', m, n, -1_ilp64, -1_ilp64 )
                 if( tpsd ) then
                    nb = max( nb, stdlib_I64_ilaenv( 1_ilp64, 'ZUNMQR', 'LN', m, nrhs, n,-1_ilp64 ) )
                 else
                    nb = max( nb, stdlib_I64_ilaenv( 1_ilp64, 'ZUNMQR', 'LC', m, nrhs, n,-1_ilp64 ) )
                 end if
              else
                 nb = stdlib_I64_ilaenv( 1_ilp64, 'ZGELQF', ' ', m, n, -1_ilp64, -1_ilp64 )
                 if( tpsd ) then
                    nb = max( nb, stdlib_I64_ilaenv( 1_ilp64, 'ZUNMLQ', 'LC', n, nrhs, m,-1_ilp64 ) )
                 else
                    nb = max( nb, stdlib_I64_ilaenv( 1_ilp64, 'ZUNMLQ', 'LN', n, nrhs, m,-1_ilp64 ) )
                 end if
              end if
              wsize = max( 1_ilp64, mn+max( mn, nrhs )*nb )
              work( 1_ilp64 ) = real( wsize,KIND=dp)
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZGELS ', -info )
              return
           else if( lquery ) then
              return
           end if
           ! quick return if possible
           if( min( m, n, nrhs )==0_ilp64 ) then
              call stdlib_I64_zlaset( 'FULL', max( m, n ), nrhs, czero, czero, b, ldb )
              return
           end if
           ! get machine parameters
           smlnum = stdlib_I64_dlamch( 'S' ) / stdlib_I64_dlamch( 'P' )
           ...
           brow = m
           if( tpsd )brow = n
           bnrm = stdlib_I64_zlange( 'M', brow, nrhs, b, ldb, rwork )
           ibscl = 0_ilp64
           if( bnrm>zero .and. bnrm<smlnum ) then
              ! scale matrix norm up to smlnum
              call stdlib_I64_zlascl( 'G', 0_ilp64, 0_ilp64, bnrm, smlnum, brow, nrhs, b, ldb,info )
              ibscl = 1_ilp64
           else if( bnrm>bignum ) then
              ! scale matrix norm down to bignum
              call stdlib_I64_zlascl( 'G', 0_ilp64, 0_ilp64, bnrm, bignum, brow, nrhs, b, ldb,info )
              ibscl = 2_ilp64
           end if
           if( m>=n ) then
              ! compute qr factorization of a
              call stdlib_I64_zgeqrf( m, n, a, lda, work( 1_ilp64 ), work( mn+1 ), lwork-mn,info )
              ! workspace at least n, optimally n*nb
              if( .not.tpsd ) then
                 ! least-squares problem min || a * x - b ||
                 ! b(1:m,1:nrhs) := q**h * b(1:m,1:nrhs)
                 call stdlib_I64_zunmqr( 'LEFT', 'CONJUGATE TRANSPOSE', m, nrhs, n, a,lda, work( 1_ilp64 ), &
                           b, ldb, work( mn+1 ), lwork-mn,info )
                 ! workspace at least nrhs, optimally nrhs*nb
                 ! b(1:n,1:nrhs) := inv(r) * b(1:n,1:nrhs)
                 call stdlib_I64_ztrtrs( 'UPPER', 'NO TRANSPOSE', 'NON-UNIT', n, nrhs,a, lda, b, ldb, &
                           info )
                 if( info>0_ilp64 ) then
                    return
                 end if
                 ...
                 ! underdetermined system of equations a**t * x = b
                 ! b(1:n,1:nrhs) := inv(r**h) * b(1:n,1:nrhs)
                 call stdlib_I64_ztrtrs( 'UPPER', 'CONJUGATE TRANSPOSE','NON-UNIT',n, nrhs, a, lda, b,&
                            ldb, info )
                 if( info>0_ilp64 ) then
                    return
                 end if
                 ...
                 ! b(1:m,1:nrhs) := q(1:n,:) * b(1:n,1:nrhs)
                 call stdlib_I64_zunmqr( 'LEFT', 'NO TRANSPOSE', m, nrhs, n, a, lda,work( 1_ilp64 ), b, ldb,&
                            work( mn+1 ), lwork-mn,info )
                 ! workspace at least nrhs, optimally nrhs*nb
                 scllen = m
              end if
              ...
              ! compute lq factorization of a
              call stdlib_I64_zgelqf( m, n, a, lda, work( 1_ilp64 ), work( mn+1 ), lwork-mn,info )
              ! workspace at least m, optimally m*nb.
              if( .not.tpsd ) then
                 ! underdetermined system of equations a * x = b
                 ! b(1:m,1:nrhs) := inv(l) * b(1:m,1:nrhs)
                 call stdlib_I64_ztrtrs( 'LOWER', 'NO TRANSPOSE', 'NON-UNIT', m, nrhs,a, lda, b, ldb, &
                           info )
                 if( info>0_ilp64 ) then
                    return
                 end if
                 ...
                 ! b(1:n,1:nrhs) := q(1:n,:)**h * b(1:m,1:nrhs)
                 call stdlib_I64_zunmlq( 'LEFT', 'CONJUGATE TRANSPOSE', n, nrhs, m, a,lda, work( 1_ilp64 ), &
                           b, ldb, work( mn+1 ), lwork-mn,info )
                 ! workspace at least nrhs, optimally nrhs*nb
                 scllen = n
              else
                 ! overdetermined system min || a**h * x - b ||
                 ! b(1:n,1:nrhs) := q * b(1:n,1:nrhs)
                 call stdlib_I64_zunmlq( 'LEFT', 'NO TRANSPOSE', n, nrhs, m, a, lda,work( 1_ilp64 ), b, ldb,&
                            work( mn+1 ), lwork-mn,info )
                 ! workspace at least nrhs, optimally nrhs*nb
                 ! b(1:m,1:nrhs) := inv(l**h) * b(1:m,1:nrhs)
                 call stdlib_I64_ztrtrs( 'LOWER', 'CONJUGATE TRANSPOSE', 'NON-UNIT',m, nrhs, a, lda, &
                           b, ldb, info )
                 if( info>0_ilp64 ) then
                    return
                 end if
           ...
           ! undo scaling
           if( iascl==1_ilp64 ) then
              call stdlib_I64_zlascl( 'G', 0_ilp64, 0_ilp64, anrm, smlnum, scllen, nrhs, b, ldb,info )
           else if( iascl==2_ilp64 ) then
              call stdlib_I64_zlascl( 'G', 0_ilp64, 0_ilp64, anrm, bignum, scllen, nrhs, b, ldb,info )
           end if
           if( ibscl==1_ilp64 ) then
              call stdlib_I64_zlascl( 'G', 0_ilp64, 0_ilp64, smlnum, bnrm, scllen, nrhs, b, ldb,info )
           else if( ibscl==2_ilp64 ) then
              call stdlib_I64_zlascl( 'G', 0_ilp64, 0_ilp64, bignum, bnrm, scllen, nrhs, b, ldb,info )
           end if
           50 continue
           work( 1_ilp64 ) = real( wsize,KIND=dp)
           return
     ...
     !! without guard digits, but we know of none.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           logical(lk) :: lquery
           integer(ilp64) :: iascl, ibscl, ie, il, itau, itaup, itauq, ldwork, liwork, maxmn, &
                     maxwrk, minmn, minwrk, mm, mnthr, nlvl, nwork, smlsiz, wlalsd
           real(sp) :: anrm, bignum, bnrm, eps, sfmin, smlnum
           ! Intrinsic Functions 
           ! Executable Statements 
           ...
           if( info==0_ilp64 ) then
              ...
              liwork = 1_ilp64
              if( minmn>0_ilp64 ) then
                 smlsiz = stdlib_I64_ilaenv( 9_ilp64, 'SGELSD', ' ', 0_ilp64, 0_ilp64, 0_ilp64, 0_ilp64 )
                 mnthr = stdlib_I64_ilaenv( 6_ilp64, 'SGELSD', ' ', m, n, nrhs, -1_ilp64 )
                 nlvl = max( int( log( real( minmn,KIND=sp) / real( smlsiz + 1_ilp64,KIND=sp) ) /log( &
                           two ),KIND=ilp64) + 1_ilp64, 0_ilp64 )
                 liwork = 3_ilp64*minmn*nlvl + 11_ilp64*minmn
                 mm = m
                 if( m>=n .and. m>=mnthr ) then
                    ! path 1a - overdetermined, with many more rows than
                              ! columns.
                    mm = n
                    maxwrk = max( maxwrk, n + n*stdlib_I64_ilaenv( 1_ilp64, 'SGEQRF', ' ', m,n, -1_ilp64, -1_ilp64 ) )
                              
                    maxwrk = max( maxwrk, n + nrhs*stdlib_I64_ilaenv( 1_ilp64, 'SORMQR', 'LT',m, nrhs, n, -&
                              1_ilp64 ) )
                 end if
                 if( m>=n ) then
                    ! path 1 - overdetermined or exactly determined.
                    maxwrk = max( maxwrk, 3_ilp64*n + ( mm + n )*stdlib_I64_ilaenv( 1_ilp64,'SGEBRD', ' ', mm, n, &
                              -1_ilp64, -1_ilp64 ) )
                    maxwrk = max( maxwrk, 3_ilp64*n + nrhs*stdlib_I64_ilaenv( 1_ilp64, 'SORMBR','QLT', mm, nrhs, &
                              n, -1_ilp64 ) )
                    maxwrk = max( maxwrk, 3_ilp64*n + ( n - 1_ilp64 )*stdlib_I64_ilaenv( 1_ilp64,'SORMBR', 'PLN', n, &
                              nrhs, n, -1_ilp64 ) )
                    wlalsd = 9_ilp64*n + 2_ilp64*n*smlsiz + 8_ilp64*n*nlvl + n*nrhs +( smlsiz + 1_ilp64 )**2_ilp64
                    maxwrk = max( maxwrk, 3_ilp64*n + wlalsd )
                    minwrk = max( 3_ilp64*n + mm, 3_ilp64*n + nrhs, 3_ilp64*n + wlalsd )
                 end if
                 if( n>m ) then
                    wlalsd = 9_ilp64*m + 2_ilp64*m*smlsiz + 8_ilp64*m*nlvl + m*nrhs +( smlsiz + 1_ilp64 )**2_ilp64
                    if( n>=mnthr ) then
                       ! path 2a - underdetermined, with many more columns
                                 ! than rows.
                       maxwrk = m + m*stdlib_I64_ilaenv( 1_ilp64, 'SGELQF', ' ', m, n, -1_ilp64,-1_ilp64 )
                       maxwrk = max( maxwrk, m*m + 4_ilp64*m + 2_ilp64*m*stdlib_I64_ilaenv( 1_ilp64,'SGEBRD', ' ', m, m,&
                                  -1_ilp64, -1_ilp64 ) )
                       maxwrk = max( maxwrk, m*m + 4_ilp64*m + nrhs*stdlib_I64_ilaenv( 1_ilp64,'SORMBR', 'QLT', m,&
                                  nrhs, m, -1_ilp64 ) )
                       maxwrk = max( maxwrk, m*m + 4_ilp64*m + ( m - 1_ilp64 )*stdlib_I64_ilaenv( 1_ilp64,'SORMBR', &
                                 'PLN', m, nrhs, m, -1_ilp64 ) )
                       if( nrhs>1_ilp64 ) then
                          maxwrk = max( maxwrk, m*m + m + m*nrhs )
                       else
                          maxwrk = max( maxwrk, m*m + 2_ilp64*m )
                       end if
                       maxwrk = max( maxwrk, m + nrhs*stdlib_I64_ilaenv( 1_ilp64, 'SORMLQ','LT', n, nrhs, m,&
                                  -1_ilp64 ) )
                       maxwrk = max( maxwrk, m*m + 4_ilp64*m + wlalsd )
           ! xxx: ensure the path 2a case below is triggered.  the workspace
           ! calculation should use queries for all routines eventually.
                       maxwrk = max( maxwrk,4_ilp64*m+m*m+max( m, 2_ilp64*m-4, nrhs, n-3*m ) )
                    else
                       ! path 2 - remaining underdetermined cases.
                       maxwrk = 3_ilp64*m + ( n + m )*stdlib_I64_ilaenv( 1_ilp64, 'SGEBRD', ' ', m,n, -1_ilp64, -1_ilp64 )
                                 
                       maxwrk = max( maxwrk, 3_ilp64*m + nrhs*stdlib_I64_ilaenv( 1_ilp64, 'SORMBR','QLT', m, nrhs,&
                                  n, -1_ilp64 ) )
                       maxwrk = max( maxwrk, 3_ilp64*m + m*stdlib_I64_ilaenv( 1_ilp64, 'SORMBR','PLN', n, nrhs, m,&
                                  -1_ilp64 ) )
                       maxwrk = max( maxwrk, 3_ilp64*m + wlalsd )
                    end if
                    minwrk = max( 3_ilp64*m + nrhs, 3_ilp64*m + m, 3_ilp64*m + wlalsd )
                 end if
              end if
              minwrk = min( minwrk, maxwrk )
              work( 1_ilp64 ) = maxwrk
              iwork( 1_ilp64 ) = liwork
              if( lwork<minwrk .and. .not.lquery ) then
                 info = -12_ilp64
              end if
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SGELSD', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           if( anrm>zero .and. anrm<smlnum ) then
              ...
              ! matrix all zero. return zero solution.
              call stdlib_I64_slaset( 'F', max( m, n ), nrhs, zero, zero, b, ldb )
              call stdlib_I64_slaset( 'F', minmn, 1_ilp64, zero, zero, s, 1_ilp64 )
              rank = 0_ilp64
              go to 10
           end if
           ! scale b if max entry outside range [smlnum,bignum].
           ...
           ! if m < n make sure certain entries of b are zero.
           if( m<n )call stdlib_I64_slaset( 'F', n-m, nrhs, zero, zero, b( m+1, 1_ilp64 ), ldb )
           ! overdetermined case.
           if( m>=n ) then
              ! path 1 - overdetermined or exactly determined.
              mm = m
              if( m>=mnthr ) then
                 ...
                 ! compute a=q*r.
                 ! (workspace: need 2*n, prefer n+n*nb)
                 call stdlib_I64_sgeqrf( m, n, a, lda, work( itau ), work( nwork ),lwork-nwork+1, &
                           info )
                 ! multiply b by transpose(q).
                 ! (workspace: need n+nrhs, prefer n+nrhs*nb)
                 call stdlib_I64_sormqr( 'L', 'T', m, nrhs, n, a, lda, work( itau ), b,ldb, work( &
                 ...
                 ! zero out below r.
                 if( n>1_ilp64 ) then
                    call stdlib_I64_slaset( 'L', n-1, n-1, zero, zero, a( 2_ilp64, 1_ilp64 ), lda )
                 end if
              end if
              ie = 1_ilp64
              itauq = ie + n
              ...
              call stdlib_I64_sormbr( 'Q', 'L', 'T', mm, nrhs, n, a, lda, work( itauq ),b, ldb, work( &
                        nwork ), lwork-nwork+1, info )
              ! solve the bidiagonal least squares problem.
              call stdlib_I64_slalsd( 'U', smlsiz, n, nrhs, s, work( ie ), b, ldb,rcond, rank, work( &
                        nwork ), iwork, info )
              if( info/=0_ilp64 ) then
                 go to 10
              end if
              ! multiply b by right bidiagonalizing vectors of r.
              call stdlib_I64_sormbr( 'P', 'L', 'N', n, nrhs, n, a, lda, work( itaup ),b, ldb, work( &
                        nwork ), lwork-nwork+1, info )
           else if( n>=mnthr .and. lwork>=4_ilp64*m+m*m+max( m, 2_ilp64*m-4, nrhs, n-3*m, wlalsd ) ) &
                     then
              ! path 2a - underdetermined, with many more columns than rows
              ! and sufficient workspace for an efficient algorithm.
              ldwork = m
              if( lwork>=max( 4_ilp64*m+m*lda+max( m, 2_ilp64*m-4, nrhs, n-3*m ),m*lda+m+m*nrhs, 4_ilp64*m+m*lda+&
                        wlalsd ) )ldwork = lda
              itau = 1_ilp64
              nwork = m + 1_ilp64
              ! compute a=l*q.
              ! (workspace: need 2*m, prefer m+m*nb)
              call stdlib_I64_sgelqf( m, n, a, lda, work( itau ), work( nwork ),lwork-nwork+1, info )
              ...
              call stdlib_I64_sormbr( 'Q', 'L', 'T', m, nrhs, m, work( il ), ldwork,work( itauq ), b, &
                        ldb, work( nwork ),lwork-nwork+1, info )
              ! solve the bidiagonal least squares problem.
              call stdlib_I64_slalsd( 'U', smlsiz, m, nrhs, s, work( ie ), b, ldb,rcond, rank, work( &
                        nwork ), iwork, info )
              if( info/=0_ilp64 ) then
                 go to 10
              end if
              ! multiply b by right bidiagonalizing vectors of l.
              call stdlib_I64_sormbr( 'P', 'L', 'N', m, nrhs, m, work( il ), ldwork,work( itaup ), b, &
                        ldb, work( nwork ),lwork-nwork+1, info )
              ! zero out below first m rows of b.
              call stdlib_I64_slaset( 'F', n-m, nrhs, zero, zero, b( m+1, 1_ilp64 ), ldb )
              nwork = itau + m
              ! multiply transpose(q) by b.
              ! (workspace: need m+nrhs, prefer m+nrhs*nb)
              call stdlib_I64_sormlq( 'L', 'T', n, nrhs, m, a, lda, work( itau ), b,ldb, work( nwork )&
                        , lwork-nwork+1, info )
           else
              ! path 2 - remaining underdetermined cases.
              ie = 1_ilp64
              ...
              call stdlib_I64_sormbr( 'Q', 'L', 'T', m, nrhs, n, a, lda, work( itauq ),b, ldb, work( &
                        nwork ), lwork-nwork+1, info )
              ! solve the bidiagonal least squares problem.
              call stdlib_I64_slalsd( 'L', smlsiz, m, nrhs, s, work( ie ), b, ldb,rcond, rank, work( &
                        nwork ), iwork, info )
              if( info/=0_ilp64 ) then
                 go to 10
              end if
              ! multiply b by right bidiagonalizing vectors of a.
              call stdlib_I64_sormbr( 'P', 'L', 'N', n, nrhs, m, a, lda, work( itaup ),b, ldb, work( &
                        nwork ), lwork-nwork+1, info )
           end if
           ! undo scaling.
           if( iascl==1_ilp64 ) then
     ...
     !! without guard digits, but we know of none.
        ...
        ! =====================================================================
           ...
           minmn = min( m, n )
           maxmn = max( m, n )
           mnthr = stdlib_I64_ilaenv( 6_ilp64, 'DGELSD', ' ', m, n, nrhs, -1_ilp64 )
           lquery = ( lwork==-1_ilp64 )
           if( m<0_ilp64 ) then
              info = -1_ilp64
           else if( n<0_ilp64 ) then
           ...
           smlsiz = stdlib_I64_ilaenv( 9_ilp64, 'DGELSD', ' ', 0_ilp64, 0_ilp64, 0_ilp64, 0_ilp64 )
           ! compute workspace.
           ! (note: comments in the code beginning "workspace:" describe the
           ! minimal amount of workspace needed at that point in the code,
           ! as well as the preferred amount for good performance.
           ...
           minwrk = 1_ilp64
           liwork = 1_ilp64
           minmn = max( 1_ilp64, minmn )
           nlvl = max( int( log( real( minmn,KIND=dp) / real( smlsiz+1,KIND=dp) ) /log( two ),&
                     KIND=ilp64) + 1_ilp64, 0_ilp64 )
           if( info==0_ilp64 ) then
              maxwrk = 0_ilp64
              liwork = 3_ilp64*minmn*nlvl + 11_ilp64*minmn
              mm = m
              if( m>=n .and. m>=mnthr ) then
                 ! path 1a - overdetermined, with many more rows than columns.
                 mm = n
                 maxwrk = max( maxwrk, n+n*stdlib_I64_ilaenv( 1_ilp64, 'DGEQRF', ' ', m, n,-1_ilp64, -1_ilp64 ) )
                           
                 maxwrk = max( maxwrk, n+nrhs*stdlib_I64_ilaenv( 1_ilp64, 'DORMQR', 'LT', m, nrhs, n, -1_ilp64 ) )
                           
              end if
              if( m>=n ) then
                 ! path 1 - overdetermined or exactly determined.
                 maxwrk = max( maxwrk, 3_ilp64*n+( mm+n )*stdlib_I64_ilaenv( 1_ilp64, 'DGEBRD', ' ', mm, n, -1_ilp64, -&
                           1_ilp64 ) )
                 maxwrk = max( maxwrk, 3_ilp64*n+nrhs*stdlib_I64_ilaenv( 1_ilp64, 'DORMBR', 'QLT', mm, nrhs, n, -&
                           1_ilp64 ) )
                 maxwrk = max( maxwrk, 3_ilp64*n+( n-1 )*stdlib_I64_ilaenv( 1_ilp64, 'DORMBR', 'PLN', n, nrhs, n, &
                           -1_ilp64 ) )
                 wlalsd = 9_ilp64*n+2*n*smlsiz+8*n*nlvl+n*nrhs+(smlsiz+1)**2_ilp64
                 maxwrk = max( maxwrk, 3_ilp64*n+wlalsd )
                 minwrk = max( 3_ilp64*n+mm, 3_ilp64*n+nrhs, 3_ilp64*n+wlalsd )
              end if
              if( n>m ) then
                 wlalsd = 9_ilp64*m+2*m*smlsiz+8*m*nlvl+m*nrhs+(smlsiz+1)**2_ilp64
                 if( n>=mnthr ) then
                    ! path 2a - underdetermined, with many more columns
                    ! than rows.
                    maxwrk = m + m*stdlib_I64_ilaenv( 1_ilp64, 'DGELQF', ' ', m, n, -1_ilp64, -1_ilp64 )
                    maxwrk = max( maxwrk, m*m+4*m+2*m*stdlib_I64_ilaenv( 1_ilp64, 'DGEBRD', ' ', m, m, -1_ilp64, -&
                              1_ilp64 ) )
                    maxwrk = max( maxwrk, m*m+4*m+nrhs*stdlib_I64_ilaenv( 1_ilp64, 'DORMBR', 'QLT', m, nrhs,&
                               m, -1_ilp64 ) )
                    maxwrk = max( maxwrk, m*m+4*m+( m-1 )*stdlib_I64_ilaenv( 1_ilp64, 'DORMBR', 'PLN', m, &
                              nrhs, m, -1_ilp64 ) )
                    if( nrhs>1_ilp64 ) then
                       maxwrk = max( maxwrk, m*m+m+m*nrhs )
                    else
                    ...
                    maxwrk = max( maxwrk, m+nrhs*stdlib_I64_ilaenv( 1_ilp64, 'DORMLQ', 'LT', n, nrhs, m, -1_ilp64 &
                              ) )
                    maxwrk = max( maxwrk, m*m+4*m+wlalsd )
           ! xxx: ensure the path 2a case below is triggered.  the workspace
           ! calculation should use queries for all routines eventually.
                    ...
                    ! path 2 - remaining underdetermined cases.
                    maxwrk = 3_ilp64*m + ( n+m )*stdlib_I64_ilaenv( 1_ilp64, 'DGEBRD', ' ', m, n,-1_ilp64, -1_ilp64 )
                    maxwrk = max( maxwrk, 3_ilp64*m+nrhs*stdlib_I64_ilaenv( 1_ilp64, 'DORMBR', 'QLT', m, nrhs, n, &
                              -1_ilp64 ) )
                    maxwrk = max( maxwrk, 3_ilp64*m+m*stdlib_I64_ilaenv( 1_ilp64, 'DORMBR', 'PLN', n, nrhs, m, -1_ilp64 &
                              ) )
                    maxwrk = max( maxwrk, 3_ilp64*m+wlalsd )
                 end if
                 minwrk = max( 3_ilp64*m+nrhs, 3_ilp64*m+m, 3_ilp64*m+wlalsd )
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DGELSD', -info )
              return
           else if( lquery ) then
              go to 10
           end if
           ...
           if( anrm>zero .and. anrm<smlnum ) then
              ...
              ! matrix all zero. return zero solution.
              call stdlib_I64_dlaset( 'F', max( m, n ), nrhs, zero, zero, b, ldb )
              call stdlib_I64_dlaset( 'F', minmn, 1_ilp64, zero, zero, s, 1_ilp64 )
              rank = 0_ilp64
              go to 10
           end if
           ! scale b if max entry outside range [smlnum,bignum].
           ...
           ! if m < n make sure certain entries of b are zero.
           if( m<n )call stdlib_I64_dlaset( 'F', n-m, nrhs, zero, zero, b( m+1, 1_ilp64 ), ldb )
           ! overdetermined case.
           if( m>=n ) then
              ! path 1 - overdetermined or exactly determined.
              mm = m
              if( m>=mnthr ) then
                 ...
                 ! compute a=q*r.
                 ! (workspace: need 2*n, prefer n+n*nb)
                 call stdlib_I64_dgeqrf( m, n, a, lda, work( itau ), work( nwork ),lwork-nwork+1, &
                           info )
                 ! multiply b by transpose(q).
                 ! (workspace: need n+nrhs, prefer n+nrhs*nb)
                 call stdlib_I64_dormqr( 'L', 'T', m, nrhs, n, a, lda, work( itau ), b,ldb, work( &
                 ...
                 ! zero out below r.
                 if( n>1_ilp64 ) then
                    call stdlib_I64_dlaset( 'L', n-1, n-1, zero, zero, a( 2_ilp64, 1_ilp64 ), lda )
                 end if
              end if
              ie = 1_ilp64
              itauq = ie + n
              ...
              call stdlib_I64_dormbr( 'Q', 'L', 'T', mm, nrhs, n, a, lda, work( itauq ),b, ldb, work( &
                        nwork ), lwork-nwork+1, info )
              ! solve the bidiagonal least squares problem.
              call stdlib_I64_dlalsd( 'U', smlsiz, n, nrhs, s, work( ie ), b, ldb,rcond, rank, work( &
                        nwork ), iwork, info )
              if( info/=0_ilp64 ) then
                 go to 10
              end if
              ! multiply b by right bidiagonalizing vectors of r.
              call stdlib_I64_dormbr( 'P', 'L', 'N', n, nrhs, n, a, lda, work( itaup ),b, ldb, work( &
                        nwork ), lwork-nwork+1, info )
           else if( n>=mnthr .and. lwork>=4_ilp64*m+m*m+max( m, 2_ilp64*m-4, nrhs, n-3*m, wlalsd ) ) &
                     then
              ! path 2a - underdetermined, with many more columns than rows
              ...
              ! compute a=l*q.
              ! (workspace: need 2*m, prefer m+m*nb)
              call stdlib_I64_dgelqf( m, n, a, lda, work( itau ), work( nwork ),lwork-nwork+1, info )
              ...
              call stdlib_I64_dormbr( 'Q', 'L', 'T', m, nrhs, m, work( il ), ldwork,work( itauq ), b, &
                        ldb, work( nwork ),lwork-nwork+1, info )
              ! solve the bidiagonal least squares problem.
              call stdlib_I64_dlalsd( 'U', smlsiz, m, nrhs, s, work( ie ), b, ldb,rcond, rank, work( &
                        nwork ), iwork, info )
              if( info/=0_ilp64 ) then
                 go to 10
              end if
              ! multiply b by right bidiagonalizing vectors of l.
              call stdlib_I64_dormbr( 'P', 'L', 'N', m, nrhs, m, work( il ), ldwork,work( itaup ), b, &
                        ldb, work( nwork ),lwork-nwork+1, info )
              ! zero out below first m rows of b.
              call stdlib_I64_dlaset( 'F', n-m, nrhs, zero, zero, b( m+1, 1_ilp64 ), ldb )
              nwork = itau + m
              ! multiply transpose(q) by b.
              ! (workspace: need m+nrhs, prefer m+nrhs*nb)
              call stdlib_I64_dormlq( 'L', 'T', n, nrhs, m, a, lda, work( itau ), b,ldb, work( nwork )&
                        , lwork-nwork+1, info )
           else
              ! path 2 - remaining underdetermined cases.
              ie = 1_ilp64
              ...
              call stdlib_I64_dormbr( 'Q', 'L', 'T', m, nrhs, n, a, lda, work( itauq ),b, ldb, work( &
                        nwork ), lwork-nwork+1, info )
              ! solve the bidiagonal least squares problem.
              call stdlib_I64_dlalsd( 'L', smlsiz, m, nrhs, s, work( ie ), b, ldb,rcond, rank, work( &
                        nwork ), iwork, info )
              if( info/=0_ilp64 ) then
                 go to 10
              end if
              ! multiply b by right bidiagonalizing vectors of a.
              call stdlib_I64_dormbr( 'P', 'L', 'N', n, nrhs, m, a, lda, work( itaup ),b, ldb, work( &
                        nwork ), lwork-nwork+1, info )
           end if
           ! undo scaling.
           if( iascl==1_ilp64 ) then
     ...
     !! without guard digits, but we know of none.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           logical(lk) :: lquery
           integer(ilp64) :: iascl, ibscl, ie, il, itau, itaup, itauq, ldwork, liwork, lrwork, &
                     maxmn, maxwrk, minmn, minwrk, mm, mnthr, nlvl, nrwork, nwork, smlsiz
           real(sp) :: anrm, bignum, bnrm, eps, sfmin, smlnum
           ! Intrinsic Functions 
           ! Executable Statements 
           ...
           if( info==0_ilp64 ) then
              ...
              lrwork = 1_ilp64
              if( minmn>0_ilp64 ) then
                 smlsiz = stdlib_I64_ilaenv( 9_ilp64, 'CGELSD', ' ', 0_ilp64, 0_ilp64, 0_ilp64, 0_ilp64 )
                 mnthr = stdlib_I64_ilaenv( 6_ilp64, 'CGELSD', ' ', m, n, nrhs, -1_ilp64 )
                 nlvl = max( int( log( real( minmn,KIND=sp) / real( smlsiz + 1_ilp64,KIND=sp) ) /log( &
                           two ),KIND=ilp64) + 1_ilp64, 0_ilp64 )
                 liwork = 3_ilp64*minmn*nlvl + 11_ilp64*minmn
                 mm = m
                 if( m>=n .and. m>=mnthr ) then
                    ! path 1a - overdetermined, with many more rows than
                              ! columns.
                    mm = n
                    maxwrk = max( maxwrk, n*stdlib_I64_ilaenv( 1_ilp64, 'CGEQRF', ' ', m, n,-1_ilp64, -1_ilp64 ) )
                              
                    maxwrk = max( maxwrk, nrhs*stdlib_I64_ilaenv( 1_ilp64, 'CUNMQR', 'LC', m,nrhs, n, -1_ilp64 ) )
                              
                 end if
                 if( m>=n ) then
                    ! path 1 - overdetermined or exactly determined.
                    lrwork = 10_ilp64*n + 2_ilp64*n*smlsiz + 8_ilp64*n*nlvl + 3_ilp64*smlsiz*nrhs +max( (smlsiz+1)**2_ilp64, n*(&
                              1_ilp64+nrhs) + 2_ilp64*nrhs )
                    maxwrk = max( maxwrk, 2_ilp64*n + ( mm + n )*stdlib_I64_ilaenv( 1_ilp64,'CGEBRD', ' ', mm, n, &
                              -1_ilp64, -1_ilp64 ) )
                    maxwrk = max( maxwrk, 2_ilp64*n + nrhs*stdlib_I64_ilaenv( 1_ilp64, 'CUNMBR','QLC', mm, nrhs, &
                              n, -1_ilp64 ) )
                    maxwrk = max( maxwrk, 2_ilp64*n + ( n - 1_ilp64 )*stdlib_I64_ilaenv( 1_ilp64,'CUNMBR', 'PLN', n, &
                              nrhs, n, -1_ilp64 ) )
                    maxwrk = max( maxwrk, 2_ilp64*n + n*nrhs )
                    minwrk = max( 2_ilp64*n + mm, 2_ilp64*n + n*nrhs )
                 end if
                 if( n>m ) then
                    lrwork = 10_ilp64*m + 2_ilp64*m*smlsiz + 8_ilp64*m*nlvl + 3_ilp64*smlsiz*nrhs +max( (smlsiz+1)**2_ilp64, n*(&
                              1_ilp64+nrhs) + 2_ilp64*nrhs )
                    if( n>=mnthr ) then
                       ! path 2a - underdetermined, with many more columns
                                 ! than rows.
                       maxwrk = m + m*stdlib_I64_ilaenv( 1_ilp64, 'CGELQF', ' ', m, n, -1_ilp64,-1_ilp64 )
                       maxwrk = max( maxwrk, m*m + 4_ilp64*m + 2_ilp64*m*stdlib_I64_ilaenv( 1_ilp64,'CGEBRD', ' ', m, m,&
                                  -1_ilp64, -1_ilp64 ) )
                       maxwrk = max( maxwrk, m*m + 4_ilp64*m + nrhs*stdlib_I64_ilaenv( 1_ilp64,'CUNMBR', 'QLC', m,&
                                  nrhs, m, -1_ilp64 ) )
                       maxwrk = max( maxwrk, m*m + 4_ilp64*m + ( m - 1_ilp64 )*stdlib_I64_ilaenv( 1_ilp64,'CUNMLQ', &
                                 'LC', n, nrhs, m, -1_ilp64 ) )
                       if( nrhs>1_ilp64 ) then
                          maxwrk = max( maxwrk, m*m + m + m*nrhs )
                       else
                       ...
                       maxwrk = max( maxwrk, m*m + 4_ilp64*m + m*nrhs )
           ! xxx: ensure the path 2a case below is triggered.  the workspace
           ! calculation should use queries for all routines eventually.
                       maxwrk = max( maxwrk,4_ilp64*m+m*m+max( m, 2_ilp64*m-4, nrhs, n-3*m ) )
                    else
                       ! path 2 - underdetermined.
                       maxwrk = 2_ilp64*m + ( n + m )*stdlib_I64_ilaenv( 1_ilp64, 'CGEBRD', ' ', m,n, -1_ilp64, -1_ilp64 )
                                 
                       maxwrk = max( maxwrk, 2_ilp64*m + nrhs*stdlib_I64_ilaenv( 1_ilp64, 'CUNMBR','QLC', m, nrhs,&
                                  m, -1_ilp64 ) )
                       maxwrk = max( maxwrk, 2_ilp64*m + m*stdlib_I64_ilaenv( 1_ilp64, 'CUNMBR','PLN', n, nrhs, m,&
                                  -1_ilp64 ) )
                       maxwrk = max( maxwrk, 2_ilp64*m + m*nrhs )
                    end if
                    minwrk = max( 2_ilp64*m + n, 2_ilp64*m + m*nrhs )
                 end if
              end if
              minwrk = min( minwrk, maxwrk )
              work( 1_ilp64 ) = maxwrk
              iwork( 1_ilp64 ) = liwork
              rwork( 1_ilp64 ) = lrwork
              if( lwork<minwrk .and. .not.lquery ) then
                 info = -12_ilp64
              end if
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CGELSD', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           ! if m < n make sure b(m+1:n,:) = 0
           if( m<n )call stdlib_I64_claset( 'F', n-m, nrhs, czero, czero, b( m+1, 1_ilp64 ), ldb )
           ! overdetermined case.
           if( m>=n ) then
              ! path 1 - overdetermined or exactly determined.
              mm = m
              if( m>=mnthr ) then
                 ...
                 ! (rworkspace: need n)
                 ! (cworkspace: need n, prefer n*nb)
                 call stdlib_I64_cgeqrf( m, n, a, lda, work( itau ), work( nwork ),lwork-nwork+1, &
                           info )
                 ! multiply b by transpose(q).
                 ! (rworkspace: need n)
                 ! (cworkspace: need nrhs, prefer nrhs*nb)
                 ...
                 ! zero out below r.
                 if( n>1_ilp64 ) then
                    call stdlib_I64_claset( 'L', n-1, n-1, czero, czero, a( 2_ilp64, 1_ilp64 ),lda )
                 end if
              end if
              itauq = 1_ilp64
              itaup = itauq + n
              ...
              call stdlib_I64_cunmbr( 'Q', 'L', 'C', mm, nrhs, n, a, lda, work( itauq ),b, ldb, work( &
                        nwork ), lwork-nwork+1, info )
              ! solve the bidiagonal least squares problem.
              call stdlib_I64_clalsd( 'U', smlsiz, n, nrhs, s, rwork( ie ), b, ldb,rcond, rank, work( &
                        nwork ), rwork( nrwork ),iwork, info )
              if( info/=0_ilp64 ) then
                 go to 10
              end if
              ! multiply b by right bidiagonalizing vectors of r.
              call stdlib_I64_cunmbr( 'P', 'L', 'N', n, nrhs, n, a, lda, work( itaup ),b, ldb, work( &
                        nwork ), lwork-nwork+1, info )
           else if( n>=mnthr .and. lwork>=4_ilp64*m+m*m+max( m, 2_ilp64*m-4, nrhs, n-3*m ) ) then
              ! path 2a - underdetermined, with many more columns than rows
              ! and sufficient workspace for an efficient algorithm.
              ...
              ! compute a=l*q.
              ! (cworkspace: need 2*m, prefer m+m*nb)
              call stdlib_I64_cgelqf( m, n, a, lda, work( itau ), work( nwork ),lwork-nwork+1, info )
              ...
              call stdlib_I64_cunmbr( 'Q', 'L', 'C', m, nrhs, m, work( il ), ldwork,work( itauq ), b, &
                        ldb, work( nwork ),lwork-nwork+1, info )
              ! solve the bidiagonal least squares problem.
              call stdlib_I64_clalsd( 'U', smlsiz, m, nrhs, s, rwork( ie ), b, ldb,rcond, rank, work( &
                        nwork ), rwork( nrwork ),iwork, info )
              if( info/=0_ilp64 ) then
                 go to 10
              end if
              ! multiply b by right bidiagonalizing vectors of l.
              call stdlib_I64_cunmbr( 'P', 'L', 'N', m, nrhs, m, work( il ), ldwork,work( itaup ), b, &
                        ldb, work( nwork ),lwork-nwork+1, info )
              ! zero out below first m rows of b.
              call stdlib_I64_claset( 'F', n-m, nrhs, czero, czero, b( m+1, 1_ilp64 ), ldb )
              nwork = itau + m
              ! multiply transpose(q) by b.
              ! (cworkspace: need nrhs, prefer nrhs*nb)
              call stdlib_I64_cunmlq( 'L', 'C', n, nrhs, m, a, lda, work( itau ), b,ldb, work( nwork )&
                        , lwork-nwork+1, info )
           else
              ! path 2 - remaining underdetermined cases.
              itauq = 1_ilp64
              ...
              call stdlib_I64_cunmbr( 'Q', 'L', 'C', m, nrhs, n, a, lda, work( itauq ),b, ldb, work( &
                        nwork ), lwork-nwork+1, info )
              ! solve the bidiagonal least squares problem.
              call stdlib_I64_clalsd( 'L', smlsiz, m, nrhs, s, rwork( ie ), b, ldb,rcond, rank, work( &
                        nwork ), rwork( nrwork ),iwork, info )
              if( info/=0_ilp64 ) then
                 go to 10
              end if
              ! multiply b by right bidiagonalizing vectors of a.
              call stdlib_I64_cunmbr( 'P', 'L', 'N', n, nrhs, m, a, lda, work( itaup ),b, ldb, work( &
                        nwork ), lwork-nwork+1, info )
           end if
           ! undo scaling.
           if( iascl==1_ilp64 ) then
     ...
     !! without guard digits, but we know of none.
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              ...
              lrwork = 1_ilp64
              if( minmn>0_ilp64 ) then
                 smlsiz = stdlib_I64_ilaenv( 9_ilp64, 'ZGELSD', ' ', 0_ilp64, 0_ilp64, 0_ilp64, 0_ilp64 )
                 mnthr = stdlib_I64_ilaenv( 6_ilp64, 'ZGELSD', ' ', m, n, nrhs, -1_ilp64 )
                 nlvl = max( int( log( real( minmn,KIND=dp) / real( smlsiz + 1_ilp64,KIND=dp) ) /log( &
                           two ),KIND=ilp64) + 1_ilp64, 0_ilp64 )
                 liwork = 3_ilp64*minmn*nlvl + 11_ilp64*minmn
                 mm = m
                 if( m>=n .and. m>=mnthr ) then
                    ! path 1a - overdetermined, with many more rows than
                              ! columns.
                    mm = n
                    maxwrk = max( maxwrk, n*stdlib_I64_ilaenv( 1_ilp64, 'ZGEQRF', ' ', m, n,-1_ilp64, -1_ilp64 ) )
                              
                    maxwrk = max( maxwrk, nrhs*stdlib_I64_ilaenv( 1_ilp64, 'ZUNMQR', 'LC', m,nrhs, n, -1_ilp64 ) )
                 ...
                 if( m>=n ) then
                    ...
                    lrwork = 10_ilp64*n + 2_ilp64*n*smlsiz + 8_ilp64*n*nlvl + 3_ilp64*smlsiz*nrhs +max( (smlsiz+1)**2_ilp64, n*(&
                              1_ilp64+nrhs) + 2_ilp64*nrhs )
                    maxwrk = max( maxwrk, 2_ilp64*n + ( mm + n )*stdlib_I64_ilaenv( 1_ilp64,'ZGEBRD', ' ', mm, n, &
                              -1_ilp64, -1_ilp64 ) )
                    maxwrk = max( maxwrk, 2_ilp64*n + nrhs*stdlib_I64_ilaenv( 1_ilp64, 'ZUNMBR','QLC', mm, nrhs, &
                              n, -1_ilp64 ) )
                    maxwrk = max( maxwrk, 2_ilp64*n + ( n - 1_ilp64 )*stdlib_I64_ilaenv( 1_ilp64,'ZUNMBR', 'PLN', n, &
                              nrhs, n, -1_ilp64 ) )
                    maxwrk = max( maxwrk, 2_ilp64*n + n*nrhs )
                    minwrk = max( 2_ilp64*n + mm, 2_ilp64*n + n*nrhs )
                 end if
                 if( n>m ) then
                    ...
                    if( n>=mnthr ) then
                       ! path 2a - underdetermined, with many more columns
                                 ! than rows.
                       maxwrk = m + m*stdlib_I64_ilaenv( 1_ilp64, 'ZGELQF', ' ', m, n, -1_ilp64,-1_ilp64 )
                       maxwrk = max( maxwrk, m*m + 4_ilp64*m + 2_ilp64*m*stdlib_I64_ilaenv( 1_ilp64,'ZGEBRD', ' ', m, m,&
                                  -1_ilp64, -1_ilp64 ) )
                       maxwrk = max( maxwrk, m*m + 4_ilp64*m + nrhs*stdlib_I64_ilaenv( 1_ilp64,'ZUNMBR', 'QLC', m,&
                                  nrhs, m, -1_ilp64 ) )
                       maxwrk = max( maxwrk, m*m + 4_ilp64*m + ( m - 1_ilp64 )*stdlib_I64_ilaenv( 1_ilp64,'ZUNMLQ', &
                                 'LC', n, nrhs, m, -1_ilp64 ) )
                       if( nrhs>1_ilp64 ) then
                          maxwrk = max( maxwrk, m*m + m + m*nrhs )
                       else
           ...
           ! calculation should use queries for all routines eventually.
                       ...
                       ! path 2 - underdetermined.
                       maxwrk = 2_ilp64*m + ( n + m )*stdlib_I64_ilaenv( 1_ilp64, 'ZGEBRD', ' ', m,n, -1_ilp64, -1_ilp64 )
                                 
                       maxwrk = max( maxwrk, 2_ilp64*m + nrhs*stdlib_I64_ilaenv( 1_ilp64, 'ZUNMBR','QLC', m, nrhs,&
                                  m, -1_ilp64 ) )
                       maxwrk = max( maxwrk, 2_ilp64*m + m*stdlib_I64_ilaenv( 1_ilp64, 'ZUNMBR','PLN', n, nrhs, m,&
                                  -1_ilp64 ) )
                       maxwrk = max( maxwrk, 2_ilp64*m + m*nrhs )
                    end if
                    minwrk = max( 2_ilp64*m + n, 2_ilp64*m + m*nrhs )
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZGELSD', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           ! if m < n make sure b(m+1:n,:) = 0
           if( m<n )call stdlib_I64_zlaset( 'F', n-m, nrhs, czero, czero, b( m+1, 1_ilp64 ), ldb )
           ! overdetermined case.
           if( m>=n ) then
              ! path 1 - overdetermined or exactly determined.
              mm = m
              if( m>=mnthr ) then
                 ...
                 ! (rworkspace: need n)
                 ! (cworkspace: need n, prefer n*nb)
                 call stdlib_I64_zgeqrf( m, n, a, lda, work( itau ), work( nwork ),lwork-nwork+1, &
                           info )
                 ! multiply b by transpose(q).
                 ! (rworkspace: need n)
                 ! (cworkspace: need nrhs, prefer nrhs*nb)
                 ...
                 ! zero out below r.
                 if( n>1_ilp64 ) then
                    call stdlib_I64_zlaset( 'L', n-1, n-1, czero, czero, a( 2_ilp64, 1_ilp64 ),lda )
                 end if
              end if
              itauq = 1_ilp64
              itaup = itauq + n
              ...
              call stdlib_I64_zunmbr( 'Q', 'L', 'C', mm, nrhs, n, a, lda, work( itauq ),b, ldb, work( &
                        nwork ), lwork-nwork+1, info )
              ! solve the bidiagonal least squares problem.
              call stdlib_I64_zlalsd( 'U', smlsiz, n, nrhs, s, rwork( ie ), b, ldb,rcond, rank, work( &
                        nwork ), rwork( nrwork ),iwork, info )
              if( info/=0_ilp64 ) then
                 go to 10
              end if
              ! multiply b by right bidiagonalizing vectors of r.
              call stdlib_I64_zunmbr( 'P', 'L', 'N', n, nrhs, n, a, lda, work( itaup ),b, ldb, work( &
                        nwork ), lwork-nwork+1, info )
           else if( n>=mnthr .and. lwork>=4_ilp64*m+m*m+max( m, 2_ilp64*m-4, nrhs, n-3*m ) ) then
              ! path 2a - underdetermined, with many more columns than rows
              ! and sufficient workspace for an efficient algorithm.
              ...
              ! compute a=l*q.
              ! (cworkspace: need 2*m, prefer m+m*nb)
              call stdlib_I64_zgelqf( m, n, a, lda, work( itau ), work( nwork ),lwork-nwork+1, info )
              ...
              call stdlib_I64_zunmbr( 'Q', 'L', 'C', m, nrhs, m, work( il ), ldwork,work( itauq ), b, &
                        ldb, work( nwork ),lwork-nwork+1, info )
              ! solve the bidiagonal least squares problem.
              call stdlib_I64_zlalsd( 'U', smlsiz, m, nrhs, s, rwork( ie ), b, ldb,rcond, rank, work( &
                        nwork ), rwork( nrwork ),iwork, info )
              if( info/=0_ilp64 ) then
                 go to 10
              end if
              ! multiply b by right bidiagonalizing vectors of l.
              call stdlib_I64_zunmbr( 'P', 'L', 'N', m, nrhs, m, work( il ), ldwork,work( itaup ), b, &
                        ldb, work( nwork ),lwork-nwork+1, info )
              ! zero out below first m rows of b.
              call stdlib_I64_zlaset( 'F', n-m, nrhs, czero, czero, b( m+1, 1_ilp64 ), ldb )
              nwork = itau + m
              ! multiply transpose(q) by b.
              ! (cworkspace: need nrhs, prefer nrhs*nb)
              call stdlib_I64_zunmlq( 'L', 'C', n, nrhs, m, a, lda, work( itau ), b,ldb, work( nwork )&
                        , lwork-nwork+1, info )
           else
              ! path 2 - remaining underdetermined cases.
              itauq = 1_ilp64
              ...
              call stdlib_I64_zunmbr( 'Q', 'L', 'C', m, nrhs, n, a, lda, work( itauq ),b, ldb, work( &
                        nwork ), lwork-nwork+1, info )
              ! solve the bidiagonal least squares problem.
              call stdlib_I64_zlalsd( 'L', smlsiz, m, nrhs, s, rwork( ie ), b, ldb,rcond, rank, work( &
                        nwork ), rwork( nrwork ),iwork, info )
              if( info/=0_ilp64 ) then
                 go to 10
              end if
              ! multiply b by right bidiagonalizing vectors of a.
              call stdlib_I64_zunmbr( 'P', 'L', 'N', n, nrhs, m, a, lda, work( itaup ),b, ldb, work( &
                        nwork ), lwork-nwork+1, info )
           end if
           ! undo scaling.
           if( iascl==1_ilp64 ) then
     ...
     !! matrix X.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           logical(lk) :: lquery, tran
           integer(ilp64) :: i, iascl, ibscl, j, maxmn, brow, scllen, tszo, tszm, lwo, lwm, lw1, &
                     lw2, wsizeo, wsizem, info2
           real(sp) :: anrm, bignum, bnrm, smlnum, tq(5_ilp64), workq(1_ilp64)
           ! Intrinsic Functions 
           ! Executable Statements 
           ! test the input arguments.
           info = 0_ilp64
           maxmn = max( m, n )
           tran  = stdlib_lsame( trans, 'T' )
           lquery = ( lwork==-1_ilp64 .or. lwork==-2_ilp64 )
           if( .not.( stdlib_lsame( trans, 'N' ) .or.stdlib_lsame( trans, 'T' ) ) ) then
              info = -1_ilp64
           else if( m<0_ilp64 ) then
              info = -2_ilp64
           ...
           ! determine the optimum and minimum lwork
            if( m>=n ) then
              call stdlib_I64_sgeqr( m, n, a, lda, tq, -1_ilp64, workq, -1_ilp64, info2 )
              tszo = int( tq( 1_ilp64 ),KIND=ilp64)
              lwo  = int( workq( 1_ilp64 ),KIND=ilp64)
              call stdlib_I64_sgemqr( 'L', trans, m, nrhs, n, a, lda, tq,tszo, b, ldb, workq, -1_ilp64, &
                        info2 )
              lwo  = max( lwo, int( workq( 1_ilp64 ),KIND=ilp64) )
              call stdlib_I64_sgeqr( m, n, a, lda, tq, -2_ilp64, workq, -2_ilp64, info2 )
              tszm = int( tq( 1_ilp64 ),KIND=ilp64)
              lwm  = int( workq( 1_ilp64 ),KIND=ilp64)
              call stdlib_I64_sgemqr( 'L', trans, m, nrhs, n, a, lda, tq,tszm, b, ldb, workq, -1_ilp64, &
                        info2 )
              lwm = max( lwm, int( workq( 1_ilp64 ),KIND=ilp64) )
              wsizeo = tszo + lwo
              wsizem = tszm + lwm
            else
              call stdlib_I64_sgelq( m, n, a, lda, tq, -1_ilp64, workq, -1_ilp64, info2 )
              tszo = int( tq( 1_ilp64 ),KIND=ilp64)
              lwo  = int( workq( 1_ilp64 ),KIND=ilp64)
              call stdlib_I64_sgemlq( 'L', trans, n, nrhs, m, a, lda, tq,tszo, b, ldb, workq, -1_ilp64, &
                        info2 )
              lwo  = max( lwo, int( workq( 1_ilp64 ),KIND=ilp64) )
              call stdlib_I64_sgelq( m, n, a, lda, tq, -2_ilp64, workq, -2_ilp64, info2 )
              tszm = int( tq( 1_ilp64 ),KIND=ilp64)
              lwm  = int( workq( 1_ilp64 ),KIND=ilp64)
              call stdlib_I64_sgemlq( 'L', trans, n, nrhs, m, a, lda, tq,tszm, b, ldb, workq, -1_ilp64, &
                        info2 )
              lwm  = max( lwm, int( workq( 1_ilp64 ),KIND=ilp64) )
              wsizeo = tszo + lwo
              wsizem = tszm + lwm
            ...
            work( 1_ilp64 ) = real( wsizeo,KIND=sp)
           end if
           if( info/=0_ilp64 ) then
             call stdlib_I64_xerbla( 'SGETSLS', -info )
             return
           end if
           if( lquery ) then
             if( lwork==-2_ilp64 ) work( 1_ilp64 ) = real( wsizem,KIND=sp)
             return
           end if
           if( lwork<wsizeo ) then
             lw1 = tszm
           ...
           if( anrm>zero .and. anrm<smlnum ) then
              ...
              ! matrix all zero. return zero solution.
              call stdlib_I64_slaset( 'F', maxmn, nrhs, zero, zero, b, ldb )
              go to 50
           end if
           brow = m
           if ( tran ) then
           ...
           bnrm = stdlib_I64_slange( 'M', brow, nrhs, b, ldb, work )
           ibscl = 0_ilp64
           if( bnrm>zero .and. bnrm<smlnum ) then
              ! scale matrix norm up to smlnum
              call stdlib_I64_slascl( 'G', 0_ilp64, 0_ilp64, bnrm, smlnum, brow, nrhs, b, ldb,info )
           ...
           if ( m>=n ) then
              ! compute qr factorization of a
             call stdlib_I64_sgeqr( m, n, a, lda, work( lw2+1 ), lw1,work( 1_ilp64 ), lw2, info )
             if ( .not.tran ) then
                 ! least-squares problem min || a * x - b ||
                 ! b(1:m,1:nrhs) := q**t * b(1:m,1:nrhs)
               call stdlib_I64_sgemqr( 'L' , 'T', m, nrhs, n, a, lda,work( lw2+1 ), lw1, b, ldb, work(&
                          1_ilp64 ), lw2,info )
                 ! b(1:n,1:nrhs) := inv(r) * b(1:n,1:nrhs)
               call stdlib_I64_strtrs( 'U', 'N', 'N', n, nrhs,a, lda, b, ldb, info )
               if( info>0_ilp64 ) then
                 return
               end if
               scllen = n
                 ...
                 ! overdetermined system of equations a**t * x = b
                 ! b(1:n,1:nrhs) := inv(r**t) * b(1:n,1:nrhs)
                 call stdlib_I64_strtrs( 'U', 'T', 'N', n, nrhs,a, lda, b, ldb, info )
                 if( info>0_ilp64 ) then
                    return
                 end if
                 ! b(n+1:m,1:nrhs) = zero
                 ...
                 ! b(1:m,1:nrhs) := q(1:n,:) * b(1:n,1:nrhs)
                 call stdlib_I64_sgemqr( 'L', 'N', m, nrhs, n, a, lda,work( lw2+1 ), lw1, b, ldb, &
                           work( 1_ilp64 ), lw2,info )
                 scllen = m
              end if
           else
              ! compute lq factorization of a
              call stdlib_I64_sgelq( m, n, a, lda, work( lw2+1 ), lw1,work( 1_ilp64 ), lw2, info )
              ! workspace at least m, optimally m*nb.
              if( .not.tran ) then
                 ! underdetermined system of equations a * x = b
                 ! b(1:m,1:nrhs) := inv(l) * b(1:m,1:nrhs)
                 call stdlib_I64_strtrs( 'L', 'N', 'N', m, nrhs,a, lda, b, ldb, info )
                 if( info>0_ilp64 ) then
                    return
                 end if
                 ! b(m+1:n,1:nrhs) = 0
                 ...
                 ! b(1:n,1:nrhs) := q(1:n,:)**t * b(1:m,1:nrhs)
                 call stdlib_I64_sgemlq( 'L', 'T', n, nrhs, m, a, lda,work( lw2+1 ), lw1, b, ldb, &
                           work( 1_ilp64 ), lw2,info )
                 ! workspace at least nrhs, optimally nrhs*nb
                 scllen = n
              else
                 ! overdetermined system min || a**t * x - b ||
                 ! b(1:n,1:nrhs) := q * b(1:n,1:nrhs)
                 call stdlib_I64_sgemlq( 'L', 'N', n, nrhs, m, a, lda,work( lw2+1 ), lw1, b, ldb, &
                           work( 1_ilp64 ), lw2,info )
                 ! workspace at least nrhs, optimally nrhs*nb
                 ! b(1:m,1:nrhs) := inv(l**t) * b(1:m,1:nrhs)
                 call stdlib_I64_strtrs( 'LOWER', 'TRANSPOSE', 'NON-UNIT', m, nrhs,a, lda, b, ldb, &
           ...
           50 continue
           work( 1_ilp64 ) = real( tszo + lwo,KIND=sp)
           return
     ...
     !! matrix X.
        ...
        ! =====================================================================
           ...
           integer(ilp64) :: i, iascl, ibscl, j, maxmn, brow, scllen, tszo, tszm, lwo, lwm, lw1, &
                     lw2, wsizeo, wsizem, info2
           real(dp) :: anrm, bignum, bnrm, smlnum, tq(5_ilp64), workq(1_ilp64)
           ! Intrinsic Functions 
           ! Executable Statements 
           ! test the input arguments.
           info = 0_ilp64
           ...
           ! determine the optimum and minimum lwork
            if( m>=n ) then
              call stdlib_I64_dgeqr( m, n, a, lda, tq, -1_ilp64, workq, -1_ilp64, info2 )
              tszo = int( tq( 1_ilp64 ),KIND=ilp64)
              lwo  = int( workq( 1_ilp64 ),KIND=ilp64)
              call stdlib_I64_dgemqr( 'L', trans, m, nrhs, n, a, lda, tq,tszo, b, ldb, workq, -1_ilp64, &
                        info2 )
              lwo  = max( lwo, int( workq( 1_ilp64 ),KIND=ilp64) )
              call stdlib_I64_dgeqr( m, n, a, lda, tq, -2_ilp64, workq, -2_ilp64, info2 )
              tszm = int( tq( 1_ilp64 ),KIND=ilp64)
              lwm  = int( workq( 1_ilp64 ),KIND=ilp64)
              call stdlib_I64_dgemqr( 'L', trans, m, nrhs, n, a, lda, tq,tszm, b, ldb, workq, -1_ilp64, &
                        info2 )
              lwm = max( lwm, int( workq( 1_ilp64 ),KIND=ilp64) )
              wsizeo = tszo + lwo
              wsizem = tszm + lwm
            else
              call stdlib_I64_dgelq( m, n, a, lda, tq, -1_ilp64, workq, -1_ilp64, info2 )
              tszo = int( tq( 1_ilp64 ),KIND=ilp64)
              lwo  = int( workq( 1_ilp64 ),KIND=ilp64)
              call stdlib_I64_dgemlq( 'L', trans, n, nrhs, m, a, lda, tq,tszo, b, ldb, workq, -1_ilp64, &
                        info2 )
              lwo  = max( lwo, int( workq( 1_ilp64 ),KIND=ilp64) )
              call stdlib_I64_dgelq( m, n, a, lda, tq, -2_ilp64, workq, -2_ilp64, info2 )
              tszm = int( tq( 1_ilp64 ),KIND=ilp64)
              lwm  = int( workq( 1_ilp64 ),KIND=ilp64)
              call stdlib_I64_dgemlq( 'L', trans, n, nrhs, m, a, lda, tq,tszm, b, ldb, workq, -1_ilp64, &
                        info2 )
              lwm  = max( lwm, int( workq( 1_ilp64 ),KIND=ilp64) )
              wsizeo = tszo + lwo
              wsizem = tszm + lwm
            ...
            work( 1_ilp64 ) = real( wsizeo,KIND=dp)
           end if
           if( info/=0_ilp64 ) then
             call stdlib_I64_xerbla( 'DGETSLS', -info )
             return
           end if
           if( lquery ) then
             if( lwork==-2_ilp64 ) work( 1_ilp64 ) = real( wsizem,KIND=dp)
             return
           end if
           if( lwork<wsizeo ) then
             lw1 = tszm
           ...
           if( anrm>zero .and. anrm<smlnum ) then
              ...
              ! matrix all zero. return zero solution.
              call stdlib_I64_dlaset( 'F', maxmn, nrhs, zero, zero, b, ldb )
              go to 50
           end if
           brow = m
           if ( tran ) then
           ...
           bnrm = stdlib_I64_dlange( 'M', brow, nrhs, b, ldb, work )
           ibscl = 0_ilp64
           if( bnrm>zero .and. bnrm<smlnum ) then
              ! scale matrix norm up to smlnum
              call stdlib_I64_dlascl( 'G', 0_ilp64, 0_ilp64, bnrm, smlnum, brow, nrhs, b, ldb,info )
           ...
           if ( m>=n ) then
              ! compute qr factorization of a
             call stdlib_I64_dgeqr( m, n, a, lda, work( lw2+1 ), lw1,work( 1_ilp64 ), lw2, info )
             if ( .not.tran ) then
                 ! least-squares problem min || a * x - b ||
                 ! b(1:m,1:nrhs) := q**t * b(1:m,1:nrhs)
               call stdlib_I64_dgemqr( 'L' , 'T', m, nrhs, n, a, lda,work( lw2+1 ), lw1, b, ldb, work(&
                          1_ilp64 ), lw2,info )
                 ! b(1:n,1:nrhs) := inv(r) * b(1:n,1:nrhs)
               call stdlib_I64_dtrtrs( 'U', 'N', 'N', n, nrhs,a, lda, b, ldb, info )
               if( info>0_ilp64 ) then
                 return
               end if
               scllen = n
                 ...
                 ! overdetermined system of equations a**t * x = b
                 ! b(1:n,1:nrhs) := inv(r**t) * b(1:n,1:nrhs)
                 call stdlib_I64_dtrtrs( 'U', 'T', 'N', n, nrhs,a, lda, b, ldb, info )
                 if( info>0_ilp64 ) then
                    return
                 end if
                 ! b(n+1:m,1:nrhs) = zero
                 ...
                 ! b(1:m,1:nrhs) := q(1:n,:) * b(1:n,1:nrhs)
                 call stdlib_I64_dgemqr( 'L', 'N', m, nrhs, n, a, lda,work( lw2+1 ), lw1, b, ldb, &
                           work( 1_ilp64 ), lw2,info )
                 scllen = m
              end if
           else
              ! compute lq factorization of a
              call stdlib_I64_dgelq( m, n, a, lda, work( lw2+1 ), lw1,work( 1_ilp64 ), lw2, info )
              ! workspace at least m, optimally m*nb.
              if( .not.tran ) then
                 ! underdetermined system of equations a * x = b
                 ! b(1:m,1:nrhs) := inv(l) * b(1:m,1:nrhs)
                 call stdlib_I64_dtrtrs( 'L', 'N', 'N', m, nrhs,a, lda, b, ldb, info )
                 if( info>0_ilp64 ) then
                    return
                 end if
                 ! b(m+1:n,1:nrhs) = 0
                 ...
                 ! b(1:n,1:nrhs) := q(1:n,:)**t * b(1:m,1:nrhs)
                 call stdlib_I64_dgemlq( 'L', 'T', n, nrhs, m, a, lda,work( lw2+1 ), lw1, b, ldb, &
                           work( 1_ilp64 ), lw2,info )
                 ! workspace at least nrhs, optimally nrhs*nb
                 scllen = n
              else
                 ! overdetermined system min || a**t * x - b ||
                 ! b(1:n,1:nrhs) := q * b(1:n,1:nrhs)
                 call stdlib_I64_dgemlq( 'L', 'N', n, nrhs, m, a, lda,work( lw2+1 ), lw1, b, ldb, &
                           work( 1_ilp64 ), lw2,info )
                 ! workspace at least nrhs, optimally nrhs*nb
                 ! b(1:m,1:nrhs) := inv(l**t) * b(1:m,1:nrhs)
                 call stdlib_I64_dtrtrs( 'LOWER', 'TRANSPOSE', 'NON-UNIT', m, nrhs,a, lda, b, ldb, &
           ...
           50 continue
           work( 1_ilp64 ) = real( tszo + lwo,KIND=dp)
           return
     ...
     !! matrix X.
        ...
        ! =====================================================================
           ...
           integer(ilp64) :: i, iascl, ibscl, j, maxmn, brow, scllen, tszo, tszm, lwo, lwm, lw1, &
                     lw2, wsizeo, wsizem, info2
           real(sp) :: anrm, bignum, bnrm, smlnum, dum(1_ilp64)
           complex(sp) :: tq(5_ilp64), workq(1_ilp64)
           ! Intrinsic Functions 
           ! Executable Statements 
           ! test the input arguments.
           info = 0_ilp64
           ...
           ! determine the optimum and minimum lwork
            if( m>=n ) then
              call stdlib_I64_cgeqr( m, n, a, lda, tq, -1_ilp64, workq, -1_ilp64, info2 )
              tszo = int( tq( 1_ilp64 ),KIND=ilp64)
              lwo  = int( workq( 1_ilp64 ),KIND=ilp64)
              call stdlib_I64_cgemqr( 'L', trans, m, nrhs, n, a, lda, tq,tszo, b, ldb, workq, -1_ilp64, &
                        info2 )
              lwo  = max( lwo, int( workq( 1_ilp64 ),KIND=ilp64) )
              call stdlib_I64_cgeqr( m, n, a, lda, tq, -2_ilp64, workq, -2_ilp64, info2 )
              tszm = int( tq( 1_ilp64 ),KIND=ilp64)
              lwm  = int( workq( 1_ilp64 ),KIND=ilp64)
              call stdlib_I64_cgemqr( 'L', trans, m, nrhs, n, a, lda, tq,tszm, b, ldb, workq, -1_ilp64, &
                        info2 )
              lwm = max( lwm, int( workq( 1_ilp64 ),KIND=ilp64) )
              wsizeo = tszo + lwo
              wsizem = tszm + lwm
            else
              call stdlib_I64_cgelq( m, n, a, lda, tq, -1_ilp64, workq, -1_ilp64, info2 )
              tszo = int( tq( 1_ilp64 ),KIND=ilp64)
              lwo  = int( workq( 1_ilp64 ),KIND=ilp64)
              call stdlib_I64_cgemlq( 'L', trans, n, nrhs, m, a, lda, tq,tszo, b, ldb, workq, -1_ilp64, &
                        info2 )
              lwo  = max( lwo, int( workq( 1_ilp64 ),KIND=ilp64) )
              call stdlib_I64_cgelq( m, n, a, lda, tq, -2_ilp64, workq, -2_ilp64, info2 )
              tszm = int( tq( 1_ilp64 ),KIND=ilp64)
              lwm  = int( workq( 1_ilp64 ),KIND=ilp64)
              call stdlib_I64_cgemlq( 'L', trans, n, nrhs, m, a, lda, tq,tszm, b, ldb, workq, -1_ilp64, &
                        info2 )
              lwm  = max( lwm, int( workq( 1_ilp64 ),KIND=ilp64) )
              wsizeo = tszo + lwo
              wsizem = tszm + lwm
           ...
           if( info/=0_ilp64 ) then
             call stdlib_I64_xerbla( 'CGETSLS', -info )
             return
           end if
           if( lquery ) then
             if( lwork==-2_ilp64 ) work( 1_ilp64 ) = real( wsizem,KIND=sp)
           ...
           ! get machine parameters
            ...
            call stdlib_I64_slabad( smlnum, bignum )
           ! scale a, b if max element outside range [smlnum,bignum]
           anrm = stdlib_I64_clange( 'M', m, n, a, lda, dum )
           iascl = 0_ilp64
           if( anrm>zero .and. anrm<smlnum ) then
              ! scale matrix norm up to smlnum
              call stdlib_I64_clascl( 'G', 0_ilp64, 0_ilp64, anrm, smlnum, m, n, a, lda, info )
              ...
              ! matrix all zero. return zero solution.
              call stdlib_I64_claset( 'F', maxmn, nrhs, czero, czero, b, ldb )
              go to 50
           end if
           brow = m
           if ( tran ) then
           ...
           bnrm = stdlib_I64_clange( 'M', brow, nrhs, b, ldb, dum )
           ibscl = 0_ilp64
           if( bnrm>zero .and. bnrm<smlnum ) then
              ! scale matrix norm up to smlnum
              call stdlib_I64_clascl( 'G', 0_ilp64, 0_ilp64, bnrm, smlnum, brow, nrhs, b, ldb,info )
           ...
           if ( m>=n ) then
              ! compute qr factorization of a
             call stdlib_I64_cgeqr( m, n, a, lda, work( lw2+1 ), lw1,work( 1_ilp64 ), lw2, info )
             if ( .not.tran ) then
                 ! least-squares problem min || a * x - b ||
                 ! b(1:m,1:nrhs) := q**t * b(1:m,1:nrhs)
               call stdlib_I64_cgemqr( 'L' , 'C', m, nrhs, n, a, lda,work( lw2+1 ), lw1, b, ldb, work(&
                          1_ilp64 ), lw2,info )
                 ! b(1:n,1:nrhs) := inv(r) * b(1:n,1:nrhs)
               call stdlib_I64_ctrtrs( 'U', 'N', 'N', n, nrhs,a, lda, b, ldb, info )
               if( info>0_ilp64 ) then
                 return
               end if
               scllen = n
                 ...
                 ! overdetermined system of equations a**t * x = b
                 ! b(1:n,1:nrhs) := inv(r**t) * b(1:n,1:nrhs)
                 call stdlib_I64_ctrtrs( 'U', 'C', 'N', n, nrhs,a, lda, b, ldb, info )
                 if( info>0_ilp64 ) then
                    return
                 end if
                 ! b(n+1:m,1:nrhs) = czero
                 ...
                 ! b(1:m,1:nrhs) := q(1:n,:) * b(1:n,1:nrhs)
                 call stdlib_I64_cgemqr( 'L', 'N', m, nrhs, n, a, lda,work( lw2+1 ), lw1, b, ldb, &
                           work( 1_ilp64 ), lw2,info )
                 scllen = m
              end if
           else
              ! compute lq factorization of a
              call stdlib_I64_cgelq( m, n, a, lda, work( lw2+1 ), lw1,work( 1_ilp64 ), lw2, info )
              ! workspace at least m, optimally m*nb.
              if( .not.tran ) then
                 ! underdetermined system of equations a * x = b
                 ! b(1:m,1:nrhs) := inv(l) * b(1:m,1:nrhs)
                 call stdlib_I64_ctrtrs( 'L', 'N', 'N', m, nrhs,a, lda, b, ldb, info )
                 if( info>0_ilp64 ) then
                    return
                 end if
                 ! b(m+1:n,1:nrhs) = 0
                 ...
                 ! b(1:n,1:nrhs) := q(1:n,:)**t * b(1:m,1:nrhs)
                 call stdlib_I64_cgemlq( 'L', 'C', n, nrhs, m, a, lda,work( lw2+1 ), lw1, b, ldb, &
                           work( 1_ilp64 ), lw2,info )
                 ! workspace at least nrhs, optimally nrhs*nb
                 scllen = n
              else
                 ! overdetermined system min || a**t * x - b ||
                 ! b(1:n,1:nrhs) := q * b(1:n,1:nrhs)
                 call stdlib_I64_cgemlq( 'L', 'N', n, nrhs, m, a, lda,work( lw2+1 ), lw1, b, ldb, &
                           work( 1_ilp64 ), lw2,info )
                 ! workspace at least nrhs, optimally nrhs*nb
                 ! b(1:m,1:nrhs) := inv(l**t) * b(1:m,1:nrhs)
                 call stdlib_I64_ctrtrs( 'L', 'C', 'N', m, nrhs,a, lda, b, ldb, info )
                 if( info>0_ilp64 ) then
                    return
                 end if
                 scllen = m
     ...
     !! matrix X.
        ...
        ! =====================================================================
           ...
           integer(ilp64) :: i, iascl, ibscl, j, maxmn, brow, scllen, tszo, tszm, lwo, lwm, lw1, &
                     lw2, wsizeo, wsizem, info2
           real(dp) :: anrm, bignum, bnrm, smlnum, dum(1_ilp64)
           complex(dp) :: tq(5_ilp64), workq(1_ilp64)
           ! Intrinsic Functions 
           ! Executable Statements 
           ! test the input arguments.
           info = 0_ilp64
           ...
           ! determine the optimum and minimum lwork
            if( m>=n ) then
              call stdlib_I64_zgeqr( m, n, a, lda, tq, -1_ilp64, workq, -1_ilp64, info2 )
              tszo = int( tq( 1_ilp64 ),KIND=ilp64)
              lwo  = int( workq( 1_ilp64 ),KIND=ilp64)
              call stdlib_I64_zgemqr( 'L', trans, m, nrhs, n, a, lda, tq,tszo, b, ldb, workq, -1_ilp64, &
                        info2 )
              lwo  = max( lwo, int( workq( 1_ilp64 ),KIND=ilp64) )
              call stdlib_I64_zgeqr( m, n, a, lda, tq, -2_ilp64, workq, -2_ilp64, info2 )
              tszm = int( tq( 1_ilp64 ),KIND=ilp64)
              lwm  = int( workq( 1_ilp64 ),KIND=ilp64)
              call stdlib_I64_zgemqr( 'L', trans, m, nrhs, n, a, lda, tq,tszm, b, ldb, workq, -1_ilp64, &
                        info2 )
              lwm = max( lwm, int( workq( 1_ilp64 ),KIND=ilp64) )
              wsizeo = tszo + lwo
              wsizem = tszm + lwm
            else
              call stdlib_I64_zgelq( m, n, a, lda, tq, -1_ilp64, workq, -1_ilp64, info2 )
              tszo = int( tq( 1_ilp64 ),KIND=ilp64)
              lwo  = int( workq( 1_ilp64 ),KIND=ilp64)
              call stdlib_I64_zgemlq( 'L', trans, n, nrhs, m, a, lda, tq,tszo, b, ldb, workq, -1_ilp64, &
                        info2 )
              lwo  = max( lwo, int( workq( 1_ilp64 ),KIND=ilp64) )
              call stdlib_I64_zgelq( m, n, a, lda, tq, -2_ilp64, workq, -2_ilp64, info2 )
              tszm = int( tq( 1_ilp64 ),KIND=ilp64)
              lwm  = int( workq( 1_ilp64 ),KIND=ilp64)
              call stdlib_I64_zgemlq( 'L', trans, n, nrhs, m, a, lda, tq,tszm, b, ldb, workq, -1_ilp64, &
                        info2 )
              lwm  = max( lwm, int( workq( 1_ilp64 ),KIND=ilp64) )
              wsizeo = tszo + lwo
              wsizem = tszm + lwm
           ...
           if( info/=0_ilp64 ) then
             call stdlib_I64_xerbla( 'ZGETSLS', -info )
             return
           end if
           if( lquery ) then
             if( lwork==-2_ilp64 ) work( 1_ilp64 ) = real( wsizem,KIND=dp)
           ...
           ! get machine parameters
            ...
            call stdlib_I64_dlabad( smlnum, bignum )
           ! scale a, b if max element outside range [smlnum,bignum]
           anrm = stdlib_I64_zlange( 'M', m, n, a, lda, dum )
           iascl = 0_ilp64
           if( anrm>zero .and. anrm<smlnum ) then
              ! scale matrix norm up to smlnum
              call stdlib_I64_zlascl( 'G', 0_ilp64, 0_ilp64, anrm, smlnum, m, n, a, lda, info )
              ...
              ! matrix all zero. return zero solution.
              call stdlib_I64_zlaset( 'F', maxmn, nrhs, czero, czero, b, ldb )
              go to 50
           end if
           brow = m
           if ( tran ) then
           ...
           bnrm = stdlib_I64_zlange( 'M', brow, nrhs, b, ldb, dum )
           ibscl = 0_ilp64
           if( bnrm>zero .and. bnrm<smlnum ) then
              ! scale matrix norm up to smlnum
              call stdlib_I64_zlascl( 'G', 0_ilp64, 0_ilp64, bnrm, smlnum, brow, nrhs, b, ldb,info )
           ...
           if ( m>=n ) then
              ! compute qr factorization of a
             call stdlib_I64_zgeqr( m, n, a, lda, work( lw2+1 ), lw1,work( 1_ilp64 ), lw2, info )
             if ( .not.tran ) then
                 ! least-squares problem min || a * x - b ||
                 ! b(1:m,1:nrhs) := q**t * b(1:m,1:nrhs)
               call stdlib_I64_zgemqr( 'L' , 'C', m, nrhs, n, a, lda,work( lw2+1 ), lw1, b, ldb, work(&
                          1_ilp64 ), lw2,info )
                 ! b(1:n,1:nrhs) := inv(r) * b(1:n,1:nrhs)
               call stdlib_I64_ztrtrs( 'U', 'N', 'N', n, nrhs,a, lda, b, ldb, info )
               if( info>0_ilp64 ) then
                 return
               end if
               scllen = n
                 ...
                 ! overdetermined system of equations a**t * x = b
                 ! b(1:n,1:nrhs) := inv(r**t) * b(1:n,1:nrhs)
                 call stdlib_I64_ztrtrs( 'U', 'C', 'N', n, nrhs,a, lda, b, ldb, info )
                 if( info>0_ilp64 ) then
                    return
                 end if
                 ! b(n+1:m,1:nrhs) = czero
                 ...
                 ! b(1:m,1:nrhs) := q(1:n,:) * b(1:n,1:nrhs)
                 call stdlib_I64_zgemqr( 'L', 'N', m, nrhs, n, a, lda,work( lw2+1 ), lw1, b, ldb, &
                           work( 1_ilp64 ), lw2,info )
                 scllen = m
              end if
           else
              ! compute lq factorization of a
              call stdlib_I64_zgelq( m, n, a, lda, work( lw2+1 ), lw1,work( 1_ilp64 ), lw2, info )
              ! workspace at least m, optimally m*nb.
              if( .not.tran ) then
                 ! underdetermined system of equations a * x = b
                 ! b(1:m,1:nrhs) := inv(l) * b(1:m,1:nrhs)
                 call stdlib_I64_ztrtrs( 'L', 'N', 'N', m, nrhs,a, lda, b, ldb, info )
                 if( info>0_ilp64 ) then
                    return
                 end if
                 ! b(m+1:n,1:nrhs) = 0
                 ...
                 ! b(1:n,1:nrhs) := q(1:n,:)**t * b(1:m,1:nrhs)
                 call stdlib_I64_zgemlq( 'L', 'C', n, nrhs, m, a, lda,work( lw2+1 ), lw1, b, ldb, &
                           work( 1_ilp64 ), lw2,info )
                 ! workspace at least nrhs, optimally nrhs*nb
                 scllen = n
              else
                 ! overdetermined system min || a**t * x - b ||
                 ! b(1:n,1:nrhs) := q * b(1:n,1:nrhs)
                 call stdlib_I64_zgemlq( 'L', 'N', n, nrhs, m, a, lda,work( lw2+1 ), lw1, b, ldb, &
                           work( 1_ilp64 ), lw2,info )
                 ! workspace at least nrhs, optimally nrhs*nb
                 ! b(1:m,1:nrhs) := inv(l**t) * b(1:m,1:nrhs)
                 call stdlib_I64_ztrtrs( 'L', 'C', 'N', m, nrhs,a, lda, b, ldb, info )
                 if( info>0_ilp64 ) then
                    return
                 end if
                 scllen = m
