  ...
  contains
     ...
     !! generalized eigenvalue problem A*x = lambda*B*x.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           integer(ilp64) :: i, icab, iflow, ip1, ir, irab, it, j, jc, jp1, k, kount, l, lcab, lm1, &
                     lrab, lsfmax, lsfmin, m, nr, nrp2
           real(sp) :: alpha, basl, beta, cab, cmax, coef, coef2, coef5, cor, ew, ewc, gamma, &
                     pgamma, rab, sfmax, sfmin, sum, t, ta, tb, tc
           ! Intrinsic Functions 
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SGGBAL', -info )
              return
           end if
           ! quick return if possible
           if( n==0_ilp64 ) then
           ...
           lscale( m ) = i
           if( i==m )go to 170
           call stdlib_I64_sswap( n-k+1, a( i, k ), lda, a( m, k ), lda )
           call stdlib_I64_sswap( n-k+1, b( i, k ), ldb, b( m, k ), ldb )
           ! permute columns m and j
           170 continue
           rscale( m ) = j
           if( j==m )go to 180
           call stdlib_I64_sswap( l, a( 1_ilp64, j ), 1_ilp64, a( 1_ilp64, m ), 1_ilp64 )
           call stdlib_I64_sswap( l, b( 1_ilp64, j ), 1_ilp64, b( 1_ilp64, m ), 1_ilp64 )
           180 continue
           go to ( 20, 90 )iflow
           190 continue
           ilo = k
           ...
           coef = one / real( 2_ilp64*nr,KIND=sp)
           coef2 = coef*coef
           coef5 = half*coef2
           nrp2 = nr + 2_ilp64
           beta = zero
           ...
           ! start generalized conjugate gradient iteration
           250 continue
           gamma = stdlib_I64_sdot( nr, work( ilo+4*n ), 1_ilp64, work( ilo+4*n ), 1_ilp64 ) +stdlib_I64_sdot( nr, &
                     work( ilo+5*n ), 1_ilp64, work( ilo+5*n ), 1_ilp64 )
           ew = zero
           ewc = zero
           do i = ilo, ihi
              ew = ew + work( i+4*n )
           ...
           gamma = coef*gamma - coef2*( ew**2_ilp64+ewc**2_ilp64 ) - coef5*( ew-ewc )**2_ilp64
           if( gamma==zero )go to 350
           if( it/=1_ilp64 )beta = gamma / pgamma
           t = coef5*( ewc-three*ew )
           tc = coef5*( ew-three*ewc )
           call stdlib_I64_sscal( nr, beta, work( ilo ), 1_ilp64 )
           call stdlib_I64_sscal( nr, beta, work( ilo+n ), 1_ilp64 )
           call stdlib_I64_saxpy( nr, coef, work( ilo+4*n ), 1_ilp64, work( ilo+n ), 1_ilp64 )
           call stdlib_I64_saxpy( nr, coef, work( ilo+5*n ), 1_ilp64, work( ilo ), 1_ilp64 )
           do i = ilo, ihi
              work( i ) = work( i ) + tc
              work( i+n ) = work( i+n ) + t
           end do
           ...
           sum = stdlib_I64_sdot( nr, work( ilo+n ), 1_ilp64, work( ilo+2*n ), 1_ilp64 ) +stdlib_I64_sdot( nr, work( &
                     ilo ), 1_ilp64, work( ilo+3*n ), 1_ilp64 )
           alpha = gamma / sum
           ! determine correction to current iteration
           cmax = zero
           do i = ilo, ihi
           ...
           if( cmax<half )go to 350
           call stdlib_I64_saxpy( nr, -alpha, work( ilo+2*n ), 1_ilp64, work( ilo+4*n ), 1_ilp64 )
           call stdlib_I64_saxpy( nr, -alpha, work( ilo+3*n ), 1_ilp64, work( ilo+5*n ), 1_ilp64 )
           pgamma = gamma
           it = it + 1_ilp64
           if( it<=nrp2 )go to 250
           ! end generalized conjugate gradient iteration
           ...
           sfmax = one / sfmin
           lsfmin = int( log10( sfmin ) / basl+one,KIND=ilp64)
           lsfmax = int( log10( sfmax ) / basl,KIND=ilp64)
           do i = ilo, ihi
              irab = stdlib_I64_isamax( n-ilo+1, a( i, ilo ), lda )
              rab = abs( a( i, irab+ilo-1 ) )
              irab = stdlib_I64_isamax( n-ilo+1, b( i, ilo ), ldb )
              rab = max( rab, abs( b( i, irab+ilo-1 ) ) )
              lrab = int( log10( rab+sfmin ) / basl+one,KIND=ilp64)
              ir = lscale( i ) + sign( half, lscale( i ) )
              ir = min( max( ir, lsfmin ), lsfmax, lsfmax-lrab )
              lscale( i ) = sclfac**ir
              icab = stdlib_I64_isamax( ihi, a( 1_ilp64, i ), 1_ilp64 )
              cab = abs( a( icab, i ) )
              icab = stdlib_I64_isamax( ihi, b( 1_ilp64, i ), 1_ilp64 )
              cab = max( cab, abs( b( icab, i ) ) )
              lcab = int( log10( cab+sfmin ) / basl+one,KIND=ilp64)
              jc = rscale( i ) + sign( half, rscale( i ) )
              jc = min( max( jc, lsfmin ), lsfmax, lsfmax-lcab )
              rscale( i ) = sclfac**jc
           end do
           ! row scaling of matrices a and b
           do i = ilo, ihi
              call stdlib_I64_sscal( n-ilo+1, lscale( i ), a( i, ilo ), lda )
              call stdlib_I64_sscal( n-ilo+1, lscale( i ), b( i, ilo ), ldb )
           end do
           ! column scaling of matrices a and b
           do j = ilo, ihi
              call stdlib_I64_sscal( ihi, rscale( j ), a( 1_ilp64, j ), 1_ilp64 )
              call stdlib_I64_sscal( ihi, rscale( j ), b( 1_ilp64, j ), 1_ilp64 )
           end do
           return
     ...
     !! generalized eigenvalue problem A*x = lambda*B*x.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DGGBAL', -info )
              return
           end if
           ! quick return if possible
           if( n==0_ilp64 ) then
           ...
           lscale( m ) = i
           if( i==m )go to 170
           call stdlib_I64_dswap( n-k+1, a( i, k ), lda, a( m, k ), lda )
           call stdlib_I64_dswap( n-k+1, b( i, k ), ldb, b( m, k ), ldb )
           ! permute columns m and j
           170 continue
           rscale( m ) = j
           if( j==m )go to 180
           call stdlib_I64_dswap( l, a( 1_ilp64, j ), 1_ilp64, a( 1_ilp64, m ), 1_ilp64 )
           call stdlib_I64_dswap( l, b( 1_ilp64, j ), 1_ilp64, b( 1_ilp64, m ), 1_ilp64 )
           180 continue
           go to ( 20, 90 )iflow
           190 continue
           ilo = k
           ...
           coef = one / real( 2_ilp64*nr,KIND=dp)
           coef2 = coef*coef
           coef5 = half*coef2
           nrp2 = nr + 2_ilp64
           beta = zero
           ...
           ! start generalized conjugate gradient iteration
           250 continue
           gamma = stdlib_I64_ddot( nr, work( ilo+4*n ), 1_ilp64, work( ilo+4*n ), 1_ilp64 ) +stdlib_I64_ddot( nr, &
                     work( ilo+5*n ), 1_ilp64, work( ilo+5*n ), 1_ilp64 )
           ew = zero
           ewc = zero
           do i = ilo, ihi
           ...
           t = coef5*( ewc-three*ew )
           tc = coef5*( ew-three*ewc )
           call stdlib_I64_dscal( nr, beta, work( ilo ), 1_ilp64 )
           call stdlib_I64_dscal( nr, beta, work( ilo+n ), 1_ilp64 )
           call stdlib_I64_daxpy( nr, coef, work( ilo+4*n ), 1_ilp64, work( ilo+n ), 1_ilp64 )
           call stdlib_I64_daxpy( nr, coef, work( ilo+5*n ), 1_ilp64, work( ilo ), 1_ilp64 )
           do i = ilo, ihi
              work( i ) = work( i ) + tc
              work( i+n ) = work( i+n ) + t
           end do
           ...
           sum = stdlib_I64_ddot( nr, work( ilo+n ), 1_ilp64, work( ilo+2*n ), 1_ilp64 ) +stdlib_I64_ddot( nr, work( &
                     ilo ), 1_ilp64, work( ilo+3*n ), 1_ilp64 )
           alpha = gamma / sum
           ! determine correction to current iteration
           cmax = zero
           ...
           if( cmax<half )go to 350
           call stdlib_I64_daxpy( nr, -alpha, work( ilo+2*n ), 1_ilp64, work( ilo+4*n ), 1_ilp64 )
           call stdlib_I64_daxpy( nr, -alpha, work( ilo+3*n ), 1_ilp64, work( ilo+5*n ), 1_ilp64 )
           pgamma = gamma
           it = it + 1_ilp64
           if( it<=nrp2 )go to 250
           ! end generalized conjugate gradient iteration
           ...
           lsfmax = int( log10( sfmax ) / basl,KIND=ilp64)
           do i = ilo, ihi
              irab = stdlib_I64_idamax( n-ilo+1, a( i, ilo ), lda )
              rab = abs( a( i, irab+ilo-1 ) )
              irab = stdlib_I64_idamax( n-ilo+1, b( i, ilo ), ldb )
              rab = max( rab, abs( b( i, irab+ilo-1 ) ) )
              lrab = int( log10( rab+sfmin ) / basl+one,KIND=ilp64)
              ir = int(lscale( i ) + sign( half, lscale( i ) ),KIND=ilp64)
              ir = min( max( ir, lsfmin ), lsfmax, lsfmax-lrab )
              lscale( i ) = sclfac**ir
              icab = stdlib_I64_idamax( ihi, a( 1_ilp64, i ), 1_ilp64 )
              cab = abs( a( icab, i ) )
              icab = stdlib_I64_idamax( ihi, b( 1_ilp64, i ), 1_ilp64 )
              cab = max( cab, abs( b( icab, i ) ) )
              lcab = int( log10( cab+sfmin ) / basl+one,KIND=ilp64)
              jc = int(rscale( i ) + sign( half, rscale( i ) ),KIND=ilp64)
              jc = min( max( jc, lsfmin ), lsfmax, lsfmax-lcab )
              rscale( i ) = sclfac**jc
           end do
           ! row scaling of matrices a and b
           do i = ilo, ihi
              call stdlib_I64_dscal( n-ilo+1, lscale( i ), a( i, ilo ), lda )
              call stdlib_I64_dscal( n-ilo+1, lscale( i ), b( i, ilo ), ldb )
           end do
           ! column scaling of matrices a and b
           do j = ilo, ihi
              call stdlib_I64_dscal( ihi, rscale( j ), a( 1_ilp64, j ), 1_ilp64 )
              call stdlib_I64_dscal( ihi, rscale( j ), b( 1_ilp64, j ), 1_ilp64 )
           end do
           return
     ...
     !! generalized eigenvalue problem A*x = lambda*B*x.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CGGBAL', -info )
              return
           end if
           ! quick return if possible
           if( n==0_ilp64 ) then
           ...
           lscale( m ) = i
           if( i==m )go to 170
           call stdlib_I64_cswap( n-k+1, a( i, k ), lda, a( m, k ), lda )
           call stdlib_I64_cswap( n-k+1, b( i, k ), ldb, b( m, k ), ldb )
           ! permute columns m and j
           170 continue
           rscale( m ) = j
           if( j==m )go to 180
           call stdlib_I64_cswap( l, a( 1_ilp64, j ), 1_ilp64, a( 1_ilp64, m ), 1_ilp64 )
           call stdlib_I64_cswap( l, b( 1_ilp64, j ), 1_ilp64, b( 1_ilp64, m ), 1_ilp64 )
           180 continue
           go to ( 20, 90 )iflow
           190 continue
           ilo = k
           ...
           lsfmax = int( log10( sfmax ) / basl,KIND=ilp64)
           do i = ilo, ihi
              irab = stdlib_I64_icamax( n-ilo+1, a( i, ilo ), lda )
              rab = abs( a( i, irab+ilo-1 ) )
              irab = stdlib_I64_icamax( n-ilo+1, b( i, ilo ), ldb )
              rab = max( rab, abs( b( i, irab+ilo-1 ) ) )
              lrab = int( log10( rab+sfmin ) / basl+one,KIND=ilp64)
              ir = lscale( i ) + sign( half, lscale( i ) )
              ir = min( max( ir, lsfmin ), lsfmax, lsfmax-lrab )
              lscale( i ) = sclfac**ir
              icab = stdlib_I64_icamax( ihi, a( 1_ilp64, i ), 1_ilp64 )
              cab = abs( a( icab, i ) )
              icab = stdlib_I64_icamax( ihi, b( 1_ilp64, i ), 1_ilp64 )
              cab = max( cab, abs( b( icab, i ) ) )
              lcab = int( log10( cab+sfmin ) / basl+one,KIND=ilp64)
              jc = rscale( i ) + sign( half, rscale( i ) )
              jc = min( max( jc, lsfmin ), lsfmax, lsfmax-lcab )
           ...
           ! row scaling of matrices a and b
           do i = ilo, ihi
              call stdlib_I64_csscal( n-ilo+1, lscale( i ), a( i, ilo ), lda )
              call stdlib_I64_csscal( n-ilo+1, lscale( i ), b( i, ilo ), ldb )
           end do
           ! column scaling of matrices a and b
           do j = ilo, ihi
              call stdlib_I64_csscal( ihi, rscale( j ), a( 1_ilp64, j ), 1_ilp64 )
              call stdlib_I64_csscal( ihi, rscale( j ), b( 1_ilp64, j ), 1_ilp64 )
           end do
           return
     ...
     !! generalized eigenvalue problem A*x = lambda*B*x.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZGGBAL', -info )
              return
           end if
           ! quick return if possible
           if( n==0_ilp64 ) then
           ...
           l = lm1
           if( l/=1 )go to 30
           rscale( 1_ilp64 ) = 1_ilp64
           lscale( 1_ilp64 ) = 1_ilp64
           go to 190
           30 continue
           lm1 = l - 1_ilp64
           loop_80: do i = l, 1, -1
           ...
           lscale( m ) = i
           if( i==m )go to 170
           call stdlib_I64_zswap( n-k+1, a( i, k ), lda, a( m, k ), lda )
           call stdlib_I64_zswap( n-k+1, b( i, k ), ldb, b( m, k ), ldb )
           ! permute columns m and j
           170 continue
           rscale( m ) = j
           if( j==m )go to 180
           call stdlib_I64_zswap( l, a( 1_ilp64, j ), 1_ilp64, a( 1_ilp64, m ), 1_ilp64 )
           call stdlib_I64_zswap( l, b( 1_ilp64, j ), 1_ilp64, b( 1_ilp64, m ), 1_ilp64 )
           180 continue
           go to ( 20, 90 )iflow
           190 continue
           ilo = k
           ...
           lsfmax = int( log10( sfmax ) / basl,KIND=ilp64)
           do i = ilo, ihi
              irab = stdlib_I64_izamax( n-ilo+1, a( i, ilo ), lda )
              rab = abs( a( i, irab+ilo-1 ) )
              irab = stdlib_I64_izamax( n-ilo+1, b( i, ilo ), ldb )
              rab = max( rab, abs( b( i, irab+ilo-1 ) ) )
              lrab = int( log10( rab+sfmin ) / basl+one,KIND=ilp64)
              ir = int(lscale( i ) + sign( half, lscale( i ) ),KIND=ilp64)
              ir = min( max( ir, lsfmin ), lsfmax, lsfmax-lrab )
              lscale( i ) = sclfac**ir
              icab = stdlib_I64_izamax( ihi, a( 1_ilp64, i ), 1_ilp64 )
              cab = abs( a( icab, i ) )
              icab = stdlib_I64_izamax( ihi, b( 1_ilp64, i ), 1_ilp64 )
              cab = max( cab, abs( b( icab, i ) ) )
              lcab = int( log10( cab+sfmin ) / basl+one,KIND=ilp64)
              jc = int(rscale( i ) + sign( half, rscale( i ) ),KIND=ilp64)
              jc = min( max( jc, lsfmin ), lsfmax, lsfmax-lcab )
           ...
           ! row scaling of matrices a and b
           do i = ilo, ihi
              call stdlib_I64_zdscal( n-ilo+1, lscale( i ), a( i, ilo ), lda )
              call stdlib_I64_zdscal( n-ilo+1, lscale( i ), b( i, ilo ), ldb )
           end do
           ! column scaling of matrices a and b
           do j = ilo, ihi
              call stdlib_I64_zdscal( ihi, rscale( j ), a( 1_ilp64, j ), 1_ilp64 )
              call stdlib_I64_zdscal( ihi, rscale( j ), b( 1_ilp64, j ), 1_ilp64 )
           end do
           return
     ...
     !! problem to generalized Hessenberg form.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           logical(lk) :: ilq, ilz
           integer(ilp64) :: icompq, icompz, jcol, jrow
           real(sp) :: c, s, temp
           ! Intrinsic Functions 
           ! Executable Statements 
           ! decode compq
           ...
           if( icompq<=0_ilp64 ) then
              ...
              info = -9_ilp64
           else if( ( ilq .and. ldq<n ) .or. ldq<1_ilp64 ) then
              info = -11_ilp64
           else if( ( ilz .and. ldz<n ) .or. ldz<1_ilp64 ) then
              info = -13_ilp64
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SGGHRD', -info )
              return
           end if
           ! initialize q and z if desired.
           if( icompq==3_ilp64 )call stdlib_I64_slaset( 'FULL', n, n, zero, one, q, ldq )
           if( icompz==3_ilp64 )call stdlib_I64_slaset( 'FULL', n, n, zero, one, z, ldz )
           ! quick return if possible
           if( n<=1 )return
           ! zero out lower triangle of b
           do jcol = 1, n - 1
           ...
           do jcol = ilo, ihi - 2
              do jrow = ihi, jcol + 2, -1
                 ! step 1: rotate rows jrow-1, jrow to kill a(jrow,jcol)
                 temp = a( jrow-1, jcol )
                 call stdlib_I64_slartg( temp, a( jrow, jcol ), c, s,a( jrow-1, jcol ) )
                 a( jrow, jcol ) = zero
                 call stdlib_I64_srot( n-jcol, a( jrow-1, jcol+1 ), lda,a( jrow, jcol+1 ), lda, c, s )
                           
                 call stdlib_I64_srot( n+2-jrow, b( jrow-1, jrow-1 ), ldb,b( jrow, jrow-1 ), ldb, c, &
                           s )
                 if( ilq )call stdlib_I64_srot( n, q( 1_ilp64, jrow-1 ), 1_ilp64, q( 1_ilp64, jrow ), 1_ilp64, c, s )
                 ! step 2: rotate columns jrow, jrow-1 to kill b(jrow,jrow-1)
                 temp = b( jrow, jrow )
                 call stdlib_I64_slartg( temp, b( jrow, jrow-1 ), c, s,b( jrow, jrow ) )
                 b( jrow, jrow-1 ) = zero
                 call stdlib_I64_srot( ihi, a( 1_ilp64, jrow ), 1_ilp64, a( 1_ilp64, jrow-1 ), 1_ilp64, c, s )
                 call stdlib_I64_srot( jrow-1, b( 1_ilp64, jrow ), 1_ilp64, b( 1_ilp64, jrow-1 ), 1_ilp64, c,s )
                 if( ilz )call stdlib_I64_srot( n, z( 1_ilp64, jrow ), 1_ilp64, z( 1_ilp64, jrow-1 ), 1_ilp64, c, s )
              end do
           end do
           return
     ...
     !! problem to generalized Hessenberg form.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DGGHRD', -info )
              return
           end if
           ! initialize q and z if desired.
           if( icompq==3_ilp64 )call stdlib_I64_dlaset( 'FULL', n, n, zero, one, q, ldq )
           if( icompz==3_ilp64 )call stdlib_I64_dlaset( 'FULL', n, n, zero, one, z, ldz )
           ! quick return if possible
           if( n<=1 )return
           ! zero out lower triangle of b
           do jcol = 1, n - 1
           ...
           do jcol = ilo, ihi - 2
              do jrow = ihi, jcol + 2, -1
                 ! step 1: rotate rows jrow-1, jrow to kill a(jrow,jcol)
                 temp = a( jrow-1, jcol )
                 call stdlib_I64_dlartg( temp, a( jrow, jcol ), c, s,a( jrow-1, jcol ) )
                 a( jrow, jcol ) = zero
                 call stdlib_I64_drot( n-jcol, a( jrow-1, jcol+1 ), lda,a( jrow, jcol+1 ), lda, c, s )
                           
                 call stdlib_I64_drot( n+2-jrow, b( jrow-1, jrow-1 ), ldb,b( jrow, jrow-1 ), ldb, c, &
                           s )
                 if( ilq )call stdlib_I64_drot( n, q( 1_ilp64, jrow-1 ), 1_ilp64, q( 1_ilp64, jrow ), 1_ilp64, c, s )
                 ! step 2: rotate columns jrow, jrow-1 to kill b(jrow,jrow-1)
                 temp = b( jrow, jrow )
                 call stdlib_I64_dlartg( temp, b( jrow, jrow-1 ), c, s,b( jrow, jrow ) )
                 b( jrow, jrow-1 ) = zero
                 call stdlib_I64_drot( ihi, a( 1_ilp64, jrow ), 1_ilp64, a( 1_ilp64, jrow-1 ), 1_ilp64, c, s )
                 call stdlib_I64_drot( jrow-1, b( 1_ilp64, jrow ), 1_ilp64, b( 1_ilp64, jrow-1 ), 1_ilp64, c,s )
                 if( ilz )call stdlib_I64_drot( n, z( 1_ilp64, jrow ), 1_ilp64, z( 1_ilp64, jrow-1 ), 1_ilp64, c, s )
              end do
           end do
           return
     ...
     !! problem to generalized Hessenberg form.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CGGHRD', -info )
              return
           end if
           ! initialize q and z if desired.
           if( icompq==3_ilp64 )call stdlib_I64_claset( 'FULL', n, n, czero, cone, q, ldq )
           if( icompz==3_ilp64 )call stdlib_I64_claset( 'FULL', n, n, czero, cone, z, ldz )
           ! quick return if possible
           if( n<=1 )return
           ! zero out lower triangle of b
           do jcol = 1, n - 1
           ...
           do jcol = ilo, ihi - 2
              do jrow = ihi, jcol + 2, -1
                 ! step 1: rotate rows jrow-1, jrow to kill a(jrow,jcol)
                 ctemp = a( jrow-1, jcol )
                 call stdlib_I64_clartg( ctemp, a( jrow, jcol ), c, s,a( jrow-1, jcol ) )
                 a( jrow, jcol ) = czero
                 call stdlib_I64_crot( n-jcol, a( jrow-1, jcol+1 ), lda,a( jrow, jcol+1 ), lda, c, s )
                           
                 call stdlib_I64_crot( n+2-jrow, b( jrow-1, jrow-1 ), ldb,b( jrow, jrow-1 ), ldb, c, &
                           s )
                 if( ilq )call stdlib_I64_crot( n, q( 1_ilp64, jrow-1 ), 1_ilp64, q( 1_ilp64, jrow ), 1_ilp64, c,conjg( s ) )
                           
                 ! step 2: rotate columns jrow, jrow-1 to kill b(jrow,jrow-1)
                 ctemp = b( jrow, jrow )
                 call stdlib_I64_clartg( ctemp, b( jrow, jrow-1 ), c, s,b( jrow, jrow ) )
                 b( jrow, jrow-1 ) = czero
                 call stdlib_I64_crot( ihi, a( 1_ilp64, jrow ), 1_ilp64, a( 1_ilp64, jrow-1 ), 1_ilp64, c, s )
                 call stdlib_I64_crot( jrow-1, b( 1_ilp64, jrow ), 1_ilp64, b( 1_ilp64, jrow-1 ), 1_ilp64, c,s )
                 if( ilz )call stdlib_I64_crot( n, z( 1_ilp64, jrow ), 1_ilp64, z( 1_ilp64, jrow-1 ), 1_ilp64, c, s )
              end do
           end do
           return
     ...
     !! problem to generalized Hessenberg form.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZGGHRD', -info )
              return
           end if
           ! initialize q and z if desired.
           if( icompq==3_ilp64 )call stdlib_I64_zlaset( 'FULL', n, n, czero, cone, q, ldq )
           if( icompz==3_ilp64 )call stdlib_I64_zlaset( 'FULL', n, n, czero, cone, z, ldz )
           ! quick return if possible
           if( n<=1 )return
           ! zero out lower triangle of b
           do jcol = 1, n - 1
           ...
           do jcol = ilo, ihi - 2
              do jrow = ihi, jcol + 2, -1
                 ! step 1: rotate rows jrow-1, jrow to kill a(jrow,jcol)
                 ctemp = a( jrow-1, jcol )
                 call stdlib_I64_zlartg( ctemp, a( jrow, jcol ), c, s,a( jrow-1, jcol ) )
                 a( jrow, jcol ) = czero
                 call stdlib_I64_zrot( n-jcol, a( jrow-1, jcol+1 ), lda,a( jrow, jcol+1 ), lda, c, s )
                           
                 call stdlib_I64_zrot( n+2-jrow, b( jrow-1, jrow-1 ), ldb,b( jrow, jrow-1 ), ldb, c, &
                           s )
                 if( ilq )call stdlib_I64_zrot( n, q( 1_ilp64, jrow-1 ), 1_ilp64, q( 1_ilp64, jrow ), 1_ilp64, c,conjg( s ) )
                           
                 ! step 2: rotate columns jrow, jrow-1 to kill b(jrow,jrow-1)
                 ctemp = b( jrow, jrow )
                 call stdlib_I64_zlartg( ctemp, b( jrow, jrow-1 ), c, s,b( jrow, jrow ) )
                 b( jrow, jrow-1 ) = czero
                 call stdlib_I64_zrot( ihi, a( 1_ilp64, jrow ), 1_ilp64, a( 1_ilp64, jrow-1 ), 1_ilp64, c, s )
                 call stdlib_I64_zrot( jrow-1, b( 1_ilp64, jrow ), 1_ilp64, b( 1_ilp64, jrow-1 ), 1_ilp64, c,s )
                 if( ilz )call stdlib_I64_zrot( n, z( 1_ilp64, jrow ), 1_ilp64, z( 1_ilp64, jrow-1 ), 1_ilp64, c, s )
              end do
           end do
           return
     ...
     !! multiplications for parts of the computation to enhance performance.
        ...
        ! =====================================================================
           ...
           logical(lk) :: blk22, initq, initz, lquery, wantq, wantz
           character :: compq2, compz2
           integer(ilp64) :: cola, i, ierr, j, j0, jcol, jj, jrow, k, kacc22, len, lwkopt, n2nb, nb,&
                      nblst, nbmin, nh, nnb, nx, ppw, ppwo, pw, top, topq
           real(sp) :: c, c1, c2, s, s1, s2, temp, temp1, temp2, temp3
           ! Intrinsic Functions 
           ! Executable Statements 
           ! decode and test the input parameters.
           info = 0_ilp64
           nb = stdlib_I64_ilaenv( 1_ilp64, 'SGGHD3', ' ', n, ilo, ihi, -1_ilp64 )
           lwkopt = max( 6_ilp64*n*nb, 1_ilp64 )
           ...
           if( .not.stdlib_lsame( compq, 'N' ) .and. .not.wantq ) then
              ...
              info = -9_ilp64
           else if( ( wantq .and. ldq<n ) .or. ldq<1_ilp64 ) then
              info = -11_ilp64
           else if( ( wantz .and. ldz<n ) .or. ldz<1_ilp64 ) then
              info = -13_ilp64
           else if( lwork<1_ilp64 .and. .not.lquery ) then
              info = -15_ilp64
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SGGHD3', -info )
              return
           else if( lquery ) then
              return
           end if
           ! initialize q and z if desired.
           if( initq )call stdlib_I64_slaset( 'ALL', n, n, zero, one, q, ldq )
           if( initz )call stdlib_I64_slaset( 'ALL', n, n, zero, one, z, ldz )
           ! zero out lower triangle of b.
           if( n>1_ilp64 )call stdlib_I64_slaset( 'LOWER', n-1, n-1, zero, zero, b(2_ilp64, 1_ilp64), ldb )
           ! quick return if possible
           nh = ihi - ilo + 1_ilp64
           if( nh<=1_ilp64 ) then
              work( 1_ilp64 ) = one
           ...
           ! determine the blocksize.
           nbmin = stdlib_I64_ilaenv( 2_ilp64, 'SGGHD3', ' ', n, ilo, ihi, -1_ilp64 )
           if( nb>1_ilp64 .and. nb<nh ) then
              ! determine when to use unblocked instead of blocked code.
              nx = max( nb, stdlib_I64_ilaenv( 3_ilp64, 'SGGHD3', ' ', n, ilo, ihi, -1_ilp64 ) )
              if( nx<nh ) then
                 ! determine if workspace is large enough for blocked code.
                 if( lwork<lwkopt ) then
                    ! not enough workspace to use optimal nb:  determine the
                    ! minimum value of nb, and reduce nb or force use of
                    ! unblocked code.
                    nbmin = max( 2_ilp64, stdlib_I64_ilaenv( 2_ilp64, 'SGGHD3', ' ', n, ilo, ihi,-1_ilp64 ) )
                    if( lwork>=6_ilp64*n*nbmin ) then
                       nb = lwork / ( 6_ilp64*n )
                    else
                       nb = 1_ilp64
                    end if
           ...
           if( nb<nbmin .or. nb>=nh ) then
              ...
              ! use blocked code
              kacc22 = stdlib_I64_ilaenv( 16_ilp64, 'SGGHD3', ' ', n, ilo, ihi, -1_ilp64 )
              blk22 = kacc22==2_ilp64
              do jcol = ilo, ihi-2, nb
                 nnb = min( nb, ihi-jcol-1 )
                 ! initialize small orthogonal factors that will hold the
                 ! accumulated givens rotations in workspace.
                 ...
                 ! nblst  denotes the (possibly smaller) order of the last
                        ! factor.
                 n2nb = ( ihi-jcol-1 ) / nnb - 1_ilp64
                 nblst = ihi - jcol - n2nb*nnb
                 call stdlib_I64_slaset( 'ALL', nblst, nblst, zero, one, work, nblst )
                 pw = nblst * nblst + 1_ilp64
                 do i = 1, n2nb
                    call stdlib_I64_slaset( 'ALL', 2_ilp64*nnb, 2_ilp64*nnb, zero, one,work( pw ), 2_ilp64*nnb )
                    pw = pw + 4_ilp64*nnb*nnb
                 end do
                 ! reduce columns jcol:jcol+nnb-1 of a to hessenberg form.
                 do j = jcol, jcol+nnb-1
                    ! reduce jth column of a. store cosines and sines in jth
                    ...
                    do i = ihi, j+2, -1
                       temp = a( i-1, j )
                       call stdlib_I64_slartg( temp, a( i, j ), c, s, a( i-1, j ) )
                       a( i, j ) = c
                       b( i, j ) = s
                    end do
                    ! accumulate givens rotations into workspace array.
                    ppw  = ( nblst + 1_ilp64 )*( nblst - 2_ilp64 ) - j + jcol + 1_ilp64
                    len  = 2_ilp64 + j - jcol
                    jrow = j + n2nb*nnb + 2_ilp64
                    do i = ihi, jrow, -1
                       c = a( i, j )
                       s = b( i, j )
                       do jj = ppw, ppw+len-1
                       ...
                       len = len + 1_ilp64
                       ppw = ppw - nblst - 1_ilp64
                    end do
                    ppwo = nblst*nblst + ( nnb+j-jcol-1 )*2_ilp64*nnb + nnb
                    j0 = jrow - nnb
                    do jrow = j0, j+2, -nnb
                       ppw = ppwo
                       len  = 2_ilp64 + j - jcol
                       do i = jrow+nnb-1, jrow, -1
                          ...
                          s = b( i, j )
                          do jj = ppw, ppw+len-1
                             temp = work( jj + 2_ilp64*nnb )
                             work( jj + 2_ilp64*nnb ) = c*temp - s*work( jj )
                             work( jj ) = s*temp + c*work( jj )
                          end do
                          len = len + 1_ilp64
                          ppw = ppw - 2_ilp64*nnb - 1_ilp64
                       end do
                       ppwo = ppwo + 4_ilp64*nnb*nnb
                    end do
                    ! top denotes the number of top rows in a and b that will
                    ! not be updated during the next steps.
                    if( jcol<=2_ilp64 ) then
                       top = 0_ilp64
                    else
                       top = jcol
                    end if
                    ...
                    do jj = n, j+1, -1
                       ...
                       if( jj<ihi ) then
                          temp = b( jj+1, jj+1 )
                          call stdlib_I64_slartg( temp, b( jj+1, jj ), c, s,b( jj+1, jj+1 ) )
                          b( jj+1, jj ) = zero
                          call stdlib_I64_srot( jj-top, b( top+1, jj+1 ), 1_ilp64,b( top+1, jj ), 1_ilp64, c, s )
                    ...
                    ! update a by transformations from right.
                    ! explicit loop unrolling provides better performance
                    ! compared to stdlib_I64_slasr.
                     ! call stdlib_I64_slasr( 'right', 'variable', 'backward', ihi-top,
           ! $                     ihi-j, a( j+2, j ), b( j+2, j ),
           ! $                     a( top+1, j+1 ), lda )
                    jj = mod( ihi-j-1, 3_ilp64 )
                    do i = ihi-j-3, jj+1, -3
                       c = a( j+1+i, j )
                       s = -b( j+1+i, j )
                       c1 = a( j+2+i, j )
                    ...
                    if( jj>0_ilp64 ) then
                       do i = jj, 1, -1
                          call stdlib_I64_srot( ihi-top, a( top+1, j+i+1 ), 1_ilp64,a( top+1, j+i ), 1_ilp64, a( &
                                    j+1+i, j ),-b( j+1+i, j ) )
                       end do
                    end if
                    ! update (j+1)th column of a by transformations from left.
                    if ( j < jcol + nnb - 1_ilp64 ) then
                       len  = 1_ilp64 + j - jcol
                       ! multiply with the trailing accumulated orthogonal
                       ! matrix, which takes the form
                              ! [  u11  u12  ]
                       ...
                       ! where u21 is a len-by-len matrix and u12 is lower
                       ! triangular.
                       jrow = ihi - nblst + 1_ilp64
                       call stdlib_I64_sgemv( 'TRANSPOSE', nblst, len, one, work,nblst, a( jrow, j+1 )&
                                 , 1_ilp64, zero,work( pw ), 1_ilp64 )
                       ppw = pw + len
                       do i = jrow, jrow+nblst-len-1
                          work( ppw ) = a( i, j+1 )
                          ppw = ppw + 1_ilp64
                       end do
                       call stdlib_I64_strmv( 'LOWER', 'TRANSPOSE', 'NON-UNIT',nblst-len, work( &
                                 len*nblst + 1_ilp64 ), nblst,work( pw+len ), 1_ilp64 )
                       call stdlib_I64_sgemv( 'TRANSPOSE', len, nblst-len, one,work( (len+1)*nblst - &
                       len + 1_ilp64 ), nblst,a( jrow+nblst-len, j+1 ), 1_ilp64, one,work( pw+len ), 1_ilp64 )
                       ...
                       do jrow = j0, jcol+1, -nnb
                          ...
                          call stdlib_I64_strmv( 'UPPER', 'TRANSPOSE', 'NON-UNIT', len,work( ppwo + &
                                    nnb ), 2_ilp64*nnb, work( pw ),1_ilp64 )
                          call stdlib_I64_strmv( 'LOWER', 'TRANSPOSE', 'NON-UNIT', nnb,work( ppwo + &
                                    2_ilp64*len*nnb ),2_ilp64*nnb, work( pw + len ), 1_ilp64 )
                          call stdlib_I64_sgemv( 'TRANSPOSE', nnb, len, one,work( ppwo ), 2_ilp64*nnb, a( &
                                    jrow, j+1 ), 1_ilp64,one, work( pw ), 1_ilp64 )
                          call stdlib_I64_sgemv( 'TRANSPOSE', len, nnb, one,work( ppwo + 2_ilp64*len*nnb + &
                                    nnb ), 2_ilp64*nnb,a( jrow+nnb, j+1 ), 1_ilp64, one,work( pw+len ), 1_ilp64 )
                          ppw = pw
                          do i = jrow, jrow+len+nnb-1
                             a( i, j+1 ) = work( ppw )
                             ppw = ppw + 1_ilp64
                 ...
                 ! apply accumulated orthogonal matrices to a.
                 cola = n - jcol - nnb + 1_ilp64
                 j = ihi - nblst + 1_ilp64
                 call stdlib_I64_sgemm( 'TRANSPOSE', 'NO TRANSPOSE', nblst,cola, nblst, one, work, &
                           nblst,a( j, jcol+nnb ), lda, zero, work( pw ),nblst )
                 call stdlib_I64_slacpy( 'ALL', nblst, cola, work( pw ), nblst,a( j, jcol+nnb ), lda )
                           
                 ppwo = nblst*nblst + 1_ilp64
                 j0 = j - nnb
                 do j = j0, jcol+1, -nnb
                    if ( blk22 ) then
                       ! exploit the structure of
                       ...
                       ! where all blocks are nnb-by-nnb, u21 is upper
                       ! triangular and u12 is lower triangular.
                       call stdlib_I64_sorm22( 'LEFT', 'TRANSPOSE', 2_ilp64*nnb, cola, nnb,nnb, work( ppwo )&
                                 , 2_ilp64*nnb,a( j, jcol+nnb ), lda, work( pw ),lwork-pw+1, ierr )
                    else
                       ! ignore the structure of u.
                       call stdlib_I64_sgemm( 'TRANSPOSE', 'NO TRANSPOSE', 2_ilp64*nnb,cola, 2_ilp64*nnb, one, &
                                 work( ppwo ), 2_ilp64*nnb,a( j, jcol+nnb ), lda, zero, work( pw ),2_ilp64*nnb )
                       call stdlib_I64_slacpy( 'ALL', 2_ilp64*nnb, cola, work( pw ), 2_ilp64*nnb,a( j, jcol+nnb ),&
                                  lda )
                    end if
                    ppwo = ppwo + 4_ilp64*nnb*nnb
                 end do
                 ...
                 if( wantq ) then
                    j = ihi - nblst + 1_ilp64
                    if ( initq ) then
                       topq = max( 2_ilp64, j - jcol + 1_ilp64 )
                       nh  = ihi - topq + 1_ilp64
                    else
                       topq = 1_ilp64
                       nh = n
                    end if
                    call stdlib_I64_sgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', nh,nblst, nblst, one, q( &
                              topq, j ), ldq,work, nblst, zero, work( pw ), nh )
                    call stdlib_I64_slacpy( 'ALL', nh, nblst, work( pw ), nh,q( topq, j ), ldq )
                    ...
                    do j = j0, jcol+1, -nnb
                       ...
                       if ( blk22 ) then
                          ! exploit the structure of u.
                          call stdlib_I64_sorm22( 'RIGHT', 'NO TRANSPOSE', nh, 2_ilp64*nnb,nnb, nnb, work( &
                                    ppwo ), 2_ilp64*nnb,q( topq, j ), ldq, work( pw ),lwork-pw+1, ierr )
                       else
                          ! ignore the structure of u.
                          call stdlib_I64_sgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', nh,2_ilp64*nnb, 2_ilp64*nnb, one,&
                                     q( topq, j ), ldq,work( ppwo ), 2_ilp64*nnb, zero, work( pw ),nh )
                          call stdlib_I64_slacpy( 'ALL', nh, 2_ilp64*nnb, work( pw ), nh,q( topq, j ), ldq )
                 ...
                 if ( wantz .or. top>0_ilp64 ) then
                    ...
                    call stdlib_I64_slaset( 'LOWER', ihi - jcol - 1_ilp64, nnb, zero, zero,a( jcol + 2_ilp64, &
                              jcol ), lda )
                    call stdlib_I64_slaset( 'LOWER', ihi - jcol - 1_ilp64, nnb, zero, zero,b( jcol + 2_ilp64, &
                              jcol ), ldb )
                 end if
                 ! apply accumulated orthogonal matrices to a and b.
                 if ( top>0_ilp64 ) then
                    j = ihi - nblst + 1_ilp64
                    call stdlib_I64_sgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', top,nblst, nblst, one, a( &
                              1_ilp64, j ), lda,work, nblst, zero, work( pw ), top )
                    call stdlib_I64_slacpy( 'ALL', top, nblst, work( pw ), top,a( 1_ilp64, j ), lda )
                    ...
                    do j = j0, jcol+1, -nnb
                       if ( blk22 ) then
                          ! exploit the structure of u.
                          call stdlib_I64_sorm22( 'RIGHT', 'NO TRANSPOSE', top, 2_ilp64*nnb,nnb, nnb, work( &
                                    ppwo ), 2_ilp64*nnb,a( 1_ilp64, j ), lda, work( pw ),lwork-pw+1, ierr )
                       else
                          ! ignore the structure of u.
                          call stdlib_I64_sgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', top,2_ilp64*nnb, 2_ilp64*nnb, &
                                    one, a( 1_ilp64, j ), lda,work( ppwo ), 2_ilp64*nnb, zero,work( pw ), top )
                          call stdlib_I64_slacpy( 'ALL', top, 2_ilp64*nnb, work( pw ), top,a( 1_ilp64, j ), lda )
                                    
                       end if
                       ppwo = ppwo + 4_ilp64*nnb*nnb
                    end do
                    j = ihi - nblst + 1_ilp64
                    call stdlib_I64_sgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', top,nblst, nblst, one, b( &
                              1_ilp64, j ), ldb,work, nblst, zero, work( pw ), top )
                    call stdlib_I64_slacpy( 'ALL', top, nblst, work( pw ), top,b( 1_ilp64, j ), ldb )
                    ...
                    do j = j0, jcol+1, -nnb
                       if ( blk22 ) then
                          ! exploit the structure of u.
                          call stdlib_I64_sorm22( 'RIGHT', 'NO TRANSPOSE', top, 2_ilp64*nnb,nnb, nnb, work( &
                                    ppwo ), 2_ilp64*nnb,b( 1_ilp64, j ), ldb, work( pw ),lwork-pw+1, ierr )
                       else
                          ! ignore the structure of u.
                          call stdlib_I64_sgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', top,2_ilp64*nnb, 2_ilp64*nnb, &
                                    one, b( 1_ilp64, j ), ldb,work( ppwo ), 2_ilp64*nnb, zero,work( pw ), top )
                          call stdlib_I64_slacpy( 'ALL', top, 2_ilp64*nnb, work( pw ), top,b( 1_ilp64, j ), ldb )
                 ...
                 if( wantz ) then
                    ...
                    call stdlib_I64_sgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', nh,nblst, nblst, one, z( &
                              topq, j ), ldz,work, nblst, zero, work( pw ), nh )
                    call stdlib_I64_slacpy( 'ALL', nh, nblst, work( pw ), nh,z( topq, j ), ldz )
                    ...
                    do j = j0, jcol+1, -nnb
                       ...
                       if ( blk22 ) then
                          ! exploit the structure of u.
                          call stdlib_I64_sorm22( 'RIGHT', 'NO TRANSPOSE', nh, 2_ilp64*nnb,nnb, nnb, work( &
                                    ppwo ), 2_ilp64*nnb,z( topq, j ), ldz, work( pw ),lwork-pw+1, ierr )
                       else
                          ! ignore the structure of u.
                          call stdlib_I64_sgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', nh,2_ilp64*nnb, 2_ilp64*nnb, one,&
                                     z( topq, j ), ldz,work( ppwo ), 2_ilp64*nnb, zero, work( pw ),nh )
                          call stdlib_I64_slacpy( 'ALL', nh, 2_ilp64*nnb, work( pw ), nh,z( topq, j ), ldz )
           ...
           if ( jcol<ihi )call stdlib_I64_sgghrd( compq2, compz2, n, jcol, ihi, a, lda, b, ldb, q,ldq,&
                      z, ldz, ierr )
           work( 1_ilp64 ) = real( lwkopt,KIND=sp)
           return
     ...
     !! multiplications for parts of the computation to enhance performance.
       ...
       ! =====================================================================
           ...
           ! decode and test the input parameters.
           info = 0_ilp64
           nb = stdlib_I64_ilaenv( 1_ilp64, 'DGGHD3', ' ', n, ilo, ihi, -1_ilp64 )
           lwkopt = max( 6_ilp64*n*nb, 1_ilp64 )
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DGGHD3', -info )
              return
           else if( lquery ) then
              return
           end if
           ! initialize q and z if desired.
           if( initq )call stdlib_I64_dlaset( 'ALL', n, n, zero, one, q, ldq )
           if( initz )call stdlib_I64_dlaset( 'ALL', n, n, zero, one, z, ldz )
           ! zero out lower triangle of b.
           if( n>1_ilp64 )call stdlib_I64_dlaset( 'LOWER', n-1, n-1, zero, zero, b(2_ilp64, 1_ilp64), ldb )
           ! quick return if possible
           nh = ihi - ilo + 1_ilp64
           if( nh<=1_ilp64 ) then
              work( 1_ilp64 ) = one
           ...
           ! determine the blocksize.
           nbmin = stdlib_I64_ilaenv( 2_ilp64, 'DGGHD3', ' ', n, ilo, ihi, -1_ilp64 )
           if( nb>1_ilp64 .and. nb<nh ) then
              ! determine when to use unblocked instead of blocked code.
              nx = max( nb, stdlib_I64_ilaenv( 3_ilp64, 'DGGHD3', ' ', n, ilo, ihi, -1_ilp64 ) )
              if( nx<nh ) then
                 ! determine if workspace is large enough for blocked code.
                 if( lwork<lwkopt ) then
                    ! not enough workspace to use optimal nb:  determine the
                    ! minimum value of nb, and reduce nb or force use of
                    ! unblocked code.
                    nbmin = max( 2_ilp64, stdlib_I64_ilaenv( 2_ilp64, 'DGGHD3', ' ', n, ilo, ihi,-1_ilp64 ) )
                    if( lwork>=6_ilp64*n*nbmin ) then
                       nb = lwork / ( 6_ilp64*n )
                    else
                       nb = 1_ilp64
           ...
           if( nb<nbmin .or. nb>=nh ) then
              ...
              ! use blocked code
              kacc22 = stdlib_I64_ilaenv( 16_ilp64, 'DGGHD3', ' ', n, ilo, ihi, -1_ilp64 )
              blk22 = kacc22==2_ilp64
              do jcol = ilo, ihi-2, nb
                 nnb = min( nb, ihi-jcol-1 )
                 ! initialize small orthogonal factors that will hold the
                 ...
                 n2nb = ( ihi-jcol-1 ) / nnb - 1_ilp64
                 nblst = ihi - jcol - n2nb*nnb
                 call stdlib_I64_dlaset( 'ALL', nblst, nblst, zero, one, work, nblst )
                 pw = nblst * nblst + 1_ilp64
                 do i = 1, n2nb
                    call stdlib_I64_dlaset( 'ALL', 2_ilp64*nnb, 2_ilp64*nnb, zero, one,work( pw ), 2_ilp64*nnb )
                    pw = pw + 4_ilp64*nnb*nnb
                 end do
                 ! reduce columns jcol:jcol+nnb-1 of a to hessenberg form.
                 do j = jcol, jcol+nnb-1
                    ...
                    do i = ihi, j+2, -1
                       temp = a( i-1, j )
                       call stdlib_I64_dlartg( temp, a( i, j ), c, s, a( i-1, j ) )
                       a( i, j ) = c
                       b( i, j ) = s
                    end do
                    ! accumulate givens rotations into workspace array.
                    ...
                    do jj = n, j+1, -1
                       ...
                       if( jj<ihi ) then
                          temp = b( jj+1, jj+1 )
                          call stdlib_I64_dlartg( temp, b( jj+1, jj ), c, s,b( jj+1, jj+1 ) )
                          b( jj+1, jj ) = zero
                          call stdlib_I64_drot( jj-top, b( top+1, jj+1 ), 1_ilp64,b( top+1, jj ), 1_ilp64, c, s )
                    ...
                    ! update a by transformations from right.
                    ! explicit loop unrolling provides better performance
                    ! compared to stdlib_I64_dlasr.
                     ! call stdlib_I64_dlasr( 'right', 'variable', 'backward', ihi-top,
           ! $                     ihi-j, a( j+2, j ), b( j+2, j ),
           ! $                     a( top+1, j+1 ), lda )
                    jj = mod( ihi-j-1, 3_ilp64 )
                    do i = ihi-j-3, jj+1, -3
                    ...
                    if( jj>0_ilp64 ) then
                       do i = jj, 1, -1
                          call stdlib_I64_drot( ihi-top, a( top+1, j+i+1 ), 1_ilp64,a( top+1, j+i ), 1_ilp64, a( &
                                    j+1+i, j ),-b( j+1+i, j ) )
                       end do
                    end if
                    ! update (j+1)th column of a by transformations from left.
                    if ( j < jcol + nnb - 1_ilp64 ) then
                       ...
                       ! triangular.
                       jrow = ihi - nblst + 1_ilp64
                       call stdlib_I64_dgemv( 'TRANSPOSE', nblst, len, one, work,nblst, a( jrow, j+1 )&
                                 , 1_ilp64, zero,work( pw ), 1_ilp64 )
                       ppw = pw + len
                       do i = jrow, jrow+nblst-len-1
                          work( ppw ) = a( i, j+1 )
                       ...
                       call stdlib_I64_dtrmv( 'LOWER', 'TRANSPOSE', 'NON-UNIT',nblst-len, work( &
                                 len*nblst + 1_ilp64 ), nblst,work( pw+len ), 1_ilp64 )
                       call stdlib_I64_dgemv( 'TRANSPOSE', len, nblst-len, one,work( (len+1)*nblst - &
                       len + 1_ilp64 ), nblst,a( jrow+nblst-len, j+1 ), 1_ilp64, one,work( pw+len ), 1_ilp64 )
                       ...
                       do jrow = j0, jcol+1, -nnb
                          ...
                          call stdlib_I64_dtrmv( 'UPPER', 'TRANSPOSE', 'NON-UNIT', len,work( ppwo + &
                                    nnb ), 2_ilp64*nnb, work( pw ),1_ilp64 )
                          call stdlib_I64_dtrmv( 'LOWER', 'TRANSPOSE', 'NON-UNIT', nnb,work( ppwo + &
                                    2_ilp64*len*nnb ),2_ilp64*nnb, work( pw + len ), 1_ilp64 )
                          call stdlib_I64_dgemv( 'TRANSPOSE', nnb, len, one,work( ppwo ), 2_ilp64*nnb, a( &
                                    jrow, j+1 ), 1_ilp64,one, work( pw ), 1_ilp64 )
                          call stdlib_I64_dgemv( 'TRANSPOSE', len, nnb, one,work( ppwo + 2_ilp64*len*nnb + &
                                    nnb ), 2_ilp64*nnb,a( jrow+nnb, j+1 ), 1_ilp64, one,work( pw+len ), 1_ilp64 )
                          ppw = pw
                          do i = jrow, jrow+len+nnb-1
                             a( i, j+1 ) = work( ppw )
                 ...
                 cola = n - jcol - nnb + 1_ilp64
                 j = ihi - nblst + 1_ilp64
                 call stdlib_I64_dgemm( 'TRANSPOSE', 'NO TRANSPOSE', nblst,cola, nblst, one, work, &
                           nblst,a( j, jcol+nnb ), lda, zero, work( pw ),nblst )
                 call stdlib_I64_dlacpy( 'ALL', nblst, cola, work( pw ), nblst,a( j, jcol+nnb ), lda )
                 ...
                 do j = j0, jcol+1, -nnb
                    if ( blk22 ) then
                       ...
                       ! where all blocks are nnb-by-nnb, u21 is upper
                       ! triangular and u12 is lower triangular.
                       call stdlib_I64_dorm22( 'LEFT', 'TRANSPOSE', 2_ilp64*nnb, cola, nnb,nnb, work( ppwo )&
                                 , 2_ilp64*nnb,a( j, jcol+nnb ), lda, work( pw ),lwork-pw+1, ierr )
                    else
                       ! ignore the structure of u.
                       call stdlib_I64_dgemm( 'TRANSPOSE', 'NO TRANSPOSE', 2_ilp64*nnb,cola, 2_ilp64*nnb, one, &
                                 work( ppwo ), 2_ilp64*nnb,a( j, jcol+nnb ), lda, zero, work( pw ),2_ilp64*nnb )
                       call stdlib_I64_dlacpy( 'ALL', 2_ilp64*nnb, cola, work( pw ), 2_ilp64*nnb,a( j, jcol+nnb ),&
                                  lda )
                    end if
                    ppwo = ppwo + 4_ilp64*nnb*nnb
                 end do
                 ...
                 if( wantq ) then
                    ...
                    call stdlib_I64_dgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', nh,nblst, nblst, one, q( &
                              topq, j ), ldq,work, nblst, zero, work( pw ), nh )
                    call stdlib_I64_dlacpy( 'ALL', nh, nblst, work( pw ), nh,q( topq, j ), ldq )
                    ...
                    do j = j0, jcol+1, -nnb
                       ...
                       if ( blk22 ) then
                          ! exploit the structure of u.
                          call stdlib_I64_dorm22( 'RIGHT', 'NO TRANSPOSE', nh, 2_ilp64*nnb,nnb, nnb, work( &
                                    ppwo ), 2_ilp64*nnb,q( topq, j ), ldq, work( pw ),lwork-pw+1, ierr )
                       else
                          ! ignore the structure of u.
                          call stdlib_I64_dgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', nh,2_ilp64*nnb, 2_ilp64*nnb, one,&
                                     q( topq, j ), ldq,work( ppwo ), 2_ilp64*nnb, zero, work( pw ),nh )
                          call stdlib_I64_dlacpy( 'ALL', nh, 2_ilp64*nnb, work( pw ), nh,q( topq, j ), ldq )
                 ...
                 if ( wantz .or. top>0_ilp64 ) then
                    ...
                    call stdlib_I64_dlaset( 'LOWER', ihi - jcol - 1_ilp64, nnb, zero, zero,a( jcol + 2_ilp64, &
                              jcol ), lda )
                    call stdlib_I64_dlaset( 'LOWER', ihi - jcol - 1_ilp64, nnb, zero, zero,b( jcol + 2_ilp64, &
                              jcol ), ldb )
                 end if
                 ! apply accumulated orthogonal matrices to a and b.
                 if ( top>0_ilp64 ) then
                    j = ihi - nblst + 1_ilp64
                    call stdlib_I64_dgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', top,nblst, nblst, one, a( &
                              1_ilp64, j ), lda,work, nblst, zero, work( pw ), top )
                    call stdlib_I64_dlacpy( 'ALL', top, nblst, work( pw ), top,a( 1_ilp64, j ), lda )
                    ...
                    do j = j0, jcol+1, -nnb
                       if ( blk22 ) then
                          ! exploit the structure of u.
                          call stdlib_I64_dorm22( 'RIGHT', 'NO TRANSPOSE', top, 2_ilp64*nnb,nnb, nnb, work( &
                                    ppwo ), 2_ilp64*nnb,a( 1_ilp64, j ), lda, work( pw ),lwork-pw+1, ierr )
                       else
                          ! ignore the structure of u.
                          call stdlib_I64_dgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', top,2_ilp64*nnb, 2_ilp64*nnb, &
                                    one, a( 1_ilp64, j ), lda,work( ppwo ), 2_ilp64*nnb, zero,work( pw ), top )
                          call stdlib_I64_dlacpy( 'ALL', top, 2_ilp64*nnb, work( pw ), top,a( 1_ilp64, j ), lda )
                                    
                       end if
                       ppwo = ppwo + 4_ilp64*nnb*nnb
                    end do
                    j = ihi - nblst + 1_ilp64
                    call stdlib_I64_dgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', top,nblst, nblst, one, b( &
                              1_ilp64, j ), ldb,work, nblst, zero, work( pw ), top )
                    call stdlib_I64_dlacpy( 'ALL', top, nblst, work( pw ), top,b( 1_ilp64, j ), ldb )
                    ...
                    do j = j0, jcol+1, -nnb
                       if ( blk22 ) then
                          ...
                          call stdlib_I64_dgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', top,2_ilp64*nnb, 2_ilp64*nnb, &
                                    one, b( 1_ilp64, j ), ldb,work( ppwo ), 2_ilp64*nnb, zero,work( pw ), top )
                          call stdlib_I64_dlacpy( 'ALL', top, 2_ilp64*nnb, work( pw ), top,b( 1_ilp64, j ), ldb )
                 ...
                 if( wantz ) then
                    ...
                    call stdlib_I64_dgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', nh,nblst, nblst, one, z( &
                              topq, j ), ldz,work, nblst, zero, work( pw ), nh )
                    call stdlib_I64_dlacpy( 'ALL', nh, nblst, work( pw ), nh,z( topq, j ), ldz )
                    ...
                    do j = j0, jcol+1, -nnb
                       ...
                       if ( blk22 ) then
                          ...
                          call stdlib_I64_dgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', nh,2_ilp64*nnb, 2_ilp64*nnb, one,&
                                     z( topq, j ), ldz,work( ppwo ), 2_ilp64*nnb, zero, work( pw ),nh )
                          call stdlib_I64_dlacpy( 'ALL', nh, 2_ilp64*nnb, work( pw ), nh,z( topq, j ), ldz )
           ...
           if ( jcol<ihi )call stdlib_I64_dgghrd( compq2, compz2, n, jcol, ihi, a, lda, b, ldb, q,ldq,&
                      z, ldz, ierr )
           work( 1_ilp64 ) = real( lwkopt,KIND=dp)
           return
     ...
     !! multiplications for parts of the computation to enhance performance.
        ...
        ! =====================================================================
           ...
           ! decode and test the input parameters.
           info = 0_ilp64
           nb = stdlib_I64_ilaenv( 1_ilp64, 'CGGHD3', ' ', n, ilo, ihi, -1_ilp64 )
           lwkopt = max( 6_ilp64*n*nb, 1_ilp64 )
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CGGHD3', -info )
              return
           else if( lquery ) then
              return
           end if
           ! initialize q and z if desired.
           if( initq )call stdlib_I64_claset( 'ALL', n, n, czero, cone, q, ldq )
           if( initz )call stdlib_I64_claset( 'ALL', n, n, czero, cone, z, ldz )
           ! zero out lower triangle of b.
           if( n>1_ilp64 )call stdlib_I64_claset( 'LOWER', n-1, n-1, czero, czero, b(2_ilp64, 1_ilp64), ldb )
           ! quick return if possible
           nh = ihi - ilo + 1_ilp64
           if( nh<=1_ilp64 ) then
              work( 1_ilp64 ) = cone
           ...
           ! determine the blocksize.
           nbmin = stdlib_I64_ilaenv( 2_ilp64, 'CGGHD3', ' ', n, ilo, ihi, -1_ilp64 )
           if( nb>1_ilp64 .and. nb<nh ) then
              ! determine when to use unblocked instead of blocked code.
              nx = max( nb, stdlib_I64_ilaenv( 3_ilp64, 'CGGHD3', ' ', n, ilo, ihi, -1_ilp64 ) )
              if( nx<nh ) then
                 ! determine if workspace is large enough for blocked code.
                 if( lwork<lwkopt ) then
                    ! not enough workspace to use optimal nb:  determine the
                    ! minimum value of nb, and reduce nb or force use of
                    ! unblocked code.
                    nbmin = max( 2_ilp64, stdlib_I64_ilaenv( 2_ilp64, 'CGGHD3', ' ', n, ilo, ihi,-1_ilp64 ) )
                    if( lwork>=6_ilp64*n*nbmin ) then
                       nb = lwork / ( 6_ilp64*n )
                    else
                       nb = 1_ilp64
           ...
           if( nb<nbmin .or. nb>=nh ) then
              ...
              ! use blocked code
              kacc22 = stdlib_I64_ilaenv( 16_ilp64, 'CGGHD3', ' ', n, ilo, ihi, -1_ilp64 )
              blk22 = kacc22==2_ilp64
              do jcol = ilo, ihi-2, nb
                 nnb = min( nb, ihi-jcol-1 )
                 ! initialize small unitary factors that will hold the
                 ...
                 n2nb = ( ihi-jcol-1 ) / nnb - 1_ilp64
                 nblst = ihi - jcol - n2nb*nnb
                 call stdlib_I64_claset( 'ALL', nblst, nblst, czero, cone, work, nblst )
                 pw = nblst * nblst + 1_ilp64
                 do i = 1, n2nb
                    call stdlib_I64_claset( 'ALL', 2_ilp64*nnb, 2_ilp64*nnb, czero, cone,work( pw ), 2_ilp64*nnb )
                 ...
                 do j = jcol, jcol+nnb-1
                    ...
                    do i = ihi, j+2, -1
                       temp = a( i-1, j )
                       call stdlib_I64_clartg( temp, a( i, j ), c, s, a( i-1, j ) )
                       a( i, j ) = cmplx( c,KIND=sp)
                       b( i, j ) = s
                    end do
                    ! accumulate givens rotations into workspace array.
                    ...
                    do jrow = j0, j+2, -nnb
                       ...
                       do i = jrow+nnb-1, jrow, -1
                          ...
                          do jj = ppw, ppw+len-1
                             temp = work( jj + 2_ilp64*nnb )
                             work( jj + 2_ilp64*nnb ) = ctemp*temp - s*work( jj )
                             work( jj ) = conjg( s )*temp + ctemp*work( jj )
                          end do
                          len = len + 1_ilp64
                          ppw = ppw - 2_ilp64*nnb - 1_ilp64
                    ...
                    do jj = n, j+1, -1
                       ...
                       if( jj<ihi ) then
                          temp = b( jj+1, jj+1 )
                          call stdlib_I64_clartg( temp, b( jj+1, jj ), c, s,b( jj+1, jj+1 ) )
                          b( jj+1, jj ) = czero
                          call stdlib_I64_crot( jj-top, b( top+1, jj+1 ), 1_ilp64,b( top+1, jj ), 1_ilp64, c, s )
                    ...
                    if( jj>0_ilp64 ) then
                       do i = jj, 1, -1
                          c = real( a( j+1+i, j ),KIND=sp)
                          call stdlib_I64_crot( ihi-top, a( top+1, j+i+1 ), 1_ilp64,a( top+1, j+i ), 1_ilp64, c,-&
                                    conjg( b( j+1+i, j ) ) )
                       end do
                    end if
                    ! update (j+1)th column of a by transformations from left.
                    if ( j < jcol + nnb - 1_ilp64 ) then
                       ...
                       ! triangular.
                       jrow = ihi - nblst + 1_ilp64
                       call stdlib_I64_cgemv( 'CONJUGATE', nblst, len, cone, work,nblst, a( jrow, j+1 &
                                 ), 1_ilp64, czero,work( pw ), 1_ilp64 )
                       ppw = pw + len
                       do i = jrow, jrow+nblst-len-1
                          work( ppw ) = a( i, j+1 )
                          ppw = ppw + 1_ilp64
                       end do
                       call stdlib_I64_ctrmv( 'LOWER', 'CONJUGATE', 'NON-UNIT',nblst-len, work( &
                                 len*nblst + 1_ilp64 ), nblst,work( pw+len ), 1_ilp64 )
                       call stdlib_I64_cgemv( 'CONJUGATE', len, nblst-len, cone,work( (len+1)*nblst - &
                       len + 1_ilp64 ), nblst,a( jrow+nblst-len, j+1 ), 1_ilp64, cone,work( pw+len ), 1_ilp64 )
                       ...
                       do jrow = j0, jcol+1, -nnb
                          ...
                          call stdlib_I64_ctrmv( 'UPPER', 'CONJUGATE', 'NON-UNIT', len,work( ppwo + &
                                    nnb ), 2_ilp64*nnb, work( pw ),1_ilp64 )
                          call stdlib_I64_ctrmv( 'LOWER', 'CONJUGATE', 'NON-UNIT', nnb,work( ppwo + &
                                    2_ilp64*len*nnb ),2_ilp64*nnb, work( pw + len ), 1_ilp64 )
                          call stdlib_I64_cgemv( 'CONJUGATE', nnb, len, cone,work( ppwo ), 2_ilp64*nnb, a( &
                                    jrow, j+1 ), 1_ilp64,cone, work( pw ), 1_ilp64 )
                          call stdlib_I64_cgemv( 'CONJUGATE', len, nnb, cone,work( ppwo + 2_ilp64*len*nnb + &
                                    nnb ), 2_ilp64*nnb,a( jrow+nnb, j+1 ), 1_ilp64, cone,work( pw+len ), 1_ilp64 )
                          ppw = pw
                          do i = jrow, jrow+len+nnb-1
                             a( i, j+1 ) = work( ppw )
                             ppw = ppw + 1_ilp64
                 ...
                 cola = n - jcol - nnb + 1_ilp64
                 j = ihi - nblst + 1_ilp64
                 call stdlib_I64_cgemm( 'CONJUGATE', 'NO TRANSPOSE', nblst,cola, nblst, cone, work, &
                           nblst,a( j, jcol+nnb ), lda, czero, work( pw ),nblst )
                 call stdlib_I64_clacpy( 'ALL', nblst, cola, work( pw ), nblst,a( j, jcol+nnb ), lda )
                 ...
                 do j = j0, jcol+1, -nnb
                    if ( blk22 ) then
                       ...
                       ! where all blocks are nnb-by-nnb, u21 is upper
                       ! triangular and u12 is lower triangular.
                       call stdlib_I64_cunm22( 'LEFT', 'CONJUGATE', 2_ilp64*nnb, cola, nnb,nnb, work( ppwo )&
                                 , 2_ilp64*nnb,a( j, jcol+nnb ), lda, work( pw ),lwork-pw+1, ierr )
                    else
                       ! ignore the structure of u.
                       call stdlib_I64_cgemm( 'CONJUGATE', 'NO TRANSPOSE', 2_ilp64*nnb,cola, 2_ilp64*nnb, cone, &
                       work( ppwo ), 2_ilp64*nnb,a( j, jcol+nnb ), lda, czero, work( pw ),2_ilp64*nnb )
                                 
                       call stdlib_I64_clacpy( 'ALL', 2_ilp64*nnb, cola, work( pw ), 2_ilp64*nnb,a( j, jcol+nnb ),&
                                  lda )
                    end if
                    ppwo = ppwo + 4_ilp64*nnb*nnb
                 end do
                 ...
                 if( wantq ) then
                    ...
                    call stdlib_I64_cgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', nh,nblst, nblst, cone, q( &
                              topq, j ), ldq,work, nblst, czero, work( pw ), nh )
                    call stdlib_I64_clacpy( 'ALL', nh, nblst, work( pw ), nh,q( topq, j ), ldq )
                    ...
                    do j = j0, jcol+1, -nnb
                       ...
                       if ( blk22 ) then
                          ! exploit the structure of u.
                          call stdlib_I64_cunm22( 'RIGHT', 'NO TRANSPOSE', nh, 2_ilp64*nnb,nnb, nnb, work( &
                                    ppwo ), 2_ilp64*nnb,q( topq, j ), ldq, work( pw ),lwork-pw+1, ierr )
                       else
                          ! ignore the structure of u.
                          call stdlib_I64_cgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', nh,2_ilp64*nnb, 2_ilp64*nnb, &
                          cone, q( topq, j ), ldq,work( ppwo ), 2_ilp64*nnb, czero, work( pw ),nh )
                                    
                          call stdlib_I64_clacpy( 'ALL', nh, 2_ilp64*nnb, work( pw ), nh,q( topq, j ), ldq )
                 ...
                 if ( wantz .or. top>0_ilp64 ) then
                    ...
                    do j = jcol, jcol+nnb-1
                       ...
                       do jrow = j0, j+2, -nnb
                          ...
                          do i = jrow+nnb-1, jrow, -1
                             ...
                             do jj = ppw, ppw+len-1
                                temp = work( jj + 2_ilp64*nnb )
                                work( jj + 2_ilp64*nnb ) = ctemp*temp -conjg( s )*work( jj )
                                work( jj ) = s*temp + ctemp*work( jj )
                             end do
                             len = len + 1_ilp64
                             ppw = ppw - 2_ilp64*nnb - 1_ilp64
                    ...
                    call stdlib_I64_claset( 'LOWER', ihi - jcol - 1_ilp64, nnb, czero, czero,a( jcol + 2_ilp64, &
                              jcol ), lda )
                    call stdlib_I64_claset( 'LOWER', ihi - jcol - 1_ilp64, nnb, czero, czero,b( jcol + 2_ilp64, &
                              jcol ), ldb )
                 end if
                 ! apply accumulated unitary matrices to a and b.
                 if ( top>0_ilp64 ) then
                    j = ihi - nblst + 1_ilp64
                    call stdlib_I64_cgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', top,nblst, nblst, cone, a( &
                              1_ilp64, j ), lda,work, nblst, czero, work( pw ), top )
                    call stdlib_I64_clacpy( 'ALL', top, nblst, work( pw ), top,a( 1_ilp64, j ), lda )
                    ...
                    do j = j0, jcol+1, -nnb
                       if ( blk22 ) then
                          ! exploit the structure of u.
                          call stdlib_I64_cunm22( 'RIGHT', 'NO TRANSPOSE', top, 2_ilp64*nnb,nnb, nnb, work( &
                                    ppwo ), 2_ilp64*nnb,a( 1_ilp64, j ), lda, work( pw ),lwork-pw+1, ierr )
                       else
                          ! ignore the structure of u.
                          call stdlib_I64_cgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', top,2_ilp64*nnb, 2_ilp64*nnb, &
                          cone, a( 1_ilp64, j ), lda,work( ppwo ), 2_ilp64*nnb, czero,work( pw ), top )
                                    
                          call stdlib_I64_clacpy( 'ALL', top, 2_ilp64*nnb, work( pw ), top,a( 1_ilp64, j ), lda )
                                    
                       end if
                       ppwo = ppwo + 4_ilp64*nnb*nnb
                    end do
                    j = ihi - nblst + 1_ilp64
                    call stdlib_I64_cgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', top,nblst, nblst, cone, b( &
                              1_ilp64, j ), ldb,work, nblst, czero, work( pw ), top )
                    call stdlib_I64_clacpy( 'ALL', top, nblst, work( pw ), top,b( 1_ilp64, j ), ldb )
                    ...
                    do j = j0, jcol+1, -nnb
                       if ( blk22 ) then
                          ...
                          ! ignore the structure of u.
                          call stdlib_I64_cgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', top,2_ilp64*nnb, 2_ilp64*nnb, &
                          cone, b( 1_ilp64, j ), ldb,work( ppwo ), 2_ilp64*nnb, czero,work( pw ), top )
                                    
                          call stdlib_I64_clacpy( 'ALL', top, 2_ilp64*nnb, work( pw ), top,b( 1_ilp64, j ), ldb )
                 ...
                 if( wantz ) then
                    ...
                    call stdlib_I64_cgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', nh,nblst, nblst, cone, z( &
                              topq, j ), ldz,work, nblst, czero, work( pw ), nh )
                    call stdlib_I64_clacpy( 'ALL', nh, nblst, work( pw ), nh,z( topq, j ), ldz )
                    ...
                    do j = j0, jcol+1, -nnb
                       ...
                       if ( blk22 ) then
                          ...
                          ! ignore the structure of u.
                          call stdlib_I64_cgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', nh,2_ilp64*nnb, 2_ilp64*nnb, &
                          cone, z( topq, j ), ldz,work( ppwo ), 2_ilp64*nnb, czero, work( pw ),nh )
                                    
                          call stdlib_I64_clacpy( 'ALL', nh, 2_ilp64*nnb, work( pw ), nh,z( topq, j ), ldz )
           ...
           if ( jcol<ihi )call stdlib_I64_cgghrd( compq2, compz2, n, jcol, ihi, a, lda, b, ldb, q,ldq,&
                      z, ldz, ierr )
           work( 1_ilp64 ) = cmplx( lwkopt,KIND=sp)
           return
     ...
     !! multiplications for parts of the computation to enhance performance.
        ...
        ! =====================================================================
           ...
           ! decode and test the input parameters.
           info = 0_ilp64
           nb = stdlib_I64_ilaenv( 1_ilp64, 'ZGGHD3', ' ', n, ilo, ihi, -1_ilp64 )
           lwkopt = max( 6_ilp64*n*nb, 1_ilp64 )
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZGGHD3', -info )
              return
           else if( lquery ) then
              return
           end if
           ! initialize q and z if desired.
           if( initq )call stdlib_I64_zlaset( 'ALL', n, n, czero, cone, q, ldq )
           if( initz )call stdlib_I64_zlaset( 'ALL', n, n, czero, cone, z, ldz )
           ! zero out lower triangle of b.
           if( n>1_ilp64 )call stdlib_I64_zlaset( 'LOWER', n-1, n-1, czero, czero, b(2_ilp64, 1_ilp64), ldb )
           ! quick return if possible
           nh = ihi - ilo + 1_ilp64
           if( nh<=1_ilp64 ) then
              work( 1_ilp64 ) = cone
           ...
           ! determine the blocksize.
           nbmin = stdlib_I64_ilaenv( 2_ilp64, 'ZGGHD3', ' ', n, ilo, ihi, -1_ilp64 )
           if( nb>1_ilp64 .and. nb<nh ) then
              ! determine when to use unblocked instead of blocked code.
              nx = max( nb, stdlib_I64_ilaenv( 3_ilp64, 'ZGGHD3', ' ', n, ilo, ihi, -1_ilp64 ) )
              if( nx<nh ) then
                 ! determine if workspace is large enough for blocked code.
                 if( lwork<lwkopt ) then
                    ! not enough workspace to use optimal nb:  determine the
                    ! minimum value of nb, and reduce nb or force use of
                    ! unblocked code.
                    nbmin = max( 2_ilp64, stdlib_I64_ilaenv( 2_ilp64, 'ZGGHD3', ' ', n, ilo, ihi,-1_ilp64 ) )
                    if( lwork>=6_ilp64*n*nbmin ) then
                       nb = lwork / ( 6_ilp64*n )
                    else
                       nb = 1_ilp64
           ...
           if( nb<nbmin .or. nb>=nh ) then
              ...
              ! use blocked code
              kacc22 = stdlib_I64_ilaenv( 16_ilp64, 'ZGGHD3', ' ', n, ilo, ihi, -1_ilp64 )
              blk22 = kacc22==2_ilp64
              do jcol = ilo, ihi-2, nb
                 nnb = min( nb, ihi-jcol-1 )
                 ! initialize small unitary factors that will hold the
                 ...
                 n2nb = ( ihi-jcol-1 ) / nnb - 1_ilp64
                 nblst = ihi - jcol - n2nb*nnb
                 call stdlib_I64_zlaset( 'ALL', nblst, nblst, czero, cone, work, nblst )
                 pw = nblst * nblst + 1_ilp64
                 do i = 1, n2nb
                    call stdlib_I64_zlaset( 'ALL', 2_ilp64*nnb, 2_ilp64*nnb, czero, cone,work( pw ), 2_ilp64*nnb )
                 ...
                 do j = jcol, jcol+nnb-1
                    ...
                    do i = ihi, j+2, -1
                       temp = a( i-1, j )
                       call stdlib_I64_zlartg( temp, a( i, j ), c, s, a( i-1, j ) )
                       a( i, j ) = cmplx( c,KIND=dp)
                       b( i, j ) = s
                    end do
                    ! accumulate givens rotations into workspace array.
                    ...
                    do jj = n, j+1, -1
                       ...
                       if( jj<ihi ) then
                          temp = b( jj+1, jj+1 )
                          call stdlib_I64_zlartg( temp, b( jj+1, jj ), c, s,b( jj+1, jj+1 ) )
                          b( jj+1, jj ) = czero
                          call stdlib_I64_zrot( jj-top, b( top+1, jj+1 ), 1_ilp64,b( top+1, jj ), 1_ilp64, c, s )
                    ...
                    if( jj>0_ilp64 ) then
                       do i = jj, 1, -1
                          c = real( a( j+1+i, j ),KIND=dp)
                          call stdlib_I64_zrot( ihi-top, a( top+1, j+i+1 ), 1_ilp64,a( top+1, j+i ), 1_ilp64, c,-&
                                    conjg( b( j+1+i, j ) ) )
                       end do
                    end if
                    ! update (j+1)th column of a by transformations from left.
                    if ( j < jcol + nnb - 1_ilp64 ) then
                       ...
                       ! triangular.
                       jrow = ihi - nblst + 1_ilp64
                       call stdlib_I64_zgemv( 'CONJUGATE', nblst, len, cone, work,nblst, a( jrow, j+1 &
                                 ), 1_ilp64, czero,work( pw ), 1_ilp64 )
                       ppw = pw + len
                       do i = jrow, jrow+nblst-len-1
                          work( ppw ) = a( i, j+1 )
                       ...
                       call stdlib_I64_ztrmv( 'LOWER', 'CONJUGATE', 'NON-UNIT',nblst-len, work( &
                                 len*nblst + 1_ilp64 ), nblst,work( pw+len ), 1_ilp64 )
                       call stdlib_I64_zgemv( 'CONJUGATE', len, nblst-len, cone,work( (len+1)*nblst - &
                       len + 1_ilp64 ), nblst,a( jrow+nblst-len, j+1 ), 1_ilp64, cone,work( pw+len ), 1_ilp64 )
                       ...
                       do jrow = j0, jcol+1, -nnb
                          ...
                          call stdlib_I64_ztrmv( 'UPPER', 'CONJUGATE', 'NON-UNIT', len,work( ppwo + &
                                    nnb ), 2_ilp64*nnb, work( pw ),1_ilp64 )
                          call stdlib_I64_ztrmv( 'LOWER', 'CONJUGATE', 'NON-UNIT', nnb,work( ppwo + &
                                    2_ilp64*len*nnb ),2_ilp64*nnb, work( pw + len ), 1_ilp64 )
                          call stdlib_I64_zgemv( 'CONJUGATE', nnb, len, cone,work( ppwo ), 2_ilp64*nnb, a( &
                                    jrow, j+1 ), 1_ilp64,cone, work( pw ), 1_ilp64 )
                          call stdlib_I64_zgemv( 'CONJUGATE', len, nnb, cone,work( ppwo + 2_ilp64*len*nnb + &
                                    nnb ), 2_ilp64*nnb,a( jrow+nnb, j+1 ), 1_ilp64, cone,work( pw+len ), 1_ilp64 )
                          ppw = pw
                          do i = jrow, jrow+len+nnb-1
                             a( i, j+1 ) = work( ppw )
                 ...
                 cola = n - jcol - nnb + 1_ilp64
                 j = ihi - nblst + 1_ilp64
                 call stdlib_I64_zgemm( 'CONJUGATE', 'NO TRANSPOSE', nblst,cola, nblst, cone, work, &
                           nblst,a( j, jcol+nnb ), lda, czero, work( pw ),nblst )
                 call stdlib_I64_zlacpy( 'ALL', nblst, cola, work( pw ), nblst,a( j, jcol+nnb ), lda )
                 ...
                 do j = j0, jcol+1, -nnb
                    if ( blk22 ) then
                       ...
                       ! where all blocks are nnb-by-nnb, u21 is upper
                       ! triangular and u12 is lower triangular.
                       call stdlib_I64_zunm22( 'LEFT', 'CONJUGATE', 2_ilp64*nnb, cola, nnb,nnb, work( ppwo )&
                                 , 2_ilp64*nnb,a( j, jcol+nnb ), lda, work( pw ),lwork-pw+1, ierr )
                    else
                       ! ignore the structure of u.
                       call stdlib_I64_zgemm( 'CONJUGATE', 'NO TRANSPOSE', 2_ilp64*nnb,cola, 2_ilp64*nnb, cone, &
                       work( ppwo ), 2_ilp64*nnb,a( j, jcol+nnb ), lda, czero, work( pw ),2_ilp64*nnb )
                                 
                       call stdlib_I64_zlacpy( 'ALL', 2_ilp64*nnb, cola, work( pw ), 2_ilp64*nnb,a( j, jcol+nnb ),&
                                  lda )
                    end if
                    ppwo = ppwo + 4_ilp64*nnb*nnb
                 end do
                 ...
                 if( wantq ) then
                    ...
                    call stdlib_I64_zgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', nh,nblst, nblst, cone, q( &
                              topq, j ), ldq,work, nblst, czero, work( pw ), nh )
                    call stdlib_I64_zlacpy( 'ALL', nh, nblst, work( pw ), nh,q( topq, j ), ldq )
                    ...
                    do j = j0, jcol+1, -nnb
                       ...
                       if ( blk22 ) then
                          ! exploit the structure of u.
                          call stdlib_I64_zunm22( 'RIGHT', 'NO TRANSPOSE', nh, 2_ilp64*nnb,nnb, nnb, work( &
                                    ppwo ), 2_ilp64*nnb,q( topq, j ), ldq, work( pw ),lwork-pw+1, ierr )
                       else
                          ! ignore the structure of u.
                          call stdlib_I64_zgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', nh,2_ilp64*nnb, 2_ilp64*nnb, &
                          cone, q( topq, j ), ldq,work( ppwo ), 2_ilp64*nnb, czero, work( pw ),nh )
                                    
                          call stdlib_I64_zlacpy( 'ALL', nh, 2_ilp64*nnb, work( pw ), nh,q( topq, j ), ldq )
                 ...
                 if ( wantz .or. top>0_ilp64 ) then
                    ...
                    call stdlib_I64_zlaset( 'LOWER', ihi - jcol - 1_ilp64, nnb, czero, czero,a( jcol + 2_ilp64, &
                              jcol ), lda )
                    call stdlib_I64_zlaset( 'LOWER', ihi - jcol - 1_ilp64, nnb, czero, czero,b( jcol + 2_ilp64, &
                              jcol ), ldb )
                 end if
                 ! apply accumulated unitary matrices to a and b.
                 if ( top>0_ilp64 ) then
                    j = ihi - nblst + 1_ilp64
                    call stdlib_I64_zgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', top,nblst, nblst, cone, a( &
                              1_ilp64, j ), lda,work, nblst, czero, work( pw ), top )
                    call stdlib_I64_zlacpy( 'ALL', top, nblst, work( pw ), top,a( 1_ilp64, j ), lda )
                    ...
                    do j = j0, jcol+1, -nnb
                       if ( blk22 ) then
                          ! exploit the structure of u.
                          call stdlib_I64_zunm22( 'RIGHT', 'NO TRANSPOSE', top, 2_ilp64*nnb,nnb, nnb, work( &
                                    ppwo ), 2_ilp64*nnb,a( 1_ilp64, j ), lda, work( pw ),lwork-pw+1, ierr )
                       else
                          ! ignore the structure of u.
                          call stdlib_I64_zgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', top,2_ilp64*nnb, 2_ilp64*nnb, &
                          cone, a( 1_ilp64, j ), lda,work( ppwo ), 2_ilp64*nnb, czero,work( pw ), top )
                                    
                          call stdlib_I64_zlacpy( 'ALL', top, 2_ilp64*nnb, work( pw ), top,a( 1_ilp64, j ), lda )
                                    
                       end if
                       ppwo = ppwo + 4_ilp64*nnb*nnb
                    end do
                    j = ihi - nblst + 1_ilp64
                    call stdlib_I64_zgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', top,nblst, nblst, cone, b( &
                              1_ilp64, j ), ldb,work, nblst, czero, work( pw ), top )
                    call stdlib_I64_zlacpy( 'ALL', top, nblst, work( pw ), top,b( 1_ilp64, j ), ldb )
                    ...
                    do j = j0, jcol+1, -nnb
                       if ( blk22 ) then
                          ...
                          call stdlib_I64_zlacpy( 'ALL', top, 2_ilp64*nnb, work( pw ), top,b( 1_ilp64, j ), ldb )
                 ...
                 if( wantz ) then
                    ...
                    call stdlib_I64_zgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', nh,nblst, nblst, cone, z( &
                              topq, j ), ldz,work, nblst, czero, work( pw ), nh )
                    call stdlib_I64_zlacpy( 'ALL', nh, nblst, work( pw ), nh,z( topq, j ), ldz )
                    ...
                    do j = j0, jcol+1, -nnb
                       ...
                       if ( blk22 ) then
                          ...
                          call stdlib_I64_zlacpy( 'ALL', nh, 2_ilp64*nnb, work( pw ), nh,z( topq, j ), ldz )
           ...
           if ( jcol<ihi )call stdlib_I64_zgghrd( compq2, compz2, n, jcol, ihi, a, lda, b, ldb, q,ldq,&
                      z, ldz, ierr )
           work( 1_ilp64 ) = cmplx( lwkopt,KIND=dp)
           return
     ...
     !! pp. 241--256.
        ...
        ! =====================================================================
          ...
          ! $                     safety = one )
           ...
           ! Local Scalars 
           logical(lk) :: ilazr2, ilazro, ilp64ivt, ilq, ilschr, ilz, lquery
           integer(ilp64) :: icompq, icompz, ifirst, ifrstm, iiter, ilast, ilastm, in, ischur, &
                     istart, j, jc, jch, jiter, jr, maxit
           real(sp) :: a11, a12, a1i, a1r, a21, a22, a2i, a2r, ad11, ad11l, ad12, ad12l, ad21, &
           ad21l, ad22, ad22l, ad32l, an, anorm, ascale, atol, b11, b1a, b1i, b1r, b22, b2a, b2i, &
           b2r, bn, bnorm, bscale, btol, c, c11i, c11r, c12, c21, c22i, c22r, cl, cq, cr, cz, &
           ...
           ! check argument values
           info = 0_ilp64
           work( 1_ilp64 ) = max( 1_ilp64, n )
           lquery = ( lwork==-1_ilp64 )
           if( ischur==0_ilp64 ) then
              info = -1_ilp64
              ...
              info = -17_ilp64
           else if( lwork<max( 1_ilp64, n ) .and. .not.lquery ) then
              info = -19_ilp64
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SHGEQZ', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           safmax = one / safmin
           ulp = stdlib_I64_slamch( 'E' )*stdlib_I64_slamch( 'B' )
           anorm = stdlib_I64_slanhs( 'F', in, h( ilo, ilo ), ldh, work )
           bnorm = stdlib_I64_slanhs( 'F', in, t( ilo, ilo ), ldt, work )
           atol = max( safmin, ulp*anorm )
           btol = max( safmin, ulp*bnorm )
           ascale = one / max( safmin, anorm )
           bscale = one / max( safmin, bnorm )
           ...
           loop_360: do jiter = 1, maxit
              ...
              if( abs( t( ilast, ilast ) )<=max( safmin, ulp*(abs( t( ilast - 1_ilp64, ilast ) ) + abs( &
                        t( ilast-1, ilast-1 )) ) ) ) then
                 t( ilast, ilast ) = zero
                 go to 70
              end if
              ...
              loop_60: do j = ilast - 1, ilo, -1
                 ...
                 ! test 2: for t(j,j)=0
                 temp = abs ( t( j, j + 1_ilp64 ) )
                 if ( j > ilo )temp = temp + abs ( t( j - 1_ilp64, j ) )
                 if( abs( t( j, j ) )<max( safmin,ulp*temp ) ) then
                    t( j, j ) = zero
                    ! test 1a: check for 2 consecutive small subdiagonals in a
                    ilazr2 = .false.
                    ...
                    if( ilazro .or. ilazr2 ) then
                       do jch = j, ilast - 1
                          temp = h( jch, jch )
                          call stdlib_I64_slartg( temp, h( jch+1, jch ), c, s,h( jch, jch ) )
                          h( jch+1, jch ) = zero
                          call stdlib_I64_srot( ilastm-jch, h( jch, jch+1 ), ldh,h( jch+1, jch+1 ), &
                                    ldh, c, s )
                          call stdlib_I64_srot( ilastm-jch, t( jch, jch+1 ), ldt,t( jch+1, jch+1 ), &
                                    ldt, c, s )
                          if( ilq )call stdlib_I64_srot( n, q( 1_ilp64, jch ), 1_ilp64, q( 1_ilp64, jch+1 ), 1_ilp64,c, s )
                       ...
                       do jch = j, ilast - 1
                          temp = t( jch, jch+1 )
                          call stdlib_I64_slartg( temp, t( jch+1, jch+1 ), c, s,t( jch, jch+1 ) )
                                    
                          t( jch+1, jch+1 ) = zero
                          if( jch<ilastm-1 )call stdlib_I64_srot( ilastm-jch-1, t( jch, jch+2 ), ldt,&
                                    t( jch+1, jch+2 ), ldt, c, s )
                          call stdlib_I64_srot( ilastm-jch+2, h( jch, jch-1 ), ldh,h( jch+1, jch-1 ), &
                                    ldh, c, s )
                          if( ilq )call stdlib_I64_srot( n, q( 1_ilp64, jch ), 1_ilp64, q( 1_ilp64, jch+1 ), 1_ilp64,c, s )
                                    
                          temp = h( jch+1, jch )
                          call stdlib_I64_slartg( temp, h( jch+1, jch-1 ), c, s,h( jch+1, jch ) )
                                    
                          h( jch+1, jch-1 ) = zero
                          call stdlib_I64_srot( jch+1-ifrstm, h( ifrstm, jch ), 1_ilp64,h( ifrstm, jch-1 ), &
                                    1_ilp64, c, s )
                          call stdlib_I64_srot( jch-ifrstm, t( ifrstm, jch ), 1_ilp64,t( ifrstm, jch-1 ), 1_ilp64,&
                                     c, s )
                          if( ilz )call stdlib_I64_srot( n, z( 1_ilp64, jch ), 1_ilp64, z( 1_ilp64, jch-1 ), 1_ilp64,c, s )
              ...
              70 continue
              temp = h( ilast, ilast )
              call stdlib_I64_slartg( temp, h( ilast, ilast-1 ), c, s,h( ilast, ilast ) )
              h( ilast, ilast-1 ) = zero
              call stdlib_I64_srot( ilast-ifrstm, h( ifrstm, ilast ), 1_ilp64,h( ifrstm, ilast-1 ), 1_ilp64, c, s &
                        )
              call stdlib_I64_srot( ilast-ifrstm, t( ifrstm, ilast ), 1_ilp64,t( ifrstm, ilast-1 ), 1_ilp64, c, s &
                        )
              if( ilz )call stdlib_I64_srot( n, z( 1_ilp64, ilast ), 1_ilp64, z( 1_ilp64, ilast-1 ), 1_ilp64, c, s )
              ! h(ilast,ilast-1)=0 -- standardize b, set alphar, alphai,
                                    ! and beta
                                    80 continue
              if( t( ilast, ilast )<zero ) then
              ...
              ! t(ifirst:ilast,ifirst,ilast) are larger than btol (in
              ! magnitude)
              if( ( iiter / 10_ilp64 )*10_ilp64==iiter ) then
                 ! exceptional shift.  chosen for no particularly good reason.
                 ! (single shift only.)
                 if( ( real( maxit,KIND=sp)*safmin )*abs( h( ilast, ilast-1 ) )<abs( t( ilast-1, &
                           ilast-1 ) ) ) then
                 ...
                 ! shifts based on the generalized eigenvalues of the
                 ! bottom-right 2x2 block of a and b. the first eigenvalue
                 ! returned by stdlib_I64_slag2 is the wilkinson shift (aep p.512_sp),
                 call stdlib_I64_slag2( h( ilast-1, ilast-1 ), ldh,t( ilast-1, ilast-1 ), ldt, &
                           safmin*safety, s1,s2, wr, wr2, wi )
                 if ( abs( (wr/s1)*t( ilast, ilast ) - h( ilast, ilast ) )> abs( (wr2/s2)*t( &
                           ilast, ilast )- h( ilast, ilast ) ) ) then
                    temp = wr
              ...
              temp = s1*h( istart, istart ) - wr*t( istart, istart )
              temp2 = s1*h( istart+1, istart )
              call stdlib_I64_slartg( temp, temp2, c, s, tempr )
              ! sweep
              loop_190: do j = istart, ilast - 1
                 if( j>istart ) then
                    temp = h( j, j-1 )
                    call stdlib_I64_slartg( temp, h( j+1, j-1 ), c, s, h( j, j-1 ) )
                    h( j+1, j-1 ) = zero
                 end if
                 do jc = j, ilastm
                    temp = c*h( j, jc ) + s*h( j+1, jc )
                 ...
                 temp = t( j+1, j+1 )
                 call stdlib_I64_slartg( temp, t( j+1, j ), c, s, t( j+1, j+1 ) )
                 t( j+1, j ) = zero
                 do jr = ifrstm, min( j+2, ilast )
                    temp = c*h( jr, j+1 ) + s*h( jr, j )
                    h( jr, j ) = -s*h( jr, j+1 ) + c*h( jr, j )
              ...
              if( ifirst+1==ilast ) then
                 ...
                 ! step 1: standardize, that is, rotate so that
                         ...
                         ! b = (         )  with b11 non-negative.
                             ! (  0  b22 )
                 call stdlib_I64_slasv2( t( ilast-1, ilast-1 ), t( ilast-1, ilast ),t( ilast, ilast ),&
                            b22, b11, sr, cr, sl, cl )
                 if( b11<zero ) then
                    cr = -cr
                    sr = -sr
                 ...
                 call stdlib_I64_srot( ilastm+1-ifirst, h( ilast-1, ilast-1 ), ldh,h( ilast, ilast-1 )&
                           , ldh, cl, sl )
                 call stdlib_I64_srot( ilast+1-ifrstm, h( ifrstm, ilast-1 ), 1_ilp64,h( ifrstm, ilast ), 1_ilp64, &
                           cr, sr )
                 if( ilast<ilastm )call stdlib_I64_srot( ilastm-ilast, t( ilast-1, ilast+1 ), ldt,t( &
                           ilast, ilast+1 ), ldt, cl, sl )
                 if( ifrstm<ilast-1 )call stdlib_I64_srot( ifirst-ifrstm, t( ifrstm, ilast-1 ), 1_ilp64,t( &
                           ifrstm, ilast ), 1_ilp64, cr, sr )
                 if( ilq )call stdlib_I64_srot( n, q( 1_ilp64, ilast-1 ), 1_ilp64, q( 1_ilp64, ilast ), 1_ilp64, cl,sl )
                           
                 if( ilz )call stdlib_I64_srot( n, z( 1_ilp64, ilast-1 ), 1_ilp64, z( 1_ilp64, ilast ), 1_ilp64, cr,sr )
                 ...
                 if( abs( c11r )+abs( c11i )+abs( c12 )>abs( c21 )+abs( c22r )+abs( c22i ) ) &
                           then
                    t1 = stdlib_I64_slapy3( c12, c11r, c11i )
                    cz = c12 / t1
                    szr = -c11r / t1
                    szi = -c11i / t1
                 else
                    cz = stdlib_I64_slapy2( c22r, c22i )
                    if( cz<=safmin ) then
                       cz = zero
                       szr = one
                       szi = zero
                 ...
                 t1 = stdlib_I64_slapy3( cq, sqr, sqi )
                 cq = cq / t1
                 sqr = sqr / t1
                 sqi = sqi / t1
                 ! compute diagonal elements of qbz
                 ...
                 b1r = cq*cz*b11 + tempr*b22
                 b1i = tempi*b22
                 b1a = stdlib_I64_slapy2( b1r, b1i )
                 b2r = cq*cz*b22 + tempr*b11
                 b2i = -tempi*b11
                 b2a = stdlib_I64_slapy2( b2r, b2i )
                 ! normalize so beta > 0, and im( alpha1 ) > 0
                 beta( ilast-1 ) = b1a
                 beta( ilast ) = b2a
                 alphar( ilast-1 ) = ( wr*b1a )*s1inv
                 ...
                 u12l = t( ifirst, ifirst+1 ) / t( ifirst+1, ifirst+1 )
                 v( 1_ilp64 ) = ( ad11-ad11l )*( ad22-ad11l ) - ad12*ad21 +ad21*u12*ad11l + ( ad12l-&
                           ad11l*u12l )*ad21l
                 v( 2_ilp64 ) = ( ( ad22l-ad11l )-ad21l*u12l-( ad11-ad11l )-( ad22-ad11l )+ad21*u12 )&
                           *ad21l
                 v( 3_ilp64 ) = ad32l*ad21l
                 istart = ifirst
                 call stdlib_I64_slarfg( 3_ilp64, v( 1_ilp64 ), v( 2_ilp64 ), 1_ilp64, tau )
                 v( 1_ilp64 ) = one
                 ! sweep
                 loop_290: do j = istart, ilast - 2
                    ! all but last elements: use 3x3 householder transforms.
                    ...
                    if( j>istart ) then
                       ...
                       v( 2_ilp64 ) = h( j+1, j-1 )
                       v( 3_ilp64 ) = h( j+2, j-1 )
                       call stdlib_I64_slarfg( 3_ilp64, h( j, j-1 ), v( 2_ilp64 ), 1_ilp64, tau )
                       v( 1_ilp64 ) = one
                       h( j+1, j-1 ) = zero
                       h( j+2, j-1 ) = zero
                    end if
                    do jc = j, ilastm
                       temp = tau*( h( j, jc )+v( 2_ilp64 )*h( j+1, jc )+v( 3_ilp64 )*h( j+2, jc ) )
                       h( j, jc ) = h( j, jc ) - temp
                       h( j+1, jc ) = h( j+1, jc ) - temp*v( 2_ilp64 )
                       h( j+2, jc ) = h( j+2, jc ) - temp*v( 3_ilp64 )
                       temp2 = tau*( t( j, jc )+v( 2_ilp64 )*t( j+1, jc )+v( 3_ilp64 )*t( j+2, jc ) )
                       t( j, jc ) = t( j, jc ) - temp2
                       t( j+1, jc ) = t( j+1, jc ) - temp2*v( 2_ilp64 )
                       t( j+2, jc ) = t( j+2, jc ) - temp2*v( 3_ilp64 )
                    end do
                    if( ilq ) then
                       do jr = 1, n
                          temp = tau*( q( jr, j )+v( 2_ilp64 )*q( jr, j+1 )+v( 3_ilp64 )*q( jr, j+2 ) )
                                    
                          q( jr, j ) = q( jr, j ) - temp
                          q( jr, j+1 ) = q( jr, j+1 ) - temp*v( 2_ilp64 )
                          q( jr, j+2 ) = q( jr, j+2 ) - temp*v( 3_ilp64 )
                       end do
                    end if
                    ! zero j-th column of b (see slagbc for details)
                    ! swap rows to pivot
                    ...
                    ! compute householder vector
                    t1 = sqrt( scale**2_ilp64+u1**2_ilp64+u2**2_ilp64 )
                    tau = one + scale / t1
                    vs = -one / ( scale+t1 )
                    v( 1_ilp64 ) = one
                    v( 2_ilp64 ) = vs*u1
                    ...
                    ! apply transformations from the right.
                    do jr = ifrstm, min( j+3, ilast )
                       temp = tau*( h( jr, j )+v( 2_ilp64 )*h( jr, j+1 )+v( 3_ilp64 )*h( jr, j+2 ) )
                       h( jr, j ) = h( jr, j ) - temp
                       h( jr, j+1 ) = h( jr, j+1 ) - temp*v( 2_ilp64 )
                       h( jr, j+2 ) = h( jr, j+2 ) - temp*v( 3_ilp64 )
                    end do
                    do jr = ifrstm, j + 2
                       temp = tau*( t( jr, j )+v( 2_ilp64 )*t( jr, j+1 )+v( 3_ilp64 )*t( jr, j+2 ) )
                       t( jr, j ) = t( jr, j ) - temp
                       t( jr, j+1 ) = t( jr, j+1 ) - temp*v( 2_ilp64 )
                       t( jr, j+2 ) = t( jr, j+2 ) - temp*v( 3_ilp64 )
                    end do
                    if( ilz ) then
                       do jr = 1, n
                          temp = tau*( z( jr, j )+v( 2_ilp64 )*z( jr, j+1 )+v( 3_ilp64 )*z( jr, j+2 ) )
                                    
                          z( jr, j ) = z( jr, j ) - temp
                          z( jr, j+1 ) = z( jr, j+1 ) - temp*v( 2_ilp64 )
                          z( jr, j+2 ) = z( jr, j+2 ) - temp*v( 3_ilp64 )
                       end do
                    end if
                    t( j+1, j ) = zero
                    t( j+2, j ) = zero
     ...
     !! pp. 241--256.
        ...
        ! =====================================================================
          ...
          ! $                     safety = one )
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DHGEQZ', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           safmax = one / safmin
           ulp = stdlib_I64_dlamch( 'E' )*stdlib_I64_dlamch( 'B' )
           anorm = stdlib_I64_dlanhs( 'F', in, h( ilo, ilo ), ldh, work )
           bnorm = stdlib_I64_dlanhs( 'F', in, t( ilo, ilo ), ldt, work )
           atol = max( safmin, ulp*anorm )
           btol = max( safmin, ulp*bnorm )
           ascale = one / max( safmin, anorm )
           bscale = one / max( safmin, bnorm )
           ...
           loop_360: do jiter = 1, maxit
              ...
              loop_60: do j = ilast - 1, ilo, -1
                 ...
                 if( abs( t( j, j ) )<max( safmin,ulp*temp ) ) then
                    ...
                    if( ilazro .or. ilazr2 ) then
                       do jch = j, ilast - 1
                          temp = h( jch, jch )
                          call stdlib_I64_dlartg( temp, h( jch+1, jch ), c, s,h( jch, jch ) )
                          h( jch+1, jch ) = zero
                          call stdlib_I64_drot( ilastm-jch, h( jch, jch+1 ), ldh,h( jch+1, jch+1 ), &
                                    ldh, c, s )
                          call stdlib_I64_drot( ilastm-jch, t( jch, jch+1 ), ldt,t( jch+1, jch+1 ), &
                                    ldt, c, s )
                          if( ilq )call stdlib_I64_drot( n, q( 1_ilp64, jch ), 1_ilp64, q( 1_ilp64, jch+1 ), 1_ilp64,c, s )
                       ...
                       do jch = j, ilast - 1
                          temp = t( jch, jch+1 )
                          call stdlib_I64_dlartg( temp, t( jch+1, jch+1 ), c, s,t( jch, jch+1 ) )
                                    
                          t( jch+1, jch+1 ) = zero
                          if( jch<ilastm-1 )call stdlib_I64_drot( ilastm-jch-1, t( jch, jch+2 ), ldt,&
                                    t( jch+1, jch+2 ), ldt, c, s )
                          call stdlib_I64_drot( ilastm-jch+2, h( jch, jch-1 ), ldh,h( jch+1, jch-1 ), &
                                    ldh, c, s )
                          if( ilq )call stdlib_I64_drot( n, q( 1_ilp64, jch ), 1_ilp64, q( 1_ilp64, jch+1 ), 1_ilp64,c, s )
                                    
                          temp = h( jch+1, jch )
                          call stdlib_I64_dlartg( temp, h( jch+1, jch-1 ), c, s,h( jch+1, jch ) )
                                    
                          h( jch+1, jch-1 ) = zero
                          call stdlib_I64_drot( jch+1-ifrstm, h( ifrstm, jch ), 1_ilp64,h( ifrstm, jch-1 ), &
                                    1_ilp64, c, s )
                          call stdlib_I64_drot( jch-ifrstm, t( ifrstm, jch ), 1_ilp64,t( ifrstm, jch-1 ), 1_ilp64,&
                                     c, s )
                          if( ilz )call stdlib_I64_drot( n, z( 1_ilp64, jch ), 1_ilp64, z( 1_ilp64, jch-1 ), 1_ilp64,c, s )
              ...
              70 continue
              temp = h( ilast, ilast )
              call stdlib_I64_dlartg( temp, h( ilast, ilast-1 ), c, s,h( ilast, ilast ) )
              h( ilast, ilast-1 ) = zero
              call stdlib_I64_drot( ilast-ifrstm, h( ifrstm, ilast ), 1_ilp64,h( ifrstm, ilast-1 ), 1_ilp64, c, s &
                        )
              call stdlib_I64_drot( ilast-ifrstm, t( ifrstm, ilast ), 1_ilp64,t( ifrstm, ilast-1 ), 1_ilp64, c, s &
                        )
              if( ilz )call stdlib_I64_drot( n, z( 1_ilp64, ilast ), 1_ilp64, z( 1_ilp64, ilast-1 ), 1_ilp64, c, s )
              ! h(ilast,ilast-1)=0 -- standardize b, set alphar, alphai,
                                    ! and beta
                                    80 continue
              if( t( ilast, ilast )<zero ) then
              ...
              if( ( iiter / 10_ilp64 )*10_ilp64==iiter ) then
                 ...
                 ! shifts based on the generalized eigenvalues of the
                 ! bottom-right 2x2 block of a and b. the first eigenvalue
                 ! returned by stdlib_I64_dlag2 is the wilkinson shift (aep p.512_dp),
                 call stdlib_I64_dlag2( h( ilast-1, ilast-1 ), ldh,t( ilast-1, ilast-1 ), ldt, &
                           safmin*safety, s1,s2, wr, wr2, wi )
                 if ( abs( (wr/s1)*t( ilast, ilast ) - h( ilast, ilast ) )> abs( (wr2/s2)*t( &
                           ilast, ilast )- h( ilast, ilast ) ) ) then
                    temp = wr
              ...
              temp = s1*h( istart, istart ) - wr*t( istart, istart )
              temp2 = s1*h( istart+1, istart )
              call stdlib_I64_dlartg( temp, temp2, c, s, tempr )
              ! sweep
              loop_190: do j = istart, ilast - 1
                 if( j>istart ) then
                    temp = h( j, j-1 )
                    call stdlib_I64_dlartg( temp, h( j+1, j-1 ), c, s, h( j, j-1 ) )
                    h( j+1, j-1 ) = zero
                 end if
                 do jc = j, ilastm
                    temp = c*h( j, jc ) + s*h( j+1, jc )
                 ...
                 temp = t( j+1, j+1 )
                 call stdlib_I64_dlartg( temp, t( j+1, j ), c, s, t( j+1, j+1 ) )
                 t( j+1, j ) = zero
                 do jr = ifrstm, min( j+2, ilast )
                    temp = c*h( jr, j+1 ) + s*h( jr, j )
                    h( jr, j ) = -s*h( jr, j+1 ) + c*h( jr, j )
              ...
              if( ifirst+1==ilast ) then
                 ...
                 ! step 1: standardize, that is, rotate so that
                         ...
                         ! b = (         )  with b11 non-negative.
                             ! (  0  b22 )
                 call stdlib_I64_dlasv2( t( ilast-1, ilast-1 ), t( ilast-1, ilast ),t( ilast, ilast ),&
                            b22, b11, sr, cr, sl, cl )
                 if( b11<zero ) then
                    cr = -cr
                    sr = -sr
                 ...
                 call stdlib_I64_drot( ilastm+1-ifirst, h( ilast-1, ilast-1 ), ldh,h( ilast, ilast-1 )&
                           , ldh, cl, sl )
                 call stdlib_I64_drot( ilast+1-ifrstm, h( ifrstm, ilast-1 ), 1_ilp64,h( ifrstm, ilast ), 1_ilp64, &
                           cr, sr )
                 if( ilast<ilastm )call stdlib_I64_drot( ilastm-ilast, t( ilast-1, ilast+1 ), ldt,t( &
                           ilast, ilast+1 ), ldt, cl, sl )
                 if( ifrstm<ilast-1 )call stdlib_I64_drot( ifirst-ifrstm, t( ifrstm, ilast-1 ), 1_ilp64,t( &
                           ifrstm, ilast ), 1_ilp64, cr, sr )
                 if( ilq )call stdlib_I64_drot( n, q( 1_ilp64, ilast-1 ), 1_ilp64, q( 1_ilp64, ilast ), 1_ilp64, cl,sl )
                           
                 if( ilz )call stdlib_I64_drot( n, z( 1_ilp64, ilast-1 ), 1_ilp64, z( 1_ilp64, ilast ), 1_ilp64, cr,sr )
                 ...
                 if( abs( c11r )+abs( c11i )+abs( c12 )>abs( c21 )+abs( c22r )+abs( c22i ) ) &
                           then
                    t1 = stdlib_I64_dlapy3( c12, c11r, c11i )
                    cz = c12 / t1
                    szr = -c11r / t1
                    szi = -c11i / t1
                 else
                    cz = stdlib_I64_dlapy2( c22r, c22i )
                    if( cz<=safmin ) then
                       cz = zero
                       szr = one
                       szi = zero
                 ...
                 t1 = stdlib_I64_dlapy3( cq, sqr, sqi )
                 cq = cq / t1
                 sqr = sqr / t1
                 sqi = sqi / t1
                 ! compute diagonal elements of qbz
                 ...
                 b1r = cq*cz*b11 + tempr*b22
                 b1i = tempi*b22
                 b1a = stdlib_I64_dlapy2( b1r, b1i )
                 b2r = cq*cz*b22 + tempr*b11
                 b2i = -tempi*b11
                 b2a = stdlib_I64_dlapy2( b2r, b2i )
                 ! normalize so beta > 0, and im( alpha1 ) > 0
                 beta( ilast-1 ) = b1a
                 beta( ilast ) = b2a
                 alphar( ilast-1 ) = ( wr*b1a )*s1inv
                 ...
                 v( 3_ilp64 ) = ad32l*ad21l
                 istart = ifirst
                 call stdlib_I64_dlarfg( 3_ilp64, v( 1_ilp64 ), v( 2_ilp64 ), 1_ilp64, tau )
                 v( 1_ilp64 ) = one
                 ! sweep
                 loop_290: do j = istart, ilast - 2
                    ! all but last elements: use 3x3 householder transforms.
                    ...
                    if( j>istart ) then
                       ...
                       v( 2_ilp64 ) = h( j+1, j-1 )
                       v( 3_ilp64 ) = h( j+2, j-1 )
                       call stdlib_I64_dlarfg( 3_ilp64, h( j, j-1 ), v( 2_ilp64 ), 1_ilp64, tau )
                       v( 1_ilp64 ) = one
                       h( j+1, j-1 ) = zero
                       h( j+2, j-1 ) = zero
                    end if
     ...
     !! pp. 241--256.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CHGEQZ', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           ! work( 1 ) = cmplx( 1,KIND=sp)
           if( n<=0_ilp64 ) then
              work( 1_ilp64 ) = cmplx( 1_ilp64,KIND=sp)
              return
           end if
           ! initialize q and z
           if( icompq==3_ilp64 )call stdlib_I64_claset( 'FULL', n, n, czero, cone, q, ldq )
           ...
           safmin = stdlib_I64_slamch( 'S' )
           ulp = stdlib_I64_slamch( 'E' )*stdlib_I64_slamch( 'B' )
           anorm = stdlib_I64_clanhs( 'F', in, h( ilo, ilo ), ldh, rwork )
           bnorm = stdlib_I64_clanhs( 'F', in, t( ilo, ilo ), ldt, rwork )
           atol = max( safmin, ulp*anorm )
           btol = max( safmin, ulp*bnorm )
           ascale = one / max( safmin, anorm )
           bscale = one / max( safmin, bnorm )
           ...
           do j = ihi + 1, n
              ...
              if( absb>safmin ) then
                 ...
                 t( j, j ) = absb
                 if( ilschr ) then
                    call stdlib_I64_cscal( j-1, signbc, t( 1_ilp64, j ), 1_ilp64 )
                    call stdlib_I64_cscal( j, signbc, h( 1_ilp64, j ), 1_ilp64 )
                 else
                    call stdlib_I64_cscal( 1_ilp64, signbc, h( j, j ), 1_ilp64 )
                 end if
                 if( ilz )call stdlib_I64_cscal( n, signbc, z( 1_ilp64, j ), 1_ilp64 )
              else
                 t( j, j ) = czero
              end if
              alpha( j ) = h( j, j )
           ...
           loop_170: do jiter = 1, maxit
              ...
              loop_40: do j = ilast - 1, ilo, -1
                 ...
                 if( abs( t( j, j ) )<max( safmin,ulp*temp ) ) then
                    ...
                    if( ilazro .or. ilazr2 ) then
                       do jch = j, ilast - 1
                          ctemp = h( jch, jch )
                          call stdlib_I64_clartg( ctemp, h( jch+1, jch ), c, s,h( jch, jch ) )
                          h( jch+1, jch ) = czero
                          call stdlib_I64_crot( ilastm-jch, h( jch, jch+1 ), ldh,h( jch+1, jch+1 ), &
                                    ldh, c, s )
                          call stdlib_I64_crot( ilastm-jch, t( jch, jch+1 ), ldt,t( jch+1, jch+1 ), &
                                    ldt, c, s )
                          if( ilq )call stdlib_I64_crot( n, q( 1_ilp64, jch ), 1_ilp64, q( 1_ilp64, jch+1 ), 1_ilp64,c, conjg(&
                                     s ) )
                          if( ilazr2 )h( jch, jch-1 ) = h( jch, jch-1 )*c
                          ilazr2 = .false.
                          if( abs1( t( jch+1, jch+1 ) )>=btol ) then
                       ...
                       do jch = j, ilast - 1
                          ctemp = t( jch, jch+1 )
                          call stdlib_I64_clartg( ctemp, t( jch+1, jch+1 ), c, s,t( jch, jch+1 ) )
                                    
                          t( jch+1, jch+1 ) = czero
                          if( jch<ilastm-1 )call stdlib_I64_crot( ilastm-jch-1, t( jch, jch+2 ), ldt,&
                                    t( jch+1, jch+2 ), ldt, c, s )
                          call stdlib_I64_crot( ilastm-jch+2, h( jch, jch-1 ), ldh,h( jch+1, jch-1 ), &
                                    ldh, c, s )
                          if( ilq )call stdlib_I64_crot( n, q( 1_ilp64, jch ), 1_ilp64, q( 1_ilp64, jch+1 ), 1_ilp64,c, conjg(&
                                     s ) )
                          ctemp = h( jch+1, jch )
                          call stdlib_I64_clartg( ctemp, h( jch+1, jch-1 ), c, s,h( jch+1, jch ) )
                                    
                          h( jch+1, jch-1 ) = czero
                          call stdlib_I64_crot( jch+1-ifrstm, h( ifrstm, jch ), 1_ilp64,h( ifrstm, jch-1 ), &
                                    1_ilp64, c, s )
                          call stdlib_I64_crot( jch-ifrstm, t( ifrstm, jch ), 1_ilp64,t( ifrstm, jch-1 ), 1_ilp64,&
                                     c, s )
                          if( ilz )call stdlib_I64_crot( n, z( 1_ilp64, jch ), 1_ilp64, z( 1_ilp64, jch-1 ), 1_ilp64,c, s )
              ...
              ! (drop-through is "impossible")
              info = 2_ilp64*n + 1_ilp64
              go to 210
              ! t(ilast,ilast)=0 -- clear h(ilast,ilast-1) to split off a
              ! 1x1 block.
              50 continue
              ctemp = h( ilast, ilast )
              call stdlib_I64_clartg( ctemp, h( ilast, ilast-1 ), c, s,h( ilast, ilast ) )
              h( ilast, ilast-1 ) = czero
              call stdlib_I64_crot( ilast-ifrstm, h( ifrstm, ilast ), 1_ilp64,h( ifrstm, ilast-1 ), 1_ilp64, c, s &
                        )
              call stdlib_I64_crot( ilast-ifrstm, t( ifrstm, ilast ), 1_ilp64,t( ifrstm, ilast-1 ), 1_ilp64, c, s &
                        )
              if( ilz )call stdlib_I64_crot( n, z( 1_ilp64, ilast ), 1_ilp64, z( 1_ilp64, ilast-1 ), 1_ilp64, c, s )
              ! h(ilast,ilast-1)=0 -- standardize b, set alpha and beta
              60 continue
              absb = abs( t( ilast, ilast ) )
              if( absb>safmin ) then
                 ...
                 t( ilast, ilast ) = absb
                 if( ilschr ) then
                    call stdlib_I64_cscal( ilast-ifrstm, signbc, t( ifrstm, ilast ), 1_ilp64 )
                    call stdlib_I64_cscal( ilast+1-ifrstm, signbc, h( ifrstm, ilast ),1_ilp64 )
                 else
                    call stdlib_I64_cscal( 1_ilp64, signbc, h( ilast, ilast ), 1_ilp64 )
                 end if
                 if( ilz )call stdlib_I64_cscal( n, signbc, z( 1_ilp64, ilast ), 1_ilp64 )
              else
                 t( ilast, ilast ) = czero
              end if
              alpha( ilast ) = h( ilast, ilast )
              ...
              if( ( iiter / 10_ilp64 )*10_ilp64/=iiter ) then
                 ...
                 if( ctemp/=zero ) then
                    ...
                    temp2 = abs1( x )
                    temp = max( temp, abs1( x ) )
                    y = temp*sqrt( ( x / temp )**2_ilp64+( ctemp / temp )**2_ilp64 )
                    if( temp2>zero ) then
                       if( real( x / temp2,KIND=sp)*real( y,KIND=sp)+aimag( x / temp2 )*aimag( y )&
                                 <zero )y = -y
                    end if
                    shift = shift - ctemp*stdlib_I64_cladiv( ctemp, ( x+y ) )
                 end if
              else
                 ! exceptional shift.  chosen for no particularly good reason.
                 if( ( iiter / 20_ilp64 )*20_ilp64==iiter .and.bscale*abs1(t( ilast, ilast ))>safmin ) &
                           then
                    eshift = eshift + ( ascale*h( ilast,ilast ) )/( bscale*t( ilast, ilast ) )
              ...
              ! initial q
              ctemp2 = ascale*h( istart+1, istart )
              call stdlib_I64_clartg( ctemp, ctemp2, c, s, ctemp3 )
              ! sweep
              loop_150: do j = istart, ilast - 1
                 if( j>istart ) then
                    ctemp = h( j, j-1 )
                    call stdlib_I64_clartg( ctemp, h( j+1, j-1 ), c, s, h( j, j-1 ) )
                    h( j+1, j-1 ) = czero
                 end if
                 do jc = j, ilastm
                    ctemp = c*h( j, jc ) + s*h( j+1, jc )
                 ...
                 ctemp = t( j+1, j+1 )
                 call stdlib_I64_clartg( ctemp, t( j+1, j ), c, s, t( j+1, j+1 ) )
                 t( j+1, j ) = czero
                 do jr = ifrstm, min( j+2, ilast )
                    ctemp = c*h( jr, j+1 ) + s*h( jr, j )
                    h( jr, j ) = -conjg( s )*h( jr, j+1 ) + c*h( jr, j )
     ...
     !! pp. 241--256.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZHGEQZ', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           ! work( 1 ) = cmplx( 1,KIND=dp)
           if( n<=0_ilp64 ) then
              work( 1_ilp64 ) = cmplx( 1_ilp64,KIND=dp)
              return
           end if
           ! initialize q and z
           if( icompq==3_ilp64 )call stdlib_I64_zlaset( 'FULL', n, n, czero, cone, q, ldq )
           ...
           safmin = stdlib_I64_dlamch( 'S' )
           ulp = stdlib_I64_dlamch( 'E' )*stdlib_I64_dlamch( 'B' )
           anorm = stdlib_I64_zlanhs( 'F', in, h( ilo, ilo ), ldh, rwork )
           bnorm = stdlib_I64_zlanhs( 'F', in, t( ilo, ilo ), ldt, rwork )
           atol = max( safmin, ulp*anorm )
           btol = max( safmin, ulp*bnorm )
           ascale = one / max( safmin, anorm )
           bscale = one / max( safmin, bnorm )
           ...
           do j = ihi + 1, n
              ...
              if( absb>safmin ) then
                 ...
                 t( j, j ) = absb
                 if( ilschr ) then
                    call stdlib_I64_zscal( j-1, signbc, t( 1_ilp64, j ), 1_ilp64 )
                    call stdlib_I64_zscal( j, signbc, h( 1_ilp64, j ), 1_ilp64 )
                 else
                    call stdlib_I64_zscal( 1_ilp64, signbc, h( j, j ), 1_ilp64 )
                 end if
                 if( ilz )call stdlib_I64_zscal( n, signbc, z( 1_ilp64, j ), 1_ilp64 )
              else
                 t( j, j ) = czero
              end if
              alpha( j ) = h( j, j )
           ...
           loop_170: do jiter = 1, maxit
              ...
              loop_40: do j = ilast - 1, ilo, -1
                 ...
                 if( abs( t( j, j ) )<max( safmin,ulp*temp ) ) then
                    ...
                    if( ilazro .or. ilazr2 ) then
                       do jch = j, ilast - 1
                          ctemp = h( jch, jch )
                          call stdlib_I64_zlartg( ctemp, h( jch+1, jch ), c, s,h( jch, jch ) )
                          h( jch+1, jch ) = czero
                          call stdlib_I64_zrot( ilastm-jch, h( jch, jch+1 ), ldh,h( jch+1, jch+1 ), &
                                    ldh, c, s )
                          call stdlib_I64_zrot( ilastm-jch, t( jch, jch+1 ), ldt,t( jch+1, jch+1 ), &
                                    ldt, c, s )
                          if( ilq )call stdlib_I64_zrot( n, q( 1_ilp64, jch ), 1_ilp64, q( 1_ilp64, jch+1 ), 1_ilp64,c, conjg(&
                                     s ) )
                          if( ilazr2 )h( jch, jch-1 ) = h( jch, jch-1 )*c
                          ilazr2 = .false.
                          if( abs1( t( jch+1, jch+1 ) )>=btol ) then
                       ...
                       do jch = j, ilast - 1
                          ctemp = t( jch, jch+1 )
                          call stdlib_I64_zlartg( ctemp, t( jch+1, jch+1 ), c, s,t( jch, jch+1 ) )
                                    
                          t( jch+1, jch+1 ) = czero
                          if( jch<ilastm-1 )call stdlib_I64_zrot( ilastm-jch-1, t( jch, jch+2 ), ldt,&
                                    t( jch+1, jch+2 ), ldt, c, s )
                          call stdlib_I64_zrot( ilastm-jch+2, h( jch, jch-1 ), ldh,h( jch+1, jch-1 ), &
                                    ldh, c, s )
                          if( ilq )call stdlib_I64_zrot( n, q( 1_ilp64, jch ), 1_ilp64, q( 1_ilp64, jch+1 ), 1_ilp64,c, conjg(&
                                     s ) )
                          ctemp = h( jch+1, jch )
                          call stdlib_I64_zlartg( ctemp, h( jch+1, jch-1 ), c, s,h( jch+1, jch ) )
                                    
                          h( jch+1, jch-1 ) = czero
                          call stdlib_I64_zrot( jch+1-ifrstm, h( ifrstm, jch ), 1_ilp64,h( ifrstm, jch-1 ), &
                                    1_ilp64, c, s )
                          call stdlib_I64_zrot( jch-ifrstm, t( ifrstm, jch ), 1_ilp64,t( ifrstm, jch-1 ), 1_ilp64,&
                                     c, s )
                          if( ilz )call stdlib_I64_zrot( n, z( 1_ilp64, jch ), 1_ilp64, z( 1_ilp64, jch-1 ), 1_ilp64,c, s )
              ...
              50 continue
              ctemp = h( ilast, ilast )
              call stdlib_I64_zlartg( ctemp, h( ilast, ilast-1 ), c, s,h( ilast, ilast ) )
              h( ilast, ilast-1 ) = czero
              call stdlib_I64_zrot( ilast-ifrstm, h( ifrstm, ilast ), 1_ilp64,h( ifrstm, ilast-1 ), 1_ilp64, c, s &
                        )
              call stdlib_I64_zrot( ilast-ifrstm, t( ifrstm, ilast ), 1_ilp64,t( ifrstm, ilast-1 ), 1_ilp64, c, s &
                        )
              if( ilz )call stdlib_I64_zrot( n, z( 1_ilp64, ilast ), 1_ilp64, z( 1_ilp64, ilast-1 ), 1_ilp64, c, s )
              ! h(ilast,ilast-1)=0 -- standardize b, set alpha and beta
              60 continue
              absb = abs( t( ilast, ilast ) )
              if( absb>safmin ) then
                 ...
                 t( ilast, ilast ) = absb
                 if( ilschr ) then
                    call stdlib_I64_zscal( ilast-ifrstm, signbc, t( ifrstm, ilast ), 1_ilp64 )
                    call stdlib_I64_zscal( ilast+1-ifrstm, signbc, h( ifrstm, ilast ),1_ilp64 )
                 else
                    call stdlib_I64_zscal( 1_ilp64, signbc, h( ilast, ilast ), 1_ilp64 )
                 end if
                 if( ilz )call stdlib_I64_zscal( n, signbc, z( 1_ilp64, ilast ), 1_ilp64 )
              else
                 t( ilast, ilast ) = czero
              end if
              alpha( ilast ) = h( ilast, ilast )
              ...
              if( ( iiter / 10_ilp64 )*10_ilp64/=iiter ) then
                 ...
                 if( ctemp/=zero ) then
                    ...
                    shift = shift - ctemp*stdlib_I64_zladiv( ctemp, ( x+y ) )
                 end if
              else
                 ! exceptional shift.  chosen for no particularly good reason.
                 if( ( iiter / 20_ilp64 )*20_ilp64==iiter .and.bscale*abs1(t( ilast, ilast ))>safmin ) &
              ...
              ! initial q
              ctemp2 = ascale*h( istart+1, istart )
              call stdlib_I64_zlartg( ctemp, ctemp2, c, s, ctemp3 )
              ! sweep
              loop_150: do j = istart, ilast - 1
                 if( j>istart ) then
                    ctemp = h( j, j-1 )
                    call stdlib_I64_zlartg( ctemp, h( j+1, j-1 ), c, s, h( j, j-1 ) )
                    h( j+1, j-1 ) = czero
                 end if
                 do jc = j, ilastm
                    ctemp = c*h( j, jc ) + s*h( j+1, jc )
                 ...
                 ctemp = t( j+1, j+1 )
                 call stdlib_I64_zlartg( ctemp, t( j+1, j ), c, s, t( j+1, j+1 ) )
                 t( j+1, j ) = czero
                 do jr = ifrstm, min( j+2, ilast )
                    ctemp = c*h( jr, j+1 ) + s*h( jr, j )
                    h( jr, j ) = -conjg( s )*h( jr, j+1 ) + c*h( jr, j )
     ...
     !! SGGBAL.
        ...
        ! =====================================================================
           ...
           if( .not.stdlib_lsame( job, 'N' ) .and. .not.stdlib_lsame( job, 'P' ) &
              ...
              info = -4_ilp64
           else if( n==0_ilp64 .and. ihi==0_ilp64 .and. ilo/=1_ilp64 ) then
              info = -4_ilp64
           else if( n>0_ilp64 .and. ( ihi<ilo .or. ihi>max( 1_ilp64, n ) ) )then
              info = -5_ilp64
           else if( n==0_ilp64 .and. ilo==1_ilp64 .and. ihi/=0_ilp64 ) then
              info = -5_ilp64
           else if( m<0_ilp64 ) then
              info = -8_ilp64
           else if( ldv<max( 1_ilp64, n ) ) then
              info = -10_ilp64
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SGGBAK', -info )
              return
           end if
           ! quick return if possible
           if( n==0 )return
           ...
           if( stdlib_lsame( job, 'S' ) .or. stdlib_lsame( job, 'B' ) ) then
              ...
              if( rightv ) then
                 do i = ilo, ihi
                    call stdlib_I64_sscal( m, rscale( i ), v( i, 1_ilp64 ), ldv )
                 end do
              end if
              ! backward transformation on left eigenvectors
              if( leftv ) then
                 do i = ilo, ihi
                    call stdlib_I64_sscal( m, lscale( i ), v( i, 1_ilp64 ), ldv )
                 end do
              end if
           end if
           ! backward permutation
           ...
           if( stdlib_lsame( job, 'P' ) .or. stdlib_lsame( job, 'B' ) ) then
              ...
              if( rightv ) then
                 ...
                 loop_40: do i = ilo - 1, 1, -1
                    k = rscale( i )
                    if( k==i )cycle loop_40
                    call stdlib_I64_sswap( m, v( i, 1_ilp64 ), ldv, v( k, 1_ilp64 ), ldv )
                 end do loop_40
                 50 continue
                 if( ihi==n )go to 70
                 loop_60: do i = ihi + 1, n
     ...
     !! DGGBAL.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DGGBAK', -info )
              return
           end if
           ! quick return if possible
           if( n==0 )return
           ...
           if( stdlib_lsame( job, 'S' ) .or. stdlib_lsame( job, 'B' ) ) then
              ...
              if( rightv ) then
                 do i = ilo, ihi
                    call stdlib_I64_dscal( m, rscale( i ), v( i, 1_ilp64 ), ldv )
                 end do
              end if
              ! backward transformation on left eigenvectors
              if( leftv ) then
                 do i = ilo, ihi
                    call stdlib_I64_dscal( m, lscale( i ), v( i, 1_ilp64 ), ldv )
                 end do
              end if
           end if
           ! backward permutation
           ...
           if( stdlib_lsame( job, 'P' ) .or. stdlib_lsame( job, 'B' ) ) then
              ...
              if( rightv ) then
                 if( ilo==1 )go to 50
                 loop_40: do i = ilo - 1, 1, -1
                    k = int(rscale( i ),KIND=ilp64)
                    if( k==i )cycle loop_40
                    call stdlib_I64_dswap( m, v( i, 1_ilp64 ), ldv, v( k, 1_ilp64 ), ldv )
                 end do loop_40
                 50 continue
                 if( ihi==n )go to 70
                 loop_60: do i = ihi + 1, n
     ...
     !! CGGBAL.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CGGBAK', -info )
              return
           end if
           ! quick return if possible
           if( n==0 )return
           ...
           if( stdlib_lsame( job, 'S' ) .or. stdlib_lsame( job, 'B' ) ) then
              ...
              if( rightv ) then
                 do i = ilo, ihi
                    call stdlib_I64_csscal( m, rscale( i ), v( i, 1_ilp64 ), ldv )
                 end do
              end if
              ! backward transformation on left eigenvectors
              if( leftv ) then
                 do i = ilo, ihi
                    call stdlib_I64_csscal( m, lscale( i ), v( i, 1_ilp64 ), ldv )
                 end do
              end if
           end if
           ! backward permutation
           ...
           if( stdlib_lsame( job, 'P' ) .or. stdlib_lsame( job, 'B' ) ) then
              ...
              if( rightv ) then
                 ...
                 loop_40: do i = ilo - 1, 1, -1
                    k = rscale( i )
                    if( k==i )cycle loop_40
                    call stdlib_I64_cswap( m, v( i, 1_ilp64 ), ldv, v( k, 1_ilp64 ), ldv )
                 end do loop_40
                 50 continue
                 if( ihi==n )go to 70
                 loop_60: do i = ihi + 1, n
     ...
     !! ZGGBAL.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZGGBAK', -info )
              return
           end if
           ! quick return if possible
           if( n==0 )return
           ...
           if( stdlib_lsame( job, 'S' ) .or. stdlib_lsame( job, 'B' ) ) then
              ...
              if( rightv ) then
                 do i = ilo, ihi
                    call stdlib_I64_zdscal( m, rscale( i ), v( i, 1_ilp64 ), ldv )
                 end do
              end if
              ! backward transformation on left eigenvectors
              if( leftv ) then
                 do i = ilo, ihi
                    call stdlib_I64_zdscal( m, lscale( i ), v( i, 1_ilp64 ), ldv )
                 end do
              end if
           end if
           ! backward permutation
           ...
           if( stdlib_lsame( job, 'P' ) .or. stdlib_lsame( job, 'B' ) ) then
              ...
              if( rightv ) then
                 ...
                 loop_40: do i = ilo - 1, 1, -1
                    k = int(rscale( i ),KIND=ilp64)
                    if( k==i )cycle loop_40
                    call stdlib_I64_zswap( m, v( i, 1_ilp64 ), ldv, v( k, 1_ilp64 ), ldv )
                 end do loop_40
                 50 continue
                 if( ihi==n )go to 70
                 loop_60: do i = ihi + 1, n
