  ...
  contains
     ...
     ldu1, u2, ldu2, v1t, ldv1t,v2t, ldv2t, b11d, b11e, b12d, b12e, b21d, b21e,b22d, b22e, work, &
        ...
        ! ===================================================================
           ...
           logical(lk) :: colmajor, lquery, restart11, restart12, restart21, restart22, wantu1, &
                     wantu2, wantv1t, wantv2t
           integer(ilp64) :: i, imin, imax, iter, iu1cs, iu1sn, iu2cs, iu2sn, iv1tcs, iv1tsn, &
                     iv2tcs, iv2tsn, j, lworkmin, lworkopt, maxit, mini
           real(sp) :: b11bulge, b12bulge, b21bulge, b22bulge, dummy, eps, mu, nu, r, sigma11, &
                     sigma21, temp, thetamax, thetamin, thresh, tol, tolmul, unfl, x1, x2, y1, y2
           ! Intrinsic Functions 
           ...
           if( m < 0_ilp64 ) then
              info = -6_ilp64
           else if( p < 0_ilp64 .or. p > m ) then
              info = -7_ilp64
           else if( q < 0_ilp64 .or. q > m ) then
              info = -8_ilp64
           else if( q > p .or. q > m-p .or. q > m-q ) then
              info = -8_ilp64
           else if( wantu1 .and. ldu1 < p ) then
           ...
           ! quick return if q = 0
           if( info == 0_ilp64 .and. q == 0_ilp64 ) then
              lworkmin = 1_ilp64
              work(1_ilp64) = lworkmin
              return
           end if
           ! compute workspace
           if( info == 0_ilp64 ) then
              ...
              iv2tcs = iv1tsn + q
              iv2tsn = iv2tcs + q
              lworkopt = iv2tsn + q - 1_ilp64
              lworkmin = lworkopt
              work(1_ilp64) = lworkopt
              if( lwork < lworkmin .and. .not. lquery ) then
                 info = -28_ilp64
              end if
           end if
           if( info /= 0_ilp64 ) then
              call stdlib_I64_xerbla( 'SBBCSD', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           do while( imax > 1 )
              ...
              if( iter > maxit ) then
                 info = 0_ilp64
                 do i = 1, q
                    if( phi(i) /= zero )info = info + 1_ilp64
                 end do
                 return
              end if
              iter = iter + imax - imin
              ...
              if( thetamax > piover2 - thresh ) then
                 ...
                 ! compute shifts for b11 and b21 and use the lesser
                 call stdlib_I64_slas2( b11d(imax-1), b11e(imax-1), b11d(imax), sigma11,dummy )
                           
                 call stdlib_I64_slas2( b21d(imax-1), b21e(imax-1), b21d(imax), sigma21,dummy )
                           
                 if( sigma11 <= sigma21 ) then
                    mu = sigma11
                    nu = sqrt( one - mu**2_ilp64 )
                    if( mu < thresh ) then
                       mu = zero
                       nu = one
                    end if
              ...
              ! rotate to produce bulges in b11 and b21
              if( mu <= nu ) then
                 call stdlib_I64_slartgs( b11d(imin), b11e(imin), mu,work(iv1tcs+imin-1), work(iv1tsn+&
                           imin-1) )
              else
                 call stdlib_I64_slartgs( b21d(imin), b21e(imin), nu,work(iv1tcs+imin-1), work(iv1tsn+&
                           imin-1) )
              end if
              temp = work(iv1tcs+imin-1)*b11d(imin) +work(iv1tsn+imin-1)*b11e(imin)
              b11e(imin) = work(iv1tcs+imin-1)*b11e(imin) -work(iv1tsn+imin-1)*b11d(imin)
              ...
              b21d(imin+1) = work(iv1tcs+imin-1)*b21d(imin+1)
              ! compute theta(imin)
              theta( imin ) = atan2( sqrt( b21d(imin)**2_ilp64+b21bulge**2_ilp64 ),sqrt( b11d(imin)**2_ilp64+&
                        b11bulge**2_ilp64 ) )
              ! chase the bulges in b11(imin+1,imin) and b21(imin+1,imin)
              if( b11d(imin)**2_ilp64+b11bulge**2_ilp64 > thresh**2_ilp64 ) then
                 call stdlib_I64_slartgp( b11bulge, b11d(imin), work(iu1sn+imin-1),work(iu1cs+imin-1),&
                            r )
              else if( mu <= nu ) then
                 call stdlib_I64_slartgs( b11e( imin ), b11d( imin + 1_ilp64 ), mu,work(iu1cs+imin-1), work(&
                           iu1sn+imin-1) )
              else
                 call stdlib_I64_slartgs( b12d( imin ), b12e( imin ), nu,work(iu1cs+imin-1), work(&
                           iu1sn+imin-1) )
              end if
              if( b21d(imin)**2_ilp64+b21bulge**2_ilp64 > thresh**2_ilp64 ) then
                 call stdlib_I64_slartgp( b21bulge, b21d(imin), work(iu2sn+imin-1),work(iu2cs+imin-1),&
                            r )
              else if( nu < mu ) then
                 call stdlib_I64_slartgs( b21e( imin ), b21d( imin + 1_ilp64 ), nu,work(iu2cs+imin-1), work(&
                           iu2sn+imin-1) )
              else
                 call stdlib_I64_slartgs( b22d(imin), b22e(imin), mu,work(iu2cs+imin-1), work(iu2sn+&
                           imin-1) )
              end if
              work(iu2cs+imin-1) = -work(iu2cs+imin-1)
              work(iu2sn+imin-1) = -work(iu2sn+imin-1)
              ...
              do i = imin+1, imax-1
                 ...
                 y1 = sin(theta(i-1))*b12d(i-1) + cos(theta(i-1))*b22d(i-1)
                 y2 = sin(theta(i-1))*b12bulge + cos(theta(i-1))*b22bulge
                 phi(i-1) = atan2( sqrt(x1**2_ilp64+x2**2_ilp64), sqrt(y1**2_ilp64+y2**2_ilp64) )
                 ! determine if there are bulges to chase or if a new direct
                 ! summand has been reached
                 restart11 = b11e(i-1)**2_ilp64 + b11bulge**2_ilp64 <= thresh**2_ilp64
                 restart21 = b21e(i-1)**2_ilp64 + b21bulge**2_ilp64 <= thresh**2_ilp64
                 restart12 = b12d(i-1)**2_ilp64 + b12bulge**2_ilp64 <= thresh**2_ilp64
                 restart22 = b22d(i-1)**2_ilp64 + b22bulge**2_ilp64 <= thresh**2_ilp64
                 ! if possible, chase bulges from b11(i-1,i+1), b12(i-1,i),
                 ! b21(i-1,i+1), and b22(i-1,i). if necessary, restart bulge-
                 ! chasing by applying the original shift again.
                 if( .not. restart11 .and. .not. restart21 ) then
                    call stdlib_I64_slartgp( x2, x1, work(iv1tsn+i-1), work(iv1tcs+i-1),r )
                 else if( .not. restart11 .and. restart21 ) then
                    call stdlib_I64_slartgp( b11bulge, b11e(i-1), work(iv1tsn+i-1),work(iv1tcs+i-1), &
                              r )
                 else if( restart11 .and. .not. restart21 ) then
                    call stdlib_I64_slartgp( b21bulge, b21e(i-1), work(iv1tsn+i-1),work(iv1tcs+i-1), &
                              r )
                 else if( mu <= nu ) then
                    call stdlib_I64_slartgs( b11d(i), b11e(i), mu, work(iv1tcs+i-1),work(iv1tsn+i-1) )
                              
                 else
                    call stdlib_I64_slartgs( b21d(i), b21e(i), nu, work(iv1tcs+i-1),work(iv1tsn+i-1) )
                              
                 end if
                 work(iv1tcs+i-1) = -work(iv1tcs+i-1)
                 work(iv1tsn+i-1) = -work(iv1tsn+i-1)
                 if( .not. restart12 .and. .not. restart22 ) then
                    call stdlib_I64_slartgp( y2, y1, work(iv2tsn+i-1-1),work(iv2tcs+i-1-1), r )
                              
                 else if( .not. restart12 .and. restart22 ) then
                    call stdlib_I64_slartgp( b12bulge, b12d(i-1), work(iv2tsn+i-1-1),work(iv2tcs+i-1-&
                              1_ilp64), r )
                 else if( restart12 .and. .not. restart22 ) then
                    call stdlib_I64_slartgp( b22bulge, b22d(i-1), work(iv2tsn+i-1-1),work(iv2tcs+i-1-&
                              1_ilp64), r )
                 else if( nu < mu ) then
                    call stdlib_I64_slartgs( b12e(i-1), b12d(i), nu, work(iv2tcs+i-1-1),work(iv2tsn+i-&
                              1_ilp64-1) )
                 else
                    call stdlib_I64_slartgs( b22e(i-1), b22d(i), mu, work(iv2tcs+i-1-1),work(iv2tsn+i-&
                              1_ilp64-1) )
                 end if
                 temp = work(iv1tcs+i-1)*b11d(i) + work(iv1tsn+i-1)*b11e(i)
                 b11e(i) = work(iv1tcs+i-1)*b11e(i) -work(iv1tsn+i-1)*b11d(i)
                 ...
                 y1 = cos(phi(i-1))*b21d(i) + sin(phi(i-1))*b22e(i-1)
                 y2 = cos(phi(i-1))*b21bulge + sin(phi(i-1))*b22bulge
                 theta(i) = atan2( sqrt(y1**2_ilp64+y2**2_ilp64), sqrt(x1**2_ilp64+x2**2_ilp64) )
                 ! determine if there are bulges to chase or if a new direct
                 ! summand has been reached
                 restart11 =   b11d(i)**2_ilp64 + b11bulge**2_ilp64 <= thresh**2_ilp64
                 restart12 = b12e(i-1)**2_ilp64 + b12bulge**2_ilp64 <= thresh**2_ilp64
                 restart21 =   b21d(i)**2_ilp64 + b21bulge**2_ilp64 <= thresh**2_ilp64
                 restart22 = b22e(i-1)**2_ilp64 + b22bulge**2_ilp64 <= thresh**2_ilp64
                 ! if possible, chase bulges from b11(i+1,i), b12(i+1,i-1),
                 ! b21(i+1,i), and b22(i+1,i-1). if necessary, restart bulge-
                 ! chasing by applying the original shift again.
                 if( .not. restart11 .and. .not. restart12 ) then
                    call stdlib_I64_slartgp( x2, x1, work(iu1sn+i-1), work(iu1cs+i-1),r )
                 else if( .not. restart11 .and. restart12 ) then
                    call stdlib_I64_slartgp( b11bulge, b11d(i), work(iu1sn+i-1),work(iu1cs+i-1), r )
                              
                 else if( restart11 .and. .not. restart12 ) then
                    call stdlib_I64_slartgp( b12bulge, b12e(i-1), work(iu1sn+i-1),work(iu1cs+i-1), r )
                              
                 else if( mu <= nu ) then
                    call stdlib_I64_slartgs( b11e(i), b11d(i+1), mu, work(iu1cs+i-1),work(iu1sn+i-1) )
                              
                 else
                    call stdlib_I64_slartgs( b12d(i), b12e(i), nu, work(iu1cs+i-1),work(iu1sn+i-1) )
                              
                 end if
                 if( .not. restart21 .and. .not. restart22 ) then
                    call stdlib_I64_slartgp( y2, y1, work(iu2sn+i-1), work(iu2cs+i-1),r )
                 else if( .not. restart21 .and. restart22 ) then
                    call stdlib_I64_slartgp( b21bulge, b21d(i), work(iu2sn+i-1),work(iu2cs+i-1), r )
                              
                 else if( restart21 .and. .not. restart22 ) then
                    call stdlib_I64_slartgp( b22bulge, b22e(i-1), work(iu2sn+i-1),work(iu2cs+i-1), r )
                              
                 else if( nu < mu ) then
                    call stdlib_I64_slartgs( b21e(i), b21e(i+1), nu, work(iu2cs+i-1),work(iu2sn+i-1) )
                              
                 else
                    call stdlib_I64_slartgs( b22d(i), b22e(i), mu, work(iu2cs+i-1),work(iu2sn+i-1) )
              ...
              y1 = sin(theta(imax-1))*b12d(imax-1) +cos(theta(imax-1))*b22d(imax-1)
              y2 = sin(theta(imax-1))*b12bulge + cos(theta(imax-1))*b22bulge
              phi(imax-1) = atan2( abs(x1), sqrt(y1**2_ilp64+y2**2_ilp64) )
              ! chase bulges from b12(imax-1,imax) and b22(imax-1,imax)
              restart12 = b12d(imax-1)**2_ilp64 + b12bulge**2_ilp64 <= thresh**2_ilp64
              restart22 = b22d(imax-1)**2_ilp64 + b22bulge**2_ilp64 <= thresh**2_ilp64
              if( .not. restart12 .and. .not. restart22 ) then
                 call stdlib_I64_slartgp( y2, y1, work(iv2tsn+imax-1-1),work(iv2tcs+imax-1-1), r )
                           
              else if( .not. restart12 .and. restart22 ) then
                 call stdlib_I64_slartgp( b12bulge, b12d(imax-1), work(iv2tsn+imax-1-1),work(iv2tcs+&
                           imax-1-1), r )
              else if( restart12 .and. .not. restart22 ) then
                 call stdlib_I64_slartgp( b22bulge, b22d(imax-1), work(iv2tsn+imax-1-1),work(iv2tcs+&
                           imax-1-1), r )
              else if( nu < mu ) then
                 call stdlib_I64_slartgs( b12e(imax-1), b12d(imax), nu,work(iv2tcs+imax-1-1), work(&
                           iv2tsn+imax-1-1) )
              else
                 call stdlib_I64_slartgs( b22e(imax-1), b22d(imax), mu,work(iv2tcs+imax-1-1), work(&
                           iv2tsn+imax-1-1) )
              end if
              temp = work(iv2tcs+imax-1-1)*b12e(imax-1) +work(iv2tsn+imax-1-1)*b12d(imax)
              b12d(imax) = work(iv2tcs+imax-1-1)*b12d(imax) -work(iv2tsn+imax-1-1)*b12e(imax-1)
              ...
              if( wantu1 ) then
                 if( colmajor ) then
                    call stdlib_I64_slasr( 'R', 'V', 'F', p, imax-imin+1,work(iu1cs+imin-1), work(&
                              iu1sn+imin-1),u1(1_ilp64,imin), ldu1 )
                 else
                    call stdlib_I64_slasr( 'L', 'V', 'F', imax-imin+1, p,work(iu1cs+imin-1), work(&
                              iu1sn+imin-1),u1(imin,1_ilp64), ldu1 )
                 end if
              end if
              if( wantu2 ) then
                 if( colmajor ) then
                    call stdlib_I64_slasr( 'R', 'V', 'F', m-p, imax-imin+1,work(iu2cs+imin-1), work(&
                              iu2sn+imin-1),u2(1_ilp64,imin), ldu2 )
                 else
                    call stdlib_I64_slasr( 'L', 'V', 'F', imax-imin+1, m-p,work(iu2cs+imin-1), work(&
                              iu2sn+imin-1),u2(imin,1_ilp64), ldu2 )
                 end if
              end if
              if( wantv1t ) then
                 if( colmajor ) then
                    call stdlib_I64_slasr( 'L', 'V', 'F', imax-imin+1, q,work(iv1tcs+imin-1), work(&
                              iv1tsn+imin-1),v1t(imin,1_ilp64), ldv1t )
                 else
                    call stdlib_I64_slasr( 'R', 'V', 'F', q, imax-imin+1,work(iv1tcs+imin-1), work(&
                              iv1tsn+imin-1),v1t(1_ilp64,imin), ldv1t )
                 end if
              end if
              if( wantv2t ) then
                 if( colmajor ) then
                    call stdlib_I64_slasr( 'L', 'V', 'F', imax-imin+1, m-q,work(iv2tcs+imin-1), work(&
                              iv2tsn+imin-1),v2t(imin,1_ilp64), ldv2t )
                 else
                    call stdlib_I64_slasr( 'R', 'V', 'F', m-q, imax-imin+1,work(iv2tcs+imin-1), work(&
                              iv2tsn+imin-1),v2t(1_ilp64,imin), ldv2t )
                 end if
              end if
              ! fix signs on b11(imax-1,imax) and b21(imax-1,imax)
              if( b11e(imax-1)+b21e(imax-1) > 0_ilp64 ) then
                 b11d(imax) = -b11d(imax)
                 b21d(imax) = -b21d(imax)
                 if( wantv1t ) then
                    if( colmajor ) then
                       call stdlib_I64_sscal( q, negone, v1t(imax,1_ilp64), ldv1t )
                    else
                       call stdlib_I64_sscal( q, negone, v1t(1_ilp64,imax), 1_ilp64 )
                    end if
                 end if
              end if
              ! compute theta(imax)
              ...
              ! fix signs on b11(imax,imax), b12(imax,imax-1), b21(imax,imax),
              ! and b22(imax,imax-1)
              if( b11d(imax)+b12e(imax-1) < 0_ilp64 ) then
                 b12d(imax) = -b12d(imax)
                 if( wantu1 ) then
                    if( colmajor ) then
                       call stdlib_I64_sscal( p, negone, u1(1_ilp64,imax), 1_ilp64 )
                    else
                       call stdlib_I64_sscal( p, negone, u1(imax,1_ilp64), ldu1 )
                    end if
                 end if
              end if
              if( b21d(imax)+b22e(imax-1) > 0_ilp64 ) then
                 b22d(imax) = -b22d(imax)
                 if( wantu2 ) then
                    if( colmajor ) then
                       call stdlib_I64_sscal( m-p, negone, u2(1_ilp64,imax), 1_ilp64 )
                    else
                       call stdlib_I64_sscal( m-p, negone, u2(imax,1_ilp64), ldu2 )
                    end if
                 end if
              end if
              ! fix signs on b12(imax,imax) and b22(imax,imax)
              if( b12d(imax)+b22d(imax) < 0_ilp64 ) then
                 if( wantv2t ) then
                    if( colmajor ) then
                       call stdlib_I64_sscal( m-q, negone, v2t(imax,1_ilp64), ldv2t )
                    else
                       call stdlib_I64_sscal( m-q, negone, v2t(1_ilp64,imax), 1_ilp64 )
                    end if
                 end if
              end if
              ! test for negligible sines or cosines
              ...
              if( imin > imax - 1_ilp64 )imin = imax - 1_ilp64
              if (imin > 1_ilp64) then
                 do while (phi(imin-1) /= zero)
                     imin = imin - 1_ilp64
                     if (imin <= 1) exit
           ...
           do i = 1, q
              ...
              if( mini /= i ) then
                 ...
                 theta(i) = thetamin
                 if( colmajor ) then
                    if( wantu1 )call stdlib_I64_sswap( p, u1(1_ilp64,i), 1_ilp64, u1(1_ilp64,mini), 1_ilp64 )
                    if( wantu2 )call stdlib_I64_sswap( m-p, u2(1_ilp64,i), 1_ilp64, u2(1_ilp64,mini), 1_ilp64 )
                    if( wantv1t )call stdlib_I64_sswap( q, v1t(i,1_ilp64), ldv1t, v1t(mini,1_ilp64), ldv1t )
                              
                    if( wantv2t )call stdlib_I64_sswap( m-q, v2t(i,1_ilp64), ldv2t, v2t(mini,1_ilp64),ldv2t )
                              
                 else
                    if( wantu1 )call stdlib_I64_sswap( p, u1(i,1_ilp64), ldu1, u1(mini,1_ilp64), ldu1 )
                    if( wantu2 )call stdlib_I64_sswap( m-p, u2(i,1_ilp64), ldu2, u2(mini,1_ilp64), ldu2 )
                    if( wantv1t )call stdlib_I64_sswap( q, v1t(1_ilp64,i), 1_ilp64, v1t(1_ilp64,mini), 1_ilp64 )
                    if( wantv2t )call stdlib_I64_sswap( m-q, v2t(1_ilp64,i), 1_ilp64, v2t(1_ilp64,mini), 1_ilp64 )
                 end if
              end if
           end do
           return
     ...
     ldu1, u2, ldu2, v1t, ldv1t,v2t, ldv2t, b11d, b11e, b12d, b12e, b21d, b21e,b22d, b22e, work, &
        ...
        ! ===================================================================
           ...
           if( info /= 0_ilp64 ) then
              call stdlib_I64_xerbla( 'DBBCSD', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           do while( imax > 1 )
              ...
              if( thetamax > piover2 - thresh ) then
                 ...
                 ! compute shifts for b11 and b21 and use the lesser
                 call stdlib_I64_dlas2( b11d(imax-1), b11e(imax-1), b11d(imax), sigma11,dummy )
                           
                 call stdlib_I64_dlas2( b21d(imax-1), b21e(imax-1), b21d(imax), sigma21,dummy )
              ...
              ! rotate to produce bulges in b11 and b21
              if( mu <= nu ) then
                 call stdlib_I64_dlartgs( b11d(imin), b11e(imin), mu,work(iv1tcs+imin-1), work(iv1tsn+&
                           imin-1) )
              else
                 call stdlib_I64_dlartgs( b21d(imin), b21e(imin), nu,work(iv1tcs+imin-1), work(iv1tsn+&
                           imin-1) )
              end if
              temp = work(iv1tcs+imin-1)*b11d(imin) +work(iv1tsn+imin-1)*b11e(imin)
              b11e(imin) = work(iv1tcs+imin-1)*b11e(imin) -work(iv1tsn+imin-1)*b11d(imin)
              ...
              ! chase the bulges in b11(imin+1,imin) and b21(imin+1,imin)
              if( b11d(imin)**2_ilp64+b11bulge**2_ilp64 > thresh**2_ilp64 ) then
                 call stdlib_I64_dlartgp( b11bulge, b11d(imin), work(iu1sn+imin-1),work(iu1cs+imin-1),&
                            r )
              else if( mu <= nu ) then
                 call stdlib_I64_dlartgs( b11e( imin ), b11d( imin + 1_ilp64 ), mu,work(iu1cs+imin-1), work(&
                           iu1sn+imin-1) )
              else
                 call stdlib_I64_dlartgs( b12d( imin ), b12e( imin ), nu,work(iu1cs+imin-1), work(&
                           iu1sn+imin-1) )
              end if
              if( b21d(imin)**2_ilp64+b21bulge**2_ilp64 > thresh**2_ilp64 ) then
                 call stdlib_I64_dlartgp( b21bulge, b21d(imin), work(iu2sn+imin-1),work(iu2cs+imin-1),&
                            r )
              else if( nu < mu ) then
                 call stdlib_I64_dlartgs( b21e( imin ), b21d( imin + 1_ilp64 ), nu,work(iu2cs+imin-1), work(&
                           iu2sn+imin-1) )
              else
                 call stdlib_I64_dlartgs( b22d(imin), b22e(imin), mu,work(iu2cs+imin-1), work(iu2sn+&
                           imin-1) )
              end if
              work(iu2cs+imin-1) = -work(iu2cs+imin-1)
              work(iu2sn+imin-1) = -work(iu2sn+imin-1)
              ...
              do i = imin+1, imax-1
                 ...
                 ! chasing by applying the original shift again.
                 if( .not. restart11 .and. .not. restart21 ) then
                    call stdlib_I64_dlartgp( x2, x1, work(iv1tsn+i-1), work(iv1tcs+i-1),r )
                 else if( .not. restart11 .and. restart21 ) then
                    call stdlib_I64_dlartgp( b11bulge, b11e(i-1), work(iv1tsn+i-1),work(iv1tcs+i-1), &
                              r )
                 else if( restart11 .and. .not. restart21 ) then
                    call stdlib_I64_dlartgp( b21bulge, b21e(i-1), work(iv1tsn+i-1),work(iv1tcs+i-1), &
                              r )
                 else if( mu <= nu ) then
                    call stdlib_I64_dlartgs( b11d(i), b11e(i), mu, work(iv1tcs+i-1),work(iv1tsn+i-1) )
                              
                 else
                    call stdlib_I64_dlartgs( b21d(i), b21e(i), nu, work(iv1tcs+i-1),work(iv1tsn+i-1) )
                              
                 end if
                 work(iv1tcs+i-1) = -work(iv1tcs+i-1)
                 work(iv1tsn+i-1) = -work(iv1tsn+i-1)
                 if( .not. restart12 .and. .not. restart22 ) then
                    call stdlib_I64_dlartgp( y2, y1, work(iv2tsn+i-1-1),work(iv2tcs+i-1-1), r )
                              
                 else if( .not. restart12 .and. restart22 ) then
                    call stdlib_I64_dlartgp( b12bulge, b12d(i-1), work(iv2tsn+i-1-1),work(iv2tcs+i-1-&
                              1_ilp64), r )
                 else if( restart12 .and. .not. restart22 ) then
                    call stdlib_I64_dlartgp( b22bulge, b22d(i-1), work(iv2tsn+i-1-1),work(iv2tcs+i-1-&
                              1_ilp64), r )
                 else if( nu < mu ) then
                    call stdlib_I64_dlartgs( b12e(i-1), b12d(i), nu, work(iv2tcs+i-1-1),work(iv2tsn+i-&
                              1_ilp64-1) )
                 else
                    call stdlib_I64_dlartgs( b22e(i-1), b22d(i), mu, work(iv2tcs+i-1-1),work(iv2tsn+i-&
                              1_ilp64-1) )
                 end if
                 temp = work(iv1tcs+i-1)*b11d(i) + work(iv1tsn+i-1)*b11e(i)
                 b11e(i) = work(iv1tcs+i-1)*b11e(i) -work(iv1tsn+i-1)*b11d(i)
                 ...
                 ! chasing by applying the original shift again.
                 if( .not. restart11 .and. .not. restart12 ) then
                    call stdlib_I64_dlartgp( x2, x1, work(iu1sn+i-1), work(iu1cs+i-1),r )
                 else if( .not. restart11 .and. restart12 ) then
                    call stdlib_I64_dlartgp( b11bulge, b11d(i), work(iu1sn+i-1),work(iu1cs+i-1), r )
                              
                 else if( restart11 .and. .not. restart12 ) then
                    call stdlib_I64_dlartgp( b12bulge, b12e(i-1), work(iu1sn+i-1),work(iu1cs+i-1), r )
                              
                 else if( mu <= nu ) then
                    call stdlib_I64_dlartgs( b11e(i), b11d(i+1), mu, work(iu1cs+i-1),work(iu1sn+i-1) )
                              
                 else
                    call stdlib_I64_dlartgs( b12d(i), b12e(i), nu, work(iu1cs+i-1),work(iu1sn+i-1) )
                              
                 end if
                 if( .not. restart21 .and. .not. restart22 ) then
                    call stdlib_I64_dlartgp( y2, y1, work(iu2sn+i-1), work(iu2cs+i-1),r )
                 else if( .not. restart21 .and. restart22 ) then
                    call stdlib_I64_dlartgp( b21bulge, b21d(i), work(iu2sn+i-1),work(iu2cs+i-1), r )
                              
                 else if( restart21 .and. .not. restart22 ) then
                    call stdlib_I64_dlartgp( b22bulge, b22e(i-1), work(iu2sn+i-1),work(iu2cs+i-1), r )
                              
                 else if( nu < mu ) then
                    call stdlib_I64_dlartgs( b21e(i), b21e(i+1), nu, work(iu2cs+i-1),work(iu2sn+i-1) )
                              
                 else
                    call stdlib_I64_dlartgs( b22d(i), b22e(i), mu, work(iu2cs+i-1),work(iu2sn+i-1) )
              ...
              restart22 = b22d(imax-1)**2_ilp64 + b22bulge**2_ilp64 <= thresh**2_ilp64
              if( .not. restart12 .and. .not. restart22 ) then
                 call stdlib_I64_dlartgp( y2, y1, work(iv2tsn+imax-1-1),work(iv2tcs+imax-1-1), r )
                           
              else if( .not. restart12 .and. restart22 ) then
                 call stdlib_I64_dlartgp( b12bulge, b12d(imax-1), work(iv2tsn+imax-1-1),work(iv2tcs+&
                           imax-1-1), r )
              else if( restart12 .and. .not. restart22 ) then
                 call stdlib_I64_dlartgp( b22bulge, b22d(imax-1), work(iv2tsn+imax-1-1),work(iv2tcs+&
                           imax-1-1), r )
              else if( nu < mu ) then
                 call stdlib_I64_dlartgs( b12e(imax-1), b12d(imax), nu,work(iv2tcs+imax-1-1), work(&
                           iv2tsn+imax-1-1) )
              else
                 call stdlib_I64_dlartgs( b22e(imax-1), b22d(imax), mu,work(iv2tcs+imax-1-1), work(&
                           iv2tsn+imax-1-1) )
              end if
              temp = work(iv2tcs+imax-1-1)*b12e(imax-1) +work(iv2tsn+imax-1-1)*b12d(imax)
              b12d(imax) = work(iv2tcs+imax-1-1)*b12d(imax) -work(iv2tsn+imax-1-1)*b12e(imax-1)
              ...
              if( wantu1 ) then
                 if( colmajor ) then
                    call stdlib_I64_dlasr( 'R', 'V', 'F', p, imax-imin+1,work(iu1cs+imin-1), work(&
                              iu1sn+imin-1),u1(1_ilp64,imin), ldu1 )
                 else
                    call stdlib_I64_dlasr( 'L', 'V', 'F', imax-imin+1, p,work(iu1cs+imin-1), work(&
                              iu1sn+imin-1),u1(imin,1_ilp64), ldu1 )
                 end if
              end if
              if( wantu2 ) then
                 if( colmajor ) then
                    call stdlib_I64_dlasr( 'R', 'V', 'F', m-p, imax-imin+1,work(iu2cs+imin-1), work(&
                              iu2sn+imin-1),u2(1_ilp64,imin), ldu2 )
                 else
                    call stdlib_I64_dlasr( 'L', 'V', 'F', imax-imin+1, m-p,work(iu2cs+imin-1), work(&
                              iu2sn+imin-1),u2(imin,1_ilp64), ldu2 )
                 end if
              end if
              if( wantv1t ) then
                 if( colmajor ) then
                    call stdlib_I64_dlasr( 'L', 'V', 'F', imax-imin+1, q,work(iv1tcs+imin-1), work(&
                              iv1tsn+imin-1),v1t(imin,1_ilp64), ldv1t )
                 else
                    call stdlib_I64_dlasr( 'R', 'V', 'F', q, imax-imin+1,work(iv1tcs+imin-1), work(&
                              iv1tsn+imin-1),v1t(1_ilp64,imin), ldv1t )
                 end if
              end if
              if( wantv2t ) then
                 if( colmajor ) then
                    call stdlib_I64_dlasr( 'L', 'V', 'F', imax-imin+1, m-q,work(iv2tcs+imin-1), work(&
                              iv2tsn+imin-1),v2t(imin,1_ilp64), ldv2t )
                 else
                    call stdlib_I64_dlasr( 'R', 'V', 'F', m-q, imax-imin+1,work(iv2tcs+imin-1), work(&
                              iv2tsn+imin-1),v2t(1_ilp64,imin), ldv2t )
                 end if
              end if
              ! fix signs on b11(imax-1,imax) and b21(imax-1,imax)
              if( b11e(imax-1)+b21e(imax-1) > 0_ilp64 ) then
                 ...
                 if( wantv1t ) then
                    if( colmajor ) then
                       call stdlib_I64_dscal( q, negone, v1t(imax,1_ilp64), ldv1t )
                    else
                       call stdlib_I64_dscal( q, negone, v1t(1_ilp64,imax), 1_ilp64 )
                    end if
                 end if
              end if
              ! compute theta(imax)
              ...
              if( b11d(imax)+b12e(imax-1) < 0_ilp64 ) then
                 ...
                 if( wantu1 ) then
                    if( colmajor ) then
                       call stdlib_I64_dscal( p, negone, u1(1_ilp64,imax), 1_ilp64 )
                    else
                       call stdlib_I64_dscal( p, negone, u1(imax,1_ilp64), ldu1 )
                    end if
                 end if
              end if
              if( b21d(imax)+b22e(imax-1) > 0_ilp64 ) then
                 ...
                 if( wantu2 ) then
                    if( colmajor ) then
                       call stdlib_I64_dscal( m-p, negone, u2(1_ilp64,imax), 1_ilp64 )
                    else
                       call stdlib_I64_dscal( m-p, negone, u2(imax,1_ilp64), ldu2 )
                    end if
                 end if
              end if
              ! fix signs on b12(imax,imax) and b22(imax,imax)
              if( b12d(imax)+b22d(imax) < 0_ilp64 ) then
                 if( wantv2t ) then
                    if( colmajor ) then
                       call stdlib_I64_dscal( m-q, negone, v2t(imax,1_ilp64), ldv2t )
                    else
                       call stdlib_I64_dscal( m-q, negone, v2t(1_ilp64,imax), 1_ilp64 )
                    end if
                 end if
              end if
              ! test for negligible sines or cosines
           ...
           do i = 1, q
              ...
              if( mini /= i ) then
                 ...
                 theta(i) = thetamin
                 if( colmajor ) then
                    if( wantu1 )call stdlib_I64_dswap( p, u1(1_ilp64,i), 1_ilp64, u1(1_ilp64,mini), 1_ilp64 )
                    if( wantu2 )call stdlib_I64_dswap( m-p, u2(1_ilp64,i), 1_ilp64, u2(1_ilp64,mini), 1_ilp64 )
                    if( wantv1t )call stdlib_I64_dswap( q, v1t(i,1_ilp64), ldv1t, v1t(mini,1_ilp64), ldv1t )
                              
                    if( wantv2t )call stdlib_I64_dswap( m-q, v2t(i,1_ilp64), ldv2t, v2t(mini,1_ilp64),ldv2t )
                              
                 else
                    if( wantu1 )call stdlib_I64_dswap( p, u1(i,1_ilp64), ldu1, u1(mini,1_ilp64), ldu1 )
                    if( wantu2 )call stdlib_I64_dswap( m-p, u2(i,1_ilp64), ldu2, u2(mini,1_ilp64), ldu2 )
                    if( wantv1t )call stdlib_I64_dswap( q, v1t(1_ilp64,i), 1_ilp64, v1t(1_ilp64,mini), 1_ilp64 )
                    if( wantv2t )call stdlib_I64_dswap( m-q, v2t(1_ilp64,i), 1_ilp64, v2t(1_ilp64,mini), 1_ilp64 )
                 end if
              end if
           end do
           return
     ...
     !! singular vector matrices.
        ...
        ! ===================================================================
           ...
           if( info == 0_ilp64 ) then
              ...
              iv2tcs = iv1tsn + q
              iv2tsn = iv2tcs + q
              lrworkopt = iv2tsn + q - 1_ilp64
              lrworkmin = lrworkopt
              rwork(1_ilp64) = lrworkopt
              if( lrwork < lrworkmin .and. .not. lquery ) then
                 info = -28_ilp64
              end if
           end if
           if( info /= 0_ilp64 ) then
              call stdlib_I64_xerbla( 'CBBCSD', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           do while( imax > 1 )
              ...
              ! rotate to produce bulges in b11 and b21
              if( mu <= nu ) then
                 call stdlib_I64_slartgs( b11d(imin), b11e(imin), mu,rwork(iv1tcs+imin-1), rwork(&
                           iv1tsn+imin-1) )
              else
                 call stdlib_I64_slartgs( b21d(imin), b21e(imin), nu,rwork(iv1tcs+imin-1), rwork(&
                           iv1tsn+imin-1) )
              end if
              temp = rwork(iv1tcs+imin-1)*b11d(imin) +rwork(iv1tsn+imin-1)*b11e(imin)
              b11e(imin) = rwork(iv1tcs+imin-1)*b11e(imin) -rwork(iv1tsn+imin-1)*b11d(imin)
              ...
              ! chase the bulges in b11(imin+1,imin) and b21(imin+1,imin)
              if( b11d(imin)**2_ilp64+b11bulge**2_ilp64 > thresh**2_ilp64 ) then
                 call stdlib_I64_slartgp( b11bulge, b11d(imin), rwork(iu1sn+imin-1),rwork(iu1cs+imin-&
                           1_ilp64), r )
              else if( mu <= nu ) then
                 call stdlib_I64_slartgs( b11e( imin ), b11d( imin + 1_ilp64 ), mu,rwork(iu1cs+imin-1), &
                           rwork(iu1sn+imin-1) )
              else
                 call stdlib_I64_slartgs( b12d( imin ), b12e( imin ), nu,rwork(iu1cs+imin-1), rwork(&
                           iu1sn+imin-1) )
              end if
              if( b21d(imin)**2_ilp64+b21bulge**2_ilp64 > thresh**2_ilp64 ) then
                 call stdlib_I64_slartgp( b21bulge, b21d(imin), rwork(iu2sn+imin-1),rwork(iu2cs+imin-&
                           1_ilp64), r )
              else if( nu < mu ) then
                 call stdlib_I64_slartgs( b21e( imin ), b21d( imin + 1_ilp64 ), nu,rwork(iu2cs+imin-1), &
                           rwork(iu2sn+imin-1) )
              else
                 call stdlib_I64_slartgs( b22d(imin), b22e(imin), mu,rwork(iu2cs+imin-1), rwork(iu2sn+&
                           imin-1) )
              end if
              rwork(iu2cs+imin-1) = -rwork(iu2cs+imin-1)
              rwork(iu2sn+imin-1) = -rwork(iu2sn+imin-1)
              ...
              do i = imin+1, imax-1
                 ...
                 ! chasing by applying the original shift again.
                 if( .not. restart11 .and. .not. restart21 ) then
                    call stdlib_I64_slartgp( x2, x1, rwork(iv1tsn+i-1),rwork(iv1tcs+i-1), r )
                 else if( .not. restart11 .and. restart21 ) then
                    call stdlib_I64_slartgp( b11bulge, b11e(i-1), rwork(iv1tsn+i-1),rwork(iv1tcs+i-1),&
                               r )
                 else if( restart11 .and. .not. restart21 ) then
                    call stdlib_I64_slartgp( b21bulge, b21e(i-1), rwork(iv1tsn+i-1),rwork(iv1tcs+i-1),&
                               r )
                 else if( mu <= nu ) then
                    call stdlib_I64_slartgs( b11d(i), b11e(i), mu, rwork(iv1tcs+i-1),rwork(iv1tsn+i-1)&
                               )
                 else
                    call stdlib_I64_slartgs( b21d(i), b21e(i), nu, rwork(iv1tcs+i-1),rwork(iv1tsn+i-1)&
                               )
                 end if
                 rwork(iv1tcs+i-1) = -rwork(iv1tcs+i-1)
                 rwork(iv1tsn+i-1) = -rwork(iv1tsn+i-1)
                 if( .not. restart12 .and. .not. restart22 ) then
                    call stdlib_I64_slartgp( y2, y1, rwork(iv2tsn+i-1-1),rwork(iv2tcs+i-1-1), r )
                              
                 else if( .not. restart12 .and. restart22 ) then
                    call stdlib_I64_slartgp( b12bulge, b12d(i-1), rwork(iv2tsn+i-1-1),rwork(iv2tcs+i-&
                              1_ilp64-1), r )
                 else if( restart12 .and. .not. restart22 ) then
                    call stdlib_I64_slartgp( b22bulge, b22d(i-1), rwork(iv2tsn+i-1-1),rwork(iv2tcs+i-&
                              1_ilp64-1), r )
                 else if( nu < mu ) then
                    call stdlib_I64_slartgs( b12e(i-1), b12d(i), nu,rwork(iv2tcs+i-1-1), rwork(iv2tsn+&
                              i-1-1) )
                 else
                    call stdlib_I64_slartgs( b22e(i-1), b22d(i), mu,rwork(iv2tcs+i-1-1), rwork(iv2tsn+&
                              i-1-1) )
                 end if
                 temp = rwork(iv1tcs+i-1)*b11d(i) + rwork(iv1tsn+i-1)*b11e(i)
                 b11e(i) = rwork(iv1tcs+i-1)*b11e(i) -rwork(iv1tsn+i-1)*b11d(i)
                 ...
                 ! chasing by applying the original shift again.
                 if( .not. restart11 .and. .not. restart12 ) then
                    call stdlib_I64_slartgp( x2, x1, rwork(iu1sn+i-1), rwork(iu1cs+i-1),r )
                 else if( .not. restart11 .and. restart12 ) then
                    call stdlib_I64_slartgp( b11bulge, b11d(i), rwork(iu1sn+i-1),rwork(iu1cs+i-1), r )
                              
                 else if( restart11 .and. .not. restart12 ) then
                    call stdlib_I64_slartgp( b12bulge, b12e(i-1), rwork(iu1sn+i-1),rwork(iu1cs+i-1), &
                              r )
                 else if( mu <= nu ) then
                    call stdlib_I64_slartgs( b11e(i), b11d(i+1), mu, rwork(iu1cs+i-1),rwork(iu1sn+i-1)&
                               )
                 else
                    call stdlib_I64_slartgs( b12d(i), b12e(i), nu, rwork(iu1cs+i-1),rwork(iu1sn+i-1) )
                              
                 end if
                 if( .not. restart21 .and. .not. restart22 ) then
                    call stdlib_I64_slartgp( y2, y1, rwork(iu2sn+i-1), rwork(iu2cs+i-1),r )
                 else if( .not. restart21 .and. restart22 ) then
                    call stdlib_I64_slartgp( b21bulge, b21d(i), rwork(iu2sn+i-1),rwork(iu2cs+i-1), r )
                              
                 else if( restart21 .and. .not. restart22 ) then
                    call stdlib_I64_slartgp( b22bulge, b22e(i-1), rwork(iu2sn+i-1),rwork(iu2cs+i-1), &
                              r )
                 else if( nu < mu ) then
                    call stdlib_I64_slartgs( b21e(i), b21e(i+1), nu, rwork(iu2cs+i-1),rwork(iu2sn+i-1)&
                               )
                 else
                    call stdlib_I64_slartgs( b22d(i), b22e(i), mu, rwork(iu2cs+i-1),rwork(iu2sn+i-1) )
              ...
              restart22 = b22d(imax-1)**2_ilp64 + b22bulge**2_ilp64 <= thresh**2_ilp64
              if( .not. restart12 .and. .not. restart22 ) then
                 call stdlib_I64_slartgp( y2, y1, rwork(iv2tsn+imax-1-1),rwork(iv2tcs+imax-1-1), r )
                           
              else if( .not. restart12 .and. restart22 ) then
                 call stdlib_I64_slartgp( b12bulge, b12d(imax-1),rwork(iv2tsn+imax-1-1),rwork(iv2tcs+&
                           imax-1-1), r )
              else if( restart12 .and. .not. restart22 ) then
                 call stdlib_I64_slartgp( b22bulge, b22d(imax-1),rwork(iv2tsn+imax-1-1),rwork(iv2tcs+&
                           imax-1-1), r )
              else if( nu < mu ) then
                 call stdlib_I64_slartgs( b12e(imax-1), b12d(imax), nu,rwork(iv2tcs+imax-1-1),rwork(&
                           iv2tsn+imax-1-1) )
              else
                 call stdlib_I64_slartgs( b22e(imax-1), b22d(imax), mu,rwork(iv2tcs+imax-1-1),rwork(&
                           iv2tsn+imax-1-1) )
              end if
              temp = rwork(iv2tcs+imax-1-1)*b12e(imax-1) +rwork(iv2tsn+imax-1-1)*b12d(imax)
              ...
              if( wantu1 ) then
                 if( colmajor ) then
                    call stdlib_I64_clasr( 'R', 'V', 'F', p, imax-imin+1,rwork(iu1cs+imin-1), rwork(&
                              iu1sn+imin-1),u1(1_ilp64,imin), ldu1 )
                 else
                    call stdlib_I64_clasr( 'L', 'V', 'F', imax-imin+1, p,rwork(iu1cs+imin-1), rwork(&
                              iu1sn+imin-1),u1(imin,1_ilp64), ldu1 )
                 end if
              end if
              if( wantu2 ) then
                 if( colmajor ) then
                    call stdlib_I64_clasr( 'R', 'V', 'F', m-p, imax-imin+1,rwork(iu2cs+imin-1), rwork(&
                              iu2sn+imin-1),u2(1_ilp64,imin), ldu2 )
                 else
                    call stdlib_I64_clasr( 'L', 'V', 'F', imax-imin+1, m-p,rwork(iu2cs+imin-1), rwork(&
                              iu2sn+imin-1),u2(imin,1_ilp64), ldu2 )
                 end if
              end if
              if( wantv1t ) then
                 if( colmajor ) then
                    call stdlib_I64_clasr( 'L', 'V', 'F', imax-imin+1, q,rwork(iv1tcs+imin-1), rwork(&
                              iv1tsn+imin-1),v1t(imin,1_ilp64), ldv1t )
                 else
                    call stdlib_I64_clasr( 'R', 'V', 'F', q, imax-imin+1,rwork(iv1tcs+imin-1), rwork(&
                              iv1tsn+imin-1),v1t(1_ilp64,imin), ldv1t )
                 end if
              end if
              if( wantv2t ) then
                 if( colmajor ) then
                    call stdlib_I64_clasr( 'L', 'V', 'F', imax-imin+1, m-q,rwork(iv2tcs+imin-1), &
                              rwork(iv2tsn+imin-1),v2t(imin,1_ilp64), ldv2t )
                 else
                    call stdlib_I64_clasr( 'R', 'V', 'F', m-q, imax-imin+1,rwork(iv2tcs+imin-1), &
                              rwork(iv2tsn+imin-1),v2t(1_ilp64,imin), ldv2t )
                 end if
              end if
              ! fix signs on b11(imax-1,imax) and b21(imax-1,imax)
              if( b11e(imax-1)+b21e(imax-1) > 0_ilp64 ) then
                 ...
                 if( wantv1t ) then
                    if( colmajor ) then
                       call stdlib_I64_cscal( q, cnegone, v1t(imax,1_ilp64), ldv1t )
                    else
                       call stdlib_I64_cscal( q, cnegone, v1t(1_ilp64,imax), 1_ilp64 )
                    end if
                 end if
              end if
              ! compute theta(imax)
              ...
              if( b11d(imax)+b12e(imax-1) < 0_ilp64 ) then
                 ...
                 if( wantu1 ) then
                    if( colmajor ) then
                       call stdlib_I64_cscal( p, cnegone, u1(1_ilp64,imax), 1_ilp64 )
                    else
                       call stdlib_I64_cscal( p, cnegone, u1(imax,1_ilp64), ldu1 )
                    end if
                 end if
              end if
              if( b21d(imax)+b22e(imax-1) > 0_ilp64 ) then
                 ...
                 if( wantu2 ) then
                    if( colmajor ) then
                       call stdlib_I64_cscal( m-p, cnegone, u2(1_ilp64,imax), 1_ilp64 )
                    else
                       call stdlib_I64_cscal( m-p, cnegone, u2(imax,1_ilp64), ldu2 )
                    end if
                 end if
              end if
              ! fix signs on b12(imax,imax) and b22(imax,imax)
              if( b12d(imax)+b22d(imax) < 0_ilp64 ) then
                 if( wantv2t ) then
                    if( colmajor ) then
                       call stdlib_I64_cscal( m-q, cnegone, v2t(imax,1_ilp64), ldv2t )
                    else
                       call stdlib_I64_cscal( m-q, cnegone, v2t(1_ilp64,imax), 1_ilp64 )
                    end if
                 end if
              end if
              ! test for negligible sines or cosines
           ...
           do i = 1, q
              ...
              if( mini /= i ) then
                 ...
                 theta(i) = thetamin
                 if( colmajor ) then
                    if( wantu1 )call stdlib_I64_cswap( p, u1(1_ilp64,i), 1_ilp64, u1(1_ilp64,mini), 1_ilp64 )
                    if( wantu2 )call stdlib_I64_cswap( m-p, u2(1_ilp64,i), 1_ilp64, u2(1_ilp64,mini), 1_ilp64 )
                    if( wantv1t )call stdlib_I64_cswap( q, v1t(i,1_ilp64), ldv1t, v1t(mini,1_ilp64), ldv1t )
                              
                    if( wantv2t )call stdlib_I64_cswap( m-q, v2t(i,1_ilp64), ldv2t, v2t(mini,1_ilp64),ldv2t )
                              
                 else
                    if( wantu1 )call stdlib_I64_cswap( p, u1(i,1_ilp64), ldu1, u1(mini,1_ilp64), ldu1 )
                    if( wantu2 )call stdlib_I64_cswap( m-p, u2(i,1_ilp64), ldu2, u2(mini,1_ilp64), ldu2 )
                    if( wantv1t )call stdlib_I64_cswap( q, v1t(1_ilp64,i), 1_ilp64, v1t(1_ilp64,mini), 1_ilp64 )
                    if( wantv2t )call stdlib_I64_cswap( m-q, v2t(1_ilp64,i), 1_ilp64, v2t(1_ilp64,mini), 1_ilp64 )
                 end if
              end if
           end do
           return
     ...
     ldu1, u2, ldu2, v1t, ldv1t,v2t, ldv2t, b11d, b11e, b12d, b12e, b21d, b21e,b22d, b22e, rwork, &
        ...
        ! ===================================================================
           ...
           if( info /= 0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZBBCSD', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           do while( imax > 1 )
              ...
              ! rotate to produce bulges in b11 and b21
              if( mu <= nu ) then
                 call stdlib_I64_dlartgs( b11d(imin), b11e(imin), mu,rwork(iv1tcs+imin-1), rwork(&
                           iv1tsn+imin-1) )
              else
                 call stdlib_I64_dlartgs( b21d(imin), b21e(imin), nu,rwork(iv1tcs+imin-1), rwork(&
                           iv1tsn+imin-1) )
              end if
              temp = rwork(iv1tcs+imin-1)*b11d(imin) +rwork(iv1tsn+imin-1)*b11e(imin)
              b11e(imin) = rwork(iv1tcs+imin-1)*b11e(imin) -rwork(iv1tsn+imin-1)*b11d(imin)
              ...
              ! chase the bulges in b11(imin+1,imin) and b21(imin+1,imin)
              if( b11d(imin)**2_ilp64+b11bulge**2_ilp64 > thresh**2_ilp64 ) then
                 call stdlib_I64_dlartgp( b11bulge, b11d(imin), rwork(iu1sn+imin-1),rwork(iu1cs+imin-&
                           1_ilp64), r )
              else if( mu <= nu ) then
                 call stdlib_I64_dlartgs( b11e( imin ), b11d( imin + 1_ilp64 ), mu,rwork(iu1cs+imin-1), &
                           rwork(iu1sn+imin-1) )
              else
                 call stdlib_I64_dlartgs( b12d( imin ), b12e( imin ), nu,rwork(iu1cs+imin-1), rwork(&
                           iu1sn+imin-1) )
              end if
              if( b21d(imin)**2_ilp64+b21bulge**2_ilp64 > thresh**2_ilp64 ) then
                 call stdlib_I64_dlartgp( b21bulge, b21d(imin), rwork(iu2sn+imin-1),rwork(iu2cs+imin-&
                           1_ilp64), r )
              else if( nu < mu ) then
                 call stdlib_I64_dlartgs( b21e( imin ), b21d( imin + 1_ilp64 ), nu,rwork(iu2cs+imin-1), &
                           rwork(iu2sn+imin-1) )
              else
                 call stdlib_I64_dlartgs( b22d(imin), b22e(imin), mu,rwork(iu2cs+imin-1), rwork(iu2sn+&
                           imin-1) )
              end if
              rwork(iu2cs+imin-1) = -rwork(iu2cs+imin-1)
              rwork(iu2sn+imin-1) = -rwork(iu2sn+imin-1)
              ...
              do i = imin+1, imax-1
                 ...
                 ! chasing by applying the original shift again.
                 if( .not. restart11 .and. .not. restart21 ) then
                    call stdlib_I64_dlartgp( x2, x1, rwork(iv1tsn+i-1),rwork(iv1tcs+i-1), r )
                 else if( .not. restart11 .and. restart21 ) then
                    call stdlib_I64_dlartgp( b11bulge, b11e(i-1), rwork(iv1tsn+i-1),rwork(iv1tcs+i-1),&
                               r )
                 else if( restart11 .and. .not. restart21 ) then
                    call stdlib_I64_dlartgp( b21bulge, b21e(i-1), rwork(iv1tsn+i-1),rwork(iv1tcs+i-1),&
                               r )
                 else if( mu <= nu ) then
                    call stdlib_I64_dlartgs( b11d(i), b11e(i), mu, rwork(iv1tcs+i-1),rwork(iv1tsn+i-1)&
                               )
                 else
                    call stdlib_I64_dlartgs( b21d(i), b21e(i), nu, rwork(iv1tcs+i-1),rwork(iv1tsn+i-1)&
                               )
                 end if
                 rwork(iv1tcs+i-1) = -rwork(iv1tcs+i-1)
                 rwork(iv1tsn+i-1) = -rwork(iv1tsn+i-1)
                 if( .not. restart12 .and. .not. restart22 ) then
                    call stdlib_I64_dlartgp( y2, y1, rwork(iv2tsn+i-1-1),rwork(iv2tcs+i-1-1), r )
                              
                 else if( .not. restart12 .and. restart22 ) then
                    call stdlib_I64_dlartgp( b12bulge, b12d(i-1), rwork(iv2tsn+i-1-1),rwork(iv2tcs+i-&
                              1_ilp64-1), r )
                 else if( restart12 .and. .not. restart22 ) then
                    call stdlib_I64_dlartgp( b22bulge, b22d(i-1), rwork(iv2tsn+i-1-1),rwork(iv2tcs+i-&
                              1_ilp64-1), r )
                 else if( nu < mu ) then
                    call stdlib_I64_dlartgs( b12e(i-1), b12d(i), nu,rwork(iv2tcs+i-1-1), rwork(iv2tsn+&
                              i-1-1) )
                 else
                    call stdlib_I64_dlartgs( b22e(i-1), b22d(i), mu,rwork(iv2tcs+i-1-1), rwork(iv2tsn+&
                              i-1-1) )
                 end if
                 temp = rwork(iv1tcs+i-1)*b11d(i) + rwork(iv1tsn+i-1)*b11e(i)
                 b11e(i) = rwork(iv1tcs+i-1)*b11e(i) -rwork(iv1tsn+i-1)*b11d(i)
                 ...
                 ! chasing by applying the original shift again.
                 if( .not. restart11 .and. .not. restart12 ) then
                    call stdlib_I64_dlartgp( x2, x1, rwork(iu1sn+i-1), rwork(iu1cs+i-1),r )
                 else if( .not. restart11 .and. restart12 ) then
                    call stdlib_I64_dlartgp( b11bulge, b11d(i), rwork(iu1sn+i-1),rwork(iu1cs+i-1), r )
                              
                 else if( restart11 .and. .not. restart12 ) then
                    call stdlib_I64_dlartgp( b12bulge, b12e(i-1), rwork(iu1sn+i-1),rwork(iu1cs+i-1), &
                              r )
                 else if( mu <= nu ) then
                    call stdlib_I64_dlartgs( b11e(i), b11d(i+1), mu, rwork(iu1cs+i-1),rwork(iu1sn+i-1)&
                               )
                 else
                    call stdlib_I64_dlartgs( b12d(i), b12e(i), nu, rwork(iu1cs+i-1),rwork(iu1sn+i-1) )
                              
                 end if
                 if( .not. restart21 .and. .not. restart22 ) then
                    call stdlib_I64_dlartgp( y2, y1, rwork(iu2sn+i-1), rwork(iu2cs+i-1),r )
                 else if( .not. restart21 .and. restart22 ) then
                    call stdlib_I64_dlartgp( b21bulge, b21d(i), rwork(iu2sn+i-1),rwork(iu2cs+i-1), r )
                              
                 else if( restart21 .and. .not. restart22 ) then
                    call stdlib_I64_dlartgp( b22bulge, b22e(i-1), rwork(iu2sn+i-1),rwork(iu2cs+i-1), &
                              r )
                 else if( nu < mu ) then
                    call stdlib_I64_dlartgs( b21e(i), b21e(i+1), nu, rwork(iu2cs+i-1),rwork(iu2sn+i-1)&
                               )
                 else
                    call stdlib_I64_dlartgs( b22d(i), b22e(i), mu, rwork(iu2cs+i-1),rwork(iu2sn+i-1) )
              ...
              restart22 = b22d(imax-1)**2_ilp64 + b22bulge**2_ilp64 <= thresh**2_ilp64
              if( .not. restart12 .and. .not. restart22 ) then
                 call stdlib_I64_dlartgp( y2, y1, rwork(iv2tsn+imax-1-1),rwork(iv2tcs+imax-1-1), r )
                           
              else if( .not. restart12 .and. restart22 ) then
                 call stdlib_I64_dlartgp( b12bulge, b12d(imax-1),rwork(iv2tsn+imax-1-1),rwork(iv2tcs+&
                           imax-1-1), r )
              else if( restart12 .and. .not. restart22 ) then
                 call stdlib_I64_dlartgp( b22bulge, b22d(imax-1),rwork(iv2tsn+imax-1-1),rwork(iv2tcs+&
                           imax-1-1), r )
              else if( nu < mu ) then
                 call stdlib_I64_dlartgs( b12e(imax-1), b12d(imax), nu,rwork(iv2tcs+imax-1-1),rwork(&
                           iv2tsn+imax-1-1) )
              else
                 call stdlib_I64_dlartgs( b22e(imax-1), b22d(imax), mu,rwork(iv2tcs+imax-1-1),rwork(&
                           iv2tsn+imax-1-1) )
              end if
              temp = rwork(iv2tcs+imax-1-1)*b12e(imax-1) +rwork(iv2tsn+imax-1-1)*b12d(imax)
              ...
              if( wantu1 ) then
                 if( colmajor ) then
                    call stdlib_I64_zlasr( 'R', 'V', 'F', p, imax-imin+1,rwork(iu1cs+imin-1), rwork(&
                              iu1sn+imin-1),u1(1_ilp64,imin), ldu1 )
                 else
                    call stdlib_I64_zlasr( 'L', 'V', 'F', imax-imin+1, p,rwork(iu1cs+imin-1), rwork(&
                              iu1sn+imin-1),u1(imin,1_ilp64), ldu1 )
                 end if
              end if
              if( wantu2 ) then
                 if( colmajor ) then
                    call stdlib_I64_zlasr( 'R', 'V', 'F', m-p, imax-imin+1,rwork(iu2cs+imin-1), rwork(&
                              iu2sn+imin-1),u2(1_ilp64,imin), ldu2 )
                 else
                    call stdlib_I64_zlasr( 'L', 'V', 'F', imax-imin+1, m-p,rwork(iu2cs+imin-1), rwork(&
                              iu2sn+imin-1),u2(imin,1_ilp64), ldu2 )
                 end if
              end if
              if( wantv1t ) then
                 if( colmajor ) then
                    call stdlib_I64_zlasr( 'L', 'V', 'F', imax-imin+1, q,rwork(iv1tcs+imin-1), rwork(&
                              iv1tsn+imin-1),v1t(imin,1_ilp64), ldv1t )
                 else
                    call stdlib_I64_zlasr( 'R', 'V', 'F', q, imax-imin+1,rwork(iv1tcs+imin-1), rwork(&
                              iv1tsn+imin-1),v1t(1_ilp64,imin), ldv1t )
                 end if
              end if
              if( wantv2t ) then
                 if( colmajor ) then
                    call stdlib_I64_zlasr( 'L', 'V', 'F', imax-imin+1, m-q,rwork(iv2tcs+imin-1), &
                              rwork(iv2tsn+imin-1),v2t(imin,1_ilp64), ldv2t )
                 else
                    call stdlib_I64_zlasr( 'R', 'V', 'F', m-q, imax-imin+1,rwork(iv2tcs+imin-1), &
                              rwork(iv2tsn+imin-1),v2t(1_ilp64,imin), ldv2t )
                 end if
              end if
              ! fix signs on b11(imax-1,imax) and b21(imax-1,imax)
              if( b11e(imax-1)+b21e(imax-1) > 0_ilp64 ) then
                 ...
                 if( wantv1t ) then
                    if( colmajor ) then
                       call stdlib_I64_zscal( q, cnegone, v1t(imax,1_ilp64), ldv1t )
                    else
                       call stdlib_I64_zscal( q, cnegone, v1t(1_ilp64,imax), 1_ilp64 )
                    end if
                 end if
              end if
              ! compute theta(imax)
              ...
              if( b11d(imax)+b12e(imax-1) < 0_ilp64 ) then
                 ...
                 if( wantu1 ) then
                    if( colmajor ) then
                       call stdlib_I64_zscal( p, cnegone, u1(1_ilp64,imax), 1_ilp64 )
                    else
                       call stdlib_I64_zscal( p, cnegone, u1(imax,1_ilp64), ldu1 )
                    end if
                 end if
              end if
              if( b21d(imax)+b22e(imax-1) > 0_ilp64 ) then
                 ...
                 if( wantu2 ) then
                    if( colmajor ) then
                       call stdlib_I64_zscal( m-p, cnegone, u2(1_ilp64,imax), 1_ilp64 )
                    else
                       call stdlib_I64_zscal( m-p, cnegone, u2(imax,1_ilp64), ldu2 )
                    end if
                 end if
              end if
              ! fix signs on b12(imax,imax) and b22(imax,imax)
              if( b12d(imax)+b22d(imax) < 0_ilp64 ) then
                 if( wantv2t ) then
                    if( colmajor ) then
                       call stdlib_I64_zscal( m-q, cnegone, v2t(imax,1_ilp64), ldv2t )
                    else
                       call stdlib_I64_zscal( m-q, cnegone, v2t(1_ilp64,imax), 1_ilp64 )
                    end if
                 end if
              end if
              ! test for negligible sines or cosines
           ...
           do i = 1, q
              ...
              if( mini /= i ) then
                 ...
                 theta(i) = thetamin
                 if( colmajor ) then
                    if( wantu1 )call stdlib_I64_zswap( p, u1(1_ilp64,i), 1_ilp64, u1(1_ilp64,mini), 1_ilp64 )
                    if( wantu2 )call stdlib_I64_zswap( m-p, u2(1_ilp64,i), 1_ilp64, u2(1_ilp64,mini), 1_ilp64 )
                    if( wantv1t )call stdlib_I64_zswap( q, v1t(i,1_ilp64), ldv1t, v1t(mini,1_ilp64), ldv1t )
                              
                    if( wantv2t )call stdlib_I64_zswap( m-q, v2t(i,1_ilp64), ldv2t, v2t(mini,1_ilp64),ldv2t )
                              
                 else
                    if( wantu1 )call stdlib_I64_zswap( p, u1(i,1_ilp64), ldu1, u1(mini,1_ilp64), ldu1 )
                    if( wantu2 )call stdlib_I64_zswap( m-p, u2(i,1_ilp64), ldu2, u2(mini,1_ilp64), ldu2 )
                    if( wantv1t )call stdlib_I64_zswap( q, v1t(1_ilp64,i), 1_ilp64, v1t(1_ilp64,mini), 1_ilp64 )
                    if( wantv2t )call stdlib_I64_zswap( m-q, v2t(1_ilp64,i), 1_ilp64, v2t(1_ilp64,mini), 1_ilp64 )
                 end if
              end if
           end do
           return
     ...
     ldx11, x12,ldx12, x21, ldx21, x22, ldx22, theta,u1, ldu1, u2, ldu2, v1t, ldv1t, v2t,ldv2t, &
        ...
        ! ===================================================================
           ...
           ! Local Scalars 
           character :: transt, signst
           integer(ilp64) :: childinfo, i, ib11d, ib11e, ib12d, ib12e, ib21d, ib21e, ib22d, ib22e, &
           ibbcsd, iorbdb, iorglq, iorgqr, iphi, itaup1, itaup2, itauq1, itauq2, j, lbbcsdwork, &
           lbbcsdworkmin, lbbcsdworkopt, lorbdbwork, lorbdbworkmin, lorbdbworkopt, lorglqwork, &
           lorglqworkmin, lorglqworkopt, lorgqrwork, lorgqrworkmin, lorgqrworkopt, lworkmin, &
                     lworkopt, p1, q1
           ...
           if( m < 0_ilp64 ) then
              ...
              info = -9_ilp64
           else if ( colmajor .and.  ldx11 < max( 1_ilp64, p ) ) then
             info = -11_ilp64
           else if (.not. colmajor .and. ldx11 < max( 1_ilp64, q ) ) then
             info = -11_ilp64
           else if (colmajor .and. ldx12 < max( 1_ilp64, p ) ) then
             info = -13_ilp64
           else if (.not. colmajor .and. ldx12 < max( 1_ilp64, m-q ) ) then
             info = -13_ilp64
           else if (colmajor .and. ldx21 < max( 1_ilp64, m-p ) ) then
             info = -15_ilp64
           else if (.not. colmajor .and. ldx21 < max( 1_ilp64, q ) ) then
             info = -15_ilp64
           else if (colmajor .and. ldx22 < max( 1_ilp64, m-p ) ) then
             info = -17_ilp64
           else if (.not. colmajor .and. ldx22 < max( 1_ilp64, m-q ) ) then
             info = -17_ilp64
           else if( wantu1 .and. ldu1 < p ) then
              info = -20_ilp64
           else if( wantu2 .and. ldu2 < m-p ) then
           ...
           ! work with transpose if convenient
           if( info == 0_ilp64 .and. min( p, m-p ) < min( q, m-q ) ) then
              if( colmajor ) then
                 transt = 'T'
              else
                 transt = 'N'
              ...
              call stdlib_I64_cuncsd( jobv1t, jobv2t, jobu1, jobu2, transt, signst, m,q, p, x11, &
              ldx11, x21, ldx21, x12, ldx12, x22,ldx22, theta, v1t, ldv1t, v2t, ldv2t, u1, ldu1,&
                        u2, ldu2, work, lwork, rwork, lrwork, iwork,info )
              return
           end if
           ! work with permutation [ 0 i; i 0 ] * x * [ 0 i; i 0 ] if
           ! convenient
           if( info == 0_ilp64 .and. m-q < q ) then
              if( defaultsigns ) then
                 signst = 'O'
              else
                 signst = 'D'
              end if
              call stdlib_I64_cuncsd( jobu2, jobu1, jobv2t, jobv1t, trans, signst, m,m-p, m-q, x22, &
              ldx22, x21, ldx21, x12, ldx12, x11,ldx11, theta, u2, ldu2, u1, ldu1, v2t, ldv2t, &
                        v1t,ldv1t, work, lwork, rwork, lrwork, iwork, info )
              return
           end if
           ...
           if( info == 0_ilp64 ) then
              ! real workspace
              iphi = 2_ilp64
              ib11d = iphi + max( 1_ilp64, q - 1_ilp64 )
              ib11e = ib11d + max( 1_ilp64, q )
              ib12d = ib11e + max( 1_ilp64, q - 1_ilp64 )
              ib12e = ib12d + max( 1_ilp64, q )
              ib21d = ib12e + max( 1_ilp64, q - 1_ilp64 )
              ib21e = ib21d + max( 1_ilp64, q )
              ib22d = ib21e + max( 1_ilp64, q - 1_ilp64 )
              ib22e = ib22d + max( 1_ilp64, q )
              ibbcsd = ib22e + max( 1_ilp64, q - 1_ilp64 )
              call stdlib_I64_cbbcsd( jobu1, jobu2, jobv1t, jobv2t, trans, m, p, q,theta, theta, u1, &
              ldu1, u2, ldu2, v1t, ldv1t,v2t, ldv2t, theta, theta, theta, theta, theta,theta, &
                        theta, theta, rwork, -1_ilp64, childinfo )
              lbbcsdworkopt = int( rwork(1_ilp64),KIND=ilp64)
              lbbcsdworkmin = lbbcsdworkopt
              lrworkopt = ibbcsd + lbbcsdworkopt - 1_ilp64
              lrworkmin = ibbcsd + lbbcsdworkmin - 1_ilp64
              rwork(1_ilp64) = lrworkopt
              ! complex workspace
              itaup1 = 2_ilp64
              itaup2 = itaup1 + max( 1_ilp64, p )
              itauq1 = itaup2 + max( 1_ilp64, m - p )
              itauq2 = itauq1 + max( 1_ilp64, q )
              iorgqr = itauq2 + max( 1_ilp64, m - q )
              call stdlib_I64_cungqr( m-q, m-q, m-q, u1, max(1_ilp64,m-q), u1, work, -1_ilp64,childinfo )
              lorgqrworkopt = int( work(1_ilp64),KIND=ilp64)
              lorgqrworkmin = max( 1_ilp64, m - q )
              iorglq = itauq2 + max( 1_ilp64, m - q )
              call stdlib_I64_cunglq( m-q, m-q, m-q, u1, max(1_ilp64,m-q), u1, work, -1_ilp64,childinfo )
              lorglqworkopt = int( work(1_ilp64),KIND=ilp64)
              lorglqworkmin = max( 1_ilp64, m - q )
              iorbdb = itauq2 + max( 1_ilp64, m - q )
              call stdlib_I64_cunbdb( trans, signs, m, p, q, x11, ldx11, x12, ldx12,x21, ldx21, x22, &
                        ldx22, theta, theta, u1, u2,v1t, v2t, work, -1_ilp64, childinfo )
              lorbdbworkopt = int( work(1_ilp64),KIND=ilp64)
              lorbdbworkmin = lorbdbworkopt
              lworkopt = max( iorgqr + lorgqrworkopt, iorglq + lorglqworkopt,iorbdb + &
                        lorbdbworkopt ) - 1_ilp64
              lworkmin = max( iorgqr + lorgqrworkmin, iorglq + lorglqworkmin,iorbdb + &
                        lorbdbworkmin ) - 1_ilp64
              work(1_ilp64) = max(lworkopt,lworkmin)
              if( lwork < lworkmin.and. .not. ( lquery .or. lrquery ) ) then
                 info = -22_ilp64
              else if( lrwork < lrworkmin.and. .not. ( lquery .or. lrquery ) ) then
                 info = -24_ilp64
              else
                 lorgqrwork = lwork - iorgqr + 1_ilp64
                 lorglqwork = lwork - iorglq + 1_ilp64
                 lorbdbwork = lwork - iorbdb + 1_ilp64
                 lbbcsdwork = lrwork - ibbcsd + 1_ilp64
              end if
           end if
           ! abort if any illegal arguments
           if( info /= 0_ilp64 ) then
              call stdlib_I64_xerbla( 'CUNCSD', -info )
              return
           else if( lquery .or. lrquery ) then
              return
           end if
           ...
           ! accumulate householder reflectors
           if( colmajor ) then
              if( wantu1 .and. p > 0_ilp64 ) then
                 call stdlib_I64_clacpy( 'L', p, q, x11, ldx11, u1, ldu1 )
                 call stdlib_I64_cungqr( p, p, q, u1, ldu1, work(itaup1), work(iorgqr),lorgqrwork, &
                           info)
              end if
              if( wantu2 .and. m-p > 0_ilp64 ) then
                 call stdlib_I64_clacpy( 'L', m-p, q, x21, ldx21, u2, ldu2 )
                 call stdlib_I64_cungqr( m-p, m-p, q, u2, ldu2, work(itaup2),work(iorgqr), lorgqrwork,&
                            info )
              end if
              if( wantv1t .and. q > 0_ilp64 ) then
                 call stdlib_I64_clacpy( 'U', q-1, q-1, x11(1_ilp64,2_ilp64), ldx11, v1t(2_ilp64,2_ilp64),ldv1t )
                 v1t(1_ilp64, 1_ilp64) = cone
                 do j = 2, q
                    v1t(1_ilp64,j) = czero
                    v1t(j,1_ilp64) = czero
                 end do
                 call stdlib_I64_cunglq( q-1, q-1, q-1, v1t(2_ilp64,2_ilp64), ldv1t, work(itauq1),work(iorglq), &
                           lorglqwork, info )
              end if
              if( wantv2t .and. m-q > 0_ilp64 ) then
                 call stdlib_I64_clacpy( 'U', p, m-q, x12, ldx12, v2t, ldv2t )
                 if( m-p > q ) then
                    call stdlib_I64_clacpy( 'U', m-p-q, m-p-q, x22(q+1,p+1), ldx22,v2t(p+1,p+1), &
                              ldv2t )
                 end if
                 if( m > q ) then
                    call stdlib_I64_cunglq( m-q, m-q, m-q, v2t, ldv2t, work(itauq2),work(iorglq), &
                              lorglqwork, info )
                 end if
              end if
           else
              if( wantu1 .and. p > 0_ilp64 ) then
                 call stdlib_I64_clacpy( 'U', q, p, x11, ldx11, u1, ldu1 )
                 call stdlib_I64_cunglq( p, p, q, u1, ldu1, work(itaup1), work(iorglq),lorglqwork, &
                           info)
              end if
              if( wantu2 .and. m-p > 0_ilp64 ) then
                 call stdlib_I64_clacpy( 'U', q, m-p, x21, ldx21, u2, ldu2 )
                 call stdlib_I64_cunglq( m-p, m-p, q, u2, ldu2, work(itaup2),work(iorglq), lorglqwork,&
                            info )
              end if
              if( wantv1t .and. q > 0_ilp64 ) then
                 call stdlib_I64_clacpy( 'L', q-1, q-1, x11(2_ilp64,1_ilp64), ldx11, v1t(2_ilp64,2_ilp64),ldv1t )
                 v1t(1_ilp64, 1_ilp64) = cone
                 do j = 2, q
                    v1t(1_ilp64,j) = czero
                    v1t(j,1_ilp64) = czero
                 end do
                 call stdlib_I64_cungqr( q-1, q-1, q-1, v1t(2_ilp64,2_ilp64), ldv1t, work(itauq1),work(iorgqr), &
                           lorgqrwork, info )
              end if
              if( wantv2t .and. m-q > 0_ilp64 ) then
                 p1 = min( p+1, m )
                 q1 = min( q+1, m )
                 call stdlib_I64_clacpy( 'L', m-q, p, x12, ldx12, v2t, ldv2t )
                 if ( m > p+q ) then
                    call stdlib_I64_clacpy( 'L', m-p-q, m-p-q, x22(p1,q1), ldx22,v2t(p+1,p+1), ldv2t )
                              
                 end if
                 call stdlib_I64_cungqr( m-q, m-q, m-q, v2t, ldv2t, work(itauq2),work(iorgqr), &
                           lorgqrwork, info )
              end if
           end if
           ! compute the csd of the matrix in bidiagonal-block form
           call stdlib_I64_cbbcsd( jobu1, jobu2, jobv1t, jobv2t, trans, m, p, q, theta,rwork(iphi), &
           u1, ldu1, u2, ldu2, v1t, ldv1t, v2t,ldv2t, rwork(ib11d), rwork(ib11e), rwork(ib12d),&
           rwork(ib12e), rwork(ib21d), rwork(ib21e),rwork(ib22d), rwork(ib22e), rwork(ibbcsd),&
                     lbbcsdwork, info )
           ! permute rows and columns to place identity submatrices in top-
           ...
           if( q > 0_ilp64 .and. wantu2 ) then
              ...
              if( colmajor ) then
                 call stdlib_I64_clapmt( .false., m-p, m-p, u2, ldu2, iwork )
              else
                 call stdlib_I64_clapmr( .false., m-p, m-p, u2, ldu2, iwork )
              end if
           end if
           if( m > 0_ilp64 .and. wantv2t ) then
              do i = 1, p
                 iwork(i) = m - p - q + i
              end do
              do i = p + 1, m - q
              ...
              if( .not. colmajor ) then
                 call stdlib_I64_clapmt( .false., m-q, m-q, v2t, ldv2t, iwork )
              else
                 call stdlib_I64_clapmr( .false., m-q, m-q, v2t, ldv2t, iwork )
              end if
           end if
           return
           ! end stdlib_I64_cuncsd
     ...
     ldx11, x12,ldx12, x21, ldx21, x22, ldx22, theta,u1, ldu1, u2, ldu2, v1t, ldv1t, v2t,ldv2t, &
        ...
        ! ===================================================================
           ...
           if( info == 0_ilp64 .and. min( p, m-p ) < min( q, m-q ) ) then
              ...
              call stdlib_I64_zuncsd( jobv1t, jobv2t, jobu1, jobu2, transt, signst, m,q, p, x11, &
              ldx11, x21, ldx21, x12, ldx12, x22,ldx22, theta, v1t, ldv1t, v2t, ldv2t, u1, ldu1,&
                        u2, ldu2, work, lwork, rwork, lrwork, iwork,info )
              return
           end if
           ...
           if( info == 0_ilp64 .and. m-q < q ) then
              ...
              call stdlib_I64_zuncsd( jobu2, jobu1, jobv2t, jobv1t, trans, signst, m,m-p, m-q, x22, &
              ldx22, x21, ldx21, x12, ldx12, x11,ldx11, theta, u2, ldu2, u1, ldu1, v2t, ldv2t, &
                        v1t,ldv1t, work, lwork, rwork, lrwork, iwork, info )
              return
           end if
           ...
           if( info == 0_ilp64 ) then
              ...
              ib22e = ib22d + max( 1_ilp64, q )
              ibbcsd = ib22e + max( 1_ilp64, q - 1_ilp64 )
              call stdlib_I64_zbbcsd( jobu1, jobu2, jobv1t, jobv2t, trans, m, p, q,theta, theta, u1, &
              ldu1, u2, ldu2, v1t, ldv1t,v2t, ldv2t, theta, theta, theta, theta, theta,theta, &
                        theta, theta, rwork, -1_ilp64, childinfo )
              lbbcsdworkopt = int( rwork(1_ilp64),KIND=ilp64)
              lbbcsdworkmin = lbbcsdworkopt
              ...
              itauq2 = itauq1 + max( 1_ilp64, q )
              iorgqr = itauq2 + max( 1_ilp64, m - q )
              call stdlib_I64_zungqr( m-q, m-q, m-q, u1, max(1_ilp64,m-q), u1, work, -1_ilp64,childinfo )
              lorgqrworkopt = int( work(1_ilp64),KIND=ilp64)
              lorgqrworkmin = max( 1_ilp64, m - q )
              iorglq = itauq2 + max( 1_ilp64, m - q )
              call stdlib_I64_zunglq( m-q, m-q, m-q, u1, max(1_ilp64,m-q), u1, work, -1_ilp64,childinfo )
              lorglqworkopt = int( work(1_ilp64),KIND=ilp64)
              lorglqworkmin = max( 1_ilp64, m - q )
              iorbdb = itauq2 + max( 1_ilp64, m - q )
              call stdlib_I64_zunbdb( trans, signs, m, p, q, x11, ldx11, x12, ldx12,x21, ldx21, x22, &
                        ldx22, theta, theta, u1, u2,v1t, v2t, work, -1_ilp64, childinfo )
              lorbdbworkopt = int( work(1_ilp64),KIND=ilp64)
              lorbdbworkmin = lorbdbworkopt
              lworkopt = max( iorgqr + lorgqrworkopt, iorglq + lorglqworkopt,iorbdb + &
           ...
           ! abort if any illegal arguments
           if( info /= 0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZUNCSD', -info )
              return
           else if( lquery .or. lrquery ) then
              return
           end if
           ...
           if( colmajor ) then
              if( wantu1 .and. p > 0_ilp64 ) then
                 call stdlib_I64_zlacpy( 'L', p, q, x11, ldx11, u1, ldu1 )
                 call stdlib_I64_zungqr( p, p, q, u1, ldu1, work(itaup1), work(iorgqr),lorgqrwork, &
                           info)
              end if
              if( wantu2 .and. m-p > 0_ilp64 ) then
                 call stdlib_I64_zlacpy( 'L', m-p, q, x21, ldx21, u2, ldu2 )
                 call stdlib_I64_zungqr( m-p, m-p, q, u2, ldu2, work(itaup2),work(iorgqr), lorgqrwork,&
                            info )
              end if
              if( wantv1t .and. q > 0_ilp64 ) then
                 call stdlib_I64_zlacpy( 'U', q-1, q-1, x11(1_ilp64,2_ilp64), ldx11, v1t(2_ilp64,2_ilp64),ldv1t )
                 v1t(1_ilp64, 1_ilp64) = cone
                 do j = 2, q
                    v1t(1_ilp64,j) = czero
                    v1t(j,1_ilp64) = czero
                 end do
                 call stdlib_I64_zunglq( q-1, q-1, q-1, v1t(2_ilp64,2_ilp64), ldv1t, work(itauq1),work(iorglq), &
                           lorglqwork, info )
              end if
              if( wantv2t .and. m-q > 0_ilp64 ) then
                 call stdlib_I64_zlacpy( 'U', p, m-q, x12, ldx12, v2t, ldv2t )
                 if( m-p > q) then
                    call stdlib_I64_zlacpy( 'U', m-p-q, m-p-q, x22(q+1,p+1), ldx22,v2t(p+1,p+1), &
                              ldv2t )
                 end if
                 if( m > q ) then
                    call stdlib_I64_zunglq( m-q, m-q, m-q, v2t, ldv2t, work(itauq2),work(iorglq), &
                              lorglqwork, info )
                 end if
              end if
           else
              if( wantu1 .and. p > 0_ilp64 ) then
                 call stdlib_I64_zlacpy( 'U', q, p, x11, ldx11, u1, ldu1 )
                 call stdlib_I64_zunglq( p, p, q, u1, ldu1, work(itaup1), work(iorglq),lorglqwork, &
                           info)
              end if
              if( wantu2 .and. m-p > 0_ilp64 ) then
                 call stdlib_I64_zlacpy( 'U', q, m-p, x21, ldx21, u2, ldu2 )
                 call stdlib_I64_zunglq( m-p, m-p, q, u2, ldu2, work(itaup2),work(iorglq), lorglqwork,&
                            info )
              end if
              if( wantv1t .and. q > 0_ilp64 ) then
                 call stdlib_I64_zlacpy( 'L', q-1, q-1, x11(2_ilp64,1_ilp64), ldx11, v1t(2_ilp64,2_ilp64),ldv1t )
                 v1t(1_ilp64, 1_ilp64) = cone
                 do j = 2, q
                    v1t(1_ilp64,j) = czero
                    v1t(j,1_ilp64) = czero
                 end do
                 call stdlib_I64_zungqr( q-1, q-1, q-1, v1t(2_ilp64,2_ilp64), ldv1t, work(itauq1),work(iorgqr), &
                           lorgqrwork, info )
              end if
              if( wantv2t .and. m-q > 0_ilp64 ) then
                 p1 = min( p+1, m )
                 q1 = min( q+1, m )
                 call stdlib_I64_zlacpy( 'L', m-q, p, x12, ldx12, v2t, ldv2t )
                 if( m > p+q ) then
                    call stdlib_I64_zlacpy( 'L', m-p-q, m-p-q, x22(p1,q1), ldx22,v2t(p+1,p+1), ldv2t )
                              
                 end if
                 call stdlib_I64_zungqr( m-q, m-q, m-q, v2t, ldv2t, work(itauq2),work(iorgqr), &
                           lorgqrwork, info )
              end if
           end if
           ! compute the csd of the matrix in bidiagonal-block form
           call stdlib_I64_zbbcsd( jobu1, jobu2, jobv1t, jobv2t, trans, m, p, q, theta,rwork(iphi), &
           u1, ldu1, u2, ldu2, v1t, ldv1t, v2t,ldv2t, rwork(ib11d), rwork(ib11e), rwork(ib12d),&
           rwork(ib12e), rwork(ib21d), rwork(ib21e),rwork(ib22d), rwork(ib22e), rwork(ibbcsd),&
                     lbbcsdwork, info )
           ! permute rows and columns to place identity submatrices in top-
           ...
           if( q > 0_ilp64 .and. wantu2 ) then
              ...
              if( colmajor ) then
                 call stdlib_I64_zlapmt( .false., m-p, m-p, u2, ldu2, iwork )
              else
                 call stdlib_I64_zlapmr( .false., m-p, m-p, u2, ldu2, iwork )
              end if
           end if
           if( m > 0_ilp64 .and. wantv2t ) then
              do i = 1, p
              ...
              if( .not. colmajor ) then
                 call stdlib_I64_zlapmt( .false., m-q, m-q, v2t, ldv2t, iwork )
              else
                 call stdlib_I64_zlapmr( .false., m-q, m-q, v2t, ldv2t, iwork )
              end if
           end if
           return
           ! end stdlib_I64_zuncsd
     ...
     !! K2-by-K2 identity matrix, where K1 = MAX(Q+P-M,0), K2 = MAX(Q-P,0).
        ...
        ! =====================================================================
           ...
           ! Local Arrays 
           real(sp) :: dum(1_ilp64)
           complex(sp) :: cdum(1_ilp64,1_ilp64)
           ! Intrinsic Function 
           ! Executable Statements 
           ! test input arguments
           info = 0_ilp64
           ...
           wantu2 = stdlib_lsame( jobu2, 'Y' )
           wantv1t = stdlib_lsame( jobv1t, 'Y' )
           lquery = ( lwork==-1_ilp64 ) .or. ( lrwork==-1_ilp64 )
           if( m < 0_ilp64 ) then
              info = -4_ilp64
           else if( p < 0_ilp64 .or. p > m ) then
              info = -5_ilp64
              ...
              info = -6_ilp64
           else if( ldx11 < max( 1_ilp64, p ) ) then
              info = -8_ilp64
           else if( ldx21 < max( 1_ilp64, m-p ) ) then
              info = -10_ilp64
           else if( wantu1 .and. ldu1 < max( 1_ilp64, p ) ) then
              info = -13_ilp64
           else if( wantu2 .and. ldu2 < max( 1_ilp64, m - p ) ) then
              info = -15_ilp64
           else if( wantv1t .and. ldv1t < max( 1_ilp64, q ) ) then
              info = -17_ilp64
           end if
           r = min( p, m-p, q, m-q )
           ! compute workspace
           ...
           ! | tauq1 (max(1,q))                        |
           ! |-----------------------------------------|
           ! | stdlib_I64_cunbdb work | stdlib_I64_cungqr work | stdlib_I64_cunglq work |
           ! |             |             |             |
           ! |             |             |             |
           ! |             |             |             |
           ! |             |             |             |
           ...
           ! | b22d (r)         |
           ! | b22e (r-1)       |
           ! | stdlib_I64_cbbcsd rwork     |
           ! |------------------|
           if( info == 0_ilp64 ) then
              iphi = 2_ilp64
              ib11d = iphi + max( 1_ilp64, r-1 )
              ib11e = ib11d + max( 1_ilp64, r )
              ib12d = ib11e + max( 1_ilp64, r - 1_ilp64 )
              ib12e = ib12d + max( 1_ilp64, r )
              ib21d = ib12e + max( 1_ilp64, r - 1_ilp64 )
              ib21e = ib21d + max( 1_ilp64, r )
              ib22d = ib21e + max( 1_ilp64, r - 1_ilp64 )
              ib22e = ib22d + max( 1_ilp64, r )
              ibbcsd = ib22e + max( 1_ilp64, r - 1_ilp64 )
              itaup1 = 2_ilp64
              itaup2 = itaup1 + max( 1_ilp64, p )
              itauq1 = itaup2 + max( 1_ilp64, m-p )
              iorbdb = itauq1 + max( 1_ilp64, q )
              iorgqr = itauq1 + max( 1_ilp64, q )
              iorglq = itauq1 + max( 1_ilp64, q )
              lorgqrmin = 1_ilp64
              lorgqropt = 1_ilp64
              lorglqmin = 1_ilp64
              lorglqopt = 1_ilp64
              if( r == q ) then
                 call stdlib_I64_cunbdb1( m, p, q, x11, ldx11, x21, ldx21, theta,dum, cdum, cdum, &
                           cdum, work, -1_ilp64,childinfo )
                 lorbdb = int( work(1_ilp64),KIND=ilp64)
                 if( wantu1 .and. p > 0_ilp64 ) then
                    call stdlib_I64_cungqr( p, p, q, u1, ldu1, cdum, work(1_ilp64), -1_ilp64,childinfo )
                    lorgqrmin = max( lorgqrmin, p )
                    lorgqropt = max( lorgqropt, int( work(1_ilp64),KIND=ilp64) )
                 endif
                 if( wantu2 .and. m-p > 0_ilp64 ) then
                    call stdlib_I64_cungqr( m-p, m-p, q, u2, ldu2, cdum, work(1_ilp64), -1_ilp64,childinfo )
                              
                    lorgqrmin = max( lorgqrmin, m-p )
                    lorgqropt = max( lorgqropt, int( work(1_ilp64),KIND=ilp64) )
                 end if
                 if( wantv1t .and. q > 0_ilp64 ) then
                    call stdlib_I64_cunglq( q-1, q-1, q-1, v1t, ldv1t,cdum, work(1_ilp64), -1_ilp64, childinfo )
                              
                    lorglqmin = max( lorglqmin, q-1 )
                    lorglqopt = max( lorglqopt, int( work(1_ilp64),KIND=ilp64) )
                 end if
                 call stdlib_I64_cbbcsd( jobu1, jobu2, jobv1t, 'N', 'N', m, p, q, theta,dum(1_ilp64), u1, &
                 ldu1, u2, ldu2, v1t, ldv1t, cdum,1_ilp64, dum, dum, dum, dum, dum, dum, dum, dum,rwork(&
                           1_ilp64), -1_ilp64, childinfo )
                 lbbcsd = int( rwork(1_ilp64),KIND=ilp64)
              else if( r == p ) then
                 call stdlib_I64_cunbdb2( m, p, q, x11, ldx11, x21, ldx21, theta, dum,cdum, cdum, &
                           cdum, work(1_ilp64), -1_ilp64, childinfo )
                 lorbdb = int( work(1_ilp64),KIND=ilp64)
                 if( wantu1 .and. p > 0_ilp64 ) then
                    call stdlib_I64_cungqr( p-1, p-1, p-1, u1(2_ilp64,2_ilp64), ldu1, cdum, work(1_ilp64),-1_ilp64, childinfo &
                              )
                    lorgqrmin = max( lorgqrmin, p-1 )
                    lorgqropt = max( lorgqropt, int( work(1_ilp64),KIND=ilp64) )
                 end if
                 ...
                 if( wantv1t .and. q > 0_ilp64 ) then
                    call stdlib_I64_cunglq( q, q, r, v1t, ldv1t, cdum, work(1_ilp64), -1_ilp64,childinfo )
                    lorglqmin = max( lorglqmin, q )
                    lorglqopt = max( lorglqopt, int( work(1_ilp64),KIND=ilp64) )
                 end if
                 call stdlib_I64_cbbcsd( jobv1t, 'N', jobu1, jobu2, 'T', m, q, p, theta,dum, v1t, &
                 ldv1t, cdum, 1_ilp64, u1, ldu1, u2, ldu2,dum, dum, dum, dum, dum, dum, dum, dum,rwork(&
                           1_ilp64), -1_ilp64, childinfo )
                 lbbcsd = int( rwork(1_ilp64),KIND=ilp64)
              else if( r == m-p ) then
                 call stdlib_I64_cunbdb3( m, p, q, x11, ldx11, x21, ldx21, theta, dum,cdum, cdum, &
                           cdum, work(1_ilp64), -1_ilp64, childinfo )
                 lorbdb = int( work(1_ilp64),KIND=ilp64)
                 if( wantu1 .and. p > 0_ilp64 ) then
                    call stdlib_I64_cungqr( p, p, q, u1, ldu1, cdum, work(1_ilp64), -1_ilp64,childinfo )
                 ...
                 if( wantu2 .and. m-p > 0_ilp64 ) then
                    call stdlib_I64_cungqr( m-p-1, m-p-1, m-p-1, u2(2_ilp64,2_ilp64), ldu2, cdum,work(1_ilp64), -1_ilp64, &
                              childinfo )
                    lorgqrmin = max( lorgqrmin, m-p-1 )
                    lorgqropt = max( lorgqropt, int( work(1_ilp64),KIND=ilp64) )
                 end if
                 ...
                 call stdlib_I64_cbbcsd( 'N', jobv1t, jobu2, jobu1, 'T', m, m-q, m-p,theta, dum, cdum,&
                  1_ilp64, v1t, ldv1t, u2, ldu2, u1,ldu1, dum, dum, dum, dum, dum, dum, dum, dum,rwork(&
                            1_ilp64), -1_ilp64, childinfo )
                 lbbcsd = int( rwork(1_ilp64),KIND=ilp64)
              else
                 call stdlib_I64_cunbdb4( m, p, q, x11, ldx11, x21, ldx21, theta, dum,cdum, cdum, &
                           cdum, cdum, work(1_ilp64), -1_ilp64, childinfo)
                 lorbdb = m + int( work(1_ilp64),KIND=ilp64)
                 if( wantu1 .and. p > 0_ilp64 ) then
                    call stdlib_I64_cungqr( p, p, m-q, u1, ldu1, cdum, work(1_ilp64), -1_ilp64,childinfo )
                    lorgqrmin = max( lorgqrmin, p )
                    lorgqropt = max( lorgqropt, int( work(1_ilp64),KIND=ilp64) )
                 end if
                 if( wantu2 .and. m-p > 0_ilp64 ) then
                    call stdlib_I64_cungqr( m-p, m-p, m-q, u2, ldu2, cdum, work(1_ilp64), -1_ilp64,childinfo )
                              
                    lorgqrmin = max( lorgqrmin, m-p )
                    lorgqropt = max( lorgqropt, int( work(1_ilp64),KIND=ilp64) )
                 end if
                 if( wantv1t .and. q > 0_ilp64 ) then
                    call stdlib_I64_cunglq( q, q, q, v1t, ldv1t, cdum, work(1_ilp64), -1_ilp64,childinfo )
                    lorglqmin = max( lorglqmin, q )
                    lorglqopt = max( lorglqopt, int( work(1_ilp64),KIND=ilp64) )
                 end if
                 call stdlib_I64_cbbcsd( jobu2, jobu1, 'N', jobv1t, 'N', m, m-p, m-q,theta, dum, u2, &
                 ldu2, u1, ldu1, cdum, 1_ilp64, v1t,ldv1t, dum, dum, dum, dum, dum, dum, dum, dum,rwork(&
                           1_ilp64), -1_ilp64, childinfo )
                 lbbcsd = int( rwork(1_ilp64),KIND=ilp64)
              end if
              lrworkmin = ibbcsd+lbbcsd-1
           ...
           if( info /= 0_ilp64 ) then
              call stdlib_I64_xerbla( 'CUNCSD2BY1', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           if( r == q ) then
              ! case 1: r = q
              ! simultaneously bidiagonalize x11 and x21
              call stdlib_I64_cunbdb1( m, p, q, x11, ldx11, x21, ldx21, theta,rwork(iphi), work(&
                        itaup1), work(itaup2),work(itauq1), work(iorbdb), lorbdb, childinfo )
              ! accumulate householder reflectors
              if( wantu1 .and. p > 0_ilp64 ) then
                 call stdlib_I64_clacpy( 'L', p, q, x11, ldx11, u1, ldu1 )
                 call stdlib_I64_cungqr( p, p, q, u1, ldu1, work(itaup1), work(iorgqr),lorgqr, &
                           childinfo )
              end if
              if( wantu2 .and. m-p > 0_ilp64 ) then
                 call stdlib_I64_clacpy( 'L', m-p, q, x21, ldx21, u2, ldu2 )
                 call stdlib_I64_cungqr( m-p, m-p, q, u2, ldu2, work(itaup2),work(iorgqr), lorgqr, &
                           childinfo )
              end if
              if( wantv1t .and. q > 0_ilp64 ) then
                 v1t(1_ilp64,1_ilp64) = cone
                 ...
                 call stdlib_I64_clacpy( 'U', q-1, q-1, x21(1_ilp64,2_ilp64), ldx21, v1t(2_ilp64,2_ilp64),ldv1t )
                 call stdlib_I64_cunglq( q-1, q-1, q-1, v1t(2_ilp64,2_ilp64), ldv1t, work(itauq1),work(iorglq), &
                           lorglq, childinfo )
              end if
              ! simultaneously diagonalize x11 and x21.
              call stdlib_I64_cbbcsd( jobu1, jobu2, jobv1t, 'N', 'N', m, p, q, theta,rwork(iphi), u1, &
              ldu1, u2, ldu2, v1t, ldv1t, cdum,1_ilp64, rwork(ib11d), rwork(ib11e), rwork(ib12d),rwork(&
              ib12e), rwork(ib21d), rwork(ib21e),rwork(ib22d), rwork(ib22e), rwork(ibbcsd),lrwork-&
                        ibbcsd+1, childinfo )
              ! permute rows and columns to place czero submatrices in
              ! preferred positions
              ...
              ! case 2: r = p
              ! simultaneously bidiagonalize x11 and x21
              call stdlib_I64_cunbdb2( m, p, q, x11, ldx11, x21, ldx21, theta,rwork(iphi), work(&
                        itaup1), work(itaup2),work(itauq1), work(iorbdb), lorbdb, childinfo )
              ! accumulate householder reflectors
              if( wantu1 .and. p > 0_ilp64 ) then
                 u1(1_ilp64,1_ilp64) = cone
                 do j = 2, p
                    u1(1_ilp64,j) = czero
                    u1(j,1_ilp64) = czero
                 end do
                 call stdlib_I64_clacpy( 'L', p-1, p-1, x11(2_ilp64,1_ilp64), ldx11, u1(2_ilp64,2_ilp64), ldu1 )
                 call stdlib_I64_cungqr( p-1, p-1, p-1, u1(2_ilp64,2_ilp64), ldu1, work(itaup1),work(iorgqr), &
                           lorgqr, childinfo )
              end if
              if( wantu2 .and. m-p > 0_ilp64 ) then
                 call stdlib_I64_clacpy( 'L', m-p, q, x21, ldx21, u2, ldu2 )
              ...
              if( wantv1t .and. q > 0_ilp64 ) then
                 call stdlib_I64_clacpy( 'U', p, q, x11, ldx11, v1t, ldv1t )
                 call stdlib_I64_cunglq( q, q, r, v1t, ldv1t, work(itauq1),work(iorglq), lorglq, &
                           childinfo )
              end if
              ! simultaneously diagonalize x11 and x21.
              call stdlib_I64_cbbcsd( jobv1t, 'N', jobu1, jobu2, 'T', m, q, p, theta,rwork(iphi), v1t,&
               ldv1t, cdum, 1_ilp64, u1, ldu1, u2,ldu2, rwork(ib11d), rwork(ib11e), rwork(ib12d),rwork(&
               ib12e), rwork(ib21d), rwork(ib21e),rwork(ib22d), rwork(ib22e), rwork(ibbcsd), &
                         lbbcsd,childinfo )
              ! permute rows and columns to place identity submatrices in
              ! preferred positions
              ...
              ! case 3: r = m-p
              ! simultaneously bidiagonalize x11 and x21
              call stdlib_I64_cunbdb3( m, p, q, x11, ldx11, x21, ldx21, theta,rwork(iphi), work(&
                        itaup1), work(itaup2),work(itauq1), work(iorbdb), lorbdb, childinfo )
              ! accumulate householder reflectors
              if( wantu1 .and. p > 0_ilp64 ) then
                 call stdlib_I64_clacpy( 'L', p, q, x11, ldx11, u1, ldu1 )
              ...
              if( wantu2 .and. m-p > 0_ilp64 ) then
                 u2(1_ilp64,1_ilp64) = cone
                 do j = 2, m-p
                    u2(1_ilp64,j) = czero
                    u2(j,1_ilp64) = czero
                 end do
                 call stdlib_I64_clacpy( 'L', m-p-1, m-p-1, x21(2_ilp64,1_ilp64), ldx21, u2(2_ilp64,2_ilp64),ldu2 )
                 call stdlib_I64_cungqr( m-p-1, m-p-1, m-p-1, u2(2_ilp64,2_ilp64), ldu2,work(itaup2), work(iorgqr)&
                           , lorgqr, childinfo )
              end if
              if( wantv1t .and. q > 0_ilp64 ) then
                 call stdlib_I64_clacpy( 'U', m-p, q, x21, ldx21, v1t, ldv1t )
                 call stdlib_I64_cunglq( q, q, r, v1t, ldv1t, work(itauq1),work(iorglq), lorglq, &
                           childinfo )
              end if
              ! simultaneously diagonalize x11 and x21.
              call stdlib_I64_cbbcsd( 'N', jobv1t, jobu2, jobu1, 'T', m, m-q, m-p,theta, rwork(iphi), &
              cdum, 1_ilp64, v1t, ldv1t, u2, ldu2,u1, ldu1, rwork(ib11d), rwork(ib11e),rwork(ib12d), &
              rwork(ib12e), rwork(ib21d),rwork(ib21e), rwork(ib22d), rwork(ib22e),rwork(ibbcsd), &
                        lbbcsd, childinfo )
              ! permute rows and columns to place identity submatrices in
              ! preferred positions
              if( q > r ) then
                 ...
                 if( wantu1 ) then
                    call stdlib_I64_clapmt( .false., p, q, u1, ldu1, iwork )
                 end if
                 if( wantv1t ) then
                    call stdlib_I64_clapmr( .false., q, q, v1t, ldv1t, iwork )
                 end if
              end if
           else
              ! case 4: r = m-q
              ! simultaneously bidiagonalize x11 and x21
              call stdlib_I64_cunbdb4( m, p, q, x11, ldx11, x21, ldx21, theta,rwork(iphi), work(&
              itaup1), work(itaup2),work(itauq1), work(iorbdb), work(iorbdb+m),lorbdb-m, &
                        childinfo )
              ! accumulate householder reflectors
              if( wantu2 .and. m-p > 0_ilp64 ) then
                 call stdlib_I64_ccopy( m-p, work(iorbdb+p), 1_ilp64, u2, 1_ilp64 )
              end if
              if( wantu1 .and. p > 0_ilp64 ) then
                 call stdlib_I64_ccopy( p, work(iorbdb), 1_ilp64, u1, 1_ilp64 )
                 do j = 2, p
                    u1(1_ilp64,j) = czero
                 end do
                 call stdlib_I64_clacpy( 'L', p-1, m-q-1, x11(2_ilp64,1_ilp64), ldx11, u1(2_ilp64,2_ilp64),ldu1 )
                 call stdlib_I64_cungqr( p, p, m-q, u1, ldu1, work(itaup1),work(iorgqr), lorgqr, &
                           childinfo )
              end if
              if( wantu2 .and. m-p > 0_ilp64 ) then
                 do j = 2, m-p
                 ...
                 call stdlib_I64_clacpy( 'L', m-p-1, m-q-1, x21(2_ilp64,1_ilp64), ldx21, u2(2_ilp64,2_ilp64),ldu2 )
                 call stdlib_I64_cungqr( m-p, m-p, m-q, u2, ldu2, work(itaup2),work(iorgqr), lorgqr, &
                           childinfo )
              end if
              if( wantv1t .and. q > 0_ilp64 ) then
                 call stdlib_I64_clacpy( 'U', m-q, q, x21, ldx21, v1t, ldv1t )
                 call stdlib_I64_clacpy( 'U', p-(m-q), q-(m-q), x11(m-q+1,m-q+1), ldx11,v1t(m-q+1,m-q+&
                           1_ilp64), ldv1t )
                 call stdlib_I64_clacpy( 'U', -p+q, q-p, x21(m-q+1,p+1), ldx21,v1t(p+1,p+1), ldv1t )
                           
                 call stdlib_I64_cunglq( q, q, q, v1t, ldv1t, work(itauq1),work(iorglq), lorglq, &
                           childinfo )
              end if
              ! simultaneously diagonalize x11 and x21.
              call stdlib_I64_cbbcsd( jobu2, jobu1, 'N', jobv1t, 'N', m, m-p, m-q,theta, rwork(iphi), &
              u2, ldu2, u1, ldu1, cdum, 1_ilp64,v1t, ldv1t, rwork(ib11d), rwork(ib11e),rwork(ib12d), &
              rwork(ib12e), rwork(ib21d),rwork(ib21e), rwork(ib22d), rwork(ib22e),rwork(ibbcsd), &
                        lbbcsd, childinfo )
              ! permute rows and columns to place identity submatrices in
              ! preferred positions
              if( p > r ) then
                 ...
                 if( wantu1 ) then
                    call stdlib_I64_clapmt( .false., p, p, u1, ldu1, iwork )
                 end if
                 if( wantv1t ) then
                    call stdlib_I64_clapmr( .false., p, q, v1t, ldv1t, iwork )
                 end if
              end if
           end if
           return
     ...
     !! K2-by-K2 identity matrix, where K1 = MAX(Q+P-M,0), K2 = MAX(Q-P,0).
        ...
        ! =====================================================================
           ...
           ! Local Arrays 
           real(dp) :: dum(1_ilp64)
           complex(dp) :: cdum(1_ilp64,1_ilp64)
           ! Intrinsic Function 
           ! Executable Statements 
           ! test input arguments
           info = 0_ilp64
           ...
           ! | tauq1 (max(1,q))                        |
           ! |-----------------------------------------|
           ! | stdlib_I64_zunbdb work | stdlib_I64_zungqr work | stdlib_I64_zunglq work |
           ! |             |             |             |
           ! |             |             |             |
           ! |             |             |             |
           ! |             |             |             |
           ...
           if( info == 0_ilp64 ) then
              ...
              lorglqopt = 1_ilp64
              if( r == q ) then
                 call stdlib_I64_zunbdb1( m, p, q, x11, ldx11, x21, ldx21, theta, dum,cdum, cdum, &
                           cdum, work, -1_ilp64, childinfo )
                 lorbdb = int( work(1_ilp64),KIND=ilp64)
                 if( wantu1 .and. p > 0_ilp64 ) then
                    call stdlib_I64_zungqr( p, p, q, u1, ldu1, cdum, work(1_ilp64), -1_ilp64,childinfo )
                    lorgqrmin = max( lorgqrmin, p )
                    lorgqropt = max( lorgqropt, int( work(1_ilp64),KIND=ilp64) )
                 endif
                 if( wantu2 .and. m-p > 0_ilp64 ) then
                    call stdlib_I64_zungqr( m-p, m-p, q, u2, ldu2, cdum, work(1_ilp64), -1_ilp64,childinfo )
                              
                    lorgqrmin = max( lorgqrmin, m-p )
                    lorgqropt = max( lorgqropt, int( work(1_ilp64),KIND=ilp64) )
                 end if
                 if( wantv1t .and. q > 0_ilp64 ) then
                    call stdlib_I64_zunglq( q-1, q-1, q-1, v1t, ldv1t,cdum, work(1_ilp64), -1_ilp64, childinfo )
                              
                    lorglqmin = max( lorglqmin, q-1 )
                    lorglqopt = max( lorglqopt, int( work(1_ilp64),KIND=ilp64) )
                 end if
                 call stdlib_I64_zbbcsd( jobu1, jobu2, jobv1t, 'N', 'N', m, p, q, theta,dum, u1, ldu1,&
                  u2, ldu2, v1t, ldv1t, cdum, 1_ilp64,dum, dum, dum, dum, dum, dum, dum, dum,rwork(1_ilp64), -&
                            1_ilp64, childinfo )
                 lbbcsd = int( rwork(1_ilp64),KIND=ilp64)
              else if( r == p ) then
                 call stdlib_I64_zunbdb2( m, p, q, x11, ldx11, x21, ldx21, theta, dum,cdum, cdum, &
                           cdum, work(1_ilp64), -1_ilp64, childinfo )
                 lorbdb = int( work(1_ilp64),KIND=ilp64)
                 if( wantu1 .and. p > 0_ilp64 ) then
                    call stdlib_I64_zungqr( p-1, p-1, p-1, u1(2_ilp64,2_ilp64), ldu1, cdum, work(1_ilp64),-1_ilp64, childinfo &
                              )
                    lorgqrmin = max( lorgqrmin, p-1 )
                    lorgqropt = max( lorgqropt, int( work(1_ilp64),KIND=ilp64) )
                 end if
                 ...
                 if( wantv1t .and. q > 0_ilp64 ) then
                    call stdlib_I64_zunglq( q, q, r, v1t, ldv1t, cdum, work(1_ilp64), -1_ilp64,childinfo )
                    lorglqmin = max( lorglqmin, q )
                    lorglqopt = max( lorglqopt, int( work(1_ilp64),KIND=ilp64) )
                 end if
                 call stdlib_I64_zbbcsd( jobv1t, 'N', jobu1, jobu2, 'T', m, q, p, theta,dum, v1t, &
                 ldv1t, cdum, 1_ilp64, u1, ldu1, u2, ldu2,dum, dum, dum, dum, dum, dum, dum, dum,rwork(&
                           1_ilp64), -1_ilp64, childinfo )
                 lbbcsd = int( rwork(1_ilp64),KIND=ilp64)
              else if( r == m-p ) then
                 call stdlib_I64_zunbdb3( m, p, q, x11, ldx11, x21, ldx21, theta, dum,cdum, cdum, &
                           cdum, work(1_ilp64), -1_ilp64, childinfo )
                 lorbdb = int( work(1_ilp64),KIND=ilp64)
                 if( wantu1 .and. p > 0_ilp64 ) then
                    call stdlib_I64_zungqr( p, p, q, u1, ldu1, cdum, work(1_ilp64), -1_ilp64,childinfo )
                 ...
                 if( wantu2 .and. m-p > 0_ilp64 ) then
                    call stdlib_I64_zungqr( m-p-1, m-p-1, m-p-1, u2(2_ilp64,2_ilp64), ldu2, cdum,work(1_ilp64), -1_ilp64, &
                              childinfo )
                    lorgqrmin = max( lorgqrmin, m-p-1 )
                    lorgqropt = max( lorgqropt, int( work(1_ilp64),KIND=ilp64) )
                 end if
                 ...
                 call stdlib_I64_zbbcsd( 'N', jobv1t, jobu2, jobu1, 'T', m, m-q, m-p,theta, dum, cdum,&
                  1_ilp64, v1t, ldv1t, u2, ldu2, u1,ldu1, dum, dum, dum, dum, dum, dum, dum, dum,rwork(&
                            1_ilp64), -1_ilp64, childinfo )
                 lbbcsd = int( rwork(1_ilp64),KIND=ilp64)
              else
                 call stdlib_I64_zunbdb4( m, p, q, x11, ldx11, x21, ldx21, theta, dum,cdum, cdum, &
                           cdum, cdum, work(1_ilp64), -1_ilp64, childinfo)
                 lorbdb = m + int( work(1_ilp64),KIND=ilp64)
                 if( wantu1 .and. p > 0_ilp64 ) then
                    call stdlib_I64_zungqr( p, p, m-q, u1, ldu1, cdum, work(1_ilp64), -1_ilp64,childinfo )
                    lorgqrmin = max( lorgqrmin, p )
                    lorgqropt = max( lorgqropt, int( work(1_ilp64),KIND=ilp64) )
                 end if
                 if( wantu2 .and. m-p > 0_ilp64 ) then
                    call stdlib_I64_zungqr( m-p, m-p, m-q, u2, ldu2, cdum, work(1_ilp64), -1_ilp64,childinfo )
                              
                    lorgqrmin = max( lorgqrmin, m-p )
                    lorgqropt = max( lorgqropt, int( work(1_ilp64),KIND=ilp64) )
                 end if
                 if( wantv1t .and. q > 0_ilp64 ) then
                    call stdlib_I64_zunglq( q, q, q, v1t, ldv1t, cdum, work(1_ilp64), -1_ilp64,childinfo )
                    lorglqmin = max( lorglqmin, q )
                    lorglqopt = max( lorglqopt, int( work(1_ilp64),KIND=ilp64) )
                 end if
                 call stdlib_I64_zbbcsd( jobu2, jobu1, 'N', jobv1t, 'N', m, m-p, m-q,theta, dum, u2, &
                 ldu2, u1, ldu1, cdum, 1_ilp64, v1t,ldv1t, dum, dum, dum, dum, dum, dum, dum, dum,rwork(&
                           1_ilp64), -1_ilp64, childinfo )
                 lbbcsd = int( rwork(1_ilp64),KIND=ilp64)
              end if
           ...
           if( info /= 0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZUNCSD2BY1', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           if( r == q ) then
              ! case 1: r = q
              ! simultaneously bidiagonalize x11 and x21
              call stdlib_I64_zunbdb1( m, p, q, x11, ldx11, x21, ldx21, theta,rwork(iphi), work(&
                        itaup1), work(itaup2),work(itauq1), work(iorbdb), lorbdb, childinfo )
              ! accumulate householder reflectors
              if( wantu1 .and. p > 0_ilp64 ) then
                 call stdlib_I64_zlacpy( 'L', p, q, x11, ldx11, u1, ldu1 )
                 call stdlib_I64_zungqr( p, p, q, u1, ldu1, work(itaup1), work(iorgqr),lorgqr, &
                           childinfo )
              end if
              if( wantu2 .and. m-p > 0_ilp64 ) then
                 call stdlib_I64_zlacpy( 'L', m-p, q, x21, ldx21, u2, ldu2 )
                 call stdlib_I64_zungqr( m-p, m-p, q, u2, ldu2, work(itaup2),work(iorgqr), lorgqr, &
                           childinfo )
              end if
              if( wantv1t .and. q > 0_ilp64 ) then
                 v1t(1_ilp64,1_ilp64) = cone
                 ...
                 call stdlib_I64_zlacpy( 'U', q-1, q-1, x21(1_ilp64,2_ilp64), ldx21, v1t(2_ilp64,2_ilp64),ldv1t )
                 call stdlib_I64_zunglq( q-1, q-1, q-1, v1t(2_ilp64,2_ilp64), ldv1t, work(itauq1),work(iorglq), &
                           lorglq, childinfo )
              end if
              ! simultaneously diagonalize x11 and x21.
              call stdlib_I64_zbbcsd( jobu1, jobu2, jobv1t, 'N', 'N', m, p, q, theta,rwork(iphi), u1, &
              ldu1, u2, ldu2, v1t, ldv1t, cdum,1_ilp64, rwork(ib11d), rwork(ib11e), rwork(ib12d),rwork(&
              ib12e), rwork(ib21d), rwork(ib21e),rwork(ib22d), rwork(ib22e), rwork(ibbcsd),lrwork-&
                        ibbcsd+1, childinfo )
              ! permute rows and columns to place czero submatrices in
              ...
              ! case 2: r = p
              ! simultaneously bidiagonalize x11 and x21
              call stdlib_I64_zunbdb2( m, p, q, x11, ldx11, x21, ldx21, theta,rwork(iphi), work(&
                        itaup1), work(itaup2),work(itauq1), work(iorbdb), lorbdb, childinfo )
              ! accumulate householder reflectors
              if( wantu1 .and. p > 0_ilp64 ) then
                 u1(1_ilp64,1_ilp64) = cone
                 ...
                 call stdlib_I64_zlacpy( 'L', p-1, p-1, x11(2_ilp64,1_ilp64), ldx11, u1(2_ilp64,2_ilp64), ldu1 )
                 call stdlib_I64_zungqr( p-1, p-1, p-1, u1(2_ilp64,2_ilp64), ldu1, work(itaup1),work(iorgqr), &
                           lorgqr, childinfo )
              end if
              if( wantu2 .and. m-p > 0_ilp64 ) then
                 call stdlib_I64_zlacpy( 'L', m-p, q, x21, ldx21, u2, ldu2 )
              ...
              if( wantv1t .and. q > 0_ilp64 ) then
                 call stdlib_I64_zlacpy( 'U', p, q, x11, ldx11, v1t, ldv1t )
                 call stdlib_I64_zunglq( q, q, r, v1t, ldv1t, work(itauq1),work(iorglq), lorglq, &
                           childinfo )
              end if
              ! simultaneously diagonalize x11 and x21.
              call stdlib_I64_zbbcsd( jobv1t, 'N', jobu1, jobu2, 'T', m, q, p, theta,rwork(iphi), v1t,&
               ldv1t, cdum, 1_ilp64, u1, ldu1, u2,ldu2, rwork(ib11d), rwork(ib11e), rwork(ib12d),rwork(&
               ib12e), rwork(ib21d), rwork(ib21e),rwork(ib22d), rwork(ib22e), rwork(ibbcsd), &
                         lbbcsd,childinfo )
              ! permute rows and columns to place identity submatrices in
              ...
              ! case 3: r = m-p
              ! simultaneously bidiagonalize x11 and x21
              call stdlib_I64_zunbdb3( m, p, q, x11, ldx11, x21, ldx21, theta,rwork(iphi), work(&
                        itaup1), work(itaup2),work(itauq1), work(iorbdb), lorbdb, childinfo )
              ! accumulate householder reflectors
              if( wantu1 .and. p > 0_ilp64 ) then
                 call stdlib_I64_zlacpy( 'L', p, q, x11, ldx11, u1, ldu1 )
              ...
              if( wantu2 .and. m-p > 0_ilp64 ) then
                 ...
                 call stdlib_I64_zlacpy( 'L', m-p-1, m-p-1, x21(2_ilp64,1_ilp64), ldx21, u2(2_ilp64,2_ilp64),ldu2 )
                 call stdlib_I64_zungqr( m-p-1, m-p-1, m-p-1, u2(2_ilp64,2_ilp64), ldu2,work(itaup2), work(iorgqr)&
                           , lorgqr, childinfo )
              end if
              if( wantv1t .and. q > 0_ilp64 ) then
                 call stdlib_I64_zlacpy( 'U', m-p, q, x21, ldx21, v1t, ldv1t )
                 call stdlib_I64_zunglq( q, q, r, v1t, ldv1t, work(itauq1),work(iorglq), lorglq, &
                           childinfo )
              end if
              ! simultaneously diagonalize x11 and x21.
              call stdlib_I64_zbbcsd( 'N', jobv1t, jobu2, jobu1, 'T', m, m-q, m-p,theta, rwork(iphi), &
              cdum, 1_ilp64, v1t, ldv1t, u2, ldu2,u1, ldu1, rwork(ib11d), rwork(ib11e),rwork(ib12d), &
              rwork(ib12e), rwork(ib21d),rwork(ib21e), rwork(ib22d), rwork(ib22e),rwork(ibbcsd), &
                        lbbcsd, childinfo )
              ! permute rows and columns to place identity submatrices in
              ...
              if( q > r ) then
                 ...
                 if( wantu1 ) then
                    call stdlib_I64_zlapmt( .false., p, q, u1, ldu1, iwork )
                 end if
                 if( wantv1t ) then
                    call stdlib_I64_zlapmr( .false., q, q, v1t, ldv1t, iwork )
                 end if
              end if
           else
              ! case 4: r = m-q
              ! simultaneously bidiagonalize x11 and x21
              call stdlib_I64_zunbdb4( m, p, q, x11, ldx11, x21, ldx21, theta,rwork(iphi), work(&
              itaup1), work(itaup2),work(itauq1), work(iorbdb), work(iorbdb+m),lorbdb-m, &
                        childinfo )
              ! accumulate householder reflectors
              if( wantu2 .and. m-p > 0_ilp64 ) then
                 call stdlib_I64_zcopy( m-p, work(iorbdb+p), 1_ilp64, u2, 1_ilp64 )
              end if
              if( wantu1 .and. p > 0_ilp64 ) then
                 call stdlib_I64_zcopy( p, work(iorbdb), 1_ilp64, u1, 1_ilp64 )
                 do j = 2, p
                    u1(1_ilp64,j) = czero
                 end do
                 call stdlib_I64_zlacpy( 'L', p-1, m-q-1, x11(2_ilp64,1_ilp64), ldx11, u1(2_ilp64,2_ilp64),ldu1 )
                 call stdlib_I64_zungqr( p, p, m-q, u1, ldu1, work(itaup1),work(iorgqr), lorgqr, &
                           childinfo )
              end if
              if( wantu2 .and. m-p > 0_ilp64 ) then
                 do j = 2, m-p
                 ...
                 call stdlib_I64_zlacpy( 'L', m-p-1, m-q-1, x21(2_ilp64,1_ilp64), ldx21, u2(2_ilp64,2_ilp64),ldu2 )
                 call stdlib_I64_zungqr( m-p, m-p, m-q, u2, ldu2, work(itaup2),work(iorgqr), lorgqr, &
                           childinfo )
              end if
              if( wantv1t .and. q > 0_ilp64 ) then
                 call stdlib_I64_zlacpy( 'U', m-q, q, x21, ldx21, v1t, ldv1t )
                 call stdlib_I64_zlacpy( 'U', p-(m-q), q-(m-q), x11(m-q+1,m-q+1), ldx11,v1t(m-q+1,m-q+&
                           1_ilp64), ldv1t )
                 call stdlib_I64_zlacpy( 'U', -p+q, q-p, x21(m-q+1,p+1), ldx21,v1t(p+1,p+1), ldv1t )
                           
                 call stdlib_I64_zunglq( q, q, q, v1t, ldv1t, work(itauq1),work(iorglq), lorglq, &
                           childinfo )
              end if
              ! simultaneously diagonalize x11 and x21.
              call stdlib_I64_zbbcsd( jobu2, jobu1, 'N', jobv1t, 'N', m, m-p, m-q,theta, rwork(iphi), &
              u2, ldu2, u1, ldu1, cdum, 1_ilp64,v1t, ldv1t, rwork(ib11d), rwork(ib11e),rwork(ib12d), &
              rwork(ib12e), rwork(ib21d),rwork(ib21e), rwork(ib22d), rwork(ib22e),rwork(ibbcsd), &
                        lbbcsd, childinfo )
              ! permute rows and columns to place identity submatrices in
              ...
              if( p > r ) then
                 ...
                 if( wantu1 ) then
                    call stdlib_I64_zlapmt( .false., p, p, u1, ldu1, iwork )
                 end if
                 if( wantv1t ) then
                    call stdlib_I64_zlapmr( .false., p, q, v1t, ldv1t, iwork )
                 end if
              end if
           end if
           return
     ...
     !! implicitly by angles THETA, PHI.
        ...
        ! ====================================================================
           ...
           ! Local Scalars 
           logical(lk) :: colmajor, lquery
           integer(ilp64) :: i, lworkmin, lworkopt
           real(sp) :: z1, z2, z3, z4
           ! Intrinsic Functions
           ! Executable Statements 
           ! test input arguments
           ...
           if( m < 0_ilp64 ) then
              ...
              info = -4_ilp64
           else if( q < 0_ilp64 .or. q > p .or. q > m-p .or.q > m-q ) then
              info = -5_ilp64
           else if( colmajor .and. ldx11 < max( 1_ilp64, p ) ) then
              info = -7_ilp64
           else if( .not.colmajor .and. ldx11 < max( 1_ilp64, q ) ) then
           ...
           if( info /= 0_ilp64 ) then
              call stdlib_I64_xerbla( 'XORBDB', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           if( colmajor ) then
              ...
              do i = 1, q
                 if( i == 1_ilp64 ) then
                    call stdlib_I64_cscal( p-i+1, cmplx( z1, 0.0_sp,KIND=sp), x11(i,i), 1_ilp64 )
                 else
                    call stdlib_I64_cscal( p-i+1, cmplx( z1*cos(phi(i-1)), 0.0_sp,KIND=sp),x11(i,i), &
                              1_ilp64 )
                    call stdlib_I64_caxpy( p-i+1, cmplx( -z1*z3*z4*sin(phi(i-1)),0.0_sp,KIND=sp), x12(&
                              i,i-1), 1_ilp64, x11(i,i), 1_ilp64 )
                 end if
                 if( i == 1_ilp64 ) then
                    call stdlib_I64_cscal( m-p-i+1, cmplx( z2, 0.0_sp,KIND=sp), x21(i,i), 1_ilp64 )
                 else
                    call stdlib_I64_cscal( m-p-i+1, cmplx( z2*cos(phi(i-1)), 0.0_sp,KIND=sp),x21(i,i),&
                               1_ilp64 )
                    call stdlib_I64_caxpy( m-p-i+1, cmplx( -z2*z3*z4*sin(phi(i-1)),0.0_sp,KIND=sp), &
                              x22(i,i-1), 1_ilp64, x21(i,i), 1_ilp64 )
                 end if
                 theta(i) = atan2( stdlib_I64_scnrm2( m-p-i+1, x21(i,i), 1_ilp64 ),stdlib_I64_scnrm2( p-i+1, &
                           x11(i,i), 1_ilp64 ) )
                 if( p > i ) then
                    call stdlib_I64_clarfgp( p-i+1, x11(i,i), x11(i+1,i), 1_ilp64, taup1(i) )
                 else if ( p == i ) then
                    call stdlib_I64_clarfgp( p-i+1, x11(i,i), x11(i,i), 1_ilp64, taup1(i) )
                 end if
                 x11(i,i) = cone
                 if ( m-p > i ) then
                    call stdlib_I64_clarfgp( m-p-i+1, x21(i,i), x21(i+1,i), 1_ilp64,taup2(i) )
                 else if ( m-p == i ) then
                    call stdlib_I64_clarfgp( m-p-i+1, x21(i,i), x21(i,i), 1_ilp64,taup2(i) )
                 end if
                 x21(i,i) = cone
                 if ( q > i ) then
                    call stdlib_I64_clarf( 'L', p-i+1, q-i, x11(i,i), 1_ilp64,conjg(taup1(i)), x11(i,i+1), &
                              ldx11, work )
                    call stdlib_I64_clarf( 'L', m-p-i+1, q-i, x21(i,i), 1_ilp64,conjg(taup2(i)), x21(i,i+1),&
                               ldx21, work )
                 end if
                 if ( m-q+1 > i ) then
                    call stdlib_I64_clarf( 'L', p-i+1, m-q-i+1, x11(i,i), 1_ilp64,conjg(taup1(i)), x12(i,i),&
                               ldx12, work )
                    call stdlib_I64_clarf( 'L', m-p-i+1, m-q-i+1, x21(i,i), 1_ilp64,conjg(taup2(i)), x22(i,&
                              i), ldx22, work )
                 end if
                 if( i < q ) then
                    call stdlib_I64_cscal( q-i, cmplx( -z1*z3*sin(theta(i)), 0.0_sp,KIND=sp),x11(i,i+&
                              1_ilp64), ldx11 )
                    call stdlib_I64_caxpy( q-i, cmplx( z2*z3*cos(theta(i)), 0.0_sp,KIND=sp),x21(i,i+1)&
                              , ldx21, x11(i,i+1), ldx11 )
                 end if
                 call stdlib_I64_cscal( m-q-i+1, cmplx( -z1*z4*sin(theta(i)), 0.0_sp,KIND=sp),x12(i,i)&
                           , ldx12 )
                 call stdlib_I64_caxpy( m-q-i+1, cmplx( z2*z4*cos(theta(i)), 0.0_sp,KIND=sp),x22(i,i),&
                            ldx22, x12(i,i), ldx12 )
                 if( i < q )phi(i) = atan2( stdlib_I64_scnrm2( q-i, x11(i,i+1), ldx11 ),stdlib_I64_scnrm2(&
                            m-q-i+1, x12(i,i), ldx12 ) )
                 if( i < q ) then
                    call stdlib_I64_clacgv( q-i, x11(i,i+1), ldx11 )
                    if ( i == q-1 ) then
                       call stdlib_I64_clarfgp( q-i, x11(i,i+1), x11(i,i+1), ldx11,tauq1(i) )
                    else
                       call stdlib_I64_clarfgp( q-i, x11(i,i+1), x11(i,i+2), ldx11,tauq1(i) )
                    end if
                    x11(i,i+1) = cone
                 end if
                 if ( m-q+1 > i ) then
                    call stdlib_I64_clacgv( m-q-i+1, x12(i,i), ldx12 )
                    if ( m-q == i ) then
                       call stdlib_I64_clarfgp( m-q-i+1, x12(i,i), x12(i,i), ldx12,tauq2(i) )
                    else
                       call stdlib_I64_clarfgp( m-q-i+1, x12(i,i), x12(i,i+1), ldx12,tauq2(i) )
                                 
                    end if
                 end if
                 x12(i,i) = cone
                 if( i < q ) then
                    call stdlib_I64_clarf( 'R', p-i, q-i, x11(i,i+1), ldx11, tauq1(i),x11(i+1,i+1), &
                              ldx11, work )
                    call stdlib_I64_clarf( 'R', m-p-i, q-i, x11(i,i+1), ldx11, tauq1(i),x21(i+1,i+1), &
                              ldx21, work )
                 end if
                 if ( p > i ) then
                    call stdlib_I64_clarf( 'R', p-i, m-q-i+1, x12(i,i), ldx12, tauq2(i),x12(i+1,i), &
                              ldx12, work )
                 end if
                 if ( m-p > i ) then
                    call stdlib_I64_clarf( 'R', m-p-i, m-q-i+1, x12(i,i), ldx12,tauq2(i), x22(i+1,i), &
                              ldx22, work )
                 end if
                 if( i < q )call stdlib_I64_clacgv( q-i, x11(i,i+1), ldx11 )
                 call stdlib_I64_clacgv( m-q-i+1, x12(i,i), ldx12 )
              end do
              ! reduce columns q + 1, ..., p of x12, x22
              do i = q + 1, p
                 call stdlib_I64_cscal( m-q-i+1, cmplx( -z1*z4, 0.0_sp,KIND=sp), x12(i,i),ldx12 )
                 ...
                 if( m-p-q >= 1_ilp64 )call stdlib_I64_clarf( 'R', m-p-q, m-q-i+1, x12(i,i), ldx12,tauq2(i),&
                            x22(q+1,i), ldx22, work )
                 call stdlib_I64_clacgv( m-q-i+1, x12(i,i), ldx12 )
              end do
              ! reduce columns p + 1, ..., m - q of x12, x22
              do i = 1, m - p - q
                 call stdlib_I64_cscal( m-p-q-i+1, cmplx( z2*z4, 0.0_sp,KIND=sp),x22(q+i,p+i), ldx22 )
                           
                 call stdlib_I64_clacgv( m-p-q-i+1, x22(q+i,p+i), ldx22 )
                 call stdlib_I64_clarfgp( m-p-q-i+1, x22(q+i,p+i), x22(q+i,p+i+1),ldx22, tauq2(p+i) )
                           
                 x22(q+i,p+i) = cone
                 call stdlib_I64_clarf( 'R', m-p-q-i, m-p-q-i+1, x22(q+i,p+i), ldx22,tauq2(p+i), x22(&
                           q+i+1,p+i), ldx22, work )
                 call stdlib_I64_clacgv( m-p-q-i+1, x22(q+i,p+i), ldx22 )
              end do
           else
              ...
              do i = 1, q
                 if( i == 1_ilp64 ) then
                    call stdlib_I64_cscal( p-i+1, cmplx( z1, 0.0_sp,KIND=sp), x11(i,i),ldx11 )
                 else
                    call stdlib_I64_cscal( p-i+1, cmplx( z1*cos(phi(i-1)), 0.0_sp,KIND=sp),x11(i,i), &
                              ldx11 )
                    call stdlib_I64_caxpy( p-i+1, cmplx( -z1*z3*z4*sin(phi(i-1)),0.0_sp,KIND=sp), x12(&
                 ...
                 if( i == 1_ilp64 ) then
                    call stdlib_I64_cscal( m-p-i+1, cmplx( z2, 0.0_sp,KIND=sp), x21(i,i),ldx21 )
                 ...
                 theta(i) = atan2( stdlib_I64_scnrm2( m-p-i+1, x21(i,i), ldx21 ),stdlib_I64_scnrm2( p-i+1,&
                            x11(i,i), ldx11 ) )
                 call stdlib_I64_clacgv( p-i+1, x11(i,i), ldx11 )
                 call stdlib_I64_clacgv( m-p-i+1, x21(i,i), ldx21 )
                 call stdlib_I64_clarfgp( p-i+1, x11(i,i), x11(i,i+1), ldx11, taup1(i) )
                 x11(i,i) = cone
                 if ( i == m-p ) then
                    call stdlib_I64_clarfgp( m-p-i+1, x21(i,i), x21(i,i), ldx21,taup2(i) )
                 else
                    call stdlib_I64_clarfgp( m-p-i+1, x21(i,i), x21(i,i+1), ldx21,taup2(i) )
                 end if
                 x21(i,i) = cone
                 call stdlib_I64_clarf( 'R', q-i, p-i+1, x11(i,i), ldx11, taup1(i),x11(i+1,i), ldx11, &
                           work )
                 call stdlib_I64_clarf( 'R', m-q-i+1, p-i+1, x11(i,i), ldx11, taup1(i),x12(i,i), &
                           ldx12, work )
                 call stdlib_I64_clarf( 'R', q-i, m-p-i+1, x21(i,i), ldx21, taup2(i),x21(i+1,i), &
                           ldx21, work )
                 call stdlib_I64_clarf( 'R', m-q-i+1, m-p-i+1, x21(i,i), ldx21,taup2(i), x22(i,i), &
                           ldx22, work )
                 call stdlib_I64_clacgv( p-i+1, x11(i,i), ldx11 )
                 call stdlib_I64_clacgv( m-p-i+1, x21(i,i), ldx21 )
                 if( i < q ) then
                    call stdlib_I64_cscal( q-i, cmplx( -z1*z3*sin(theta(i)), 0.0_sp,KIND=sp),x11(i+1,&
                              i), 1_ilp64 )
                    call stdlib_I64_caxpy( q-i, cmplx( z2*z3*cos(theta(i)), 0.0_sp,KIND=sp),x21(i+1,i)&
                              , 1_ilp64, x11(i+1,i), 1_ilp64 )
                 end if
                 call stdlib_I64_cscal( m-q-i+1, cmplx( -z1*z4*sin(theta(i)), 0.0_sp,KIND=sp),x12(i,i)&
                           , 1_ilp64 )
                 call stdlib_I64_caxpy( m-q-i+1, cmplx( z2*z4*cos(theta(i)), 0.0_sp,KIND=sp),x22(i,i),&
                            1_ilp64, x12(i,i), 1_ilp64 )
                 if( i < q )phi(i) = atan2( stdlib_I64_scnrm2( q-i, x11(i+1,i), 1_ilp64 ),stdlib_I64_scnrm2( m-&
                           q-i+1, x12(i,i), 1_ilp64 ) )
                 if( i < q ) then
                    call stdlib_I64_clarfgp( q-i, x11(i+1,i), x11(i+2,i), 1_ilp64, tauq1(i) )
                    x11(i+1,i) = cone
                 end if
                 call stdlib_I64_clarfgp( m-q-i+1, x12(i,i), x12(i+1,i), 1_ilp64, tauq2(i) )
                 x12(i,i) = cone
                 if( i < q ) then
                    call stdlib_I64_clarf( 'L', q-i, p-i, x11(i+1,i), 1_ilp64,conjg(tauq1(i)), x11(i+1,i+1),&
                               ldx11, work )
                    call stdlib_I64_clarf( 'L', q-i, m-p-i, x11(i+1,i), 1_ilp64,conjg(tauq1(i)), x21(i+1,i+&
                              1_ilp64), ldx21, work )
                 end if
                 call stdlib_I64_clarf( 'L', m-q-i+1, p-i, x12(i,i), 1_ilp64, conjg(tauq2(i)),x12(i,i+1), &
                           ldx12, work )
                 if ( m-p > i ) then
                    call stdlib_I64_clarf( 'L', m-q-i+1, m-p-i, x12(i,i), 1_ilp64,conjg(tauq2(i)), x22(i,i+&
                              1_ilp64), ldx22, work )
                 end if
              end do
              ! reduce columns q + 1, ..., p of x12, x22
              do i = q + 1, p
                 call stdlib_I64_cscal( m-q-i+1, cmplx( -z1*z4, 0.0_sp,KIND=sp), x12(i,i), 1_ilp64 )
                 call stdlib_I64_clarfgp( m-q-i+1, x12(i,i), x12(i+1,i), 1_ilp64, tauq2(i) )
                 x12(i,i) = cone
                 if ( p > i ) then
                    call stdlib_I64_clarf( 'L', m-q-i+1, p-i, x12(i,i), 1_ilp64,conjg(tauq2(i)), x12(i,i+1),&
                 ...
                 if( m-p-q >= 1_ilp64 )call stdlib_I64_clarf( 'L', m-q-i+1, m-p-q, x12(i,i), 1_ilp64,conjg(tauq2(&
                           i)), x22(i,q+1), ldx22, work )
              end do
              ! reduce columns p + 1, ..., m - q of x12, x22
              do i = 1, m - p - q
                 call stdlib_I64_cscal( m-p-q-i+1, cmplx( z2*z4, 0.0_sp,KIND=sp),x22(p+i,q+i), 1_ilp64 )
                           
                 call stdlib_I64_clarfgp( m-p-q-i+1, x22(p+i,q+i), x22(p+i+1,q+i), 1_ilp64,tauq2(p+i) )
                           
                 x22(p+i,q+i) = cone
                 if ( m-p-q /= i ) then
                    call stdlib_I64_clarf( 'L', m-p-q-i+1, m-p-q-i, x22(p+i,q+i), 1_ilp64,conjg(tauq2(p+i)),&
                               x22(p+i,q+i+1), ldx22,work )
                 end if
              end do
           end if
     ...
     !! implicitly by angles THETA, PHI.
        ...
        ! ====================================================================
           ...
           if( colmajor ) then
              ...
              do i = 1, q
                 if( i == 1_ilp64 ) then
                    call stdlib_I64_zscal( p-i+1, cmplx( z1, 0.0_dp,KIND=dp), x11(i,i), 1_ilp64 )
                 else
                    call stdlib_I64_zscal( p-i+1, cmplx( z1*cos(phi(i-1)), 0.0_dp,KIND=dp),x11(i,i), &
                              1_ilp64 )
                    call stdlib_I64_zaxpy( p-i+1, cmplx( -z1*z3*z4*sin(phi(i-1)),0.0_dp,KIND=dp), x12(&
                              i,i-1), 1_ilp64, x11(i,i), 1_ilp64 )
                 end if
                 if( i == 1_ilp64 ) then
                    call stdlib_I64_zscal( m-p-i+1, cmplx( z2, 0.0_dp,KIND=dp), x21(i,i), 1_ilp64 )
                 else
                    call stdlib_I64_zscal( m-p-i+1, cmplx( z2*cos(phi(i-1)), 0.0_dp,KIND=dp),x21(i,i),&
                               1_ilp64 )
                    call stdlib_I64_zaxpy( m-p-i+1, cmplx( -z2*z3*z4*sin(phi(i-1)),0.0_dp,KIND=dp), &
                              x22(i,i-1), 1_ilp64, x21(i,i), 1_ilp64 )
                 end if
                 theta(i) = atan2( stdlib_I64_dznrm2( m-p-i+1, x21(i,i), 1_ilp64 ),stdlib_I64_dznrm2( p-i+1, &
                           x11(i,i), 1_ilp64 ) )
                 if( p > i ) then
                    call stdlib_I64_zlarfgp( p-i+1, x11(i,i), x11(i+1,i), 1_ilp64, taup1(i) )
                 else if ( p == i ) then
                    call stdlib_I64_zlarfgp( p-i+1, x11(i,i), x11(i,i), 1_ilp64, taup1(i) )
                 end if
                 x11(i,i) = cone
                 if ( m-p > i ) then
                    call stdlib_I64_zlarfgp( m-p-i+1, x21(i,i), x21(i+1,i), 1_ilp64,taup2(i) )
                 else if ( m-p == i ) then
                    call stdlib_I64_zlarfgp( m-p-i+1, x21(i,i), x21(i,i), 1_ilp64,taup2(i) )
                 end if
                 x21(i,i) = cone
                 if ( q > i ) then
                    call stdlib_I64_zlarf( 'L', p-i+1, q-i, x11(i,i), 1_ilp64,conjg(taup1(i)), x11(i,i+1), &
                              ldx11, work )
                    call stdlib_I64_zlarf( 'L', m-p-i+1, q-i, x21(i,i), 1_ilp64,conjg(taup2(i)), x21(i,i+1),&
                               ldx21, work )
                 end if
                 if ( m-q+1 > i ) then
                    call stdlib_I64_zlarf( 'L', p-i+1, m-q-i+1, x11(i,i), 1_ilp64,conjg(taup1(i)), x12(i,i),&
                               ldx12, work )
                    call stdlib_I64_zlarf( 'L', m-p-i+1, m-q-i+1, x21(i,i), 1_ilp64,conjg(taup2(i)), x22(i,&
                              i), ldx22, work )
                 end if
                 if( i < q ) then
                    call stdlib_I64_zscal( q-i, cmplx( -z1*z3*sin(theta(i)), 0.0_dp,KIND=dp),x11(i,i+&
                              1_ilp64), ldx11 )
                    call stdlib_I64_zaxpy( q-i, cmplx( z2*z3*cos(theta(i)), 0.0_dp,KIND=dp),x21(i,i+1)&
                              , ldx21, x11(i,i+1), ldx11 )
                 end if
                 call stdlib_I64_zscal( m-q-i+1, cmplx( -z1*z4*sin(theta(i)), 0.0_dp,KIND=dp),x12(i,i)&
                           , ldx12 )
                 call stdlib_I64_zaxpy( m-q-i+1, cmplx( z2*z4*cos(theta(i)), 0.0_dp,KIND=dp),x22(i,i),&
                            ldx22, x12(i,i), ldx12 )
                 if( i < q )phi(i) = atan2( stdlib_I64_dznrm2( q-i, x11(i,i+1), ldx11 ),stdlib_I64_dznrm2(&
                            m-q-i+1, x12(i,i), ldx12 ) )
                 if( i < q ) then
                    call stdlib_I64_zlacgv( q-i, x11(i,i+1), ldx11 )
                    if ( i == q-1 ) then
                       call stdlib_I64_zlarfgp( q-i, x11(i,i+1), x11(i,i+1), ldx11,tauq1(i) )
                    else
                       call stdlib_I64_zlarfgp( q-i, x11(i,i+1), x11(i,i+2), ldx11,tauq1(i) )
                    end if
                    x11(i,i+1) = cone
                 end if
                 if ( m-q+1 > i ) then
                    call stdlib_I64_zlacgv( m-q-i+1, x12(i,i), ldx12 )
                    if ( m-q == i ) then
                       call stdlib_I64_zlarfgp( m-q-i+1, x12(i,i), x12(i,i), ldx12,tauq2(i) )
                    else
                       call stdlib_I64_zlarfgp( m-q-i+1, x12(i,i), x12(i,i+1), ldx12,tauq2(i) )
                                 
                    end if
                 end if
                 x12(i,i) = cone
                 if( i < q ) then
                    call stdlib_I64_zlarf( 'R', p-i, q-i, x11(i,i+1), ldx11, tauq1(i),x11(i+1,i+1), &
                              ldx11, work )
                    call stdlib_I64_zlarf( 'R', m-p-i, q-i, x11(i,i+1), ldx11, tauq1(i),x21(i+1,i+1), &
                              ldx21, work )
                 end if
                 if ( p > i ) then
                    call stdlib_I64_zlarf( 'R', p-i, m-q-i+1, x12(i,i), ldx12, tauq2(i),x12(i+1,i), &
                              ldx12, work )
                 end if
                 if ( m-p > i ) then
                    call stdlib_I64_zlarf( 'R', m-p-i, m-q-i+1, x12(i,i), ldx12,tauq2(i), x22(i+1,i), &
                              ldx22, work )
                 end if
                 if( i < q )call stdlib_I64_zlacgv( q-i, x11(i,i+1), ldx11 )
                 call stdlib_I64_zlacgv( m-q-i+1, x12(i,i), ldx12 )
              end do
              ! reduce columns q + 1, ..., p of x12, x22
              do i = q + 1, p
                 call stdlib_I64_zscal( m-q-i+1, cmplx( -z1*z4, 0.0_dp,KIND=dp), x12(i,i),ldx12 )
                 ...
                 if( m-p-q >= 1_ilp64 )call stdlib_I64_zlarf( 'R', m-p-q, m-q-i+1, x12(i,i), ldx12,tauq2(i),&
                            x22(q+1,i), ldx22, work )
                 call stdlib_I64_zlacgv( m-q-i+1, x12(i,i), ldx12 )
              end do
              ! reduce columns p + 1, ..., m - q of x12, x22
              do i = 1, m - p - q
                 call stdlib_I64_zscal( m-p-q-i+1, cmplx( z2*z4, 0.0_dp,KIND=dp),x22(q+i,p+i), ldx22 )
                           
                 call stdlib_I64_zlacgv( m-p-q-i+1, x22(q+i,p+i), ldx22 )
                 call stdlib_I64_zlarfgp( m-p-q-i+1, x22(q+i,p+i), x22(q+i,p+i+1),ldx22, tauq2(p+i) )
                           
                 x22(q+i,p+i) = cone
                 call stdlib_I64_zlarf( 'R', m-p-q-i, m-p-q-i+1, x22(q+i,p+i), ldx22,tauq2(p+i), x22(&
                           q+i+1,p+i), ldx22, work )
                 call stdlib_I64_zlacgv( m-p-q-i+1, x22(q+i,p+i), ldx22 )
              end do
           else
              ...
              do i = 1, q
                 if( i == 1_ilp64 ) then
                    call stdlib_I64_zscal( p-i+1, cmplx( z1, 0.0_dp,KIND=dp), x11(i,i),ldx11 )
                 else
                    call stdlib_I64_zscal( p-i+1, cmplx( z1*cos(phi(i-1)), 0.0_dp,KIND=dp),x11(i,i), &
                              ldx11 )
                    call stdlib_I64_zaxpy( p-i+1, cmplx( -z1*z3*z4*sin(phi(i-1)),0.0_dp,KIND=dp), x12(&
                 ...
                 if( i == 1_ilp64 ) then
                    call stdlib_I64_zscal( m-p-i+1, cmplx( z2, 0.0_dp,KIND=dp), x21(i,i),ldx21 )
                 ...
                 theta(i) = atan2( stdlib_I64_dznrm2( m-p-i+1, x21(i,i), ldx21 ),stdlib_I64_dznrm2( p-i+1,&
                            x11(i,i), ldx11 ) )
                 call stdlib_I64_zlacgv( p-i+1, x11(i,i), ldx11 )
                 call stdlib_I64_zlacgv( m-p-i+1, x21(i,i), ldx21 )
                 call stdlib_I64_zlarfgp( p-i+1, x11(i,i), x11(i,i+1), ldx11, taup1(i) )
                 x11(i,i) = cone
                 if ( i == m-p ) then
                    call stdlib_I64_zlarfgp( m-p-i+1, x21(i,i), x21(i,i), ldx21,taup2(i) )
                 else
                    call stdlib_I64_zlarfgp( m-p-i+1, x21(i,i), x21(i,i+1), ldx21,taup2(i) )
                 end if
                 x21(i,i) = cone
                 call stdlib_I64_zlarf( 'R', q-i, p-i+1, x11(i,i), ldx11, taup1(i),x11(i+1,i), ldx11, &
                           work )
                 call stdlib_I64_zlarf( 'R', m-q-i+1, p-i+1, x11(i,i), ldx11, taup1(i),x12(i,i), &
                           ldx12, work )
                 call stdlib_I64_zlarf( 'R', q-i, m-p-i+1, x21(i,i), ldx21, taup2(i),x21(i+1,i), &
                           ldx21, work )
                 call stdlib_I64_zlarf( 'R', m-q-i+1, m-p-i+1, x21(i,i), ldx21,taup2(i), x22(i,i), &
                           ldx22, work )
                 call stdlib_I64_zlacgv( p-i+1, x11(i,i), ldx11 )
                 call stdlib_I64_zlacgv( m-p-i+1, x21(i,i), ldx21 )
                 if( i < q ) then
                    call stdlib_I64_zscal( q-i, cmplx( -z1*z3*sin(theta(i)), 0.0_dp,KIND=dp),x11(i+1,&
                              i), 1_ilp64 )
                    call stdlib_I64_zaxpy( q-i, cmplx( z2*z3*cos(theta(i)), 0.0_dp,KIND=dp),x21(i+1,i)&
                              , 1_ilp64, x11(i+1,i), 1_ilp64 )
                 end if
                 call stdlib_I64_zscal( m-q-i+1, cmplx( -z1*z4*sin(theta(i)), 0.0_dp,KIND=dp),x12(i,i)&
                           , 1_ilp64 )
                 call stdlib_I64_zaxpy( m-q-i+1, cmplx( z2*z4*cos(theta(i)), 0.0_dp,KIND=dp),x22(i,i),&
                            1_ilp64, x12(i,i), 1_ilp64 )
                 if( i < q )phi(i) = atan2( stdlib_I64_dznrm2( q-i, x11(i+1,i), 1_ilp64 ),stdlib_I64_dznrm2( m-&
                           q-i+1, x12(i,i), 1_ilp64 ) )
                 if( i < q ) then
                    call stdlib_I64_zlarfgp( q-i, x11(i+1,i), x11(i+2,i), 1_ilp64, tauq1(i) )
                    x11(i+1,i) = cone
                 end if
                 call stdlib_I64_zlarfgp( m-q-i+1, x12(i,i), x12(i+1,i), 1_ilp64, tauq2(i) )
                 x12(i,i) = cone
                 if( i < q ) then
                    call stdlib_I64_zlarf( 'L', q-i, p-i, x11(i+1,i), 1_ilp64,conjg(tauq1(i)), x11(i+1,i+1),&
                               ldx11, work )
                    call stdlib_I64_zlarf( 'L', q-i, m-p-i, x11(i+1,i), 1_ilp64,conjg(tauq1(i)), x21(i+1,i+&
                              1_ilp64), ldx21, work )
                 end if
                 call stdlib_I64_zlarf( 'L', m-q-i+1, p-i, x12(i,i), 1_ilp64,conjg(tauq2(i)), x12(i,i+1), &
                           ldx12, work )
                 if ( m-p > i ) then
                    call stdlib_I64_zlarf( 'L', m-q-i+1, m-p-i, x12(i,i), 1_ilp64,conjg(tauq2(i)), x22(i,i+&
                              1_ilp64), ldx22, work )
                 end if
              end do
              ! reduce columns q + 1, ..., p of x12, x22
              do i = q + 1, p
                 call stdlib_I64_zscal( m-q-i+1, cmplx( -z1*z4, 0.0_dp,KIND=dp), x12(i,i), 1_ilp64 )
                 call stdlib_I64_zlarfgp( m-q-i+1, x12(i,i), x12(i+1,i), 1_ilp64, tauq2(i) )
                 x12(i,i) = cone
                 if ( p > i ) then
                    call stdlib_I64_zlarf( 'L', m-q-i+1, p-i, x12(i,i), 1_ilp64,conjg(tauq2(i)), x12(i,i+1),&
                 ...
                 if( m-p-q >= 1_ilp64 )call stdlib_I64_zlarf( 'L', m-q-i+1, m-p-q, x12(i,i), 1_ilp64,conjg(tauq2(&
                           i)), x22(i,q+1), ldx22, work )
              end do
              ! reduce columns p + 1, ..., m - q of x12, x22
              do i = 1, m - p - q
                 call stdlib_I64_zscal( m-p-q-i+1, cmplx( z2*z4, 0.0_dp,KIND=dp),x22(p+i,q+i), 1_ilp64 )
                           
                 call stdlib_I64_zlarfgp( m-p-q-i+1, x22(p+i,q+i), x22(p+i+1,q+i), 1_ilp64,tauq2(p+i) )
                           
                 x22(p+i,q+i) = cone
                 if ( m-p-q /= i ) then
                    call stdlib_I64_zlarf( 'L', m-p-q-i+1, m-p-q-i, x22(p+i,q+i), 1_ilp64,conjg(tauq2(p+i)),&
                               x22(p+i,q+i+1), ldx22,work )
                 end if
              end do
           end if
     ...
     !! angles THETA, PHI.
        ...
        ! ====================================================================
           ...
           ! Local Scalars 
           real(sp) :: c, s
           integer(ilp64) :: childinfo, i, ilarf, iorbdb5, llarf, lorbdb5, lworkmin, &
                     lworkopt
           logical(lk) :: lquery
           ! Intrinsic Function 
           ! Executable Statements 
           ...
           if( m < 0_ilp64 ) then
              ...
              info = -2_ilp64
           else if( q < 0_ilp64 .or. m-q < q ) then
              info = -3_ilp64
           else if( ldx11 < max( 1_ilp64, p ) ) then
              info = -5_ilp64
           else if( ldx21 < max( 1_ilp64, m-p ) ) then
           ...
           if( info /= 0_ilp64 ) then
              call stdlib_I64_xerbla( 'CUNBDB1', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           do i = 1, q
              ...
              x11(i,i) = cone
              x21(i,i) = cone
              call stdlib_I64_clarf( 'L', p-i+1, q-i, x11(i,i), 1_ilp64, conjg(taup1(i)),x11(i,i+1), ldx11, &
                        work(ilarf) )
              call stdlib_I64_clarf( 'L', m-p-i+1, q-i, x21(i,i), 1_ilp64, conjg(taup2(i)),x21(i,i+1), &
                        ldx21, work(ilarf) )
              if( i < q ) then
                 call stdlib_I64_csrot( q-i, x11(i,i+1), ldx11, x21(i,i+1), ldx21, c,s )
                 call stdlib_I64_clacgv( q-i, x21(i,i+1), ldx21 )
                 call stdlib_I64_clarfgp( q-i, x21(i,i+1), x21(i,i+2), ldx21, tauq1(i) )
                 s = real( x21(i,i+1),KIND=sp)
                 x21(i,i+1) = cone
                 call stdlib_I64_clarf( 'R', p-i, q-i, x21(i,i+1), ldx21, tauq1(i),x11(i+1,i+1), &
                           ldx11, work(ilarf) )
                 call stdlib_I64_clarf( 'R', m-p-i, q-i, x21(i,i+1), ldx21, tauq1(i),x21(i+1,i+1), &
                           ldx21, work(ilarf) )
                 call stdlib_I64_clacgv( q-i, x21(i,i+1), ldx21 )
                 c = sqrt( stdlib_I64_scnrm2( p-i, x11(i+1,i+1), 1_ilp64 )**2_ilp64+ stdlib_I64_scnrm2( m-p-i, x21(i+&
                           1_ilp64,i+1), 1_ilp64 )**2_ilp64 )
                 phi(i) = atan2( s, c )
                 call stdlib_I64_cunbdb5( p-i, m-p-i, q-i-1, x11(i+1,i+1), 1_ilp64,x21(i+1,i+1), 1_ilp64, x11(i+1,&
                           i+2), ldx11,x21(i+1,i+2), ldx21, work(iorbdb5), lorbdb5,childinfo )
              end if
           end do
           return
     ...
     !! angles THETA, PHI.
        ...
        ! ====================================================================
           ...
           if( info /= 0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZUNBDB1', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           do i = 1, q
              ...
              x11(i,i) = cone
              x21(i,i) = cone
              call stdlib_I64_zlarf( 'L', p-i+1, q-i, x11(i,i), 1_ilp64, conjg(taup1(i)),x11(i,i+1), ldx11, &
                        work(ilarf) )
              call stdlib_I64_zlarf( 'L', m-p-i+1, q-i, x21(i,i), 1_ilp64, conjg(taup2(i)),x21(i,i+1), &
                        ldx21, work(ilarf) )
              if( i < q ) then
                 call stdlib_I64_zdrot( q-i, x11(i,i+1), ldx11, x21(i,i+1), ldx21, c,s )
                 call stdlib_I64_zlacgv( q-i, x21(i,i+1), ldx21 )
                 call stdlib_I64_zlarfgp( q-i, x21(i,i+1), x21(i,i+2), ldx21, tauq1(i) )
                 s = real( x21(i,i+1),KIND=dp)
                 x21(i,i+1) = cone
                 call stdlib_I64_zlarf( 'R', p-i, q-i, x21(i,i+1), ldx21, tauq1(i),x11(i+1,i+1), &
                           ldx11, work(ilarf) )
                 call stdlib_I64_zlarf( 'R', m-p-i, q-i, x21(i,i+1), ldx21, tauq1(i),x21(i+1,i+1), &
                           ldx21, work(ilarf) )
                 call stdlib_I64_zlacgv( q-i, x21(i,i+1), ldx21 )
                 c = sqrt( stdlib_I64_dznrm2( p-i, x11(i+1,i+1), 1_ilp64 )**2_ilp64+ stdlib_I64_dznrm2( m-p-i, x21(i+&
                           1_ilp64,i+1), 1_ilp64 )**2_ilp64 )
                 phi(i) = atan2( s, c )
                 call stdlib_I64_zunbdb5( p-i, m-p-i, q-i-1, x11(i+1,i+1), 1_ilp64,x21(i+1,i+1), 1_ilp64, x11(i+1,&
                           i+2), ldx11,x21(i+1,i+2), ldx21, work(iorbdb5), lorbdb5,childinfo )
              end if
           end do
           return
     ...
     !! angles THETA, PHI.
        ...
        ! ====================================================================
           ...
           if( m < 0_ilp64 ) then
              info = -1_ilp64
           else if( p < 0_ilp64 .or. p > m-p ) then
              info = -2_ilp64
           else if( q < 0_ilp64 .or. q < p .or. m-q < p ) then
              info = -3_ilp64
           else if( ldx11 < max( 1_ilp64, p ) ) then
              info = -5_ilp64
           else if( ldx21 < max( 1_ilp64, m-p ) ) then
           ...
           if( info /= 0_ilp64 ) then
              call stdlib_I64_xerbla( 'CUNBDB2', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           do i = 1, p
              if( i > 1_ilp64 ) then
                 call stdlib_I64_csrot( q-i+1, x11(i,i), ldx11, x21(i-1,i), ldx21, c,s )
              end if
              call stdlib_I64_clacgv( q-i+1, x11(i,i), ldx11 )
              call stdlib_I64_clarfgp( q-i+1, x11(i,i), x11(i,i+1), ldx11, tauq1(i) )
              c = real( x11(i,i),KIND=sp)
              x11(i,i) = cone
              call stdlib_I64_clarf( 'R', p-i, q-i+1, x11(i,i), ldx11, tauq1(i),x11(i+1,i), ldx11, &
                        work(ilarf) )
              call stdlib_I64_clarf( 'R', m-p-i+1, q-i+1, x11(i,i), ldx11, tauq1(i),x21(i,i), ldx21, &
                        work(ilarf) )
              call stdlib_I64_clacgv( q-i+1, x11(i,i), ldx11 )
              s = sqrt( stdlib_I64_scnrm2( p-i, x11(i+1,i), 1_ilp64 )**2_ilp64+ stdlib_I64_scnrm2( m-p-i+1, x21(i,i), &
                        1_ilp64 )**2_ilp64 )
              theta(i) = atan2( s, c )
              call stdlib_I64_cunbdb5( p-i, m-p-i+1, q-i, x11(i+1,i), 1_ilp64, x21(i,i), 1_ilp64,x11(i+1,i+1), &
                        ldx11, x21(i,i+1), ldx21,work(iorbdb5), lorbdb5, childinfo )
              call stdlib_I64_cscal( p-i, cnegone, x11(i+1,i), 1_ilp64 )
              call stdlib_I64_clarfgp( m-p-i+1, x21(i,i), x21(i+1,i), 1_ilp64, taup2(i) )
              if( i < p ) then
                 call stdlib_I64_clarfgp( p-i, x11(i+1,i), x11(i+2,i), 1_ilp64, taup1(i) )
                 phi(i) = atan2( real( x11(i+1,i),KIND=sp), real( x21(i,i),KIND=sp) )
                 c = cos( phi(i) )
                 s = sin( phi(i) )
                 x11(i+1,i) = cone
                 call stdlib_I64_clarf( 'L', p-i, q-i, x11(i+1,i), 1_ilp64, conjg(taup1(i)),x11(i+1,i+1), &
                           ldx11, work(ilarf) )
              end if
              x21(i,i) = cone
              call stdlib_I64_clarf( 'L', m-p-i+1, q-i, x21(i,i), 1_ilp64, conjg(taup2(i)),x21(i,i+1), &
     ...
     !! angles THETA, PHI.
        ...
        ! ====================================================================
           ...
           if( info /= 0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZUNBDB2', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           do i = 1, p
              if( i > 1_ilp64 ) then
                 call stdlib_I64_zdrot( q-i+1, x11(i,i), ldx11, x21(i-1,i), ldx21, c,s )
              end if
              call stdlib_I64_zlacgv( q-i+1, x11(i,i), ldx11 )
              call stdlib_I64_zlarfgp( q-i+1, x11(i,i), x11(i,i+1), ldx11, tauq1(i) )
              c = real( x11(i,i),KIND=dp)
              x11(i,i) = cone
              call stdlib_I64_zlarf( 'R', p-i, q-i+1, x11(i,i), ldx11, tauq1(i),x11(i+1,i), ldx11, &
                        work(ilarf) )
              call stdlib_I64_zlarf( 'R', m-p-i+1, q-i+1, x11(i,i), ldx11, tauq1(i),x21(i,i), ldx21, &
                        work(ilarf) )
              call stdlib_I64_zlacgv( q-i+1, x11(i,i), ldx11 )
              s = sqrt( stdlib_I64_dznrm2( p-i, x11(i+1,i), 1_ilp64 )**2_ilp64+ stdlib_I64_dznrm2( m-p-i+1, x21(i,i), &
                        1_ilp64 )**2_ilp64 )
              theta(i) = atan2( s, c )
              call stdlib_I64_zunbdb5( p-i, m-p-i+1, q-i, x11(i+1,i), 1_ilp64, x21(i,i), 1_ilp64,x11(i+1,i+1), &
                        ldx11, x21(i,i+1), ldx21,work(iorbdb5), lorbdb5, childinfo )
              call stdlib_I64_zscal( p-i, cnegone, x11(i+1,i), 1_ilp64 )
              call stdlib_I64_zlarfgp( m-p-i+1, x21(i,i), x21(i+1,i), 1_ilp64, taup2(i) )
              if( i < p ) then
                 call stdlib_I64_zlarfgp( p-i, x11(i+1,i), x11(i+2,i), 1_ilp64, taup1(i) )
                 phi(i) = atan2( real( x11(i+1,i),KIND=dp), real( x21(i,i),KIND=dp) )
                 c = cos( phi(i) )
                 s = sin( phi(i) )
                 x11(i+1,i) = cone
                 call stdlib_I64_zlarf( 'L', p-i, q-i, x11(i+1,i), 1_ilp64, conjg(taup1(i)),x11(i+1,i+1), &
                           ldx11, work(ilarf) )
              end if
              x21(i,i) = cone
              call stdlib_I64_zlarf( 'L', m-p-i+1, q-i, x21(i,i), 1_ilp64, conjg(taup2(i)),x21(i,i+1), &
     ...
     !! implicitly by angles THETA, PHI.
        ...
        ! ====================================================================
           ...
           if( m < 0_ilp64 ) then
              info = -1_ilp64
           else if( 2_ilp64*p < m .or. p > m ) then
              info = -2_ilp64
           else if( q < m-p .or. m-q < m-p ) then
              info = -3_ilp64
           else if( ldx11 < max( 1_ilp64, p ) ) then
           ...
           if( info /= 0_ilp64 ) then
              call stdlib_I64_xerbla( 'CUNBDB3', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           do i = 1, m-p
              if( i > 1_ilp64 ) then
                 call stdlib_I64_csrot( q-i+1, x11(i-1,i), ldx11, x21(i,i), ldx11, c,s )
              end if
              call stdlib_I64_clacgv( q-i+1, x21(i,i), ldx21 )
              call stdlib_I64_clarfgp( q-i+1, x21(i,i), x21(i,i+1), ldx21, tauq1(i) )
              s = real( x21(i,i),KIND=sp)
              x21(i,i) = cone
              call stdlib_I64_clarf( 'R', p-i+1, q-i+1, x21(i,i), ldx21, tauq1(i),x11(i,i), ldx11, &
                        work(ilarf) )
              call stdlib_I64_clarf( 'R', m-p-i, q-i+1, x21(i,i), ldx21, tauq1(i),x21(i+1,i), ldx21, &
                        work(ilarf) )
              call stdlib_I64_clacgv( q-i+1, x21(i,i), ldx21 )
              c = sqrt( stdlib_I64_scnrm2( p-i+1, x11(i,i), 1_ilp64 )**2_ilp64+ stdlib_I64_scnrm2( m-p-i, x21(i+1,i), &
                        1_ilp64 )**2_ilp64 )
              theta(i) = atan2( s, c )
              call stdlib_I64_cunbdb5( p-i+1, m-p-i, q-i, x11(i,i), 1_ilp64, x21(i+1,i), 1_ilp64,x11(i,i+1), &
                        ldx11, x21(i+1,i+1), ldx21,work(iorbdb5), lorbdb5, childinfo )
              call stdlib_I64_clarfgp( p-i+1, x11(i,i), x11(i+1,i), 1_ilp64, taup1(i) )
              if( i < m-p ) then
                 call stdlib_I64_clarfgp( m-p-i, x21(i+1,i), x21(i+2,i), 1_ilp64, taup2(i) )
                 phi(i) = atan2( real( x21(i+1,i),KIND=sp), real( x11(i,i),KIND=sp) )
                 c = cos( phi(i) )
                 s = sin( phi(i) )
                 x21(i+1,i) = cone
                 call stdlib_I64_clarf( 'L', m-p-i, q-i, x21(i+1,i), 1_ilp64, conjg(taup2(i)),x21(i+1,i+1), &
                           ldx21, work(ilarf) )
              end if
              x11(i,i) = cone
              call stdlib_I64_clarf( 'L', p-i+1, q-i, x11(i,i), 1_ilp64, conjg(taup1(i)),x11(i,i+1), ldx11, &
     ...
     !! implicitly by angles THETA, PHI.
        ...
        ! ====================================================================
           ...
           if( info /= 0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZUNBDB3', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           do i = 1, m-p
              if( i > 1_ilp64 ) then
                 call stdlib_I64_zdrot( q-i+1, x11(i-1,i), ldx11, x21(i,i), ldx11, c,s )
              end if
              call stdlib_I64_zlacgv( q-i+1, x21(i,i), ldx21 )
              call stdlib_I64_zlarfgp( q-i+1, x21(i,i), x21(i,i+1), ldx21, tauq1(i) )
              s = real( x21(i,i),KIND=dp)
              x21(i,i) = cone
              call stdlib_I64_zlarf( 'R', p-i+1, q-i+1, x21(i,i), ldx21, tauq1(i),x11(i,i), ldx11, &
                        work(ilarf) )
              call stdlib_I64_zlarf( 'R', m-p-i, q-i+1, x21(i,i), ldx21, tauq1(i),x21(i+1,i), ldx21, &
                        work(ilarf) )
              call stdlib_I64_zlacgv( q-i+1, x21(i,i), ldx21 )
              c = sqrt( stdlib_I64_dznrm2( p-i+1, x11(i,i), 1_ilp64 )**2_ilp64+ stdlib_I64_dznrm2( m-p-i, x21(i+1,i), &
                        1_ilp64 )**2_ilp64 )
              theta(i) = atan2( s, c )
              call stdlib_I64_zunbdb5( p-i+1, m-p-i, q-i, x11(i,i), 1_ilp64, x21(i+1,i), 1_ilp64,x11(i,i+1), &
                        ldx11, x21(i+1,i+1), ldx21,work(iorbdb5), lorbdb5, childinfo )
              call stdlib_I64_zlarfgp( p-i+1, x11(i,i), x11(i+1,i), 1_ilp64, taup1(i) )
              if( i < m-p ) then
                 call stdlib_I64_zlarfgp( m-p-i, x21(i+1,i), x21(i+2,i), 1_ilp64, taup2(i) )
                 phi(i) = atan2( real( x21(i+1,i),KIND=dp), real( x11(i,i),KIND=dp) )
                 c = cos( phi(i) )
                 s = sin( phi(i) )
                 x21(i+1,i) = cone
                 call stdlib_I64_zlarf( 'L', m-p-i, q-i, x21(i+1,i), 1_ilp64,conjg(taup2(i)), x21(i+1,i+1), &
                           ldx21,work(ilarf) )
              end if
              x11(i,i) = cone
              call stdlib_I64_zlarf( 'L', p-i+1, q-i, x11(i,i), 1_ilp64, conjg(taup1(i)),x11(i,i+1), ldx11, &
     ...
     !! implicitly by angles THETA, PHI.
        ...
        ! ====================================================================
           ...
           ! Local Scalars 
           real(sp) :: c, s
           integer(ilp64) :: childinfo, i, ilarf, iorbdb5, j, llarf, lorbdb5, lworkmin, &
                     lworkopt
           logical(lk) :: lquery
           ! Intrinsic Function 
           ! Executable Statements 
           ...
           if( info == 0_ilp64 ) then
              ...
              iorbdb5 = 2_ilp64
              lorbdb5 = q
              lworkopt = ilarf + llarf - 1_ilp64
              lworkopt = max( lworkopt, iorbdb5 + lorbdb5 - 1_ilp64 )
              lworkmin = lworkopt
              work(1_ilp64) = lworkopt
              if( lwork < lworkmin .and. .not.lquery ) then
                info = -14_ilp64
           ...
           if( info /= 0_ilp64 ) then
              call stdlib_I64_xerbla( 'CUNBDB4', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           do i = 1, m-q
              if( i == 1_ilp64 ) then
                 ...
                 call stdlib_I64_cunbdb5( p, m-p, q, phantom(1_ilp64), 1_ilp64, phantom(p+1), 1_ilp64,x11, ldx11, x21, &
                           ldx21, work(iorbdb5),lorbdb5, childinfo )
                 call stdlib_I64_cscal( p, cnegone, phantom(1_ilp64), 1_ilp64 )
                 call stdlib_I64_clarfgp( p, phantom(1_ilp64), phantom(2_ilp64), 1_ilp64, taup1(1_ilp64) )
                 call stdlib_I64_clarfgp( m-p, phantom(p+1), phantom(p+2), 1_ilp64, taup2(1_ilp64) )
                 theta(i) = atan2( real( phantom(1_ilp64),KIND=sp), real( phantom(p+1),KIND=sp) )
                           
                 c = cos( theta(i) )
                 s = sin( theta(i) )
                 phantom(1_ilp64) = cone
                 phantom(p+1) = cone
                 call stdlib_I64_clarf( 'L', p, q, phantom(1_ilp64), 1_ilp64, conjg(taup1(1_ilp64)), x11,ldx11, work(&
                           ilarf) )
                 call stdlib_I64_clarf( 'L', m-p, q, phantom(p+1), 1_ilp64, conjg(taup2(1_ilp64)),x21, ldx21, &
                           work(ilarf) )
              else
                 call stdlib_I64_cunbdb5( p-i+1, m-p-i+1, q-i+1, x11(i,i-1), 1_ilp64,x21(i,i-1), 1_ilp64, x11(i,i)&
                           , ldx11, x21(i,i),ldx21, work(iorbdb5), lorbdb5, childinfo )
                 call stdlib_I64_cscal( p-i+1, cnegone, x11(i,i-1), 1_ilp64 )
                 call stdlib_I64_clarfgp( p-i+1, x11(i,i-1), x11(i+1,i-1), 1_ilp64, taup1(i) )
                 call stdlib_I64_clarfgp( m-p-i+1, x21(i,i-1), x21(i+1,i-1), 1_ilp64,taup2(i) )
                 theta(i) = atan2( real( x11(i,i-1),KIND=sp), real( x21(i,i-1),KIND=sp) )
                 c = cos( theta(i) )
                 s = sin( theta(i) )
                 x11(i,i-1) = cone
                 x21(i,i-1) = cone
                 call stdlib_I64_clarf( 'L', p-i+1, q-i+1, x11(i,i-1), 1_ilp64,conjg(taup1(i)), x11(i,i), &
                           ldx11, work(ilarf) )
                 call stdlib_I64_clarf( 'L', m-p-i+1, q-i+1, x21(i,i-1), 1_ilp64,conjg(taup2(i)), x21(i,i), &
                           ldx21, work(ilarf) )
              end if
              call stdlib_I64_csrot( q-i+1, x11(i,i), ldx11, x21(i,i), ldx21, s, -c )
              call stdlib_I64_clacgv( q-i+1, x21(i,i), ldx21 )
              call stdlib_I64_clarfgp( q-i+1, x21(i,i), x21(i,i+1), ldx21, tauq1(i) )
              c = real( x21(i,i),KIND=sp)
              x21(i,i) = cone
              call stdlib_I64_clarf( 'R', p-i, q-i+1, x21(i,i), ldx21, tauq1(i),x11(i+1,i), ldx11, &
                        work(ilarf) )
              call stdlib_I64_clarf( 'R', m-p-i, q-i+1, x21(i,i), ldx21, tauq1(i),x21(i+1,i), ldx21, &
                        work(ilarf) )
              call stdlib_I64_clacgv( q-i+1, x21(i,i), ldx21 )
              if( i < m-q ) then
                 s = sqrt( stdlib_I64_scnrm2( p-i, x11(i+1,i), 1_ilp64 )**2_ilp64+ stdlib_I64_scnrm2( m-p-i, x21(i+1,&
                           i), 1_ilp64 )**2_ilp64 )
                 phi(i) = atan2( s, c )
              end if
           end do
           ...
           do i = m - q + 1, p
              ...
              call stdlib_I64_clarf( 'R', p-i, q-i+1, x11(i,i), ldx11, tauq1(i),x11(i+1,i), ldx11, &
                        work(ilarf) )
              call stdlib_I64_clarf( 'R', q-p, q-i+1, x11(i,i), ldx11, tauq1(i),x21(m-q+1,i), ldx21, &
                        work(ilarf) )
              call stdlib_I64_clacgv( q-i+1, x11(i,i), ldx11 )
           end do
           ! reduce the bottom-right portion of x21 to [ 0 i ]
           do i = p + 1, q
              call stdlib_I64_clacgv( q-i+1, x21(m-q+i-p,i), ldx21 )
              call stdlib_I64_clarfgp( q-i+1, x21(m-q+i-p,i), x21(m-q+i-p,i+1), ldx21,tauq1(i) )
                        
              x21(m-q+i-p,i) = cone
              call stdlib_I64_clarf( 'R', q-i, q-i+1, x21(m-q+i-p,i), ldx21, tauq1(i),x21(m-q+i-p+1,i)&
                        , ldx21, work(ilarf) )
              call stdlib_I64_clacgv( q-i+1, x21(m-q+i-p,i), ldx21 )
           end do
           return
     ...
     !! implicitly by angles THETA, PHI.
        ...
        ! ====================================================================
           ...
           if( info /= 0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZUNBDB4', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           do i = 1, m-q
              if( i == 1_ilp64 ) then
                 ...
                 call stdlib_I64_zunbdb5( p, m-p, q, phantom(1_ilp64), 1_ilp64, phantom(p+1), 1_ilp64,x11, ldx11, x21, &
                           ldx21, work(iorbdb5),lorbdb5, childinfo )
                 call stdlib_I64_zscal( p, cnegone, phantom(1_ilp64), 1_ilp64 )
                 call stdlib_I64_zlarfgp( p, phantom(1_ilp64), phantom(2_ilp64), 1_ilp64, taup1(1_ilp64) )
                 call stdlib_I64_zlarfgp( m-p, phantom(p+1), phantom(p+2), 1_ilp64, taup2(1_ilp64) )
                 theta(i) = atan2( real( phantom(1_ilp64),KIND=dp), real( phantom(p+1),KIND=dp) )
                           
                 c = cos( theta(i) )
                 s = sin( theta(i) )
                 phantom(1_ilp64) = cone
                 phantom(p+1) = cone
                 call stdlib_I64_zlarf( 'L', p, q, phantom(1_ilp64), 1_ilp64, conjg(taup1(1_ilp64)), x11,ldx11, work(&
                           ilarf) )
                 call stdlib_I64_zlarf( 'L', m-p, q, phantom(p+1), 1_ilp64, conjg(taup2(1_ilp64)),x21, ldx21, &
                           work(ilarf) )
              else
                 call stdlib_I64_zunbdb5( p-i+1, m-p-i+1, q-i+1, x11(i,i-1), 1_ilp64,x21(i,i-1), 1_ilp64, x11(i,i)&
                           , ldx11, x21(i,i),ldx21, work(iorbdb5), lorbdb5, childinfo )
                 call stdlib_I64_zscal( p-i+1, cnegone, x11(i,i-1), 1_ilp64 )
                 call stdlib_I64_zlarfgp( p-i+1, x11(i,i-1), x11(i+1,i-1), 1_ilp64, taup1(i) )
                 call stdlib_I64_zlarfgp( m-p-i+1, x21(i,i-1), x21(i+1,i-1), 1_ilp64,taup2(i) )
                 theta(i) = atan2( real( x11(i,i-1),KIND=dp), real( x21(i,i-1),KIND=dp) )
                 c = cos( theta(i) )
                 s = sin( theta(i) )
                 x11(i,i-1) = cone
                 x21(i,i-1) = cone
                 call stdlib_I64_zlarf( 'L', p-i+1, q-i+1, x11(i,i-1), 1_ilp64,conjg(taup1(i)), x11(i,i), &
                           ldx11, work(ilarf) )
                 call stdlib_I64_zlarf( 'L', m-p-i+1, q-i+1, x21(i,i-1), 1_ilp64,conjg(taup2(i)), x21(i,i), &
                           ldx21, work(ilarf) )
              end if
              call stdlib_I64_zdrot( q-i+1, x11(i,i), ldx11, x21(i,i), ldx21, s, -c )
              call stdlib_I64_zlacgv( q-i+1, x21(i,i), ldx21 )
              call stdlib_I64_zlarfgp( q-i+1, x21(i,i), x21(i,i+1), ldx21, tauq1(i) )
              c = real( x21(i,i),KIND=dp)
              x21(i,i) = cone
              call stdlib_I64_zlarf( 'R', p-i, q-i+1, x21(i,i), ldx21, tauq1(i),x11(i+1,i), ldx11, &
                        work(ilarf) )
              call stdlib_I64_zlarf( 'R', m-p-i, q-i+1, x21(i,i), ldx21, tauq1(i),x21(i+1,i), ldx21, &
                        work(ilarf) )
              call stdlib_I64_zlacgv( q-i+1, x21(i,i), ldx21 )
              if( i < m-q ) then
                 s = sqrt( stdlib_I64_dznrm2( p-i, x11(i+1,i), 1_ilp64 )**2_ilp64+ stdlib_I64_dznrm2( m-p-i, x21(i+1,&
                           i), 1_ilp64 )**2_ilp64 )
                 phi(i) = atan2( s, c )
              end if
           end do
           ...
           do i = m - q + 1, p
              ...
              call stdlib_I64_zlarf( 'R', p-i, q-i+1, x11(i,i), ldx11, tauq1(i),x11(i+1,i), ldx11, &
                        work(ilarf) )
              call stdlib_I64_zlarf( 'R', q-p, q-i+1, x11(i,i), ldx11, tauq1(i),x21(m-q+1,i), ldx21, &
                        work(ilarf) )
              call stdlib_I64_zlacgv( q-i+1, x11(i,i), ldx11 )
           end do
           ! reduce the bottom-right portion of x21 to [ 0 i ]
           do i = p + 1, q
              call stdlib_I64_zlacgv( q-i+1, x21(m-q+i-p,i), ldx21 )
              call stdlib_I64_zlarfgp( q-i+1, x21(m-q+i-p,i), x21(m-q+i-p,i+1), ldx21,tauq1(i) )
                        
              x21(m-q+i-p,i) = cone
              call stdlib_I64_zlarf( 'R', q-i, q-i+1, x21(m-q+i-p,i), ldx21, tauq1(i),x21(m-q+i-p+1,i)&
                        , ldx21, work(ilarf) )
              call stdlib_I64_zlacgv( q-i+1, x21(m-q+i-p,i), ldx21 )
           end do
           return
     ...
     !! way.
        ...
        ! =====================================================================
           ...
           if( m1 < 0_ilp64 ) then
              info = -1_ilp64
           else if( m2 < 0_ilp64 ) then
              info = -2_ilp64
           else if( n < 0_ilp64 ) then
              info = -3_ilp64
           else if( incx1 < 1_ilp64 ) then
              info = -5_ilp64
           else if( incx2 < 1_ilp64 ) then
              info = -7_ilp64
           else if( ldq1 < max( 1_ilp64, m1 ) ) then
              info = -9_ilp64
           else if( ldq2 < max( 1_ilp64, m2 ) ) then
              info = -11_ilp64
           else if( lwork < n ) then
              info = -13_ilp64
           end if
           if( info /= 0_ilp64 ) then
              call stdlib_I64_xerbla( 'CUNBDB5', -info )
              return
           end if
           ! project x onto the orthogonal complement of q
           call stdlib_I64_cunbdb6( m1, m2, n, x1, incx1, x2, incx2, q1, ldq1, q2, ldq2,work, lwork, &
                     childinfo )
           ! if the projection is nonzero, then return
           if( stdlib_I64_scnrm2(m1,x1,incx1) /= czero.or. stdlib_I64_scnrm2(m2,x2,incx2) /= czero ) &
                     then
              return
           end if
           ! project each standard basis vector e_1,...,e_m1 in turn, stopping
           ...
           do i = 1, m1
              ...
              call stdlib_I64_cunbdb6( m1, m2, n, x1, incx1, x2, incx2, q1, ldq1, q2,ldq2, work, &
                        lwork, childinfo )
              if( stdlib_I64_scnrm2(m1,x1,incx1) /= czero.or. stdlib_I64_scnrm2(m2,x2,incx2) /= czero ) &
                        then
                 return
     ...
     !! way.
        ...
        ! =====================================================================
           ...
           if( info /= 0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZUNBDB5', -info )
              return
           end if
           ! project x onto the orthogonal complement of q
           call stdlib_I64_zunbdb6( m1, m2, n, x1, incx1, x2, incx2, q1, ldq1, q2, ldq2,work, lwork, &
                     childinfo )
           ! if the projection is nonzero, then return
           if( stdlib_I64_dznrm2(m1,x1,incx1) /= czero.or. stdlib_I64_dznrm2(m2,x2,incx2) /= czero ) &
                     then
              return
           end if
           ! project each standard basis vector e_1,...,e_m1 in turn, stopping
           ...
           do i = 1, m1
              ...
              call stdlib_I64_zunbdb6( m1, m2, n, x1, incx1, x2, incx2, q1, ldq1, q2,ldq2, work, &
                        lwork, childinfo )
              if( stdlib_I64_dznrm2(m1,x1,incx1) /= czero.or. stdlib_I64_dznrm2(m2,x2,incx2) /= czero ) &
                        then
                 return
     ...
     !! criterion, then the zero vector is returned.
        ...
        ! =====================================================================
           ...
           if( info /= 0_ilp64 ) then
              call stdlib_I64_xerbla( 'CUNBDB6', -info )
              return
           end if
           ! first, project x onto the orthogonal complement of q's column
           ! space
           scl1 = zero
           ssq1 = one
           call stdlib_I64_classq( m1, x1, incx1, scl1, ssq1 )
           scl2 = zero
           ssq2 = one
           call stdlib_I64_classq( m2, x2, incx2, scl2, ssq2 )
           normsq1 = scl1**2_ilp64*ssq1 + scl2**2_ilp64*ssq2
           if( m1 == 0_ilp64 ) then
              do i = 1, n
                 work(i) = czero
              end do
           else
              call stdlib_I64_cgemv( 'C', m1, n, cone, q1, ldq1, x1, incx1, czero, work,1_ilp64 )
           end if
           call stdlib_I64_cgemv( 'C', m2, n, cone, q2, ldq2, x2, incx2, cone, work, 1_ilp64 )
           call stdlib_I64_cgemv( 'N', m1, n, cnegone, q1, ldq1, work, 1_ilp64, cone, x1,incx1 )
           call stdlib_I64_cgemv( 'N', m2, n, cnegone, q2, ldq2, work, 1_ilp64, cone, x2,incx2 )
           scl1 = zero
           ssq1 = one
           call stdlib_I64_classq( m1, x1, incx1, scl1, ssq1 )
           scl2 = zero
           ssq2 = one
           call stdlib_I64_classq( m2, x2, incx2, scl2, ssq2 )
           normsq2 = scl1**2_ilp64*ssq1 + scl2**2_ilp64*ssq2
           ! if projection is sufficiently large in norm, then stop.
           ! if projection is czero, then stop.
           ! otherwise, project again.
           if( normsq2 >= alphasq*normsq1 ) then
     ...
     !! criterion, then the zero vector is returned.
        ...
        ! =====================================================================
           ...
           if( info /= 0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZUNBDB6', -info )
              return
           end if
           ! first, project x onto the orthogonal complement of q's column
           ! space
           scl1 = zero
           ssq1 = one
           call stdlib_I64_zlassq( m1, x1, incx1, scl1, ssq1 )
           scl2 = zero
           ssq2 = one
           call stdlib_I64_zlassq( m2, x2, incx2, scl2, ssq2 )
           normsq1 = scl1**2_ilp64*ssq1 + scl2**2_ilp64*ssq2
           if( m1 == 0_ilp64 ) then
              do i = 1, n
                 work(i) = czero
              ...
              call stdlib_I64_zgemv( 'C', m1, n, cone, q1, ldq1, x1, incx1, czero, work,1_ilp64 )
           end if
           call stdlib_I64_zgemv( 'C', m2, n, cone, q2, ldq2, x2, incx2, cone, work, 1_ilp64 )
           call stdlib_I64_zgemv( 'N', m1, n, cnegone, q1, ldq1, work, 1_ilp64, cone, x1,incx1 )
           call stdlib_I64_zgemv( 'N', m2, n, cnegone, q2, ldq2, work, 1_ilp64, cone, x2,incx2 )
           scl1 = zero
           ssq1 = one
           call stdlib_I64_zlassq( m1, x1, incx1, scl1, ssq1 )
           scl2 = zero
