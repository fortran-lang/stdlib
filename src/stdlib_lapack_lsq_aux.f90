  ...
  contains
     ...
     !! where  alpha =  x**T*w.
        ...
        ! =====================================================================
           ...
           alpha = stdlib_I64_sdot( j, x, 1_ilp64, w, 1_ilp64 )
           absalp = abs( alpha )
           absgam = abs( gamma )
           absest = abs( sest )
           if( job==1_ilp64 ) then
     ...
     !! where  alpha =  x**T*w.
        ...
        ! =====================================================================
           ...
           alpha = stdlib_I64_ddot( j, x, 1_ilp64, w, 1_ilp64 )
           absalp = abs( alpha )
           absgam = abs( gamma )
           absest = abs( sest )
           if( job==1_ilp64 ) then
     ...
     !! where  alpha =  x**H*w.
        ...
        ! =====================================================================
           ...
           ! Executable Statements 
           eps = stdlib_I64_slamch( 'EPSILON' )
           alpha = stdlib_I64_cdotc( j, x, 1_ilp64, w, 1_ilp64 )
           absalp = abs( alpha )
           absgam = abs( gamma )
           absest = abs( sest )
           if( job==1_ilp64 ) then
     ...
     !! where  alpha =  x**H * w.
        ...
        ! =====================================================================
           ...
           ! Executable Statements 
           eps = stdlib_I64_dlamch( 'EPSILON' )
           alpha = stdlib_I64_zdotc( j, x, 1_ilp64, w, 1_ilp64 )
           absalp = abs( alpha )
           absgam = abs( gamma )
           absest = abs( sest )
           if( job==1_ilp64 ) then
     ...
     !! (4R) The inverse transformation of (1L).
        ...
        ! =====================================================================
           ...
           info = 0_ilp64
           n = nl + nr + 1_ilp64
           if( ( icompq<0_ilp64 ) .or. ( icompq>1_ilp64 ) ) then
              info = -1_ilp64
           else if( nl<1_ilp64 ) then
              info = -2_ilp64
           else if( nr<1_ilp64 ) then
              info = -3_ilp64
           else if( ( sqre<0_ilp64 ) .or. ( sqre>1_ilp64 ) ) then
              info = -4_ilp64
              ...
              info = -9_ilp64
           else if( givptr<0_ilp64 ) then
              info = -11_ilp64
           else if( ldgcol<n ) then
              info = -13_ilp64
           else if( ldgnum<n ) then
              info = -15_ilp64
           else if( k<1_ilp64 ) then
              info = -20_ilp64
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SLALS0', -info )
              return
           end if
           m = n + sqre
           nlp1 = nl + 1_ilp64
           if( icompq==0_ilp64 ) then
              ...
              ! step (1l): apply back the givens rotations performed.
              do i = 1, givptr
                 call stdlib_I64_srot( nrhs, b( givcol( i, 2_ilp64 ), 1_ilp64 ), ldb,b( givcol( i, 1_ilp64 ), 1_ilp64 ), ldb, &
                           givnum( i, 2_ilp64 ),givnum( i, 1_ilp64 ) )
              end do
              ! step (2l): permute rows of b.
              call stdlib_I64_scopy( nrhs, b( nlp1, 1_ilp64 ), ldb, bx( 1_ilp64, 1_ilp64 ), ldbx )
              do i = 2, n
                 call stdlib_I64_scopy( nrhs, b( perm( i ), 1_ilp64 ), ldb, bx( i, 1_ilp64 ), ldbx )
              end do
              ! step (3l): apply the inverse of the left singular vector
              ! matrix to bx.
              if( k==1_ilp64 ) then
                 call stdlib_I64_scopy( nrhs, bx, ldbx, b, ldb )
                 if( z( 1_ilp64 )<zero ) then
                    call stdlib_I64_sscal( nrhs, negone, b, ldb )
                 end if
              else
                 loop_50: do j = 1, k
                    diflj = difl( j )
                    dj = poles( j, 1_ilp64 )
                    dsigj = -poles( j, 2_ilp64 )
                    if( j<k ) then
                       difrj = -difr( j, 1_ilp64 )
                       dsigjp = -poles( j+1, 2_ilp64 )
                    end if
                    if( ( z( j )==zero ) .or. ( poles( j, 2_ilp64 )==zero ) )then
                       work( j ) = zero
                    else
                       work( j ) = -poles( j, 2_ilp64 )*z( j ) / diflj /( poles( j, 2_ilp64 )+dj )
                    end if
                    do i = 1, j - 1
                       if( ( z( i )==zero ) .or.( poles( i, 2_ilp64 )==zero ) ) then
                          work( i ) = zero
                       else
                          work( i ) = poles( i, 2_ilp64 )*z( i ) /( stdlib_I64_slamc3( poles( i, 2_ilp64 ), dsigj &
                                    )-diflj ) / ( poles( i, 2_ilp64 )+dj )
                       end if
                    end do
                    do i = j + 1, k
                       if( ( z( i )==zero ) .or.( poles( i, 2_ilp64 )==zero ) ) then
                          ...
                          work( i ) = poles( i, 2_ilp64 )*z( i ) /( stdlib_I64_slamc3( poles( i, 2_ilp64 ), &
                                    dsigjp )+difrj ) / ( poles( i, 2_ilp64 )+dj )
                       end if
                    end do
                    work( 1_ilp64 ) = negone
                    temp = stdlib_I64_snrm2( k, work, 1_ilp64 )
                    call stdlib_I64_sgemv( 'T', k, nrhs, one, bx, ldbx, work, 1_ilp64, zero,b( j, 1_ilp64 ), ldb )
                              
                    call stdlib_I64_slascl( 'G', 0_ilp64, 0_ilp64, temp, one, 1_ilp64, nrhs, b( j, 1_ilp64 ),ldb, info )
                              
                 end do loop_50
              end if
              ! move the deflated rows of bx to b also.
              if( k<max( m, n ) )call stdlib_I64_slacpy( 'A', n-k, nrhs, bx( k+1, 1_ilp64 ), ldbx,b( k+1, 1_ilp64 &
                        ), ldb )
           else
              ! apply back the right orthogonal transformations.
              ! step (1r): apply back the new right singular vector matrix
              ! to b.
              if( k==1_ilp64 ) then
                 call stdlib_I64_scopy( nrhs, b, ldb, bx, ldbx )
              else
                 do j = 1, k
                    dsigj = poles( j, 2_ilp64 )
                    if( z( j )==zero ) then
                       ...
                       work( j ) = -z( j ) / difl( j ) /( dsigj+poles( j, 1_ilp64 ) ) / difr( j, 2_ilp64 )
                    ...
                    do i = 1, j - 1
                       if( z( j )==zero ) then
                          ...
                          work( i ) = z( j ) / ( stdlib_I64_slamc3( dsigj, -poles( i+1,2_ilp64 ) )-difr( i, &
                                    1_ilp64 ) ) /( dsigj+poles( i, 1_ilp64 ) ) / difr( i, 2_ilp64 )
                       end if
                    end do
                    do i = j + 1, k
                       if( z( j )==zero ) then
                          ...
                          work( i ) = z( j ) / ( stdlib_I64_slamc3( dsigj, -poles( i,2_ilp64 ) )-difl( i ) )&
                                     /( dsigj+poles( i, 1_ilp64 ) ) / difr( i, 2_ilp64 )
                       end if
                    end do
                    call stdlib_I64_sgemv( 'T', k, nrhs, one, b, ldb, work, 1_ilp64, zero,bx( j, 1_ilp64 ), ldbx )
                              
                 end do
              end if
              ! step (2r): if sqre = 1, apply back the rotation that is
              ! related to the right null space of the subproblem.
              if( sqre==1_ilp64 ) then
                 call stdlib_I64_scopy( nrhs, b( m, 1_ilp64 ), ldb, bx( m, 1_ilp64 ), ldbx )
                 call stdlib_I64_srot( nrhs, bx( 1_ilp64, 1_ilp64 ), ldbx, bx( m, 1_ilp64 ), ldbx, c, s )
              end if
              if( k<max( m, n ) )call stdlib_I64_slacpy( 'A', n-k, nrhs, b( k+1, 1_ilp64 ), ldb, bx( k+1, 1_ilp64 &
                        ),ldbx )
              ! step (3r): permute rows of b.
              call stdlib_I64_scopy( nrhs, bx( 1_ilp64, 1_ilp64 ), ldbx, b( nlp1, 1_ilp64 ), ldb )
              if( sqre==1_ilp64 ) then
                 call stdlib_I64_scopy( nrhs, bx( m, 1_ilp64 ), ldbx, b( m, 1_ilp64 ), ldb )
              end if
              do i = 2, n
                 call stdlib_I64_scopy( nrhs, bx( i, 1_ilp64 ), ldbx, b( perm( i ), 1_ilp64 ), ldb )
              end do
              ! step (4r): apply back the givens rotations performed.
              do i = givptr, 1, -1
                 call stdlib_I64_srot( nrhs, b( givcol( i, 2_ilp64 ), 1_ilp64 ), ldb,b( givcol( i, 1_ilp64 ), 1_ilp64 ), ldb, &
     ...
     !! (4R) The inverse transformation of (1L).
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DLALS0', -info )
              return
           end if
           m = n + sqre
           nlp1 = nl + 1_ilp64
           if( icompq==0_ilp64 ) then
              ...
              ! step (1l): apply back the givens rotations performed.
              do i = 1, givptr
                 call stdlib_I64_drot( nrhs, b( givcol( i, 2_ilp64 ), 1_ilp64 ), ldb,b( givcol( i, 1_ilp64 ), 1_ilp64 ), ldb, &
                           givnum( i, 2_ilp64 ),givnum( i, 1_ilp64 ) )
              end do
              ! step (2l): permute rows of b.
              call stdlib_I64_dcopy( nrhs, b( nlp1, 1_ilp64 ), ldb, bx( 1_ilp64, 1_ilp64 ), ldbx )
              do i = 2, n
                 call stdlib_I64_dcopy( nrhs, b( perm( i ), 1_ilp64 ), ldb, bx( i, 1_ilp64 ), ldbx )
              end do
              ! step (3l): apply the inverse of the left singular vector
              ! matrix to bx.
              if( k==1_ilp64 ) then
                 call stdlib_I64_dcopy( nrhs, bx, ldbx, b, ldb )
                 if( z( 1_ilp64 )<zero ) then
                    call stdlib_I64_dscal( nrhs, negone, b, ldb )
                 end if
              else
                 loop_50: do j = 1, k
                    diflj = difl( j )
                    ...
                    do i = 1, j - 1
                       if( ( z( i )==zero ) .or.( poles( i, 2_ilp64 )==zero ) ) then
                          ...
                          work( i ) = poles( i, 2_ilp64 )*z( i ) /( stdlib_I64_dlamc3( poles( i, 2_ilp64 ), dsigj &
                                    )-diflj ) / ( poles( i, 2_ilp64 )+dj )
                       end if
                    end do
                    do i = j + 1, k
                       if( ( z( i )==zero ) .or.( poles( i, 2_ilp64 )==zero ) ) then
                          ...
                          work( i ) = poles( i, 2_ilp64 )*z( i ) /( stdlib_I64_dlamc3( poles( i, 2_ilp64 ), &
                                    dsigjp )+difrj ) / ( poles( i, 2_ilp64 )+dj )
                       end if
                    end do
                    work( 1_ilp64 ) = negone
                    temp = stdlib_I64_dnrm2( k, work, 1_ilp64 )
                    call stdlib_I64_dgemv( 'T', k, nrhs, one, bx, ldbx, work, 1_ilp64, zero,b( j, 1_ilp64 ), ldb )
                              
                    call stdlib_I64_dlascl( 'G', 0_ilp64, 0_ilp64, temp, one, 1_ilp64, nrhs, b( j, 1_ilp64 ),ldb, info )
                              
                 end do loop_50
              end if
              ! move the deflated rows of bx to b also.
              if( k<max( m, n ) )call stdlib_I64_dlacpy( 'A', n-k, nrhs, bx( k+1, 1_ilp64 ), ldbx,b( k+1, 1_ilp64 &
                        ), ldb )
           else
              ! apply back the right orthogonal transformations.
              ! step (1r): apply back the new right singular vector matrix
              ! to b.
              if( k==1_ilp64 ) then
                 call stdlib_I64_dcopy( nrhs, b, ldb, bx, ldbx )
              else
                 do j = 1, k
                    dsigj = poles( j, 2_ilp64 )
                    if( z( j )==zero ) then
                    ...
                    do i = 1, j - 1
                       if( z( j )==zero ) then
                          ...
                          work( i ) = z( j ) / ( stdlib_I64_dlamc3( dsigj, -poles( i+1,2_ilp64 ) )-difr( i, &
                                    1_ilp64 ) ) /( dsigj+poles( i, 1_ilp64 ) ) / difr( i, 2_ilp64 )
                       end if
                    end do
                    do i = j + 1, k
                       if( z( j )==zero ) then
                          ...
                          work( i ) = z( j ) / ( stdlib_I64_dlamc3( dsigj, -poles( i,2_ilp64 ) )-difl( i ) )&
                                     /( dsigj+poles( i, 1_ilp64 ) ) / difr( i, 2_ilp64 )
                       end if
                    end do
                    call stdlib_I64_dgemv( 'T', k, nrhs, one, b, ldb, work, 1_ilp64, zero,bx( j, 1_ilp64 ), ldbx )
              ...
              ! related to the right null space of the subproblem.
              if( sqre==1_ilp64 ) then
                 call stdlib_I64_dcopy( nrhs, b( m, 1_ilp64 ), ldb, bx( m, 1_ilp64 ), ldbx )
                 call stdlib_I64_drot( nrhs, bx( 1_ilp64, 1_ilp64 ), ldbx, bx( m, 1_ilp64 ), ldbx, c, s )
              end if
              if( k<max( m, n ) )call stdlib_I64_dlacpy( 'A', n-k, nrhs, b( k+1, 1_ilp64 ), ldb, bx( k+1, 1_ilp64 &
                        ),ldbx )
              ! step (3r): permute rows of b.
              call stdlib_I64_dcopy( nrhs, bx( 1_ilp64, 1_ilp64 ), ldbx, b( nlp1, 1_ilp64 ), ldb )
              if( sqre==1_ilp64 ) then
                 call stdlib_I64_dcopy( nrhs, bx( m, 1_ilp64 ), ldbx, b( m, 1_ilp64 ), ldb )
              end if
              do i = 2, n
                 call stdlib_I64_dcopy( nrhs, bx( i, 1_ilp64 ), ldbx, b( perm( i ), 1_ilp64 ), ldb )
              end do
              ! step (4r): apply back the givens rotations performed.
              do i = givptr, 1, -1
                 call stdlib_I64_drot( nrhs, b( givcol( i, 2_ilp64 ), 1_ilp64 ), ldb,b( givcol( i, 1_ilp64 ), 1_ilp64 ), ldb, &
     ...
     !! (4R) The inverse transformation of (1L).
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           integer(ilp64) :: i, j, jcol, jrow, m, n, nlp1
           real(sp) :: diflj, difrj, dj, dsigj, dsigjp, temp
           ! Intrinsic Functions 
           ! Executable Statements 
           ! test the input parameters.
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CLALS0', -info )
              return
           end if
           m = n + sqre
           nlp1 = nl + 1_ilp64
           if( icompq==0_ilp64 ) then
              ...
              ! step (1l): apply back the givens rotations performed.
              do i = 1, givptr
                 call stdlib_I64_csrot( nrhs, b( givcol( i, 2_ilp64 ), 1_ilp64 ), ldb,b( givcol( i, 1_ilp64 ), 1_ilp64 ), ldb,&
                            givnum( i, 2_ilp64 ),givnum( i, 1_ilp64 ) )
              end do
              ! step (2l): permute rows of b.
              call stdlib_I64_ccopy( nrhs, b( nlp1, 1_ilp64 ), ldb, bx( 1_ilp64, 1_ilp64 ), ldbx )
              do i = 2, n
                 call stdlib_I64_ccopy( nrhs, b( perm( i ), 1_ilp64 ), ldb, bx( i, 1_ilp64 ), ldbx )
              end do
              ! step (3l): apply the inverse of the left singular vector
              ! matrix to bx.
              if( k==1_ilp64 ) then
                 call stdlib_I64_ccopy( nrhs, bx, ldbx, b, ldb )
                 if( z( 1_ilp64 )<zero ) then
                    call stdlib_I64_csscal( nrhs, negone, b, ldb )
                 end if
              else
                 loop_100: do j = 1, k
                    diflj = difl( j )
                    ...
                    if( ( z( j )==zero ) .or. ( poles( j, 2_ilp64 )==zero ) )then
                       ...
                       rwork( j ) = -poles( j, 2_ilp64 )*z( j ) / diflj /( poles( j, 2_ilp64 )+dj )
                    end if
                    do i = 1, j - 1
                       if( ( z( i )==zero ) .or.( poles( i, 2_ilp64 )==zero ) ) then
                          rwork( i ) = zero
                       else
                          rwork( i ) = poles( i, 2_ilp64 )*z( i ) /( stdlib_I64_slamc3( poles( i, 2_ilp64 ), &
                                    dsigj )-diflj ) / ( poles( i, 2_ilp64 )+dj )
                       end if
                    end do
                    do i = j + 1, k
                       if( ( z( i )==zero ) .or.( poles( i, 2_ilp64 )==zero ) ) then
                    ...
                    rwork( 1_ilp64 ) = negone
                    temp = stdlib_I64_snrm2( k, rwork, 1_ilp64 )
                    ! since b and bx are complex, the following call to stdlib_I64_sgemv
                    ! is performed in two steps (real and imaginary parts).
                    ! call stdlib_I64_sgemv( 't', k, nrhs, one, bx, ldbx, work, 1, zero,
          ! $                     b( j, 1 ), ldb )
                    i = k + nrhs*2_ilp64
                    do jcol = 1, nrhs
                       do jrow = 1, k
                    ...
                    call stdlib_I64_sgemv( 'T', k, nrhs, one, rwork( 1_ilp64+k+nrhs*2_ilp64 ), k,rwork( 1_ilp64 ), 1_ilp64, &
                              zero, rwork( 1_ilp64+k ), 1_ilp64 )
                    i = k + nrhs*2_ilp64
                    do jcol = 1, nrhs
                       do jrow = 1, k
                          i = i + 1_ilp64
                    ...
                    call stdlib_I64_sgemv( 'T', k, nrhs, one, rwork( 1_ilp64+k+nrhs*2_ilp64 ), k,rwork( 1_ilp64 ), 1_ilp64, &
                              zero, rwork( 1_ilp64+k+nrhs ), 1_ilp64 )
                    do jcol = 1, nrhs
                       b( j, jcol ) = cmplx( rwork( jcol+k ),rwork( jcol+k+nrhs ),KIND=sp)
                    end do
                    call stdlib_I64_clascl( 'G', 0_ilp64, 0_ilp64, temp, one, 1_ilp64, nrhs, b( j, 1_ilp64 ),ldb, info )
                              
                 end do loop_100
              end if
              ! move the deflated rows of bx to b also.
              if( k<max( m, n ) )call stdlib_I64_clacpy( 'A', n-k, nrhs, bx( k+1, 1_ilp64 ), ldbx,b( k+1, 1_ilp64 &
                        ), ldb )
           else
              ! apply back the right orthogonal transformations.
              ! step (1r): apply back the new right singular vector matrix
              ! to b.
              if( k==1_ilp64 ) then
                 call stdlib_I64_ccopy( nrhs, b, ldb, bx, ldbx )
              else
                 loop_180: do j = 1, k
                    dsigj = poles( j, 2_ilp64 )
                    if( z( j )==zero ) then
                       ...
                       rwork( j ) = -z( j ) / difl( j ) /( dsigj+poles( j, 1_ilp64 ) ) / difr( j, 2_ilp64 )
                    ...
                    do i = 1, j - 1
                       if( z( j )==zero ) then
                          ...
                          rwork( i ) = z( j ) / ( stdlib_I64_slamc3( dsigj, -poles( i+1,2_ilp64 ) )-difr( i,&
                                     1_ilp64 ) ) /( dsigj+poles( i, 1_ilp64 ) ) / difr( i, 2_ilp64 )
                       end if
                    end do
                    do i = j + 1, k
                       if( z( j )==zero ) then
                          ...
                          rwork( i ) = z( j ) / ( stdlib_I64_slamc3( dsigj, -poles( i,2_ilp64 ) )-difl( i ) &
                                    ) /( dsigj+poles( i, 1_ilp64 ) ) / difr( i, 2_ilp64 )
                       end if
                    end do
                    ! since b and bx are complex, the following call to stdlib_I64_sgemv
                    ! is performed in two steps (real and imaginary parts).
                    ! call stdlib_I64_sgemv( 't', k, nrhs, one, b, ldb, work, 1, zero,
          ! $                     bx( j, 1 ), ldbx )
                    i = k + nrhs*2_ilp64
                    do jcol = 1, nrhs
                       do jrow = 1, k
              ...
              ! related to the right null space of the subproblem.
              if( sqre==1_ilp64 ) then
                 call stdlib_I64_ccopy( nrhs, b( m, 1_ilp64 ), ldb, bx( m, 1_ilp64 ), ldbx )
                 call stdlib_I64_csrot( nrhs, bx( 1_ilp64, 1_ilp64 ), ldbx, bx( m, 1_ilp64 ), ldbx, c, s )
              end if
              if( k<max( m, n ) )call stdlib_I64_clacpy( 'A', n-k, nrhs, b( k+1, 1_ilp64 ), ldb,bx( k+1, 1_ilp64 )&
                        , ldbx )
              ! step (3r): permute rows of b.
              call stdlib_I64_ccopy( nrhs, bx( 1_ilp64, 1_ilp64 ), ldbx, b( nlp1, 1_ilp64 ), ldb )
              if( sqre==1_ilp64 ) then
                 call stdlib_I64_ccopy( nrhs, bx( m, 1_ilp64 ), ldbx, b( m, 1_ilp64 ), ldb )
              end if
              do i = 2, n
                 call stdlib_I64_ccopy( nrhs, bx( i, 1_ilp64 ), ldbx, b( perm( i ), 1_ilp64 ), ldb )
              end do
              ! step (4r): apply back the givens rotations performed.
              do i = givptr, 1, -1
                 call stdlib_I64_csrot( nrhs, b( givcol( i, 2_ilp64 ), 1_ilp64 ), ldb,b( givcol( i, 1_ilp64 ), 1_ilp64 ), ldb,&
     ...
     !! (4R) The inverse transformation of (1L).
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZLALS0', -info )
              return
           end if
           m = n + sqre
           nlp1 = nl + 1_ilp64
           if( icompq==0_ilp64 ) then
              ...
              ! step (1l): apply back the givens rotations performed.
              do i = 1, givptr
                 call stdlib_I64_zdrot( nrhs, b( givcol( i, 2_ilp64 ), 1_ilp64 ), ldb,b( givcol( i, 1_ilp64 ), 1_ilp64 ), ldb,&
                            givnum( i, 2_ilp64 ),givnum( i, 1_ilp64 ) )
              end do
              ! step (2l): permute rows of b.
              call stdlib_I64_zcopy( nrhs, b( nlp1, 1_ilp64 ), ldb, bx( 1_ilp64, 1_ilp64 ), ldbx )
              do i = 2, n
                 call stdlib_I64_zcopy( nrhs, b( perm( i ), 1_ilp64 ), ldb, bx( i, 1_ilp64 ), ldbx )
              end do
              ! step (3l): apply the inverse of the left singular vector
              ! matrix to bx.
              if( k==1_ilp64 ) then
                 call stdlib_I64_zcopy( nrhs, bx, ldbx, b, ldb )
                 if( z( 1_ilp64 )<zero ) then
                    call stdlib_I64_zdscal( nrhs, negone, b, ldb )
                 end if
              else
                 loop_100: do j = 1, k
                    diflj = difl( j )
                    ...
                    do i = 1, j - 1
                       if( ( z( i )==zero ) .or.( poles( i, 2_ilp64 )==zero ) ) then
                          ...
                          rwork( i ) = poles( i, 2_ilp64 )*z( i ) /( stdlib_I64_dlamc3( poles( i, 2_ilp64 ), &
                                    dsigj )-diflj ) / ( poles( i, 2_ilp64 )+dj )
                       end if
                    end do
                    do i = j + 1, k
                    ...
                    rwork( 1_ilp64 ) = negone
                    temp = stdlib_I64_dnrm2( k, rwork, 1_ilp64 )
                    ! since b and bx are complex, the following call to stdlib_I64_dgemv
                    ! is performed in two steps (real and imaginary parts).
                    ! call stdlib_I64_dgemv( 't', k, nrhs, one, bx, ldbx, work, 1, zero,
          ! $                     b( j, 1 ), ldb )
                    i = k + nrhs*2_ilp64
                    do jcol = 1, nrhs
                       do jrow = 1, k
                    ...
                    call stdlib_I64_dgemv( 'T', k, nrhs, one, rwork( 1_ilp64+k+nrhs*2_ilp64 ), k,rwork( 1_ilp64 ), 1_ilp64, &
                              zero, rwork( 1_ilp64+k ), 1_ilp64 )
                    i = k + nrhs*2_ilp64
                    do jcol = 1, nrhs
                       do jrow = 1, k
                    ...
                    call stdlib_I64_zlascl( 'G', 0_ilp64, 0_ilp64, temp, one, 1_ilp64, nrhs, b( j, 1_ilp64 ),ldb, info )
                              
                 end do loop_100
              end if
              ! move the deflated rows of bx to b also.
              if( k<max( m, n ) )call stdlib_I64_zlacpy( 'A', n-k, nrhs, bx( k+1, 1_ilp64 ), ldbx,b( k+1, 1_ilp64 &
                        ), ldb )
           else
              ! apply back the right orthogonal transformations.
              ! step (1r): apply back the new right singular vector matrix
              ! to b.
              if( k==1_ilp64 ) then
                 call stdlib_I64_zcopy( nrhs, b, ldb, bx, ldbx )
              else
                 loop_180: do j = 1, k
                    dsigj = poles( j, 2_ilp64 )
                    if( z( j )==zero ) then
                    ...
                    do i = 1, j - 1
                       if( z( j )==zero ) then
                          ...
                          rwork( i ) = z( j ) / ( stdlib_I64_dlamc3( dsigj, -poles( i+1,2_ilp64 ) )-difr( i,&
                                     1_ilp64 ) ) /( dsigj+poles( i, 1_ilp64 ) ) / difr( i, 2_ilp64 )
                       end if
                    end do
                    do i = j + 1, k
                       if( z( j )==zero ) then
                          ...
                          rwork( i ) = z( j ) / ( stdlib_I64_dlamc3( dsigj, -poles( i,2_ilp64 ) )-difl( i ) &
                                    ) /( dsigj+poles( i, 1_ilp64 ) ) / difr( i, 2_ilp64 )
                       end if
                    end do
                    ! since b and bx are complex, the following call to stdlib_I64_dgemv
                    ! is performed in two steps (real and imaginary parts).
                    ! call stdlib_I64_dgemv( 't', k, nrhs, one, b, ldb, work, 1, zero,
          ! $                     bx( j, 1 ), ldbx )
                    i = k + nrhs*2_ilp64
                    do jcol = 1, nrhs
                       do jrow = 1, k
              ...
              ! related to the right null space of the subproblem.
              if( sqre==1_ilp64 ) then
                 call stdlib_I64_zcopy( nrhs, b( m, 1_ilp64 ), ldb, bx( m, 1_ilp64 ), ldbx )
                 call stdlib_I64_zdrot( nrhs, bx( 1_ilp64, 1_ilp64 ), ldbx, bx( m, 1_ilp64 ), ldbx, c, s )
              end if
              if( k<max( m, n ) )call stdlib_I64_zlacpy( 'A', n-k, nrhs, b( k+1, 1_ilp64 ), ldb, bx( k+1, 1_ilp64 &
                        ),ldbx )
              ! step (3r): permute rows of b.
              call stdlib_I64_zcopy( nrhs, bx( 1_ilp64, 1_ilp64 ), ldbx, b( nlp1, 1_ilp64 ), ldb )
              if( sqre==1_ilp64 ) then
                 call stdlib_I64_zcopy( nrhs, bx( m, 1_ilp64 ), ldbx, b( m, 1_ilp64 ), ldb )
              end if
              do i = 2, n
                 call stdlib_I64_zcopy( nrhs, bx( i, 1_ilp64 ), ldbx, b( perm( i ), 1_ilp64 ), ldb )
              end do
              ! step (4r): apply back the givens rotations performed.
              do i = givptr, 1, -1
                 call stdlib_I64_zdrot( nrhs, b( givcol( i, 2_ilp64 ), 1_ilp64 ), ldb,b( givcol( i, 1_ilp64 ), 1_ilp64 ), ldb,&
     ...
     !! compact form by SLALSA.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           integer(ilp64) :: i, i1, ic, im1, inode, j, lf, ll, lvl, lvl2, nd, ndb1, ndiml, ndimr, &
                     nl, nlf, nlp1, nlvl, nr, nrf, nrp1, sqre
           ! Executable Statements 
           ! test the input parameters.
           info = 0_ilp64
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SLALSA', -info )
              return
           end if
           ! book-keeping and  setting up the computation tree.
           inode = 1_ilp64
           ndiml = inode + n
           ndimr = ndiml + n
           call stdlib_I64_slasdt( n, nlvl, nd, iwork( inode ), iwork( ndiml ),iwork( ndimr ), smlsiz &
                     )
           ! the following code applies back the left singular vector factors.
           ! for applying back the right singular vector factors, go to 50.
           ...
           ! the nodes on the bottom level of the tree were solved
           ! by stdlib_I64_slasdq. the corresponding left and right singular vector
           ! matrices are in explicit form. first apply back the left
           ! singular vector matrices.
           ndb1 = ( nd+1 ) / 2_ilp64
           do i = ndb1, nd
              ...
              nlf = ic - nl
              nrf = ic + 1_ilp64
              call stdlib_I64_sgemm( 'T', 'N', nl, nrhs, nl, one, u( nlf, 1_ilp64 ), ldu,b( nlf, 1_ilp64 ), ldb, &
                        zero, bx( nlf, 1_ilp64 ), ldbx )
              call stdlib_I64_sgemm( 'T', 'N', nr, nrhs, nr, one, u( nrf, 1_ilp64 ), ldu,b( nrf, 1_ilp64 ), ldb, &
                        zero, bx( nrf, 1_ilp64 ), ldbx )
           end do
           ! next copy the rows of b that correspond to unchanged rows
           ! in the bidiagonal matrix to bx.
           do i = 1, nd
              ic = iwork( inode+i-1 )
              call stdlib_I64_scopy( nrhs, b( ic, 1_ilp64 ), ldb, bx( ic, 1_ilp64 ), ldbx )
           end do
           ! finally go through the left singular vector matrices of all
           ! the other subproblems bottom-up on the tree.
           j = 2_ilp64**nlvl
           ...
           do lvl = nlvl, 1, -1
              ...
              do i = lf, ll
                 ...
                 nrf = ic + 1_ilp64
                 j = j - 1_ilp64
                 call stdlib_I64_slals0( icompq, nl, nr, sqre, nrhs, bx( nlf, 1_ilp64 ), ldbx,b( nlf, 1_ilp64 ), &
                 ldb, perm( nlf, lvl ),givptr( j ), givcol( nlf, lvl2 ), ldgcol,givnum( nlf, lvl2 &
                 ), ldu, poles( nlf, lvl2 ),difl( nlf, lvl ), difr( nlf, lvl2 ),z( nlf, lvl ), k( &
                           j ), c( j ), s( j ), work,info )
              end do
           ...
           loop_70: do lvl = 1, nlvl
              ...
              do i = ll, lf, -1
                 ...
                 j = j + 1_ilp64
                 call stdlib_I64_slals0( icompq, nl, nr, sqre, nrhs, b( nlf, 1_ilp64 ), ldb,bx( nlf, 1_ilp64 ), &
                 ldbx, perm( nlf, lvl ),givptr( j ), givcol( nlf, lvl2 ), ldgcol,givnum( nlf, &
                 lvl2 ), ldu, poles( nlf, lvl2 ),difl( nlf, lvl ), difr( nlf, lvl2 ),z( nlf, lvl )&
                           , k( j ), c( j ), s( j ), work,info )
              end do
           ...
           ! the nodes on the bottom level of the tree were solved
           ! by stdlib_I64_slasdq. the corresponding right singular vector
           ! matrices are in explicit form. apply them back.
           ndb1 = ( nd+1 ) / 2_ilp64
           do i = ndb1, nd
              i1 = i - 1_ilp64
              ...
              nlf = ic - nl
              nrf = ic + 1_ilp64
              call stdlib_I64_sgemm( 'T', 'N', nlp1, nrhs, nlp1, one, vt( nlf, 1_ilp64 ), ldu,b( nlf, 1_ilp64 ), &
                        ldb, zero, bx( nlf, 1_ilp64 ), ldbx )
              call stdlib_I64_sgemm( 'T', 'N', nrp1, nrhs, nrp1, one, vt( nrf, 1_ilp64 ), ldu,b( nrf, 1_ilp64 ), &
                        ldb, zero, bx( nrf, 1_ilp64 ), ldbx )
           end do
           90 continue
           return
     ...
     !! compact form by DLALSA.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DLALSA', -info )
              return
           end if
           ! book-keeping and  setting up the computation tree.
           inode = 1_ilp64
           ndiml = inode + n
           ndimr = ndiml + n
           call stdlib_I64_dlasdt( n, nlvl, nd, iwork( inode ), iwork( ndiml ),iwork( ndimr ), smlsiz &
                     )
           ! the following code applies back the left singular vector factors.
           ! for applying back the right singular vector factors, go to 50.
           if( icompq==1_ilp64 ) then
           ...
           ! the nodes on the bottom level of the tree were solved
           ! by stdlib_I64_dlasdq. the corresponding left and right singular vector
           ! matrices are in explicit form. first apply back the left
           ! singular vector matrices.
           ndb1 = ( nd+1 ) / 2_ilp64
           do i = ndb1, nd
              ...
              nlf = ic - nl
              nrf = ic + 1_ilp64
              call stdlib_I64_dgemm( 'T', 'N', nl, nrhs, nl, one, u( nlf, 1_ilp64 ), ldu,b( nlf, 1_ilp64 ), ldb, &
                        zero, bx( nlf, 1_ilp64 ), ldbx )
              call stdlib_I64_dgemm( 'T', 'N', nr, nrhs, nr, one, u( nrf, 1_ilp64 ), ldu,b( nrf, 1_ilp64 ), ldb, &
                        zero, bx( nrf, 1_ilp64 ), ldbx )
           end do
           ! next copy the rows of b that correspond to unchanged rows
           ! in the bidiagonal matrix to bx.
           do i = 1, nd
              ic = iwork( inode+i-1 )
              call stdlib_I64_dcopy( nrhs, b( ic, 1_ilp64 ), ldb, bx( ic, 1_ilp64 ), ldbx )
           end do
           ! finally go through the left singular vector matrices of all
           ! the other subproblems bottom-up on the tree.
           j = 2_ilp64**nlvl
           ...
           do lvl = nlvl, 1, -1
              ...
              do i = lf, ll
                 ...
                 nrf = ic + 1_ilp64
                 j = j - 1_ilp64
                 call stdlib_I64_dlals0( icompq, nl, nr, sqre, nrhs, bx( nlf, 1_ilp64 ), ldbx,b( nlf, 1_ilp64 ), &
                 ldb, perm( nlf, lvl ),givptr( j ), givcol( nlf, lvl2 ), ldgcol,givnum( nlf, lvl2 &
                 ), ldu, poles( nlf, lvl2 ),difl( nlf, lvl ), difr( nlf, lvl2 ),z( nlf, lvl ), k( &
                           j ), c( j ), s( j ), work,info )
              end do
           ...
           loop_70: do lvl = 1, nlvl
              ...
              do i = ll, lf, -1
                 ...
                 j = j + 1_ilp64
                 call stdlib_I64_dlals0( icompq, nl, nr, sqre, nrhs, b( nlf, 1_ilp64 ), ldb,bx( nlf, 1_ilp64 ), &
                 ldbx, perm( nlf, lvl ),givptr( j ), givcol( nlf, lvl2 ), ldgcol,givnum( nlf, &
                 lvl2 ), ldu, poles( nlf, lvl2 ),difl( nlf, lvl ), difr( nlf, lvl2 ),z( nlf, lvl )&
                           , k( j ), c( j ), s( j ), work,info )
              end do
           ...
           ! the nodes on the bottom level of the tree were solved
           ! by stdlib_I64_dlasdq. the corresponding right singular vector
           ! matrices are in explicit form. apply them back.
           ndb1 = ( nd+1 ) / 2_ilp64
           do i = ndb1, nd
              i1 = i - 1_ilp64
              ...
              nlf = ic - nl
              nrf = ic + 1_ilp64
              call stdlib_I64_dgemm( 'T', 'N', nlp1, nrhs, nlp1, one, vt( nlf, 1_ilp64 ), ldu,b( nlf, 1_ilp64 ), &
                        ldb, zero, bx( nlf, 1_ilp64 ), ldbx )
              call stdlib_I64_dgemm( 'T', 'N', nrp1, nrhs, nrp1, one, vt( nrf, 1_ilp64 ), ldu,b( nrf, 1_ilp64 ), &
                        ldb, zero, bx( nrf, 1_ilp64 ), ldbx )
           end do
           90 continue
           return
     ...
     !! compact form by CLALSA.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           integer(ilp64) :: i, i1, ic, im1, inode, j, jcol, jimag, jreal, jrow, lf, ll, lvl, lvl2, &
                     nd, ndb1, ndiml, ndimr, nl, nlf, nlp1, nlvl, nr, nrf, nrp1, sqre
           ! Intrinsic Functions 
           ! Executable Statements 
           ! test the input parameters.
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CLALSA', -info )
              return
           end if
           ! book-keeping and  setting up the computation tree.
           inode = 1_ilp64
           ...
           loop_130: do i = ndb1, nd
              ...
              nlf = ic - nl
              nrf = ic + 1_ilp64
              ! since b and bx are complex, the following call to stdlib_I64_sgemm
              ! is performed in two steps (real and imaginary parts).
              ! call stdlib_I64_sgemm( 't', 'n', nl, nrhs, nl, one, u( nlf, 1 ), ldu,
           ! $               b( nlf, 1 ), ldb, zero, bx( nlf, 1 ), ldbx )
              j = nl*nrhs*2_ilp64
              do jcol = 1, nrhs
                 do jrow = nlf, nlf + nl - 1
              ...
              call stdlib_I64_sgemm( 'T', 'N', nl, nrhs, nl, one, u( nlf, 1_ilp64 ), ldu,rwork( 1_ilp64+nl*nrhs*2_ilp64 &
                        ), nl, zero, rwork( 1_ilp64 ), nl )
              j = nl*nrhs*2_ilp64
              do jcol = 1, nrhs
                 do jrow = nlf, nlf + nl - 1
                    j = j + 1_ilp64
              ...
              call stdlib_I64_sgemm( 'T', 'N', nl, nrhs, nl, one, u( nlf, 1_ilp64 ), ldu,rwork( 1_ilp64+nl*nrhs*2_ilp64 &
                        ), nl, zero, rwork( 1_ilp64+nl*nrhs ),nl )
              jreal = 0_ilp64
              jimag = nl*nrhs
              do jcol = 1, nrhs
                 do jrow = nlf, nlf + nl - 1
              ...
              ! since b and bx are complex, the following call to stdlib_I64_sgemm
              ! is performed in two steps (real and imaginary parts).
              ! call stdlib_I64_sgemm( 't', 'n', nr, nrhs, nr, one, u( nrf, 1 ), ldu,
          ! $               b( nrf, 1 ), ldb, zero, bx( nrf, 1 ), ldbx )
              j = nr*nrhs*2_ilp64
              do jcol = 1, nrhs
                 do jrow = nrf, nrf + nr - 1
              ...
              call stdlib_I64_sgemm( 'T', 'N', nr, nrhs, nr, one, u( nrf, 1_ilp64 ), ldu,rwork( 1_ilp64+nr*nrhs*2_ilp64 &
                        ), nr, zero, rwork( 1_ilp64 ), nr )
              j = nr*nrhs*2_ilp64
              do jcol = 1, nrhs
                 do jrow = nrf, nrf + nr - 1
                    j = j + 1_ilp64
              ...
              call stdlib_I64_sgemm( 'T', 'N', nr, nrhs, nr, one, u( nrf, 1_ilp64 ), ldu,rwork( 1_ilp64+nr*nrhs*2_ilp64 &
                        ), nr, zero, rwork( 1_ilp64+nr*nrhs ),nr )
              jreal = 0_ilp64
              jimag = nr*nrhs
              do jcol = 1, nrhs
                 do jrow = nrf, nrf + nr - 1
           ...
           do i = 1, nd
              ic = iwork( inode+i-1 )
              call stdlib_I64_ccopy( nrhs, b( ic, 1_ilp64 ), ldb, bx( ic, 1_ilp64 ), ldbx )
           end do
           ! finally go through the left singular vector matrices of all
           ! the other subproblems bottom-up on the tree.
           j = 2_ilp64**nlvl
           ...
           do lvl = nlvl, 1, -1
              ...
              do i = lf, ll
                 ...
                 nrf = ic + 1_ilp64
                 j = j - 1_ilp64
                 call stdlib_I64_clals0( icompq, nl, nr, sqre, nrhs, bx( nlf, 1_ilp64 ), ldbx,b( nlf, 1_ilp64 ), &
                 ldb, perm( nlf, lvl ),givptr( j ), givcol( nlf, lvl2 ), ldgcol,givnum( nlf, lvl2 &
                 ), ldu, poles( nlf, lvl2 ),difl( nlf, lvl ), difr( nlf, lvl2 ),z( nlf, lvl ), k( &
                           j ), c( j ), s( j ), rwork,info )
              end do
           ...
           loop_190: do lvl = 1, nlvl
              ...
              do i = ll, lf, -1
                 ...
                 j = j + 1_ilp64
                 call stdlib_I64_clals0( icompq, nl, nr, sqre, nrhs, b( nlf, 1_ilp64 ), ldb,bx( nlf, 1_ilp64 ), &
                 ldbx, perm( nlf, lvl ),givptr( j ), givcol( nlf, lvl2 ), ldgcol,givnum( nlf, &
                 lvl2 ), ldu, poles( nlf, lvl2 ),difl( nlf, lvl ), difr( nlf, lvl2 ),z( nlf, lvl )&
                           , k( j ), c( j ), s( j ), rwork,info )
              end do
           ...
           loop_320: do i = ndb1, nd
              ...
              nlf = ic - nl
              nrf = ic + 1_ilp64
              ! since b and bx are complex, the following call to stdlib_I64_sgemm is
              ! performed in two steps (real and imaginary parts).
              ! call stdlib_I64_sgemm( 't', 'n', nlp1, nrhs, nlp1, one, vt( nlf, 1 ), ldu,
          ! $               b( nlf, 1 ), ldb, zero, bx( nlf, 1 ), ldbx )
              j = nlp1*nrhs*2_ilp64
              do jcol = 1, nrhs
                 do jrow = nlf, nlf + nlp1 - 1
              ...
              call stdlib_I64_sgemm( 'T', 'N', nlp1, nrhs, nlp1, one, vt( nlf, 1_ilp64 ), ldu,rwork( 1_ilp64+&
                        nlp1*nrhs*2_ilp64 ), nlp1, zero, rwork( 1_ilp64 ),nlp1 )
              j = nlp1*nrhs*2_ilp64
              do jcol = 1, nrhs
                 do jrow = nlf, nlf + nlp1 - 1
                    j = j + 1_ilp64
              ...
              call stdlib_I64_sgemm( 'T', 'N', nlp1, nrhs, nlp1, one, vt( nlf, 1_ilp64 ), ldu,rwork( 1_ilp64+&
                        nlp1*nrhs*2_ilp64 ), nlp1, zero,rwork( 1_ilp64+nlp1*nrhs ), nlp1 )
              jreal = 0_ilp64
              jimag = nlp1*nrhs
              do jcol = 1, nrhs
                 do jrow = nlf, nlf + nlp1 - 1
              ...
              ! since b and bx are complex, the following call to stdlib_I64_sgemm is
              ! performed in two steps (real and imaginary parts).
              ! call stdlib_I64_sgemm( 't', 'n', nrp1, nrhs, nrp1, one, vt( nrf, 1 ), ldu,
          ! $               b( nrf, 1 ), ldb, zero, bx( nrf, 1 ), ldbx )
              j = nrp1*nrhs*2_ilp64
              do jcol = 1, nrhs
                 do jrow = nrf, nrf + nrp1 - 1
              ...
              call stdlib_I64_sgemm( 'T', 'N', nrp1, nrhs, nrp1, one, vt( nrf, 1_ilp64 ), ldu,rwork( 1_ilp64+&
                        nrp1*nrhs*2_ilp64 ), nrp1, zero, rwork( 1_ilp64 ),nrp1 )
              j = nrp1*nrhs*2_ilp64
              do jcol = 1, nrhs
                 do jrow = nrf, nrf + nrp1 - 1
                    j = j + 1_ilp64
              ...
              call stdlib_I64_sgemm( 'T', 'N', nrp1, nrhs, nrp1, one, vt( nrf, 1_ilp64 ), ldu,rwork( 1_ilp64+&
                        nrp1*nrhs*2_ilp64 ), nrp1, zero,rwork( 1_ilp64+nrp1*nrhs ), nrp1 )
              jreal = 0_ilp64
              jimag = nrp1*nrhs
              do jcol = 1, nrhs
                 do jrow = nrf, nrf + nrp1 - 1
     ...
     !! compact form by ZLALSA.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZLALSA', -info )
              return
           end if
           ! book-keeping and  setting up the computation tree.
           inode = 1_ilp64
           ...
           loop_130: do i = ndb1, nd
              ...
              nlf = ic - nl
              nrf = ic + 1_ilp64
              ! since b and bx are complex, the following call to stdlib_I64_dgemm
              ! is performed in two steps (real and imaginary parts).
              ! call stdlib_I64_dgemm( 't', 'n', nl, nrhs, nl, one, u( nlf, 1 ), ldu,
           ! $               b( nlf, 1 ), ldb, zero, bx( nlf, 1 ), ldbx )
              j = nl*nrhs*2_ilp64
              do jcol = 1, nrhs
                 do jrow = nlf, nlf + nl - 1
              ...
              call stdlib_I64_dgemm( 'T', 'N', nl, nrhs, nl, one, u( nlf, 1_ilp64 ), ldu,rwork( 1_ilp64+nl*nrhs*2_ilp64 &
                        ), nl, zero, rwork( 1_ilp64 ), nl )
              j = nl*nrhs*2_ilp64
              do jcol = 1, nrhs
                 do jrow = nlf, nlf + nl - 1
              ...
              ! since b and bx are complex, the following call to stdlib_I64_dgemm
              ! is performed in two steps (real and imaginary parts).
              ! call stdlib_I64_dgemm( 't', 'n', nr, nrhs, nr, one, u( nrf, 1 ), ldu,
          ! $               b( nrf, 1 ), ldb, zero, bx( nrf, 1 ), ldbx )
              j = nr*nrhs*2_ilp64
              do jcol = 1, nrhs
                 do jrow = nrf, nrf + nr - 1
              ...
              call stdlib_I64_dgemm( 'T', 'N', nr, nrhs, nr, one, u( nrf, 1_ilp64 ), ldu,rwork( 1_ilp64+nr*nrhs*2_ilp64 &
                        ), nr, zero, rwork( 1_ilp64 ), nr )
              j = nr*nrhs*2_ilp64
              do jcol = 1, nrhs
                 do jrow = nrf, nrf + nr - 1
           ...
           do i = 1, nd
              ic = iwork( inode+i-1 )
              call stdlib_I64_zcopy( nrhs, b( ic, 1_ilp64 ), ldb, bx( ic, 1_ilp64 ), ldbx )
           end do
           ! finally go through the left singular vector matrices of all
           ! the other subproblems bottom-up on the tree.
           j = 2_ilp64**nlvl
           ...
           do lvl = nlvl, 1, -1
              ...
              do i = lf, ll
                 ...
                 nrf = ic + 1_ilp64
                 j = j - 1_ilp64
                 call stdlib_I64_zlals0( icompq, nl, nr, sqre, nrhs, bx( nlf, 1_ilp64 ), ldbx,b( nlf, 1_ilp64 ), &
                 ldb, perm( nlf, lvl ),givptr( j ), givcol( nlf, lvl2 ), ldgcol,givnum( nlf, lvl2 &
                 ), ldu, poles( nlf, lvl2 ),difl( nlf, lvl ), difr( nlf, lvl2 ),z( nlf, lvl ), k( &
                           j ), c( j ), s( j ), rwork,info )
              end do
           ...
           loop_190: do lvl = 1, nlvl
              ...
              do i = ll, lf, -1
                 ...
                 j = j + 1_ilp64
                 call stdlib_I64_zlals0( icompq, nl, nr, sqre, nrhs, b( nlf, 1_ilp64 ), ldb,bx( nlf, 1_ilp64 ), &
                 ldbx, perm( nlf, lvl ),givptr( j ), givcol( nlf, lvl2 ), ldgcol,givnum( nlf, &
                 lvl2 ), ldu, poles( nlf, lvl2 ),difl( nlf, lvl ), difr( nlf, lvl2 ),z( nlf, lvl )&
                           , k( j ), c( j ), s( j ), rwork,info )
              end do
           ...
           loop_320: do i = ndb1, nd
              ...
              nlf = ic - nl
              nrf = ic + 1_ilp64
              ! since b and bx are complex, the following call to stdlib_I64_dgemm is
              ! performed in two steps (real and imaginary parts).
              ! call stdlib_I64_dgemm( 't', 'n', nlp1, nrhs, nlp1, one, vt( nlf, 1 ), ldu,
          ! $               b( nlf, 1 ), ldb, zero, bx( nlf, 1 ), ldbx )
              j = nlp1*nrhs*2_ilp64
              do jcol = 1, nrhs
                 do jrow = nlf, nlf + nlp1 - 1
              ...
              call stdlib_I64_dgemm( 'T', 'N', nlp1, nrhs, nlp1, one, vt( nlf, 1_ilp64 ), ldu,rwork( 1_ilp64+&
                        nlp1*nrhs*2_ilp64 ), nlp1, zero, rwork( 1_ilp64 ),nlp1 )
              j = nlp1*nrhs*2_ilp64
              do jcol = 1, nrhs
                 do jrow = nlf, nlf + nlp1 - 1
              ...
              ! since b and bx are complex, the following call to stdlib_I64_dgemm is
              ! performed in two steps (real and imaginary parts).
              ! call stdlib_I64_dgemm( 't', 'n', nrp1, nrhs, nrp1, one, vt( nrf, 1 ), ldu,
          ! $               b( nrf, 1 ), ldb, zero, bx( nrf, 1 ), ldbx )
              j = nrp1*nrhs*2_ilp64
              do jcol = 1, nrhs
                 do jrow = nrf, nrf + nrp1 - 1
              ...
              call stdlib_I64_dgemm( 'T', 'N', nrp1, nrhs, nrp1, one, vt( nrf, 1_ilp64 ), ldu,rwork( 1_ilp64+&
                        nrp1*nrhs*2_ilp64 ), nrp1, zero, rwork( 1_ilp64 ),nrp1 )
              j = nrp1*nrhs*2_ilp64
              do jcol = 1, nrhs
                 do jrow = nrf, nrf + nrp1 - 1
     ...
     !! without guard digits, but we know of none.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           integer(ilp64) :: bx, bxst, c, difl, difr, givcol, givnum, givptr, i, icmpq1, icmpq2, &
           iwk, j, k, nlvl, nm1, nsize, nsub, nwork, perm, poles, s, sizei, smlszp, sqre, st, st1,&
                      u, vt, z
           real(sp) :: cs, eps, orgnrm, r, rcnd, sn, tol
           ! Intrinsic Functions 
           ...
           if( n<0_ilp64 ) then
              ...
              info = -4_ilp64
           else if( ( ldb<1_ilp64 ) .or. ( ldb<n ) ) then
              info = -8_ilp64
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SLALSD', -info )
              return
           end if
           eps = stdlib_I64_slamch( 'EPSILON' )
           ! set up the tolerance.
           ...
           if( n==0_ilp64 ) then
              ...
              if( d( 1_ilp64 )==zero ) then
                 call stdlib_I64_slaset( 'A', 1_ilp64, nrhs, zero, zero, b, ldb )
              else
                 rank = 1_ilp64
                 call stdlib_I64_slascl( 'G', 0_ilp64, 0_ilp64, d( 1_ilp64 ), one, 1_ilp64, nrhs, b, ldb, info )
           ...
           if( uplo=='L' ) then
              do i = 1, n - 1
                 ...
                 if( nrhs==1_ilp64 ) then
                    call stdlib_I64_srot( 1_ilp64, b( i, 1_ilp64 ), 1_ilp64, b( i+1, 1_ilp64 ), 1_ilp64, cs, sn )
                 else
                    work( i*2_ilp64-1 ) = cs
                    work( i*2_ilp64 ) = sn
                 end if
              ...
              if( nrhs>1_ilp64 ) then
                 do i = 1, nrhs
                    do j = 1, n - 1
                       cs = work( j*2_ilp64-1 )
                       sn = work( j*2_ilp64 )
                       call stdlib_I64_srot( 1_ilp64, b( j, i ), 1_ilp64, b( j+1, i ), 1_ilp64, cs, sn )
                    end do
                 end do
              end if
           end if
           ...
           if( orgnrm==zero ) then
              call stdlib_I64_slaset( 'A', n, nrhs, zero, zero, b, ldb )
              return
           end if
           call stdlib_I64_slascl( 'G', 0_ilp64, 0_ilp64, orgnrm, one, n, 1_ilp64, d, n, info )
           call stdlib_I64_slascl( 'G', 0_ilp64, 0_ilp64, orgnrm, one, nm1, 1_ilp64, e, nm1, info )
           ! if n is smaller than the minimum divide size smlsiz, then solve
           ! the problem with another solver.
           if( n<=smlsiz ) then
              nwork = 1_ilp64 + n*n
              call stdlib_I64_slaset( 'A', n, n, zero, one, work, n )
              call stdlib_I64_slasdq( 'U', 0_ilp64, n, n, 0_ilp64, nrhs, d, e, work, n, work, n, b,ldb, work( &
                        nwork ), info )
              if( info/=0_ilp64 ) then
                 return
              end if
              tol = rcnd*abs( d( stdlib_I64_isamax( n, d, 1_ilp64 ) ) )
              do i = 1, n
                 if( d( i )<=tol ) then
                    call stdlib_I64_slaset( 'A', 1_ilp64, nrhs, zero, zero, b( i, 1_ilp64 ), ldb )
                 else
                    call stdlib_I64_slascl( 'G', 0_ilp64, 0_ilp64, d( i ), one, 1_ilp64, nrhs, b( i, 1_ilp64 ),ldb, info )
                              
                    rank = rank + 1_ilp64
                 end if
              end do
              call stdlib_I64_sgemm( 'T', 'N', n, nrhs, n, one, work, n, b, ldb, zero,work( nwork ), &
                        n )
              call stdlib_I64_slacpy( 'A', n, nrhs, work( nwork ), n, b, ldb )
              ! unscale.
              call stdlib_I64_slascl( 'G', 0_ilp64, 0_ilp64, one, orgnrm, n, 1_ilp64, d, n, info )
              call stdlib_I64_slasrt( 'D', n, d, info )
              call stdlib_I64_slascl( 'G', 0_ilp64, 0_ilp64, orgnrm, one, n, nrhs, b, ldb, info )
              return
           end if
           ! book-keeping and setting up some constants.
           nlvl = int( log( real( n,KIND=sp) / real( smlsiz+1,KIND=sp) ) / log( two ),KIND=ilp64) + &
                     1_ilp64
           ...
           difl = vt + smlszp*n
           difr = difl + nlvl*n
           z = difr + nlvl*n*2_ilp64
           c = z + nlvl*n
           s = c + n
           poles = s + n
           givnum = poles + 2_ilp64*nlvl*n
           bx = givnum + 2_ilp64*nlvl*n
           nwork = bx + n*nrhs
           sizei = 1_ilp64 + n
           k = sizei + n
           givptr = k + n
           ...
           loop_60: do i = 1, nm1
              if( ( abs( e( i ) )<eps ) .or. ( i==nm1 ) ) then
                 ...
                 if( i<nm1 ) then
                    ...
                    nsub = nsub + 1_ilp64
                    iwork( nsub ) = n
                    iwork( sizei+nsub-1 ) = 1_ilp64
                    call stdlib_I64_scopy( nrhs, b( n, 1_ilp64 ), ldb, work( bx+nm1 ), n )
                 end if
                 st1 = st - 1_ilp64
                 if( nsize==1_ilp64 ) then
                    ! this is a 1-by-1 subproblem and is not solved
                    ! explicitly.
                    call stdlib_I64_scopy( nrhs, b( st, 1_ilp64 ), ldb, work( bx+st1 ), n )
                 else if( nsize<=smlsiz ) then
                    ! this is a small subproblem and is solved by stdlib_I64_slasdq.
                    call stdlib_I64_slaset( 'A', nsize, nsize, zero, one,work( vt+st1 ), n )
                    call stdlib_I64_slasdq( 'U', 0_ilp64, nsize, nsize, 0_ilp64, nrhs, d( st ),e( st ), work( vt+&
                              st1 ), n, work( nwork ),n, b( st, 1_ilp64 ), ldb, work( nwork ), info )
                    if( info/=0_ilp64 ) then
                       return
                    end if
                    call stdlib_I64_slacpy( 'A', nsize, nrhs, b( st, 1_ilp64 ), ldb,work( bx+st1 ), n )
                              
                 else
                    ! a large problem. solve it using divide and conquer.
                    call stdlib_I64_slasda( icmpq1, smlsiz, nsize, sqre, d( st ),e( st ), work( u+st1 &
                    ), n, work( vt+st1 ),iwork( k+st1 ), work( difl+st1 ),work( difr+st1 ), work( &
                    z+st1 ),work( poles+st1 ), iwork( givptr+st1 ),iwork( givcol+st1 ), n, iwork( &
                    perm+st1 ),work( givnum+st1 ), work( c+st1 ),work( s+st1 ), work( nwork ), &
                              iwork( iwk ),info )
                    ...
                    bxst = bx + st1
                    call stdlib_I64_slalsa( icmpq2, smlsiz, nsize, nrhs, b( st, 1_ilp64 ),ldb, work( bxst ),&
                     n, work( u+st1 ), n,work( vt+st1 ), iwork( k+st1 ),work( difl+st1 ), work( &
                     difr+st1 ),work( z+st1 ), work( poles+st1 ),iwork( givptr+st1 ), iwork( &
                     givcol+st1 ), n,iwork( perm+st1 ), work( givnum+st1 ),work( c+st1 ), work( s+&
                               st1 ), work( nwork ),iwork( iwk ), info )
           ...
           do i = 1, n
              ...
              ! subproblems were not solved explicitly.
              if( abs( d( i ) )<=tol ) then
                 call stdlib_I64_slaset( 'A', 1_ilp64, nrhs, zero, zero, work( bx+i-1 ), n )
              else
                 rank = rank + 1_ilp64
                 call stdlib_I64_slascl( 'G', 0_ilp64, 0_ilp64, d( i ), one, 1_ilp64, nrhs,work( bx+i-1 ), n, info )
           ...
           do i = 1, nsub
              ...
              bxst = bx + st1
              if( nsize==1_ilp64 ) then
                 call stdlib_I64_scopy( nrhs, work( bxst ), n, b( st, 1_ilp64 ), ldb )
              else if( nsize<=smlsiz ) then
                 call stdlib_I64_sgemm( 'T', 'N', nsize, nrhs, nsize, one,work( vt+st1 ), n, work( &
                           bxst ), n, zero,b( st, 1_ilp64 ), ldb )
              else
                 call stdlib_I64_slalsa( icmpq2, smlsiz, nsize, nrhs, work( bxst ), n,b( st, 1_ilp64 ), ldb,&
                  work( u+st1 ), n,work( vt+st1 ), iwork( k+st1 ),work( difl+st1 ), work( difr+&
                  st1 ),work( z+st1 ), work( poles+st1 ),iwork( givptr+st1 ), iwork( givcol+st1 ),&
                   n,iwork( perm+st1 ), work( givnum+st1 ),work( c+st1 ), work( s+st1 ), work( &
                             nwork ),iwork( iwk ), info )
     ...
     !! without guard digits, but we know of none.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DLALSD', -info )
              return
           end if
           eps = stdlib_I64_dlamch( 'EPSILON' )
           ! set up the tolerance.
           ...
           if( n==0_ilp64 ) then
              ...
              if( d( 1_ilp64 )==zero ) then
                 call stdlib_I64_dlaset( 'A', 1_ilp64, nrhs, zero, zero, b, ldb )
              else
                 rank = 1_ilp64
                 call stdlib_I64_dlascl( 'G', 0_ilp64, 0_ilp64, d( 1_ilp64 ), one, 1_ilp64, nrhs, b, ldb, info )
                 d( 1_ilp64 ) = abs( d( 1_ilp64 ) )
              end if
              return
           end if
           ...
           if( uplo=='L' ) then
              do i = 1, n - 1
                 ...
                 d( i+1 ) = cs*d( i+1 )
                 if( nrhs==1_ilp64 ) then
                    call stdlib_I64_drot( 1_ilp64, b( i, 1_ilp64 ), 1_ilp64, b( i+1, 1_ilp64 ), 1_ilp64, cs, sn )
                 else
                    work( i*2_ilp64-1 ) = cs
                    work( i*2_ilp64 ) = sn
                 end if
              ...
              if( nrhs>1_ilp64 ) then
                 do i = 1, nrhs
                    do j = 1, n - 1
                       cs = work( j*2_ilp64-1 )
                       sn = work( j*2_ilp64 )
                       call stdlib_I64_drot( 1_ilp64, b( j, i ), 1_ilp64, b( j+1, i ), 1_ilp64, cs, sn )
                    end do
                 end do
              end if
           end if
           ...
           if( orgnrm==zero ) then
              call stdlib_I64_dlaset( 'A', n, nrhs, zero, zero, b, ldb )
              return
           end if
           call stdlib_I64_dlascl( 'G', 0_ilp64, 0_ilp64, orgnrm, one, n, 1_ilp64, d, n, info )
           call stdlib_I64_dlascl( 'G', 0_ilp64, 0_ilp64, orgnrm, one, nm1, 1_ilp64, e, nm1, info )
           ! if n is smaller than the minimum divide size smlsiz, then solve
           ! the problem with another solver.
           if( n<=smlsiz ) then
              nwork = 1_ilp64 + n*n
              call stdlib_I64_dlaset( 'A', n, n, zero, one, work, n )
              call stdlib_I64_dlasdq( 'U', 0_ilp64, n, n, 0_ilp64, nrhs, d, e, work, n, work, n, b,ldb, work( &
                        nwork ), info )
              if( info/=0_ilp64 ) then
                 return
              end if
              tol = rcnd*abs( d( stdlib_I64_idamax( n, d, 1_ilp64 ) ) )
              do i = 1, n
                 if( d( i )<=tol ) then
                    call stdlib_I64_dlaset( 'A', 1_ilp64, nrhs, zero, zero, b( i, 1_ilp64 ), ldb )
                 else
                    call stdlib_I64_dlascl( 'G', 0_ilp64, 0_ilp64, d( i ), one, 1_ilp64, nrhs, b( i, 1_ilp64 ),ldb, info )
                              
                    rank = rank + 1_ilp64
                 end if
              end do
              call stdlib_I64_dgemm( 'T', 'N', n, nrhs, n, one, work, n, b, ldb, zero,work( nwork ), &
                        n )
              call stdlib_I64_dlacpy( 'A', n, nrhs, work( nwork ), n, b, ldb )
              ! unscale.
              call stdlib_I64_dlascl( 'G', 0_ilp64, 0_ilp64, one, orgnrm, n, 1_ilp64, d, n, info )
              call stdlib_I64_dlasrt( 'D', n, d, info )
              call stdlib_I64_dlascl( 'G', 0_ilp64, 0_ilp64, orgnrm, one, n, nrhs, b, ldb, info )
              return
           end if
           ! book-keeping and setting up some constants.
           nlvl = int( log( real( n,KIND=dp) / real( smlsiz+1,KIND=dp) ) / log( two ),KIND=ilp64) + &
                     1_ilp64
           smlszp = smlsiz + 1_ilp64
           u = 1_ilp64
           vt = 1_ilp64 + smlsiz*n
           ...
           loop_60: do i = 1, nm1
              if( ( abs( e( i ) )<eps ) .or. ( i==nm1 ) ) then
                 ...
                 if( i<nm1 ) then
                    ...
                    iwork( nsub ) = n
                    iwork( sizei+nsub-1 ) = 1_ilp64
                    call stdlib_I64_dcopy( nrhs, b( n, 1_ilp64 ), ldb, work( bx+nm1 ), n )
                 end if
                 st1 = st - 1_ilp64
                 if( nsize==1_ilp64 ) then
                    ! this is a 1-by-1 subproblem and is not solved
                    ! explicitly.
                    call stdlib_I64_dcopy( nrhs, b( st, 1_ilp64 ), ldb, work( bx+st1 ), n )
                 else if( nsize<=smlsiz ) then
                    ! this is a small subproblem and is solved by stdlib_I64_dlasdq.
                    call stdlib_I64_dlaset( 'A', nsize, nsize, zero, one,work( vt+st1 ), n )
                    call stdlib_I64_dlasdq( 'U', 0_ilp64, nsize, nsize, 0_ilp64, nrhs, d( st ),e( st ), work( vt+&
                              st1 ), n, work( nwork ),n, b( st, 1_ilp64 ), ldb, work( nwork ), info )
                    if( info/=0_ilp64 ) then
                       return
                    end if
                    call stdlib_I64_dlacpy( 'A', nsize, nrhs, b( st, 1_ilp64 ), ldb,work( bx+st1 ), n )
                              
                 else
                    ! a large problem. solve it using divide and conquer.
                    call stdlib_I64_dlasda( icmpq1, smlsiz, nsize, sqre, d( st ),e( st ), work( u+st1 &
                    ), n, work( vt+st1 ),iwork( k+st1 ), work( difl+st1 ),work( difr+st1 ), work( &
                    z+st1 ),work( poles+st1 ), iwork( givptr+st1 ),iwork( givcol+st1 ), n, iwork( &
                    perm+st1 ),work( givnum+st1 ), work( c+st1 ),work( s+st1 ), work( nwork ), &
                              iwork( iwk ),info )
                    ...
                    bxst = bx + st1
                    call stdlib_I64_dlalsa( icmpq2, smlsiz, nsize, nrhs, b( st, 1_ilp64 ),ldb, work( bxst ),&
                     n, work( u+st1 ), n,work( vt+st1 ), iwork( k+st1 ),work( difl+st1 ), work( &
                     difr+st1 ),work( z+st1 ), work( poles+st1 ),iwork( givptr+st1 ), iwork( &
                     givcol+st1 ), n,iwork( perm+st1 ), work( givnum+st1 ),work( c+st1 ), work( s+&
                               st1 ), work( nwork ),iwork( iwk ), info )
           ...
           do i = 1, n
              ...
              ! subproblems were not solved explicitly.
              if( abs( d( i ) )<=tol ) then
                 call stdlib_I64_dlaset( 'A', 1_ilp64, nrhs, zero, zero, work( bx+i-1 ), n )
              else
                 rank = rank + 1_ilp64
                 call stdlib_I64_dlascl( 'G', 0_ilp64, 0_ilp64, d( i ), one, 1_ilp64, nrhs,work( bx+i-1 ), n, info )
           ...
           do i = 1, nsub
              ...
              bxst = bx + st1
              if( nsize==1_ilp64 ) then
                 call stdlib_I64_dcopy( nrhs, work( bxst ), n, b( st, 1_ilp64 ), ldb )
              else if( nsize<=smlsiz ) then
                 call stdlib_I64_dgemm( 'T', 'N', nsize, nrhs, nsize, one,work( vt+st1 ), n, work( &
                           bxst ), n, zero,b( st, 1_ilp64 ), ldb )
              else
                 call stdlib_I64_dlalsa( icmpq2, smlsiz, nsize, nrhs, work( bxst ), n,b( st, 1_ilp64 ), ldb,&
                  work( u+st1 ), n,work( vt+st1 ), iwork( k+st1 ),work( difl+st1 ), work( difr+&
                  st1 ),work( z+st1 ), work( poles+st1 ),iwork( givptr+st1 ), iwork( givcol+st1 ),&
                   n,iwork( perm+st1 ), work( givnum+st1 ),work( c+st1 ), work( s+st1 ), work( &
                             nwork ),iwork( iwk ), info )
     ...
     !! without guard digits, but we know of none.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CLALSD', -info )
              return
           end if
           eps = stdlib_I64_slamch( 'EPSILON' )
           ! set up the tolerance.
           ...
           if( n==0_ilp64 ) then
              ...
              if( d( 1_ilp64 )==zero ) then
                 call stdlib_I64_claset( 'A', 1_ilp64, nrhs, czero, czero, b, ldb )
              else
                 rank = 1_ilp64
                 call stdlib_I64_clascl( 'G', 0_ilp64, 0_ilp64, d( 1_ilp64 ), one, 1_ilp64, nrhs, b, ldb, info )
                 d( 1_ilp64 ) = abs( d( 1_ilp64 ) )
              end if
              return
           end if
           ...
           if( uplo=='L' ) then
              do i = 1, n - 1
                 ...
                 d( i+1 ) = cs*d( i+1 )
                 if( nrhs==1_ilp64 ) then
                    call stdlib_I64_csrot( 1_ilp64, b( i, 1_ilp64 ), 1_ilp64, b( i+1, 1_ilp64 ), 1_ilp64, cs, sn )
                 else
                    rwork( i*2_ilp64-1 ) = cs
                    rwork( i*2_ilp64 ) = sn
                 end if
              end do
              if( nrhs>1_ilp64 ) then
                 do i = 1, nrhs
                    do j = 1, n - 1
                       cs = rwork( j*2_ilp64-1 )
                       sn = rwork( j*2_ilp64 )
                       call stdlib_I64_csrot( 1_ilp64, b( j, i ), 1_ilp64, b( j+1, i ), 1_ilp64, cs, sn )
                    end do
                 end do
              end if
           end if
           ...
           orgnrm = stdlib_I64_slanst( 'M', n, d, e )
           if( orgnrm==zero ) then
              call stdlib_I64_claset( 'A', n, nrhs, czero, czero, b, ldb )
              return
           end if
           call stdlib_I64_slascl( 'G', 0_ilp64, 0_ilp64, orgnrm, one, n, 1_ilp64, d, n, info )
           call stdlib_I64_slascl( 'G', 0_ilp64, 0_ilp64, orgnrm, one, nm1, 1_ilp64, e, nm1, info )
           ...
           if( n<=smlsiz ) then
              ...
              irwib = irwrb + n*nrhs
              irwb = irwib + n*nrhs
              call stdlib_I64_slaset( 'A', n, n, zero, one, rwork( irwu ), n )
              call stdlib_I64_slaset( 'A', n, n, zero, one, rwork( irwvt ), n )
              call stdlib_I64_slasdq( 'U', 0_ilp64, n, n, n, 0_ilp64, d, e, rwork( irwvt ), n,rwork( irwu ), n, &
                        rwork( irwwrk ), 1_ilp64,rwork( irwwrk ), info )
              if( info/=0_ilp64 ) then
                 return
              end if
              ! in the real version, b is passed to stdlib_I64_slasdq and multiplied
              ! internally by q**h. here b is complex and that product is
              ! computed below in two steps (real and imaginary parts).
              j = irwb - 1_ilp64
              do jcol = 1, nrhs
              ...
              call stdlib_I64_sgemm( 'T', 'N', n, nrhs, n, one, rwork( irwu ), n,rwork( irwb ), n, &
                        zero, rwork( irwrb ), n )
              j = irwb - 1_ilp64
              do jcol = 1, nrhs
                 do jrow = 1, n
              ...
              do i = 1, n
                 if( d( i )<=tol ) then
                    call stdlib_I64_claset( 'A', 1_ilp64, nrhs, czero, czero, b( i, 1_ilp64 ), ldb )
                 else
                    call stdlib_I64_clascl( 'G', 0_ilp64, 0_ilp64, d( i ), one, 1_ilp64, nrhs, b( i, 1_ilp64 ),ldb, info )
                              
                    rank = rank + 1_ilp64
                 end if
              end do
              ! since b is complex, the following call to stdlib_I64_sgemm is performed
              ! in two steps (real and imaginary parts). that is for v * b
              ! (in the real version of the code v**h is stored in work).
              ! call stdlib_I64_sgemm( 't', 'n', n, nrhs, n, one, work, n, b, ldb, zero,
          ! $               work( nwork ), n )
              j = irwb - 1_ilp64
              do jcol = 1, nrhs
                 do jrow = 1, n
              ...
              call stdlib_I64_sgemm( 'T', 'N', n, nrhs, n, one, rwork( irwvt ), n,rwork( irwb ), n, &
                        zero, rwork( irwrb ), n )
              j = irwb - 1_ilp64
              do jcol = 1, nrhs
                 do jrow = 1, n
              ...
              call stdlib_I64_slascl( 'G', 0_ilp64, 0_ilp64, one, orgnrm, n, 1_ilp64, d, n, info )
              call stdlib_I64_slasrt( 'D', n, d, info )
              call stdlib_I64_clascl( 'G', 0_ilp64, 0_ilp64, orgnrm, one, n, nrhs, b, ldb, info )
              return
           end if
           ! book-keeping and setting up some constants.
           nlvl = int( log( real( n,KIND=sp) / real( smlsiz+1,KIND=sp) ) / log( two ),KIND=ilp64) + &
           ...
           loop_240: do i = 1, nm1
              if( ( abs( e( i ) )<eps ) .or. ( i==nm1 ) ) then
                 ...
                 if( i<nm1 ) then
                    ...
                    iwork( nsub ) = n
                    iwork( sizei+nsub-1 ) = 1_ilp64
                    call stdlib_I64_ccopy( nrhs, b( n, 1_ilp64 ), ldb, work( bx+nm1 ), n )
                 end if
                 st1 = st - 1_ilp64
                 if( nsize==1_ilp64 ) then
                    ! this is a 1-by-1 subproblem and is not solved
                    ! explicitly.
                    call stdlib_I64_ccopy( nrhs, b( st, 1_ilp64 ), ldb, work( bx+st1 ), n )
                 else if( nsize<=smlsiz ) then
                    ! this is a small subproblem and is solved by stdlib_I64_slasdq.
                    call stdlib_I64_slaset( 'A', nsize, nsize, zero, one,rwork( vt+st1 ), n )
                    call stdlib_I64_slaset( 'A', nsize, nsize, zero, one,rwork( u+st1 ), n )
                    call stdlib_I64_slasdq( 'U', 0_ilp64, nsize, nsize, nsize, 0_ilp64, d( st ),e( st ), rwork( &
                    vt+st1 ), n, rwork( u+st1 ),n, rwork( nrwork ), 1_ilp64, rwork( nrwork ),info )
                    ...
                    call stdlib_I64_sgemm( 'T', 'N', nsize, nrhs, nsize, one,rwork( u+st1 ), n, rwork(&
                               irwb ), nsize,zero, rwork( irwrb ), nsize )
                    j = irwb - 1_ilp64
                    do jcol = 1, nrhs
                       do jrow = st, st + nsize - 1
                    ...
                    call stdlib_I64_clacpy( 'A', nsize, nrhs, b( st, 1_ilp64 ), ldb,work( bx+st1 ), n )
                              
                 else
                    ! a large problem. solve it using divide and conquer.
                    call stdlib_I64_slasda( icmpq1, smlsiz, nsize, sqre, d( st ),e( st ), rwork( u+&
                    st1 ), n, rwork( vt+st1 ),iwork( k+st1 ), rwork( difl+st1 ),rwork( difr+st1 ),&
                     rwork( z+st1 ),rwork( poles+st1 ), iwork( givptr+st1 ),iwork( givcol+st1 ), &
                     n, iwork( perm+st1 ),rwork( givnum+st1 ), rwork( c+st1 ),rwork( s+st1 ), &
                               rwork( nrwork ),iwork( iwk ), info )
                    ...
                    bxst = bx + st1
                    call stdlib_I64_clalsa( icmpq2, smlsiz, nsize, nrhs, b( st, 1_ilp64 ),ldb, work( bxst ),&
                     n, rwork( u+st1 ), n,rwork( vt+st1 ), iwork( k+st1 ),rwork( difl+st1 ), &
                     rwork( difr+st1 ),rwork( z+st1 ), rwork( poles+st1 ),iwork( givptr+st1 ), &
                     iwork( givcol+st1 ), n,iwork( perm+st1 ), rwork( givnum+st1 ),rwork( c+st1 ),&
                                rwork( s+st1 ),rwork( nrwork ), iwork( iwk ), info )
           ...
           do i = 1, n
              ...
              ! subproblems were not solved explicitly.
              if( abs( d( i ) )<=tol ) then
                 call stdlib_I64_claset( 'A', 1_ilp64, nrhs, czero, czero, work( bx+i-1 ), n )
              else
                 rank = rank + 1_ilp64
                 call stdlib_I64_clascl( 'G', 0_ilp64, 0_ilp64, d( i ), one, 1_ilp64, nrhs,work( bx+i-1 ), n, info )
           ...
           loop_320: do i = 1, nsub
              ...
              bxst = bx + st1
              if( nsize==1_ilp64 ) then
                 call stdlib_I64_ccopy( nrhs, work( bxst ), n, b( st, 1_ilp64 ), ldb )
              else if( nsize<=smlsiz ) then
                 ! since b and bx are complex, the following call to stdlib_I64_sgemm
                 ! is performed in two steps (real and imaginary parts).
                 ! call stdlib_I64_sgemm( 't', 'n', nsize, nrhs, nsize, one,
          ! $                  rwork( vt+st1 ), n, rwork( bxst ), n, zero,
          ! $                  b( st, 1 ), ldb )
                 j = bxst - n - 1_ilp64
                 jreal = irwb - 1_ilp64
                 ...
                 call stdlib_I64_sgemm( 'T', 'N', nsize, nrhs, nsize, one,rwork( vt+st1 ), n, rwork( &
                           irwb ), nsize, zero,rwork( irwrb ), nsize )
                 j = bxst - n - 1_ilp64
                 jimag = irwb - 1_ilp64
                 do jcol = 1, nrhs
                 ...
                 call stdlib_I64_clalsa( icmpq2, smlsiz, nsize, nrhs, work( bxst ), n,b( st, 1_ilp64 ), ldb,&
                  rwork( u+st1 ), n,rwork( vt+st1 ), iwork( k+st1 ),rwork( difl+st1 ), rwork( &
                  difr+st1 ),rwork( z+st1 ), rwork( poles+st1 ),iwork( givptr+st1 ), iwork( &
                  givcol+st1 ), n,iwork( perm+st1 ), rwork( givnum+st1 ),rwork( c+st1 ), rwork( s+&
                            st1 ),rwork( nrwork ), iwork( iwk ), info )
     ...
     !! without guard digits, but we know of none.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZLALSD', -info )
              return
           end if
           eps = stdlib_I64_dlamch( 'EPSILON' )
           ! set up the tolerance.
           ...
           if( n==0_ilp64 ) then
              ...
              if( d( 1_ilp64 )==zero ) then
                 call stdlib_I64_zlaset( 'A', 1_ilp64, nrhs, czero, czero, b, ldb )
              else
                 rank = 1_ilp64
                 call stdlib_I64_zlascl( 'G', 0_ilp64, 0_ilp64, d( 1_ilp64 ), one, 1_ilp64, nrhs, b, ldb, info )
                 d( 1_ilp64 ) = abs( d( 1_ilp64 ) )
              end if
              return
           end if
           ...
           if( uplo=='L' ) then
              do i = 1, n - 1
                 ...
                 d( i+1 ) = cs*d( i+1 )
                 if( nrhs==1_ilp64 ) then
                    call stdlib_I64_zdrot( 1_ilp64, b( i, 1_ilp64 ), 1_ilp64, b( i+1, 1_ilp64 ), 1_ilp64, cs, sn )
                 else
                    rwork( i*2_ilp64-1 ) = cs
                    rwork( i*2_ilp64 ) = sn
                 end if
              ...
              if( nrhs>1_ilp64 ) then
                 do i = 1, nrhs
                    do j = 1, n - 1
                       cs = rwork( j*2_ilp64-1 )
                       sn = rwork( j*2_ilp64 )
                       call stdlib_I64_zdrot( 1_ilp64, b( j, i ), 1_ilp64, b( j+1, i ), 1_ilp64, cs, sn )
                    end do
                 end do
              end if
           end if
           ...
           orgnrm = stdlib_I64_dlanst( 'M', n, d, e )
           if( orgnrm==zero ) then
              call stdlib_I64_zlaset( 'A', n, nrhs, czero, czero, b, ldb )
              return
           end if
           call stdlib_I64_dlascl( 'G', 0_ilp64, 0_ilp64, orgnrm, one, n, 1_ilp64, d, n, info )
           call stdlib_I64_dlascl( 'G', 0_ilp64, 0_ilp64, orgnrm, one, nm1, 1_ilp64, e, nm1, info )
           ...
           if( n<=smlsiz ) then
              ...
              irwib = irwrb + n*nrhs
              irwb = irwib + n*nrhs
              call stdlib_I64_dlaset( 'A', n, n, zero, one, rwork( irwu ), n )
              call stdlib_I64_dlaset( 'A', n, n, zero, one, rwork( irwvt ), n )
              call stdlib_I64_dlasdq( 'U', 0_ilp64, n, n, n, 0_ilp64, d, e, rwork( irwvt ), n,rwork( irwu ), n, &
                        rwork( irwwrk ), 1_ilp64,rwork( irwwrk ), info )
              if( info/=0_ilp64 ) then
                 return
              end if
              ! in the real version, b is passed to stdlib_I64_dlasdq and multiplied
              ! internally by q**h. here b is complex and that product is
              ! computed below in two steps (real and imaginary parts).
              j = irwb - 1_ilp64
              do jcol = 1, nrhs
              ...
              call stdlib_I64_dgemm( 'T', 'N', n, nrhs, n, one, rwork( irwu ), n,rwork( irwb ), n, &
                        zero, rwork( irwrb ), n )
              j = irwb - 1_ilp64
              do jcol = 1, nrhs
                 do jrow = 1, n
              ...
              do i = 1, n
                 if( d( i )<=tol ) then
                    call stdlib_I64_zlaset( 'A', 1_ilp64, nrhs, czero, czero, b( i, 1_ilp64 ), ldb )
                 else
                    call stdlib_I64_zlascl( 'G', 0_ilp64, 0_ilp64, d( i ), one, 1_ilp64, nrhs, b( i, 1_ilp64 ),ldb, info )
                              
                    rank = rank + 1_ilp64
                 end if
              end do
              ! since b is complex, the following call to stdlib_I64_dgemm is performed
              ! in two steps (real and imaginary parts). that is for v * b
              ! (in the real version of the code v**h is stored in work).
              ! call stdlib_I64_dgemm( 't', 'n', n, nrhs, n, one, work, n, b, ldb, zero,
          ! $               work( nwork ), n )
              j = irwb - 1_ilp64
              do jcol = 1, nrhs
                 do jrow = 1, n
              ...
              call stdlib_I64_dgemm( 'T', 'N', n, nrhs, n, one, rwork( irwvt ), n,rwork( irwb ), n, &
                        zero, rwork( irwrb ), n )
              j = irwb - 1_ilp64
              do jcol = 1, nrhs
                 do jrow = 1, n
              ...
              call stdlib_I64_dlascl( 'G', 0_ilp64, 0_ilp64, one, orgnrm, n, 1_ilp64, d, n, info )
              call stdlib_I64_dlasrt( 'D', n, d, info )
              call stdlib_I64_zlascl( 'G', 0_ilp64, 0_ilp64, orgnrm, one, n, nrhs, b, ldb, info )
              return
           end if
           ! book-keeping and setting up some constants.
           nlvl = int( log( real( n,KIND=dp) / real( smlsiz+1,KIND=dp) ) / log( two ),KIND=ilp64) + &
           ...
           loop_240: do i = 1, nm1
              if( ( abs( e( i ) )<eps ) .or. ( i==nm1 ) ) then
                 ...
                 if( i<nm1 ) then
                    ...
                    iwork( nsub ) = n
                    iwork( sizei+nsub-1 ) = 1_ilp64
                    call stdlib_I64_zcopy( nrhs, b( n, 1_ilp64 ), ldb, work( bx+nm1 ), n )
                 end if
                 st1 = st - 1_ilp64
                 if( nsize==1_ilp64 ) then
                    ! this is a 1-by-1 subproblem and is not solved
                    ! explicitly.
                    call stdlib_I64_zcopy( nrhs, b( st, 1_ilp64 ), ldb, work( bx+st1 ), n )
                 else if( nsize<=smlsiz ) then
                    ! this is a small subproblem and is solved by stdlib_I64_dlasdq.
                    call stdlib_I64_dlaset( 'A', nsize, nsize, zero, one,rwork( vt+st1 ), n )
                    call stdlib_I64_dlaset( 'A', nsize, nsize, zero, one,rwork( u+st1 ), n )
                    call stdlib_I64_dlasdq( 'U', 0_ilp64, nsize, nsize, nsize, 0_ilp64, d( st ),e( st ), rwork( &
                    vt+st1 ), n, rwork( u+st1 ),n, rwork( nrwork ), 1_ilp64, rwork( nrwork ),info )
                    ...
                    call stdlib_I64_dgemm( 'T', 'N', nsize, nrhs, nsize, one,rwork( u+st1 ), n, rwork(&
                               irwb ), nsize,zero, rwork( irwrb ), nsize )
                    j = irwb - 1_ilp64
                    do jcol = 1, nrhs
                       do jrow = st, st + nsize - 1
                    ...
                    call stdlib_I64_zlacpy( 'A', nsize, nrhs, b( st, 1_ilp64 ), ldb,work( bx+st1 ), n )
                              
                 else
                    ! a large problem. solve it using divide and conquer.
                    call stdlib_I64_dlasda( icmpq1, smlsiz, nsize, sqre, d( st ),e( st ), rwork( u+&
                    st1 ), n, rwork( vt+st1 ),iwork( k+st1 ), rwork( difl+st1 ),rwork( difr+st1 ),&
                     rwork( z+st1 ),rwork( poles+st1 ), iwork( givptr+st1 ),iwork( givcol+st1 ), &
                     n, iwork( perm+st1 ),rwork( givnum+st1 ), rwork( c+st1 ),rwork( s+st1 ), &
                               rwork( nrwork ),iwork( iwk ), info )
                    ...
                    bxst = bx + st1
                    call stdlib_I64_zlalsa( icmpq2, smlsiz, nsize, nrhs, b( st, 1_ilp64 ),ldb, work( bxst ),&
                     n, rwork( u+st1 ), n,rwork( vt+st1 ), iwork( k+st1 ),rwork( difl+st1 ), &
                     rwork( difr+st1 ),rwork( z+st1 ), rwork( poles+st1 ),iwork( givptr+st1 ), &
                     iwork( givcol+st1 ), n,iwork( perm+st1 ), rwork( givnum+st1 ),rwork( c+st1 ),&
                                rwork( s+st1 ),rwork( nrwork ), iwork( iwk ), info )
           ...
           do i = 1, n
              ...
              ! subproblems were not solved explicitly.
              if( abs( d( i ) )<=tol ) then
                 call stdlib_I64_zlaset( 'A', 1_ilp64, nrhs, czero, czero, work( bx+i-1 ), n )
              else
                 rank = rank + 1_ilp64
                 call stdlib_I64_zlascl( 'G', 0_ilp64, 0_ilp64, d( i ), one, 1_ilp64, nrhs,work( bx+i-1 ), n, info )
           ...
           loop_320: do i = 1, nsub
              ...
              bxst = bx + st1
              if( nsize==1_ilp64 ) then
                 call stdlib_I64_zcopy( nrhs, work( bxst ), n, b( st, 1_ilp64 ), ldb )
              else if( nsize<=smlsiz ) then
                 ! since b and bx are complex, the following call to stdlib_I64_dgemm
                 ! is performed in two steps (real and imaginary parts).
                 ! call stdlib_I64_dgemm( 't', 'n', nsize, nrhs, nsize, one,
          ! $                  rwork( vt+st1 ), n, rwork( bxst ), n, zero,
          ! $                  b( st, 1 ), ldb )
                 j = bxst - n - 1_ilp64
                 jreal = irwb - 1_ilp64
                 ...
                 call stdlib_I64_dgemm( 'T', 'N', nsize, nrhs, nsize, one,rwork( vt+st1 ), n, rwork( &
                           irwb ), nsize, zero,rwork( irwrb ), nsize )
                 j = bxst - n - 1_ilp64
                 jimag = irwb - 1_ilp64
                 do jcol = 1, nrhs
                 ...
                 call stdlib_I64_zlalsa( icmpq2, smlsiz, nsize, nrhs, work( bxst ), n,b( st, 1_ilp64 ), ldb,&
                  rwork( u+st1 ), n,rwork( vt+st1 ), iwork( k+st1 ),rwork( difl+st1 ), rwork( &
                  difr+st1 ),rwork( z+st1 ), rwork( poles+st1 ),iwork( givptr+st1 ), iwork( &
                  givcol+st1 ), n,iwork( perm+st1 ), rwork( givnum+st1 ),rwork( c+st1 ), rwork( s+&
                            st1 ),rwork( nrwork ), iwork( iwk ), info )
