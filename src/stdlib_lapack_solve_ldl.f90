  ...
  contains
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              if( n==0_ilp64 ) then
                 ...
                 call stdlib_I64_ssytrf( uplo, n, a, lda, ipiv, work, -1_ilp64, info )
                 lwkopt = work(1_ilp64)
              end if
              work( 1_ilp64 ) = lwkopt
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SSYSV ', -info )
              return
           else if( lquery ) then
              return
           end if
           ! compute the factorization a = u*d*u**t or a = l*d*l**t.
           call stdlib_I64_ssytrf( uplo, n, a, lda, ipiv, work, lwork, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              if ( lwork<n ) then
              ! solve with trs ( use level blas 2)
                 call stdlib_I64_ssytrs( uplo, n, nrhs, a, lda, ipiv, b, ldb, info )
              else
              ! solve with trs2 ( use level blas 3)
                 call stdlib_I64_ssytrs2( uplo,n,nrhs,a,lda,ipiv,b,ldb,work,info )
              end if
           end if
           work( 1_ilp64 ) = lwkopt
           return
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              if( n==0_ilp64 ) then
                 ...
                 call stdlib_I64_dsytrf( uplo, n, a, lda, ipiv, work, -1_ilp64, info )
                 lwkopt = work(1_ilp64)
              end if
              work( 1_ilp64 ) = lwkopt
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DSYSV ', -info )
              return
           else if( lquery ) then
              return
           end if
           ! compute the factorization a = u*d*u**t or a = l*d*l**t.
           call stdlib_I64_dsytrf( uplo, n, a, lda, ipiv, work, lwork, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              if ( lwork<n ) then
              ! solve with trs ( use level blas 2)
                 call stdlib_I64_dsytrs( uplo, n, nrhs, a, lda, ipiv, b, ldb, info )
              else
              ! solve with trs2 ( use level blas 3)
                 call stdlib_I64_dsytrs2( uplo,n,nrhs,a,lda,ipiv,b,ldb,work,info )
              end if
           end if
           work( 1_ilp64 ) = lwkopt
           return
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              if( n==0_ilp64 ) then
                 ...
                 call stdlib_I64_csytrf( uplo, n, a, lda, ipiv, work, -1_ilp64, info )
                 lwkopt = real( work(1_ilp64),KIND=sp)
              end if
              work( 1_ilp64 ) = lwkopt
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CSYSV ', -info )
              return
           else if( lquery ) then
              return
           end if
           ! compute the factorization a = u*d*u**t or a = l*d*l**t.
           call stdlib_I64_csytrf( uplo, n, a, lda, ipiv, work, lwork, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              if ( lwork<n ) then
              ! solve with trs ( use level blas 2)
                 call stdlib_I64_csytrs( uplo, n, nrhs, a, lda, ipiv, b, ldb, info )
              else
              ! solve with trs2 ( use level blas 3)
                 call stdlib_I64_csytrs2( uplo,n,nrhs,a,lda,ipiv,b,ldb,work,info )
              end if
           end if
           work( 1_ilp64 ) = lwkopt
           return
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              if( n==0_ilp64 ) then
                 ...
                 call stdlib_I64_zsytrf( uplo, n, a, lda, ipiv, work, -1_ilp64, info )
                 lwkopt = real( work(1_ilp64),KIND=dp)
              end if
              work( 1_ilp64 ) = lwkopt
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZSYSV ', -info )
              return
           else if( lquery ) then
              return
           end if
           ! compute the factorization a = u*d*u**t or a = l*d*l**t.
           call stdlib_I64_zsytrf( uplo, n, a, lda, ipiv, work, lwork, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              if ( lwork<n ) then
              ! solve with trs ( use level blas 2)
                 call stdlib_I64_zsytrs( uplo, n, nrhs, a, lda, ipiv, b, ldb, info )
              else
              ! solve with trs2 ( use level blas 3)
                 call stdlib_I64_zsytrs2( uplo,n,nrhs,a,lda,ipiv,b,ldb,work,info )
              end if
           end if
           work( 1_ilp64 ) = lwkopt
           return
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( .not.nofact .and. .not.stdlib_lsame( fact, 'F' ) ) then
              ...
              info = -13_ilp64
           else if( lwork<max( 1_ilp64, 3_ilp64*n ) .and. .not.lquery ) then
              info = -18_ilp64
           end if
           if( info==0_ilp64 ) then
              lwkopt = max( 1_ilp64, 3_ilp64*n )
              if( nofact ) then
                 nb = stdlib_I64_ilaenv( 1_ilp64, 'SSYTRF', uplo, n, -1_ilp64, -1_ilp64, -1_ilp64 )
                 lwkopt = max( lwkopt, n*nb )
              end if
              work( 1_ilp64 ) = lwkopt
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SSYSVX', -info )
              return
           else if( lquery ) then
              return
           end if
           if( nofact ) then
              ...
              call stdlib_I64_ssytrf( uplo, n, af, ldaf, ipiv, work, lwork, info )
              ! return if info is non-zero.
              if( info>0_ilp64 )then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = stdlib_I64_slansy( 'I', uplo, n, a, lda, work )
           ! compute the reciprocal of the condition number of a.
           call stdlib_I64_ssycon( uplo, n, af, ldaf, ipiv, anorm, rcond, work, iwork,info )
           ! compute the solution vectors x.
           call stdlib_I64_slacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_I64_ssytrs( uplo, n, nrhs, af, ldaf, ipiv, x, ldx, info )
           ! use iterative refinement to improve the computed solutions and
           ! compute error bounds and backward error estimates for them.
           call stdlib_I64_ssyrfs( uplo, n, nrhs, a, lda, af, ldaf, ipiv, b, ldb, x,ldx, ferr, berr, &
                     work, iwork, info )
           ! set info = n+1 if the matrix is singular to working precision.
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              lwkopt = max( 1_ilp64, 3_ilp64*n )
              if( nofact ) then
                 nb = stdlib_I64_ilaenv( 1_ilp64, 'DSYTRF', uplo, n, -1_ilp64, -1_ilp64, -1_ilp64 )
                 lwkopt = max( lwkopt, n*nb )
              end if
              work( 1_ilp64 ) = lwkopt
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DSYSVX', -info )
              return
           else if( lquery ) then
              return
           end if
           if( nofact ) then
              ...
              call stdlib_I64_dsytrf( uplo, n, af, ldaf, ipiv, work, lwork, info )
              ! return if info is non-zero.
              if( info>0_ilp64 )then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = stdlib_I64_dlansy( 'I', uplo, n, a, lda, work )
           ! compute the reciprocal of the condition number of a.
           call stdlib_I64_dsycon( uplo, n, af, ldaf, ipiv, anorm, rcond, work, iwork,info )
           ! compute the solution vectors x.
           call stdlib_I64_dlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_I64_dsytrs( uplo, n, nrhs, af, ldaf, ipiv, x, ldx, info )
           ! use iterative refinement to improve the computed solutions and
           ! compute error bounds and backward error estimates for them.
           call stdlib_I64_dsyrfs( uplo, n, nrhs, a, lda, af, ldaf, ipiv, b, ldb, x,ldx, ferr, berr, &
                     work, iwork, info )
           ! set info = n+1 if the matrix is singular to working precision.
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              lwkopt = max( 1_ilp64, 2_ilp64*n )
              if( nofact ) then
                 nb = stdlib_I64_ilaenv( 1_ilp64, 'CSYTRF', uplo, n, -1_ilp64, -1_ilp64, -1_ilp64 )
                 lwkopt = max( lwkopt, n*nb )
              end if
              work( 1_ilp64 ) = lwkopt
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CSYSVX', -info )
              return
           else if( lquery ) then
              return
           end if
           if( nofact ) then
              ...
              call stdlib_I64_csytrf( uplo, n, af, ldaf, ipiv, work, lwork, info )
              ! return if info is non-zero.
              if( info>0_ilp64 )then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = stdlib_I64_clansy( 'I', uplo, n, a, lda, rwork )
           ! compute the reciprocal of the condition number of a.
           call stdlib_I64_csycon( uplo, n, af, ldaf, ipiv, anorm, rcond, work, info )
           ! compute the solution vectors x.
           call stdlib_I64_clacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_I64_csytrs( uplo, n, nrhs, af, ldaf, ipiv, x, ldx, info )
           ! use iterative refinement to improve the computed solutions and
           ! compute error bounds and backward error estimates for them.
           call stdlib_I64_csyrfs( uplo, n, nrhs, a, lda, af, ldaf, ipiv, b, ldb, x,ldx, ferr, berr, &
                     work, rwork, info )
           ! set info = n+1 if the matrix is singular to working precision.
           if( rcond<stdlib_I64_slamch( 'EPSILON' ) )info = n + 1_ilp64
           work( 1_ilp64 ) = lwkopt
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              lwkopt = max( 1_ilp64, 2_ilp64*n )
              if( nofact ) then
                 nb = stdlib_I64_ilaenv( 1_ilp64, 'ZSYTRF', uplo, n, -1_ilp64, -1_ilp64, -1_ilp64 )
                 lwkopt = max( lwkopt, n*nb )
              end if
              work( 1_ilp64 ) = lwkopt
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZSYSVX', -info )
              return
           else if( lquery ) then
              return
           end if
           if( nofact ) then
              ...
              call stdlib_I64_zsytrf( uplo, n, af, ldaf, ipiv, work, lwork, info )
              ! return if info is non-zero.
              if( info>0_ilp64 )then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = stdlib_I64_zlansy( 'I', uplo, n, a, lda, rwork )
           ! compute the reciprocal of the condition number of a.
           call stdlib_I64_zsycon( uplo, n, af, ldaf, ipiv, anorm, rcond, work, info )
           ! compute the solution vectors x.
           call stdlib_I64_zlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_I64_zsytrs( uplo, n, nrhs, af, ldaf, ipiv, x, ldx, info )
           ! use iterative refinement to improve the computed solutions and
           ! compute error bounds and backward error estimates for them.
           call stdlib_I64_zsyrfs( uplo, n, nrhs, a, lda, af, ldaf, ipiv, b, ldb, x,ldx, ferr, berr, &
                     work, rwork, info )
           ! set info = n+1 if the matrix is singular to working precision.
           if( rcond<stdlib_I64_dlamch( 'EPSILON' ) )info = n + 1_ilp64
           work( 1_ilp64 ) = lwkopt
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              if( n==0_ilp64 ) then
                 ...
                 call stdlib_I64_ssytrf_rk( uplo, n, a, lda, e, ipiv, work, -1_ilp64, info )
                 lwkopt = work(1_ilp64)
              end if
              work( 1_ilp64 ) = lwkopt
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SSYSV_RK ', -info )
              return
           else if( lquery ) then
              return
           end if
           ! compute the factorization a = p*u*d*(u**t)*(p**t) or
           ! a = p*u*d*(u**t)*(p**t).
           call stdlib_I64_ssytrf_rk( uplo, n, a, lda, e, ipiv, work, lwork, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b with blas3 solver, overwriting b with x.
              call stdlib_I64_ssytrs_3( uplo, n, nrhs, a, lda, e, ipiv, b, ldb, info )
           end if
           work( 1_ilp64 ) = lwkopt
           return
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              if( n==0_ilp64 ) then
                 ...
                 call stdlib_I64_dsytrf_rk( uplo, n, a, lda, e, ipiv, work, -1_ilp64, info )
                 lwkopt = work(1_ilp64)
              end if
              work( 1_ilp64 ) = lwkopt
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DSYSV_RK ', -info )
              return
           else if( lquery ) then
              return
           end if
           ! compute the factorization a = p*u*d*(u**t)*(p**t) or
           ! a = p*u*d*(u**t)*(p**t).
           call stdlib_I64_dsytrf_rk( uplo, n, a, lda, e, ipiv, work, lwork, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b with blas3 solver, overwriting b with x.
              call stdlib_I64_dsytrs_3( uplo, n, nrhs, a, lda, e, ipiv, b, ldb, info )
           end if
           work( 1_ilp64 ) = lwkopt
           return
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              if( n==0_ilp64 ) then
                 ...
                 call stdlib_I64_csytrf_rk( uplo, n, a, lda, e, ipiv, work, -1_ilp64, info )
                 lwkopt = real( work(1_ilp64),KIND=sp)
              end if
              work( 1_ilp64 ) = lwkopt
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CSYSV_RK ', -info )
              return
           else if( lquery ) then
              return
           end if
           ! compute the factorization a = u*d*u**t or a = l*d*l**t.
           call stdlib_I64_csytrf_rk( uplo, n, a, lda, e, ipiv, work, lwork, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b with blas3 solver, overwriting b with x.
              call stdlib_I64_csytrs_3( uplo, n, nrhs, a, lda, e, ipiv, b, ldb, info )
           end if
           work( 1_ilp64 ) = lwkopt
           return
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              if( n==0_ilp64 ) then
                 ...
                 call stdlib_I64_zsytrf_rk( uplo, n, a, lda, e, ipiv, work, -1_ilp64, info )
                 lwkopt = real( work(1_ilp64),KIND=dp)
              end if
              work( 1_ilp64 ) = lwkopt
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZSYSV_RK ', -info )
              return
           else if( lquery ) then
              return
           end if
           ! compute the factorization a = p*u*d*(u**t)*(p**t) or
           ! a = p*u*d*(u**t)*(p**t).
           call stdlib_I64_zsytrf_rk( uplo, n, a, lda, e, ipiv, work, lwork, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b with blas3 solver, overwriting b with x.
              call stdlib_I64_zsytrs_3( uplo, n, nrhs, a, lda, e, ipiv, b, ldb, info )
           end if
           work( 1_ilp64 ) = lwkopt
           return
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              if( n==0_ilp64 ) then
                 ...
                 call stdlib_I64_ssytrf_rook( uplo, n, a, lda, ipiv, work, -1_ilp64, info )
                 lwkopt = work(1_ilp64)
              end if
              work( 1_ilp64 ) = lwkopt
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SSYSV_ROOK ', -info )
              return
           else if( lquery ) then
              return
           end if
           ! compute the factorization a = u*d*u**t or a = l*d*l**t.
           call stdlib_I64_ssytrf_rook( uplo, n, a, lda, ipiv, work, lwork, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              ! solve with trs_rook ( use level 2 blas)
              call stdlib_I64_ssytrs_rook( uplo, n, nrhs, a, lda, ipiv, b, ldb, info )
           end if
           work( 1_ilp64 ) = lwkopt
           return
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              if( n==0_ilp64 ) then
                 ...
                 call stdlib_I64_dsytrf_rook( uplo, n, a, lda, ipiv, work, -1_ilp64, info )
                 lwkopt = work(1_ilp64)
              end if
              work( 1_ilp64 ) = lwkopt
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DSYSV_ROOK ', -info )
              return
           else if( lquery ) then
              return
           end if
           ! compute the factorization a = u*d*u**t or a = l*d*l**t.
           call stdlib_I64_dsytrf_rook( uplo, n, a, lda, ipiv, work, lwork, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              ! solve with trs_rook ( use level 2 blas)
              call stdlib_I64_dsytrs_rook( uplo, n, nrhs, a, lda, ipiv, b, ldb, info )
           end if
           work( 1_ilp64 ) = lwkopt
           return
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              if( n==0_ilp64 ) then
                 ...
                 call stdlib_I64_csytrf_rook( uplo, n, a, lda, ipiv, work, -1_ilp64, info )
                 lwkopt = real( work(1_ilp64),KIND=sp)
              end if
              work( 1_ilp64 ) = lwkopt
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CSYSV_ROOK ', -info )
              return
           else if( lquery ) then
              return
           end if
           ! compute the factorization a = u*d*u**t or a = l*d*l**t.
           call stdlib_I64_csytrf_rook( uplo, n, a, lda, ipiv, work, lwork, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              ! solve with trs_rook ( use level 2 blas)
              call stdlib_I64_csytrs_rook( uplo, n, nrhs, a, lda, ipiv, b, ldb, info )
           end if
           work( 1_ilp64 ) = lwkopt
           return
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              if( n==0_ilp64 ) then
                 ...
                 call stdlib_I64_zsytrf_rook( uplo, n, a, lda, ipiv, work, -1_ilp64, info )
                 lwkopt = real( work(1_ilp64),KIND=dp)
              end if
              work( 1_ilp64 ) = lwkopt
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZSYSV_ROOK ', -info )
              return
           else if( lquery ) then
              return
           end if
           ! compute the factorization a = u*d*u**t or a = l*d*l**t.
           call stdlib_I64_zsytrf_rook( uplo, n, a, lda, ipiv, work, lwork, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              ! solve with trs_rook ( use level 2 blas)
              call stdlib_I64_zsytrs_rook( uplo, n, nrhs, a, lda, ipiv, b, ldb, info )
           end if
           work( 1_ilp64 ) = lwkopt
           return
     ...
     !! used to solve the system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CHESV ', -info )
              return
           else if( lquery ) then
              return
           end if
           ! compute the factorization a = u*d*u**h or a = l*d*l**h.
           call stdlib_I64_chetrf( uplo, n, a, lda, ipiv, work, lwork, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              if ( lwork<n ) then
              ! solve with trs ( use level blas 2)
                 call stdlib_I64_chetrs( uplo, n, nrhs, a, lda, ipiv, b, ldb, info )
              else
              ! solve with trs2 ( use level blas 3)
                 call stdlib_I64_chetrs2( uplo,n,nrhs,a,lda,ipiv,b,ldb,work,info )
              end if
           end if
           work( 1_ilp64 ) = lwkopt
           return
     ...
     !! used to solve the system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZHESV ', -info )
              return
           else if( lquery ) then
              return
           end if
           ! compute the factorization a = u*d*u**h or a = l*d*l**h.
           call stdlib_I64_zhetrf( uplo, n, a, lda, ipiv, work, lwork, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              if ( lwork<n ) then
              ! solve with trs ( use level blas 2)
                 call stdlib_I64_zhetrs( uplo, n, nrhs, a, lda, ipiv, b, ldb, info )
              else
              ! solve with trs2 ( use level blas 3)
                 call stdlib_I64_zhetrs2( uplo,n,nrhs,a,lda,ipiv,b,ldb,work,info )
              end if
           end if
           work( 1_ilp64 ) = lwkopt
           return
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CHESVX', -info )
              return
           else if( lquery ) then
              return
           end if
           if( nofact ) then
              ! compute the factorization a = u*d*u**h or a = l*d*l**h.
              call stdlib_I64_clacpy( uplo, n, n, a, lda, af, ldaf )
              call stdlib_I64_chetrf( uplo, n, af, ldaf, ipiv, work, lwork, info )
              ! return if info is non-zero.
              if( info>0_ilp64 )then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = stdlib_I64_clanhe( 'I', uplo, n, a, lda, rwork )
           ! compute the reciprocal of the condition number of a.
           call stdlib_I64_checon( uplo, n, af, ldaf, ipiv, anorm, rcond, work, info )
           ! compute the solution vectors x.
           call stdlib_I64_clacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_I64_chetrs( uplo, n, nrhs, af, ldaf, ipiv, x, ldx, info )
           ! use iterative refinement to improve the computed solutions and
           ! compute error bounds and backward error estimates for them.
           call stdlib_I64_cherfs( uplo, n, nrhs, a, lda, af, ldaf, ipiv, b, ldb, x,ldx, ferr, berr, &
                     work, rwork, info )
           ! set info = n+1 if the matrix is singular to working precision.
           if( rcond<stdlib_I64_slamch( 'EPSILON' ) )info = n + 1_ilp64
           work( 1_ilp64 ) = lwkopt
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZHESVX', -info )
              return
           else if( lquery ) then
              return
           end if
           if( nofact ) then
              ! compute the factorization a = u*d*u**h or a = l*d*l**h.
              call stdlib_I64_zlacpy( uplo, n, n, a, lda, af, ldaf )
              call stdlib_I64_zhetrf( uplo, n, af, ldaf, ipiv, work, lwork, info )
              ! return if info is non-zero.
              if( info>0_ilp64 )then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = stdlib_I64_zlanhe( 'I', uplo, n, a, lda, rwork )
           ! compute the reciprocal of the condition number of a.
           call stdlib_I64_zhecon( uplo, n, af, ldaf, ipiv, anorm, rcond, work, info )
           ! compute the solution vectors x.
           call stdlib_I64_zlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_I64_zhetrs( uplo, n, nrhs, af, ldaf, ipiv, x, ldx, info )
           ! use iterative refinement to improve the computed solutions and
           ! compute error bounds and backward error estimates for them.
           call stdlib_I64_zherfs( uplo, n, nrhs, a, lda, af, ldaf, ipiv, b, ldb, x,ldx, ferr, berr, &
                     work, rwork, info )
           ! set info = n+1 if the matrix is singular to working precision.
           if( rcond<stdlib_I64_dlamch( 'EPSILON' ) )info = n + 1_ilp64
           work( 1_ilp64 ) = lwkopt
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              if( n==0_ilp64 ) then
                 ...
                 call stdlib_I64_chetrf_rk( uplo, n, a, lda, e, ipiv, work, -1_ilp64, info )
                 lwkopt = real( work(1_ilp64),KIND=sp)
              end if
              work( 1_ilp64 ) = lwkopt
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CHESV_RK ', -info )
              return
           else if( lquery ) then
              return
           end if
           ! compute the factorization a = u*d*u**t or a = l*d*l**t.
           call stdlib_I64_chetrf_rk( uplo, n, a, lda, e, ipiv, work, lwork, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b with blas3 solver, overwriting b with x.
              call stdlib_I64_chetrs_3( uplo, n, nrhs, a, lda, e, ipiv, b, ldb, info )
           end if
           work( 1_ilp64 ) = lwkopt
           return
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              if( n==0_ilp64 ) then
                 ...
                 call stdlib_I64_zhetrf_rk( uplo, n, a, lda, e, ipiv, work, -1_ilp64, info )
                 lwkopt = real( work(1_ilp64),KIND=dp)
              end if
              work( 1_ilp64 ) = lwkopt
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZHESV_RK ', -info )
              return
           else if( lquery ) then
              return
           end if
           ! compute the factorization a = p*u*d*(u**h)*(p**t) or
           ! a = p*u*d*(u**h)*(p**t).
           call stdlib_I64_zhetrf_rk( uplo, n, a, lda, e, ipiv, work, lwork, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b with blas3 solver, overwriting b with x.
              call stdlib_I64_zhetrs_3( uplo, n, nrhs, a, lda, e, ipiv, b, ldb, info )
           end if
           work( 1_ilp64 ) = lwkopt
           return
     ...
     !! of equations A * X = B by calling CHETRS_ROOK (uses BLAS 2).
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CHESV_ROOK ', -info )
              return
           else if( lquery ) then
              return
           end if
           ! compute the factorization a = u*d*u**h or a = l*d*l**h.
           call stdlib_I64_chetrf_rook( uplo, n, a, lda, ipiv, work, lwork, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              ! solve with trs ( use level blas 2)
              call stdlib_I64_chetrs_rook( uplo, n, nrhs, a, lda, ipiv, b, ldb, info )
           end if
           work( 1_ilp64 ) = lwkopt
           return
     ...
     !! of equations A * X = B by calling ZHETRS_ROOK (uses BLAS 2).
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZHESV_ROOK ', -info )
              return
           else if( lquery ) then
              return
           end if
           ! compute the factorization a = u*d*u**h or a = l*d*l**h.
           call stdlib_I64_zhetrf_rook( uplo, n, a, lda, ipiv, work, lwork, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              ! solve with trs ( use level blas 2)
              call stdlib_I64_zhetrs_rook( uplo, n, nrhs, a, lda, ipiv, b, ldb, info )
           end if
           work( 1_ilp64 ) = lwkopt
           return
     ...
     !! solve the system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SSPSV ', -info )
              return
           end if
           ! compute the factorization a = u*d*u**t or a = l*d*l**t.
           call stdlib_I64_ssptrf( uplo, n, ap, ipiv, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_I64_ssptrs( uplo, n, nrhs, ap, ipiv, b, ldb, info )
           end if
           return
     ...
     !! solve the system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DSPSV ', -info )
              return
           end if
           ! compute the factorization a = u*d*u**t or a = l*d*l**t.
           call stdlib_I64_dsptrf( uplo, n, ap, ipiv, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_I64_dsptrs( uplo, n, nrhs, ap, ipiv, b, ldb, info )
           end if
           return
     ...
     !! solve the system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CSPSV ', -info )
              return
           end if
           ! compute the factorization a = u*d*u**t or a = l*d*l**t.
           call stdlib_I64_csptrf( uplo, n, ap, ipiv, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_I64_csptrs( uplo, n, nrhs, ap, ipiv, b, ldb, info )
           end if
           return
     ...
     !! solve the system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZSPSV ', -info )
              return
           end if
           ! compute the factorization a = u*d*u**t or a = l*d*l**t.
           call stdlib_I64_zsptrf( uplo, n, ap, ipiv, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_I64_zsptrs( uplo, n, nrhs, ap, ipiv, b, ldb, info )
           end if
           return
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SSPSVX', -info )
              return
           end if
           if( nofact ) then
              ! compute the factorization a = u*d*u**t or a = l*d*l**t.
              call stdlib_I64_scopy( n*( n+1 ) / 2_ilp64, ap, 1_ilp64, afp, 1_ilp64 )
              call stdlib_I64_ssptrf( uplo, n, afp, ipiv, info )
              ! return if info is non-zero.
              if( info>0_ilp64 )then
                 rcond = zero
                 return
           ...
           ! compute the reciprocal of the condition number of a.
           call stdlib_I64_sspcon( uplo, n, afp, ipiv, anorm, rcond, work, iwork, info )
           ! compute the solution vectors x.
           call stdlib_I64_slacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_I64_ssptrs( uplo, n, nrhs, afp, ipiv, x, ldx, info )
           ! use iterative refinement to improve the computed solutions and
           ! compute error bounds and backward error estimates for them.
           call stdlib_I64_ssprfs( uplo, n, nrhs, ap, afp, ipiv, b, ldb, x, ldx, ferr,berr, work, &
                     iwork, info )
           ! set info = n+1 if the matrix is singular to working precision.
           if( rcond<stdlib_I64_slamch( 'EPSILON' ) )info = n + 1_ilp64
           return
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DSPSVX', -info )
              return
           end if
           if( nofact ) then
              ! compute the factorization a = u*d*u**t or a = l*d*l**t.
              call stdlib_I64_dcopy( n*( n+1 ) / 2_ilp64, ap, 1_ilp64, afp, 1_ilp64 )
              call stdlib_I64_dsptrf( uplo, n, afp, ipiv, info )
              ! return if info is non-zero.
              if( info>0_ilp64 )then
                 rcond = zero
                 return
           ...
           ! compute the reciprocal of the condition number of a.
           call stdlib_I64_dspcon( uplo, n, afp, ipiv, anorm, rcond, work, iwork, info )
           ! compute the solution vectors x.
           call stdlib_I64_dlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_I64_dsptrs( uplo, n, nrhs, afp, ipiv, x, ldx, info )
           ! use iterative refinement to improve the computed solutions and
           ! compute error bounds and backward error estimates for them.
           call stdlib_I64_dsprfs( uplo, n, nrhs, ap, afp, ipiv, b, ldb, x, ldx, ferr,berr, work, &
                     iwork, info )
           ! set info = n+1 if the matrix is singular to working precision.
           if( rcond<stdlib_I64_dlamch( 'EPSILON' ) )info = n + 1_ilp64
           return
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CSPSVX', -info )
              return
           end if
           if( nofact ) then
              ! compute the factorization a = u*d*u**t or a = l*d*l**t.
              call stdlib_I64_ccopy( n*( n+1 ) / 2_ilp64, ap, 1_ilp64, afp, 1_ilp64 )
              call stdlib_I64_csptrf( uplo, n, afp, ipiv, info )
              ! return if info is non-zero.
              if( info>0_ilp64 )then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = stdlib_I64_clansp( 'I', uplo, n, ap, rwork )
           ! compute the reciprocal of the condition number of a.
           call stdlib_I64_cspcon( uplo, n, afp, ipiv, anorm, rcond, work, info )
           ! compute the solution vectors x.
           call stdlib_I64_clacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_I64_csptrs( uplo, n, nrhs, afp, ipiv, x, ldx, info )
           ! use iterative refinement to improve the computed solutions and
           ! compute error bounds and backward error estimates for them.
           call stdlib_I64_csprfs( uplo, n, nrhs, ap, afp, ipiv, b, ldb, x, ldx, ferr,berr, work, &
                     rwork, info )
           ! set info = n+1 if the matrix is singular to working precision.
           if( rcond<stdlib_I64_slamch( 'EPSILON' ) )info = n + 1_ilp64
           return
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZSPSVX', -info )
              return
           end if
           if( nofact ) then
              ! compute the factorization a = u*d*u**t or a = l*d*l**t.
              call stdlib_I64_zcopy( n*( n+1 ) / 2_ilp64, ap, 1_ilp64, afp, 1_ilp64 )
              call stdlib_I64_zsptrf( uplo, n, afp, ipiv, info )
              ! return if info is non-zero.
              if( info>0_ilp64 )then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = stdlib_I64_zlansp( 'I', uplo, n, ap, rwork )
           ! compute the reciprocal of the condition number of a.
           call stdlib_I64_zspcon( uplo, n, afp, ipiv, anorm, rcond, work, info )
           ! compute the solution vectors x.
           call stdlib_I64_zlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_I64_zsptrs( uplo, n, nrhs, afp, ipiv, x, ldx, info )
           ! use iterative refinement to improve the computed solutions and
           ! compute error bounds and backward error estimates for them.
           call stdlib_I64_zsprfs( uplo, n, nrhs, ap, afp, ipiv, b, ldb, x, ldx, ferr,berr, work, &
                     rwork, info )
           ! set info = n+1 if the matrix is singular to working precision.
           if( rcond<stdlib_I64_dlamch( 'EPSILON' ) )info = n + 1_ilp64
           return
     ...
     !! solve the system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CHPSV ', -info )
              return
           end if
           ! compute the factorization a = u*d*u**h or a = l*d*l**h.
           call stdlib_I64_chptrf( uplo, n, ap, ipiv, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_I64_chptrs( uplo, n, nrhs, ap, ipiv, b, ldb, info )
           end if
           return
     ...
     !! solve the system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZHPSV ', -info )
              return
           end if
           ! compute the factorization a = u*d*u**h or a = l*d*l**h.
           call stdlib_I64_zhptrf( uplo, n, ap, ipiv, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_I64_zhptrs( uplo, n, nrhs, ap, ipiv, b, ldb, info )
           end if
           return
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CHPSVX', -info )
              return
           end if
           if( nofact ) then
              ! compute the factorization a = u*d*u**h or a = l*d*l**h.
              call stdlib_I64_ccopy( n*( n+1 ) / 2_ilp64, ap, 1_ilp64, afp, 1_ilp64 )
              call stdlib_I64_chptrf( uplo, n, afp, ipiv, info )
              ! return if info is non-zero.
              if( info>0_ilp64 )then
                 rcond = zero
                 return
           ...
           ! compute the reciprocal of the condition number of a.
           call stdlib_I64_chpcon( uplo, n, afp, ipiv, anorm, rcond, work, info )
           ! compute the solution vectors x.
           call stdlib_I64_clacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_I64_chptrs( uplo, n, nrhs, afp, ipiv, x, ldx, info )
           ! use iterative refinement to improve the computed solutions and
           ! compute error bounds and backward error estimates for them.
           call stdlib_I64_chprfs( uplo, n, nrhs, ap, afp, ipiv, b, ldb, x, ldx, ferr,berr, work, &
                     rwork, info )
           ! set info = n+1 if the matrix is singular to working precision.
           if( rcond<stdlib_I64_slamch( 'EPSILON' ) )info = n + 1_ilp64
           return
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZHPSVX', -info )
              return
           end if
           if( nofact ) then
              ! compute the factorization a = u*d*u**h or a = l*d*l**h.
              call stdlib_I64_zcopy( n*( n+1 ) / 2_ilp64, ap, 1_ilp64, afp, 1_ilp64 )
              call stdlib_I64_zhptrf( uplo, n, afp, ipiv, info )
              ! return if info is non-zero.
              if( info>0_ilp64 )then
                 rcond = zero
                 return
           ...
           ! compute the reciprocal of the condition number of a.
           call stdlib_I64_zhpcon( uplo, n, afp, ipiv, anorm, rcond, work, info )
           ! compute the solution vectors x.
           call stdlib_I64_zlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_I64_zhptrs( uplo, n, nrhs, afp, ipiv, x, ldx, info )
           ! use iterative refinement to improve the computed solutions and
           ! compute error bounds and backward error estimates for them.
           call stdlib_I64_zhprfs( uplo, n, nrhs, ap, afp, ipiv, b, ldb, x, ldx, ferr,berr, work, &
                     rwork, info )
           ! set info = n+1 if the matrix is singular to working precision.
           if( rcond<stdlib_I64_dlamch( 'EPSILON' ) )info = n + 1_ilp64
           return
     ...
     !! form of A is then used to solve the system of equations A * X = B.
        ...
        ! =====================================================================
           ! Local Scalars 
           logical(lk) :: lquery
           integer(ilp64) :: lwkopt, lwkopt_sytrf, lwkopt_sytrs
           ! Intrinsic Functions 
           ! Executable Statements 
           ! test the input parameters.
           info = 0_ilp64
           ...
           if( .not.stdlib_lsame( uplo, 'U' ) .and. .not.stdlib_lsame( uplo, 'L' ) ) then
              ...
              info = -8_ilp64
           else if( lwork<max(2_ilp64*n, 3_ilp64*n-2) .and. .not.lquery ) then
              info = -10_ilp64
           end if
           if( info==0_ilp64 ) then
              call stdlib_I64_ssytrf_aa( uplo, n, a, lda, ipiv, work, -1_ilp64, info )
              lwkopt_sytrf = int( work(1_ilp64),KIND=ilp64)
              call stdlib_I64_ssytrs_aa( uplo, n, nrhs, a, lda, ipiv, b, ldb, work,-1_ilp64, info )
              lwkopt_sytrs = int( work(1_ilp64),KIND=ilp64)
              lwkopt = max( lwkopt_sytrf, lwkopt_sytrs )
              work( 1_ilp64 ) = lwkopt
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SSYSV_AA', -info )
              return
           else if( lquery ) then
              return
           end if
           ! compute the factorization a = u**t*t*u or a = l*t*l**t.
           call stdlib_I64_ssytrf_aa( uplo, n, a, lda, ipiv, work, lwork, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_I64_ssytrs_aa( uplo, n, nrhs, a, lda, ipiv, b, ldb, work,lwork, info )
     ...
     !! form of A is then used to solve the system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              call stdlib_I64_dsytrf_aa( uplo, n, a, lda, ipiv, work, -1_ilp64, info )
              lwkopt_sytrf = int( work(1_ilp64),KIND=ilp64)
              call stdlib_I64_dsytrs_aa( uplo, n, nrhs, a, lda, ipiv, b, ldb, work,-1_ilp64, info )
              lwkopt_sytrs = int( work(1_ilp64),KIND=ilp64)
              lwkopt = max( lwkopt_sytrf, lwkopt_sytrs )
              work( 1_ilp64 ) = lwkopt
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DSYSV_AA ', -info )
              return
           else if( lquery ) then
              return
           end if
           ! compute the factorization a = u**t*t*u or a = l*t*l**t.
           call stdlib_I64_dsytrf_aa( uplo, n, a, lda, ipiv, work, lwork, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_I64_dsytrs_aa( uplo, n, nrhs, a, lda, ipiv, b, ldb, work,lwork, info )
     ...
     !! form of A is then used to solve the system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              call stdlib_I64_csytrf_aa( uplo, n, a, lda, ipiv, work, -1_ilp64, info )
              lwkopt_sytrf = int( work(1_ilp64),KIND=ilp64)
              call stdlib_I64_csytrs_aa( uplo, n, nrhs, a, lda, ipiv, b, ldb, work,-1_ilp64, info )
              lwkopt_sytrs = int( work(1_ilp64),KIND=ilp64)
              lwkopt = max( lwkopt_sytrf, lwkopt_sytrs )
              work( 1_ilp64 ) = lwkopt
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CSYSV_AA ', -info )
              return
           else if( lquery ) then
              return
           end if
           ! compute the factorization a = u**t*t*u or a = l*t*l**t.
           call stdlib_I64_csytrf_aa( uplo, n, a, lda, ipiv, work, lwork, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_I64_csytrs_aa( uplo, n, nrhs, a, lda, ipiv, b, ldb, work,lwork, info )
     ...
     !! form of A is then used to solve the system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              call stdlib_I64_zsytrf_aa( uplo, n, a, lda, ipiv, work, -1_ilp64, info )
              lwkopt_sytrf = int( work(1_ilp64),KIND=ilp64)
              call stdlib_I64_zsytrs_aa( uplo, n, nrhs, a, lda, ipiv, b, ldb, work,-1_ilp64, info )
              lwkopt_sytrs = int( work(1_ilp64),KIND=ilp64)
              lwkopt = max( lwkopt_sytrf, lwkopt_sytrs )
              work( 1_ilp64 ) = lwkopt
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZSYSV_AA ', -info )
              return
           else if( lquery ) then
              return
           end if
           ! compute the factorization a = u**t*t*u or a = l*t*l**t.
           call stdlib_I64_zsytrf_aa( uplo, n, a, lda, ipiv, work, lwork, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_I64_zsytrs_aa( uplo, n, nrhs, a, lda, ipiv, b, ldb, work,lwork, info )
     ...
     !! of A is then used to solve the system of equations A * X = B.
        ...
        ! =====================================================================
           ! Local Scalars 
           logical(lk) :: lquery
           integer(ilp64) :: lwkopt, lwkopt_hetrf, lwkopt_hetrs
           ! Intrinsic Functions 
           ! Executable Statements 
           ! test the input parameters.
           info = 0_ilp64
           ...
           if( info==0_ilp64 ) then
              call stdlib_I64_chetrf_aa( uplo, n, a, lda, ipiv, work, -1_ilp64, info )
              lwkopt_hetrf = int( work(1_ilp64),KIND=ilp64)
              call stdlib_I64_chetrs_aa( uplo, n, nrhs, a, lda, ipiv, b, ldb, work,-1_ilp64, info )
              lwkopt_hetrs = int( work(1_ilp64),KIND=ilp64)
              lwkopt = max( lwkopt_hetrf, lwkopt_hetrs )
              work( 1_ilp64 ) = lwkopt
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CHESV_AA ', -info )
              return
           else if( lquery ) then
              return
           end if
           ! compute the factorization a = u**h*t*u or a = l*t*l**h.
           call stdlib_I64_chetrf_aa( uplo, n, a, lda, ipiv, work, lwork, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_I64_chetrs_aa( uplo, n, nrhs, a, lda, ipiv, b, ldb, work,lwork, info )
     ...
     !! of A is then used to solve the system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              call stdlib_I64_zhetrf_aa( uplo, n, a, lda, ipiv, work, -1_ilp64, info )
              lwkopt_hetrf = int( work(1_ilp64),KIND=ilp64)
              call stdlib_I64_zhetrs_aa( uplo, n, nrhs, a, lda, ipiv, b, ldb, work,-1_ilp64, info )
              lwkopt_hetrs = int( work(1_ilp64),KIND=ilp64)
              lwkopt = max( lwkopt_hetrf, lwkopt_hetrs )
              work( 1_ilp64 ) = lwkopt
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZHESV_AA ', -info )
              return
           else if( lquery ) then
              return
           end if
           ! compute the factorization a = u**h*t*u or a = l*t*l**h.
           call stdlib_I64_zhetrf_aa( uplo, n, a, lda, ipiv, work, lwork, info )
           if( info==0_ilp64 ) then
              ! solve the system a*x = b, overwriting b with x.
              call stdlib_I64_zhetrs_aa( uplo, n, nrhs, a, lda, ipiv, b, ldb, work,lwork, info )
