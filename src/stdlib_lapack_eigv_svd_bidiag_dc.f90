submodule(stdlib_lapack_eig_svd_lsq) stdlib_lapack_eigv_svd_bidiag_dc
  ...
  contains
     ...
     !! and optionally, the singular vectors in compact form.
        ...
        ! =====================================================================
           ! Local Scalars 
           integer(ilp64) :: i, i1, ic, idxq, idxqc, im1, inode, itemp, iwk, j, lf, ll, lvl, m, ncc,&
                      nd, ndb1, ndiml, ndimr, nl, nlf, nlp1, nlvl, nr, nrf, nrp1, sqrei
           real(sp) :: alpha, beta
           ! Executable Statements 
           ! test the input parameters.
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SLASD0', -info )
              return
           end if
           ! if the input matrix is too small, call stdlib_I64_slasdq to find the svd.
           if( n<=smlsiz ) then
              call stdlib_I64_slasdq( 'U', sqre, n, m, n, 0_ilp64, d, e, vt, ldvt, u, ldu, u,ldu, work, &
                        info )
              return
           end if
           ! set up the computation tree.
           ...
           ! for the nodes on bottom level of the tree, solve
           ! their subproblems by stdlib_I64_slasdq.
           ndb1 = ( nd+1 ) / 2_ilp64
           ncc = 0_ilp64
           loop_30: do i = ndb1, nd
           ! ic : center row of each node
           ...
           ! nrf: starting row of the right  subproblem
              ...
              nrf = ic + 1_ilp64
              sqrei = 1_ilp64
              call stdlib_I64_slasdq( 'U', sqrei, nl, nlp1, nl, ncc, d( nlf ), e( nlf ),vt( nlf, nlf )&
                        , ldvt, u( nlf, nlf ), ldu,u( nlf, nlf ), ldu, work, info )
              if( info/=0_ilp64 ) then
                 return
              end if
              itemp = idxq + nlf - 2_ilp64
              do j = 1, nl
                 iwork( itemp+j ) = j
              end do
              if( i==nd ) then
              ...
              nrp1 = nr + sqrei
              call stdlib_I64_slasdq( 'U', sqrei, nr, nrp1, nr, ncc, d( nrf ), e( nrf ),vt( nrf, nrf )&
                        , ldvt, u( nrf, nrf ), ldu,u( nrf, nrf ), ldu, work, info )
              if( info/=0_ilp64 ) then
                 return
              end if
           ...
           loop_50: do lvl = nlvl, 1, -1
              ...
              do i = lf, ll
                 ...
                 nr = iwork( ndimr+im1 )
                 nlf = ic - nl
                 if( ( sqre==0_ilp64 ) .and. ( i==ll ) ) then
                    sqrei = sqre
                 else
                    sqrei = 1_ilp64
                 end if
                 idxqc = idxq + nlf - 1_ilp64
                 alpha = d( ic )
                 beta = e( ic )
                 call stdlib_I64_slasd1( nl, nr, sqrei, d( nlf ), alpha, beta,u( nlf, nlf ), ldu, vt( &
                           nlf, nlf ), ldvt,iwork( idxqc ), iwork( iwk ), work, info )
           ! report the possible convergence failure.
                 if( info/=0_ilp64 ) then
                    return
     ...
     !! and optionally, the singular vectors in compact form.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DLASD0', -info )
              return
           end if
           ! if the input matrix is too small, call stdlib_I64_dlasdq to find the svd.
           if( n<=smlsiz ) then
              call stdlib_I64_dlasdq( 'U', sqre, n, m, n, 0_ilp64, d, e, vt, ldvt, u, ldu, u,ldu, work, &
                        info )
              return
           end if
           ! set up the computation tree.
           ...
           ! for the nodes on bottom level of the tree, solve
           ! their subproblems by stdlib_I64_dlasdq.
           ndb1 = ( nd+1 ) / 2_ilp64
           ncc = 0_ilp64
           loop_30: do i = ndb1, nd
           ! ic : center row of each node
           ...
           ! nrf: starting row of the right  subproblem
              ...
              nrf = ic + 1_ilp64
              sqrei = 1_ilp64
              call stdlib_I64_dlasdq( 'U', sqrei, nl, nlp1, nl, ncc, d( nlf ), e( nlf ),vt( nlf, nlf )&
                        , ldvt, u( nlf, nlf ), ldu,u( nlf, nlf ), ldu, work, info )
              if( info/=0_ilp64 ) then
                 return
              end if
              ...
              nrp1 = nr + sqrei
              call stdlib_I64_dlasdq( 'U', sqrei, nr, nrp1, nr, ncc, d( nrf ), e( nrf ),vt( nrf, nrf )&
                        , ldvt, u( nrf, nrf ), ldu,u( nrf, nrf ), ldu, work, info )
              if( info/=0_ilp64 ) then
                 return
              end if
           ...
           loop_50: do lvl = nlvl, 1, -1
              ...
              do i = lf, ll
                 ...
                 alpha = d( ic )
                 beta = e( ic )
                 call stdlib_I64_dlasd1( nl, nr, sqrei, d( nlf ), alpha, beta,u( nlf, nlf ), ldu, vt( &
                           nlf, nlf ), ldvt,iwork( idxqc ), iwork( iwk ), work, info )
              ! report the possible convergence failure.
                 if( info/=0_ilp64 ) then
                    return
     ...
     !! conquer.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           integer(ilp64) :: i, il, ir, llst, maxn, ncrnt, nlvl
           real(sp) :: temp
           ! Intrinsic Functions 
           ! Executable Statements 
           ! find the number of levels on the tree.
           maxn = max( 1_ilp64, n )
           temp = log( real( maxn,KIND=sp) / real( msub+1,KIND=sp) ) / log( two )
           lvl = int( temp,KIND=ilp64) + 1_ilp64
           i = n / 2_ilp64
           inode( 1_ilp64 ) = i + 1_ilp64
           ndiml( 1_ilp64 ) = i
           ndimr( 1_ilp64 ) = n - i - 1_ilp64
           il = 0_ilp64
           ir = 1_ilp64
           llst = 1_ilp64
           do nlvl = 1, lvl - 1
              ...
              do i = 0, llst - 1
                 ...
                 ir = ir + 2_ilp64
                 ncrnt = llst + i
                 ndiml( il ) = ndiml( ncrnt ) / 2_ilp64
                 ndimr( il ) = ndiml( ncrnt ) - ndiml( il ) - 1_ilp64
                 inode( il ) = inode( ncrnt ) - ndimr( il ) - 1_ilp64
                 ndiml( ir ) = ndimr( ncrnt ) / 2_ilp64
                 ndimr( ir ) = ndimr( ncrnt ) - ndiml( ir ) - 1_ilp64
                 inode( ir ) = inode( ncrnt ) + ndiml( ir ) + 1_ilp64
              end do
              llst = llst*2_ilp64
           end do
           nd = llst*2_ilp64 - 1_ilp64
           return
     ...
     !! from the overall problem.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           integer(ilp64) :: coltyp, i, idx, idxc, idxp, iq, isigma, iu2, ivt2, iz, k, ldq, ldu2, &
                     ldvt2, m, n, n1, n2
           real(sp) :: orgnrm
           ! Intrinsic Functions 
           ! Executable Statements 
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SLASD1', -info )
              return
           end if
           n = nl + nr + 1_ilp64
           m = n + sqre
           ! the following values are for bookkeeping purposes only.  they are
           ! integer pointers which indicate the portion of the workspace
           ! used by a particular array in stdlib_I64_slasd2 and stdlib_I64_slasd3.
           ldu2 = n
           ldvt2 = m
           iz = 1_ilp64
           isigma = iz + m
           ...
           beta = beta / orgnrm
           ! deflate singular values.
           call stdlib_I64_slasd2( nl, nr, sqre, k, d, work( iz ), alpha, beta, u, ldu,vt, ldvt, work(&
            isigma ), work( iu2 ), ldu2,work( ivt2 ), ldvt2, iwork( idxp ), iwork( idx ),iwork( &
                      idxc ), idxq, iwork( coltyp ), info )
           ! solve secular equation and update singular vectors.
           ldq = k
           call stdlib_I64_slasd3( nl, nr, sqre, k, d, work( iq ), ldq, work( isigma ),u, ldu, work( &
           iu2 ), ldu2, vt, ldvt, work( ivt2 ),ldvt2, iwork( idxc ), iwork( coltyp ), work( iz ),&
                     info )
           ! report the possible convergence failure.
           if( info/=0_ilp64 ) then
           ...
           n1 = k
           n2 = n - k
           call stdlib_I64_slamrg( n1, n2, d, 1_ilp64, -1_ilp64, idxq )
           return
     ...
     !! from the overall problem.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DLASD1', -info )
              return
           end if
           n = nl + nr + 1_ilp64
           m = n + sqre
           ! the following values are for bookkeeping purposes only.  they are
           ! integer pointers which indicate the portion of the workspace
           ! used by a particular array in stdlib_I64_dlasd2 and stdlib_I64_dlasd3.
           ldu2 = n
           ldvt2 = m
           iz = 1_ilp64
           isigma = iz + m
           ...
           beta = beta / orgnrm
           ! deflate singular values.
           call stdlib_I64_dlasd2( nl, nr, sqre, k, d, work( iz ), alpha, beta, u, ldu,vt, ldvt, work(&
            isigma ), work( iu2 ), ldu2,work( ivt2 ), ldvt2, iwork( idxp ), iwork( idx ),iwork( &
                      idxc ), idxq, iwork( coltyp ), info )
           ! solve secular equation and update singular vectors.
           ldq = k
           call stdlib_I64_dlasd3( nl, nr, sqre, k, d, work( iq ), ldq, work( isigma ),u, ldu, work( &
           iu2 ), ldu2, vt, ldvt, work( ivt2 ),ldvt2, iwork( idxc ), iwork( coltyp ), work( iz ),&
                     info )
           ! report the convergence failure.
           if( info/=0_ilp64 ) then
           ...
           n1 = k
           n2 = n - k
           call stdlib_I64_dlamrg( n1, n2, d, 1_ilp64, -1_ilp64, idxq )
           return
     ...
     !! SLASD2 is called from SLASD1.
        ...
        ! =====================================================================
           ...
           ! Local Arrays 
           integer(ilp64) :: ctot(4_ilp64), psm(4_ilp64)
           ! Local Scalars 
           integer(ilp64) :: ct, i, idxi, idxj, idxjp, j, jp, jprev, k2, m, n, nlp1, nlp2
           real(sp) :: c, eps, hlftol, s, tau, tol, z1
           ! Intrinsic Functions 
           ! Executable Statements 
           ! test the input parameters.
           ...
           if( nl<1_ilp64 ) then
              ...
              info = -2_ilp64
           else if( ( sqre/=1_ilp64 ) .and. ( sqre/=0_ilp64 ) ) then
              info = -3_ilp64
           end if
           n = nl + nr + 1_ilp64
           m = n + sqre
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SLASD2', -info )
              return
           end if
           nlp1 = nl + 1_ilp64
           nlp2 = nl + 2_ilp64
           ...
           do i = 2, n
              dsigma( i ) = d( idxq( i ) )
              u2( i, 1_ilp64 ) = z( idxq( i ) )
              idxc( i ) = coltyp( idxq( i ) )
           end do
           call stdlib_I64_slamrg( nl, nr, dsigma( 2_ilp64 ), 1_ilp64, 1_ilp64, idx( 2_ilp64 ) )
           do i = 2, n
              idxi = 1_ilp64 + idx( i )
              d( i ) = dsigma( idxi )
              z( i ) = u2( idxi, 1_ilp64 )
           ...
           if( abs( z( j ) )<=tol ) then
              ...
              if( abs( d( j )-d( jprev ) )<=tol ) then
                 ...
                 ! apply back the givens rotation to the left and right
                 ! singular vector matrices.
                 idxjp = idxq( idx( jprev )+1_ilp64 )
                 idxj = idxq( idx( j )+1_ilp64 )
                 if( idxjp<=nlp1 ) then
                    idxjp = idxjp - 1_ilp64
                 end if
                 if( idxj<=nlp1 ) then
                 ...
                 call stdlib_I64_srot( n, u( 1_ilp64, idxjp ), 1_ilp64, u( 1_ilp64, idxj ), 1_ilp64, c, s )
                 call stdlib_I64_srot( m, vt( idxjp, 1_ilp64 ), ldvt, vt( idxj, 1_ilp64 ), ldvt, c,s )
                 if( coltyp( j )/=coltyp( jprev ) ) then
                    coltyp( j ) = 3_ilp64
                 end if
                 coltyp( jprev ) = 4_ilp64
                 k2 = k2 - 1_ilp64
                 idxp( k2 ) = jprev
                 jprev = j
              else
                 k = k + 1_ilp64
                 u2( k, 1_ilp64 ) = z( jprev )
                 dsigma( k ) = d( jprev )
                 idxp( k ) = jprev
                 jprev = j
              end if
           ...
           do j = 2, n
              ct = coltyp( j )
              ctot( ct ) = ctot( ct ) + 1_ilp64
           end do
           ! psm(*) = position in submatrix (of types 1 through 4)
           psm( 1_ilp64 ) = 2_ilp64
           psm( 2_ilp64 ) = 2_ilp64 + ctot( 1_ilp64 )
           psm( 3_ilp64 ) = psm( 2_ilp64 ) + ctot( 2_ilp64 )
           psm( 4_ilp64 ) = psm( 3_ilp64 ) + ctot( 3_ilp64 )
           ! fill out the idxc array so that the permutation which it induces
           ! will place all type-1 columns first, all type-2 columns next,
           ! then all type-3's, and finally all type-4's, starting from the
           ! second column. this applies similarly to the rows of vt.
           ...
           do j = 2, n
              jp = idxp( j )
              dsigma( j ) = d( jp )
              idxj = idxq( idx( idxp( idxc( j ) ) )+1_ilp64 )
              if( idxj<=nlp1 ) then
                 idxj = idxj - 1_ilp64
              end if
              call stdlib_I64_scopy( n, u( 1_ilp64, idxj ), 1_ilp64, u2( 1_ilp64, j ), 1_ilp64 )
              call stdlib_I64_scopy( m, vt( idxj, 1_ilp64 ), ldvt, vt2( j, 1_ilp64 ), ldvt2 )
           end do
           ! determine dsigma(1), dsigma(2) and z(1)
           dsigma( 1_ilp64 ) = zero
           hlftol = tol / two
           if( abs( dsigma( 2_ilp64 ) )<=hlftol )dsigma( 2_ilp64 ) = hlftol
           if( m>n ) then
              z( 1_ilp64 ) = stdlib_I64_slapy2( z1, z( m ) )
              if( z( 1_ilp64 )<=tol ) then
                 c = one
                 s = zero
                 z( 1_ilp64 ) = tol
              else
           ...
           ! move the rest of the updating row to z.
           call stdlib_I64_scopy( k-1, u2( 2_ilp64, 1_ilp64 ), 1_ilp64, z( 2_ilp64 ), 1_ilp64 )
           ! determine the first column of u2, the first row of vt2 and the
           ! last row of vt.
           call stdlib_I64_slaset( 'A', n, 1_ilp64, zero, zero, u2, ldu2 )
           u2( nlp1, 1_ilp64 ) = one
           if( m>n ) then
              do i = 1, nlp1
                 vt( m, i ) = -s*vt( nlp1, i )
                 vt2( 1_ilp64, i ) = c*vt( nlp1, i )
              end do
              do i = nlp2, m
                 vt2( 1_ilp64, i ) = s*vt( m, i )
                 vt( m, i ) = c*vt( m, i )
              end do
           else
              call stdlib_I64_scopy( m, vt( nlp1, 1_ilp64 ), ldvt, vt2( 1_ilp64, 1_ilp64 ), ldvt2 )
           end if
           if( m>n ) then
              call stdlib_I64_scopy( m, vt( m, 1_ilp64 ), ldvt, vt2( m, 1_ilp64 ), ldvt2 )
           end if
           ! the deflated singular values and their corresponding vectors go
           ! into the back of d, u, and v respectively.
           if( n>k ) then
              call stdlib_I64_scopy( n-k, dsigma( k+1 ), 1_ilp64, d( k+1 ), 1_ilp64 )
              call stdlib_I64_slacpy( 'A', n, n-k, u2( 1_ilp64, k+1 ), ldu2, u( 1_ilp64, k+1 ),ldu )
              call stdlib_I64_slacpy( 'A', n-k, m, vt2( k+1, 1_ilp64 ), ldvt2, vt( k+1, 1_ilp64 ),ldvt )
           end if
           ! copy ctot into coltyp for referencing in stdlib_I64_slasd3.
           do j = 1, 4
              coltyp( j ) = ctot( j )
           end do
           return
     ...
     !! DLASD2 is called from DLASD1.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DLASD2', -info )
              return
           end if
           nlp1 = nl + 1_ilp64
           nlp2 = nl + 2_ilp64
           ...
           call stdlib_I64_dlamrg( nl, nr, dsigma( 2_ilp64 ), 1_ilp64, 1_ilp64, idx( 2_ilp64 ) )
           do i = 2, n
              idxi = 1_ilp64 + idx( i )
              d( i ) = dsigma( idxi )
              z( i ) = u2( idxi, 1_ilp64 )
           ...
           if( abs( z( j ) )<=tol ) then
              ...
              if( abs( d( j )-d( jprev ) )<=tol ) then
                 ...
                 call stdlib_I64_drot( n, u( 1_ilp64, idxjp ), 1_ilp64, u( 1_ilp64, idxj ), 1_ilp64, c, s )
                 call stdlib_I64_drot( m, vt( idxjp, 1_ilp64 ), ldvt, vt( idxj, 1_ilp64 ), ldvt, c,s )
                 if( coltyp( j )/=coltyp( jprev ) ) then
                    coltyp( j ) = 3_ilp64
                 end if
                 coltyp( jprev ) = 4_ilp64
           ...
           do j = 2, n
              ...
              call stdlib_I64_dcopy( n, u( 1_ilp64, idxj ), 1_ilp64, u2( 1_ilp64, j ), 1_ilp64 )
              call stdlib_I64_dcopy( m, vt( idxj, 1_ilp64 ), ldvt, vt2( j, 1_ilp64 ), ldvt2 )
           end do
           ! determine dsigma(1), dsigma(2) and z(1)
           dsigma( 1_ilp64 ) = zero
           hlftol = tol / two
           if( abs( dsigma( 2_ilp64 ) )<=hlftol )dsigma( 2_ilp64 ) = hlftol
           if( m>n ) then
              z( 1_ilp64 ) = stdlib_I64_dlapy2( z1, z( m ) )
              if( z( 1_ilp64 )<=tol ) then
                 c = one
                 s = zero
                 z( 1_ilp64 ) = tol
           ...
           ! move the rest of the updating row to z.
           call stdlib_I64_dcopy( k-1, u2( 2_ilp64, 1_ilp64 ), 1_ilp64, z( 2_ilp64 ), 1_ilp64 )
           ! determine the first column of u2, the first row of vt2 and the
           ! last row of vt.
           call stdlib_I64_dlaset( 'A', n, 1_ilp64, zero, zero, u2, ldu2 )
           u2( nlp1, 1_ilp64 ) = one
           if( m>n ) then
              do i = 1, nlp1
                 vt( m, i ) = -s*vt( nlp1, i )
              ...
              call stdlib_I64_dcopy( m, vt( nlp1, 1_ilp64 ), ldvt, vt2( 1_ilp64, 1_ilp64 ), ldvt2 )
           end if
           if( m>n ) then
              call stdlib_I64_dcopy( m, vt( m, 1_ilp64 ), ldvt, vt2( m, 1_ilp64 ), ldvt2 )
           end if
           ! the deflated singular values and their corresponding vectors go
           ! into the back of d, u, and v respectively.
           if( n>k ) then
              call stdlib_I64_dcopy( n-k, dsigma( k+1 ), 1_ilp64, d( k+1 ), 1_ilp64 )
              call stdlib_I64_dlacpy( 'A', n, n-k, u2( 1_ilp64, k+1 ), ldu2, u( 1_ilp64, k+1 ),ldu )
              call stdlib_I64_dlacpy( 'A', n-k, m, vt2( k+1, 1_ilp64 ), ldvt2, vt( k+1, 1_ilp64 ),ldvt )
           end if
           ! copy ctot into coltyp for referencing in stdlib_I64_dlasd3.
           do j = 1, 4
              coltyp( j ) = ctot( j )
           end do
           return
     ...
     !! SLASD3 is called from SLASD1.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           integer(ilp64) :: ctemp, i, j, jc, ktemp, m, n, nlp1, nlp2, nrp1
           real(sp) :: rho, temp
           ! Intrinsic Functions 
           ! Executable Statements 
           ! test the input parameters.
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SLASD3', -info )
              return
           end if
           ! quick return if possible
           if( k==1_ilp64 ) then
              d( 1_ilp64 ) = abs( z( 1_ilp64 ) )
              call stdlib_I64_scopy( m, vt2( 1_ilp64, 1_ilp64 ), ldvt2, vt( 1_ilp64, 1_ilp64 ), ldvt )
              if( z( 1_ilp64 )>zero ) then
                 call stdlib_I64_scopy( n, u2( 1_ilp64, 1_ilp64 ), 1_ilp64, u( 1_ilp64, 1_ilp64 ), 1_ilp64 )
              else
                 do i = 1, n
                    u( i, 1_ilp64 ) = -u2( i, 1_ilp64 )
                 end do
              end if
              return
           end if
           ...
           ! this code.
           do i = 1, k
              dsigma( i ) = stdlib_I64_slamc3( dsigma( i ), dsigma( i ) ) - dsigma( i )
           end do
           ! keep a copy of z.
           call stdlib_I64_scopy( k, z, 1_ilp64, q, 1_ilp64 )
           ! normalize z.
           rho = stdlib_I64_snrm2( k, z, 1_ilp64 )
           call stdlib_I64_slascl( 'G', 0_ilp64, 0_ilp64, rho, one, k, 1_ilp64, z, k, info )
           rho = rho*rho
           ! find the new singular values.
           do j = 1, k
              call stdlib_I64_slasd4( k, j, dsigma, z, u( 1_ilp64, j ), rho, d( j ),vt( 1_ilp64, j ), info )
           ...
           do i = 1, k
              ...
              z( i ) = sign( sqrt( abs( z( i ) ) ), q( i, 1_ilp64 ) )
           end do
           ! compute left singular vectors of the modified diagonal matrix,
           ! and store related information for the right singular vectors.
           do i = 1, k
              vt( 1_ilp64, i ) = z( 1_ilp64 ) / u( 1_ilp64, i ) / vt( 1_ilp64, i )
              u( 1_ilp64, i ) = negone
              do j = 2, k
                 vt( j, i ) = z( j ) / u( j, i ) / vt( j, i )
                 u( j, i ) = dsigma( j )*vt( j, i )
              end do
              temp = stdlib_I64_snrm2( k, u( 1_ilp64, i ), 1_ilp64 )
              q( 1_ilp64, i ) = u( 1_ilp64, i ) / temp
              do j = 2, k
                 jc = idxc( j )
                 q( j, i ) = u( jc, i ) / temp
              end do
           ...
           ! update the left singular vector matrix.
           if( k==2_ilp64 ) then
              call stdlib_I64_sgemm( 'N', 'N', n, k, k, one, u2, ldu2, q, ldq, zero, u,ldu )
              go to 100
           end if
           if( ctot( 1_ilp64 )>0_ilp64 ) then
              call stdlib_I64_sgemm( 'N', 'N', nl, k, ctot( 1_ilp64 ), one, u2( 1_ilp64, 2_ilp64 ), ldu2,q( 2_ilp64, 1_ilp64 ),&
                  & ldq,&
                         zero, u( 1_ilp64, 1_ilp64 ), ldu )
              if( ctot( 3_ilp64 )>0_ilp64 ) then
                 ktemp = 2_ilp64 + ctot( 1_ilp64 ) + ctot( 2_ilp64 )
                 call stdlib_I64_sgemm( 'N', 'N', nl, k, ctot( 3_ilp64 ), one, u2( 1_ilp64, ktemp ),ldu2, q( &
                           ktemp, 1_ilp64 ), ldq, one, u( 1_ilp64, 1_ilp64 ), ldu )
              end if
           else if( ctot( 3_ilp64 )>0_ilp64 ) then
              ktemp = 2_ilp64 + ctot( 1_ilp64 ) + ctot( 2_ilp64 )
              call stdlib_I64_sgemm( 'N', 'N', nl, k, ctot( 3_ilp64 ), one, u2( 1_ilp64, ktemp ),ldu2, q( ktemp, &
                        1_ilp64 ), ldq, zero, u( 1_ilp64, 1_ilp64 ), ldu )
           else
              call stdlib_I64_slacpy( 'F', nl, k, u2, ldu2, u, ldu )
           end if
           call stdlib_I64_scopy( k, q( 1_ilp64, 1_ilp64 ), ldq, u( nlp1, 1_ilp64 ), ldu )
           ktemp = 2_ilp64 + ctot( 1_ilp64 )
           ctemp = ctot( 2_ilp64 ) + ctot( 3_ilp64 )
           call stdlib_I64_sgemm( 'N', 'N', nr, k, ctemp, one, u2( nlp2, ktemp ), ldu2,q( ktemp, 1_ilp64 ), &
                     ldq, zero, u( nlp2, 1_ilp64 ), ldu )
           ! generate the right singular vectors.
           100 continue
           do i = 1, k
              temp = stdlib_I64_snrm2( k, vt( 1_ilp64, i ), 1_ilp64 )
              q( i, 1_ilp64 ) = vt( 1_ilp64, i ) / temp
              do j = 2, k
                 jc = idxc( j )
                 q( i, j ) = vt( jc, i ) / temp
              end do
           ...
           ! update the right singular vector matrix.
           if( k==2_ilp64 ) then
              call stdlib_I64_sgemm( 'N', 'N', k, m, k, one, q, ldq, vt2, ldvt2, zero,vt, ldvt )
                        
              return
           end if
           ktemp = 1_ilp64 + ctot( 1_ilp64 )
           call stdlib_I64_sgemm( 'N', 'N', k, nlp1, ktemp, one, q( 1_ilp64, 1_ilp64 ), ldq,vt2( 1_ilp64, 1_ilp64 ), ldvt2, &
                     zero, vt( 1_ilp64, 1_ilp64 ), ldvt )
           ktemp = 2_ilp64 + ctot( 1_ilp64 ) + ctot( 2_ilp64 )
           if( ktemp<=ldvt2 )call stdlib_I64_sgemm( 'N', 'N', k, nlp1, ctot( 3_ilp64 ), one, q( 1_ilp64, ktemp ),&
                     ldq, vt2( ktemp, 1_ilp64 ), ldvt2, one, vt( 1_ilp64, 1_ilp64 ),ldvt )
           ktemp = ctot( 1_ilp64 ) + 1_ilp64
           nrp1 = nr + sqre
           if( ktemp>1_ilp64 ) then
              do i = 1, k
                 q( i, ktemp ) = q( i, 1_ilp64 )
              end do
              do i = nlp2, m
                 vt2( ktemp, i ) = vt2( 1_ilp64, i )
              end do
           end if
           ctemp = 1_ilp64 + ctot( 2_ilp64 ) + ctot( 3_ilp64 )
           call stdlib_I64_sgemm( 'N', 'N', k, nrp1, ctemp, one, q( 1_ilp64, ktemp ), ldq,vt2( ktemp, nlp2 )&
                     , ldvt2, zero, vt( 1_ilp64, nlp2 ), ldvt )
           return
     ...
     !! DLASD3 is called from DLASD1.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DLASD3', -info )
              return
           end if
           ! quick return if possible
           if( k==1_ilp64 ) then
              d( 1_ilp64 ) = abs( z( 1_ilp64 ) )
              call stdlib_I64_dcopy( m, vt2( 1_ilp64, 1_ilp64 ), ldvt2, vt( 1_ilp64, 1_ilp64 ), ldvt )
              if( z( 1_ilp64 )>zero ) then
                 call stdlib_I64_dcopy( n, u2( 1_ilp64, 1_ilp64 ), 1_ilp64, u( 1_ilp64, 1_ilp64 ), 1_ilp64 )
              else
                 do i = 1, n
                    u( i, 1_ilp64 ) = -u2( i, 1_ilp64 )
                 end do
           ...
           ! this code.
           do i = 1, k
              dsigma( i ) = stdlib_I64_dlamc3( dsigma( i ), dsigma( i ) ) - dsigma( i )
           end do
           ! keep a copy of z.
           call stdlib_I64_dcopy( k, z, 1_ilp64, q, 1_ilp64 )
           ! normalize z.
           rho = stdlib_I64_dnrm2( k, z, 1_ilp64 )
           call stdlib_I64_dlascl( 'G', 0_ilp64, 0_ilp64, rho, one, k, 1_ilp64, z, k, info )
           rho = rho*rho
           ! find the new singular values.
           do j = 1, k
              call stdlib_I64_dlasd4( k, j, dsigma, z, u( 1_ilp64, j ), rho, d( j ),vt( 1_ilp64, j ), info )
           ...
           do i = 1, k
              ...
              temp = stdlib_I64_dnrm2( k, u( 1_ilp64, i ), 1_ilp64 )
              q( 1_ilp64, i ) = u( 1_ilp64, i ) / temp
              do j = 2, k
                 jc = idxc( j )
                 q( j, i ) = u( jc, i ) / temp
           ...
           ! update the left singular vector matrix.
           if( k==2_ilp64 ) then
              call stdlib_I64_dgemm( 'N', 'N', n, k, k, one, u2, ldu2, q, ldq, zero, u,ldu )
              go to 100
           end if
           if( ctot( 1_ilp64 )>0_ilp64 ) then
              call stdlib_I64_dgemm( 'N', 'N', nl, k, ctot( 1_ilp64 ), one, u2( 1_ilp64, 2_ilp64 ), ldu2,q( 2_ilp64, 1_ilp64 ),&
                  & ldq,&
                         zero, u( 1_ilp64, 1_ilp64 ), ldu )
              if( ctot( 3_ilp64 )>0_ilp64 ) then
                 ktemp = 2_ilp64 + ctot( 1_ilp64 ) + ctot( 2_ilp64 )
                 call stdlib_I64_dgemm( 'N', 'N', nl, k, ctot( 3_ilp64 ), one, u2( 1_ilp64, ktemp ),ldu2, q( &
                           ktemp, 1_ilp64 ), ldq, one, u( 1_ilp64, 1_ilp64 ), ldu )
              end if
           else if( ctot( 3_ilp64 )>0_ilp64 ) then
              ktemp = 2_ilp64 + ctot( 1_ilp64 ) + ctot( 2_ilp64 )
              call stdlib_I64_dgemm( 'N', 'N', nl, k, ctot( 3_ilp64 ), one, u2( 1_ilp64, ktemp ),ldu2, q( ktemp, &
                        1_ilp64 ), ldq, zero, u( 1_ilp64, 1_ilp64 ), ldu )
           else
              call stdlib_I64_dlacpy( 'F', nl, k, u2, ldu2, u, ldu )
           end if
           call stdlib_I64_dcopy( k, q( 1_ilp64, 1_ilp64 ), ldq, u( nlp1, 1_ilp64 ), ldu )
           ktemp = 2_ilp64 + ctot( 1_ilp64 )
           ctemp = ctot( 2_ilp64 ) + ctot( 3_ilp64 )
           call stdlib_I64_dgemm( 'N', 'N', nr, k, ctemp, one, u2( nlp2, ktemp ), ldu2,q( ktemp, 1_ilp64 ), &
                     ldq, zero, u( nlp2, 1_ilp64 ), ldu )
           ! generate the right singular vectors.
           100 continue
           do i = 1, k
              temp = stdlib_I64_dnrm2( k, vt( 1_ilp64, i ), 1_ilp64 )
              q( i, 1_ilp64 ) = vt( 1_ilp64, i ) / temp
              do j = 2, k
                 jc = idxc( j )
                 q( i, j ) = vt( jc, i ) / temp
           ...
           ! update the right singular vector matrix.
           if( k==2_ilp64 ) then
              call stdlib_I64_dgemm( 'N', 'N', k, m, k, one, q, ldq, vt2, ldvt2, zero,vt, ldvt )
                        
              return
           end if
           ktemp = 1_ilp64 + ctot( 1_ilp64 )
           call stdlib_I64_dgemm( 'N', 'N', k, nlp1, ktemp, one, q( 1_ilp64, 1_ilp64 ), ldq,vt2( 1_ilp64, 1_ilp64 ), ldvt2, &
                     zero, vt( 1_ilp64, 1_ilp64 ), ldvt )
           ktemp = 2_ilp64 + ctot( 1_ilp64 ) + ctot( 2_ilp64 )
           if( ktemp<=ldvt2 )call stdlib_I64_dgemm( 'N', 'N', k, nlp1, ctot( 3_ilp64 ), one, q( 1_ilp64, ktemp ),&
                     ldq, vt2( ktemp, 1_ilp64 ), ldvt2, one, vt( 1_ilp64, 1_ilp64 ),ldvt )
           ktemp = ctot( 1_ilp64 ) + 1_ilp64
           nrp1 = nr + sqre
           if( ktemp>1_ilp64 ) then
           ...
           ctemp = 1_ilp64 + ctot( 2_ilp64 ) + ctot( 3_ilp64 )
           call stdlib_I64_dgemm( 'N', 'N', k, nrp1, ctemp, one, q( 1_ilp64, ktemp ), ldq,vt2( ktemp, nlp2 )&
                     , ldvt2, zero, vt( 1_ilp64, nlp2 ), ldvt )
           return
     ...
     !! secular equation by simpler interpolating rational functions.
        ...
        ! =====================================================================
           ! Parameters 
           integer(ilp64), parameter :: maxit = 400_ilp64
           
           
           ! Local Scalars 
           logical(lk) :: orgati, swtch, swtch3, geomavg
           integer(ilp64) :: ii, iim1, iip1, ip1, iter, j, niter
           real(sp) :: a, b, c, delsq, delsq2, sq2, dphi, dpsi, dtiim, dtiip, dtipsq, dtisq, &
           dtnsq, dtnsq1, dw, eps, erretm, eta, phi, prew, psi, rhoinv, sglb, sgub, tau, tau2, &
                     temp, temp1, temp2, w
           ! Local Arrays 
           real(sp) :: dd(3_ilp64), zz(3_ilp64)
           ! Intrinsic Functions 
           ! Executable Statements 
           ! since this routine is called in an inner loop, we do no argument
           ! checking.
           ...
           if( n==1_ilp64 ) then
              ! presumably, i=1 upon entry
              sigma = sqrt( d( 1_ilp64 )*d( 1_ilp64 )+rho*z( 1_ilp64 )*z( 1_ilp64 ) )
              delta( 1_ilp64 ) = one
              work( 1_ilp64 ) = one
              return
           end if
           if( n==2_ilp64 ) then
              call stdlib_I64_slasd5( i, d, z, delta, rho, sigma, work )
              return
           end if
           ! compute machine epsilon
           eps = stdlib_I64_slamch( 'EPSILON' )
          ...
          ! $             + abs( tau2 )*( dpsi+dphi )
              ...
              if( ii==1_ilp64 .or. ii==n )swtch3 = .false.
              temp = z( ii ) / ( work( ii )*delta( ii ) )
              dw = dpsi + dphi + temp*temp
              temp = z( ii )*temp
              w = w + temp
          ...
          ! $          + abs( tau2 )*dw
              ...
              if( .not.swtch3 ) then
                 ...
                 dtisq = work( i )*delta( i )
                 if( orgati ) then
                    c = w - dtipsq*dw + delsq*( z( i ) / dtisq )**2_ilp64
                 else
                    c = w - dtisq*dw - delsq*( z( ip1 ) / dtipsq )**2_ilp64
                 end if
                 a = ( dtipsq+dtisq )*w - dtipsq*dtisq*dw
                 b = dtipsq*dtisq*w
                 if( c==zero ) then
                 ...
                 if( orgati ) then
                    ...
                    c = ( temp - dtiip*( dpsi+dphi ) ) -( d( iim1 )-d( iip1 ) )*( d( iim1 )+d( &
                              iip1 ) )*temp1
                    zz( 1_ilp64 ) = z( iim1 )*z( iim1 )
                    if( dpsi<temp1 ) then
                       zz( 3_ilp64 ) = dtiip*dtiip*dphi
                    else
                       zz( 3_ilp64 ) = dtiip*dtiip*( ( dpsi-temp1 )+dphi )
                    end if
                 else
                    temp1 = z( iip1 ) / dtiip
                    temp1 = temp1*temp1
                    c = ( temp - dtiim*( dpsi+dphi ) ) -( d( iip1 )-d( iim1 ) )*( d( iim1 )+d( &
                              iip1 ) )*temp1
                    if( dphi<temp1 ) then
                       zz( 1_ilp64 ) = dtiim*dtiim*dpsi
                    else
                       zz( 1_ilp64 ) = dtiim*dtiim*( dpsi+( dphi-temp1 ) )
                    end if
                    zz( 3_ilp64 ) = z( iip1 )*z( iip1 )
                 end if
                 zz( 2_ilp64 ) = z( ii )*z( ii )
                 dd( 1_ilp64 ) = dtiim
                 dd( 2_ilp64 ) = delta( ii )*work( ii )
                 dd( 3_ilp64 ) = dtiip
                 call stdlib_I64_slaed6( niter, orgati, c, dd, zz, w, eta, info )
                 if( info/=0_ilp64 ) then
                    ! if info is not 0, i.e., stdlib_I64_slaed6 failed, switch back
                    ! to 2 pole interpolation.
                    swtch3 = .false.
                    info = 0_ilp64
                    dtipsq = work( ip1 )*delta( ip1 )
          ...
          ! $          + abs( tau2 )*dw
           ...
           ! $          .or. (sgub-sglb)<=eight*abs(sgub+sglb) ) then
                 ...
                 if( .not.swtch3 ) then
                    ...
                    call stdlib_I64_slaed6( niter, orgati, c, dd, zz, w, eta, info )
                    if( info/=0_ilp64 ) then
                       ! if info is not 0, i.e., stdlib_I64_slaed6 failed, switch
                       ! back to two pole interpolation
                       swtch3 = .false.
                       info = 0_ilp64
                       dtipsq = work( ip1 )*delta( ip1 )
     ...
     !! secular equation by simpler interpolating rational functions.
        ...
        ! =====================================================================
           ...
           if( n==2_ilp64 ) then
              call stdlib_I64_dlasd5( i, d, z, delta, rho, sigma, work )
              return
           end if
           ! compute machine epsilon
           eps = stdlib_I64_dlamch( 'EPSILON' )
          ...
          ! $          + abs( tau2 )*dw
              ...
              if( .not.swtch3 ) then
                 ...
                 dd( 2_ilp64 ) = delta( ii )*work( ii )
                 dd( 3_ilp64 ) = dtiip
                 call stdlib_I64_dlaed6( niter, orgati, c, dd, zz, w, eta, info )
                 if( info/=0_ilp64 ) then
                    ! if info is not 0, i.e., stdlib_I64_dlaed6 failed, switch back
                    ! to 2 pole interpolation.
                    swtch3 = .false.
                    info = 0_ilp64
                    dtipsq = work( ip1 )*delta( ip1 )
          ...
          ! $          + abs( tau2 )*dw
           ...
           ! $          .or. (sgub-sglb)<=eight*abs(sgub+sglb) ) then
                 ...
                 if( .not.swtch3 ) then
                    ...
                    call stdlib_I64_dlaed6( niter, orgati, c, dd, zz, w, eta, info )
                    if( info/=0_ilp64 ) then
                       ! if info is not 0, i.e., stdlib_I64_dlaed6 failed, switch
                       ! back to two pole interpolation
                       swtch3 = .false.
                       info = 0_ilp64
                       dtipsq = work( ip1 )*delta( ip1 )
     ...
     !! Z is one.
        ...
        ! =====================================================================
           ...
           ! Intrinsic Functions 
           ! Executable Statements 
           del = d( 2_ilp64 ) - d( 1_ilp64 )
           delsq = del*( d( 2_ilp64 )+d( 1_ilp64 ) )
           if( i==1_ilp64 ) then
              w = one + four*rho*( z( 2_ilp64 )*z( 2_ilp64 ) / ( d( 1_ilp64 )+three*d( 2_ilp64 ) )-z( 1_ilp64 )*z( 1_ilp64 ) / ( &
                        three*d( 1_ilp64 )+d( 2_ilp64 ) ) ) / del
              if( w>zero ) then
                 b = delsq + rho*( z( 1_ilp64 )*z( 1_ilp64 )+z( 2_ilp64 )*z( 2_ilp64 ) )
                 c = rho*z( 1_ilp64 )*z( 1_ilp64 )*delsq
                 ! b > zero, always
                 ! the following tau is dsigma * dsigma - d( 1 ) * d( 1 )
                 tau = two*c / ( b+sqrt( abs( b*b-four*c ) ) )
                 ! the following tau is dsigma - d( 1 )
                 tau = tau / ( d( 1_ilp64 )+sqrt( d( 1_ilp64 )*d( 1_ilp64 )+tau ) )
                 dsigma = d( 1_ilp64 ) + tau
                 delta( 1_ilp64 ) = -tau
                 delta( 2_ilp64 ) = del - tau
                 work( 1_ilp64 ) = two*d( 1_ilp64 ) + tau
                 work( 2_ilp64 ) = ( d( 1_ilp64 )+tau ) + d( 2_ilp64 )
                 ! delta( 1 ) = -z( 1 ) / tau
                 ! delta( 2 ) = z( 2 ) / ( del-tau )
              else
                 b = -delsq + rho*( z( 1_ilp64 )*z( 1_ilp64 )+z( 2_ilp64 )*z( 2_ilp64 ) )
                 c = rho*z( 2_ilp64 )*z( 2_ilp64 )*delsq
                 ! the following tau is dsigma * dsigma - d( 2 ) * d( 2 )
                 if( b>zero ) then
                    tau = -two*c / ( b+sqrt( b*b+four*c ) )
                 else
                 ...
                 ! the following tau is dsigma - d( 2 )
                 tau = tau / ( d( 2_ilp64 )+sqrt( abs( d( 2_ilp64 )*d( 2_ilp64 )+tau ) ) )
                 dsigma = d( 2_ilp64 ) + tau
                 delta( 1_ilp64 ) = -( del+tau )
                 delta( 2_ilp64 ) = -tau
                 work( 1_ilp64 ) = d( 1_ilp64 ) + tau + d( 2_ilp64 )
                 work( 2_ilp64 ) = two*d( 2_ilp64 ) + tau
                 ! delta( 1 ) = -z( 1 ) / ( del+tau )
                 ! delta( 2 ) = -z( 2 ) / tau
              end if
              ! temp = sqrt( delta( 1 )*delta( 1 )+delta( 2 )*delta( 2 ) )
              ...
              ! the following tau is dsigma - d( 2 )
              tau = tau / ( d( 2_ilp64 )+sqrt( d( 2_ilp64 )*d( 2_ilp64 )+tau ) )
              dsigma = d( 2_ilp64 ) + tau
              delta( 1_ilp64 ) = -( del+tau )
              delta( 2_ilp64 ) = -tau
              work( 1_ilp64 ) = d( 1_ilp64 ) + tau + d( 2_ilp64 )
     ...
     !! LAPACK Working Note #3, for a detailed description of the algorithm.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           logical(lk) :: rotate
           integer(ilp64) :: i, isub, iuplo, j, np1, sqre1
           real(sp) :: cs, r, smin, sn
           ! Intrinsic Functions 
           ! Executable Statements 
           ! test the input parameters.
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SLASDQ', -info )
              return
           end if
           if( n==0 )return
           ! rotate is true if any singular vectors desired, false otherwise
           ...
           ! if matrix non-square upper bidiagonal, rotate to be lower
           ! bidiagonal.  the rotations are on the right.
           if( ( iuplo==1_ilp64 ) .and. ( sqre1==1_ilp64 ) ) then
              do i = 1, n - 1
              ...
              call stdlib_I64_slartg( d( n ), e( n ), cs, sn, r )
              d( n ) = r
              e( n ) = zero
              if( rotate ) then
                 work( n ) = cs
              ...
              sqre1 = 0_ilp64
              ! update singular vectors if desired.
              if( ncvt>0_ilp64 )call stdlib_I64_slasr( 'L', 'V', 'F', np1, ncvt, work( 1_ilp64 ),work( np1 ), vt, &
                        ldvt )
           end if
           ! if matrix lower bidiagonal, rotate to be upper bidiagonal
           ! by applying givens rotations on the left.
           if( iuplo==2_ilp64 ) then
              ...
              ! update singular vectors if desired.
              if( nru>0_ilp64 ) then
                 if( sqre1==0_ilp64 ) then
                    call stdlib_I64_slasr( 'R', 'V', 'F', nru, n, work( 1_ilp64 ),work( np1 ), u, ldu )
                              
                 else
                    call stdlib_I64_slasr( 'R', 'V', 'F', nru, np1, work( 1_ilp64 ),work( np1 ), u, ldu )
                              
                 end if
              end if
              if( ncc>0_ilp64 ) then
                 if( sqre1==0_ilp64 ) then
                    call stdlib_I64_slasr( 'L', 'V', 'F', n, ncc, work( 1_ilp64 ),work( np1 ), c, ldc )
                              
                 else
                    call stdlib_I64_slasr( 'L', 'V', 'F', np1, ncc, work( 1_ilp64 ),work( np1 ), c, ldc )
                              
                 end if
              end if
           end if
           ! call stdlib_I64_sbdsqr to compute the svd of the reduced real
           ! n-by-n upper bidiagonal matrix.
           call stdlib_I64_sbdsqr( 'U', n, ncvt, nru, ncc, d, e, vt, ldvt, u, ldu, c,ldc, work, info )
           ...
           do i = 1, n
              ...
              if( isub/=i ) then
                 ...
                 d( isub ) = d( i )
                 d( i ) = smin
                 if( ncvt>0_ilp64 )call stdlib_I64_sswap( ncvt, vt( isub, 1_ilp64 ), ldvt, vt( i, 1_ilp64 ), ldvt )
                           
                 if( nru>0_ilp64 )call stdlib_I64_sswap( nru, u( 1_ilp64, isub ), 1_ilp64, u( 1_ilp64, i ), 1_ilp64 )
                 if( ncc>0_ilp64 )call stdlib_I64_sswap( ncc, c( isub, 1_ilp64 ), ldc, c( i, 1_ilp64 ), ldc )
              end if
           end do
           return
     ...
     !! LAPACK Working Note #3, for a detailed description of the algorithm.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DLASDQ', -info )
              return
           end if
           if( n==0 )return
           ! rotate is true if any singular vectors desired, false otherwise
           ...
           if( ( iuplo==1_ilp64 ) .and. ( sqre1==1_ilp64 ) ) then
              ...
              call stdlib_I64_dlartg( d( n ), e( n ), cs, sn, r )
              d( n ) = r
              e( n ) = zero
              if( rotate ) then
                 work( n ) = cs
              ...
              sqre1 = 0_ilp64
              ! update singular vectors if desired.
              if( ncvt>0_ilp64 )call stdlib_I64_dlasr( 'L', 'V', 'F', np1, ncvt, work( 1_ilp64 ),work( np1 ), vt, &
                        ldvt )
           end if
           ! if matrix lower bidiagonal, rotate to be upper bidiagonal
           ! by applying givens rotations on the left.
           if( iuplo==2_ilp64 ) then
              ...
              if( nru>0_ilp64 ) then
                 if( sqre1==0_ilp64 ) then
                    call stdlib_I64_dlasr( 'R', 'V', 'F', nru, n, work( 1_ilp64 ),work( np1 ), u, ldu )
                              
                 else
                    call stdlib_I64_dlasr( 'R', 'V', 'F', nru, np1, work( 1_ilp64 ),work( np1 ), u, ldu )
                              
                 end if
              end if
              if( ncc>0_ilp64 ) then
                 if( sqre1==0_ilp64 ) then
                    call stdlib_I64_dlasr( 'L', 'V', 'F', n, ncc, work( 1_ilp64 ),work( np1 ), c, ldc )
                              
                 else
                    call stdlib_I64_dlasr( 'L', 'V', 'F', np1, ncc, work( 1_ilp64 ),work( np1 ), c, ldc )
                              
                 end if
              end if
           end if
           ! call stdlib_I64_dbdsqr to compute the svd of the reduced real
           ! n-by-n upper bidiagonal matrix.
           call stdlib_I64_dbdsqr( 'U', n, ncvt, nru, ncc, d, e, vt, ldvt, u, ldu, c,ldc, work, info )
           ...
           do i = 1, n
              ...
              if( isub/=i ) then
                 ...
                 d( isub ) = d( i )
                 d( i ) = smin
                 if( ncvt>0_ilp64 )call stdlib_I64_dswap( ncvt, vt( isub, 1_ilp64 ), ldvt, vt( i, 1_ilp64 ), ldvt )
                           
                 if( nru>0_ilp64 )call stdlib_I64_dswap( nru, u( 1_ilp64, isub ), 1_ilp64, u( 1_ilp64, i ), 1_ilp64 )
                 if( ncc>0_ilp64 )call stdlib_I64_dswap( ncc, c( isub, 1_ilp64 ), ldc, c( i, 1_ilp64 ), ldc )
              end if
           end do
           return
     ...
     !! the singular vectors in explicit form.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           integer(ilp64) :: i, i1, ic, idxq, idxqi, im1, inode, itemp, iwk, j, lf, ll, lvl, lvl2, &
           m, ncc, nd, ndb1, ndiml, ndimr, nl, nlf, nlp1, nlvl, nr, nrf, nrp1, nru, nwork1, &
                     nwork2, smlszp, sqrei, vf, vfi, vl, vli
           real(sp) :: alpha, beta
           ! Executable Statements 
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SLASDA', -info )
              return
           end if
           m = n + sqre
           ! if the input matrix is too small, call stdlib_I64_slasdq to find the svd.
           if( n<=smlsiz ) then
              if( icompq==0_ilp64 ) then
                 call stdlib_I64_slasdq( 'U', sqre, n, 0_ilp64, 0_ilp64, 0_ilp64, d, e, vt, ldu, u, ldu,u, ldu, work, &
                           info )
              else
                 call stdlib_I64_slasdq( 'U', sqre, n, m, n, 0_ilp64, d, e, vt, ldu, u, ldu,u, ldu, work, &
                           info )
              end if
              return
           end if
           ...
           loop_30: do i = ndb1, nd
              ...
              nlf = ic - nl
              nrf = ic + 1_ilp64
              idxqi = idxq + nlf - 2_ilp64
              vfi = vf + nlf - 1_ilp64
              vli = vl + nlf - 1_ilp64
              sqrei = 1_ilp64
              if( icompq==0_ilp64 ) then
                 call stdlib_I64_slaset( 'A', nlp1, nlp1, zero, one, work( nwork1 ),smlszp )
                 call stdlib_I64_slasdq( 'U', sqrei, nl, nlp1, nru, ncc, d( nlf ),e( nlf ), work( &
                 nwork1 ), smlszp,work( nwork2 ), nl, work( nwork2 ), nl,work( nwork2 ), info )
                           
                 itemp = nwork1 + nl*smlszp
                 call stdlib_I64_scopy( nlp1, work( nwork1 ), 1_ilp64, work( vfi ), 1_ilp64 )
                 call stdlib_I64_scopy( nlp1, work( itemp ), 1_ilp64, work( vli ), 1_ilp64 )
              else
                 call stdlib_I64_slaset( 'A', nl, nl, zero, one, u( nlf, 1_ilp64 ), ldu )
                 call stdlib_I64_slaset( 'A', nlp1, nlp1, zero, one, vt( nlf, 1_ilp64 ), ldu )
                 call stdlib_I64_slasdq( 'U', sqrei, nl, nlp1, nl, ncc, d( nlf ),e( nlf ), vt( nlf, 1_ilp64 &
                           ), ldu, u( nlf, 1_ilp64 ), ldu,u( nlf, 1_ilp64 ), ldu, work( nwork1 ), info )
                 call stdlib_I64_scopy( nlp1, vt( nlf, 1_ilp64 ), 1_ilp64, work( vfi ), 1_ilp64 )
                 call stdlib_I64_scopy( nlp1, vt( nlf, nlp1 ), 1_ilp64, work( vli ), 1_ilp64 )
              end if
              if( info/=0_ilp64 ) then
                 return
              end if
              ...
              nrp1 = nr + sqrei
              if( icompq==0_ilp64 ) then
                 call stdlib_I64_slaset( 'A', nrp1, nrp1, zero, one, work( nwork1 ),smlszp )
                 call stdlib_I64_slasdq( 'U', sqrei, nr, nrp1, nru, ncc, d( nrf ),e( nrf ), work( &
                 nwork1 ), smlszp,work( nwork2 ), nr, work( nwork2 ), nr,work( nwork2 ), info )
                           
                 itemp = nwork1 + ( nrp1-1 )*smlszp
                 call stdlib_I64_scopy( nrp1, work( nwork1 ), 1_ilp64, work( vfi ), 1_ilp64 )
                 call stdlib_I64_scopy( nrp1, work( itemp ), 1_ilp64, work( vli ), 1_ilp64 )
              else
                 call stdlib_I64_slaset( 'A', nr, nr, zero, one, u( nrf, 1_ilp64 ), ldu )
                 call stdlib_I64_slaset( 'A', nrp1, nrp1, zero, one, vt( nrf, 1_ilp64 ), ldu )
                 call stdlib_I64_slasdq( 'U', sqrei, nr, nrp1, nr, ncc, d( nrf ),e( nrf ), vt( nrf, 1_ilp64 &
                           ), ldu, u( nrf, 1_ilp64 ), ldu,u( nrf, 1_ilp64 ), ldu, work( nwork1 ), info )
                 call stdlib_I64_scopy( nrp1, vt( nrf, 1_ilp64 ), 1_ilp64, work( vfi ), 1_ilp64 )
                 call stdlib_I64_scopy( nrp1, vt( nrf, nrp1 ), 1_ilp64, work( vli ), 1_ilp64 )
              end if
              if( info/=0_ilp64 ) then
                 return
              end if
           ...
           loop_50: do lvl = nlvl, 1, -1
              ...
              loop_40: do i = lf, ll
                 ...
                 vfi = vf + nlf - 1_ilp64
                 vli = vl + nlf - 1_ilp64
                 idxqi = idxq + nlf - 1_ilp64
                 alpha = d( ic )
                 beta = e( ic )
                 if( icompq==0_ilp64 ) then
                    call stdlib_I64_slasd6( icompq, nl, nr, sqrei, d( nlf ),work( vfi ), work( vli ), &
                    alpha, beta,iwork( idxqi ), perm, givptr( 1_ilp64 ), givcol,ldgcol, givnum, ldu, &
                    poles, difl, difr, z,k( 1_ilp64 ), c( 1_ilp64 ), s( 1_ilp64 ), work( nwork1 ),iwork( iwk ), &
                              info )
                 else
                    j = j - 1_ilp64
                    call stdlib_I64_slasd6( icompq, nl, nr, sqrei, d( nlf ),work( vfi ), work( vli ), &
     ...
     !! the singular vectors in explicit form.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DLASDA', -info )
              return
           end if
           m = n + sqre
           ! if the input matrix is too small, call stdlib_I64_dlasdq to find the svd.
           if( n<=smlsiz ) then
              if( icompq==0_ilp64 ) then
                 call stdlib_I64_dlasdq( 'U', sqre, n, 0_ilp64, 0_ilp64, 0_ilp64, d, e, vt, ldu, u, ldu,u, ldu, work, &
                           info )
              else
                 call stdlib_I64_dlasdq( 'U', sqre, n, m, n, 0_ilp64, d, e, vt, ldu, u, ldu,u, ldu, work, &
                           info )
              end if
              return
           end if
           ...
           loop_30: do i = ndb1, nd
              ...
              sqrei = 1_ilp64
              if( icompq==0_ilp64 ) then
                 call stdlib_I64_dlaset( 'A', nlp1, nlp1, zero, one, work( nwork1 ),smlszp )
                 call stdlib_I64_dlasdq( 'U', sqrei, nl, nlp1, nru, ncc, d( nlf ),e( nlf ), work( &
                 nwork1 ), smlszp,work( nwork2 ), nl, work( nwork2 ), nl,work( nwork2 ), info )
                           
                 itemp = nwork1 + nl*smlszp
                 call stdlib_I64_dcopy( nlp1, work( nwork1 ), 1_ilp64, work( vfi ), 1_ilp64 )
                 call stdlib_I64_dcopy( nlp1, work( itemp ), 1_ilp64, work( vli ), 1_ilp64 )
              else
                 call stdlib_I64_dlaset( 'A', nl, nl, zero, one, u( nlf, 1_ilp64 ), ldu )
                 call stdlib_I64_dlaset( 'A', nlp1, nlp1, zero, one, vt( nlf, 1_ilp64 ), ldu )
                 call stdlib_I64_dlasdq( 'U', sqrei, nl, nlp1, nl, ncc, d( nlf ),e( nlf ), vt( nlf, 1_ilp64 &
                           ), ldu, u( nlf, 1_ilp64 ), ldu,u( nlf, 1_ilp64 ), ldu, work( nwork1 ), info )
                 call stdlib_I64_dcopy( nlp1, vt( nlf, 1_ilp64 ), 1_ilp64, work( vfi ), 1_ilp64 )
                 call stdlib_I64_dcopy( nlp1, vt( nlf, nlp1 ), 1_ilp64, work( vli ), 1_ilp64 )
              end if
              if( info/=0_ilp64 ) then
                 return
              end if
              ...
              nrp1 = nr + sqrei
              if( icompq==0_ilp64 ) then
                 call stdlib_I64_dlaset( 'A', nrp1, nrp1, zero, one, work( nwork1 ),smlszp )
                 call stdlib_I64_dlasdq( 'U', sqrei, nr, nrp1, nru, ncc, d( nrf ),e( nrf ), work( &
                 nwork1 ), smlszp,work( nwork2 ), nr, work( nwork2 ), nr,work( nwork2 ), info )
                           
                 itemp = nwork1 + ( nrp1-1 )*smlszp
                 call stdlib_I64_dcopy( nrp1, work( nwork1 ), 1_ilp64, work( vfi ), 1_ilp64 )
                 call stdlib_I64_dcopy( nrp1, work( itemp ), 1_ilp64, work( vli ), 1_ilp64 )
              else
                 call stdlib_I64_dlaset( 'A', nr, nr, zero, one, u( nrf, 1_ilp64 ), ldu )
                 call stdlib_I64_dlaset( 'A', nrp1, nrp1, zero, one, vt( nrf, 1_ilp64 ), ldu )
                 call stdlib_I64_dlasdq( 'U', sqrei, nr, nrp1, nr, ncc, d( nrf ),e( nrf ), vt( nrf, 1_ilp64 &
                           ), ldu, u( nrf, 1_ilp64 ), ldu,u( nrf, 1_ilp64 ), ldu, work( nwork1 ), info )
                 call stdlib_I64_dcopy( nrp1, vt( nrf, 1_ilp64 ), 1_ilp64, work( vfi ), 1_ilp64 )
                 call stdlib_I64_dcopy( nrp1, vt( nrf, nrp1 ), 1_ilp64, work( vli ), 1_ilp64 )
              end if
              if( info/=0_ilp64 ) then
                 return
              end if
           ...
           loop_50: do lvl = nlvl, 1, -1
              ...
              loop_40: do i = lf, ll
                 ...
                 beta = e( ic )
                 if( icompq==0_ilp64 ) then
                    call stdlib_I64_dlasd6( icompq, nl, nr, sqrei, d( nlf ),work( vfi ), work( vli ), &
                    alpha, beta,iwork( idxqi ), perm, givptr( 1_ilp64 ), givcol,ldgcol, givnum, ldu, &
                    poles, difl, difr, z,k( 1_ilp64 ), c( 1_ilp64 ), s( 1_ilp64 ), work( nwork1 ),iwork( iwk ), &
                              info )
                 else
     ...
     givptr, givcol, ldgcol, givnum,ldgnum, poles, difl, difr, z, k, c, s, work,iwork, info )
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           integer(ilp64) :: i, idx, idxc, idxp, isigma, ivfw, ivlw, iw, m, n, n1, n2
           real(sp) :: orgnrm
           ! Intrinsic Functions 
           ! Executable Statements 
           ! test the input parameters.
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SLASD6', -info )
              return
           end if
           ! the following values are for bookkeeping purposes only.  they are
           ! integer pointers which indicate the portion of the workspace
           ! used by a particular array in stdlib_I64_slasd7 and stdlib_I64_slasd8.
           isigma = 1_ilp64
           iw = isigma + n
           ivfw = iw + m
           ivlw = ivfw + m
           ...
           beta = beta / orgnrm
           ! sort and deflate singular values.
           call stdlib_I64_slasd7( icompq, nl, nr, sqre, k, d, z, work( iw ), vf,work( ivfw ), vl, &
           work( ivlw ), alpha, beta,work( isigma ), iwork( idx ), iwork( idxp ), idxq,perm, &
                     givptr, givcol, ldgcol, givnum, ldgnum, c, s,info )
           ! solve secular equation, compute difl, difr, and update vf, vl.
           call stdlib_I64_slasd8( icompq, k, d, z, vf, vl, difl, difr, ldgnum,work( isigma ), work( &
                     iw ), info )
           ! report the possible convergence failure.
           if( info/=0_ilp64 ) then
              return
           ...
           if( icompq==1_ilp64 ) then
              call stdlib_I64_scopy( k, d, 1_ilp64, poles( 1_ilp64, 1_ilp64 ), 1_ilp64 )
              call stdlib_I64_scopy( k, work( isigma ), 1_ilp64, poles( 1_ilp64, 2_ilp64 ), 1_ilp64 )
           end if
           ! unscale.
           call stdlib_I64_slascl( 'G', 0_ilp64, 0_ilp64, one, orgnrm, n, 1_ilp64, d, n, info )
           ! prepare the idxq sorting permutation.
     ...
     givptr, givcol, ldgcol, givnum,ldgnum, poles, difl, difr, z, k, c, s, work,iwork, info )
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DLASD6', -info )
              return
           end if
           ! the following values are for bookkeeping purposes only.  they are
           ! integer pointers which indicate the portion of the workspace
           ! used by a particular array in stdlib_I64_dlasd7 and stdlib_I64_dlasd8.
           isigma = 1_ilp64
           iw = isigma + n
           ivfw = iw + m
           ivlw = ivfw + m
           ...
           beta = beta / orgnrm
           ! sort and deflate singular values.
           call stdlib_I64_dlasd7( icompq, nl, nr, sqre, k, d, z, work( iw ), vf,work( ivfw ), vl, &
           work( ivlw ), alpha, beta,work( isigma ), iwork( idx ), iwork( idxp ), idxq,perm, &
                     givptr, givcol, ldgcol, givnum, ldgnum, c, s,info )
           ! solve secular equation, compute difl, difr, and update vf, vl.
           call stdlib_I64_dlasd8( icompq, k, d, z, vf, vl, difl, difr, ldgnum,work( isigma ), work( &
                     iw ), info )
           ! report the possible convergence failure.
           if( info/=0_ilp64 ) then
              return
           ...
           ! save the poles if icompq = 1.
           if( icompq==1_ilp64 ) then
              call stdlib_I64_dcopy( k, d, 1_ilp64, poles( 1_ilp64, 1_ilp64 ), 1_ilp64 )
              call stdlib_I64_dcopy( k, work( isigma ), 1_ilp64, poles( 1_ilp64, 2_ilp64 ), 1_ilp64 )
           end if
           ! unscale.
           call stdlib_I64_dlascl( 'G', 0_ilp64, 0_ilp64, one, orgnrm, n, 1_ilp64, d, n, info )
           ! prepare the idxq sorting permutation.
     ...
     beta, dsigma, idx, idxp, idxq,perm, givptr, givcol, ldgcol, givnum, ldgnum,c, s, info )
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           integer(ilp64) :: i, idxi, idxj, idxjp, j, jp, jprev, k2, m, n, nlp1, nlp2
           real(sp) :: eps, hlftol, tau, tol, z1
           ! Intrinsic Functions 
           ! Executable Statements 
           ! test the input parameters.
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SLASD7', -info )
              return
           end if
           nlp1 = nl + 1_ilp64
           nlp2 = nl + 2_ilp64
           ...
           if( abs( z( j ) )<=tol ) then
              ...
              if( abs( d( j )-d( jprev ) )<=tol ) then
                 ...
                 if( icompq==1_ilp64 ) then
                    ...
                    givcol( givptr, 2_ilp64 ) = idxjp
                    givcol( givptr, 1_ilp64 ) = idxj
                    givnum( givptr, 2_ilp64 ) = c
                    givnum( givptr, 1_ilp64 ) = s
                 end if
                 call stdlib_I64_srot( 1_ilp64, vf( jprev ), 1_ilp64, vf( j ), 1_ilp64, c, s )
                 call stdlib_I64_srot( 1_ilp64, vl( jprev ), 1_ilp64, vl( j ), 1_ilp64, c, s )
                 k2 = k2 - 1_ilp64
                 idxp( k2 ) = jprev
                 jprev = j
              else
           ...
           if( icompq==1_ilp64 ) then
              do j = 2, n
                 jp = idxp( j )
                 perm( j ) = idxq( idx( jp )+1_ilp64 )
                 if( perm( j )<=nlp1 ) then
                    perm( j ) = perm( j ) - 1_ilp64
                 end if
              end do
           end if
           ! the deflated singular values go back into the last n - k slots of
           ...
           if( m>n ) then
              ...
              call stdlib_I64_srot( 1_ilp64, vf( m ), 1_ilp64, vf( 1_ilp64 ), 1_ilp64, c, s )
              call stdlib_I64_srot( 1_ilp64, vl( m ), 1_ilp64, vl( 1_ilp64 ), 1_ilp64, c, s )
           else
              if( abs( z1 )<=tol ) then
                 z( 1_ilp64 ) = tol
              else
           ...
           ! restore z, vf, and vl.
           call stdlib_I64_scopy( k-1, zw( 2_ilp64 ), 1_ilp64, z( 2_ilp64 ), 1_ilp64 )
           call stdlib_I64_scopy( n-1, vfw( 2_ilp64 ), 1_ilp64, vf( 2_ilp64 ), 1_ilp64 )
           call stdlib_I64_scopy( n-1, vlw( 2_ilp64 ), 1_ilp64, vl( 2_ilp64 ), 1_ilp64 )
           return
     ...
     beta, dsigma, idx, idxp, idxq,perm, givptr, givcol, ldgcol, givnum, ldgnum,c, s, info )
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DLASD7', -info )
              return
           end if
           nlp1 = nl + 1_ilp64
           nlp2 = nl + 2_ilp64
           ...
           if( abs( z( j ) )<=tol ) then
              ...
              if( abs( d( j )-d( jprev ) )<=tol ) then
                 ...
                 call stdlib_I64_drot( 1_ilp64, vf( jprev ), 1_ilp64, vf( j ), 1_ilp64, c, s )
                 call stdlib_I64_drot( 1_ilp64, vl( jprev ), 1_ilp64, vl( j ), 1_ilp64, c, s )
                 k2 = k2 - 1_ilp64
                 idxp( k2 ) = jprev
                 jprev = j
              else
           ...
           if( m>n ) then
              ...
              call stdlib_I64_drot( 1_ilp64, vf( m ), 1_ilp64, vf( 1_ilp64 ), 1_ilp64, c, s )
              call stdlib_I64_drot( 1_ilp64, vl( m ), 1_ilp64, vl( 1_ilp64 ), 1_ilp64, c, s )
           else
              if( abs( z1 )<=tol ) then
                 z( 1_ilp64 ) = tol
              else
           ...
           ! restore z, vf, and vl.
           call stdlib_I64_dcopy( k-1, zw( 2_ilp64 ), 1_ilp64, z( 2_ilp64 ), 1_ilp64 )
           call stdlib_I64_dcopy( n-1, vfw( 2_ilp64 ), 1_ilp64, vf( 2_ilp64 ), 1_ilp64 )
           call stdlib_I64_dcopy( n-1, vlw( 2_ilp64 ), 1_ilp64, vl( 2_ilp64 ), 1_ilp64 )
           return
     ...
     !! SLASD8 is called from SLASD6.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           integer(ilp64) :: i, iwk1, iwk2, iwk2i, iwk3, iwk3i, j
           real(sp) :: diflj, difrj, dj, dsigj, dsigjp, rho, temp
           ! Intrinsic Functions 
           ! Executable Statements 
           ! test the input parameters.
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SLASD8', -info )
              return
           end if
           ! quick return if possible
           if( k==1_ilp64 ) then
              d( 1_ilp64 ) = abs( z( 1_ilp64 ) )
              difl( 1_ilp64 ) = d( 1_ilp64 )
              if( icompq==1_ilp64 ) then
                 difl( 2_ilp64 ) = one
                 difr( 1_ilp64, 2_ilp64 ) = one
              end if
              return
           end if
           ! modify values dsigma(i) to make sure all dsigma(i)-dsigma(j) can
           ...
           rho = rho*rho
           ! initialize work(iwk3).
           call stdlib_I64_slaset( 'A', k, 1_ilp64, one, one, work( iwk3 ), k )
           ! compute the updated singular values, the arrays difl, difr,
           ! and the updated z.
           do j = 1, k
              call stdlib_I64_slasd4( k, j, dsigma, z, work( iwk1 ), rho, d( j ),work( iwk2 ), info )
           ...
           do j = 1, k
              ...
              work( j ) = -z( j ) / diflj / ( dsigma( j )+dj )
              do i = 1, j - 1
                 work( i ) = z( i ) / ( stdlib_I64_slamc3( dsigma( i ), dsigj )-diflj )/ ( dsigma( i )&
                           +dj )
              end do
              do i = j + 1, k
                 work( i ) = z( i ) / ( stdlib_I64_slamc3( dsigma( i ), dsigjp )+difrj )/ ( dsigma( i &
                           )+dj )
              end do
              temp = stdlib_I64_snrm2( k, work, 1_ilp64 )
              work( iwk2i+j ) = stdlib_I64_sdot( k, work, 1_ilp64, vf, 1_ilp64 ) / temp
              work( iwk3i+j ) = stdlib_I64_sdot( k, work, 1_ilp64, vl, 1_ilp64 ) / temp
              if( icompq==1_ilp64 ) then
                 difr( j, 2_ilp64 ) = temp
              end if
           end do
           call stdlib_I64_scopy( k, work( iwk2 ), 1_ilp64, vf, 1_ilp64 )
           call stdlib_I64_scopy( k, work( iwk3 ), 1_ilp64, vl, 1_ilp64 )
           return
     ...
     !! DLASD8 is called from DLASD6.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DLASD8', -info )
              return
           end if
           ! quick return if possible
           if( k==1_ilp64 ) then
           ...
           rho = rho*rho
           ! initialize work(iwk3).
           call stdlib_I64_dlaset( 'A', k, 1_ilp64, one, one, work( iwk3 ), k )
           ! compute the updated singular values, the arrays difl, difr,
           ! and the updated z.
           do j = 1, k
              call stdlib_I64_dlasd4( k, j, dsigma, z, work( iwk1 ), rho, d( j ),work( iwk2 ), info )
           ...
           do j = 1, k
              ...
              work( j ) = -z( j ) / diflj / ( dsigma( j )+dj )
              do i = 1, j - 1
                 work( i ) = z( i ) / ( stdlib_I64_dlamc3( dsigma( i ), dsigj )-diflj )/ ( dsigma( i )&
                           +dj )
              end do
              do i = j + 1, k
                 work( i ) = z( i ) / ( stdlib_I64_dlamc3( dsigma( i ), dsigjp )+difrj )/ ( dsigma( i &
                           )+dj )
              end do
              temp = stdlib_I64_dnrm2( k, work, 1_ilp64 )
              work( iwk2i+j ) = stdlib_I64_ddot( k, work, 1_ilp64, vf, 1_ilp64 ) / temp
              work( iwk3i+j ) = stdlib_I64_ddot( k, work, 1_ilp64, vl, 1_ilp64 ) / temp
              if( icompq==1_ilp64 ) then
                 difr( j, 2_ilp64 ) = temp
              end if
           end do
           call stdlib_I64_dcopy( k, work( iwk2 ), 1_ilp64, vf, 1_ilp64 )
           call stdlib_I64_dcopy( k, work( iwk3 ), 1_ilp64, vl, 1_ilp64 )
           return
