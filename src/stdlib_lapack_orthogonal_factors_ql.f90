  ...
  contains
     ...
     !! 0 is a M-by-(N-M) zero matrix, if M < N.
        ...
        ! =====================================================================
           ! Local Scalars 
           logical(lk) :: lquery, lminws, mint, minw
           integer(ilp64) :: mb, nb, mintsz, nblcks, lwmin, lwopt, lwreq
           ! Intrinsic Functions 
           ! Executable Statements 
           ! test the input arguments
           info = 0_ilp64
           lquery = ( tsize==-1_ilp64 .or. tsize==-2_ilp64 .or.lwork==-1_ilp64 .or. lwork==-2_ilp64 )
           mint = .false.
           minw = .false.
           if( tsize==-2_ilp64 .or. lwork==-2_ilp64 ) then
             if( tsize/=-1_ilp64 ) mint = .true.
             if( lwork/=-1_ilp64 ) minw = .true.
           end if
           ! determine the block size
           if( min( m, n )>0_ilp64 ) then
             mb = stdlib_I64_ilaenv( 1_ilp64, 'SGELQ ', ' ', m, n, 1_ilp64, -1_ilp64 )
             nb = stdlib_I64_ilaenv( 1_ilp64, 'SGELQ ', ' ', m, n, 2_ilp64, -1_ilp64 )
           else
             mb = 1_ilp64
             nb = n
           end if
           if( mb>min( m, n ) .or. mb<1_ilp64 ) mb = 1_ilp64
           if( nb>n .or. nb<=m ) nb = n
           mintsz = m + 5_ilp64
           if ( nb>m .and. n>m ) then
             if( mod( n - m, nb - m )==0_ilp64 ) then
               nblcks = ( n - m ) / ( nb - m )
             else
               nblcks = ( n - m ) / ( nb - m ) + 1_ilp64
             end if
           else
             nblcks = 1_ilp64
           end if
           ...
           if( ( n<=m ) .or. ( nb<=m ) .or. ( nb>=n ) ) then
              lwmin = max( 1_ilp64, n )
              lwopt = max( 1_ilp64, mb*n )
           else
              lwmin = max( 1_ilp64, m )
              lwopt = max( 1_ilp64, mb*m )
           end if
           lminws = .false.
           if( ( tsize<max( 1_ilp64, mb*m*nblcks + 5_ilp64 ) .or. lwork<lwopt ).and. ( lwork>=lwmin ) .and. ( &
                     tsize>=mintsz ).and. ( .not.lquery ) ) then
             if( tsize<max( 1_ilp64, mb*m*nblcks + 5_ilp64 ) ) then
               lminws = .true.
               mb = 1_ilp64
               nb = n
             end if
           ...
           if( ( n<=m ) .or. ( nb<=m ) .or. ( nb>=n ) ) then
              lwreq = max( 1_ilp64, mb*n )
           else
              lwreq = max( 1_ilp64, mb*m )
           end if
           if( m<0_ilp64 ) then
             info = -1_ilp64
             ...
             info = -4_ilp64
           else if( tsize<max( 1_ilp64, mb*m*nblcks + 5_ilp64 ).and. ( .not.lquery ) .and. ( .not.lminws ) ) &
                     then
             info = -6_ilp64
           else if( ( lwork<lwreq ) .and.( .not.lquery ).and. ( .not.lminws ) ) then
             info = -8_ilp64
           ...
           if( info/=0_ilp64 ) then
             call stdlib_I64_xerbla( 'SGELQ', -info )
             return
           else if( lquery ) then
             return
           end if
           ...
           ! the lq decomposition
           if( ( n<=m ) .or. ( nb<=m ) .or. ( nb>=n ) ) then
             call stdlib_I64_sgelqt( m, n, mb, a, lda, t( 6_ilp64 ), mb, work, info )
           else
             call stdlib_I64_slaswlq( m, n, mb, nb, a, lda, t( 6_ilp64 ), mb, work,lwork, info )
           end if
           work( 1_ilp64 ) = lwreq
           return
     ...
     !! 0 is a M-by-(N-M) zero matrix, if M < N.
        ...
        ! =====================================================================
           ...
           ! determine the block size
           if( min( m, n )>0_ilp64 ) then
             mb = stdlib_I64_ilaenv( 1_ilp64, 'DGELQ ', ' ', m, n, 1_ilp64, -1_ilp64 )
             nb = stdlib_I64_ilaenv( 1_ilp64, 'DGELQ ', ' ', m, n, 2_ilp64, -1_ilp64 )
           else
             mb = 1_ilp64
             nb = n
           end if
           ...
           if( info/=0_ilp64 ) then
             call stdlib_I64_xerbla( 'DGELQ', -info )
             return
           else if( lquery ) then
             return
           end if
           ...
           ! the lq decomposition
           if( ( n<=m ) .or. ( nb<=m ) .or. ( nb>=n ) ) then
             call stdlib_I64_dgelqt( m, n, mb, a, lda, t( 6_ilp64 ), mb, work, info )
           else
             call stdlib_I64_dlaswlq( m, n, mb, nb, a, lda, t( 6_ilp64 ), mb, work,lwork, info )
           end if
           work( 1_ilp64 ) = lwreq
           return
     ...
     !! 0 is a M-by-(N-M) zero matrix, if M < N.
        ...
        ! =====================================================================
           ...
           ! determine the block size
           if( min( m, n )>0_ilp64 ) then
             mb = stdlib_I64_ilaenv( 1_ilp64, 'CGELQ ', ' ', m, n, 1_ilp64, -1_ilp64 )
             nb = stdlib_I64_ilaenv( 1_ilp64, 'CGELQ ', ' ', m, n, 2_ilp64, -1_ilp64 )
           else
             mb = 1_ilp64
             nb = n
           end if
           ...
           if( info/=0_ilp64 ) then
             call stdlib_I64_xerbla( 'CGELQ', -info )
             return
           else if( lquery ) then
             return
           end if
           ...
           ! the lq decomposition
           if( ( n<=m ) .or. ( nb<=m ) .or. ( nb>=n ) ) then
             call stdlib_I64_cgelqt( m, n, mb, a, lda, t( 6_ilp64 ), mb, work, info )
           else
             call stdlib_I64_claswlq( m, n, mb, nb, a, lda, t( 6_ilp64 ), mb, work,lwork, info )
           end if
           work( 1_ilp64 ) = lwreq
           return
     ...
     !! 0 is a M-by-(N-M) zero matrix, if M < N.
        ...
        ! =====================================================================
           ...
           ! determine the block size
           if( min( m, n )>0_ilp64 ) then
             mb = stdlib_I64_ilaenv( 1_ilp64, 'ZGELQ ', ' ', m, n, 1_ilp64, -1_ilp64 )
             nb = stdlib_I64_ilaenv( 1_ilp64, 'ZGELQ ', ' ', m, n, 2_ilp64, -1_ilp64 )
           else
             mb = 1_ilp64
             nb = n
           end if
           ...
           if( info/=0_ilp64 ) then
             call stdlib_I64_xerbla( 'ZGELQ', -info )
             return
           else if( lquery ) then
             return
           end if
           ...
           ! the lq decomposition
           if( ( n<=m ) .or. ( nb<=m ) .or. ( nb>=n ) ) then
             call stdlib_I64_zgelqt( m, n, mb, a, lda, t( 6_ilp64 ), mb, work, info )
           else
             call stdlib_I64_zlaswlq( m, n, mb, nb, a, lda, t( 6_ilp64 ), mb, work,lwork, info )
           end if
           work( 1_ilp64 ) = lwreq
           return
     ...
     !! factorization (SGELQ)
       ...
       ! =====================================================================
           ! Local Scalars 
           logical(lk) :: left, right, tran, notran, lquery
           integer(ilp64) :: mb, nb, lw, nblcks, mn
           ! Intrinsic Functions 
           ! Executable Statements 
           ! test the input arguments
           ...
           left    = stdlib_lsame( side, 'L' )
           right   = stdlib_lsame( side, 'R' )
           mb = int( t( 2_ilp64 ),KIND=ilp64)
           nb = int( t( 3_ilp64 ),KIND=ilp64)
           if( left ) then
             lw = n * mb
             mn = m
           else
           ...
           if( ( nb>k ) .and. ( mn>k ) ) then
             if( mod( mn - k, nb - k ) == 0_ilp64 ) then
               nblcks = ( mn - k ) / ( nb - k )
             else
               nblcks = ( mn - k ) / ( nb - k ) + 1_ilp64
             end if
           else
             nblcks = 1_ilp64
           end if
           ...
           if( .not.left .and. .not.right ) then
             ...
             info = -4_ilp64
           else if( k<0_ilp64 .or. k>mn ) then
             info = -5_ilp64
           else if( lda<max( 1_ilp64, k ) ) then
             info = -7_ilp64
           else if( tsize<5_ilp64 ) then
             info = -9_ilp64
           else if( ldc<max( 1_ilp64, m ) ) then
             info = -11_ilp64
           else if( ( lwork<max( 1_ilp64, lw ) ) .and. ( .not.lquery ) ) then
             info = -13_ilp64
           end if
           if( info==0_ilp64 ) then
             work( 1_ilp64 ) = real( lw,KIND=sp)
           end if
           if( info/=0_ilp64 ) then
             call stdlib_I64_xerbla( 'SGEMLQ', -info )
             return
           else if( lquery ) then
             return
           end if
           ! quick return if possible
           if( min( m, n, k )==0_ilp64 ) then
             return
           end if
           if( ( left .and. m<=k ) .or. ( right .and. n<=k ).or. ( nb<=k ) .or. ( nb>=max( m, n, &
                     k ) ) ) then
             call stdlib_I64_sgemlqt( side, trans, m, n, k, mb, a, lda,t( 6_ilp64 ), mb, c, ldc, work, info &
                       )
           else
             call stdlib_I64_slamswlq( side, trans, m, n, k, mb, nb, a, lda, t( 6_ilp64 ),mb, c, ldc, work, &
                       lwork, info )
           end if
           work( 1_ilp64 ) = real( lw,KIND=sp)
           return
     ...
     !! factorization (DGELQ)
       ...
       ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
             call stdlib_I64_xerbla( 'DGEMLQ', -info )
             return
           else if( lquery ) then
             return
           end if
           ...
           if( ( left .and. m<=k ) .or. ( right .and. n<=k ).or. ( nb<=k ) .or. ( nb>=max( m, n, &
                     k ) ) ) then
             call stdlib_I64_dgemlqt( side, trans, m, n, k, mb, a, lda,t( 6_ilp64 ), mb, c, ldc, work, info &
                       )
           else
             call stdlib_I64_dlamswlq( side, trans, m, n, k, mb, nb, a, lda, t( 6_ilp64 ),mb, c, ldc, work, &
                       lwork, info )
           end if
           work( 1_ilp64 ) = lw
           return
     ...
     !! LQ factorization (CGELQ)
       ...
       ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
             call stdlib_I64_xerbla( 'CGEMLQ', -info )
             return
           else if( lquery ) then
             return
           end if
           ...
           if( ( left .and. m<=k ) .or. ( right .and. n<=k ).or. ( nb<=k ) .or. ( nb>=max( m, n, &
                     k ) ) ) then
             call stdlib_I64_cgemlqt( side, trans, m, n, k, mb, a, lda,t( 6_ilp64 ), mb, c, ldc, work, info &
                       )
           else
             call stdlib_I64_clamswlq( side, trans, m, n, k, mb, nb, a, lda, t( 6_ilp64 ),mb, c, ldc, work, &
                       lwork, info )
           end if
           work( 1_ilp64 ) = real( lw,KIND=sp)
           return
     ...
     !! LQ factorization (ZGELQ)
       ...
       ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
             call stdlib_I64_xerbla( 'ZGEMLQ', -info )
             return
           else if( lquery ) then
             return
           end if
           ...
           if( ( left .and. m<=k ) .or. ( right .and. n<=k ).or. ( nb<=k ) .or. ( nb>=max( m, n, &
                     k ) ) ) then
             call stdlib_I64_zgemlqt( side, trans, m, n, k, mb, a, lda,t( 6_ilp64 ), mb, c, ldc, work, info &
                       )
           else
             call stdlib_I64_zlamswlq( side, trans, m, n, k, mb, nb, a, lda, t( 6_ilp64 ),mb, c, ldc, work, &
                       lwork, info )
           end if
           work( 1_ilp64 ) = lw
           return
     ...
     !! 0 is a M-by-(N-M) zero matrix, if M < N.
        ...
        ! =====================================================================
           ! Local Scalars 
           logical(lk) :: lquery
           integer(ilp64) :: i, ib, iinfo, iws, k, ldwork, lwkopt, nb, nbmin, nx
           ! Intrinsic Functions 
           ! Executable Statements 
           ! test the input arguments
           info = 0_ilp64
           ...
           if( m<0_ilp64 ) then
              ...
              info = -4_ilp64
           else if( lwork<max( 1_ilp64, m ) .and. .not.lquery ) then
              info = -7_ilp64
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SGELQF', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           if( nb>1_ilp64 .and. nb<k ) then
              ! determine when to cross over from blocked to unblocked code.
              nx = max( 0_ilp64, stdlib_I64_ilaenv( 3_ilp64, 'SGELQF', ' ', m, n, -1_ilp64, -1_ilp64 ) )
              if( nx<k ) then
                 ! determine if workspace is large enough for blocked code.
                 ldwork = m
                 iws = ldwork*nb
                 if( lwork<iws ) then
                    ...
                    ! determine the minimum value of nb.
                    nb = lwork / ldwork
                    nbmin = max( 2_ilp64, stdlib_I64_ilaenv( 2_ilp64, 'SGELQF', ' ', m, n, -1_ilp64,-1_ilp64 ) )
                 end if
              end if
           end if
           if( nb>=nbmin .and. nb<k .and. nx<k ) then
              ...
              do i = 1, k - nx, nb
                 ...
                 ! compute the lq factorization of the current block
                 ! a(i:i+ib-1,i:n)
                 call stdlib_I64_sgelq2( ib, n-i+1, a( i, i ), lda, tau( i ), work,iinfo )
                 if( i+ib<=m ) then
                    ! form the triangular factor of the block reflector
                    ! h = h(i) h(i+1) . . . h(i+ib-1)
                    call stdlib_I64_slarft( 'FORWARD', 'ROWWISE', n-i+1, ib, a( i, i ),lda, tau( i ), &
                              work, ldwork )
                    ! apply h to a(i+ib:m,i:n) from the right
                    call stdlib_I64_slarfb( 'RIGHT', 'NO TRANSPOSE', 'FORWARD','ROWWISE', m-i-ib+1, n-&
                    i+1, ib, a( i, i ),lda, work, ldwork, a( i+ib, i ), lda,work( ib+1 ), ldwork )
           ...
           ! use unblocked code to factor the last or only block.
           if( i<=k )call stdlib_I64_sgelq2( m-i+1, n-i+1, a( i, i ), lda, tau( i ), work,iinfo )
     ...
     !! 0 is a M-by-(N-M) zero matrix, if M < N.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DGELQF', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           if( nb>1_ilp64 .and. nb<k ) then
              ! determine when to cross over from blocked to unblocked code.
              nx = max( 0_ilp64, stdlib_I64_ilaenv( 3_ilp64, 'DGELQF', ' ', m, n, -1_ilp64, -1_ilp64 ) )
              if( nx<k ) then
                 ! determine if workspace is large enough for blocked code.
                 ldwork = m
                 iws = ldwork*nb
                 if( lwork<iws ) then
                    ...
                    ! determine the minimum value of nb.
                    nb = lwork / ldwork
                    nbmin = max( 2_ilp64, stdlib_I64_ilaenv( 2_ilp64, 'DGELQF', ' ', m, n, -1_ilp64,-1_ilp64 ) )
                 end if
              end if
           end if
           if( nb>=nbmin .and. nb<k .and. nx<k ) then
              ...
              do i = 1, k - nx, nb
                 ...
                 ! compute the lq factorization of the current block
                 ! a(i:i+ib-1,i:n)
                 call stdlib_I64_dgelq2( ib, n-i+1, a( i, i ), lda, tau( i ), work,iinfo )
                 if( i+ib<=m ) then
                    ! form the triangular factor of the block reflector
                    ! h = h(i) h(i+1) . . . h(i+ib-1)
                    call stdlib_I64_dlarft( 'FORWARD', 'ROWWISE', n-i+1, ib, a( i, i ),lda, tau( i ), &
                              work, ldwork )
                    ! apply h to a(i+ib:m,i:n) from the right
                    call stdlib_I64_dlarfb( 'RIGHT', 'NO TRANSPOSE', 'FORWARD','ROWWISE', m-i-ib+1, n-&
                    i+1, ib, a( i, i ),lda, work, ldwork, a( i+ib, i ), lda,work( ib+1 ), ldwork )
           ...
           ! use unblocked code to factor the last or only block.
           if( i<=k )call stdlib_I64_dgelq2( m-i+1, n-i+1, a( i, i ), lda, tau( i ), work,iinfo )
     ...
     !! 0 is a M-by-(N-M) zero matrix, if M < N.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CGELQF', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           if( nb>1_ilp64 .and. nb<k ) then
              ! determine when to cross over from blocked to unblocked code.
              nx = max( 0_ilp64, stdlib_I64_ilaenv( 3_ilp64, 'CGELQF', ' ', m, n, -1_ilp64, -1_ilp64 ) )
              if( nx<k ) then
                 ! determine if workspace is large enough for blocked code.
                 ldwork = m
                 iws = ldwork*nb
                 if( lwork<iws ) then
                    ...
                    ! determine the minimum value of nb.
                    nb = lwork / ldwork
                    nbmin = max( 2_ilp64, stdlib_I64_ilaenv( 2_ilp64, 'CGELQF', ' ', m, n, -1_ilp64,-1_ilp64 ) )
                 end if
              end if
           end if
           if( nb>=nbmin .and. nb<k .and. nx<k ) then
              ...
              do i = 1, k - nx, nb
                 ...
                 ! compute the lq factorization of the current block
                 ! a(i:i+ib-1,i:n)
                 call stdlib_I64_cgelq2( ib, n-i+1, a( i, i ), lda, tau( i ), work,iinfo )
                 if( i+ib<=m ) then
                    ! form the triangular factor of the block reflector
                    ! h = h(i) h(i+1) . . . h(i+ib-1)
                    call stdlib_I64_clarft( 'FORWARD', 'ROWWISE', n-i+1, ib, a( i, i ),lda, tau( i ), &
                              work, ldwork )
                    ! apply h to a(i+ib:m,i:n) from the right
                    call stdlib_I64_clarfb( 'RIGHT', 'NO TRANSPOSE', 'FORWARD','ROWWISE', m-i-ib+1, n-&
                    i+1, ib, a( i, i ),lda, work, ldwork, a( i+ib, i ), lda,work( ib+1 ), ldwork )
           ...
           ! use unblocked code to factor the last or only block.
           if( i<=k )call stdlib_I64_cgelq2( m-i+1, n-i+1, a( i, i ), lda, tau( i ), work,iinfo )
     ...
     !! 0 is a M-by-(N-M) zero matrix, if M < N.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZGELQF', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           if( nb>1_ilp64 .and. nb<k ) then
              ! determine when to cross over from blocked to unblocked code.
              nx = max( 0_ilp64, stdlib_I64_ilaenv( 3_ilp64, 'ZGELQF', ' ', m, n, -1_ilp64, -1_ilp64 ) )
              if( nx<k ) then
                 ! determine if workspace is large enough for blocked code.
                 ldwork = m
                 iws = ldwork*nb
                 if( lwork<iws ) then
                    ...
                    ! determine the minimum value of nb.
                    nb = lwork / ldwork
                    nbmin = max( 2_ilp64, stdlib_I64_ilaenv( 2_ilp64, 'ZGELQF', ' ', m, n, -1_ilp64,-1_ilp64 ) )
                 end if
              end if
           end if
           if( nb>=nbmin .and. nb<k .and. nx<k ) then
              ...
              do i = 1, k - nx, nb
                 ...
                 ! compute the lq factorization of the current block
                 ! a(i:i+ib-1,i:n)
                 call stdlib_I64_zgelq2( ib, n-i+1, a( i, i ), lda, tau( i ), work,iinfo )
                 if( i+ib<=m ) then
                    ! form the triangular factor of the block reflector
                    ! h = h(i) h(i+1) . . . h(i+ib-1)
                    call stdlib_I64_zlarft( 'FORWARD', 'ROWWISE', n-i+1, ib, a( i, i ),lda, tau( i ), &
                              work, ldwork )
                    ! apply h to a(i+ib:m,i:n) from the right
                    call stdlib_I64_zlarfb( 'RIGHT', 'NO TRANSPOSE', 'FORWARD','ROWWISE', m-i-ib+1, n-&
                    i+1, ib, a( i, i ),lda, work, ldwork, a( i+ib, i ), lda,work( ib+1 ), ldwork )
           ...
           ! use unblocked code to factor the last or only block.
           if( i<=k )call stdlib_I64_zgelq2( m-i+1, n-i+1, a( i, i ), lda, tau( i ), work,iinfo )
     ...
     !! 0 is a m-by-(n-m) zero matrix, if m < n.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SGELQ2', -info )
              return
           end if
           k = min( m, n )
           do i = 1, k
              ! generate elementary reflector h(i) to annihilate a(i,i+1:n)
              call stdlib_I64_slarfg( n-i+1, a( i, i ), a( i, min( i+1, n ) ), lda,tau( i ) )
              if( i<m ) then
                 ! apply h(i) to a(i+1:m,i:n) from the right
                 aii = a( i, i )
                 a( i, i ) = one
                 call stdlib_I64_slarf( 'RIGHT', m-i, n-i+1, a( i, i ), lda, tau( i ),a( i+1, i ), &
                           lda, work )
                 a( i, i ) = aii
              end if
           end do
     ...
     !! 0 is a m-by-(n-m) zero matrix, if m < n.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DGELQ2', -info )
              return
           end if
           k = min( m, n )
           do i = 1, k
              ! generate elementary reflector h(i) to annihilate a(i,i+1:n)
              call stdlib_I64_dlarfg( n-i+1, a( i, i ), a( i, min( i+1, n ) ), lda,tau( i ) )
              if( i<m ) then
                 ! apply h(i) to a(i+1:m,i:n) from the right
                 aii = a( i, i )
                 a( i, i ) = one
                 call stdlib_I64_dlarf( 'RIGHT', m-i, n-i+1, a( i, i ), lda, tau( i ),a( i+1, i ), &
                           lda, work )
                 a( i, i ) = aii
              end if
           end do
     ...
     !! 0 is a m-by-(n-m) zero matrix, if m < n.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CGELQ2', -info )
              return
           end if
           k = min( m, n )
           do i = 1, k
              ! generate elementary reflector h(i) to annihilate a(i,i+1:n)
              call stdlib_I64_clacgv( n-i+1, a( i, i ), lda )
              alpha = a( i, i )
              call stdlib_I64_clarfg( n-i+1, alpha, a( i, min( i+1, n ) ), lda,tau( i ) )
              if( i<m ) then
                 ! apply h(i) to a(i+1:m,i:n) from the right
                 a( i, i ) = cone
                 call stdlib_I64_clarf( 'RIGHT', m-i, n-i+1, a( i, i ), lda, tau( i ),a( i+1, i ), &
                           lda, work )
              end if
              a( i, i ) = alpha
              call stdlib_I64_clacgv( n-i+1, a( i, i ), lda )
     ...
     !! 0 is a m-by-(n-m) zero matrix, if m < n.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZGELQ2', -info )
              return
           end if
           k = min( m, n )
           do i = 1, k
              ! generate elementary reflector h(i) to annihilate a(i,i+1:n)
              call stdlib_I64_zlacgv( n-i+1, a( i, i ), lda )
              alpha = a( i, i )
              call stdlib_I64_zlarfg( n-i+1, alpha, a( i, min( i+1, n ) ), lda,tau( i ) )
              if( i<m ) then
                 ! apply h(i) to a(i+1:m,i:n) from the right
                 a( i, i ) = cone
                 call stdlib_I64_zlarf( 'RIGHT', m-i, n-i+1, a( i, i ), lda, tau( i ),a( i+1, i ), &
                           lda, work )
              end if
              a( i, i ) = alpha
              call stdlib_I64_zlacgv( n-i+1, a( i, i ), lda )
     ...
     !! as returned by CGELQF.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           logical(lk) :: lquery
           integer(ilp64) :: i, ib, iinfo, iws, j, ki, kk, l, ldwork, lwkopt, nb, nbmin, nx
           ! Intrinsic Functions 
           ! Executable Statements 
           ! test the input arguments
           info = 0_ilp64
           nb = stdlib_I64_ilaenv( 1_ilp64, 'CUNGLQ', ' ', m, n, k, -1_ilp64 )
           lwkopt = max( 1_ilp64, m )*nb
           work( 1_ilp64 ) = lwkopt
           lquery = ( lwork==-1_ilp64 )
           if( m<0_ilp64 ) then
              info = -1_ilp64
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CUNGLQ', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           if( nb>1_ilp64 .and. nb<k ) then
              ! determine when to cross over from blocked to unblocked code.
              nx = max( 0_ilp64, stdlib_I64_ilaenv( 3_ilp64, 'CUNGLQ', ' ', m, n, k, -1_ilp64 ) )
              if( nx<k ) then
                 ! determine if workspace is large enough for blocked code.
                 ldwork = m
                 iws = ldwork*nb
                 if( lwork<iws ) then
                    ...
                    ! determine the minimum value of nb.
                    nb = lwork / ldwork
                    nbmin = max( 2_ilp64, stdlib_I64_ilaenv( 2_ilp64, 'CUNGLQ', ' ', m, n, k, -1_ilp64 ) )
                 end if
              end if
           end if
           if( nb>=nbmin .and. nb<k .and. nx<k ) then
           ...
           ! use unblocked code for the last or only block.
           if( kk<m )call stdlib_I64_cungl2( m-kk, n-kk, k-kk, a( kk+1, kk+1 ), lda,tau( kk+1 ), work,&
                      iinfo )
           if( kk>0_ilp64 ) then
              ! use blocked code
              do i = ki + 1, 1, -nb
                 ...
                 if( i+ib<=m ) then
                    ...
                    call stdlib_I64_clarft( 'FORWARD', 'ROWWISE', n-i+1, ib, a( i, i ),lda, tau( i ), &
                              work, ldwork )
                    ! apply h**h to a(i+ib:m,i:n) from the right
                    call stdlib_I64_clarfb( 'RIGHT', 'CONJUGATE TRANSPOSE', 'FORWARD','ROWWISE', m-i-&
                    ib+1, n-i+1, ib, a( i, i ),lda, work, ldwork, a( i+ib, i ), lda,work( ib+1 ), &
                              ldwork )
                 end if
                 ! apply h**h to columns i:n of current block
                 call stdlib_I64_cungl2( ib, n-i+1, ib, a( i, i ), lda, tau( i ), work,iinfo )
                 ! set columns 1:i-1 of current block to czero
                 do j = 1, i - 1
                    do l = i, i + ib - 1
                       a( l, j ) = czero
     ...
     !! as returned by ZGELQF.
        ...
        ! =====================================================================
           ...
           ! test the input arguments
           info = 0_ilp64
           nb = stdlib_I64_ilaenv( 1_ilp64, 'ZUNGLQ', ' ', m, n, k, -1_ilp64 )
           lwkopt = max( 1_ilp64, m )*nb
           work( 1_ilp64 ) = lwkopt
           lquery = ( lwork==-1_ilp64 )
           if( m<0_ilp64 ) then
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZUNGLQ', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           if( nb>1_ilp64 .and. nb<k ) then
              ! determine when to cross over from blocked to unblocked code.
              nx = max( 0_ilp64, stdlib_I64_ilaenv( 3_ilp64, 'ZUNGLQ', ' ', m, n, k, -1_ilp64 ) )
              if( nx<k ) then
                 ! determine if workspace is large enough for blocked code.
                 ldwork = m
                 iws = ldwork*nb
                 if( lwork<iws ) then
                    ...
                    ! determine the minimum value of nb.
                    nb = lwork / ldwork
                    nbmin = max( 2_ilp64, stdlib_I64_ilaenv( 2_ilp64, 'ZUNGLQ', ' ', m, n, k, -1_ilp64 ) )
                 end if
              end if
           end if
           if( nb>=nbmin .and. nb<k .and. nx<k ) then
           ...
           ! use unblocked code for the last or only block.
           if( kk<m )call stdlib_I64_zungl2( m-kk, n-kk, k-kk, a( kk+1, kk+1 ), lda,tau( kk+1 ), work,&
                      iinfo )
           if( kk>0_ilp64 ) then
              ! use blocked code
              do i = ki + 1, 1, -nb
                 ...
                 if( i+ib<=m ) then
                    ...
                    call stdlib_I64_zlarft( 'FORWARD', 'ROWWISE', n-i+1, ib, a( i, i ),lda, tau( i ), &
                              work, ldwork )
                    ! apply h**h to a(i+ib:m,i:n) from the right
                    call stdlib_I64_zlarfb( 'RIGHT', 'CONJUGATE TRANSPOSE', 'FORWARD','ROWWISE', m-i-&
                    ib+1, n-i+1, ib, a( i, i ),lda, work, ldwork, a( i+ib, i ), lda,work( ib+1 ), &
                              ldwork )
                 end if
                 ! apply h**h to columns i:n of current block
                 call stdlib_I64_zungl2( ib, n-i+1, ib, a( i, i ), lda, tau( i ), work,iinfo )
                 ! set columns 1:i-1 of current block to czero
                 do j = 1, i - 1
                    do l = i, i + ib - 1
                       a( l, j ) = czero
     ...
     !! as returned by CGELQF.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CUNGL2', -info )
              return
           end if
           ! quick return if possible
           if( m<=0 )return
           ...
           do i = k, 1, -1
              ! apply h(i)**h to a(i:m,i:n) from the right
              if( i<n ) then
                 call stdlib_I64_clacgv( n-i, a( i, i+1 ), lda )
                 if( i<m ) then
                    a( i, i ) = cone
                    call stdlib_I64_clarf( 'RIGHT', m-i, n-i+1, a( i, i ), lda,conjg( tau( i ) ), a( &
                              i+1, i ), lda, work )
                 end if
                 call stdlib_I64_cscal( n-i, -tau( i ), a( i, i+1 ), lda )
                 call stdlib_I64_clacgv( n-i, a( i, i+1 ), lda )
              end if
              a( i, i ) = cone - conjg( tau( i ) )
              ! set a(i,1:i-1,i) to czero
     ...
     !! as returned by ZGELQF.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZUNGL2', -info )
              return
           end if
           ! quick return if possible
           if( m<=0 )return
           ...
           do i = k, 1, -1
              ! apply h(i)**h to a(i:m,i:n) from the right
              if( i<n ) then
                 call stdlib_I64_zlacgv( n-i, a( i, i+1 ), lda )
                 if( i<m ) then
                    a( i, i ) = cone
                    call stdlib_I64_zlarf( 'RIGHT', m-i, n-i+1, a( i, i ), lda,conjg( tau( i ) ), a( &
                              i+1, i ), lda, work )
                 end if
                 call stdlib_I64_zscal( n-i, -tau( i ), a( i, i+1 ), lda )
                 call stdlib_I64_zlacgv( n-i, a( i, i+1 ), lda )
              end if
              a( i, i ) = cone - conjg( tau( i ) )
              ! set a(i,1:i-1) to czero
     ...
     !! if SIDE = 'R'.
        ...
        ! =====================================================================
           ...
           logical(lk) :: left, lquery, notran
           character :: transt
           integer(ilp64) :: i, i1, i2, i3, ib, ic, iinfo, iwt, jc, ldwork, lwkopt, mi, nb, nbmin, &
                     ni, nq, nw
           ! Intrinsic Functions 
           ! Executable Statements 
           ! test the input arguments
           ...
           if( info==0_ilp64 ) then
              ! compute the workspace requirements
              if( m==0_ilp64 .or. n==0_ilp64 .or. k==0_ilp64 ) then
                 lwkopt = 1_ilp64
              else
                 nb = min( nbmax, stdlib_I64_ilaenv( 1_ilp64, 'CUNMLQ', side // trans, m, n,k, -1_ilp64 ) )
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CUNMLQ', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           if( nb>1_ilp64 .and. nb<k ) then
              if( lwork<lwkopt ) then
                 nb = (lwork-tsize) / ldwork
                 nbmin = max( 2_ilp64, stdlib_I64_ilaenv( 2_ilp64, 'CUNMLQ', side // trans, m, n, k,-1_ilp64 ) )
              end if
           end if
           if( nb<nbmin .or. nb>=k ) then
              ! use unblocked code
              call stdlib_I64_cunml2( side, trans, m, n, k, a, lda, tau, c, ldc, work,iinfo )
           else
              ! use blocked code
              iwt = 1_ilp64 + nw*nb
              if( ( left .and. notran ) .or.( .not.left .and. .not.notran ) ) then
              ...
              do i = i1, i2, i3
                 ...
                 ! form the triangular factor of the block reflector
                 ! h = h(i) h(i+1) . . . h(i+ib-1)
                 call stdlib_I64_clarft( 'FORWARD', 'ROWWISE', nq-i+1, ib, a( i, i ),lda, tau( i ), &
                           work( iwt ), ldt )
                 if( left ) then
                    ! h or h**h is applied to c(i:m,1:n)
                    mi = m - i + 1_ilp64
                 ...
                 ! apply h or h**h
                 call stdlib_I64_clarfb( side, transt, 'FORWARD', 'ROWWISE', mi, ni, ib,a( i, i ), &
                           lda, work( iwt ), ldt,c( ic, jc ), ldc, work, ldwork )
              end do
           end if
           work( 1_ilp64 ) = lwkopt
     ...
     !! if SIDE = 'R'.
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              ! compute the workspace requirements
              nb = min( nbmax, stdlib_I64_ilaenv( 1_ilp64, 'ZUNMLQ', side // trans, m, n, k,-1_ilp64 ) )
              lwkopt = nw*nb + tsize
              work( 1_ilp64 ) = lwkopt
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZUNMLQ', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           if( nb>1_ilp64 .and. nb<k ) then
              if( lwork<lwkopt ) then
                 nb = (lwork-tsize) / ldwork
                 nbmin = max( 2_ilp64, stdlib_I64_ilaenv( 2_ilp64, 'ZUNMLQ', side // trans, m, n, k,-1_ilp64 ) )
              end if
           end if
           if( nb<nbmin .or. nb>=k ) then
              ! use unblocked code
              call stdlib_I64_zunml2( side, trans, m, n, k, a, lda, tau, c, ldc, work,iinfo )
           else
              ! use blocked code
              iwt = 1_ilp64 + nw*nb
              if( ( left .and. notran ) .or.( .not.left .and. .not.notran ) ) then
              ...
              do i = i1, i2, i3
                 ...
                 ! form the triangular factor of the block reflector
                 ! h = h(i) h(i+1) . . . h(i+ib-1)
                 call stdlib_I64_zlarft( 'FORWARD', 'ROWWISE', nq-i+1, ib, a( i, i ),lda, tau( i ), &
                           work( iwt ), ldt )
                 if( left ) then
                    ! h or h**h is applied to c(i:m,1:n)
                    mi = m - i + 1_ilp64
                 ...
                 ! apply h or h**h
                 call stdlib_I64_zlarfb( side, transt, 'FORWARD', 'ROWWISE', mi, ni, ib,a( i, i ), &
                           lda, work( iwt ), ldt,c( ic, jc ), ldc, work, ldwork )
              end do
           end if
           work( 1_ilp64 ) = lwkopt
     ...
     !! if SIDE = 'R'.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           logical(lk) :: left, notran
           integer(ilp64) :: i, i1, i2, i3, ic, jc, mi, ni, nq
           complex(sp) :: aii, taui
           ! Intrinsic Functions 
           ! Executable Statements 
           ! test the input arguments
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CUNML2', -info )
              return
           end if
           ! quick return if possible
           if( m==0 .or. n==0 .or. k==0 )return
           ...
           do i = i1, i2, i3
              ...
              if( i<nq )call stdlib_I64_clacgv( nq-i, a( i, i+1 ), lda )
              aii = a( i, i )
              a( i, i ) = cone
              call stdlib_I64_clarf( side, mi, ni, a( i, i ), lda, taui, c( ic, jc ),ldc, work )
     ...
     !! if SIDE = 'R'.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZUNML2', -info )
              return
           end if
           ! quick return if possible
           if( m==0 .or. n==0 .or. k==0 )return
           ...
           do i = i1, i2, i3
              ...
              if( i<nq )call stdlib_I64_zlacgv( nq-i, a( i, i+1 ), lda )
              aii = a( i, i )
              a( i, i ) = cone
              call stdlib_I64_zlarf( side, mi, ni, a( i, i ), lda, taui, c( ic, jc ),ldc, work )
     ...
     !! as returned by SGELQF.
        ...
        ! =====================================================================
           ...
           ! test the input arguments
           info = 0_ilp64
           nb = stdlib_I64_ilaenv( 1_ilp64, 'SORGLQ', ' ', m, n, k, -1_ilp64 )
           lwkopt = max( 1_ilp64, m )*nb
           work( 1_ilp64 ) = lwkopt
           lquery = ( lwork==-1_ilp64 )
           if( m<0_ilp64 ) then
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SORGLQ', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           if( nb>1_ilp64 .and. nb<k ) then
              ! determine when to cross over from blocked to unblocked code.
              nx = max( 0_ilp64, stdlib_I64_ilaenv( 3_ilp64, 'SORGLQ', ' ', m, n, k, -1_ilp64 ) )
              if( nx<k ) then
                 ! determine if workspace is large enough for blocked code.
                 ldwork = m
                 iws = ldwork*nb
                 if( lwork<iws ) then
                    ...
                    ! determine the minimum value of nb.
                    nb = lwork / ldwork
                    nbmin = max( 2_ilp64, stdlib_I64_ilaenv( 2_ilp64, 'SORGLQ', ' ', m, n, k, -1_ilp64 ) )
                 end if
              end if
           end if
           if( nb>=nbmin .and. nb<k .and. nx<k ) then
           ...
           ! use unblocked code for the last or only block.
           if( kk<m )call stdlib_I64_sorgl2( m-kk, n-kk, k-kk, a( kk+1, kk+1 ), lda,tau( kk+1 ), work,&
                      iinfo )
           if( kk>0_ilp64 ) then
              ! use blocked code
              do i = ki + 1, 1, -nb
                 ...
                 if( i+ib<=m ) then
                    ...
                    call stdlib_I64_slarft( 'FORWARD', 'ROWWISE', n-i+1, ib, a( i, i ),lda, tau( i ), &
                              work, ldwork )
                    ! apply h**t to a(i+ib:m,i:n) from the right
                    call stdlib_I64_slarfb( 'RIGHT', 'TRANSPOSE', 'FORWARD', 'ROWWISE',m-i-ib+1, n-i+&
                    1_ilp64, ib, a( i, i ), lda, work,ldwork, a( i+ib, i ), lda, work( ib+1 ),ldwork )
                              
                 end if
                 ! apply h**t to columns i:n of current block
                 call stdlib_I64_sorgl2( ib, n-i+1, ib, a( i, i ), lda, tau( i ), work,iinfo )
                 ! set columns 1:i-1 of current block to zero
                 do j = 1, i - 1
                    do l = i, i + ib - 1
                       a( l, j ) = zero
     ...
     !! as returned by DGELQF.
        ...
        ! =====================================================================
           ...
           ! test the input arguments
           info = 0_ilp64
           nb = stdlib_I64_ilaenv( 1_ilp64, 'DORGLQ', ' ', m, n, k, -1_ilp64 )
           lwkopt = max( 1_ilp64, m )*nb
           work( 1_ilp64 ) = lwkopt
           lquery = ( lwork==-1_ilp64 )
           if( m<0_ilp64 ) then
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DORGLQ', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           if( nb>1_ilp64 .and. nb<k ) then
              ! determine when to cross over from blocked to unblocked code.
              nx = max( 0_ilp64, stdlib_I64_ilaenv( 3_ilp64, 'DORGLQ', ' ', m, n, k, -1_ilp64 ) )
              if( nx<k ) then
                 ! determine if workspace is large enough for blocked code.
                 ldwork = m
                 iws = ldwork*nb
                 if( lwork<iws ) then
                    ...
                    ! determine the minimum value of nb.
                    nb = lwork / ldwork
                    nbmin = max( 2_ilp64, stdlib_I64_ilaenv( 2_ilp64, 'DORGLQ', ' ', m, n, k, -1_ilp64 ) )
                 end if
              end if
           end if
           if( nb>=nbmin .and. nb<k .and. nx<k ) then
           ...
           ! use unblocked code for the last or only block.
           if( kk<m )call stdlib_I64_dorgl2( m-kk, n-kk, k-kk, a( kk+1, kk+1 ), lda,tau( kk+1 ), work,&
                      iinfo )
           if( kk>0_ilp64 ) then
              ! use blocked code
              do i = ki + 1, 1, -nb
                 ...
                 if( i+ib<=m ) then
                    ...
                    call stdlib_I64_dlarft( 'FORWARD', 'ROWWISE', n-i+1, ib, a( i, i ),lda, tau( i ), &
                              work, ldwork )
                    ! apply h**t to a(i+ib:m,i:n) from the right
                    call stdlib_I64_dlarfb( 'RIGHT', 'TRANSPOSE', 'FORWARD', 'ROWWISE',m-i-ib+1, n-i+&
                    1_ilp64, ib, a( i, i ), lda, work,ldwork, a( i+ib, i ), lda, work( ib+1 ),ldwork )
                              
                 end if
                 ! apply h**t to columns i:n of current block
                 call stdlib_I64_dorgl2( ib, n-i+1, ib, a( i, i ), lda, tau( i ), work,iinfo )
                 ! set columns 1:i-1 of current block to zero
                 do j = 1, i - 1
                    do l = i, i + ib - 1
                       a( l, j ) = zero
     ...
     !! as returned by SGELQF.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SORGL2', -info )
              return
           end if
           ! quick return if possible
           if( m<=0 )return
           ...
           do i = k, 1, -1
              ...
              if( i<n ) then
                 ...
                 call stdlib_I64_sscal( n-i, -tau( i ), a( i, i+1 ), lda )
              end if
              a( i, i ) = one - tau( i )
              ! set a(i,1:i-1) to zero
              do l = 1, i - 1
     ...
     !! as returned by DGELQF.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DORGL2', -info )
              return
           end if
           ! quick return if possible
           if( m<=0 )return
           ...
           do i = k, 1, -1
              ...
              if( i<n ) then
                 ...
                 call stdlib_I64_dscal( n-i, -tau( i ), a( i, i+1 ), lda )
              end if
              a( i, i ) = one - tau( i )
              ! set a(i,1:i-1) to zero
              do l = 1, i - 1
     ...
     !! if SIDE = 'R'.
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              ! compute the workspace requirements
              nb = min( nbmax, stdlib_I64_ilaenv( 1_ilp64, 'SORMLQ', side // trans, m, n, k,-1_ilp64 ) )
              lwkopt = nw*nb + tsize
              work( 1_ilp64 ) = lwkopt
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SORMLQ', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           if( nb>1_ilp64 .and. nb<k ) then
              if( lwork<lwkopt ) then
                 nb = (lwork-tsize) / ldwork
                 nbmin = max( 2_ilp64, stdlib_I64_ilaenv( 2_ilp64, 'SORMLQ', side // trans, m, n, k,-1_ilp64 ) )
              end if
           end if
           if( nb<nbmin .or. nb>=k ) then
              ! use unblocked code
              call stdlib_I64_sorml2( side, trans, m, n, k, a, lda, tau, c, ldc, work,iinfo )
           else
              ! use blocked code
              iwt = 1_ilp64 + nw*nb
              if( ( left .and. notran ) .or.( .not.left .and. .not.notran ) ) then
              ...
              do i = i1, i2, i3
                 ...
                 ! form the triangular factor of the block reflector
                 ! h = h(i) h(i+1) . . . h(i+ib-1)
                 call stdlib_I64_slarft( 'FORWARD', 'ROWWISE', nq-i+1, ib, a( i, i ),lda, tau( i ), &
                           work( iwt ), ldt )
                 if( left ) then
                    ! h or h**t is applied to c(i:m,1:n)
                    mi = m - i + 1_ilp64
                 ...
                 ! apply h or h**t
                 call stdlib_I64_slarfb( side, transt, 'FORWARD', 'ROWWISE', mi, ni, ib,a( i, i ), &
                           lda, work( iwt ), ldt,c( ic, jc ), ldc, work, ldwork )
              end do
           end if
           work( 1_ilp64 ) = lwkopt
     ...
     !! if SIDE = 'R'.
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              ! compute the workspace requirements
              nb = min( nbmax, stdlib_I64_ilaenv( 1_ilp64, 'DORMLQ', side // trans, m, n, k,-1_ilp64 ) )
              lwkopt = nw*nb + tsize
              work( 1_ilp64 ) = lwkopt
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DORMLQ', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           if( nb>1_ilp64 .and. nb<k ) then
              if( lwork<lwkopt ) then
                 nb = (lwork-tsize) / ldwork
                 nbmin = max( 2_ilp64, stdlib_I64_ilaenv( 2_ilp64, 'DORMLQ', side // trans, m, n, k,-1_ilp64 ) )
              end if
           end if
           if( nb<nbmin .or. nb>=k ) then
              ! use unblocked code
              call stdlib_I64_dorml2( side, trans, m, n, k, a, lda, tau, c, ldc, work,iinfo )
           else
              ! use blocked code
              iwt = 1_ilp64 + nw*nb
              if( ( left .and. notran ) .or.( .not.left .and. .not.notran ) ) then
              ...
              do i = i1, i2, i3
                 ...
                 ! form the triangular factor of the block reflector
                 ! h = h(i) h(i+1) . . . h(i+ib-1)
                 call stdlib_I64_dlarft( 'FORWARD', 'ROWWISE', nq-i+1, ib, a( i, i ),lda, tau( i ), &
                           work( iwt ), ldt )
                 if( left ) then
                    ! h or h**t is applied to c(i:m,1:n)
                    mi = m - i + 1_ilp64
                 ...
                 ! apply h or h**t
                 call stdlib_I64_dlarfb( side, transt, 'FORWARD', 'ROWWISE', mi, ni, ib,a( i, i ), &
                           lda, work( iwt ), ldt,c( ic, jc ), ldc, work, ldwork )
              end do
           end if
           work( 1_ilp64 ) = lwkopt
     ...
     !! if SIDE = 'R'.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SORML2', -info )
              return
           end if
           ! quick return if possible
           if( m==0 .or. n==0 .or. k==0 )return
           ...
           do i = i1, i2, i3
              ...
              aii = a( i, i )
              a( i, i ) = one
              call stdlib_I64_slarf( side, mi, ni, a( i, i ), lda, tau( i ),c( ic, jc ), ldc, work )
     ...
     !! if SIDE = 'R'.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DORML2', -info )
              return
           end if
           ! quick return if possible
           if( m==0 .or. n==0 .or. k==0 )return
           ...
           do i = i1, i2, i3
              ...
              aii = a( i, i )
              a( i, i ) = one
              call stdlib_I64_dlarf( side, mi, ni, a( i, i ), lda, tau( i ),c( ic, jc ), ldc, work )
     ...
     !! using the compact WY representation of Q.
       ...
       ! =====================================================================
           ...
           if( m<0_ilp64 ) then
              ...
              info = -2_ilp64
           else if( mb<1_ilp64 .or. ( mb>min(m,n) .and. min(m,n)>0_ilp64 ) )then
              info = -3_ilp64
           else if( lda<max( 1_ilp64, m ) ) then
              info = -5_ilp64
           else if( ldt<mb ) then
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SGELQT', -info )
              return
           end if
           ! quick return if possible
           k = min( m, n )
           ...
           do i = 1, k,  mb
              ib = min( k-i+1, mb )
           ! compute the lq factorization of the current block a(i:m,i:i+ib-1)
              call stdlib_I64_sgelqt3( ib, n-i+1, a(i,i), lda, t(1_ilp64,i), ldt, iinfo )
              if( i+ib<=m ) then
           ! update by applying h**t to a(i:m,i+ib:n) from the right
              call stdlib_I64_slarfb( 'R', 'N', 'F', 'R', m-i-ib+1, n-i+1, ib,a( i, i ), lda, t( 1_ilp64, i &
                        ), ldt,a( i+ib, i ), lda, work , m-i-ib+1 )
              end if
           end do
           return
     ...
     !! using the compact WY representation of Q.
       ...
       ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DGELQT', -info )
              return
           end if
           ! quick return if possible
           k = min( m, n )
           ...
           do i = 1, k,  mb
              ib = min( k-i+1, mb )
           ! compute the lq factorization of the current block a(i:m,i:i+ib-1)
              call stdlib_I64_dgelqt3( ib, n-i+1, a(i,i), lda, t(1_ilp64,i), ldt, iinfo )
              if( i+ib<=m ) then
           ! update by applying h**t to a(i:m,i+ib:n) from the right
              call stdlib_I64_dlarfb( 'R', 'N', 'F', 'R', m-i-ib+1, n-i+1, ib,a( i, i ), lda, t( 1_ilp64, i &
                        ), ldt,a( i+ib, i ), lda, work , m-i-ib+1 )
              end if
           end do
           return
     ...
     !! using the compact WY representation of Q.
       ...
       ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CGELQT', -info )
              return
           end if
           ! quick return if possible
           k = min( m, n )
           ...
           do i = 1, k,  mb
              ib = min( k-i+1, mb )
           ! compute the lq factorization of the current block a(i:m,i:i+ib-1)
              call stdlib_I64_cgelqt3( ib, n-i+1, a(i,i), lda, t(1_ilp64,i), ldt, iinfo )
              if( i+ib<=m ) then
           ! update by applying h**t to a(i:m,i+ib:n) from the right
              call stdlib_I64_clarfb( 'R', 'N', 'F', 'R', m-i-ib+1, n-i+1, ib,a( i, i ), lda, t( 1_ilp64, i &
                        ), ldt,a( i+ib, i ), lda, work , m-i-ib+1 )
              end if
           end do
           return
     ...
     !! using the compact WY representation of Q.
       ...
       ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZGELQT', -info )
              return
           end if
           ! quick return if possible
           k = min( m, n )
           ...
           do i = 1, k,  mb
              ib = min( k-i+1, mb )
           ! compute the lq factorization of the current block a(i:m,i:i+ib-1)
              call stdlib_I64_zgelqt3( ib, n-i+1, a(i,i), lda, t(1_ilp64,i), ldt, iinfo )
              if( i+ib<=m ) then
           ! update by applying h**t to a(i:m,i+ib:n) from the right
              call stdlib_I64_zlarfb( 'R', 'N', 'F', 'R', m-i-ib+1, n-i+1, ib,a( i, i ), lda, t( 1_ilp64, i &
                        ), ldt,a( i+ib, i ), lda, work , m-i-ib+1 )
              end if
           end do
           return
     ...
     !! IBM J. Res. Develop. Vol 44 No. 4 July 2000.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           integer(ilp64) :: i, i1, j, j1, m1, m2, iinfo
           ! Executable Statements 
           info = 0_ilp64
           if( m < 0_ilp64 ) then
              info = -1_ilp64
              ...
              info = -4_ilp64
           else if( ldt < max( 1_ilp64, m ) ) then
              info = -6_ilp64
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SGELQT3', -info )
              return
           end if
           if( m==1_ilp64 ) then
              ! compute householder transform when m=1
              call stdlib_I64_slarfg( n, a(1_ilp64,1_ilp64), a( 1_ilp64, min( 2_ilp64, n ) ), lda, t(1_ilp64,1_ilp64) )
           else
              ! otherwise, split a into blocks...
              m1 = m/2_ilp64
              m2 = m-m1
              ...
              j1 = min( m+1, n )
              ! compute a(1:m1,1:n) <- (y1,r1,t1), where q1 = i - y1 t1 y1^h
              call stdlib_I64_sgelqt3( m1, n, a, lda, t, ldt, iinfo )
              ! compute a(j1:m,1:n) = q1^h a(j1:m,1:n) [workspace: t(1:n1,j1:n)]
              do i=1,m2
                 do j=1,m1
                    t(  i+m1, j ) = a( i+m1, j )
              ...
              call stdlib_I64_strmm( 'R', 'U', 'T', 'U', m2, m1, one,a, lda, t( i1, 1_ilp64 ), ldt )
              call stdlib_I64_sgemm( 'N', 'T', m2, m1, n-m1, one, a( i1, i1 ), lda,a( 1_ilp64, i1 ), lda, &
                        one, t( i1, 1_ilp64 ), ldt)
              call stdlib_I64_strmm( 'R', 'U', 'N', 'N', m2, m1, one,t, ldt, t( i1, 1_ilp64 ), ldt )
              call stdlib_I64_sgemm( 'N', 'N', m2, n-m1, m1, -one, t( i1, 1_ilp64 ), ldt,a( 1_ilp64, i1 ), lda, &
                        one, a( i1, i1 ), lda )
              call stdlib_I64_strmm( 'R', 'U', 'N', 'U', m2, m1 , one,a, lda, t( i1, 1_ilp64 ), ldt )
              ...
              ! compute a(j1:m,j1:n) <- (y2,r2,t2) where q2 = i - y2 t2 y2^h
              call stdlib_I64_sgelqt3( m2, n-m1, a( i1, i1 ), lda,t( i1, i1 ), ldt, iinfo )
              ! compute t3 = t(j1:n1,1:n) = -t1 y1^h y2 t2
              do i=1,m2
                 do j=1,m1
                    t( j, i+m1  ) = (a( j, i+m1 ))
              ...
              call stdlib_I64_strmm( 'R', 'U', 'T', 'U', m1, m2, one,a( i1, i1 ), lda, t( 1_ilp64, i1 ), &
                        ldt )
              call stdlib_I64_sgemm( 'N', 'T', m1, m2, n-m, one, a( 1_ilp64, j1 ), lda,a( i1, j1 ), lda, &
                        one, t( 1_ilp64, i1 ), ldt )
              call stdlib_I64_strmm( 'L', 'U', 'N', 'N', m1, m2, -one, t, ldt,t( 1_ilp64, i1 ), ldt )
                        
              call stdlib_I64_strmm( 'R', 'U', 'N', 'N', m1, m2, one,t( i1, i1 ), ldt, t( 1_ilp64, i1 ), &
                        ldt )
              ! y = (y1,y2); l = [ l1            0  ];  t = [t1 t3]
                               ! [ a(1:n1,j1:n)  l2 ]       [ 0 t2]
           end if
     ...
     !! IBM J. Res. Develop. Vol 44 No. 4 July 2000.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DGELQT3', -info )
              return
           end if
           if( m==1_ilp64 ) then
              ! compute householder transform when m=1
              call stdlib_I64_dlarfg( n, a(1_ilp64,1_ilp64), a( 1_ilp64, min( 2_ilp64, n ) ), lda, t(1_ilp64,1_ilp64) )
           else
              ! otherwise, split a into blocks...
              m1 = m/2_ilp64
              m2 = m-m1
              ...
              j1 = min( m+1, n )
              ! compute a(1:m1,1:n) <- (y1,r1,t1), where q1 = i - y1 t1 y1^h
              call stdlib_I64_dgelqt3( m1, n, a, lda, t, ldt, iinfo )
              ! compute a(j1:m,1:n) = q1^h a(j1:m,1:n) [workspace: t(1:n1,j1:n)]
              do i=1,m2
                 do j=1,m1
                    t(  i+m1, j ) = a( i+m1, j )
              ...
              call stdlib_I64_dtrmm( 'R', 'U', 'T', 'U', m2, m1, one,a, lda, t( i1, 1_ilp64 ), ldt )
              call stdlib_I64_dgemm( 'N', 'T', m2, m1, n-m1, one, a( i1, i1 ), lda,a( 1_ilp64, i1 ), lda, &
                        one, t( i1, 1_ilp64 ), ldt)
              call stdlib_I64_dtrmm( 'R', 'U', 'N', 'N', m2, m1, one,t, ldt, t( i1, 1_ilp64 ), ldt )
              call stdlib_I64_dgemm( 'N', 'N', m2, n-m1, m1, -one, t( i1, 1_ilp64 ), ldt,a( 1_ilp64, i1 ), lda, &
                        one, a( i1, i1 ), lda )
              call stdlib_I64_dtrmm( 'R', 'U', 'N', 'U', m2, m1 , one,a, lda, t( i1, 1_ilp64 ), ldt )
              ...
              ! compute a(j1:m,j1:n) <- (y2,r2,t2) where q2 = i - y2 t2 y2^h
              call stdlib_I64_dgelqt3( m2, n-m1, a( i1, i1 ), lda,t( i1, i1 ), ldt, iinfo )
              ! compute t3 = t(j1:n1,1:n) = -t1 y1^h y2 t2
              do i=1,m2
                 do j=1,m1
                    t( j, i+m1  ) = (a( j, i+m1 ))
              ...
              call stdlib_I64_dtrmm( 'R', 'U', 'T', 'U', m1, m2, one,a( i1, i1 ), lda, t( 1_ilp64, i1 ), &
                        ldt )
              call stdlib_I64_dgemm( 'N', 'T', m1, m2, n-m, one, a( 1_ilp64, j1 ), lda,a( i1, j1 ), lda, &
                        one, t( 1_ilp64, i1 ), ldt )
              call stdlib_I64_dtrmm( 'L', 'U', 'N', 'N', m1, m2, -one, t, ldt,t( 1_ilp64, i1 ), ldt )
                        
              call stdlib_I64_dtrmm( 'R', 'U', 'N', 'N', m1, m2, one,t( i1, i1 ), ldt, t( 1_ilp64, i1 ), &
                        ldt )
              ! y = (y1,y2); l = [ l1            0  ];  t = [t1 t3]
                               ! [ a(1:n1,j1:n)  l2 ]       [ 0 t2]
           end if
     ...
     !! IBM J. Res. Develop. Vol 44 No. 4 July 2000.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CGELQT3', -info )
              return
           end if
           if( m==1_ilp64 ) then
              ! compute householder transform when m=1
              call stdlib_I64_clarfg( n, a(1_ilp64,1_ilp64), a( 1_ilp64, min( 2_ilp64, n ) ), lda, t(1_ilp64,1_ilp64) )
              t(1_ilp64,1_ilp64)=conjg(t(1_ilp64,1_ilp64))
           else
              ! otherwise, split a into blocks...
              m1 = m/2_ilp64
              m2 = m-m1
              ...
              j1 = min( m+1, n )
              ! compute a(1:m1,1:n) <- (y1,r1,t1), where q1 = i - y1 t1 y1^h
              call stdlib_I64_cgelqt3( m1, n, a, lda, t, ldt, iinfo )
              ! compute a(j1:m,1:n) =  a(j1:m,1:n) q1^h [workspace: t(1:n1,j1:n)]
              do i=1,m2
                 do j=1,m1
                    t(  i+m1, j ) = a( i+m1, j )
              ...
              call stdlib_I64_ctrmm( 'R', 'U', 'C', 'U', m2, m1, cone,a, lda, t( i1, 1_ilp64 ), ldt )
                        
              call stdlib_I64_cgemm( 'N', 'C', m2, m1, n-m1, cone, a( i1, i1 ), lda,a( 1_ilp64, i1 ), lda, &
                        cone, t( i1, 1_ilp64 ), ldt)
              call stdlib_I64_ctrmm( 'R', 'U', 'N', 'N', m2, m1, cone,t, ldt, t( i1, 1_ilp64 ), ldt )
                        
              call stdlib_I64_cgemm( 'N', 'N', m2, n-m1, m1, -cone, t( i1, 1_ilp64 ), ldt,a( 1_ilp64, i1 ), lda, &
                        cone, a( i1, i1 ), lda )
              call stdlib_I64_ctrmm( 'R', 'U', 'N', 'U', m2, m1 , cone,a, lda, t( i1, 1_ilp64 ), ldt )
              ...
              ! compute a(j1:m,j1:n) <- (y2,r2,t2) where q2 = i - y2 t2 y2^h
              call stdlib_I64_cgelqt3( m2, n-m1, a( i1, i1 ), lda,t( i1, i1 ), ldt, iinfo )
              ! compute t3 = t(j1:n1,1:n) = -t1 y1^h y2 t2
              do i=1,m2
                 do j=1,m1
                    t( j, i+m1  ) = (a( j, i+m1 ))
              ...
              call stdlib_I64_ctrmm( 'R', 'U', 'C', 'U', m1, m2, cone,a( i1, i1 ), lda, t( 1_ilp64, i1 ), &
                        ldt )
              call stdlib_I64_cgemm( 'N', 'C', m1, m2, n-m, cone, a( 1_ilp64, j1 ), lda,a( i1, j1 ), lda, &
                        cone, t( 1_ilp64, i1 ), ldt )
              call stdlib_I64_ctrmm( 'L', 'U', 'N', 'N', m1, m2, -cone, t, ldt,t( 1_ilp64, i1 ), ldt )
                        
              call stdlib_I64_ctrmm( 'R', 'U', 'N', 'N', m1, m2, cone,t( i1, i1 ), ldt, t( 1_ilp64, i1 ), &
                        ldt )
              ! y = (y1,y2); l = [ l1            0  ];  t = [t1 t3]
                               ! [ a(1:n1,j1:n)  l2 ]       [ 0 t2]
           end if
     ...
     !! IBM J. Res. Develop. Vol 44 No. 4 July 2000.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZGELQT3', -info )
              return
           end if
           if( m==1_ilp64 ) then
              ! compute householder transform when m=1
              call stdlib_I64_zlarfg( n, a(1_ilp64,1_ilp64), a( 1_ilp64, min( 2_ilp64, n ) ), lda, t(1_ilp64,1_ilp64) )
              t(1_ilp64,1_ilp64)=conjg(t(1_ilp64,1_ilp64))
           else
              ! otherwise, split a into blocks...
              m1 = m/2_ilp64
              ...
              j1 = min( m+1, n )
              ! compute a(1:m1,1:n) <- (y1,r1,t1), where q1 = i - y1 t1 y1^h
              call stdlib_I64_zgelqt3( m1, n, a, lda, t, ldt, iinfo )
              ! compute a(j1:m,1:n) =  a(j1:m,1:n) q1^h [workspace: t(1:n1,j1:n)]
              do i=1,m2
                 do j=1,m1
                    t(  i+m1, j ) = a( i+m1, j )
              ...
              call stdlib_I64_ztrmm( 'R', 'U', 'C', 'U', m2, m1, cone,a, lda, t( i1, 1_ilp64 ), ldt )
                        
              call stdlib_I64_zgemm( 'N', 'C', m2, m1, n-m1, cone, a( i1, i1 ), lda,a( 1_ilp64, i1 ), lda, &
                        cone, t( i1, 1_ilp64 ), ldt)
              call stdlib_I64_ztrmm( 'R', 'U', 'N', 'N', m2, m1, cone,t, ldt, t( i1, 1_ilp64 ), ldt )
                        
              call stdlib_I64_zgemm( 'N', 'N', m2, n-m1, m1, -cone, t( i1, 1_ilp64 ), ldt,a( 1_ilp64, i1 ), lda, &
                        cone, a( i1, i1 ), lda )
              call stdlib_I64_ztrmm( 'R', 'U', 'N', 'U', m2, m1 , cone,a, lda, t( i1, 1_ilp64 ), ldt )
              ...
              ! compute a(j1:m,j1:n) <- (y2,r2,t2) where q2 = i - y2 t2 y2^h
              call stdlib_I64_zgelqt3( m2, n-m1, a( i1, i1 ), lda,t( i1, i1 ), ldt, iinfo )
              ! compute t3 = t(j1:n1,1:n) = -t1 y1^h y2 t2
              do i=1,m2
                 do j=1,m1
                    t( j, i+m1  ) = (a( j, i+m1 ))
              ...
              call stdlib_I64_ztrmm( 'R', 'U', 'C', 'U', m1, m2, cone,a( i1, i1 ), lda, t( 1_ilp64, i1 ), &
                        ldt )
              call stdlib_I64_zgemm( 'N', 'C', m1, m2, n-m, cone, a( 1_ilp64, j1 ), lda,a( i1, j1 ), lda, &
                        cone, t( 1_ilp64, i1 ), ldt )
              call stdlib_I64_ztrmm( 'L', 'U', 'N', 'N', m1, m2, -cone, t, ldt,t( 1_ilp64, i1 ), ldt )
                        
              call stdlib_I64_ztrmm( 'R', 'U', 'N', 'N', m1, m2, cone,t( i1, i1 ), ldt, t( 1_ilp64, i1 ), &
                        ldt )
              ! y = (y1,y2); l = [ l1            0  ];  t = [t1 t3]
                               ! [ a(1:n1,j1:n)  l2 ]       [ 0 t2]
           end if
     ...
     !! Q is of order M if SIDE = 'L' and of order N  if SIDE = 'R'.
        ...
        ! =====================================================================
           ! Local Scalars 
           logical(lk) :: left, right, tran, notran
           integer(ilp64) :: i, ib, ldwork, kf, q
           ! Intrinsic Functions 
           ! Executable Statements 
           ! Test The Input Arguments 
           info   = 0_ilp64
           ...
           notran = stdlib_lsame( trans, 'N' )
           if( left ) then
              ldwork = max( 1_ilp64, n )
              q = m
           else if ( right ) then
              ldwork = max( 1_ilp64, m )
              q = n
           end if
           if( .not.left .and. .not.right ) then
              info = -1_ilp64
              ...
              info = -4_ilp64
           else if( k<0_ilp64 .or. k>q ) then
              info = -5_ilp64
           else if( mb<1_ilp64 .or. (mb>k .and. k>0_ilp64)) then
              info = -6_ilp64
           else if( ldv<max( 1_ilp64, k ) ) then
               info = -8_ilp64
           else if( ldt<mb ) then
              info = -10_ilp64
           else if( ldc<max( 1_ilp64, m ) ) then
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SGEMLQT', -info )
              return
           end if
           ! Quick Return If Possible 
           if( m==0 .or. n==0 .or. k==0 ) return
           if( left .and. notran ) then
              do i = 1, k, mb
                 ib = min( mb, k-i+1 )
                 call stdlib_I64_slarfb( 'L', 'T', 'F', 'R', m-i+1, n, ib,v( i, i ), ldv, t( 1_ilp64, i ), &
                           ldt,c( i, 1_ilp64 ), ldc, work, ldwork )
              end do
           else if( right .and. tran ) then
              do i = 1, k, mb
                 ib = min( mb, k-i+1 )
                 call stdlib_I64_slarfb( 'R', 'N', 'F', 'R', m, n-i+1, ib,v( i, i ), ldv, t( 1_ilp64, i ), &
                           ldt,c( 1_ilp64, i ), ldc, work, ldwork )
              end do
           else if( left .and. tran ) then
              kf = ((k-1)/mb)*mb+1
              do i = kf, 1, -mb
                 ib = min( mb, k-i+1 )
                 call stdlib_I64_slarfb( 'L', 'N', 'F', 'R', m-i+1, n, ib,v( i, i ), ldv, t( 1_ilp64, i ), &
                           ldt,c( i, 1_ilp64 ), ldc, work, ldwork )
              end do
           else if( right .and. notran ) then
              kf = ((k-1)/mb)*mb+1
              do i = kf, 1, -mb
                 ib = min( mb, k-i+1 )
                 call stdlib_I64_slarfb( 'R', 'T', 'F', 'R', m, n-i+1, ib,v( i, i ), ldv, t( 1_ilp64, i ), &
                           ldt,c( 1_ilp64, i ), ldc, work, ldwork )
              end do
           end if
           return
     ...
     !! Q is of order M if SIDE = 'L' and of order N  if SIDE = 'R'.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DGEMLQT', -info )
              return
           end if
           ! Quick Return If Possible 
           if( m==0 .or. n==0 .or. k==0 ) return
           if( left .and. notran ) then
              do i = 1, k, mb
                 ib = min( mb, k-i+1 )
                 call stdlib_I64_dlarfb( 'L', 'T', 'F', 'R', m-i+1, n, ib,v( i, i ), ldv, t( 1_ilp64, i ), &
                           ldt,c( i, 1_ilp64 ), ldc, work, ldwork )
              end do
           else if( right .and. tran ) then
              do i = 1, k, mb
                 ib = min( mb, k-i+1 )
                 call stdlib_I64_dlarfb( 'R', 'N', 'F', 'R', m, n-i+1, ib,v( i, i ), ldv, t( 1_ilp64, i ), &
                           ldt,c( 1_ilp64, i ), ldc, work, ldwork )
              end do
           else if( left .and. tran ) then
              kf = ((k-1)/mb)*mb+1
              do i = kf, 1, -mb
                 ib = min( mb, k-i+1 )
                 call stdlib_I64_dlarfb( 'L', 'N', 'F', 'R', m-i+1, n, ib,v( i, i ), ldv, t( 1_ilp64, i ), &
                           ldt,c( i, 1_ilp64 ), ldc, work, ldwork )
              end do
           else if( right .and. notran ) then
              kf = ((k-1)/mb)*mb+1
              do i = kf, 1, -mb
                 ib = min( mb, k-i+1 )
                 call stdlib_I64_dlarfb( 'R', 'T', 'F', 'R', m, n-i+1, ib,v( i, i ), ldv, t( 1_ilp64, i ), &
                           ldt,c( 1_ilp64, i ), ldc, work, ldwork )
              end do
           end if
           return
     ...
     !! Q is of order M if SIDE = 'L' and of order N  if SIDE = 'R'.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CGEMLQT', -info )
              return
           end if
           ! Quick Return If Possible 
           if( m==0 .or. n==0 .or. k==0 ) return
           if( left .and. notran ) then
              do i = 1, k, mb
                 ib = min( mb, k-i+1 )
                 call stdlib_I64_clarfb( 'L', 'C', 'F', 'R', m-i+1, n, ib,v( i, i ), ldv, t( 1_ilp64, i ), &
                           ldt,c( i, 1_ilp64 ), ldc, work, ldwork )
              end do
           else if( right .and. tran ) then
              do i = 1, k, mb
                 ib = min( mb, k-i+1 )
                 call stdlib_I64_clarfb( 'R', 'N', 'F', 'R', m, n-i+1, ib,v( i, i ), ldv, t( 1_ilp64, i ), &
                           ldt,c( 1_ilp64, i ), ldc, work, ldwork )
              end do
           else if( left .and. tran ) then
              kf = ((k-1)/mb)*mb+1
              do i = kf, 1, -mb
                 ib = min( mb, k-i+1 )
                 call stdlib_I64_clarfb( 'L', 'N', 'F', 'R', m-i+1, n, ib,v( i, i ), ldv, t( 1_ilp64, i ), &
                           ldt,c( i, 1_ilp64 ), ldc, work, ldwork )
              end do
           else if( right .and. notran ) then
              kf = ((k-1)/mb)*mb+1
              do i = kf, 1, -mb
                 ib = min( mb, k-i+1 )
                 call stdlib_I64_clarfb( 'R', 'C', 'F', 'R', m, n-i+1, ib,v( i, i ), ldv, t( 1_ilp64, i ), &
                           ldt,c( 1_ilp64, i ), ldc, work, ldwork )
              end do
           end if
           return
     ...
     !! Q is of order M if SIDE = 'L' and of order N  if SIDE = 'R'.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZGEMLQT', -info )
              return
           end if
           ! Quick Return If Possible 
           if( m==0 .or. n==0 .or. k==0 ) return
           if( left .and. notran ) then
              do i = 1, k, mb
                 ib = min( mb, k-i+1 )
                 call stdlib_I64_zlarfb( 'L', 'C', 'F', 'R', m-i+1, n, ib,v( i, i ), ldv, t( 1_ilp64, i ), &
                           ldt,c( i, 1_ilp64 ), ldc, work, ldwork )
              end do
           else if( right .and. tran ) then
              do i = 1, k, mb
                 ib = min( mb, k-i+1 )
                 call stdlib_I64_zlarfb( 'R', 'N', 'F', 'R', m, n-i+1, ib,v( i, i ), ldv, t( 1_ilp64, i ), &
                           ldt,c( 1_ilp64, i ), ldc, work, ldwork )
              end do
           else if( left .and. tran ) then
              kf = ((k-1)/mb)*mb+1
              do i = kf, 1, -mb
                 ib = min( mb, k-i+1 )
                 call stdlib_I64_zlarfb( 'L', 'N', 'F', 'R', m-i+1, n, ib,v( i, i ), ldv, t( 1_ilp64, i ), &
                           ldt,c( i, 1_ilp64 ), ldc, work, ldwork )
              end do
           else if( right .and. notran ) then
              kf = ((k-1)/mb)*mb+1
              do i = kf, 1, -mb
                 ib = min( mb, k-i+1 )
                 call stdlib_I64_zlarfb( 'R', 'C', 'F', 'R', m, n-i+1, ib,v( i, i ), ldv, t( 1_ilp64, i ), &
                           ldt,c( 1_ilp64, i ), ldc, work, ldwork )
              end do
           end if
           return
     ...
     !! 0 is a M-by-(N-M) zero matrix, if M < N, and is not stored.
        ...
        ! =====================================================================
           ...
           if( m<0_ilp64 ) then
             info = -1_ilp64
           else if( n<0_ilp64 .or. n<m ) then
             info = -2_ilp64
           else if( mb<1_ilp64 .or. ( mb>m .and. m>0_ilp64 )) then
             info = -3_ilp64
           else if( nb<=0_ilp64 ) then
             info = -4_ilp64
           else if( lda<max( 1_ilp64, m ) ) then
             info = -6_ilp64
           else if( ldt<mb ) then
           ...
           if( info/=0_ilp64 ) then
             call stdlib_I64_xerbla( 'SLASWLQ', -info )
             return
           else if (lquery) then
            return
           end if
           ...
           ! the lq decomposition
            if((m>=n).or.(nb<=m).or.(nb>=n)) then
             call stdlib_I64_sgelqt( m, n, mb, a, lda, t, ldt, work, info)
             return
            end if
            kk = mod((n-m),(nb-m))
            ii=n-kk+1
            ! compute the lq factorization of the first block a(1:m,1:nb)
            call stdlib_I64_sgelqt( m, nb, mb, a(1_ilp64,1_ilp64), lda, t, ldt, work, info)
            ctr = 1_ilp64
            do i = nb+1, ii-nb+m , (nb-m)
            ! compute the qr factorization of the current block a(1:m,i:i+nb-m)
              call stdlib_I64_stplqt( m, nb-m, 0_ilp64, mb, a(1_ilp64,1_ilp64), lda, a( 1_ilp64, i ),lda, t(1_ilp64, ctr * m +&
                  & 1_ilp64),&
                        ldt, work, info )
              ctr = ctr + 1_ilp64
            end do
           ! compute the qr factorization of the last block a(1:m,ii:n)
            if (ii<=n) then
             call stdlib_I64_stplqt( m, kk, 0_ilp64, mb, a(1_ilp64,1_ilp64), lda, a( 1_ilp64, ii ),lda, t(1_ilp64, ctr * m +&
                 & 1_ilp64), &
                       ldt,work, info )
            end if
           work( 1_ilp64 ) = m * mb
     ...
     !! 0 is a M-by-(N-M) zero matrix, if M < N, and is not stored.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
             call stdlib_I64_xerbla( 'DLASWLQ', -info )
             return
           else if (lquery) then
            return
           end if
           ...
           ! the lq decomposition
            if((m>=n).or.(nb<=m).or.(nb>=n)) then
             call stdlib_I64_dgelqt( m, n, mb, a, lda, t, ldt, work, info)
             return
            end if
            kk = mod((n-m),(nb-m))
            ii=n-kk+1
            ! compute the lq factorization of the first block a(1:m,1:nb)
            call stdlib_I64_dgelqt( m, nb, mb, a(1_ilp64,1_ilp64), lda, t, ldt, work, info)
            ctr = 1_ilp64
            do i = nb+1, ii-nb+m , (nb-m)
            ! compute the qr factorization of the current block a(1:m,i:i+nb-m)
              call stdlib_I64_dtplqt( m, nb-m, 0_ilp64, mb, a(1_ilp64,1_ilp64), lda, a( 1_ilp64, i ),lda, t(1_ilp64, ctr * m +&
                  & 1_ilp64),&
                        ldt, work, info )
              ctr = ctr + 1_ilp64
            end do
           ! compute the qr factorization of the last block a(1:m,ii:n)
            if (ii<=n) then
             call stdlib_I64_dtplqt( m, kk, 0_ilp64, mb, a(1_ilp64,1_ilp64), lda, a( 1_ilp64, ii ),lda, t(1_ilp64, ctr * m +&
                 & 1_ilp64), &
                       ldt,work, info )
            end if
           work( 1_ilp64 ) = m * mb
     ...
     !! 0 is a M-by-(N-M) zero matrix, if M < N, and is not stored.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
             call stdlib_I64_xerbla( 'CLASWLQ', -info )
             return
           else if (lquery) then
            return
           end if
           ...
           ! the lq decomposition
            if((m>=n).or.(nb<=m).or.(nb>=n)) then
             call stdlib_I64_cgelqt( m, n, mb, a, lda, t, ldt, work, info)
             return
            end if
            kk = mod((n-m),(nb-m))
            ii=n-kk+1
            ! compute the lq factorization of the first block a(1:m,1:nb)
            call stdlib_I64_cgelqt( m, nb, mb, a(1_ilp64,1_ilp64), lda, t, ldt, work, info)
            ctr = 1_ilp64
            do i = nb+1, ii-nb+m , (nb-m)
            ! compute the qr factorization of the current block a(1:m,i:i+nb-m)
              call stdlib_I64_ctplqt( m, nb-m, 0_ilp64, mb, a(1_ilp64,1_ilp64), lda, a( 1_ilp64, i ),lda, t(1_ilp64,ctr*m+1),ldt, &
                        work, info )
              ctr = ctr + 1_ilp64
            end do
           ! compute the qr factorization of the last block a(1:m,ii:n)
            if (ii<=n) then
             call stdlib_I64_ctplqt( m, kk, 0_ilp64, mb, a(1_ilp64,1_ilp64), lda, a( 1_ilp64, ii ),lda, t(1_ilp64,ctr*m+1), ldt,&
                       work, info )
            end if
           work( 1_ilp64 ) = m * mb
           return
     ...
     !! 0 is a M-by-(N-M) zero matrix, if M < N, and is not stored.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
             call stdlib_I64_xerbla( 'ZLASWLQ', -info )
             return
           else if (lquery) then
            return
           end if
           ...
           ! the lq decomposition
            if((m>=n).or.(nb<=m).or.(nb>=n)) then
             call stdlib_I64_zgelqt( m, n, mb, a, lda, t, ldt, work, info)
             return
            end if
            kk = mod((n-m),(nb-m))
            ii=n-kk+1
            ! compute the lq factorization of the first block a(1:m,1:nb)
            call stdlib_I64_zgelqt( m, nb, mb, a(1_ilp64,1_ilp64), lda, t, ldt, work, info)
            ctr = 1_ilp64
            do i = nb+1, ii-nb+m , (nb-m)
            ! compute the qr factorization of the current block a(1:m,i:i+nb-m)
              call stdlib_I64_ztplqt( m, nb-m, 0_ilp64, mb, a(1_ilp64,1_ilp64), lda, a( 1_ilp64, i ),lda, t(1_ilp64, ctr * m +&
                  & 1_ilp64),&
                        ldt, work, info )
              ctr = ctr + 1_ilp64
            end do
           ! compute the qr factorization of the last block a(1:m,ii:n)
            if (ii<=n) then
             call stdlib_I64_ztplqt( m, kk, 0_ilp64, mb, a(1_ilp64,1_ilp64), lda, a( 1_ilp64, ii ),lda, t(1_ilp64, ctr * m +&
                 & 1_ilp64), &
                       ldt,work, info )
            end if
           work( 1_ilp64 ) = m * mb
     ...
     !! factorization (SLASWLQ)
       ...
       ! =====================================================================
           ! Local Scalars 
           logical(lk) :: left, right, tran, notran, lquery
           integer(ilp64) :: i, ii, kk, lw, ctr
           ! External Subroutines 
           ! Executable Statements 
           ! test the input arguments
           lquery  = lwork<0_ilp64
           notran  = stdlib_lsame( trans, 'N' )
           tran    = stdlib_lsame( trans, 'T' )
           left    = stdlib_lsame( side, 'L' )
           right   = stdlib_lsame( side, 'R' )
           ...
           if( .not.left .and. .not.right ) then
              ...
              info = -2_ilp64
           else if( k<0_ilp64 ) then
             info = -5_ilp64
           else if( m<k ) then
             info = -3_ilp64
           else if( n<0_ilp64 ) then
             info = -4_ilp64
           else if( k<mb .or. mb<1_ilp64) then
             info = -6_ilp64
           else if( lda<max( 1_ilp64, k ) ) then
             info = -9_ilp64
           else if( ldt<max( 1_ilp64, mb) ) then
             info = -11_ilp64
           else if( ldc<max( 1_ilp64, m ) ) then
              info = -13_ilp64
           else if(( lwork<max(1_ilp64,lw)).and.(.not.lquery)) then
           ...
           if( info/=0_ilp64 ) then
             call stdlib_I64_xerbla( 'SLAMSWLQ', -info )
             work(1_ilp64) = lw
             return
           else if (lquery) then
             work(1_ilp64) = lw
           ...
           if((nb<=k).or.(nb>=max(m,n,k))) then
             call stdlib_I64_sgemlqt( side, trans, m, n, k, mb, a, lda,t, ldt, c, ldc, work, info)
           ...
           if(left.and.tran) then
               ...
               if (kk>0_ilp64) then
                 ii=m-kk+1
                 call stdlib_I64_stpmlqt('L','T',kk , n, k, 0_ilp64, mb, a(1_ilp64,ii), lda,t(1_ilp64,ctr*k+1), ldt, c(&
                           1_ilp64,1_ilp64), ldc,c(ii,1_ilp64), ldc, work, info )
               else
                 ii=m+1
               end if
               do i=ii-(nb-k),nb+1,-(nb-k)
               ! multiply q to the current block of c (1:m,i:i+nb)
                 ctr = ctr - 1_ilp64
                 call stdlib_I64_stpmlqt('L','T',nb-k , n, k, 0_ilp64,mb, a(1_ilp64,i), lda,t(1_ilp64,ctr*k+1),ldt, c(1_ilp64,&
                           1_ilp64), ldc,c(i,1_ilp64), ldc, work, info )
               end do
               ! multiply q to the first block of c (1:m,1:nb)
               call stdlib_I64_sgemlqt('L','T',nb , n, k, mb, a(1_ilp64,1_ilp64), lda, t,ldt ,c(1_ilp64,1_ilp64), ldc, work, &
                         info )
           else if (left.and.notran) then
               ! multiply q to the first block of c
              kk = mod((m-k),(nb-k))
              ii=m-kk+1
              ctr = 1_ilp64
              call stdlib_I64_sgemlqt('L','N',nb , n, k, mb, a(1_ilp64,1_ilp64), lda, t,ldt ,c(1_ilp64,1_ilp64), ldc, work, &
                        info )
              do i=nb+1,ii-nb+k,(nb-k)
               ! multiply q to the current block of c (i:i+nb,1:n)
               call stdlib_I64_stpmlqt('L','N',nb-k , n, k, 0_ilp64,mb, a(1_ilp64,i), lda,t(1_ilp64,ctr * k+1), ldt, c(&
                         1_ilp64,1_ilp64), ldc,c(i,1_ilp64), ldc, work, info )
               ctr = ctr + 1_ilp64
              end do
              if(ii<=m) then
               ! multiply q to the last block of c
               call stdlib_I64_stpmlqt('L','N',kk , n, k, 0_ilp64, mb, a(1_ilp64,ii), lda,t(1_ilp64,ctr*k+1), ldt, c(1_ilp64,&
                         1_ilp64), ldc,c(ii,1_ilp64), ldc, work, info )
              end if
           else if(right.and.notran) then
               ! multiply q to the last block of c
               kk = mod((n-k),(nb-k))
               ...
               if (kk>0_ilp64) then
                 ii=n-kk+1
                 call stdlib_I64_stpmlqt('R','N',m , kk, k, 0_ilp64, mb, a(1_ilp64, ii), lda,t(1_ilp64,ctr*k+1), ldt, c(&
                           1_ilp64,1_ilp64), ldc,c(1_ilp64,ii), ldc, work, info )
               else
                 ii=n+1
               end if
               do i=ii-(nb-k),nb+1,-(nb-k)
               ! multiply q to the current block of c (1:m,i:i+mb)
                  ctr = ctr - 1_ilp64
                  call stdlib_I64_stpmlqt('R','N', m, nb-k, k, 0_ilp64, mb, a(1_ilp64, i), lda,t(1_ilp64,ctr*k+1), ldt, &
                            c(1_ilp64,1_ilp64), ldc,c(1_ilp64,i), ldc, work, info )
               end do
               ! multiply q to the first block of c (1:m,1:mb)
               call stdlib_I64_sgemlqt('R','N',m , nb, k, mb, a(1_ilp64,1_ilp64), lda, t,ldt ,c(1_ilp64,1_ilp64), ldc, work, &
                         info )
           else if (right.and.tran) then
             ! multiply q to the first block of c
              kk = mod((n-k),(nb-k))
              ii=n-kk+1
              ctr = 1_ilp64
              call stdlib_I64_sgemlqt('R','T',m , nb, k, mb, a(1_ilp64,1_ilp64), lda, t,ldt ,c(1_ilp64,1_ilp64), ldc, work, &
                        info )
              do i=nb+1,ii-nb+k,(nb-k)
               ! multiply q to the current block of c (1:m,i:i+mb)
               call stdlib_I64_stpmlqt('R','T',m , nb-k, k, 0_ilp64,mb, a(1_ilp64,i), lda,t(1_ilp64, ctr*k+1), ldt, c(1_ilp64,&
                         1_ilp64), ldc,c(1_ilp64,i), ldc, work, info )
               ctr = ctr + 1_ilp64
              end do
              if(ii<=n) then
             ! multiply q to the last block of c
               call stdlib_I64_stpmlqt('R','T',m , kk, k, 0_ilp64,mb, a(1_ilp64,ii), lda,t(1_ilp64,ctr*k+1),ldt,&
                   & c(1_ilp64,1_ilp64),&
                          ldc,c(1_ilp64,ii), ldc, work, info )
              end if
           end if
           work(1_ilp64) = lw
           return
     ...
     !! factorization (DLASWLQ)
       ...
       ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
             call stdlib_I64_xerbla( 'DLAMSWLQ', -info )
             work(1_ilp64) = lw
             return
           else if (lquery) then
             work(1_ilp64) = lw
           ...
           if((nb<=k).or.(nb>=max(m,n,k))) then
             call stdlib_I64_dgemlqt( side, trans, m, n, k, mb, a, lda,t, ldt, c, ldc, work, info)
           ...
           if(left.and.tran) then
               ...
               if (kk>0_ilp64) then
                 ii=m-kk+1
                 call stdlib_I64_dtpmlqt('L','T',kk , n, k, 0_ilp64, mb, a(1_ilp64,ii), lda,t(1_ilp64,ctr*k+1), ldt, c(&
                           1_ilp64,1_ilp64), ldc,c(ii,1_ilp64), ldc, work, info )
               else
                 ii=m+1
               end if
               ...
               ! multiply q to the current block of c (1:m,i:i+nb)
                 ctr = ctr - 1_ilp64
                 call stdlib_I64_dtpmlqt('L','T',nb-k , n, k, 0_ilp64,mb, a(1_ilp64,i), lda,t(1_ilp64, ctr*k+1),ldt, c(&
                           1_ilp64,1_ilp64), ldc,c(i,1_ilp64), ldc, work, info )
               end do
               ! multiply q to the first block of c (1:m,1:nb)
               call stdlib_I64_dgemlqt('L','T',nb , n, k, mb, a(1_ilp64,1_ilp64), lda, t,ldt ,c(1_ilp64,1_ilp64), ldc, work, &
                         info )
           else if (left.and.notran) then
               ! multiply q to the first block of c
              kk = mod((m-k),(nb-k))
              ii=m-kk+1
              ctr = 1_ilp64
              call stdlib_I64_dgemlqt('L','N',nb , n, k, mb, a(1_ilp64,1_ilp64), lda, t,ldt ,c(1_ilp64,1_ilp64), ldc, work, &
                        info )
              do i=nb+1,ii-nb+k,(nb-k)
               ! multiply q to the current block of c (i:i+nb,1:n)
               call stdlib_I64_dtpmlqt('L','N',nb-k , n, k, 0_ilp64,mb, a(1_ilp64,i), lda,t(1_ilp64,ctr*k+1), ldt, c(1_ilp64,&
                         1_ilp64), ldc,c(i,1_ilp64), ldc, work, info )
               ctr = ctr + 1_ilp64
              end do
              if(ii<=m) then
               ! multiply q to the last block of c
               call stdlib_I64_dtpmlqt('L','N',kk , n, k, 0_ilp64, mb, a(1_ilp64,ii), lda,t(1_ilp64,ctr*k+1), ldt, c(1_ilp64,&
                         1_ilp64), ldc,c(ii,1_ilp64), ldc, work, info )
              end if
           else if(right.and.notran) then
               ! multiply q to the last block of c
               ...
               if (kk>0_ilp64) then
                 ii=n-kk+1
                 call stdlib_I64_dtpmlqt('R','N',m , kk, k, 0_ilp64, mb, a(1_ilp64, ii), lda,t(1_ilp64,ctr *k+1), ldt, &
                           c(1_ilp64,1_ilp64), ldc,c(1_ilp64,ii), ldc, work, info )
               else
                 ii=n+1
               end if
               do i=ii-(nb-k),nb+1,-(nb-k)
               ! multiply q to the current block of c (1:m,i:i+mb)
                  ctr = ctr - 1_ilp64
                  call stdlib_I64_dtpmlqt('R','N', m, nb-k, k, 0_ilp64, mb, a(1_ilp64, i), lda,t(1_ilp64,ctr*k+1), ldt, &
                            c(1_ilp64,1_ilp64), ldc,c(1_ilp64,i), ldc, work, info )
               end do
               ! multiply q to the first block of c (1:m,1:mb)
               call stdlib_I64_dgemlqt('R','N',m , nb, k, mb, a(1_ilp64,1_ilp64), lda, t,ldt ,c(1_ilp64,1_ilp64), ldc, work, &
                         info )
           else if (right.and.tran) then
             ! multiply q to the first block of c
              kk = mod((n-k),(nb-k))
              ctr = 1_ilp64
              ii=n-kk+1
              call stdlib_I64_dgemlqt('R','T',m , nb, k, mb, a(1_ilp64,1_ilp64), lda, t,ldt ,c(1_ilp64,1_ilp64), ldc, work, &
                        info )
              do i=nb+1,ii-nb+k,(nb-k)
               ! multiply q to the current block of c (1:m,i:i+mb)
               call stdlib_I64_dtpmlqt('R','T',m , nb-k, k, 0_ilp64,mb, a(1_ilp64,i), lda,t(1_ilp64,ctr*k+1), ldt, c(1_ilp64,&
                         1_ilp64), ldc,c(1_ilp64,i), ldc, work, info )
               ctr = ctr + 1_ilp64
              end do
              if(ii<=n) then
             ! multiply q to the last block of c
               call stdlib_I64_dtpmlqt('R','T',m , kk, k, 0_ilp64,mb, a(1_ilp64,ii), lda,t(1_ilp64,ctr*k+1),ldt,&
                   & c(1_ilp64,1_ilp64),&
                          ldc,c(1_ilp64,ii), ldc, work, info )
              end if
           end if
     ...
     !! factorization (CLASWLQ)
       ...
       ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
             call stdlib_I64_xerbla( 'CLAMSWLQ', -info )
             work(1_ilp64) = lw
             return
           else if (lquery) then
             work(1_ilp64) = lw
           ...
           if((nb<=k).or.(nb>=max(m,n,k))) then
             call stdlib_I64_cgemlqt( side, trans, m, n, k, mb, a, lda,t, ldt, c, ldc, work, info)
           ...
           if(left.and.tran) then
               ...
               if (kk>0_ilp64) then
                 ii=m-kk+1
                 call stdlib_I64_ctpmlqt('L','C',kk , n, k, 0_ilp64, mb, a(1_ilp64,ii), lda,t(1_ilp64,ctr*k+1), ldt, c(&
                           1_ilp64,1_ilp64), ldc,c(ii,1_ilp64), ldc, work, info )
               else
                 ii=m+1
               end if
               ...
               ! multiply q to the current block of c (1:m,i:i+nb)
                 ctr = ctr - 1_ilp64
                 call stdlib_I64_ctpmlqt('L','C',nb-k , n, k, 0_ilp64,mb, a(1_ilp64,i), lda,t(1_ilp64,ctr*k+1),ldt, c(1_ilp64,&
                           1_ilp64), ldc,c(i,1_ilp64), ldc, work, info )
               end do
               ! multiply q to the first block of c (1:m,1:nb)
               call stdlib_I64_cgemlqt('L','C',nb , n, k, mb, a(1_ilp64,1_ilp64), lda, t,ldt ,c(1_ilp64,1_ilp64), ldc, work, &
                         info )
           else if (left.and.notran) then
               ! multiply q to the first block of c
              kk  = mod((m-k),(nb-k))
              ii  = m-kk+1
              ctr = 1_ilp64
              call stdlib_I64_cgemlqt('L','N',nb , n, k, mb, a(1_ilp64,1_ilp64), lda, t,ldt ,c(1_ilp64,1_ilp64), ldc, work, &
                        info )
              do i=nb+1,ii-nb+k,(nb-k)
               ! multiply q to the current block of c (i:i+nb,1:n)
               call stdlib_I64_ctpmlqt('L','N',nb-k , n, k, 0_ilp64,mb, a(1_ilp64,i), lda,t(1_ilp64, ctr *k+1), ldt, c(&
                         1_ilp64,1_ilp64), ldc,c(i,1_ilp64), ldc, work, info )
               ctr = ctr + 1_ilp64
              end do
              if(ii<=m) then
               ! multiply q to the last block of c
               call stdlib_I64_ctpmlqt('L','N',kk , n, k, 0_ilp64, mb, a(1_ilp64,ii), lda,t(1_ilp64, ctr*k+1), ldt, c(1_ilp64,&
                         1_ilp64), ldc,c(ii,1_ilp64), ldc, work, info )
              end if
           else if(right.and.notran) then
               ! multiply q to the last block of c
               ...
               if (kk>0_ilp64) then
                 ii=n-kk+1
                 call stdlib_I64_ctpmlqt('R','N',m , kk, k, 0_ilp64, mb, a(1_ilp64, ii), lda,t(1_ilp64,ctr*k+1), ldt, c(&
                           1_ilp64,1_ilp64), ldc,c(1_ilp64,ii), ldc, work, info )
               else
                 ii=n+1
               end if
               ...
               ! multiply q to the current block of c (1:m,i:i+mb)
                   ctr = ctr - 1_ilp64
                   call stdlib_I64_ctpmlqt('R','N', m, nb-k, k, 0_ilp64, mb, a(1_ilp64, i), lda,t(1_ilp64,ctr*k+1), ldt,&
                              c(1_ilp64,1_ilp64), ldc,c(1_ilp64,i), ldc, work, info )
               end do
               ! multiply q to the first block of c (1:m,1:mb)
               call stdlib_I64_cgemlqt('R','N',m , nb, k, mb, a(1_ilp64,1_ilp64), lda, t,ldt ,c(1_ilp64,1_ilp64), ldc, work, &
                         info )
           else if (right.and.tran) then
             ! multiply q to the first block of c
              kk = mod((n-k),(nb-k))
              ii=n-kk+1
              ctr = 1_ilp64
              call stdlib_I64_cgemlqt('R','C',m , nb, k, mb, a(1_ilp64,1_ilp64), lda, t,ldt ,c(1_ilp64,1_ilp64), ldc, work, &
                        info )
              do i=nb+1,ii-nb+k,(nb-k)
               ! multiply q to the current block of c (1:m,i:i+mb)
               call stdlib_I64_ctpmlqt('R','C',m , nb-k, k, 0_ilp64,mb, a(1_ilp64,i), lda,t(1_ilp64,ctr*k+1), ldt, c(1_ilp64,&
                         1_ilp64), ldc,c(1_ilp64,i), ldc, work, info )
               ctr = ctr + 1_ilp64
              end do
              if(ii<=n) then
             ! multiply q to the last block of c
               call stdlib_I64_ctpmlqt('R','C',m , kk, k, 0_ilp64,mb, a(1_ilp64,ii), lda,t(1_ilp64,ctr*k+1),ldt,&
                   & c(1_ilp64,1_ilp64),&
                          ldc,c(1_ilp64,ii), ldc, work, info )
              end if
           end if
     ...
     !! factorization (ZLASWLQ)
       ...
       ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
             call stdlib_I64_xerbla( 'ZLAMSWLQ', -info )
             work(1_ilp64) = lw
             return
           else if (lquery) then
             work(1_ilp64) = lw
           ...
           if((nb<=k).or.(nb>=max(m,n,k))) then
             call stdlib_I64_zgemlqt( side, trans, m, n, k, mb, a, lda,t, ldt, c, ldc, work, info)
           ...
           if(left.and.tran) then
               ...
               if (kk>0_ilp64) then
                 ii=m-kk+1
                 call stdlib_I64_ztpmlqt('L','C',kk , n, k, 0_ilp64, mb, a(1_ilp64,ii), lda,t(1_ilp64,ctr*k+1), ldt, c(&
                           1_ilp64,1_ilp64), ldc,c(ii,1_ilp64), ldc, work, info )
               else
                 ii=m+1
               end if
               ...
               ! multiply q to the current block of c (1:m,i:i+nb)
                 ctr = ctr - 1_ilp64
                 call stdlib_I64_ztpmlqt('L','C',nb-k , n, k, 0_ilp64,mb, a(1_ilp64,i), lda,t(1_ilp64,ctr*k+1),ldt, c(1_ilp64,&
                           1_ilp64), ldc,c(i,1_ilp64), ldc, work, info )
               end do
               ! multiply q to the first block of c (1:m,1:nb)
               call stdlib_I64_zgemlqt('L','C',nb , n, k, mb, a(1_ilp64,1_ilp64), lda, t,ldt ,c(1_ilp64,1_ilp64), ldc, work, &
                         info )
           else if (left.and.notran) then
               ! multiply q to the first block of c
              kk = mod((m-k),(nb-k))
              ii=m-kk+1
              ctr = 1_ilp64
              call stdlib_I64_zgemlqt('L','N',nb , n, k, mb, a(1_ilp64,1_ilp64), lda, t,ldt ,c(1_ilp64,1_ilp64), ldc, work, &
                        info )
              do i=nb+1,ii-nb+k,(nb-k)
               ! multiply q to the current block of c (i:i+nb,1:n)
               call stdlib_I64_ztpmlqt('L','N',nb-k , n, k, 0_ilp64,mb, a(1_ilp64,i), lda,t(1_ilp64, ctr * k + 1_ilp64), ldt, &
                         c(1_ilp64,1_ilp64), ldc,c(i,1_ilp64), ldc, work, info )
               ctr = ctr + 1_ilp64
              end do
              if(ii<=m) then
               ! multiply q to the last block of c
               call stdlib_I64_ztpmlqt('L','N',kk , n, k, 0_ilp64, mb, a(1_ilp64,ii), lda,t(1_ilp64, ctr * k + 1_ilp64), ldt, &
                         c(1_ilp64,1_ilp64), ldc,c(ii,1_ilp64), ldc, work, info )
              end if
           else if(right.and.notran) then
               ! multiply q to the last block of c
               kk = mod((n-k),(nb-k))
               ...
               if (kk>0_ilp64) then
                 ii=n-kk+1
                 call stdlib_I64_ztpmlqt('R','N',m , kk, k, 0_ilp64, mb, a(1_ilp64, ii), lda,t(1_ilp64, ctr * k + 1_ilp64), &
                           ldt, c(1_ilp64,1_ilp64), ldc,c(1_ilp64,ii), ldc, work, info )
               else
                 ii=n+1
               end if
               do i=ii-(nb-k),nb+1,-(nb-k)
               ! multiply q to the current block of c (1:m,i:i+mb)
               ctr = ctr - 1_ilp64
               call stdlib_I64_ztpmlqt('R','N', m, nb-k, k, 0_ilp64, mb, a(1_ilp64, i), lda,t(1_ilp64, ctr * k + 1_ilp64), &
                         ldt, c(1_ilp64,1_ilp64), ldc,c(1_ilp64,i), ldc, work, info )
               end do
               ! multiply q to the first block of c (1:m,1:mb)
               call stdlib_I64_zgemlqt('R','N',m , nb, k, mb, a(1_ilp64,1_ilp64), lda, t,ldt ,c(1_ilp64,1_ilp64), ldc, work, &
                         info )
           else if (right.and.tran) then
             ! multiply q to the first block of c
              kk = mod((n-k),(nb-k))
              ii=n-kk+1
              call stdlib_I64_zgemlqt('R','C',m , nb, k, mb, a(1_ilp64,1_ilp64), lda, t,ldt ,c(1_ilp64,1_ilp64), ldc, work, &
                        info )
              ctr = 1_ilp64
              do i=nb+1,ii-nb+k,(nb-k)
               ! multiply q to the current block of c (1:m,i:i+mb)
               call stdlib_I64_ztpmlqt('R','C',m , nb-k, k, 0_ilp64,mb, a(1_ilp64,i), lda,t(1_ilp64,ctr *k+1), ldt, c(1_ilp64,&
                         1_ilp64), ldc,c(1_ilp64,i), ldc, work, info )
               ctr = ctr + 1_ilp64
              end do
              if(ii<=n) then
             ! multiply q to the last block of c
               call stdlib_I64_ztpmlqt('R','C',m , kk, k, 0_ilp64,mb, a(1_ilp64,ii), lda,t(1_ilp64, ctr * k + 1_ilp64),ldt, c(&
                         1_ilp64,1_ilp64), ldc,c(1_ilp64,ii), ldc, work, info )
              end if
           end if
           work(1_ilp64) = lw
     ...
     !! WY representation for Q.
       ...
       ! =====================================================================
           ! Local Scalars 
           integer(ilp64) :: i, ib, lb, nb, iinfo
           ! Executable Statements 
           ! test the input arguments
           info = 0_ilp64
           if( m<0_ilp64 ) then
              ...
              info = -2_ilp64
           else if( l<0_ilp64 .or. (l>min(m,n) .and. min(m,n)>=0_ilp64)) then
              info = -3_ilp64
           else if( mb<1_ilp64 .or. (mb>m .and. m>0_ilp64)) then
              info = -4_ilp64
           else if( lda<max( 1_ilp64, m ) ) then
              info = -6_ilp64
           else if( ldb<max( 1_ilp64, m ) ) then
              info = -8_ilp64
           else if( ldt<mb ) then
              info = -10_ilp64
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'STPLQT', -info )
              return
           end if
           ! quick return if possible
           if( m==0 .or. n==0 ) return
           ...
           ! compute the qr factorization of the current block
              ...
              call stdlib_I64_stplqt2( ib, nb, lb, a(i,i), lda, b( i, 1_ilp64 ), ldb,t(1_ilp64, i ), ldt, iinfo )
                        
           ! update by applying h**t to b(i+ib:m,:) from the right
              if( i+ib<=m ) then
                 call stdlib_I64_stprfb( 'R', 'N', 'F', 'R', m-i-ib+1, nb, ib, lb,b( i, 1_ilp64 ), ldb, t( &
                           1_ilp64, i ), ldt,a( i+ib, i ), lda, b( i+ib, 1_ilp64 ), ldb,work, m-i-ib+1)
              end if
           end do
           return
     ...
     !! WY representation for Q.
       ...
       ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DTPLQT', -info )
              return
           end if
           ! quick return if possible
           if( m==0 .or. n==0 ) return
           ...
           ! compute the qr factorization of the current block
              ...
              call stdlib_I64_dtplqt2( ib, nb, lb, a(i,i), lda, b( i, 1_ilp64 ), ldb,t(1_ilp64, i ), ldt, iinfo )
                        
           ! update by applying h**t to b(i+ib:m,:) from the right
              if( i+ib<=m ) then
                 call stdlib_I64_dtprfb( 'R', 'N', 'F', 'R', m-i-ib+1, nb, ib, lb,b( i, 1_ilp64 ), ldb, t( &
                           1_ilp64, i ), ldt,a( i+ib, i ), lda, b( i+ib, 1_ilp64 ), ldb,work, m-i-ib+1)
              end if
           end do
           return
     ...
     !! WY representation for Q.
       ...
       ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CTPLQT', -info )
              return
           end if
           ! quick return if possible
           if( m==0 .or. n==0 ) return
           ...
           ! compute the qr factorization of the current block
              ...
              call stdlib_I64_ctplqt2( ib, nb, lb, a(i,i), lda, b( i, 1_ilp64 ), ldb,t(1_ilp64, i ), ldt, iinfo )
                        
           ! update by applying h**t to b(i+ib:m,:) from the right
              if( i+ib<=m ) then
                 call stdlib_I64_ctprfb( 'R', 'N', 'F', 'R', m-i-ib+1, nb, ib, lb,b( i, 1_ilp64 ), ldb, t( &
                           1_ilp64, i ), ldt,a( i+ib, i ), lda, b( i+ib, 1_ilp64 ), ldb,work, m-i-ib+1)
              end if
           end do
           return
     ...
     !! WY representation for Q.
       ...
       ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZTPLQT', -info )
              return
           end if
           ! quick return if possible
           if( m==0 .or. n==0 ) return
           ...
           ! compute the qr factorization of the current block
              ...
              call stdlib_I64_ztplqt2( ib, nb, lb, a(i,i), lda, b( i, 1_ilp64 ), ldb,t(1_ilp64, i ), ldt, iinfo )
                        
           ! update by applying h**t to b(i+ib:m,:) from the right
              if( i+ib<=m ) then
                 call stdlib_I64_ztprfb( 'R', 'N', 'F', 'R', m-i-ib+1, nb, ib, lb,b( i, 1_ilp64 ), ldb, t( &
                           1_ilp64, i ), ldt,a( i+ib, i ), lda, b( i+ib, 1_ilp64 ), ldb,work, m-i-ib+1)
              end if
           end do
           return
     ...
     !! using the compact WY representation for Q.
        ...
        ! =====================================================================
           ...
           if( m<0_ilp64 ) then
              ...
              info = -2_ilp64
           else if( l<0_ilp64 .or. l>min(m,n) ) then
              info = -3_ilp64
           else if( lda<max( 1_ilp64, m ) ) then
              info = -5_ilp64
           else if( ldb<max( 1_ilp64, m ) ) then
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'STPLQT2', -info )
              return
           end if
           ! quick return if possible
           if( n==0 .or. m==0 ) return
           do i = 1, m
              ! generate elementary reflector h(i) to annihilate b(i,:)
              p = n-l+min( l, i )
              call stdlib_I64_slarfg( p+1, a( i, i ), b( i, 1_ilp64 ), ldb, t( 1_ilp64, i ) )
              if( i<m ) then
                 ! w(m-i:1) := c(i+1:m,i:n) * c(i,i:n) [use w = t(m,:)]
                 do j = 1, m-i
                    t( m, j ) = (a( i+j, i ))
                 end do
                 call stdlib_I64_sgemv( 'N', m-i, p, one, b( i+1, 1_ilp64 ), ldb,b( i, 1_ilp64 ), ldb, one, t( m, &
                           1_ilp64 ), ldt )
                 ! c(i+1:m,i:n) = c(i+1:m,i:n) + alpha * c(i,i:n)*w(m-1:1)^h
                 alpha = -(t( 1_ilp64, i ))
                 do j = 1, m-i
                 ...
                 call stdlib_I64_sger( m-i, p, alpha,  t( m, 1_ilp64 ), ldt,b( i, 1_ilp64 ), ldb, b( i+1, 1_ilp64 ), &
                           ldb )
              end if
           end do
           do i = 2, m
              ...
              call stdlib_I64_strmv( 'L', 'N', 'N', p, b( 1_ilp64, np ), ldb,t( i, 1_ilp64 ), ldt )
              ! rectangular part of b2
              call stdlib_I64_sgemv( 'N', i-1-p, l,  alpha, b( mp, np ), ldb,b( i, np ), ldb, zero, t(&
                         i,mp ), ldt )
              ! b1
              call stdlib_I64_sgemv( 'N', i-1, n-l, alpha, b, ldb, b( i, 1_ilp64 ), ldb,one, t( i, 1_ilp64 ), ldt &
                        )
              ! t(1:i-1,i) := t(1:i-1,1:i-1) * t(i,1:i-1)
             call stdlib_I64_strmv( 'L', 'T', 'N', i-1, t, ldt, t( i, 1_ilp64 ), ldt )
              ! t(i,i) = tau(i)
              t( i, i ) = t( 1_ilp64, i )
              t( 1_ilp64, i ) = zero
           end do
     ...
     !! using the compact WY representation for Q.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DTPLQT2', -info )
              return
           end if
           ! quick return if possible
           if( n==0 .or. m==0 ) return
           do i = 1, m
              ! generate elementary reflector h(i) to annihilate b(i,:)
              p = n-l+min( l, i )
              call stdlib_I64_dlarfg( p+1, a( i, i ), b( i, 1_ilp64 ), ldb, t( 1_ilp64, i ) )
              if( i<m ) then
                 ! w(m-i:1) := c(i+1:m,i:n) * c(i,i:n) [use w = t(m,:)]
                 do j = 1, m-i
                    t( m, j ) = (a( i+j, i ))
                 end do
                 call stdlib_I64_dgemv( 'N', m-i, p, one, b( i+1, 1_ilp64 ), ldb,b( i, 1_ilp64 ), ldb, one, t( m, &
                           1_ilp64 ), ldt )
                 ! c(i+1:m,i:n) = c(i+1:m,i:n) + alpha * c(i,i:n)*w(m-1:1)^h
                 alpha = -(t( 1_ilp64, i ))
                 do j = 1, m-i
                 ...
                 call stdlib_I64_dger( m-i, p, alpha,  t( m, 1_ilp64 ), ldt,b( i, 1_ilp64 ), ldb, b( i+1, 1_ilp64 ), &
                           ldb )
              end if
           end do
           do i = 2, m
              ...
              call stdlib_I64_dtrmv( 'L', 'N', 'N', p, b( 1_ilp64, np ), ldb,t( i, 1_ilp64 ), ldt )
              ! rectangular part of b2
              call stdlib_I64_dgemv( 'N', i-1-p, l,  alpha, b( mp, np ), ldb,b( i, np ), ldb, zero, t(&
                         i,mp ), ldt )
              ! b1
              call stdlib_I64_dgemv( 'N', i-1, n-l, alpha, b, ldb, b( i, 1_ilp64 ), ldb,one, t( i, 1_ilp64 ), ldt &
                        )
              ! t(1:i-1,i) := t(1:i-1,1:i-1) * t(i,1:i-1)
             call stdlib_I64_dtrmv( 'L', 'T', 'N', i-1, t, ldt, t( i, 1_ilp64 ), ldt )
              ! t(i,i) = tau(i)
              t( i, i ) = t( 1_ilp64, i )
              t( 1_ilp64, i ) = zero
           end do
     ...
     !! using the compact WY representation for Q.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CTPLQT2', -info )
              return
           end if
           ! quick return if possible
           if( n==0 .or. m==0 ) return
           do i = 1, m
              ! generate elementary reflector h(i) to annihilate b(i,:)
              p = n-l+min( l, i )
              call stdlib_I64_clarfg( p+1, a( i, i ), b( i, 1_ilp64 ), ldb, t( 1_ilp64, i ) )
              t(1_ilp64,i)=conjg(t(1_ilp64,i))
              if( i<m ) then
                 do j = 1, p
                    b( i, j ) = conjg(b(i,j))
                 end do
                 ...
                 call stdlib_I64_cgemv( 'N', m-i, p, cone, b( i+1, 1_ilp64 ), ldb,b( i, 1_ilp64 ), ldb, cone, t( &
                           m, 1_ilp64 ), ldt )
                 ! c(i+1:m,i:n) = c(i+1:m,i:n) + alpha * c(i,i:n)*w(m-1:1)^h
                 alpha = -(t( 1_ilp64, i ))
                 do j = 1, m-i
                 ...
                 call stdlib_I64_cgerc( m-i, p, (alpha),  t( m, 1_ilp64 ), ldt,b( i, 1_ilp64 ), ldb, b( i+1, 1_ilp64 ), &
                           ldb )
                 do j = 1, p
                    b( i, j ) = conjg(b(i,j))
                 end do
           ...
           do i = 2, m
              ...
              call stdlib_I64_ctrmv( 'L', 'N', 'N', p, b( 1_ilp64, np ), ldb,t( i, 1_ilp64 ), ldt )
              ! rectangular part of b2
              call stdlib_I64_cgemv( 'N', i-1-p, l,  alpha, b( mp, np ), ldb,b( i, np ), ldb, czero, &
                        t( i,mp ), ldt )
              ! b1
              call stdlib_I64_cgemv( 'N', i-1, n-l, alpha, b, ldb, b( i, 1_ilp64 ), ldb,cone, t( i, 1_ilp64 ), &
                        ldt )
              ! t(1:i-1,i) := t(1:i-1,1:i-1) * t(i,1:i-1)
              do j = 1, i-1
                 t(i,j)=conjg(t(i,j))
              end do
              call stdlib_I64_ctrmv( 'L', 'C', 'N', i-1, t, ldt, t( i, 1_ilp64 ), ldt )
              do j = 1, i-1
                 t(i,j)=conjg(t(i,j))
              end do
              do j = 1, n-l+p
     ...
     !! using the compact WY representation for Q.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZTPLQT2', -info )
              return
           end if
           ! quick return if possible
           if( n==0 .or. m==0 ) return
           do i = 1, m
              ! generate elementary reflector h(i) to annihilate b(i,:)
              p = n-l+min( l, i )
              call stdlib_I64_zlarfg( p+1, a( i, i ), b( i, 1_ilp64 ), ldb, t( 1_ilp64, i ) )
              t(1_ilp64,i)=conjg(t(1_ilp64,i))
              if( i<m ) then
                 do j = 1, p
                    b( i, j ) = conjg(b(i,j))
                 ...
                 call stdlib_I64_zgemv( 'N', m-i, p, cone, b( i+1, 1_ilp64 ), ldb,b( i, 1_ilp64 ), ldb, cone, t( &
                           m, 1_ilp64 ), ldt )
                 ! c(i+1:m,i:n) = c(i+1:m,i:n) + alpha * c(i,i:n)*w(m-1:1)^h
                 alpha = -(t( 1_ilp64, i ))
                 do j = 1, m-i
                 ...
                 call stdlib_I64_zgerc( m-i, p, (alpha),  t( m, 1_ilp64 ), ldt,b( i, 1_ilp64 ), ldb, b( i+1, 1_ilp64 ), &
                           ldb )
                 do j = 1, p
                    b( i, j ) = conjg(b(i,j))
                 end do
           ...
           do i = 2, m
              ...
              call stdlib_I64_ztrmv( 'L', 'N', 'N', p, b( 1_ilp64, np ), ldb,t( i, 1_ilp64 ), ldt )
              ! rectangular part of b2
              call stdlib_I64_zgemv( 'N', i-1-p, l,  alpha, b( mp, np ), ldb,b( i, np ), ldb, czero, &
                        t( i,mp ), ldt )
              ! b1
              call stdlib_I64_zgemv( 'N', i-1, n-l, alpha, b, ldb, b( i, 1_ilp64 ), ldb,cone, t( i, 1_ilp64 ), &
                        ldt )
              ! t(1:i-1,i) := t(1:i-1,1:i-1) * t(i,1:i-1)
              do j = 1, i-1
                 t(i,j)=conjg(t(i,j))
              end do
              call stdlib_I64_ztrmv( 'L', 'C', 'N', i-1, t, ldt, t( i, 1_ilp64 ), ldt )
              do j = 1, i-1
                 t(i,j)=conjg(t(i,j))
              end do
              do j = 1, n-l+p
     ...
     !! real matrix C, which consists of two blocks A and B.
        ...
        ! =====================================================================
           ! Local Scalars 
           logical(lk) :: left, right, tran, notran
           integer(ilp64) :: i, ib, nb, lb, kf, ldaq
           ! Intrinsic Functions 
           ! Executable Statements 
           ! Test The Input Arguments 
           info   = 0_ilp64
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'STPMLQT', -info )
              return
           end if
           ! Quick Return If Possible 
           if( m==0 .or. n==0 .or. k==0 ) return
           if( left .and. notran ) then
              do i = 1, k, mb
                 ...
                 call stdlib_I64_stprfb( 'L', 'T', 'F', 'R', nb, n, ib, lb,v( i, 1_ilp64 ), ldv, t( 1_ilp64, i ), &
                           ldt,a( i, 1_ilp64 ), lda, b, ldb, work, ib )
              end do
           else if( right .and. tran ) then
              do i = 1, k, mb
                 ib = min( mb, k-i+1 )
                 ...
                 call stdlib_I64_stprfb( 'R', 'N', 'F', 'R', m, nb, ib, lb,v( i, 1_ilp64 ), ldv, t( 1_ilp64, i ), &
                           ldt,a( 1_ilp64, i ), lda, b, ldb, work, m )
              end do
           else if( left .and. tran ) then
              kf = ((k-1)/mb)*mb+1
              do i = kf, 1, -mb
                 ...
                 call stdlib_I64_stprfb( 'L', 'N', 'F', 'R', nb, n, ib, lb,v( i, 1_ilp64 ), ldv, t( 1_ilp64, i ), &
                           ldt,a( i, 1_ilp64 ), lda, b, ldb, work, ib )
              end do
           else if( right .and. notran ) then
              kf = ((k-1)/mb)*mb+1
              do i = kf, 1, -mb
                 ...
                 call stdlib_I64_stprfb( 'R', 'T', 'F', 'R', m, nb, ib, lb,v( i, 1_ilp64 ), ldv, t( 1_ilp64, i ), &
                           ldt,a( 1_ilp64, i ), lda, b, ldb, work, m )
              end do
           end if
           return
     ...
     !! real matrix C, which consists of two blocks A and B.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DTPMLQT', -info )
              return
           end if
           ! Quick Return If Possible 
           if( m==0 .or. n==0 .or. k==0 ) return
           if( left .and. notran ) then
              do i = 1, k, mb
                 ...
                 call stdlib_I64_dtprfb( 'L', 'T', 'F', 'R', nb, n, ib, lb,v( i, 1_ilp64 ), ldv, t( 1_ilp64, i ), &
                           ldt,a( i, 1_ilp64 ), lda, b, ldb, work, ib )
              end do
           else if( right .and. tran ) then
              do i = 1, k, mb
                 ...
                 call stdlib_I64_dtprfb( 'R', 'N', 'F', 'R', m, nb, ib, lb,v( i, 1_ilp64 ), ldv, t( 1_ilp64, i ), &
                           ldt,a( 1_ilp64, i ), lda, b, ldb, work, m )
              end do
           else if( left .and. tran ) then
              kf = ((k-1)/mb)*mb+1
              do i = kf, 1, -mb
                 ...
                 call stdlib_I64_dtprfb( 'L', 'N', 'F', 'R', nb, n, ib, lb,v( i, 1_ilp64 ), ldv, t( 1_ilp64, i ), &
                           ldt,a( i, 1_ilp64 ), lda, b, ldb, work, ib )
              end do
           else if( right .and. notran ) then
              kf = ((k-1)/mb)*mb+1
              do i = kf, 1, -mb
                 ...
                 call stdlib_I64_dtprfb( 'R', 'T', 'F', 'R', m, nb, ib, lb,v( i, 1_ilp64 ), ldv, t( 1_ilp64, i ), &
                           ldt,a( 1_ilp64, i ), lda, b, ldb, work, m )
              end do
           end if
           return
     ...
     !! complex matrix C, which consists of two blocks A and B.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CTPMLQT', -info )
              return
           end if
           ! Quick Return If Possible 
           if( m==0 .or. n==0 .or. k==0 ) return
           if( left .and. notran ) then
              do i = 1, k, mb
                 ...
                 call stdlib_I64_ctprfb( 'L', 'C', 'F', 'R', nb, n, ib, lb,v( i, 1_ilp64 ), ldv, t( 1_ilp64, i ), &
                           ldt,a( i, 1_ilp64 ), lda, b, ldb, work, ib )
              end do
           else if( right .and. tran ) then
              do i = 1, k, mb
                 ...
                 call stdlib_I64_ctprfb( 'R', 'N', 'F', 'R', m, nb, ib, lb,v( i, 1_ilp64 ), ldv, t( 1_ilp64, i ), &
                           ldt,a( 1_ilp64, i ), lda, b, ldb, work, m )
              end do
           else if( left .and. tran ) then
              kf = ((k-1)/mb)*mb+1
              do i = kf, 1, -mb
                 ...
                 call stdlib_I64_ctprfb( 'L', 'N', 'F', 'R', nb, n, ib, lb,v( i, 1_ilp64 ), ldv, t( 1_ilp64, i ), &
                           ldt,a( i, 1_ilp64 ), lda, b, ldb, work, ib )
              end do
           else if( right .and. notran ) then
              kf = ((k-1)/mb)*mb+1
              do i = kf, 1, -mb
                 ...
                 call stdlib_I64_ctprfb( 'R', 'C', 'F', 'R', m, nb, ib, lb,v( i, 1_ilp64 ), ldv, t( 1_ilp64, i ), &
                           ldt,a( 1_ilp64, i ), lda, b, ldb, work, m )
              end do
           end if
           return
     ...
     !! complex matrix C, which consists of two blocks A and B.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZTPMLQT', -info )
              return
           end if
           ! Quick Return If Possible 
           if( m==0 .or. n==0 .or. k==0 ) return
           if( left .and. notran ) then
              do i = 1, k, mb
                 ...
                 call stdlib_I64_ztprfb( 'L', 'C', 'F', 'R', nb, n, ib, lb,v( i, 1_ilp64 ), ldv, t( 1_ilp64, i ), &
                           ldt,a( i, 1_ilp64 ), lda, b, ldb, work, ib )
              end do
           else if( right .and. tran ) then
              do i = 1, k, mb
                 ...
                 call stdlib_I64_ztprfb( 'R', 'N', 'F', 'R', m, nb, ib, lb,v( i, 1_ilp64 ), ldv, t( 1_ilp64, i ), &
                           ldt,a( 1_ilp64, i ), lda, b, ldb, work, m )
              end do
           else if( left .and. tran ) then
              kf = ((k-1)/mb)*mb+1
              do i = kf, 1, -mb
                 ...
                 call stdlib_I64_ztprfb( 'L', 'N', 'F', 'R', nb, n, ib, lb,v( i, 1_ilp64 ), ldv, t( 1_ilp64, i ), &
                           ldt,a( i, 1_ilp64 ), lda, b, ldb, work, ib )
              end do
           else if( right .and. notran ) then
              kf = ((k-1)/mb)*mb+1
              do i = kf, 1, -mb
                 ...
                 call stdlib_I64_ztprfb( 'R', 'C', 'F', 'R', m, nb, ib, lb,v( i, 1_ilp64 ), ldv, t( 1_ilp64, i ), &
                           ldt,a( 1_ilp64, i ), lda, b, ldb, work, m )
              end do
           end if
           return
     ...
     !! A = Q * L.
        ...
        ! =====================================================================
           ! Local Scalars 
           logical(lk) :: lquery
           integer(ilp64) :: i, ib, iinfo, iws, k, ki, kk, ldwork, lwkopt, mu, nb, nbmin, nu, &
                     nx
           ! Intrinsic Functions 
           ! Executable Statements 
           ! test the input arguments
           ...
           if( info==0_ilp64 ) then
              ...
              if( k==0_ilp64 ) then
                 ...
                 nb = stdlib_I64_ilaenv( 1_ilp64, 'SGEQLF', ' ', m, n, -1_ilp64, -1_ilp64 )
                 lwkopt = n*nb
              end if
              work( 1_ilp64 ) = lwkopt
              if( lwork<max( 1_ilp64, n ) .and. .not.lquery ) then
                 info = -7_ilp64
              end if
           end if
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SGEQLF', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           if( nb>1_ilp64 .and. nb<k ) then
              ! determine when to cross over from blocked to unblocked code.
              nx = max( 0_ilp64, stdlib_I64_ilaenv( 3_ilp64, 'SGEQLF', ' ', m, n, -1_ilp64, -1_ilp64 ) )
              if( nx<k ) then
                 ! determine if workspace is large enough for blocked code.
                 ldwork = n
                 iws = ldwork*nb
                 if( lwork<iws ) then
                    ...
                    ! determine the minimum value of nb.
                    nb = lwork / ldwork
                    nbmin = max( 2_ilp64, stdlib_I64_ilaenv( 2_ilp64, 'SGEQLF', ' ', m, n, -1_ilp64,-1_ilp64 ) )
                 end if
              end if
           end if
           if( nb>=nbmin .and. nb<k .and. nx<k ) then
              ...
              do i = k - kk + ki + 1, k - kk + 1, -nb
                 ...
                 ! compute the ql factorization of the current block
                 ! a(1:m-k+i+ib-1,n-k+i:n-k+i+ib-1)
                 call stdlib_I64_sgeql2( m-k+i+ib-1, ib, a( 1_ilp64, n-k+i ), lda, tau( i ),work, iinfo )
                           
                 if( n-k+i>1_ilp64 ) then
                    ! form the triangular factor of the block reflector
                    ! h = h(i+ib-1) . . . h(i+1) h(i)
                    call stdlib_I64_slarft( 'BACKWARD', 'COLUMNWISE', m-k+i+ib-1, ib,a( 1_ilp64, n-k+i ), &
                              lda, tau( i ), work, ldwork )
                    ! apply h**t to a(1:m-k+i+ib-1,1:n-k+i-1) from the left
                    call stdlib_I64_slarfb( 'LEFT', 'TRANSPOSE', 'BACKWARD','COLUMNWISE', m-k+i+ib-1, &
                    n-k+i-1, ib,a( 1_ilp64, n-k+i ), lda, work, ldwork, a, lda,work( ib+1 ), ldwork )
           ...
           ! use unblocked code to factor the last or only block
           if( mu>0_ilp64 .and. nu>0_ilp64 )call stdlib_I64_sgeql2( mu, nu, a, lda, tau, work, iinfo )
           work( 1_ilp64 ) = iws
           return
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              ...
              if( k==0_ilp64 ) then
                 ...
                 nb = stdlib_I64_ilaenv( 1_ilp64, 'DGEQLF', ' ', m, n, -1_ilp64, -1_ilp64 )
                 lwkopt = n*nb
              end if
              work( 1_ilp64 ) = lwkopt
              if( lwork<max( 1_ilp64, n ) .and. .not.lquery ) then
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DGEQLF', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           if( nb>1_ilp64 .and. nb<k ) then
              ! determine when to cross over from blocked to unblocked code.
              nx = max( 0_ilp64, stdlib_I64_ilaenv( 3_ilp64, 'DGEQLF', ' ', m, n, -1_ilp64, -1_ilp64 ) )
              if( nx<k ) then
                 ! determine if workspace is large enough for blocked code.
                 ldwork = n
                 iws = ldwork*nb
                 if( lwork<iws ) then
                    ...
                    ! determine the minimum value of nb.
                    nb = lwork / ldwork
                    nbmin = max( 2_ilp64, stdlib_I64_ilaenv( 2_ilp64, 'DGEQLF', ' ', m, n, -1_ilp64,-1_ilp64 ) )
                 end if
              end if
           end if
           if( nb>=nbmin .and. nb<k .and. nx<k ) then
              ...
              do i = k - kk + ki + 1, k - kk + 1, -nb
                 ...
                 ! compute the ql factorization of the current block
                 ! a(1:m-k+i+ib-1,n-k+i:n-k+i+ib-1)
                 call stdlib_I64_dgeql2( m-k+i+ib-1, ib, a( 1_ilp64, n-k+i ), lda, tau( i ),work, iinfo )
                           
                 if( n-k+i>1_ilp64 ) then
                    ! form the triangular factor of the block reflector
                    ! h = h(i+ib-1) . . . h(i+1) h(i)
                    call stdlib_I64_dlarft( 'BACKWARD', 'COLUMNWISE', m-k+i+ib-1, ib,a( 1_ilp64, n-k+i ), &
                              lda, tau( i ), work, ldwork )
                    ! apply h**t to a(1:m-k+i+ib-1,1:n-k+i-1) from the left
                    call stdlib_I64_dlarfb( 'LEFT', 'TRANSPOSE', 'BACKWARD','COLUMNWISE', m-k+i+ib-1, &
                    n-k+i-1, ib,a( 1_ilp64, n-k+i ), lda, work, ldwork, a, lda,work( ib+1 ), ldwork )
           ...
           ! use unblocked code to factor the last or only block
           if( mu>0_ilp64 .and. nu>0_ilp64 )call stdlib_I64_dgeql2( mu, nu, a, lda, tau, work, iinfo )
           work( 1_ilp64 ) = iws
           return
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              ...
              if( k==0_ilp64 ) then
                 ...
                 nb = stdlib_I64_ilaenv( 1_ilp64, 'CGEQLF', ' ', m, n, -1_ilp64, -1_ilp64 )
                 lwkopt = n*nb
              end if
              work( 1_ilp64 ) = lwkopt
              if( lwork<max( 1_ilp64, n ) .and. .not.lquery ) then
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CGEQLF', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           if( nb>1_ilp64 .and. nb<k ) then
              ! determine when to cross over from blocked to unblocked code.
              nx = max( 0_ilp64, stdlib_I64_ilaenv( 3_ilp64, 'CGEQLF', ' ', m, n, -1_ilp64, -1_ilp64 ) )
              if( nx<k ) then
                 ! determine if workspace is large enough for blocked code.
                 ldwork = n
                 iws = ldwork*nb
                 if( lwork<iws ) then
                    ...
                    ! determine the minimum value of nb.
                    nb = lwork / ldwork
                    nbmin = max( 2_ilp64, stdlib_I64_ilaenv( 2_ilp64, 'CGEQLF', ' ', m, n, -1_ilp64,-1_ilp64 ) )
                 end if
              end if
           end if
           if( nb>=nbmin .and. nb<k .and. nx<k ) then
              ...
              do i = k - kk + ki + 1, k - kk + 1, -nb
                 ...
                 ! compute the ql factorization of the current block
                 ! a(1:m-k+i+ib-1,n-k+i:n-k+i+ib-1)
                 call stdlib_I64_cgeql2( m-k+i+ib-1, ib, a( 1_ilp64, n-k+i ), lda, tau( i ),work, iinfo )
                           
                 if( n-k+i>1_ilp64 ) then
                    ! form the triangular factor of the block reflector
                    ! h = h(i+ib-1) . . . h(i+1) h(i)
                    call stdlib_I64_clarft( 'BACKWARD', 'COLUMNWISE', m-k+i+ib-1, ib,a( 1_ilp64, n-k+i ), &
                              lda, tau( i ), work, ldwork )
                    ! apply h**h to a(1:m-k+i+ib-1,1:n-k+i-1) from the left
                    call stdlib_I64_clarfb( 'LEFT', 'CONJUGATE TRANSPOSE', 'BACKWARD','COLUMNWISE', m-&
                    k+i+ib-1, n-k+i-1, ib,a( 1_ilp64, n-k+i ), lda, work, ldwork, a, lda,work( ib+1 ), &
                              ldwork )
                 end if
              end do
              mu = m - k + i + nb - 1_ilp64
           ...
           ! use unblocked code to factor the last or only block
           if( mu>0_ilp64 .and. nu>0_ilp64 )call stdlib_I64_cgeql2( mu, nu, a, lda, tau, work, iinfo )
           work( 1_ilp64 ) = iws
           return
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              ...
              if( k==0_ilp64 ) then
                 ...
                 nb = stdlib_I64_ilaenv( 1_ilp64, 'ZGEQLF', ' ', m, n, -1_ilp64, -1_ilp64 )
                 lwkopt = n*nb
              end if
              work( 1_ilp64 ) = lwkopt
              if( lwork<max( 1_ilp64, n ) .and. .not.lquery ) then
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZGEQLF', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           if( nb>1_ilp64 .and. nb<k ) then
              ! determine when to cross over from blocked to unblocked code.
              nx = max( 0_ilp64, stdlib_I64_ilaenv( 3_ilp64, 'ZGEQLF', ' ', m, n, -1_ilp64, -1_ilp64 ) )
              if( nx<k ) then
                 ! determine if workspace is large enough for blocked code.
                 ldwork = n
                 iws = ldwork*nb
                 if( lwork<iws ) then
                    ...
                    ! determine the minimum value of nb.
                    nb = lwork / ldwork
                    nbmin = max( 2_ilp64, stdlib_I64_ilaenv( 2_ilp64, 'ZGEQLF', ' ', m, n, -1_ilp64,-1_ilp64 ) )
                 end if
              end if
           end if
           if( nb>=nbmin .and. nb<k .and. nx<k ) then
              ...
              do i = k - kk + ki + 1, k - kk + 1, -nb
                 ...
                 ! compute the ql factorization of the current block
                 ! a(1:m-k+i+ib-1,n-k+i:n-k+i+ib-1)
                 call stdlib_I64_zgeql2( m-k+i+ib-1, ib, a( 1_ilp64, n-k+i ), lda, tau( i ),work, iinfo )
                           
                 if( n-k+i>1_ilp64 ) then
                    ! form the triangular factor of the block reflector
                    ! h = h(i+ib-1) . . . h(i+1) h(i)
                    call stdlib_I64_zlarft( 'BACKWARD', 'COLUMNWISE', m-k+i+ib-1, ib,a( 1_ilp64, n-k+i ), &
                              lda, tau( i ), work, ldwork )
                    ! apply h**h to a(1:m-k+i+ib-1,1:n-k+i-1) from the left
                    call stdlib_I64_zlarfb( 'LEFT', 'CONJUGATE TRANSPOSE', 'BACKWARD','COLUMNWISE', m-&
                    k+i+ib-1, n-k+i-1, ib,a( 1_ilp64, n-k+i ), lda, work, ldwork, a, lda,work( ib+1 ), &
                              ldwork )
                 end if
              end do
           ...
           ! use unblocked code to factor the last or only block
           if( mu>0_ilp64 .and. nu>0_ilp64 )call stdlib_I64_zgeql2( mu, nu, a, lda, tau, work, iinfo )
           work( 1_ilp64 ) = iws
           return
     ...
     !! A = Q * L.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SGEQL2', -info )
              return
           end if
           k = min( m, n )
           do i = k, 1, -1
              ! generate elementary reflector h(i) to annihilate
              ! a(1:m-k+i-1,n-k+i)
              call stdlib_I64_slarfg( m-k+i, a( m-k+i, n-k+i ), a( 1_ilp64, n-k+i ), 1_ilp64,tau( i ) )
              ! apply h(i) to a(1:m-k+i,1:n-k+i-1) from the left
              aii = a( m-k+i, n-k+i )
              a( m-k+i, n-k+i ) = one
              call stdlib_I64_slarf( 'LEFT', m-k+i, n-k+i-1, a( 1_ilp64, n-k+i ), 1_ilp64, tau( i ),a, lda, work )
     ...
     !! A = Q * L.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DGEQL2', -info )
              return
           end if
           k = min( m, n )
           do i = k, 1, -1
              ! generate elementary reflector h(i) to annihilate
              ! a(1:m-k+i-1,n-k+i)
              call stdlib_I64_dlarfg( m-k+i, a( m-k+i, n-k+i ), a( 1_ilp64, n-k+i ), 1_ilp64,tau( i ) )
              ! apply h(i) to a(1:m-k+i,1:n-k+i-1) from the left
              aii = a( m-k+i, n-k+i )
              a( m-k+i, n-k+i ) = one
              call stdlib_I64_dlarf( 'LEFT', m-k+i, n-k+i-1, a( 1_ilp64, n-k+i ), 1_ilp64, tau( i ),a, lda, work )
     ...
     !! A = Q * L.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CGEQL2', -info )
              return
           end if
           k = min( m, n )
           do i = k, 1, -1
              ...
              ! a(1:m-k+i-1,n-k+i)
              alpha = a( m-k+i, n-k+i )
              call stdlib_I64_clarfg( m-k+i, alpha, a( 1_ilp64, n-k+i ), 1_ilp64, tau( i ) )
              ! apply h(i)**h to a(1:m-k+i,1:n-k+i-1) from the left
              a( m-k+i, n-k+i ) = cone
              call stdlib_I64_clarf( 'LEFT', m-k+i, n-k+i-1, a( 1_ilp64, n-k+i ), 1_ilp64,conjg( tau( i ) ), a, &
                        lda, work )
              a( m-k+i, n-k+i ) = alpha
           end do
           return
     ...
     !! A = Q * L.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZGEQL2', -info )
              return
           end if
           k = min( m, n )
           do i = k, 1, -1
              ...
              ! a(1:m-k+i-1,n-k+i)
              alpha = a( m-k+i, n-k+i )
              call stdlib_I64_zlarfg( m-k+i, alpha, a( 1_ilp64, n-k+i ), 1_ilp64, tau( i ) )
              ! apply h(i)**h to a(1:m-k+i,1:n-k+i-1) from the left
              a( m-k+i, n-k+i ) = cone
              call stdlib_I64_zlarf( 'LEFT', m-k+i, n-k+i-1, a( 1_ilp64, n-k+i ), 1_ilp64,conjg( tau( i ) ), a, &
                        lda, work )
              a( m-k+i, n-k+i ) = alpha
           end do
           return
     ...
     !! as returned by CGEQLF.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           logical(lk) :: lquery
           integer(ilp64) :: i, ib, iinfo, iws, j, kk, l, ldwork, lwkopt, nb, nbmin, nx
           ! Intrinsic Functions 
           ! Executable Statements 
           ! test the input arguments
           info = 0_ilp64
           ...
           if( info==0_ilp64 ) then
              if( n==0_ilp64 ) then
                 ...
                 nb = stdlib_I64_ilaenv( 1_ilp64, 'CUNGQL', ' ', m, n, k, -1_ilp64 )
                 lwkopt = n*nb
              end if
              work( 1_ilp64 ) = lwkopt
              if( lwork<max( 1_ilp64, n ) .and. .not.lquery ) then
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CUNGQL', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           if( nb>1_ilp64 .and. nb<k ) then
              ! determine when to cross over from blocked to unblocked code.
              nx = max( 0_ilp64, stdlib_I64_ilaenv( 3_ilp64, 'CUNGQL', ' ', m, n, k, -1_ilp64 ) )
              if( nx<k ) then
                 ! determine if workspace is large enough for blocked code.
                 ldwork = n
                 iws = ldwork*nb
                 if( lwork<iws ) then
                    ...
                    ! determine the minimum value of nb.
                    nb = lwork / ldwork
                    nbmin = max( 2_ilp64, stdlib_I64_ilaenv( 2_ilp64, 'CUNGQL', ' ', m, n, k, -1_ilp64 ) )
                 end if
              end if
           end if
           if( nb>=nbmin .and. nb<k .and. nx<k ) then
           ...
           ! use unblocked code for the first or only block.
           call stdlib_I64_cung2l( m-kk, n-kk, k-kk, a, lda, tau, work, iinfo )
           if( kk>0_ilp64 ) then
              ! use blocked code
              do i = k - kk + 1, k, nb
                 ib = min( nb, k-i+1 )
                 if( n-k+i>1_ilp64 ) then
                    ...
                    call stdlib_I64_clarft( 'BACKWARD', 'COLUMNWISE', m-k+i+ib-1, ib,a( 1_ilp64, n-k+i ), &
                              lda, tau( i ), work, ldwork )
                    ! apply h to a(1:m-k+i+ib-1,1:n-k+i-1) from the left
                    call stdlib_I64_clarfb( 'LEFT', 'NO TRANSPOSE', 'BACKWARD','COLUMNWISE', m-k+i+ib-&
                    1_ilp64, n-k+i-1, ib,a( 1_ilp64, n-k+i ), lda, work, ldwork, a, lda,work( ib+1 ), ldwork )
                              
                 end if
                 ! apply h to rows 1:m-k+i+ib-1 of current block
                 call stdlib_I64_cung2l( m-k+i+ib-1, ib, ib, a( 1_ilp64, n-k+i ), lda,tau( i ), work, iinfo &
                           )
                 ! set rows m-k+i+ib:m of current block to czero
                 do j = n - k + i, n - k + i + ib - 1
                    do l = m - k + i + ib, m
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              if( n==0_ilp64 ) then
                 ...
                 nb = stdlib_I64_ilaenv( 1_ilp64, 'ZUNGQL', ' ', m, n, k, -1_ilp64 )
                 lwkopt = n*nb
              end if
              work( 1_ilp64 ) = lwkopt
              if( lwork<max( 1_ilp64, n ) .and. .not.lquery ) then
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZUNGQL', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           if( nb>1_ilp64 .and. nb<k ) then
              ! determine when to cross over from blocked to unblocked code.
              nx = max( 0_ilp64, stdlib_I64_ilaenv( 3_ilp64, 'ZUNGQL', ' ', m, n, k, -1_ilp64 ) )
              if( nx<k ) then
                 ! determine if workspace is large enough for blocked code.
                 ldwork = n
                 iws = ldwork*nb
                 if( lwork<iws ) then
                    ...
                    ! determine the minimum value of nb.
                    nb = lwork / ldwork
                    nbmin = max( 2_ilp64, stdlib_I64_ilaenv( 2_ilp64, 'ZUNGQL', ' ', m, n, k, -1_ilp64 ) )
                 end if
              end if
           end if
           if( nb>=nbmin .and. nb<k .and. nx<k ) then
           ...
           ! use unblocked code for the first or only block.
           call stdlib_I64_zung2l( m-kk, n-kk, k-kk, a, lda, tau, work, iinfo )
           if( kk>0_ilp64 ) then
              ! use blocked code
              do i = k - kk + 1, k, nb
                 ib = min( nb, k-i+1 )
                 if( n-k+i>1_ilp64 ) then
                    ...
                    call stdlib_I64_zlarft( 'BACKWARD', 'COLUMNWISE', m-k+i+ib-1, ib,a( 1_ilp64, n-k+i ), &
                              lda, tau( i ), work, ldwork )
                    ! apply h to a(1:m-k+i+ib-1,1:n-k+i-1) from the left
                    call stdlib_I64_zlarfb( 'LEFT', 'NO TRANSPOSE', 'BACKWARD','COLUMNWISE', m-k+i+ib-&
                    1_ilp64, n-k+i-1, ib,a( 1_ilp64, n-k+i ), lda, work, ldwork, a, lda,work( ib+1 ), ldwork )
                              
                 end if
                 ! apply h to rows 1:m-k+i+ib-1 of current block
                 call stdlib_I64_zung2l( m-k+i+ib-1, ib, ib, a( 1_ilp64, n-k+i ), lda,tau( i ), work, iinfo &
                           )
                 ! set rows m-k+i+ib:m of current block to czero
                 do j = n - k + i, n - k + i + ib - 1
                    do l = m - k + i + ib, m
     ...
     !! if SIDE = 'R'.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           logical(lk) :: left, lquery, notran
           integer(ilp64) :: i, i1, i2, i3, ib, iinfo, iwt, ldwork, lwkopt, mi, nb, nbmin, ni, nq, &
                     nw
           ! Intrinsic Functions 
           ! Executable Statements 
           ! test the input arguments
           ...
           if( info==0_ilp64 ) then
              ...
              if( m==0_ilp64 .or. n==0_ilp64 ) then
                 ...
                 nb = min( nbmax, stdlib_I64_ilaenv( 1_ilp64, 'CUNMQL', side // trans, m, n,k, -1_ilp64 ) )
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CUNMQL', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           if( nb>1_ilp64 .and. nb<k ) then
              if( lwork<lwkopt ) then
                 nb = (lwork-tsize) / ldwork
                 nbmin = max( 2_ilp64, stdlib_I64_ilaenv( 2_ilp64, 'CUNMQL', side // trans, m, n, k,-1_ilp64 ) )
              end if
           end if
           if( nb<nbmin .or. nb>=k ) then
              ! use unblocked code
              call stdlib_I64_cunm2l( side, trans, m, n, k, a, lda, tau, c, ldc, work,iinfo )
           else
              ! use blocked code
              iwt = 1_ilp64 + nw*nb
              if( ( left .and. notran ) .or.( .not.left .and. .not.notran ) ) then
              ...
              do i = i1, i2, i3
                 ...
                 ! form the triangular factor of the block reflector
                 ! h = h(i+ib-1) . . . h(i+1) h(i)
                 call stdlib_I64_clarft( 'BACKWARD', 'COLUMNWISE', nq-k+i+ib-1, ib,a( 1_ilp64, i ), lda, &
                           tau( i ), work( iwt ), ldt )
                 if( left ) then
                    ! h or h**h is applied to c(1:m-k+i+ib-1,1:n)
                    mi = m - k + i + ib - 1_ilp64
                 ...
                 ! apply h or h**h
                 call stdlib_I64_clarfb( side, trans, 'BACKWARD', 'COLUMNWISE', mi, ni,ib, a( 1_ilp64, i ), &
                           lda, work( iwt ), ldt, c, ldc,work, ldwork )
              end do
           end if
           work( 1_ilp64 ) = lwkopt
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              ...
              if( m==0_ilp64 .or. n==0_ilp64 ) then
                 ...
                 nb = min( nbmax, stdlib_I64_ilaenv( 1_ilp64, 'ZUNMQL', side // trans, m, n,k, -1_ilp64 ) )
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZUNMQL', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           if( nb>1_ilp64 .and. nb<k ) then
              if( lwork<lwkopt ) then
                 nb = (lwork-tsize) / ldwork
                 nbmin = max( 2_ilp64, stdlib_I64_ilaenv( 2_ilp64, 'ZUNMQL', side // trans, m, n, k,-1_ilp64 ) )
              end if
           end if
           if( nb<nbmin .or. nb>=k ) then
              ! use unblocked code
              call stdlib_I64_zunm2l( side, trans, m, n, k, a, lda, tau, c, ldc, work,iinfo )
           else
              ! use blocked code
              iwt = 1_ilp64 + nw*nb
              if( ( left .and. notran ) .or.( .not.left .and. .not.notran ) ) then
              ...
              do i = i1, i2, i3
                 ...
                 ! form the triangular factor of the block reflector
                 ! h = h(i+ib-1) . . . h(i+1) h(i)
                 call stdlib_I64_zlarft( 'BACKWARD', 'COLUMNWISE', nq-k+i+ib-1, ib,a( 1_ilp64, i ), lda, &
                           tau( i ), work( iwt ), ldt )
                 if( left ) then
                    ! h or h**h is applied to c(1:m-k+i+ib-1,1:n)
                    mi = m - k + i + ib - 1_ilp64
                 ...
                 ! apply h or h**h
                 call stdlib_I64_zlarfb( side, trans, 'BACKWARD', 'COLUMNWISE', mi, ni,ib, a( 1_ilp64, i ), &
                           lda, work( iwt ), ldt, c, ldc,work, ldwork )
              end do
           end if
           work( 1_ilp64 ) = lwkopt
     ...
     !! as returned by CGEQLF.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CUNG2L', -info )
              return
           end if
           ! quick return if possible
           if( n<=0 )return
           ...
           do i = 1, k
              ...
              ! apply h(i) to a(1:m-k+i,1:n-k+i) from the left
              a( m-n+ii, ii ) = cone
              call stdlib_I64_clarf( 'LEFT', m-n+ii, ii-1, a( 1_ilp64, ii ), 1_ilp64, tau( i ), a,lda, work )
                        
              call stdlib_I64_cscal( m-n+ii-1, -tau( i ), a( 1_ilp64, ii ), 1_ilp64 )
              a( m-n+ii, ii ) = cone - tau( i )
              ! set a(m-k+i+1:m,n-k+i) to czero
              do l = m - n + ii + 1, m
                 a( l, ii ) = czero
     ...
     !! as returned by ZGEQLF.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZUNG2L', -info )
              return
           end if
           ! quick return if possible
           if( n<=0 )return
           ...
           do i = 1, k
              ...
              ! apply h(i) to a(1:m-k+i,1:n-k+i) from the left
              a( m-n+ii, ii ) = cone
              call stdlib_I64_zlarf( 'LEFT', m-n+ii, ii-1, a( 1_ilp64, ii ), 1_ilp64, tau( i ), a,lda, work )
                        
              call stdlib_I64_zscal( m-n+ii-1, -tau( i ), a( 1_ilp64, ii ), 1_ilp64 )
              a( m-n+ii, ii ) = cone - tau( i )
              ! set a(m-k+i+1:m,n-k+i) to czero
              do l = m - n + ii + 1, m
                 a( l, ii ) = czero
     ...
     !! if SIDE = 'R'.
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           logical(lk) :: left, notran
           integer(ilp64) :: i, i1, i2, i3, mi, ni, nq
           complex(sp) :: aii, taui
           ! Intrinsic Functions 
           ! Executable Statements 
           ! test the input arguments
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CUNM2L', -info )
              return
           end if
           ! quick return if possible
           if( m==0 .or. n==0 .or. k==0 )return
           ...
           do i = i1, i2, i3
              ...
              aii = a( nq-k+i, i )
              a( nq-k+i, i ) = cone
              call stdlib_I64_clarf( side, mi, ni, a( 1_ilp64, i ), 1_ilp64, taui, c, ldc, work )
              a( nq-k+i, i ) = aii
           end do
           return
     ...
     !! if SIDE = 'R'.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZUNM2L', -info )
              return
           end if
           ! quick return if possible
           if( m==0 .or. n==0 .or. k==0 )return
           ...
           do i = i1, i2, i3
              ...
              aii = a( nq-k+i, i )
              a( nq-k+i, i ) = cone
              call stdlib_I64_zlarf( side, mi, ni, a( 1_ilp64, i ), 1_ilp64, taui, c, ldc, work )
              a( nq-k+i, i ) = aii
           end do
           return
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              if( n==0_ilp64 ) then
                 ...
                 nb = stdlib_I64_ilaenv( 1_ilp64, 'SORGQL', ' ', m, n, k, -1_ilp64 )
                 lwkopt = n*nb
              end if
              work( 1_ilp64 ) = lwkopt
              if( lwork<max( 1_ilp64, n ) .and. .not.lquery ) then
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SORGQL', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           if( nb>1_ilp64 .and. nb<k ) then
              ! determine when to cross over from blocked to unblocked code.
              nx = max( 0_ilp64, stdlib_I64_ilaenv( 3_ilp64, 'SORGQL', ' ', m, n, k, -1_ilp64 ) )
              if( nx<k ) then
                 ! determine if workspace is large enough for blocked code.
                 ldwork = n
                 iws = ldwork*nb
                 if( lwork<iws ) then
                    ...
                    ! determine the minimum value of nb.
                    nb = lwork / ldwork
                    nbmin = max( 2_ilp64, stdlib_I64_ilaenv( 2_ilp64, 'SORGQL', ' ', m, n, k, -1_ilp64 ) )
                 end if
              end if
           end if
           if( nb>=nbmin .and. nb<k .and. nx<k ) then
           ...
           ! use unblocked code for the first or only block.
           call stdlib_I64_sorg2l( m-kk, n-kk, k-kk, a, lda, tau, work, iinfo )
           if( kk>0_ilp64 ) then
              ! use blocked code
              do i = k - kk + 1, k, nb
                 ib = min( nb, k-i+1 )
                 if( n-k+i>1_ilp64 ) then
                    ...
                    call stdlib_I64_slarft( 'BACKWARD', 'COLUMNWISE', m-k+i+ib-1, ib,a( 1_ilp64, n-k+i ), &
                              lda, tau( i ), work, ldwork )
                    ! apply h to a(1:m-k+i+ib-1,1:n-k+i-1) from the left
                    call stdlib_I64_slarfb( 'LEFT', 'NO TRANSPOSE', 'BACKWARD','COLUMNWISE', m-k+i+ib-&
                    1_ilp64, n-k+i-1, ib,a( 1_ilp64, n-k+i ), lda, work, ldwork, a, lda,work( ib+1 ), ldwork )
                              
                 end if
                 ! apply h to rows 1:m-k+i+ib-1 of current block
                 call stdlib_I64_sorg2l( m-k+i+ib-1, ib, ib, a( 1_ilp64, n-k+i ), lda,tau( i ), work, iinfo &
                           )
                 ! set rows m-k+i+ib:m of current block to zero
                 do j = n - k + i, n - k + i + ib - 1
                    do l = m - k + i + ib, m
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              if( n==0_ilp64 ) then
                 ...
                 nb = stdlib_I64_ilaenv( 1_ilp64, 'DORGQL', ' ', m, n, k, -1_ilp64 )
                 lwkopt = n*nb
              end if
              work( 1_ilp64 ) = lwkopt
              if( lwork<max( 1_ilp64, n ) .and. .not.lquery ) then
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DORGQL', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           if( nb>1_ilp64 .and. nb<k ) then
              ! determine when to cross over from blocked to unblocked code.
              nx = max( 0_ilp64, stdlib_I64_ilaenv( 3_ilp64, 'DORGQL', ' ', m, n, k, -1_ilp64 ) )
              if( nx<k ) then
                 ! determine if workspace is large enough for blocked code.
                 ldwork = n
                 iws = ldwork*nb
                 if( lwork<iws ) then
                    ...
                    ! determine the minimum value of nb.
                    nb = lwork / ldwork
                    nbmin = max( 2_ilp64, stdlib_I64_ilaenv( 2_ilp64, 'DORGQL', ' ', m, n, k, -1_ilp64 ) )
                 end if
              end if
           end if
           if( nb>=nbmin .and. nb<k .and. nx<k ) then
           ...
           ! use unblocked code for the first or only block.
           call stdlib_I64_dorg2l( m-kk, n-kk, k-kk, a, lda, tau, work, iinfo )
           if( kk>0_ilp64 ) then
              ! use blocked code
              do i = k - kk + 1, k, nb
                 ib = min( nb, k-i+1 )
                 if( n-k+i>1_ilp64 ) then
                    ...
                    call stdlib_I64_dlarft( 'BACKWARD', 'COLUMNWISE', m-k+i+ib-1, ib,a( 1_ilp64, n-k+i ), &
                              lda, tau( i ), work, ldwork )
                    ! apply h to a(1:m-k+i+ib-1,1:n-k+i-1) from the left
                    call stdlib_I64_dlarfb( 'LEFT', 'NO TRANSPOSE', 'BACKWARD','COLUMNWISE', m-k+i+ib-&
                    1_ilp64, n-k+i-1, ib,a( 1_ilp64, n-k+i ), lda, work, ldwork, a, lda,work( ib+1 ), ldwork )
                              
                 end if
                 ! apply h to rows 1:m-k+i+ib-1 of current block
                 call stdlib_I64_dorg2l( m-k+i+ib-1, ib, ib, a( 1_ilp64, n-k+i ), lda,tau( i ), work, iinfo &
                           )
                 ! set rows m-k+i+ib:m of current block to zero
                 do j = n - k + i, n - k + i + ib - 1
                    do l = m - k + i + ib, m
        ...
        ! =====================================================================
           ...
           ! compute the workspace requirements
              if( m==0_ilp64 .or. n==0_ilp64 ) then
                 ...
                 nb = min( nbmax, stdlib_I64_ilaenv( 1_ilp64, 'SORMQL', side // trans, m, n,k, -1_ilp64 ) )
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SORMQL', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           if( nb>1_ilp64 .and. nb<k ) then
              if( lwork<lwkopt ) then
                 nb = (lwork-tsize) / ldwork
                 nbmin = max( 2_ilp64, stdlib_I64_ilaenv( 2_ilp64, 'SORMQL', side // trans, m, n, k,-1_ilp64 ) )
              end if
           end if
           if( nb<nbmin .or. nb>=k ) then
              ! use unblocked code
              call stdlib_I64_sorm2l( side, trans, m, n, k, a, lda, tau, c, ldc, work,iinfo )
           else
              ! use blocked code
              iwt = 1_ilp64 + nw*nb
              if( ( left .and. notran ) .or.( .not.left .and. .not.notran ) ) then
              ...
              do i = i1, i2, i3
                 ...
                 ! form the triangular factor of the block reflector
                 ! h = h(i+ib-1) . . . h(i+1) h(i)
                 call stdlib_I64_slarft( 'BACKWARD', 'COLUMNWISE', nq-k+i+ib-1, ib,a( 1_ilp64, i ), lda, &
                           tau( i ), work( iwt ), ldt )
                 if( left ) then
                    ! h or h**t is applied to c(1:m-k+i+ib-1,1:n)
                    mi = m - k + i + ib - 1_ilp64
                 ...
                 ! apply h or h**t
                 call stdlib_I64_slarfb( side, trans, 'BACKWARD', 'COLUMNWISE', mi, ni,ib, a( 1_ilp64, i ), &
                           lda, work( iwt ), ldt, c, ldc,work, ldwork )
              end do
           end if
           work( 1_ilp64 ) = lwkopt
        ...
        ! =====================================================================
           ...
           if( info==0_ilp64 ) then
              ...
              if( m==0_ilp64 .or. n==0_ilp64 ) then
                 ...
                 nb = min( nbmax, stdlib_I64_ilaenv( 1_ilp64, 'DORMQL', side // trans, m, n,k, -1_ilp64 ) )
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DORMQL', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           if( nb>1_ilp64 .and. nb<k ) then
              if( lwork<lwkopt ) then
                 nb = (lwork-tsize) / ldwork
                 nbmin = max( 2_ilp64, stdlib_I64_ilaenv( 2_ilp64, 'DORMQL', side // trans, m, n, k,-1_ilp64 ) )
              end if
           end if
           if( nb<nbmin .or. nb>=k ) then
              ! use unblocked code
              call stdlib_I64_dorm2l( side, trans, m, n, k, a, lda, tau, c, ldc, work,iinfo )
           else
              ! use blocked code
              iwt = 1_ilp64 + nw*nb
              if( ( left .and. notran ) .or.( .not.left .and. .not.notran ) ) then
              ...
              do i = i1, i2, i3
                 ...
                 ! form the triangular factor of the block reflector
                 ! h = h(i+ib-1) . . . h(i+1) h(i)
                 call stdlib_I64_dlarft( 'BACKWARD', 'COLUMNWISE', nq-k+i+ib-1, ib,a( 1_ilp64, i ), lda, &
                           tau( i ), work( iwt ), ldt )
                 if( left ) then
                    ! h or h**t is applied to c(1:m-k+i+ib-1,1:n)
                    mi = m - k + i + ib - 1_ilp64
                 ...
                 ! apply h or h**t
                 call stdlib_I64_dlarfb( side, trans, 'BACKWARD', 'COLUMNWISE', mi, ni,ib, a( 1_ilp64, i ), &
                           lda, work( iwt ), ldt, c, ldc,work, ldwork )
              end do
           end if
           work( 1_ilp64 ) = lwkopt
     ...
     !! as returned by SGEQLF.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SORG2L', -info )
              return
           end if
           ! quick return if possible
           if( n<=0 )return
           ...
           do i = 1, k
              ...
              ! apply h(i) to a(1:m-k+i,1:n-k+i) from the left
              a( m-n+ii, ii ) = one
              call stdlib_I64_slarf( 'LEFT', m-n+ii, ii-1, a( 1_ilp64, ii ), 1_ilp64, tau( i ), a,lda, work )
                        
              call stdlib_I64_sscal( m-n+ii-1, -tau( i ), a( 1_ilp64, ii ), 1_ilp64 )
              a( m-n+ii, ii ) = one - tau( i )
              ! set a(m-k+i+1:m,n-k+i) to zero
              do l = m - n + ii + 1, m
                 a( l, ii ) = zero
     ...
     !! as returned by DGEQLF.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DORG2L', -info )
              return
           end if
           ! quick return if possible
           if( n<=0 )return
           ...
           do i = 1, k
              ...
              ! apply h(i) to a(1:m-k+i,1:n-k+i) from the left
              a( m-n+ii, ii ) = one
              call stdlib_I64_dlarf( 'LEFT', m-n+ii, ii-1, a( 1_ilp64, ii ), 1_ilp64, tau( i ), a,lda, work )
                        
              call stdlib_I64_dscal( m-n+ii-1, -tau( i ), a( 1_ilp64, ii ), 1_ilp64 )
              a( m-n+ii, ii ) = one - tau( i )
              ! set a(m-k+i+1:m,n-k+i) to zero
              do l = m - n + ii + 1, m
                 a( l, ii ) = zero
     ...
     !! if SIDE = 'R'.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'SORM2L', -info )
              return
           end if
           ! quick return if possible
           if( m==0 .or. n==0 .or. k==0 )return
           ...
           do i = i1, i2, i3
              ...
              aii = a( nq-k+i, i )
              a( nq-k+i, i ) = one
              call stdlib_I64_slarf( side, mi, ni, a( 1_ilp64, i ), 1_ilp64, tau( i ), c, ldc,work )
              a( nq-k+i, i ) = aii
           end do
           return
     ...
     !! if SIDE = 'R'.
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'DORM2L', -info )
              return
           end if
           ! quick return if possible
           if( m==0 .or. n==0 .or. k==0 )return
           ...
           do i = i1, i2, i3
              ...
              aii = a( nq-k+i, i )
              a( nq-k+i, i ) = one
              call stdlib_I64_dlarf( side, mi, ni, a( 1_ilp64, i ), 1_ilp64, tau( i ), c, ldc,work )
              a( nq-k+i, i ) = aii
           end do
           return
     ...
     pure module subroutine stdlib_I64_cunm22( side, trans, m, n, n1, n2, q, ldq, c, ldc,work, lwork, info )
        ...
        ! =====================================================================
           ...
           ! Local Scalars 
           logical(lk) :: left, lquery, notran
           integer(ilp64) :: i, ldwork, len, lwkopt, nb, nq, nw
           ! Intrinsic Functions 
           ! Executable Statements 
           ! test the input arguments
           info = 0_ilp64
           ...
           nw = nq
           if( n1==0_ilp64 .or. n2==0_ilp64 ) nw = 1_ilp64
           if( .not.left .and. .not.stdlib_lsame( side, 'R' ) ) then
              info = -1_ilp64
           else if( .not.stdlib_lsame( trans, 'N' ) .and. .not.stdlib_lsame( trans, 'C' ) )&
                     then
              ...
              info = -4_ilp64
           else if( n1<0_ilp64 .or. n1+n2/=nq ) then
              info = -5_ilp64
           else if( n2<0_ilp64 ) then
              info = -6_ilp64
           else if( ldq<max( 1_ilp64, nq ) ) then
              info = -8_ilp64
           else if( ldc<max( 1_ilp64, m ) ) then
              info = -10_ilp64
           else if( lwork<nw .and. .not.lquery ) then
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'CUNM22', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           ! degenerate cases (n1 = 0 or n2 = 0) are handled using stdlib_I64_ctrmm.
           if( n1==0_ilp64 ) then
              call stdlib_I64_ctrmm( side, 'UPPER', trans, 'NON-UNIT', m, n, cone,q, ldq, c, ldc )
                        
              work( 1_ilp64 ) = cone
              return
           else if( n2==0_ilp64 ) then
              call stdlib_I64_ctrmm( side, 'LOWER', trans, 'NON-UNIT', m, n, cone,q, ldq, c, ldc )
                        
              work( 1_ilp64 ) = cone
              return
           end if
           ! compute the largest chunk size available from the workspace.
           nb = max( 1_ilp64, min( lwork, lwkopt ) / nq )
           if( left ) then
              if( notran ) then
                 do i = 1, n, nb
                    len = min( nb, n-i+1 )
                    ldwork = m
                    ! multiply bottom part of c by q12.
                    call stdlib_I64_clacpy( 'ALL', n1, len, c( n2+1, i ), ldc, work,ldwork )
                    call stdlib_I64_ctrmm( 'LEFT', 'LOWER', 'NO TRANSPOSE', 'NON-UNIT',n1, len, cone, &
                              q( 1_ilp64, n2+1 ), ldq, work,ldwork )
                    ! multiply top part of c by q11.
                    call stdlib_I64_cgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', n1, len, n2,cone, q, ldq, &
                              c( 1_ilp64, i ), ldc, cone, work,ldwork )
                    ! multiply top part of c by q21.
                    call stdlib_I64_clacpy( 'ALL', n2, len, c( 1_ilp64, i ), ldc,work( n1+1 ), ldwork )
                              
                    call stdlib_I64_ctrmm( 'LEFT', 'UPPER', 'NO TRANSPOSE', 'NON-UNIT',n2, len, cone, &
                              q( n1+1, 1_ilp64 ), ldq,work( n1+1 ), ldwork )
                    ! multiply bottom part of c by q22.
                    call stdlib_I64_cgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', n2, len, n1,cone, q( n1+1, &
                              n2+1 ), ldq, c( n2+1, i ), ldc,cone, work( n1+1 ), ldwork )
                    ! copy everything back.
                    call stdlib_I64_clacpy( 'ALL', m, len, work, ldwork, c( 1_ilp64, i ),ldc )
                 end do
              else
                 do i = 1, n, nb
                    len = min( nb, n-i+1 )
                    ldwork = m
                    ! multiply bottom part of c by q21**h.
                    call stdlib_I64_clacpy( 'ALL', n2, len, c( n1+1, i ), ldc, work,ldwork )
                    call stdlib_I64_ctrmm( 'LEFT', 'UPPER', 'CONJUGATE', 'NON-UNIT',n2, len, cone, q( &
                              n1+1, 1_ilp64 ), ldq, work,ldwork )
                    ! multiply top part of c by q11**h.
                    call stdlib_I64_cgemm( 'CONJUGATE', 'NO TRANSPOSE', n2, len, n1,cone, q, ldq, c( &
                              1_ilp64, i ), ldc, cone, work,ldwork )
                    ! multiply top part of c by q12**h.
                    call stdlib_I64_clacpy( 'ALL', n1, len, c( 1_ilp64, i ), ldc,work( n2+1 ), ldwork )
                              
                    call stdlib_I64_ctrmm( 'LEFT', 'LOWER', 'CONJUGATE', 'NON-UNIT',n1, len, cone, q( &
                              1_ilp64, n2+1 ), ldq,work( n2+1 ), ldwork )
                    ! multiply bottom part of c by q22**h.
                    call stdlib_I64_cgemm( 'CONJUGATE', 'NO TRANSPOSE', n1, len, n2,cone, q( n1+1, n2+&
                              1_ilp64 ), ldq, c( n1+1, i ), ldc,cone, work( n2+1 ), ldwork )
                    ! copy everything back.
                    call stdlib_I64_clacpy( 'ALL', m, len, work, ldwork, c( 1_ilp64, i ),ldc )
                 end do
              end if
              ...
              if( notran ) then
                 do i = 1, m, nb
                    ...
                    ldwork = len
                    ! multiply right part of c by q21.
                    call stdlib_I64_clacpy( 'ALL', len, n2, c( i, n1+1 ), ldc, work,ldwork )
                    call stdlib_I64_ctrmm( 'RIGHT', 'UPPER', 'NO TRANSPOSE', 'NON-UNIT',len, n2, cone,&
                               q( n1+1, 1_ilp64 ), ldq, work,ldwork )
                    ! multiply left part of c by q11.
                    call stdlib_I64_cgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', len, n2, n1,cone, c( i, 1_ilp64 )&
                              , ldc, q, ldq, cone, work,ldwork )
                    ! multiply left part of c by q12.
                    call stdlib_I64_clacpy( 'ALL', len, n1, c( i, 1_ilp64 ), ldc,work( 1_ilp64 + n2*ldwork ), &
                              ldwork )
                    call stdlib_I64_ctrmm( 'RIGHT', 'LOWER', 'NO TRANSPOSE', 'NON-UNIT',len, n1, cone,&
                               q( 1_ilp64, n2+1 ), ldq,work( 1_ilp64 + n2*ldwork ), ldwork )
                    ! multiply right part of c by q22.
                    call stdlib_I64_cgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', len, n1, n2,cone, c( i, n1+&
                              1_ilp64 ), ldc, q( n1+1, n2+1 ), ldq,cone, work( 1_ilp64 + n2*ldwork ), ldwork )
                    ! copy everything back.
                    call stdlib_I64_clacpy( 'ALL', len, n, work, ldwork, c( i, 1_ilp64 ),ldc )
                 end do
              else
                 do i = 1, m, nb
                    len = min( nb, m-i+1 )
                    ldwork = len
                    ! multiply right part of c by q12**h.
                    call stdlib_I64_clacpy( 'ALL', len, n1, c( i, n2+1 ), ldc, work,ldwork )
                    call stdlib_I64_ctrmm( 'RIGHT', 'LOWER', 'CONJUGATE', 'NON-UNIT',len, n1, cone, q(&
                               1_ilp64, n2+1 ), ldq, work,ldwork )
                    ! multiply left part of c by q11**h.
                    call stdlib_I64_cgemm( 'NO TRANSPOSE', 'CONJUGATE', len, n1, n2,cone, c( i, 1_ilp64 ), &
                              ldc, q, ldq, cone, work,ldwork )
                    ! multiply left part of c by q21**h.
                    call stdlib_I64_clacpy( 'ALL', len, n2, c( i, 1_ilp64 ), ldc,work( 1_ilp64 + n1*ldwork ), &
                              ldwork )
                    call stdlib_I64_ctrmm( 'RIGHT', 'UPPER', 'CONJUGATE', 'NON-UNIT',len, n2, cone, q(&
                               n1+1, 1_ilp64 ), ldq,work( 1_ilp64 + n1*ldwork ), ldwork )
                    ! multiply right part of c by q22**h.
                    call stdlib_I64_cgemm( 'NO TRANSPOSE', 'CONJUGATE', len, n2, n1,cone, c( i, n2+1 )&
                              , ldc, q( n1+1, n2+1 ), ldq,cone, work( 1_ilp64 + n1*ldwork ), ldwork )
                    ! copy everything back.
                    call stdlib_I64_clacpy( 'ALL', len, n, work, ldwork, c( i, 1_ilp64 ),ldc )
                 end do
              end if
     ...
     pure module subroutine stdlib_I64_zunm22( side, trans, m, n, n1, n2, q, ldq, c, ldc,work, lwork, info )
        ...
        ! =====================================================================
           ...
           if( info/=0_ilp64 ) then
              call stdlib_I64_xerbla( 'ZUNM22', -info )
              return
           else if( lquery ) then
              return
           end if
           ...
           ! degenerate cases (n1 = 0 or n2 = 0) are handled using stdlib_I64_ztrmm.
           if( n1==0_ilp64 ) then
              call stdlib_I64_ztrmm( side, 'UPPER', trans, 'NON-UNIT', m, n, cone,q, ldq, c, ldc )
                        
              work( 1_ilp64 ) = cone
              return
           else if( n2==0_ilp64 ) then
              call stdlib_I64_ztrmm( side, 'LOWER', trans, 'NON-UNIT', m, n, cone,q, ldq, c, ldc )
           ...
           if( left ) then
              if( notran ) then
                 do i = 1, n, nb
                    ...
                    ldwork = m
                    ! multiply bottom part of c by q12.
                    call stdlib_I64_zlacpy( 'ALL', n1, len, c( n2+1, i ), ldc, work,ldwork )
                    call stdlib_I64_ztrmm( 'LEFT', 'LOWER', 'NO TRANSPOSE', 'NON-UNIT',n1, len, cone, &
                              q( 1_ilp64, n2+1 ), ldq, work,ldwork )
                    ! multiply top part of c by q11.
                    call stdlib_I64_zgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', n1, len, n2,cone, q, ldq, &
                              c( 1_ilp64, i ), ldc, cone, work,ldwork )
                    ! multiply top part of c by q21.
                    call stdlib_I64_zlacpy( 'ALL', n2, len, c( 1_ilp64, i ), ldc,work( n1+1 ), ldwork )
                              
                    call stdlib_I64_ztrmm( 'LEFT', 'UPPER', 'NO TRANSPOSE', 'NON-UNIT',n2, len, cone, &
                              q( n1+1, 1_ilp64 ), ldq,work( n1+1 ), ldwork )
                    ! multiply bottom part of c by q22.
                    call stdlib_I64_zgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', n2, len, n1,cone, q( n1+1, &
                              n2+1 ), ldq, c( n2+1, i ), ldc,cone, work( n1+1 ), ldwork )
                    ! copy everything back.
                    call stdlib_I64_zlacpy( 'ALL', m, len, work, ldwork, c( 1_ilp64, i ),ldc )
                 end do
              else
                 do i = 1, n, nb
                    len = min( nb, n-i+1 )
                    ldwork = m
                    ! multiply bottom part of c by q21**h.
                    call stdlib_I64_zlacpy( 'ALL', n2, len, c( n1+1, i ), ldc, work,ldwork )
                    call stdlib_I64_ztrmm( 'LEFT', 'UPPER', 'CONJUGATE', 'NON-UNIT',n2, len, cone, q( &
                              n1+1, 1_ilp64 ), ldq, work,ldwork )
                    ! multiply top part of c by q11**h.
                    call stdlib_I64_zgemm( 'CONJUGATE', 'NO TRANSPOSE', n2, len, n1,cone, q, ldq, c( &
                              1_ilp64, i ), ldc, cone, work,ldwork )
                    ! multiply top part of c by q12**h.
                    call stdlib_I64_zlacpy( 'ALL', n1, len, c( 1_ilp64, i ), ldc,work( n2+1 ), ldwork )
                              
                    call stdlib_I64_ztrmm( 'LEFT', 'LOWER', 'CONJUGATE', 'NON-UNIT',n1, len, cone, q( &
                              1_ilp64, n2+1 ), ldq,work( n2+1 ), ldwork )
                    ! multiply bottom part of c by q22**h.
                    call stdlib_I64_zgemm( 'CONJUGATE', 'NO TRANSPOSE', n1, len, n2,cone, q( n1+1, n2+&
                              1_ilp64 ), ldq, c( n1+1, i ), ldc,cone, work( n2+1 ), ldwork )
                    ! copy everything back.
                    call stdlib_I64_zlacpy( 'ALL', m, len, work, ldwork, c( 1_ilp64, i ),ldc )
                 end do
              ...
              if( notran ) then
                 do i = 1, m, nb
                    ...
                    ldwork = len
                    ! multiply right part of c by q21.
                    call stdlib_I64_zlacpy( 'ALL', len, n2, c( i, n1+1 ), ldc, work,ldwork )
                    call stdlib_I64_ztrmm( 'RIGHT', 'UPPER', 'NO TRANSPOSE', 'NON-UNIT',len, n2, cone,&
                               q( n1+1, 1_ilp64 ), ldq, work,ldwork )
                    ! multiply left part of c by q11.
                    call stdlib_I64_zgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', len, n2, n1,cone, c( i, 1_ilp64 )&
                              , ldc, q, ldq, cone, work,ldwork )
                    ! multiply left part of c by q12.
                    call stdlib_I64_zlacpy( 'ALL', len, n1, c( i, 1_ilp64 ), ldc,work( 1_ilp64 + n2*ldwork ), &
                              ldwork )
                    call stdlib_I64_ztrmm( 'RIGHT', 'LOWER', 'NO TRANSPOSE', 'NON-UNIT',len, n1, cone,&
                               q( 1_ilp64, n2+1 ), ldq,work( 1_ilp64 + n2*ldwork ), ldwork )
                    ! multiply right part of c by q22.
                    call stdlib_I64_zgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', len, n1, n2,cone, c( i, n1+&
                              1_ilp64 ), ldc, q( n1+1, n2+1 ), ldq,cone, work( 1_ilp64 + n2*ldwork ), ldwork )
                    ! copy everything back.
                    call stdlib_I64_zlacpy( 'ALL', len, n, work, ldwork, c( i, 1_ilp64 ),ldc )
                 end do
              else
                 do i = 1, m, nb
                    len = min( nb, m-i+1 )
                    ldwork = len
                    ! multiply right part of c by q12**h.
                    call stdlib_I64_zlacpy( 'ALL', len, n1, c( i, n2+1 ), ldc, work,ldwork )
                    call stdlib_I64_ztrmm( 'RIGHT', 'LOWER', 'CONJUGATE', 'NON-UNIT',len, n1, cone, q(&
                               1_ilp64, n2+1 ), ldq, work,ldwork )
                    ! multiply left part of c by q11**h.
                    call stdlib_I64_zgemm( 'NO TRANSPOSE', 'CONJUGATE', len, n1, n2,cone, c( i, 1_ilp64 ), &
                              ldc, q, ldq, cone, work,ldwork )
                    ! multiply left part of c by q21**h.
                    call stdlib_I64_zlacpy( 'ALL', len, n2, c( i, 1_ilp64 ), ldc,work( 1_ilp64 + n1*ldwork ), &
                              ldwork )
                    call stdlib_I64_ztrmm( 'RIGHT', 'UPPER', 'CONJUGATE', 'NON-UNIT',len, n2, cone, q(&
                               n1+1, 1_ilp64 ), ldq,work( 1_ilp64 + n1*ldwork ), ldwork )
                    ! multiply right part of c by q22**h.
                    call stdlib_I64_zgemm( 'NO TRANSPOSE', 'CONJUGATE', len, n2, n1,cone, c( i, n2+1 )&
                              , ldc, q( n1+1, n2+1 ), ldq,cone, work( 1_ilp64 + n1*ldwork ), ldwork )
                    ! copy everything back.
                    call stdlib_I64_zlacpy( 'ALL', len, n, work, ldwork, c( i, 1_ilp64 ),ldc )
                 end do
